...
static unsigned int is_ppe_support_type(const struct net_device *in, struct sk_buff *skb)
{
	struct ethhdr *eth = NULL;
	struct iphdr *iph = NULL;
	struct ipv6hdr *ip6h = NULL;
...
{
	if (!is_ppe_support_type(state->in, skb)) {
			...
			const struct nf_hook_state *state)
{
	...
	if (!HAS_HQOS_MAGIC_TAG(skb) && !is_ppe_support_type(state->in, skb)) {
		hnat_set_head_frags(state, skb, 1, hnat_set_alg);
		return NF_ACCEPT;
	}
...
void mtk_ppe_dev_register_hook(struct net_device *dev)
{
	int i, number = 0;
	struct extdev_entry *ext_entry;
	char *interface_blackList[] = {"ra2", "rax2", NULL};

	i = 0;
	while (interface_blackList[i] != NULL)
	{
		if (!strcmp(dev->name, interface_blackList[i]))
		{
			pr_info("%s :  %s is in hwnat blackList, not to register\n", __func__, dev->name);
			return;
		}
