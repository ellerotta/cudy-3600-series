...
{
    OVERALL_RATE_LIMITER_DESCRIPTOR_STRUCT *ovl_rl_entry;
    uint32_t alloc_rate, limit_rate, budget_residue;
    rdd_rl_float_t rl_float;

    ovl_rl_entry = (OVERALL_RATE_LIMITER_DESCRIPTOR_STRUCT *)RDD_OVERALL_RATE_LIMITER_TABLE_PTR(get_runner_idx(us_tm_runner_image));
    ...
    /* set overall rl rate */
    alloc_rate = rdd_rate_to_alloc_unit(rate, US_RATE_LIMITER_TIMER_PERIOD_IN_USEC, &budget_residue);
    ...
    limit_rate = rdd_rate_to_alloc_unit(limit, US_RATE_LIMITER_TIMER_PERIOD_IN_USEC, &budget_residue);
...
bdmf_error_t rdd_overall_rate_limiter_bbh_queue_cfg(uint8_t bbh_queue_index, bdmf_boolean is_high_priority)
{
    ...
    BASIC_SCHEDULER_DESCRIPTOR_STRUCT *basic_scheduler_entry;
    COMPLEX_SCHEDULER_DESCRIPTOR_STRUCT *complex_scheduler_entry;
    uint32_t bbh_queue_vec, sched_idx;
    bdmf_boolean sched_type;

    if (bbh_queue_index >= 8*(sizeof(ovl_rl_entry->bbh_queue_en_vec)))
        return BDMF_ERR_PARM;

    ovl_rl_entry = (OVERALL_RATE_LIMITER_DESCRIPTOR_STRUCT *)RDD_OVERALL_RATE_LIMITER_TABLE_PTR(get_runner_idx(us_tm_runner_image));
    bbh_queue_entry = ((BBH_QUEUE_DESCRIPTOR_STRUCT *)RDD_US_TM_BBH_QUEUE_TABLE_PTR(get_runner_idx(us_tm_runner_image))) + bbh_queue_index;
    ...
    {
        basic_scheduler_entry = (BASIC_SCHEDULER_DESCRIPTOR_STRUCT *)RDD_BASIC_SCHEDULER_TABLE_US_PTR(get_runner_idx(us_tm_runner_image)) + sched_idx;
        RDD_BASIC_SCHEDULER_DESCRIPTOR_OVL_RL_EN_WRITE(1, basic_scheduler_entry);
    }
    else
    {
        complex_scheduler_entry = (COMPLEX_SCHEDULER_DESCRIPTOR_STRUCT *)RDD_COMPLEX_SCHEDULER_TABLE_US_PTR(get_runner_idx(us_tm_runner_image)) + sched_idx;
