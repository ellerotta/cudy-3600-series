...
{
    GHOST_REPORTING_GLOBAL_CFG_STRUCT *entry;
...
{
    REPORTING_QUEUE_COUNTER_STRUCT *entry;
    uint16_t counter_ptr;
    BYTES_2_STRUCT *bit_vector_ptr;
    ...
    bit_vector_ptr = (BYTES_2_STRUCT *)RDD_QUEUE_TO_REPORT_BIT_VECTOR_PTR(get_runner_idx(reporting_runner_image));
...
#else
    /* For DSL_XRDP, channel range has been checked. And wan_channel value has been
     * adjusted to 0 based. */
    if (!enable)
#endif
    {
    ...
    entry = ((REPORTING_QUEUE_COUNTER_STRUCT *)RDD_REPORTING_QUEUE_COUNTER_TABLE_PTR(get_runner_idx(reporting_runner_image))) + queue_id;
    counter_ptr = REPORTING_COUNTER_ADDRESS + (wan_channel * sizeof(BYTES_4_STRUCT));
    RDD_REPORTING_QUEUE_COUNTER_COUNTER_PTR_WRITE(counter_ptr, entry);
...
void rdd_ghost_reporting_last_queue_init(uint8_t last_queue)
{
    ...
    RDD_BTRACE("last_queue = %d\n", last_queue);

    entry = RDD_GHOST_REPORTING_GLOBAL_CFG_PTR(get_runner_idx(reporting_runner_image));
    RDD_GHOST_REPORTING_GLOBAL_CFG_ENTRY_LAST_QUEUE_WRITE(last_queue, entry);
}
...
bdmf_error_t rdd_ghost_reporting_ctr_get(uint8_t wan_channel, uint32_t *report)
{
    ...
    RDD_BTRACE("wan_channel = %d, report = %px\n", wan_channel, report);
    ...
    entry = (BYTES_4_STRUCT *)RDD_REPORTING_COUNTER_TABLE_PTR(get_runner_idx(reporting_runner_image)) + wan_channel;
