...
{
    SERVICE_QUEUES_BUDGET_ALLOCATION_TIMER_VALUE_STRUCT *entry;
...
static void rdd_service_queues_scheduler_init(void)
{
    ...
    entry = (COMPLEX_SCHEDULER_DESCRIPTOR_STRUCT *)RDD_SERVICE_QUEUES_COMPLEX_SCHEDULER_TABLE_PTR(get_runner_idx(service_queues_runner_image));
...
bdmf_error_t rdd_service_queue_scheduler_block_cfg(uint16_t queue_index, complex_scheduler_block_t *block)
{
    bdmf_error_t err;
...
bdmf_error_t rdd_service_queue_scheduler_block_remove(uint16_t queue_index)
{
...
{
    bdmf_error_t rc;
    uint32_t sir_alloc, pir_alloc, pir_limit, budget_residue;
    rdd_rl_float_t complex_rl_float;
    uint32_t *complex_rate_limiter_table_ptr;
    int rl_offset = rl_index * 2;

    RDD_BTRACE("rl_index = %d, rl_cfg %px = { sustain_budget = %d, peak_limit = %d, peak_rate = %d, "
    ...
    sir_alloc = rdd_rate_to_alloc_unit(rl_cfg->sustain_budget, DS_RATE_LIMITER_TIMER_PERIOD_IN_USEC, &budget_residue);
    ...
    RDD_COMPLEX_RATE_LIMITER_DESCRIPTOR_ALLOC_RESIDUE_WRITE_G(budget_residue, complex_rate_limiter_table_ptr, rl_index);
    ...
    pir_alloc = rdd_rate_to_alloc_unit(rl_cfg->peak_rate, DS_RATE_LIMITER_TIMER_PERIOD_IN_USEC, &budget_residue);
...
bdmf_error_t rdd_service_queue_rate_limiter_remove(uint8_t rl_index)
{
    ...
    if (rl_index >= RDD_COMPLEX_SCHEDULER_DESCRIPTOR_BLOCK_INDEX_NUMBER)
...
bdmf_error_t rdd_service_queues_basic_rate_limiter_cfg(uint8_t basic_rl_index, rdd_basic_rl_cfg_t *rl_cfg)
{
    ...
    RDD_BTRACE("basic_rl_index = %d, rl_cfg %px = { rate = %d, block type = %d, block_index = %d limit = %d }\n",
    ...
    alloc_rate = rdd_rate_to_alloc_unit(rl_cfg->rate, DS_RATE_LIMITER_TIMER_PERIOD_IN_USEC, &budget_residue);
    ...
    RDD_BASIC_RATE_LIMITER_DESCRIPTOR_ALLOC_RESIDUE_WRITE_G(budget_residue, basic_rate_limiter_table_ptr, rl_offset);
...
{
#if (defined(BCM6878) || defined(BCM6846)) && defined(FLUSH_ON_SERVICE_QUEUES)
    if (drv_qm_get_sq_start() != QM_ILLEGAL_QUEUE) {
        ...
        MWRITE_8(RDD_SERVICE_QUEUES_FLUSH_AGGREGATION_TASK_DISABLE_PTR(get_runner_idx(us_tm_runner_image)), 0);
    } else
        /* disable service queue flush task, when no service queues in the system */
        MWRITE_8(RDD_SERVICE_QUEUES_FLUSH_AGGREGATION_TASK_DISABLE_PTR(get_runner_idx(us_tm_runner_image)), 1);
#else
    rdd_ag_service_queues_first_queue_mapping_set(drv_qm_get_sq_start());
