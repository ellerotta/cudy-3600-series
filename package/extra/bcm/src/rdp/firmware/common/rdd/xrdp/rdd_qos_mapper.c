...
#define QM_QUEUE_TO_BYTE_QUEUE(qm_queue) (qm_queue < drv_qm_get_ds_start(qm_queue) ? qm_queue : (qm_queue - drv_qm_get_ds_start(qm_queue)))
...
static int32_t qos_table_index_allocated[RDD_QOS_MAPPER_ID_MAX_TABLES];
...
static void print_buf_dbg(void)
{
    ...
    {
        if (qos_table_index_allocated[i])
            RDD_BTRACE("reference_count[%d] = %d,", i, qos_table_index_allocated[i]);
    }
    BDMF_TRACE_DBG("\n");
}
...
static void _clean_tcont_ptr_to_rdd_table(uint8_t rdd_index)
{
    ...
    for (i = 0; i < RDD_TX_FLOW_TABLE_SIZE; i++)
    {
        if (qos_mapper_tables_manager[i].table_index == rdd_index)
        {
...
void rdd_qos_mapper_init()
{
    ...
    memset(qos_table_index_allocated, 0, sizeof(qos_table_index_allocated));
    for (i = 0; i < RDPA_MAX_TCONT; i++)
        g_tcont_ptr_to_rdd_table_index_mapping[i] = RDD_QOS_MAPPER_INDEX_UNASSIGNED;
    ...
    GROUP_MWRITE_8(RDD_GLOBAL_PBIT_TO_DSCP_ENABLE_ADDRESS_ARR, 0, 0);
#endif
...
void rdd_qos_mapper_invalidate_table(uint16_t port_or_wan_flow, rdpa_traffic_dir dir, uint8_t size, bdmf_boolean is_pbit)
{
    ...
    BDMF_TRACE_DBG("Invalidating is_pbit %d, tc2q_linked %d, pbit2q_linked %d, do_unlink %d, ref_count %d\n",
        ...
        qos_table_index_allocated[rdd_index]);
    ...
    if (do_unlink)
    {
        ...
        qos_table_index_allocated[rdd_index] = 0;
    ...
    bdmf_boolean is_pbit, uint8_t *rdd_index_out, bdmf_boolean is_tcont, bdmf_number tcont_idx)
{
    ...
    uint8_t tcont_rdd_index;
    bdmf_error_t rc = BDMF_ERR_OK;
    if (is_tcont)
    {
        tcont_rdd_index = g_tcont_ptr_to_rdd_table_index_mapping[tcont_idx];
        if (tcont_rdd_index != rdd_index)
        {
            rdd_index = RDD_QOS_MAPPER_INDEX_UNASSIGNED;
        }
    }
