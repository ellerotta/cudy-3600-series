...
void rdd_bbh_queue_init(void)
{
...
#else
    /* init lan0 */
    entry = ((BBH_QUEUE_DESCRIPTOR_STRUCT *)RDD_DS_TM_BBH_QUEUE_TABLE_PTR(get_runner_idx(ds_tm_runner_image)));
#ifdef BCM6858_B0
    ...
    /* init lan1 */
    entry = ((BBH_QUEUE_DESCRIPTOR_STRUCT *)RDD_DS_TM_BBH_QUEUE_TABLE_PTR(get_runner_idx(ds_tm_runner_image))) + 1;
    RDD_BBH_QUEUE_DESCRIPTOR_BB_DESTINATION_WRITE(BB_ID_TX_BBH_1, entry);

    /* init lan2 */
    entry = ((BBH_QUEUE_DESCRIPTOR_STRUCT *)RDD_DS_TM_BBH_QUEUE_TABLE_PTR(get_runner_idx(ds_tm_runner_image))) + 2;
    RDD_BBH_QUEUE_DESCRIPTOR_BB_DESTINATION_WRITE(BB_ID_TX_BBH_2, entry);

    /* init lan3 */
    entry = ((BBH_QUEUE_DESCRIPTOR_STRUCT *)RDD_DS_TM_BBH_QUEUE_TABLE_PTR(get_runner_idx(ds_tm_runner_image))) + 3;
    RDD_BBH_QUEUE_DESCRIPTOR_BB_DESTINATION_WRITE(BB_ID_TX_BBH_3, entry);

    /* init lan4 */
    entry = ((BBH_QUEUE_DESCRIPTOR_STRUCT *)RDD_DS_TM_BBH_QUEUE_TABLE_PTR(get_runner_idx(ds_tm_runner_image))) + 4;
#ifdef BCM6858_B0
    RDD_BBH_QUEUE_DESCRIPTOR_BB_DESTINATION_WRITE(BB_ID_TX_BBH_0, entry);
#else
    RDD_BBH_QUEUE_DESCRIPTOR_BB_DESTINATION_WRITE(BB_ID_TX_BBH_4, entry);
    ...
    /* init lan5 */
    entry = ((BBH_QUEUE_DESCRIPTOR_STRUCT *)RDD_DS_TM_BBH_QUEUE_TABLE_PTR(get_runner_idx(ds_tm_runner_image))) + 5;
    RDD_BBH_QUEUE_DESCRIPTOR_BB_DESTINATION_WRITE(BB_ID_TX_BBH_5, entry);

    /* init lan6 */
    entry = ((BBH_QUEUE_DESCRIPTOR_STRUCT *)RDD_DS_TM_BBH_QUEUE_TABLE_PTR(get_runner_idx(ds_tm_runner_image))) + 6;
    RDD_BBH_QUEUE_DESCRIPTOR_BB_DESTINATION_WRITE(BB_ID_TX_BBH_6, entry);

#ifdef BCM6858_B0
    /* init lan7 */
    entry = ((BBH_QUEUE_DESCRIPTOR_STRUCT *)RDD_DS_TM_BBH_QUEUE_TABLE_PTR(get_runner_idx(ds_tm_runner_image))) + 7;
...
#ifdef SERVICE_QUEUES_FLUSH_THREAD_NUMBER
    rc = rc ? rc : ag_drv_rnr_regs_cfg_cpu_wakeup_set(get_runner_idx(service_queues_runner_image), SERVICE_QUEUES_FLUSH_THREAD_NUMBER);
#endif

    return rc;
}

bdmf_error_t rdd_scheduling_flush_us_queue_vector_update(uint32_t queue_vector)
{
    bdmf_error_t rc = BDMF_ERR_OK;
    US_TM_FLUSH_QUEUE_VECTOR_TABLE_STRUCT *entry;

    entry = RDD_US_TM_FLUSH_QUEUE_VECTOR_TABLE_PTR(get_runner_idx(us_tm_runner_image));
    RDD_BYTE_1_BITS_WRITE(queue_vector, entry);
    WMB();
...
{
    bdmf_error_t err;
    int bbh_queue = scheduler_cfg->bbh_queue_index;
    ...
    else
    {
        ...
        /* have to declare an int for bbh_queue = scheduler_cfg->bbh_queue_index,
         * or else there is a compilation error with flag -Werror=type-limits when
         * RDD_US_CHANNEL_OFFSET_TCONT == 0 */
        if ((bbh_queue >= RDD_US_CHANNEL_OFFSET_TCONT) &&
            (bbh_queue <= RDD_US_CHANNEL_OFFSET_TCONT_END))
        {
            rdd_ghost_reporting_mapping_queue_to_wan_channel((qm_queue_index - drv_qm_get_us_start()), bbh_queue - RDD_US_CHANNEL_OFFSET_TCONT, enable);
        }
...
static void rdd_tm_debug_cs_get(rdpa_traffic_dir dir, uint8_t complex_scheduler_index, rdd_tm_info *info)
{
    ...
    if (dir == rdpa_dir_ds)
    {
        entry = ((COMPLEX_SCHEDULER_DESCRIPTOR_STRUCT *)RDD_COMPLEX_SCHEDULER_TABLE_US_PTR(get_runner_idx(ds_tm_runner_image))) + complex_scheduler_index;
    ...
    for (i = 0; i < MAX_NUM_OF_QUEUES_IN_SCHED; i++)
    {
        ...
        else
        {
            ...
            if (dir == rdpa_dir_us)
                ...
                cs_entry = ((COMPLEX_SCHEDULER_DESCRIPTOR_STRUCT *)RDD_COMPLEX_SCHEDULER_TABLE_US_PTR(get_runner_idx(ds_tm_runner_image))) + info->queue_info[i].queue_index;
