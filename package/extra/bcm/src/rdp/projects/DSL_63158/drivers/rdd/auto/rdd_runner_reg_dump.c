...
void dump_RDD_BBH_TX_QUEUE_ID_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	...
	RDP_FIELD_MREAD_32((uint8_t *)(p), 8, 24, (r));
...
void dump_RDD_TX_FLOW_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	...
	RDP_FIELD_MREAD_8((uint8_t *)(p), 0, 7, (r));
...
void dump_RDD_VPORT_CFG_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	...
	RDP_FIELD_MREAD_8((uint8_t *)(p), 1, 1, (r)) /*defined by rdd_bridge_and_vlan_ctx_ingress_lookup_method enumeration*/;
	bdmf_session_print(session, "\tbridge_and_vlan_ingress_lookup_method= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	RDP_FIELD_MREAD_8((uint8_t *)(p), 0, 1, (r)) /*defined by rdd_bridge_and_vlan_ctx_egress_lookup_method enumeration*/;
	...
	RDP_FIELD_MREAD_8((uint8_t *)(p) + 1, 2, 2, (r)) /*defined by rdd_action enumeration*/;
	bdmf_session_print(session, "\tsa_lookup_miss_action    = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	RDP_FIELD_MREAD_8((uint8_t *)(p) + 1, 0, 2, (r)) /*defined by rdd_action enumeration*/;
	...
	RDP_FIELD_MREAD_8((uint8_t *)(p) + 3, 4, 3, (r));
	bdmf_session_print(session, "\treserved3                = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
