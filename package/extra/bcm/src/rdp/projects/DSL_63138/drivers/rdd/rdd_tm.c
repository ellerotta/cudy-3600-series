                                                     ...
                                                     rdpa_stat_1way_t                     *stat )
{
    ...
    if (xi_wan_channel_id == 0)
    {
       ...
       rdd_error = rdd_4_bytes_counter_get ( WAN_TX_QUEUES_BYTES_GROUP, tx_queue, &temp32 );
       stat->passed.bytes = temp32;
       if ( rdd_error != BL_LILAC_RDD_OK )
           return ( rdd_error );
       ...
       rdd_error = rdd_4_bytes_counter_get ( WAN_TX_QUEUES_DROPPED_BYTES_GROUP, tx_queue, &temp32 );
       stat->discarded.bytes = temp32;
    }
    else
    {
       ...
       rdd_error = rdd_4_bytes_counter_get ( WAN0_TX_QUEUES_BYTES_GROUP, tx_queue, &temp32 );
       stat->passed.bytes = temp32;
       if ( rdd_error != BL_LILAC_RDD_OK )
           return ( rdd_error );
       ...
       rdd_error = rdd_4_bytes_counter_get ( WAN0_TX_QUEUES_DROPPED_BYTES_GROUP, tx_queue, &temp32 );
       stat->discarded.bytes = temp32;
    }
                                                     ...
                                                     rdpa_stat_1way_t           *stat )
{
    ...
    rdd_error = rdd_4_bytes_counter_get ( LAN_TX_QUEUES_BYTES_GROUP, tx_queue, &temp32 );
    stat->passed.bytes = temp32;
    if ( rdd_error != BL_LILAC_RDD_OK )
        return ( rdd_error );
    ...
    rdd_error = rdd_4_bytes_counter_get ( LAN_TX_QUEUES_DROPPED_BYTES_GROUP, tx_queue, &temp32 );
    stat->discarded.bytes = temp32;
...
static uint32_t rdd_tm_calc_num_us_queues(void)
{
    ...
    for ( idx = 0; idx < RDD_WAN_TX_QUEUES_TABLE_SIZE; idx++ ) {
        ...
        RDD_WAN_TX_QUEUE_DESCRIPTOR_USE_AS_SCHEDULER_READ(use_as_scheduler, wan_tx_queue_descriptor_ptr);
        if (use_as_scheduler)
            num_us_queues += 4;
        else
            num_us_queues += 1;
