...
void dump_RDD_PROCESSING_TX_DESCRIPTOR(bdmf_session_handle session, unsigned char *p)
{
	...
	RDP_FIELD_MREAD_32((uint8_t *)(p) + 4, 14, 7, (r));
	...
	RDP_FIELD_MREAD_8((uint8_t *)(p) + 8, 3, 3, (r));
	...
	RDP_FIELD_MREAD_32((uint8_t *)(p) + 8, 0, 18, (r));
	...
	RDP_FIELD_MREAD_8((uint8_t *)(p) + 12, 5, 1, (r));
	bdmf_session_print(session, "\tabs                      = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	RDP_FIELD_MREAD_16((uint8_t *)(p) + 12, 2, 11, (r));
...
void dump_RDD_PI2_PROBABILITY_CALC_DESCRIPTOR(bdmf_session_handle session, unsigned char *p)
{
	...
	bdmf_session_print(session, "\tus_aqm_q_timer_table     = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_16((uint8_t *)(p) + 52, (r));
	bdmf_session_print(session, "\tds_aqm_q_timer_table     = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_16((uint8_t *)(p) + 54, (r));
	bdmf_session_print(session, "\tsr_aqm_q_timer_table     = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
...
void dump_RDD_AQM_QUEUE_TIMER_DESCRIPTOR(bdmf_session_handle session, unsigned char *p)
{
	...
	MREAD_8((uint8_t *)(p), (r));
	bdmf_session_print(session, "\tPI2_QUEUE_TIMER_DESCRIPTOR_q_delay= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
...
void dump_RDD_CNTR_STASH_DESCRIPTOR(bdmf_session_handle session, unsigned char *p)
{
	unsigned int r;
	bdmf_session_print(session, "  Register CNTR_STASH_DESCRIPTOR\n");

	MREAD_8((uint8_t *)(p), (r));
	bdmf_session_print(session, "\tnum_of_pds               = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
...
void dump_RDD_PROCESSING_RX_DESCRIPTOR(bdmf_session_handle session, unsigned char *p)
{
	...
	RDP_FIELD_MREAD_8((uint8_t *)(p) + 12, 1, 4, (r));
	bdmf_session_print(session, "\terror_type_or_cpu_tx     = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	RDP_FIELD_MREAD_16((uint8_t *)(p) + 12, 2, 7, (r));
	bdmf_session_print(session, "\tbuf_sop                  = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	RDP_FIELD_MREAD_32((uint8_t *)(p) + 12, 0, 18, (r));
	bdmf_session_print(session, "\tbuffer_number_0          = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
...
void dump_RDD_PROCESSING_GROUP_PARAMS_GEN4_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	unsigned int r;
	bdmf_session_print(session, "  Register PROCESSING_GROUP_PARAMS_GEN4_ENTRY\n");
	...
	MREAD_16((uint8_t *)(p) + 2, (r));
	bdmf_session_print(session, "\tclients_size_in_word     = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_16((uint8_t *)(p) + 4, (r));
	bdmf_session_print(session, "\texception_drop_cntr_id   = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_16((uint8_t *)(p) + 6, (r));
	bdmf_session_print(session, "\tflow_miss_drop_cntr_id   = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)(p) + 8, (r));
	bdmf_session_print(session, "\treg_bck_up_0             = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)(p) + 12, (r));
	bdmf_session_print(session, "\treg_bck_up_1             = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
...
void dump_RDD_TUNNEL_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	...
	RDP_FIELD_MREAD_8((uint8_t *)(p) + 6, 0, 7, (r));
	bdmf_session_print(session, "\treserved0                = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
	...
	bdmf_session_print(session, "\tlayer4_offset            = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_8((uint8_t *)(p) + 14, (r));
	bdmf_session_print(session, "\tl2tp_header_size         = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_8((uint8_t *)(p) + 15, (r));
	bdmf_session_print(session, "\tppp_header_size          = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
...
void dump_RDD_VPORT_CFG_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	...
	RDP_FIELD_MREAD_8((uint8_t *)(p) + 1, 5, 3, (r)) /*defined by rdd_action enumeration*/;
	bdmf_session_print(session, "\tsa_lookup_miss_action    = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	RDP_FIELD_MREAD_8((uint8_t *)(p) + 1, 3, 2, (r)) /*defined by rdd_action enumeration*/;
	bdmf_session_print(session, "\tda_lookup_miss_action    = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	RDP_FIELD_MREAD_8((uint8_t *)(p) + 1, 2, 1, (r));
	bdmf_session_print(session, "\ttunnel_termination       = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
	...
	RDP_FIELD_MREAD_8((uint8_t *)(p) + 3, 7, 1, (r)) /*defined by rdd_bridge_and_vlan_ctx_ingress_lookup_method enumeration*/;
	bdmf_session_print(session, "\tbridge_and_vlan_ingress_lookup_method= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	RDP_FIELD_MREAD_8((uint8_t *)(p) + 3, 6, 1, (r)) /*defined by rdd_bridge_and_vlan_ctx_egress_lookup_method enumeration*/;
...
void dump_RDD_POLICER_PARAMS_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	...
	RDP_FIELD_MREAD_8((uint8_t *)(p), 5, 1, (r));
	bdmf_session_print(session, "\tpbit_mode                = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	RDP_FIELD_MREAD_8((uint8_t *)(p), 2, 3, (r));
	bdmf_session_print(session, "\tpbit_green_val           = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	RDP_FIELD_MREAD_16((uint8_t *)(p), 7, 3, (r));
	bdmf_session_print(session, "\tpbit_yellow_val          = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
...
void dump_RDD_SYSTEM_CONFIGURATION_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	...
	bdmf_session_print(session, "\tg9991_skip_parser_error_or_generic_class_extra_lkp_enable= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
...
void dump_RDD_VPORT_CFG_EX_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	...
	RDP_FIELD_MREAD_8((uint8_t *)(p) + 1, 2, 3, (r));
	...
	RDP_FIELD_MREAD_8((uint8_t *)(p) + 2, 1, 1, (r));
...
void dump_RDD_NAT_CACHE_CFG(bdmf_session_handle session, unsigned char *p)
{
	...
	RDP_FIELD_MREAD_8((uint8_t *)(p) + 6, 5, 1, (r));
	bdmf_session_print(session, "\tesp_filter_action        = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	RDP_FIELD_MREAD_8((uint8_t *)(p) + 6, 4, 1, (r));
...
void dump_RDD_RING_CPU_TX_DESCRIPTOR(bdmf_session_handle session, unsigned char *p)
{
	...
	MREAD_8((uint8_t *)(p) + 10, (r));
	bdmf_session_print(session, "\tingress_or_egress_0      = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
...
void dump_RDD_TCAM_IC_CFG(bdmf_session_handle session, unsigned char *p)
{
	...
	RDP_FIELD_MREAD_8((uint8_t *)(p) + 8, 5, 3, (r));
	bdmf_session_print(session, "\ttable_id                 = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	RDP_FIELD_MREAD_16((uint8_t *)(p) + 8, 0, 13, (r));
	bdmf_session_print(session, "\tkey_size                 = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	RDP_FIELD_MREAD_16((uint8_t *)(p) + 10, 1, 15, (r));
	bdmf_session_print(session, "\treserved                 = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
...
void dump_RDD_SERVICE_QUEUES_CONTEXT_DATA(bdmf_session_handle session, unsigned char *p)
{
	unsigned int r;
	bdmf_session_print(session, "  Register SERVICE_QUEUES_CONTEXT_DATA\n");
