...
int rdd_ag_timer_common_fw_policer_budget_entry_get_core(uint32_t _entry, rdd_fw_policer_budget_entry_t *fw_policer_budget_entry, int core_id)
{
    if(!fw_policer_budget_entry || _entry >= RDD_FW_POLICER_BUDGET_SIZE)
         return BDMF_ERR_PARM;

    RDD_FW_POLICER_BUDGET_ENTRY_BUDGET_MANTISSA_READ_CORE(fw_policer_budget_entry->budget_mantissa, RDD_FW_POLICER_BUDGET_ADDRESS_ARR, _entry, core_id);
    RDD_FW_POLICER_BUDGET_ENTRY_BUDGET_EXPONENT_READ_CORE(fw_policer_budget_entry->budget_exponent, RDD_FW_POLICER_BUDGET_ADDRESS_ARR, _entry, core_id);

    return BDMF_ERR_OK;
}

int rdd_ag_timer_common_fw_policer_budget_entry_set_core(uint32_t _entry, rdd_fw_policer_budget_entry_t *fw_policer_budget_entry, int core_id)
{
    if(!fw_policer_budget_entry || _entry >= RDD_FW_POLICER_BUDGET_SIZE || fw_policer_budget_entry->budget_mantissa >= 16384 || fw_policer_budget_entry->budget_exponent >= 4)
          return BDMF_ERR_PARM;

    RDD_FW_POLICER_BUDGET_ENTRY_BUDGET_MANTISSA_WRITE_CORE(fw_policer_budget_entry->budget_mantissa, RDD_FW_POLICER_BUDGET_ADDRESS_ARR, _entry, core_id);
    RDD_FW_POLICER_BUDGET_ENTRY_BUDGET_EXPONENT_WRITE_CORE(fw_policer_budget_entry->budget_exponent, RDD_FW_POLICER_BUDGET_ADDRESS_ARR, _entry, core_id);
...
int rdd_ag_timer_common_cbs_entry_get_core(uint32_t _entry, rdd_cbs_entry_t *cbs_entry, int core_id)
{
    if(!cbs_entry || _entry >= RDD_FW_POLICER_CBS_SIZE)
         return BDMF_ERR_PARM;

    RDD_CBS_ENTRY_COMMITTED_BURST_SIZE_READ_CORE(cbs_entry->committed_burst_size, RDD_FW_POLICER_CBS_ADDRESS_ARR, _entry, core_id);

    return BDMF_ERR_OK;
}

int rdd_ag_timer_common_cbs_entry_set_core(uint32_t _entry, rdd_cbs_entry_t *cbs_entry, int core_id)
{
    if(!cbs_entry || _entry >= RDD_FW_POLICER_CBS_SIZE)
         return BDMF_ERR_PARM;

    RDD_CBS_ENTRY_COMMITTED_BURST_SIZE_WRITE_CORE(cbs_entry->committed_burst_size, RDD_FW_POLICER_CBS_ADDRESS_ARR, _entry, core_id);
...
int rdd_ag_timer_common_emac_flow_ctrl_budget_budget_set_core(uint32_t _entry, uint32_t budget, int core_id)
{
    if(_entry >= RDD_EMAC_FLOW_CTRL_BUDGET_SIZE)
         return BDMF_ERR_PARM;

    RDD_EMAC_FLOW_CTRL_BUDGET_ENTRY_BUDGET_WRITE_CORE(budget, RDD_EMAC_FLOW_CTRL_BUDGET_ADDRESS_ARR, _entry, core_id);
...
int rdd_ag_timer_common_emac_flow_ctrl_budget_budget_get_core(uint32_t _entry, uint32_t *budget, int core_id)
{
    if(_entry >= RDD_EMAC_FLOW_CTRL_BUDGET_SIZE)
         return BDMF_ERR_PARM;

    RDD_EMAC_FLOW_CTRL_BUDGET_ENTRY_BUDGET_READ_CORE(*budget, RDD_EMAC_FLOW_CTRL_BUDGET_ADDRESS_ARR, _entry, core_id);
...
int rdd_ag_timer_common_emac_flow_ctrl_budget_remainder_set_core(uint32_t _entry, uint8_t bits, int core_id)
{
    if(_entry >= RDD_EMAC_FLOW_CTRL_BUDGET_REMAINDER_SIZE)
          return BDMF_ERR_PARM;

    RDD_BYTE_1_BITS_WRITE_CORE(bits, RDD_EMAC_FLOW_CTRL_BUDGET_REMAINDER_ADDRESS_ARR, _entry, core_id);
...
int rdd_ag_timer_common_emac_flow_ctrl_budget_remainder_get_core(uint32_t _entry, uint8_t *bits, int core_id)
{
    if(_entry >= RDD_EMAC_FLOW_CTRL_BUDGET_REMAINDER_SIZE)
          return BDMF_ERR_PARM;

    RDD_BYTE_1_BITS_READ_CORE(*bits, RDD_EMAC_FLOW_CTRL_BUDGET_REMAINDER_ADDRESS_ARR, _entry, core_id);
...
int rdd_ag_timer_common_emac_flow_ctrl_vector_set_core(uint8_t bits, int core_id)
{
    RDD_BYTE_1_BITS_WRITE_CORE(bits, RDD_EMAC_FLOW_CTRL_VECTOR_ADDRESS_ARR, 0, core_id);
...
int rdd_ag_timer_common_emac_flow_ctrl_vector_get_core(uint8_t *bits, int core_id)
{
    RDD_BYTE_1_BITS_READ_CORE(*bits, RDD_EMAC_FLOW_CTRL_VECTOR_ADDRESS_ARR, 0, core_id);
...
int rdd_ag_timer_common_fw_policer_budget_budget_mantissa_set_core(uint32_t _entry, uint16_t budget_mantissa, int core_id)
{
    if(_entry >= RDD_FW_POLICER_BUDGET_SIZE || budget_mantissa >= 16384)
          return BDMF_ERR_PARM;

    RDD_FW_POLICER_BUDGET_ENTRY_BUDGET_MANTISSA_WRITE_CORE(budget_mantissa, RDD_FW_POLICER_BUDGET_ADDRESS_ARR, _entry, core_id);
...
int rdd_ag_timer_common_fw_policer_budget_budget_mantissa_get_core(uint32_t _entry, uint16_t *budget_mantissa, int core_id)
{
    if(_entry >= RDD_FW_POLICER_BUDGET_SIZE)
         return BDMF_ERR_PARM;

    RDD_FW_POLICER_BUDGET_ENTRY_BUDGET_MANTISSA_READ_CORE(*budget_mantissa, RDD_FW_POLICER_BUDGET_ADDRESS_ARR, _entry, core_id);
...
int rdd_ag_timer_common_fw_policer_budget_budget_exponent_set_core(uint32_t _entry, uint8_t budget_exponent, int core_id)
{
    if(_entry >= RDD_FW_POLICER_BUDGET_SIZE || budget_exponent >= 4)
          return BDMF_ERR_PARM;

    RDD_FW_POLICER_BUDGET_ENTRY_BUDGET_EXPONENT_WRITE_CORE(budget_exponent, RDD_FW_POLICER_BUDGET_ADDRESS_ARR, _entry, core_id);
...
int rdd_ag_timer_common_fw_policer_budget_budget_exponent_get_core(uint32_t _entry, uint8_t *budget_exponent, int core_id)
{
    if(_entry >= RDD_FW_POLICER_BUDGET_SIZE)
         return BDMF_ERR_PARM;

    RDD_FW_POLICER_BUDGET_ENTRY_BUDGET_EXPONENT_READ_CORE(*budget_exponent, RDD_FW_POLICER_BUDGET_ADDRESS_ARR, _entry, core_id);
...
int rdd_ag_timer_common_fw_policer_budget_remainder_set_core(uint32_t _entry, uint8_t bits, int core_id)
{
    if(_entry >= RDD_FW_POLICER_BUDGET_REMAINDER_SIZE)
          return BDMF_ERR_PARM;

    RDD_BYTE_1_BITS_WRITE_CORE(bits, RDD_FW_POLICER_BUDGET_REMAINDER_ADDRESS_ARR, _entry, core_id);
...
int rdd_ag_timer_common_fw_policer_budget_remainder_get_core(uint32_t _entry, uint8_t *bits, int core_id)
{
    if(_entry >= RDD_FW_POLICER_BUDGET_REMAINDER_SIZE)
          return BDMF_ERR_PARM;

    RDD_BYTE_1_BITS_READ_CORE(*bits, RDD_FW_POLICER_BUDGET_REMAINDER_ADDRESS_ARR, _entry, core_id);
...
int rdd_ag_timer_common_fw_policer_cbs_committed_burst_size_set_core(uint32_t _entry, uint32_t committed_burst_size, int core_id)
{
    if(_entry >= RDD_FW_POLICER_CBS_SIZE)
         return BDMF_ERR_PARM;

    RDD_CBS_ENTRY_COMMITTED_BURST_SIZE_WRITE_CORE(committed_burst_size, RDD_FW_POLICER_CBS_ADDRESS_ARR, _entry, core_id);
...
int rdd_ag_timer_common_fw_policer_cbs_committed_burst_size_get_core(uint32_t _entry, uint32_t *committed_burst_size, int core_id)
{
    if(_entry >= RDD_FW_POLICER_CBS_SIZE)
         return BDMF_ERR_PARM;

    RDD_CBS_ENTRY_COMMITTED_BURST_SIZE_READ_CORE(*committed_burst_size, RDD_FW_POLICER_CBS_ADDRESS_ARR, _entry, core_id);
...
int rdd_ag_timer_common_fw_policer_vector_set_core(uint32_t _entry, uint32_t bits, int core_id)
{
    if(_entry >= RDD_FW_POLICER_VECTOR_SIZE)
          return BDMF_ERR_PARM;

    RDD_BYTES_4_BITS_WRITE_CORE(bits, RDD_FW_POLICER_VECTOR_ADDRESS_ARR, _entry, core_id);
...
int rdd_ag_timer_common_fw_policer_vector_get_core(uint32_t _entry, uint32_t *bits, int core_id)
{
    if(_entry >= RDD_FW_POLICER_VECTOR_SIZE)
          return BDMF_ERR_PARM;

    RDD_BYTES_4_BITS_READ_CORE(*bits, RDD_FW_POLICER_VECTOR_ADDRESS_ARR, _entry, core_id);
