...
int rdd_ag_cpu_rx_vport_to_flow_idx_set_core(uint32_t _entry, uint8_t bits, int core_id)
{
    if(_entry >= RDD_VPORT_TO_FLOW_IDX_SIZE)
          return BDMF_ERR_PARM;

    RDD_BYTE_1_BITS_WRITE_CORE(bits, RDD_VPORT_TO_FLOW_IDX_ADDRESS_ARR, _entry, core_id);
...
int rdd_ag_cpu_rx_vport_to_flow_idx_get_core(uint32_t _entry, uint8_t *bits, int core_id)
{
    if(_entry >= RDD_VPORT_TO_FLOW_IDX_SIZE)
          return BDMF_ERR_PARM;

    RDD_BYTE_1_BITS_READ_CORE(*bits, RDD_VPORT_TO_FLOW_IDX_ADDRESS_ARR, _entry, core_id);
