...
void dump_RDD_SYSTEM_CONFIGURATION_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	...
	bdmf_session_print(session, "\tgeneric_class_extra_lkp_enable= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
	...
	RDP_FIELD_MREAD_32((uint8_t *)(p) + 4, 0, 21, (r));
	bdmf_session_print(session, "\treserved1                = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
...
void dump_RDD_PROCESSING_TX_DESCRIPTOR(bdmf_session_handle session, unsigned char *p)
{
	...
	bdmf_session_print(session, "\tct_lkp_status            = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
...
{
	unsigned int r;
	bdmf_session_print(session, "  Register RNR_COUNTER_REPLY_ENTRY\n");

	MREAD_32((uint8_t *)(p), (r));
	bdmf_session_print(session, "\tcounter_reply            = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
	...
	MREAD_32((uint8_t *)(p) + 16, (r));
	bdmf_session_print(session, "\tcounter_rt_val           = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)(p) + 20, (r));
	bdmf_session_print(session, "\trnr_sync_cnt_val         = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)(p) + 24, (r));
	bdmf_session_print(session, "\trecv_total_pkt           = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)(p) + 28, (r));
	bdmf_session_print(session, "\tsent_total_pkt           = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
