...
BUILD_UBOOT_DIR=$(BUILD_DIR)/bcm-bootloader_1.0
...
KERNEL_DIR := $(LINUX_DIR)
HOSTTOOLS_DIR := $(BCM_TARGET_PATH)/bcm-bootloader/hostTools
...
$(KERNEL_DIR)/.pre_kernelbuild: $(BCM_KF_KCONFIG_FILE)
	@echo
	@echo -------------------------------------------
	@echo ... starting kernel build at $(LINUX_DIR)
	@echo PROFILE_KERNEL_VER is $(PROFILE_KERNEL_VER)
	@echo BCM_KF is $(if $(BCM_KF),,un)defined
	@cd $(INC_KERNEL_BASE); \
#	if [ -e $(LINUX_DIR)/.git_repo ]; then \
#		cd $(LINUX_DIR)/ && chmod +x ./get_kernel && ./get_kernel && cd -; \
#	elif [ ! -e $(LINUX_DIR)/.untar_complete ]; then \
#		echo "Untarring original Linux kernel source: $(LINUX_ZIP_FILE)"; \
#		(tar $(COMPRESS_OPTION) -xkpf $(LINUX_ZIP_FILE) 2> /dev/null || true); \
#		touch $(LINUX_DIR)/.untar_complete; \
#	fi && \
	cat $(HOSTTOOLS_DIR)/scripts/defconfig-bcm.template > $(LINUX_DIR)/arch/$(KARCH)/defconfig && \
	$(GENDEFCONFIG_CMD) $(PROFILE_PATH) ${MAKEFLAGS} && \
	cp -f $(LINUX_DIR)/arch/$(KARCH)/defconfig $(LINUX_DIR)/.config && \
	$(if $(strip $(BRCM_ANDROID)), $(call android_kernel_merge_cfg), true) && \
	$(MAKE) -C $(BCM_TARGET_PATH)/build -f Bcmkernel.mk olddefconfig && \
	$(if $(BCM_KF), true, $(call kernel_cfg_rm_bcm_kf)) && \
	touch $(LINUX_DIR)/.pre_kernelbuild;
