...
int main(int argc, char *argv[])
{
   ...
   if (argc != 2)
   {
      fprintf(stderr, "CRC32 generator, usage: %s [filename of image to CRC]\n", argv[0]);
      return 0;
   }
   ...
   {
      printf("ERROR!!! Could not read image from file %s\n", argv[1]);
      free(buffer);
      fclose(fp);
      return -1;
   }

   printf("0x%08X\n", getCrc32(buffer, size, CRC32_INIT_VALUE));

   free(buffer);
   fclose(fp);

   return 0; // return the amount we copied
}
