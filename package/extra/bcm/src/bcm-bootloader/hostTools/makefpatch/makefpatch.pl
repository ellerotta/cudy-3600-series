...
my $usage = q{
Usage: makefpatch [--help] [--dbg] [--git] [-FL featureListFile] [-P patchDir] [-T tempDir] [-ignore filter] [[-vm] [-vk] -verify tarfile] dir

options:
  --dbg    turn on verbose debugging
  --git    kernel directory is a git repo
...
sub getFilesWithStrings {
    ...
    {
       @$fileList =
          split( /\n/, `git --git-dir=$dir/.git grep -l \"$stringList\" HEAD |sed -e 's/^HEAD://g' -e 's%^%$dir/%' ` );
    }
}
...
sub testAgainstVanilla {
    ...
    if($git == 0)
    {
        $modFilesStr = `cd $rootDir; find . -type f;`;
    }
    else
    {
        $modFilesStr = `cd $rootDir; git diff --name-only;`;
    }
    ...
    open my $ffh, '>', "modfiles.txt"; 
    print $ffh $modFilesStr;
    close $ffh;
    ...
    print "cd $tarFileDir; tar -xf $tarFile -T ./modfiles.txt\n" if ($dbg);
    `cd $tarFileDir; tar -xf $lTarFile -T ./modfiles.txt 2>/dev/null`;
    ...
    foreach $vanillaFile (@vanillaFiles) {
        ...
        if (     $strippedFile =~ m/\.[chS]$/ 
              ...
              || ($verifyKconfigs && $strippedFile =~ m/Kconfig.*/) ) {
            print
    ...
    if ($verifyNew) {    
        ...
        foreach $newFile (@newFiles) {
            if (     $newFile =~ m/\.[chS]$/ 
                  ...
                  || ($verifyKconfigs && $newFile =~ m/Kconfig.*/) ) {
