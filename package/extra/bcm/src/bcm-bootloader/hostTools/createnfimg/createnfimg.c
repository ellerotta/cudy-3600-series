...
int  p_ecc_bflip_correct=0;       /* No ECC bitflip correct by default */
int  p_swap=0;                   /* No bytes swap by default */
...
void usage()
{
    ...
    printf("    -s                 -- Swap bytes in data words before ECC computation, no swap by default\n");
    printf("    -f                 -- ECC bitflip correct, disabled by default\n");
    printf("\n");
    exit(1);
}
...
int main(int argc, char **argv)
{
    ...
    uint8_t   erase_ecc_code[70]; 
    int       init_erase_ecc_code = 1;
    ...
    /* Parse options */
    opterr = 0;
    while (-1 != (optchar = getopt(argc, argv, "b:B:c:C:i:I:j:J:l:L:m:M:n:N:p:P:r:R:sSt:T:d:D:fF"))) {
        switch (optchar) {
        case 'b':
        case 'B':
    ...
    */
    ...
    while (1)
    {
        ...
        if (init_erase_ecc_code && p_ecc_bflip_correct)
        {
            nand_calculate_ecc_BRCM_bch(data, erase_ecc_code, m, n, k, t, u, v);
            init_erase_ecc_code = 0;
        }
        ...
        /********************************************************************************/
                ...
                for (subpage = 0; subpage < partial_pages_per_page; subpage++)
                {
                    ...
                    if (t == 1)
                        ...
                        for(i = 0; i < v; i++)
                        {
                            if (p_ecc_bflip_correct)
                                ecc_code[i] = ~(erase_ecc_code[i] ^ ecc_code[i]);
