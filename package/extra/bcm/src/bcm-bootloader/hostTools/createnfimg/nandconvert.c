...
int  p_pages = 64; // pages per block
...
void usage()
{
   ...
   printf("  %s <options>\n", pgm_name);
   printf("\nDESCRIPTION:\n");
   printf("  %s takes a 4K page serial NAND input .w file and converts it to be compatible with\n", pgm_name);
   printf("  a boot rom that addresses serial NAND using 2K bytes/page.\n");
   printf("  If needed be sure to do this before using createnfimg.\n");
   printf("\nOPTIONS:\n");
   printf("    -i <input file name>\n");
   printf("    -o <output file name>\n");
   printf("    -b <bootrom page size> -- page size boot rom expects, default = %d.\n", p_bootrom);
   printf("    -n <target page size> -- serial NAND page size, default = %d (this is also the build file page size).\n", p_nand);
   printf("    -l <little endian> -- endianess of CRC, 0 for MIPS (63381/6838), 1 for ARM (63138/6848/6858 and newer chips.) default = %d.\n", p_littlendian);
   printf("    -p <pages per block> -- default = %d.\n", p_pages);
   printf("\n");
   exit(1);
}
...
int main(int argc, char **argv)
{
   ...
   int nandblocksize = p_nand * p_pages;
   int romblocksize = p_bootrom * p_pages;
   unsigned char nandbuf[nandblocksize];
   unsigned char rombuf[romblocksize];
   int boot_block = 1;
   int done = 0;
   int page;
   int position = 0;
   ...
   while (!done)
   {
      ...
      { // write CFERAM, Linux blocks normally
         printf("writing rest of file, reading position 0x%x\n", position);
         fseek(infile, position, SEEK_SET);
         bytes_read = fread(nandbuf, 1, nandblocksize, infile);
         if (bytes_read != nandblocksize)
         { // we are at the end of the file
            if (bytes_read) // write trailer
            { // might need to fix for endianess
               if (p_littlendian)
               {
                  nandbuf[3] = crc >> 24 & 0xFF;
                  nandbuf[2] = crc >> 16 & 0xFF;
                  nandbuf[1] = crc >> 8  & 0xFF;
                  nandbuf[0] = crc       & 0xFF;
               }
               else
               {
                  nandbuf[0] = crc >> 24 & 0xFF;
                  nandbuf[1] = crc >> 16 & 0xFF;
                  nandbuf[2] = crc >> 8  & 0xFF;
                  nandbuf[3] = crc       & 0xFF;
               }
               printf("crc 0x%8x\n", crc);
            }
         ...
         {
            fwrite(nandbuf, bytes_read, 1, outfile);
            crc = getCrc32(nandbuf, bytes_read, crc);
            position += bytes_read;
         }
      }
      else
      { // write modified CFEROM
         fseek(infile, position, SEEK_SET);
         bytes_read = fread(rombuf, 1, 4, infile);

         if ( ((rombuf[0] == 0x19) && (rombuf[1] == 0x85)) || ((rombuf[1] == 0x19) && (rombuf[0] == 0x85)) || ((rombuf[0] == 'U') && (rombuf[1] == 'B') && (rombuf[2] == 'I')) || ((rombuf[0] == 'v') && (rombuf[1] == 'n') && (rombuf[2] == 'E') && (rombuf[3] == 'u')) )
         { // we've reached the end of CFEROM
            boot_block = 0;
            printf("tag discovered, end of first level bootloader found at input file position 0x%x\n", position);
            continue;
         }

         printf("reading first level bootloader input file position 0x%x\n", position);
         fseek(infile, position, SEEK_SET);
         bytes_read = fread(rombuf, 1, romblocksize, infile);
         if (bytes_read == romblocksize)
         { // read in one bootrom's blocksize worth of data
            //for (i = 0; i < bytes_read; i++)
            {
               //if (rombuf[i] != 0xff)
               { // there is data in this block, we need to copy it
                  for (page = 0; page < p_pages; page++)
                  { // pad out the pages
                     memcpy(nandbuf + (page * p_nand), rombuf + (page * p_bootrom), p_bootrom);
                     memset(nandbuf + (page * p_nand) + p_bootrom, 0xFF, p_nand - p_bootrom);
                  }

                  fwrite(nandbuf, nandblocksize, 1, outfile);
                  crc = getCrc32(nandbuf, bytes_read, crc);
                  position += bytes_read;
                  //break;
               }
            ...
            //printf("empty block found at input file position 0x%x, assuming end of bootrom blocks\n", position);
            //boot_block = 0; // block is empty, figure that we're done
         }
         else
         {
            printf("ERROR!!! End of file reached but still reading boot rom blocks\n");
            break; // something screwed up, we got to the end of the file but didn't get past reading bootrom blocks
         }
      }
   }
