	...
	do {
		if (tstc()) {
			...
			if (slow_equals(sha, sha_env, SHA256_SUM_LEN))
				abort = 1;
			/* add 1 second per character typed */
			etime += (uint64_t) (1) * get_tbclk();
		}
...
static int __abortboot(int bootdelay)
{
...
#endif
	{
		...
		/* just key abort if RELAXED and NONSECURE */
		unsigned long ts;
		abort = 0;
...
#endif
#if defined(CONFIG_AUTOBOOT_KEYED)
		printf("bootcommand failed .. last chance\n");
#if defined(CONFIG_BCMBCA_RELAX_CHECKS_WHEN_NONSECURE)
		if ((sec_state != SEC_STATE_UNSEC) && !abortboot(20)) {
#else
		if (!abortboot(20)) {
#endif
