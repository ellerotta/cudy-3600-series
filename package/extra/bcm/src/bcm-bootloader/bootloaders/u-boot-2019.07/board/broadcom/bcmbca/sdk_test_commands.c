...
int flash_rootfs_nand( ulong addr, ulong size, int img_index );
int flash_bootfs_nand( ulong addr, ulong size, int img_index );
...
static int do_flash_bootfs_rootfs( char * bootfs_filename, char * rootfs_filename,  char * uboot_filename, int image_index );
#endif
...
int nand_restorerootfs(void)
{
	char cmd[128];
	int ret = -1;
	int rootfs_dataId = 15;
	int size = 0x1400000;
	if(run_command("ubi part image", 0) == 0) {

		/* If data exists, delete it */
		if(run_command("ubi check rootfs_data", 0) == 0 ) {
			ret = run_command("ubi remove rootfs_data", 0);

			/* Create rootfs volume */
			sprintf(cmd, "ubi create rootfs_data %x  dynamic %d", size,
									rootfs_dataId);
			ret = run_command(cmd, 0);
...
static int flash_uboot_nand( ulong addr, ulong size, int img_index )
{
	char cmd[128];
	int ret = CMD_RET_FAILURE;
	int ubootfs_volid = (img_index == 1? UBOOT_VOL_ID_1:UBOOT_VOL_ID_2);
	int active_img_idx;
	uint64_t avail_bytes = 0;
	uint64_t extra_space_bytes = nand_get_part_extra_bytes("uboot");
	...
	if(ret == 0) {
		/* check if new uboot will fit in flash */
		sprintf(cmd, "uboot%d", img_index);
		avail_bytes =  nand_get_avail_space(cmd);
		if( avail_bytes < size+extra_space_bytes ) {
			printf("Error: Insufficient space in NAND for 0x%llx Bytes . Available size is 0x%llx\n", size+extra_space_bytes, avail_bytes);
			return -1;
		...
		/* If uboot exists, delete it */
		sprintf(cmd, "ubi check uboot%d", img_index);
		if(run_command(cmd, 0) == 0) {
			sprintf(cmd, "ubi remove uboot%d", img_index);
			run_command(cmd, 0);
		}

		/* Create uboot volume */
		sprintf(cmd, "ubi create uboot%d %llx  static %d", img_index, size+extra_space_bytes,
								ubootfs_volid);
		ret = run_command(cmd, 0);

		/* Write img data */
		if( ret == 0 ) {
			sprintf(cmd, "ubi write %lx uboot%d %lx\n", addr, img_index, size);
			ret = run_command(cmd, 0);
		}
...
static int flash_ubootfs( ulong addr, ulong size , int img_index)
{
	int ret = -1;
	...
	if( strcasecmp_image_media(FLASH_DEV_STR_NAND) == 0 ) {
#ifdef BCA_SDK_NAND_CMD
		ret = flash_uboot_nand(addr, size, img_index);
#endif
	}

	if( ret && ( strcasecmp_image_media(FLASH_DEV_STR_SPINOR) == 0 ) ) {
#ifdef BCA_SDK_SPINOR_CMD
		ret = write_spinor_partition(UBOOT_PART, addr, size);
#endif
	}
#endif
...
static int do_flash_bootfs_rootfs( char * bootfs_filename, char * rootfs_filename, char * uboot_filename, int img_index )
{
	char cmd[128];
	ulong size;
	ulong addr;
	...
	/* Write uboot */
	if(uboot_filename) {
		sprintf(cmd, "tftp %lx %s\n", load_addr, uboot_filename);

		if( run_command(cmd, 0) == 0 ) {
			size = env_get_hex("filesize", 0);
			addr = env_get_hex("fileaddr", 0);
			ret = flash_ubootfs(addr, size, img_index);
		} else
			printf("ERROR!!: Failed to tftp rootfs binary!!\n");
	}
			...
			char *const argv[])
{
	...
	if( (strcmp(argv[0], "flash_uboot_raw") == 0) ) {
		uboot_fname = argv[1];
	}
	...
	ret = do_flash_bootfs_rootfs(bootfs_fname, rootfs_fname, uboot_fname, img_index);
...
U_BOOT_CMD_WITH_SUBCMDS(bca_test, "Broadcom test commands", usage,
			...
			U_BOOT_SUBCMD_MKENT(flash_uboot_raw, 3, 0, do_flash_bins),
