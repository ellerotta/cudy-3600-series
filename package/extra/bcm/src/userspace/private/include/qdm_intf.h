...
/** Given a fullpath, return TRUE if the object in fullpath is enabled.
 *
 *  This function is only used in TR181 code, so only _dev2 version exists.
 *
 * @param fullPath  (IN) fullpath
 *
 * @return TRUE if the object in fullpath is enabled.
 */
UBOOL8 qdmIntf_isEnabledOnFullPathLocked_dev2(const char *fullPath);


/** Given a pathDescriptor, return TRUE if the object in pathDescriptor is enabled.
 *
 *  This function is only used in TR181 code, so only _dev2 version exists.
 *
 * @param pathDescIn  (IN) pathDescriptor
 *
 * @return TRUE if the object in pathDescriptor is enabled.
 */
