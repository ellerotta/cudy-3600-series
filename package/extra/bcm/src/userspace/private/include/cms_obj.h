...
/** Used in cmsPhl_getParameterNames and cmsPhl_getParameterValues.
 *  Tells these functions to only get the local names (do not traverse to
 *  remote MDM's.)  By default, these functions will traverse to remote MDM's
 *  if remote_objd is configured (only in sysmgmt, tr69, and usp components).
 */
#define OGF_LOCAL_MDM_ONLY    0x0004

/** Used in cmsPhl_getParameterNames, cmsPhl_getParameterValues, cmsPhl_getParameterAttribute
 *  Tells these functions to omit all obj/parameter that are marked hiddenFromACS.
 *  When this flag is set, USP obj/parameter marked by bbfProtocol="usp" are also checked and treated
 *  as hiddenFromACS by these PHL routines.
 *  By default, these functions will include all objects and parameters
 */
#define OGF_OMIT_HIDDEN_OBJ_PARAM    0x0008

/** Used in cmsPhl_getParameterNames, cmsPhl_getParameterValues, cmsPhl_getParameterAttribute
 *  Tells these functions hidden eMTA obj/paramter for TR69C.
 *  By default, these functions will include all objects and parameters
 */
#define OGF_EID_TR69C_1    0x0010

/** Used in cmsPhl_getParameterNames, cmsPhl_getParameterValues, cmsPhl_getParameterAttribute
 *  Tells these functions hidden ePTA obj/paramter for TR69C.
 *  By default, these functions will include all objects and parameters
 */
#define OGF_EID_TR69C_2    0x0020

/** Used in cmsPhl_getParameterValues: if a string type parameter is NULL,
 *  omit from response list.  By default, string type params with NULL values
 *  are reported as an empty string.  This flag value is chosen very carefully
 *  so that it does not collide with CMS MEM alloc flags or ZBUS flags.
 */
#define OGF_OMIT_NULL_VALUES  0x8000

/** These values are used by zbus interface module- defined in bcm_zbus_intf.h
 *  Save on success is used in TR69 to set the attribute of remote components' parameters,
 */
#define OGF_ZBUS_SAVE_ON_SUCCESS 0x00010000
#define OGF_ZBUS_INTF_RESERVED   0x00FF0000
...
/** Do validation only, do not set to MDM and do not call RCL.
 *
 * Used by cmsPhl_setParameterValues when an error has been detected on
 * in one of the params, but we still need to validate all the other params
 * as required by TR69/CDRouter validation tests.
 */
#define OSF_VALIDATE_ONLY          0x0004
