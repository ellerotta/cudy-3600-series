...
#if defined(CHIP_63178) || defined(CHIP_47622) || defined(CHIP_6756) || defined(CHIP_6765) || defined(CHIP_6766) || defined(CHIP_6764)
 #define HAS_SF2        1
 #define HAS_ARCHER     1
#else
 #define HAS_RUNNER     1
...
typedef enum {
   ...
   __TS_5000_MBPS        ,
   __TS_10000_MBPS       ,
   __TS_LAST   /*End sentinel*/
} transmission_speeds_t;
...
/*set vlan isolation by port name*/
int bcm_port_vlan_isolation_set_by_name(const char *ifname, unsigned char us_enable, unsigned char ds_enable, int is_object);

/*set broadcast rate limit per port*/
int bcm_port_bc_rate_limit_set(int unit, bcm_port_t port, unsigned int rate);

/*set broadcast rate limit by port name*/
...
#define bcm_port_traffic_enable(a, b) bcm_port_traffic_enable_ex(a, b, 0) 
...
#define bcm_port_loopback_set_by_name(ifname, status) bcm_port_subport_loopback_set_by_name(ifname, -1, 0, status, 0)
...
#define bcm_port_loopback_get_by_name(ifname, status) bcm_port_subport_loopback_get_by_name(ifname, -1, 0, status, 0)
 ...
 *    is_max_speed_caps (IN)  enables to decide whether to return the lowest (wan assymetric interace) or the highest (either wan/lan ethernet) link speed
 ...
 *  bcm_snoop_ctrl_set
 * Description:
 *  Configure the switch for snooping control
 * Parameters:
 *  unit - Ethernet switch unit number (Robo-switch unit=0 does not support)
 *         Always set unit = 1 for external switch
 *  pSnoopCtrlParams  - See structure definition below.
 * Returns:
 *  BCM_E_NONE - Success.
 */
struct bcm_snoop_ctrl_params
{
    unsigned int val;
    int          usz;
    char*        uptr;
...
// Set and Get WAN port.
int     bcm_enet_driver_wan_interface_set(char *ifname, unsigned int val);
#define bcm_enet_driver_wan_interface_get(ifname, sz) \
            bcm_enet_driver_get_port_list_name(ifname, sz, SIOCGWANPORT, "SIOCGWANPORT")

// Get port(s) that are WAN only, LAN or WAN, or LAN only.
...
// Get all port(s) which the port_cap is LAN_WAN or LAN_ONLY.
int bcm_enet_driver_getLANEthPortIfNameList(char *ifNameBuf, unsigned int sz, int *count);

// Given 2 enet devices, check if can be bonded (return 0), otherwise return error message length
int bcm_enet_driver_bonding_check(char *ifname1, char *ifname2,  char *err, unsigned int sz);
...
// The following functions are obsolete.  Use the getWANOnlyEthPortIfNameList,
// getLanWanPortIfNameList, and getLANOnlyEthPortIfNameList functions above.
...
extern int getIntfStats(const char *devName, struct IntfStats_s *s);
