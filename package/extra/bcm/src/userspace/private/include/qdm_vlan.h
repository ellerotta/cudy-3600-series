...
/* 802.1Q TPID values */
#define C_TAG_TPID_STANDARD      0x8100
#define S_TAG_TPID_STANDARD      0x88A8
#define S_TAG_TPID_NON_STANDARD  0x9100
...
/** TR181 function
 * Return the VLAN TPID, VID and Priority of the bridge port. 
 *
 * @param fullpath  (IN) Full path either to the bridge port object or
 *                       to the VLAN termination object which the bridge port
 *                       object points to.
 * @param vlanTPID  (OUT) VLAN TPID assinged to the port.
 * @param vlanId (OUT) VLAN ID of the bridge port.
 *                     Returns -1 if port is untagged.
 *                              0 if port is priority tagged.
 *                             >0 if port is vlan tagged.
 * @param vlanPr (OUT) VLAN priority of the bridge port.
 *
 * @return CmsRet
 */

#if defined(SUPPORT_DM_LEGACY98)
#define qdmVlan_getVlanTPIDByIntfNameLocked(i)  0x8100
#elif defined(SUPPORT_DM_HYBRID)
#define qdmVlan_getVlanTPIDByIntfNameLocked(i)  0x8100
#elif defined(SUPPORT_DM_PURE181)
#define qdmVlan_getVlanTPIDByIntfNameLocked(i)  qdmVlan_getVlanTPIDByIntfNameLocked_dev2((i))
#elif defined(SUPPORT_DM_DETECT)
#define qdmVlan_getVlanTPIDByIntfNameLocked(i)  (cmsMdm_isDataModelDevice2() ? \
                          qdmVlan_getVlanTPIDByIntfNameLocked_dev2((i)) : \
                          0x8100)
#endif
