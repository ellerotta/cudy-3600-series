...
#define DEFAULT_ACCESS_HOST_FILES      FALSE
...
typedef struct
{
   ...
   ContMounts_t mounts[CONT_MOUNT_MAX_ENTRIES];
   ContHooks_t hooks;
   ContSeccomp_t scmp;
   ContBusInfo_t busInfo;   
} AppManContainer_t;
...
*     @param swSystemVersion(OUT) modsw system version(BEEP version x.0 or openplat version x.0)
*     @param swSystemType   (OUT) modsw system type (BEEP or OpenPlat for now).
*  RETURNS:
