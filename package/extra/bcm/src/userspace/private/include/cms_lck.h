 ...
 *
 *    The current number of zone locks (MDM_MAX_LOCK_ZONES) is 16.
 *    These are the assignments:
 *    0 : system and default zone.
 *    1 : reserved by Broadcom for future use.
 *    2 : reserved by Broadcom for future use.
 *    3 : TR-369 Universal Services Platform (USP).
 *    4 : WiFi and IEEE1905.
 *    5 : Voice.
 *    6 : Software Modules (OpenPlat).
 *    7 : Application configuration settings.
 *    8 : TR-069 management server.
 *    9 : PON (EPON and GPON).
...
/** Maximum number of meta locks (for internal use only) */
#define MDM_MAX_META_LOCKS          4
...
/** The number of bytes used to store function names of lock holders.  (actually 47+null term) */
#define CMSLCK_FUNC_NAME_LENGTH     48
...
typedef struct
{
   ...
   MdmObjectId singleOid;  /**< OID of the single mdmObj above */
   const void **array; /**< Array of MAX_TRACKED_MDMOBJS, alloc when needed */
   MdmObjectId *arrayOids; /**< OIDs corresponding to the array of mdmObjs above, alloc when needed */
} TrackedMdmObjs;


#ifdef MDM_LOCK_TRACING
/*
 * This feature can be enabled in menuconfig, in the Debug section.
 * Can be useful when an app has too many nested locking calls, and somewhere
 * someone forgot to unlock a lock.  It is heavy weight in terms of memory 
 * and CPU required, so is not enabled by default.
 * If the app's lock nesting depth goes beyond xyz_LOCK_TRACE_ENTRIES, the
 * tracing will not be accurate anymore, but the actual MDM lock mechanism will
 * still work correctly.  There are more zone lock entries because there is a
 * "lockAllZones" API which will consume MDM_MAX_LOCK_ZONES (16) slots with a
 * single call.  Also, the lock tracing algorithm assumes the function which
 * acquired the lock will also release the lock.  If this assumption is
 * violated, then the tracing array will fill up with "false positives".
 *
 * Shared Memory requirements for lock tracing:
 * Each thread has these arrays:
 * Global Lock trace: GLOBAL_LOCK_TRACE_ENTRIES (10) * sizeof(LockTraceEntry)(64)
 * Zone Lock trace: ZONE_LOCK_TRACE_ENTRIES (64) * sizeof(LockTraceEntry)(64)
 * Zone Entry trace: ZONE_ENTRY_TRACE_ENTRIES (10) * sizeof(LockTraceEntry)(64)
 * Multiply by MDM_MAX_LOCK_THREADS (20)
 * total == 105KB
 * Since this is pretty large, the shared MDM size will be increased if this
 * feature is enabled, see MDM_SHM_LOCK_TRACING_EXTRA in cms_mdm.h
 */
#define GLOBAL_LOCK_TRACE_ENTRIES  10
#define ZONE_LOCK_TRACE_ENTRIES    64
#define ZONE_ENTRY_TRACE_ENTRIES   10

typedef struct
{
   UINT8        lockCount;  /**< lock depth count.  0 means this slot is free. */
   UINT8        lockZone;   /**< 0 - MDM_MAX_LOCK_ZONES-1  */
   char         lockType;   /**< 'g'=global, 'z'=zone, 'e'=entry/autolock */
   char         unused1;
   MdmObjectId  oid;   /**< only entry locks have the oid, global and zone locks do not, so this could be 0  */
   CmsTimestamp ts;
   char funcName[CMSLCK_FUNC_NAME_LENGTH];
} LockTraceEntry;
...
#ifdef MDM_LOCK_TRACING
   LockTraceEntry globalLockTrace[GLOBAL_LOCK_TRACE_ENTRIES];
   LockTraceEntry zoneLockTrace[ZONE_LOCK_TRACE_ENTRIES];
   LockTraceEntry zoneEntryTrace[ZONE_ENTRY_TRACE_ENTRIES];
#endif
} CmsLockThreadInfo;
...
/** Get the local MDM lock zone for the specified oid.
 *
...
/** Get MDM lock zone for local or remote object.
 *
 * @param oid      (IN) The oid of the object node.
 * @param getFlags (IN) Bitmask of GET_OBJNODE_xxx FLAGS defined in mdm.h
 * @param actualFlags (OUT) if not NULL, will be set to where the node was
 *                          found (GET_OBJNODE_LOCAL or GET_OBJNODE_REMOTE).
 * @return MDM_INVALID_LOCK_ZONE if the oid is not valid.
 */
UINT8 cmsLck_getLockZoneFlags(MdmObjectId oid,
                              UINT32 getFlags, UINT32 *actualFlags);
...
void cmsLck_toggleTracing(void);
 ...
 *
 * @return array of MDM_MAX_LOCK_THREADS CmsLockThreadInfo structs, which must
 *         be freed by calling cmsLck_freeLockThreadInfo();
 */
CmsLockThreadInfo * cmsLck_getLockThreadInfo(void);
...
#define MDM_ZONE_LOCK_SEMAPHORE_KEY 0x5eb0

/** Semaphore key for the MDM lock meta-info tables (for internal use). */
#define MDM_META_LOCK_SEMAPHORE_KEY 0x6eb0
