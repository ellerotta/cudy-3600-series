...
typedef struct {
   ...
   UINT32 l2tpIpsecEnable;
   UINT32 ipsPh2AntiReplayWindow;
   char ipsPh2ESN[BUFLEN_16];
   UINT32 isL2tpServerPort;
#endif
#ifdef SUPPORT_VXLAN_TUNNEL_TR181
   char vxlanTunnelId[BUFLEN_40];
   char vxlanTunnelProto[BUFLEN_16];
   char vxlanTunnelRemoteEP[CMS_IPADDR_LENGTH*4];
   UINT32 vxlanTunnelSourcePort;
   UINT32 vxlanTunnelRemotePort;
   char vxlanTunnelKeepAlivePolicy[BUFLEN_16];
   UINT32 vxlanTunnelKeepAliveTimeout;
   UINT32 vxlanTunnelKeepAliveThrsd;
   UINT32 vxlanTunnelDftDSCPMark;

   SINT32 vxlanTunnelIsBridge;
   char vxlanTunnelIfName[BUFLEN_40];
   char vxlanTunnelIfId[BUFLEN_40];
   char vxlanTunnelIfLocalGwIf[BUFLEN_32];
   SINT32 vxlanTunnelIfVNI;
   char vxlanTunnelIfIpAddress[CMS_IPADDR_LENGTH];
   char vxlanTunnelIfSubnet[CMS_IPADDR_LENGTH];
   SINT32 vxlanTunnelIfZeroCsumTx;
   SINT32 vxlanTunnelIfZeroCsumRx;
#endif
#ifdef SUPPORT_GRE_TUNNEL
   char greTunnelId[BUFLEN_40];
   char greTunnelProto[BUFLEN_16];
   char greTunnelRemoteEP[CMS_IPADDR_LENGTH*4];
   char greTunnelKeepAlivePolicy[BUFLEN_16];
   UINT32 greTunnelKeepAliveTimeout;
   UINT32 greTunnelKeepAliveThrsd;
   UINT32 greTunnelDftDSCPMark;

   SINT32 greTunnelIfL2Mode;
   char greTunnelIfName[BUFLEN_40];
   char greTunnelIfId[BUFLEN_40];
   char greTunnelIfLocalGwIf[BUFLEN_32];
   UINT32 greTunnelIfProtocol;
   UBOOL8 greTunnelIfUseCS;
   char greTunnelIfKeyPolicy[BUFLEN_40];
   UINT32 greTunnelIfKeyId;
   UBOOL8 greTunnelIfUseSN;
#endif
...
#endif
#ifdef SUPPORT_TR69C
   UBOOL8  tr69cEnable;        /**< tr69cEnable */
   ...
   char baspURL[BUFLEN_1024];
   char status[BUFLEN_512];
   char deviceId[BUFLEN_64];
   char username[BUFLEN_256];
   char password[BUFLEN_256];
   char basdAction[BUFLEN_32];
   char basdInfo[BUFLEN_1024];
#endif
#ifdef DMP_DEVICE2_LOCALAGENT_1
   char uspAction[BUFLEN_32];
   char uspInfo[BUFLEN_1024*2];
#endif
   ...
   char boardID[BUFLEN_64];   /**< Board Id */
   char cfeVers[BUFLEN_64];   /**< cfe version string */
...
typedef struct L2TP_LNS{
   char enablelns[BUFLEN_4];
   char l2tpnsname[BUFLEN_32];
   char l2tpnslocalip[BUFLEN_16];
   char peerstartip[BUFLEN_16];
   char peerendip[BUFLEN_16];
   char l2tpnsusername[BUFLEN_32];
   char l2tpnspassword[BUFLEN_32];
   char enablechap[BUFLEN_4];
   char enablepap[BUFLEN_4];
   char enablemschap[BUFLEN_4];
   char enablemschapv2[BUFLEN_4];
   char enablelenbit[BUFLEN_4];
} L2TP_LNS, *PL2TP_LNS;
#endif

#ifdef DMP_X_BROADCOM_COM_PPTPNS_2
typedef struct PPTPNS{
   char enablepptpns[BUFLEN_4];
   char pptpnsname[BUFLEN_32];
   char pptpnslocalip[BUFLEN_16];
   char peerstartip[BUFLEN_16];
   char peerendip[BUFLEN_16];
   char pptpnsusername[BUFLEN_32];
   char pptpnspassword[BUFLEN_32];
   char enablechap[BUFLEN_4];
   char enablepap[BUFLEN_4];
   char enablemschap[BUFLEN_4];
   char enablemschapv2[BUFLEN_4];
   char enabledebugpptpns[BUFLEN_4];
} PPTPNS, *PPPTPNS;
#endif
...
/** Get info from the X_BROADCOM_COM_BasdCfg object.
 *
 * @param webVar (OUT) webvar variable to be filled in.
 */
...
/** Set the X_BROADCOM_COM_BasdCfg object.
 *
 * @param (IN) pointer to WEB_NTWK_VAR
 *
 * @return CmsRet enum.
...
/** Use enable parameter in X_BROADCOM_COM_BasdCfg object to restart BAS-D.
 *
 * @return CmsRet enum.
 */
...
/** edit Wan interface attribute.
 *
 * @param ifName              (IN) interface name
 * @param alias               (IN) serviceName
 * @param isBridge            (IN) isBridge
 * @param isIPv4Enabled       (IN) isIPv4Enabled
 * @param isIPv6Enabled       (IN) isIPv6Enabled
 *
 * @return CmsRet enum.
 **/
CmsRet  dalWan_setWanConByIntfName(const char *ifName, const char *alias, UBOOL8 *isBridge, UBOOL8 *isIPv4Enabled, UBOOL8 *isIPv6Enabled);

CmsRet  dalWan_setWanConByIntfName_igd(const char *ifName, const char *alias, UBOOL8 *isBridge, UBOOL8 *isIPv4Enabled, UBOOL8 *isIPv6Enabled);

CmsRet  dalWan_setWanConByIntfName_dev2(const char *ifName, const char *alias, UBOOL8 *isBridge, UBOOL8 *isIPv4Enabled, UBOOL8 *isIPv6Enabled);

/** get Wan interface attribute.
 *
 * @param ifName              (IN) interface name
 * @param isBridge            (OUT) isBridge
 * @param isIPv4Enabled       (OUT) isIPv4Enabled
 * @param isIPv6Enabled       (OUT) isIPv6Enabled
 *
 * @return CmsRet enum.
 **/
CmsRet  dalWan_getWanConByIntfName(const char *ifName, UBOOL8 *isBridge, UBOOL8 *isIPv4Enabled, UBOOL8 *isIPv6Enabled);

CmsRet  dalWan_getWanConByIntfName_igd(const char *ifName, UBOOL8 *isBridge, UBOOL8 *isIPv4Enabled, UBOOL8 *isIPv6Enabled);

CmsRet  dalWan_getWanConByIntfName_dev2(const char *ifName, UBOOL8 *isBridge, UBOOL8 *isIPv4Enabled, UBOOL8 *isIPv6Enabled);

#if defined(SUPPORT_DM_LEGACY98)
#define dalWan_setWanConByIntfName(ifName,alias,isBridge,isIPv4Enabled,isIPv6Enabled)  dalWan_setWanConByIntfName_igd((ifName),(alias),(isBridge),(isIPv4Enabled),(isIPv6Enabled))
#define dalWan_getWanConByIntfName(ifName,isBridge,isIPv4Enabled,isIPv6Enabled)   dalWan_getWanConByIntfName_igd((ifName),(isBridge),(isIPv4Enabled),(isIPv6Enabled))
#elif defined(SUPPORT_DM_HYBRID)
#define dalWan_setWanConByIntfName(ifName,alias,isBridge,isIPv4Enabled,isIPv6Enabled)  dalWan_setWanConByIntfName_igd((ifName),(alias),(isBridge),(isIPv4Enabled),(isIPv6Enabled))
#define dalWan_getWanConByIntfName(ifName,isBridge,isIPv4Enabled,isIPv6Enabled)   dalWan_getWanConByIntfName_igd((ifName),(isBridge),(isIPv4Enabled),(isIPv6Enabled))
#elif defined(SUPPORT_DM_PURE181)
#define dalWan_setWanConByIntfName(ifName,alias,isBridge,IPv4Enable,IPv6Enable)  dalWan_setWanConByIntfName_dev2((ifName),(alias),(isBridge),(IPv4Enable),(IPv6Enable))
#define dalWan_getWanConByIntfName(ifName,isBridge,IPv4Enable,IPv6Enable)   dalWan_getWanConByIntfName_dev2((ifName),(isBridge),(IPv4Enable),(IPv6Enable))
#elif defined(SUPPORT_DM_DETECT)
#define dalWan_setWanConByIntfName(ifName,alias,isBridge,IPv4Enable,IPv6Enable)  (cmsMdm_isDataModelDevice2() ? \
                               dalWan_setWanConByIntfName_dev2((ifName),(alias),(isBridge),(isIPv4Enabled),(isIPv6Enabled)) : \
                               dalWan_setWanConByIntfName_igd((ifName),(alias),(isBridge),(IPv4Enable),(IPv6Enable)))
#define dalWan_getWanConByIntfName(ifName,isBridge,IPv4Enable,IPv6Enable)  (cmsMdm_isDataModelDevice2() ? \
                               dalWan_getWanConByIntfName_dev2((ifName),(isBridge),(IPv4Enable),(IPv6Enable)) : \
                               dalWan_getWanConByIntfName_igd((ifName),(isBridge),(isIPv4Enabled),(isIPv6Enabled)))
#endif
 ...
 */
UBOOL8 dalEth_isEthWanMode(void);
 ...
 */
UBOOL8 dalEth_hasDslWanService(void);
...
/**  Set the LimitedConnections in the layer 2 objects. For now, just EthWAN
 *
...
#include "omci_msg.h"  // For GponWanServiceParams
...
CmsRet dalGetOpenVSPorts(char *ports);
#endif

#ifdef DMP_X_BROADCOM_COM_OPENVSWITCH_2
UBOOL8 dalOpenVS_isEnabled_dev2();
CmsRet dalOpenVS_getCfg_dev2(char *info);
CmsRet dalOpenVS_setCfg_dev2(const char *enable, const char *ofControllerAddr, UINT32 ofControllerPort);
 ...
 *
 * @param enable          (IN)
 * @param pptpIpAddress   (IN)
 * @param pptpusertname   (IN)
 * @param pptppassword    (IN)
 *
 * @return CmsRet enum.
 */
CmsRet dalPptpCfg(const char *enable, const char *pptpIpAddress, const char *pptpusertname, const char *pptppassword);

/** get PPTP information.
 *
 * @param info      (IN)
 *
 * @return CmsRet enum.
 */
CmsRet dalGetPptpInfo(char *info);
...
#ifdef SUPPORT_VXLAN_TUNNEL_TR181
/** add/delete an Vxlan tunnel/interface/filter entry.
 *
 * @param webData (IN) pointer to global web data structure
 *
 * @return CmsRet enum.
...
CmsRet dalVxlan_addFilter_dev2(const PWEB_NTWK_VAR pWebVar);

/** add/delete an Vxlan tunnel entry.
 *
 * @param name      (IN) Vxlan tunnel/interface/filter name
 *
 * @return CmsRet enum.
 **/
CmsRet dalVxlan_deleteTunnel_dev2(const char* name);
CmsRet dalVxlan_deleteTunnelIf_dev2(const char* name);
CmsRet dalVxlan_deleteFilter_dev2(const char* name);

/** Reset VxLan parameters.
 *
 * @param name      (IN) pWebVar
 *
 * @return CmsRet enum.
...
/** add an GRE tunnel/interface entry.
 *
 * @param webData (IN) pointer to global web data structure
 *
 * @return CmsRet enum.
...
/** delete an GRE tunnel entry.
 *
 * @param name      (IN) GRE tunnel/interface name
 *
 * @return CmsRet enum.
 **/
CmsRet dalGre_deleteTunnel_dev2(const char* name);
CmsRet dalGre_deleteTunnelIf_dev2(const char* name);

/** Reset GRE parameters.
 *
 * @param name      (IN) pWebVar
 *
 * @return CmsRet enum.
