...
BcmRet xdslSeltGetAfeIdAndSeltData(unsigned char lineId, unsigned int *pAfeId, SeltData *pSeltData, long *pSeltDataLen)
{
   BcmRet      bcmRet;
   adslMibInfo adslMib;
   long        size = sizeof(adslMib);
   
   bcmRet = xdslCtl_GetObjectValue(lineId, NULL, 0, (char *)&adslMib, &size);
   if (bcmRet != BCMRET_SUCCESS) {
   ...
   if (1 == lineId) {
      size = sizeof(adslMib);
      bcmRet = xdslCtl_GetObjectValue(0, NULL, 0, (char *)&adslMib, &size);
      if (bcmRet != BCMRET_SUCCESS) {
         //cmsLog_error("%s: xdslCtl_GetObjectValue error\n", __FUNCTION__ );
         return (BCMRET_INTERNAL_ERROR);
      }
...
BcmRet xdslSeltGetSeltData(unsigned char lineId, SeltData *pSeltData, long *pSeltDataLen)
{
...
BcmRet xdslSeltGetUER(unsigned char lineId, int *pData, long *pDataLen)
{
...
BcmRet xdslSeltUpdateSeltData(unsigned char lineId, SeltData *pSeltData, long *pSeltDataLen)
{
...
BcmRet xdslSeltSetSeltNextMode(unsigned char lineId)
{
