...
CmsRet devCtl_xtmConfig( PXTM_CONFIGURATION_PARMS pConfigParms )
{
    ...
    memcpy(&Arg.Config, pConfigParms, sizeof(XTM_CONFIGURATION_PARMS));
    Arg.bxStatus = XTMSTS_ERROR;
...
CmsRet devCtl_xtmManageThreshold( PXTM_THRESHOLD_PARMS pThresholdParms )
{
    ...
    memcpy(&Arg.Threshold, pThresholdParms, sizeof(XTM_THRESHOLD_PARMS));
    Arg.bxStatus = XTMSTS_ERROR;
...
/***************************************************************************
 * Function Name: devCtl_xtmAccessSAR
 * Description  : Informs the direct I/O operation on SAR to XTM from XTMCTL
 * Returns      : CmsRet return code
 ***************************************************************************/
CmsRet devCtl_xtmAccessSAR( int SarIO )
{
    XTMCFGDRV_ACCESS_SAR_PARAMS Arg;

    devCtl_xtmAccessSARTrace();
    ...
    if( g_nXtmFd != -1  )
        ioctl( g_nXtmFd, XTMCFGIOCTL_ACCESS_SAR, &Arg );
    else
        Arg.bxStatus = XTMSTS_STATE_ERROR;
...
/***************************************************************************
 * Function Name: devCtl_xtmSARRegDump
 * Description  : Sends an IOCTL to read SAR register content.
 * Returns      : CmsRet return code
 ***************************************************************************/
CmsRet devCtl_xtmSARRegDump( PXTM_SAR_REG_DUMP pSarRegDump)
{
    XTMCFGDRV_SAR_REG_DUMP Arg;

    memset(&Arg, 0, sizeof(XTMCFGDRV_SAR_REG_DUMP)); 
    devCtl_xtmSARRegDumpTrace();

    Arg.regCategoryId = pSarRegDump->regCategoryId;
    Arg.buffSize      = pSarRegDump->buffSize;
    Arg.pRegBuff      = pSarRegDump->regBuff;
    ...
    if( g_nXtmFd != -1  )
        ioctl( g_nXtmFd, XTMCFGIOCTL_SAR_REG_DUMP, &Arg );
    else
        Arg.bxStatus = XTMSTS_STATE_ERROR;
    pSarRegDump->numRegs  = Arg.numRegs;
    pSarRegDump->baseAddr = Arg.baseAddr;
...
/***************************************************************************
 * Function Name: devCtl_xtmGetXTMPStatus
 * Description  : Returns XTMP Status debug info.
 * Returns      : CmsRet return code
 ***************************************************************************/
CmsRet devCtl_xtmGetXTMPStatus ( PXTM_XTMP_STATUS pXTMPStatus )
{
    XTMCFGDRV_XTMP_STATUS Arg;

    devCtl_xtmGetXTMPStatusTrace();

    memcpy(&Arg.XTMPStatus, pXTMPStatus, sizeof(XTM_XTMP_STATUS));
    Arg.bxStatus = XTMSTS_ERROR;
    ...
    if( g_nXtmFd != -1  )
    {
        ioctl( g_nXtmFd, XTMCFGIOCTL_GET_XTMP_STATUS, &Arg );
        memcpy(pXTMPStatus, &Arg.XTMPStatus, sizeof(XTM_XTMP_STATUS));
    }
    else
        Arg.bxStatus = XTMSTS_STATE_ERROR;
