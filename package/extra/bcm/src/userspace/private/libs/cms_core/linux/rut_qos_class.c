...
here use skb priority 8~15(priority + SKB_PRIO_OFFSET) to do classification.
*/
#define SKB_PRIO_OFFSET  8
...
static CmsRet rutQos_brIngressClass(const CmsQosClassInfo *clsObj,
                                    QosCommandType cmdType);
static CmsRet rutQos_roEgressMarkCfg(const CmsQosClassInfo *clsObj,
                                  UINT32 priority,
                                  QosCommandType cmdType,
                                  char *tblStr);
...
CmsRet rutQos_qMgmtClassConfig(QosCommandType cmdType, const void *cObj)
{
   ...
   cmsLog_debug("icmpv6Type=%d", cInfo->icmpv6Type);
   cmsLog_debug("icmpv6Code=%d", cInfo->icmpv6Code);
   cmsLog_debug("icmpv6TypeCodeExclude=%d", cInfo->icmpv6TypeCodeExclude);
...
#if defined(SUPPORT_WANVLANMUX) && defined(SUPPORT_VLANCTL)
         else if (strchr(cInfo->egressIntfName, '.') != NULL && qdmVlan_getVlanIdByIntfNameLocked(cInfo->egressIntfName) > 0)
         {
                           ...
                           QosCommandType cmdType)
{
   ...
   switch (cInfo->etherType)
   {
      case ETH_P_IP:
         ...
         if (cInfo->DSCPCheck != QOS_CRITERION_UNUSED)
         {
            ...
            sprintf(dscpCheck, "--ip-dscp-extend %s 0x%x", exclude, cInfo->DSCPCheck << 2);
         }
      ...
      case ETH_P_IPV6:
         ...
         if(cInfo->protocol == IPPROTO_ICMPV6 && !cInfo->protocolExclude)
         {
            if(cInfo->icmpv6Type != QOS_CRITERION_UNUSED)
            {
               exclude = cInfo->icmpv6TypeCodeExclude? "!" : "";
               if(cInfo->icmpv6Code != QOS_CRITERION_UNUSED)
               {
                  sprintf(icmp6TypeCode, "--ip6-icmp-type %s %d/%d", exclude, cInfo->icmpv6Type, cInfo->icmpv6Code);
               }
               else
               {
                  sprintf(icmp6TypeCode, "--ip6-icmp-type %s %d", exclude, cInfo->icmpv6Type);
               }
            }
         }               
   ...
   if (cmdType == QOS_COMMAND_CONFIG)
   {
      ...
      snprintf(cmd, sizeof(cmd), "ebtables --concurrent -t nat -A OUTPUT %s %s %s %s %s %s %s -j mark --mark-or 0x%x",
              egressIfc, ethertype, proto, icmp6TypeCode, sport, dport, dscpCheck, flowId);
   }
   else
   {
      snprintf(cmd, sizeof(cmd), "ebtables --concurrent -t nat -D OUTPUT %s %s %s %s %s %s %s -j mark --mark-or 0x%x 2>/dev/null",
              egressIfc, ethertype, proto, icmp6TypeCode, sport, dport, dscpCheck, flowId);
   }
                           ...
                           QosCommandType cmdType)
{
   ...
   if(cInfo->protocol == IPPROTO_ICMPV6 && !cInfo->protocolExclude)
   {
      if(cInfo->icmpv6Type != QOS_CRITERION_UNUSED)
      {
         ...
         if(cInfo->icmpv6Code != QOS_CRITERION_UNUSED)
         {
            sprintf(icmp6TypeCode, "%s --icmpv6-type %d/%d", exclude, cInfo->icmpv6Type, cInfo->icmpv6Code);
         }
         else
         {
            sprintf(icmp6TypeCode, "%s --icmpv6-type %d", exclude, cInfo->icmpv6Type);
         }
      }
   }
   ...
   if (cmdType == QOS_COMMAND_CONFIG)
   {
      ...
      snprintf(cmd, sizeof(cmd), "%s -w -t mangle -A OUTPUT %s %s %s %s %s %s -j MARK --or-mark 0x%x",
              tblStr, egressIfc, proto, icmp6TypeCode, sport, dport, dscpCheck, flowId);
   }
   else
   {
      snprintf(cmd, sizeof(cmd), "%s -w -t mangle -D OUTPUT %s %s %s %s %s %s -j MARK --or-mark 0x%x 2>/dev/null",
              tblStr, egressIfc, proto, icmp6TypeCode, sport, dport, dscpCheck, flowId);
   }
                             ...
                             QosCommandType cmdType)
{
   ...
   if (cmdType == QOS_COMMAND_CONFIG)
   {
      ...
      snprintf(cmd, sizeof(cmd), "%s -w -t mangle -A PREROUTING %s %s %s %s %s %s %s %s %s -j MARK --or-mark 0x%x",
              tblStr, proto, icmp6TypeCode, ifcStr, smac, src, dst, sport, dport, dscpCheck, flowId);
   }
   else
   {
      snprintf(cmd, sizeof(cmd), "%s -w -t mangle -D PREROUTING %s %s %s %s %s %s %s %s %s -j MARK --or-mark 0x%x 2>/dev/null",
              tblStr, proto, icmp6TypeCode, ifcStr, smac, src, dst, sport, dport, dscpCheck, flowId);
   }
#ifdef SUPPORT_NF_MANGLE
                             ...
                             QosCommandType cmdType)
{
   ...
   if (cmdType == QOS_COMMAND_CONFIG)
   {
      ...
      snprintf(cmd, sizeof(cmd), "ebtables --concurrent -t broute -A BROUTING %s %s %s %s %s %s %s %s %s %s %s %s -j mark --mark-or 0x%x",
              ethertype, proto, icmp6TypeCode, src, sport, dst, dport, smac, dmac, vlan8021p, ifcStr, dscpCheck, flowId);
   }
   else
   {
      snprintf(cmd, sizeof(cmd),"ebtables --concurrent -t broute -D BROUTING %s %s %s %s %s %s %s %s %s %s %s %s -j mark --mark-or 0x%x 2>/dev/null",
              ethertype, proto, icmp6TypeCode, src, sport, dst, dport, smac, dmac, vlan8021p, ifcStr, dscpCheck, flowId);
   }
                           ...
                           QosCommandType cmdType)
{
   ...
   if (cInfo->DSCPMark != QOS_RESULT_NO_CHANGE)
   {
      ...
      {
         /* set_tos_dscp is used by ebtables command */
         sprintf(set_tos_dscp, "-j ftos --8021q-ftos");
      }
      else
      {
         /* set_tos_dscp is used by ebtables command */
         sprintf(set_tos_dscp, "-j dscp --set-dscp 0x%x --dscp-target CONTINUE", cInfo->DSCPMark);
      }
   }
   ...
   if (!IS_EMPTY_STRING(cInfo->egressIntfName))
   {
           ...
           cInfo->ethernetPriorityMark     != QOS_RESULT_NO_CHANGE))
      {
         ...
         sprintf(set_vtag, "-j vtag --vtag-set 0x%x%x", vlanIdMark, vlan8021pMark);
   
         /* change the mark target to be non-terminating */
         strcat(set_vtag, " --vtag-target CONTINUE");
      }
   }
   ...
   if (cmdType == QOS_COMMAND_CONFIG)
   {
       ...
       */
      snprintf(cmd, sizeof(cmd), "ebtables --concurrent -t nat -N rule%d", clsKey);
      ...
      if (egressWlan)
      {
         ...
         snprintf(cmd, sizeof(cmd), "ebtables --concurrent -t nat -A rule%d %s --wmm-mark-target CONTINUE", clsKey, priorityStr);
      }
      else
#endif
      {
...
#ifdef SUPPORT_RATE_LIMIT
         if (cInfo->classRate != QOS_RESULT_NO_CHANGE)
         {
            snprintf(cmd, sizeof(cmd), "ebtables --concurrent -t nat -A rule%d -o %s+ -j SKIPLOG", clsKey, cInfo->egressIntfName);
            rut_doSystemAction("rutQos_brEgressMark", cmd);
         }
#endif
#if defined(SUPPORT_POLICING) && defined(TC_POLICING)
         if (cInfo->policerInstance > 0)
         {
            snprintf(cmd, sizeof(cmd), "ebtables --concurrent -t nat -A rule%d -o %s+ -j SKIPLOG", clsKey, cInfo->egressIntfName);
            rut_doSystemAction("rutQos_brEgressMark", cmd);
      ...
      if (!IS_EMPTY_STRING(set_tos_dscp))
      {
         /* mark dscp */
         snprintf(cmd, sizeof(cmd), "ebtables --concurrent -t nat -A rule%d %s", clsKey, set_tos_dscp);
         rut_doSystemAction("rutQos_brEgressMark", cmd);
      }
      ...
      {
         /* mark 8021p */
         snprintf(cmd, sizeof(cmd), "ebtables --concurrent -t nat -A rule%d %s", clsKey, set_vtag);
         rut_doSystemAction("rutQos_brEgressMark", cmd);
      }
      else if(cInfo->ethernetPriorityMark != QOS_RESULT_NO_CHANGE)
      {
         snprintf(cmd, sizeof(cmd), "ebtables --concurrent -t nat -A rule%d -j skbpriority --set-class 0:0x%x --skbpriority-target CONTINUE",
                    clsKey, (cInfo->ethernetPriorityMark & 0x7) + SKB_PRIO_OFFSET);   

         rut_doSystemAction("rutQos_brEgressMark", cmd);
      }

      snprintf(cmd, sizeof(cmd), "ebtables --concurrent -t nat -A POSTROUTING %s --mark 0x%x/0x%x -j rule%d",
              egressIfc, flowId, SKBMARK_FLOW_ID_M, clsKey);
      rut_doSystemAction("rutQos_brEgressMark", cmd);
   }
   else
   {
      snprintf(cmd, sizeof(cmd), "ebtables --concurrent -t nat -D POSTROUTING %s --mark 0x%x/0x%x -j rule%d 2>/dev/null",
              egressIfc, flowId, SKBMARK_FLOW_ID_M, clsKey);
      rut_doSystemAction("rutQos_brEgressMark", cmd);
      snprintf(cmd, sizeof(cmd), "ebtables --concurrent -t nat -F rule%d 2>/dev/null", clsKey);
      rut_doSystemAction("rutQos_brEgressMark", cmd);
      snprintf(cmd, sizeof(cmd), "ebtables --concurrent -t nat -X rule%d 2>/dev/null", clsKey);
      rut_doSystemAction("rutQos_brEgressMark", cmd);
   }
...
/***************************************************************************
// Function Name: rutQos_roEgressMarkCfg
// Description  : set up POSTROUTING mangle iptables rules for traffic class marking.
// Parameters   : clsObj - QoS class information.
//                priority - egress queue priority information.
//                cmdType - command type either config or unconfig.
//                tblStr - iptables or ip6tables
// Returns      : CmsRet
****************************************************************************/
static CmsRet rutQos_roEgressMarkCfg(const CmsQosClassInfo *cInfo,
                           UINT32 priority,
                           QosCommandType cmdType,
                           char *tblStr)
{
...
//   }
   ...
   if (cmdType == QOS_COMMAND_CONFIG)
   {
       ...
       */
      snprintf(cmd, sizeof(cmd), "%s -w -t mangle -N rule%d", tblStr, clsKey);
      rut_doSystemAction("rutQos_roEgressMarkCfg", cmd);

#ifdef SUPPORT_RATE_LIMIT
      if (cInfo->classRate != QOS_RESULT_NO_CHANGE)
      {
         snprintf(cmd, sizeof(cmd), "%s -w -t mangle -A rule%d -o %s+ -j SKIPLOG", tblStr, clsKey, cInfo->egressIntfName);
         rut_doSystemAction("rutQos_roEgressMarkCfg", cmd);
      }
#endif
#if defined(SUPPORT_POLICING) && defined(TC_POLICING)
...
#endif
      /* mark priority queue */
      snprintf(cmd, sizeof(cmd), "%s -w -t mangle -A rule%d -j MARK --or-mark 0x%x", tblStr, clsKey, queueMark);
      rut_doSystemAction("rutQos_roEgressMarkCfg", cmd);

      if (!IS_EMPTY_STRING(set_dscp))
      {
         /* mark dscp */
         snprintf(cmd, sizeof(cmd), "%s -w -t mangle -A rule%d %s", tblStr, clsKey, set_dscp);
         rut_doSystemAction("rutQos_roEgressMarkCfg", cmd);
      }
      
      if (cInfo->ethernetPriorityMark != QOS_RESULT_NO_CHANGE)
      {
         snprintf(cmd, sizeof(cmd), "%s -w -t mangle -A rule%d -j CLASSIFY --set-class 0:0x%x", tblStr, clsKey, cInfo->ethernetPriorityMark + SKB_PRIO_OFFSET);
         rut_doSystemAction("rutQos_roEgressMarkCfg", cmd);
      }
      
      snprintf(cmd, sizeof(cmd), "%s -w -t mangle -A rule%d -j ACCEPT", tblStr, clsKey);      
      rut_doSystemAction("rutQos_roEgressMarkCfg", cmd);
		  
      snprintf(cmd, sizeof(cmd), "%s -w -t mangle -A POSTROUTING %s -m mark --mark 0x%x/0x%x -j rule%d",
              tblStr, egressIfc, flowId, SKBMARK_FLOW_ID_M, clsKey);
      rut_doSystemAction("rutQos_roEgressMarkCfg", cmd);
   }
   else
   {
      snprintf(cmd, sizeof(cmd), "%s -w -t mangle -D POSTROUTING %s -m mark --mark 0x%x/0x%x -j rule%d 2>/dev/null",
              tblStr, egressIfc, flowId, SKBMARK_FLOW_ID_M, clsKey);
      rut_doSystemAction("rutQos_roEgressMarkCfg", cmd);
      snprintf(cmd, sizeof(cmd), "%s -w -t mangle -F rule%d 2>/dev/null", tblStr, clsKey);
      rut_doSystemAction("rutQos_roEgressMarkCfg", cmd);
      snprintf(cmd, sizeof(cmd), "%s -w -t mangle -X rule%d 2>/dev/null", tblStr, clsKey);
      rut_doSystemAction("rutQos_roEgressMarkCfg", cmd);
   }
#endif // SUPPORT_NF_MANGLE
                           ...
                           QosCommandType cmdType)
{
   ...
   if (cInfo->etherType == ETH_P_IP || cInfo->etherType == ETH_P_ARP)
   {
      strcpy(tblStr, "iptables");
      ret = rutQos_roEgressMarkCfg(cInfo, priority, cmdType, tblStr);
   }
   else if (cInfo->etherType == ETH_P_IPV6)
   {
#ifdef SUPPORT_IPV6
      strcpy(tblStr, "ip6tables");
      ret = rutQos_roEgressMarkCfg(cInfo, priority, cmdType, tblStr);
                      ...
                      QosCommandType cmdType)
{
   if (cInfo->ethernetPriorityMark != QOS_RESULT_NO_CHANGE)
   {
      ...
      if (cmdType == QOS_COMMAND_CONFIG)
      {
         SINT32 vlanId = -1;
         UINT32 tpId = 0x8100;
         vlansubif_qos_options_t vlan_subif_qos_options = {
         .tx_if_name = l3IfName,
         .flow_id    = SKBMARK_GET_FLOW_ID(flowId),
         .tpid       = 0x8100,
         .pbit       = cInfo->ethernetPriorityMark,
         .vid        = 0
         };
         ...
         tpId = qdmVlan_getVlanTPIDByIntfNameLocked(cInfo->egressIntfName);
         
         vlan_subif_qos_options.vid = vlanId;
         vlan_subif_qos_options.tpid = tpId!=0?tpId:0x8100;
         (void)vlanSubif_addQosVlanTagRule(l2IfName, &vlan_subif_qos_options);
      ...
      else
      {
         ...
         (void)vlanSubif_delQosVlanTagRule(l2IfName, &vlan_subif_qos_options);
...
void rutQos_sendDhcp(SINT32 type, char *cmd)
{
   ...
   hdr->type          = type;
   hdr->src           = cmsMsg_getHandleEid(msgHandle);
   hdr->dst           = EID_DHCPD;
   hdr->flags_request = 1;
   ...
   ret = cmsMsg_send(msgHandle, hdr);
   if (ret != CMSRET_SUCCESS)
   {
      cmsLog_error("Failed to send message to DHCP daemon (ret=%d)", ret);
   }
...
CmsRet rutQos_fillPolicerInfo_igd(const SINT32 instance, const UINT32 policerInfo)
{
