                                   ...
                                   UBOOL8 enable __attribute__((unused)))
{
    ...
    while ((ret = cmsObj_getNextInSubTreeFlags(MDMOID_WL_VIRT_INTF_CFG, iidStack, &virtIidStack, OGF_NO_VALUE_UPDATE, (void **) &virtIntfObj)) == CMSRET_SUCCESS)
    {
        ...
        else if (!enable)
        {
            ...
            {
                /*
                 * wlBrName is an internal book-keeping parameter used by wlssk.
...
UBOOL8 rutWlan_isValidTr98WlAuthMode(const char *wlAuthMode)
{
	const char tr98AuthMode[] = "radius shared psk wpa psk2 wpa2 open ";
	char strBuf[BUFLEN_16] = {};

	strncpy(strBuf, wlAuthMode, sizeof(strBuf)-2);
	strBuf[strlen(strBuf)] = ' ';

	return strstr(tr98AuthMode, strBuf)?TRUE:FALSE;
}

void rutWlan_WlAuthModeStripNonTr98(char *inAuthMode, char *outAuthMode, int len)
{
    char strBuf[BUFLEN_128] = {0};
    char out[BUFLEN_128] = {0};
    char *splitAuthMode = NULL;
    char *savePtr = NULL;

    cmsUtl_strncpy(strBuf, inAuthMode, sizeof(strBuf));
    splitAuthMode = strtok_r(strBuf, " ", &savePtr);
    while (splitAuthMode != NULL)
    {
        if (rutWlan_isValidTr98WlAuthMode(splitAuthMode))
        {
            strcat(out, splitAuthMode);
            strcat(out, " ");
        }
    ...
    cmsUtl_strncpy(outAuthMode, out, len);
}
