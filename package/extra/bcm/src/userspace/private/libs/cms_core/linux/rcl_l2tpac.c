...

{
    ...
    if (ADD_NEW(newObj,currObj))
    {
        ...
        if (!IS_EMPTY_STRING(newObj->tunnelName))
        {
            rutL2tp_createTunnelConfig(newObj);
        }
    }   
    ...
    {     
        rutL2tp_stopL2tpd(newObj->tunnelName);
        ret = rutL2tp_createTunnelConfig(newObj);
        cmsLog_debug("L2tp create Tunnel config ret(%d)", ret);
    }
    else if (DELETE_OR_DISABLE_EXISTING(newObj, currObj))
    {
        ...
        rutL2tp_stopL2tpd(currObj->tunnelName);
    }
                ...
                CmsRet *errorCode __attribute__((unused)))
{
        ...
        POTENTIAL_CHANGE_OF_EXISTING(newObj, currObj))
    {
                                    ...
                                    &iidStackSub, (void **)&pppCon)) == CMSRET_SUCCESS))
        {
            ...
            {
                ret = rutL2tp_createLinkConfig(newObj, pppCon);
            ...
            rutL2tp_startL2tpd(newObj->tunnelName);
        }
