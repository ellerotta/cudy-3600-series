                               ...
                               const char *DNSServer6)
{
   ...
   {
      strncpy(wanIfName, ifName4, sizeof(wanIfName)-1);
      wanIfName[sizeof(wanIfName)-1] = '\0';
      
   }
   else if (!IS_EMPTY_STRING(ifName6))
   {
      strncpy(wanIfName, ifName6, sizeof(wanIfName)-1);
      wanIfName[sizeof(wanIfName)-1] = '\0';
   }
...
CmsRet rutDns_dumpDnsInfo_igd(FILE *fp, UBOOL8 needDefaultDns)
{
         ...
         else
         {
            ...
            */
            if (isStaticIPoE)
            {
               ...
               {
                  cmsUtl_strncpy(defaultDNSIfName, networkCfg->DNSIfName, sizeof(defaultDNSIfName));
                  cmsObj_free((void *) &networkCfg);
               }
               while (!found && 
            ...
            if (isStaticIPoE6)
            {
               ...
               {
                  cmsUtl_strncpy(defaultDNSIfName, ipv6LanCfgObj->IPv6DNSWANConnection, sizeof(defaultDNSIfName));
                  ...
                  cmsObj_getNext(MDMOID_WAN_IP_CONN, &defaultDnsIpIid, (void **) &defaultDnsIp) == CMSRET_SUCCESS)
               {
                  if (!cmsUtl_strcmp(defaultDnsIp->X_BROADCOM_COM_IfName, defaultDNSIfName))
                  {
                     ...
                     if ((cmsUtl_parseDNS(defaultDnsIp->X_BROADCOM_COM_IPv6DNSServers, dns1, dns2, FALSE) == CMSRET_SUCCESS) &&
                         ...
                         cmsUtl_isValidIpAddress(AF_INET6, dns1))
                     {
                         /* Default IPv6 Gateway may be changed, here delete existing DNS route first,
                            and then check Gateway status and re-add it. */
                        snprintf(cmd, sizeof(cmd), "route -A inet6 del %s dev %s 2>/dev/null", dns1, defaultDNSIfName);
                        rut_doSystemAction("Del IPv6 DNS static route", cmd);
            ...
            else
            {
                   ...
                   cmsUtl_isValidIpAddress(AF_INET6, dns1))
               {
                  /* Default IPv6 Gateway may be changed, here delete existing DNS route first,
                     and then check Gateway status and re-add it. */
                  snprintf(cmd, sizeof(cmd), "route -A inet6 del %s dev %s 2>/dev/null", dns1, ifName6);
                  rut_doSystemAction("Del IPv6 DNS static route", cmd);
                     
                  if (!IS_EMPTY_STRING(IPoEGateway6))
                  {
                      if (0 > snprintf(cmd, sizeof(cmd), "route -A inet6 add %s gw %s dev %s 2>/dev/null", dns1, IPoEGateway6, ifName6))
                      {
                         cmsLog_error("snprintf failed to write 'route -A inet6 add %s gw %s dev %s 2>/dev/null' into cmd", dns1, IPoEGateway6, ifName6);
                         cmd[0] = 0;
                      }
                  }
                  else 
   ...
   /*
    * this is needed for devices using dhcpc on LAN side.  In this
