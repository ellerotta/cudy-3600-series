...
CmsRet rutQos_qMgmtQueueConfig(QosCommandType cmdType, const void *qObj)
{
   ...
   {
      /* Ethernet interface : could be LAN or WAN */
      rutQos_tmQueueConfig_pre(qInfo.intfName);
      ...
      {
          if (!qdmQos_isAnyInterfaceOfQueuesMatchedLocked(qObj))
          {
      ...
      rutQos_tmQueueConfig_post(qInfo.intfName);
   }
            ...
            ((cmsUtl_strstr(qInfo.intfName, "GPON")) != NULL) ||
...
CmsRet rutQos_qMgmtClassReconfig_igd(char *ifcname, UBOOL8 layer2Intf)
{
   CmsRet   ret;
   InstanceIdStack iidStack;
   QMgmtQueueObject *qObj = NULL;
...
#ifdef SUPPORT_FCCTL
   char cmd[BUFLEN_64];
   snprintf(cmd, sizeof(cmd), "fcctl flush --if %s --silent ", ifcname);
   rut_doSystemAction("rutQos_qMgmtClassReconfig", cmd);
#endif
   
   return CMSRET_SUCCESS;

}  /* End of rutQos_qMgmtClassReconfig_igd() */
                                      ...
                                      UINT32 *numQueues, UINT32 *numLevels)
{
   ...
   else
   {
      ...
      {
         cmsLog_error("Unsupported l2IfName %s, assume eth", l2IfName);
      }
...
static int rutQos_get_queues_per_uni(void)
{
   UINT32 totalQueues;
   UINT32 queuesPerUni = MAX_ETH_TRANSMIT_QUEUES;
   UINT32 uniNum;
   ...
   INIT_INSTANCE_ID_STACK(&iidStack);
   if ((ret = cmsObj_getNext(MDMOID_LAN_DEV, &iidStack, (void **) &lanDevObj)) != CMSRET_SUCCESS)
   {
      cmsLog_error("could not get LAN DEV object, ret=%d", ret);
      return queuesPerUni;
   }

   totalQueues = get_rdpa_user_num_ds_queues(); 
   uniNum = lanDevObj->LANEthernetInterfaceNumberOfEntries;

   if (uniNum)
   {
      queuesPerUni = totalQueues / uniNum;
      queuesPerUni = (queuesPerUni > MAX_ETH_TRANSMIT_QUEUES)? 
         MAX_ETH_TRANSMIT_QUEUES : queuesPerUni;
   }
...
#if defined(SUPPORT_RDPA) && defined(BCM_PON)
      /*
         Low end PON platforms have not enough queues to allocate 
         MAX_ETH_TRANSMIT_QUEUES per each UNI. The solution is
         to get total queues number from RDPA and to allocate 
         queues evenly among UNIs
      */
      maxQueues = rutQos_get_queues_per_uni();
#else      
...
static UBOOL8 rutQos_tmIsOwnerFE(void)
{
   static eTmctlOwner owner = TMCTL_OWNER_MIN;

   if (owner == TMCTL_OWNER_MIN)
   {
      owner = rut_tmctl_getQueueOwner();
   }

   if (owner == TMCTL_OWNER_FE)
   {
      return TRUE;
   }
   else
...
CmsRet rutQos_tmPortInit(const char *l2ifname, UBOOL8 isWan)
{
   ...
   if(isIntf_USB((char *)l2ifname))
      goto Exit;   

   if ((ret = rutwrap_tmctl_GetDevInfoByIfName(l2ifname, &devType, &intf)) != CMSRET_SUCCESS)
   {
...
CmsRet rutQos_tmQueueConfig(QosCommandType cmdType, CmsQosQueueInfo *qInfo)
{
   ...
   if ((ret = rutwrap_tmctl_GetDevInfoByIfName(qInfo->intfName, &devType, &intf)) != CMSRET_SUCCESS)
   {
   ...
   if(devType == TMCTL_DEV_ETH)
   {
...
#if defined(BCM_PON)
         bestEffort = (qInfo->queuePrecedence == 1);
#else
         bestEffort = (qInfo->queuePrecedence == ETHWAN_QOS_LEVELS);
#endif
#else
         return CMSRET_SUCCESS;
      ...
      if (cmdType == QOS_COMMAND_UNCONFIG)
        return( (CmsRet) rutcwmp_tmctl_delQueueCfg(devType, &intf, qcfg.qid));
...

      {
         ...
         qcfg.qsize                   = TMCTL_DEFAULT_QUEUE_SIZE;
         ...
         else if (qcfg.qsize > 0) /*qcfg.qsize with TMCTL_DEFAULT_QUEUE_SIZE is updated upon the successful return of tmctl_setQueueCfg*/
         {
   ...
   {
      tmctl_queueDropAlg_t dropAlgCfg;
       ...
       */
      priority = XTM_QOS_LEVELS - qInfo->queuePrecedence;
      ...
      qcfg.qsize = HOST_XTM_NR_TXBDS;
      qcfg.priority = priority;
      qcfg.schedMode = schedMode;
      qcfg.weight = qInfo->queueWeight;
      qcfg.shaper.shapingRate = (qInfo->shapingRate > 0)? qInfo->shapingRate/1000 : 0;
      qcfg.shaper.shapingBurstSize = qInfo->shapingBurstSize;
      qcfg.shaper.minRate = (qInfo->minBitRate > 0)? qInfo->minBitRate/1000 : 0;
      qcfg.bestEffort = bestEffort;

      /* Check DSL link is up by getPortTmParms. Config or unconfig XTM queue when DSL link is up. */
      if (rutcwmp_tmctl_getPortTmParms(devType, &intf, &portTmParms) == TMCTL_ERROR)
      {
         cmsLog_debug("Failed to getPortTmParms. Please confirm DSL link is up. Skipping queue operation.");
         return CMSRET_SUCCESS;
      }
      ...
      if (cmdType == QOS_COMMAND_CONFIG)
      {
         ...
         if (rutcwmp_tmctl_setQueueCfg(devType, &intf, &qcfg) == TMCTL_ERROR)
         {
            cmsLog_error("Add TM queue. ifname=%s qid=%d priority=%d schedMode=%d qWt=%d bestEffort=%d",
                         qInfo->intfName, qInfo->queueId - 1, priority, schedMode, qInfo->queueWeight, bestEffort ? 1 : 0);
            return CMSRET_INTERNAL_ERROR;
         }

         /* XTM qsize is determined by link mode. Get actual qsize after queue is set. */
         if (rutcwmp_tmctl_getQueueCfg(devType, &intf, qInfo->queueId - 1, &qcfg) == TMCTL_ERROR)
         {
            cmsLog_error("Get TM queue failed, qid=%d.", qcfg.qid);
            return CMSRET_INTERNAL_ERROR;
         }

         memset(&dropAlgCfg, 0x0, sizeof(tmctl_queueDropAlg_t));
   
         if (cmsUtl_strcmp(qInfo->dropAlgorithm, MDMVS_RED) == 0)
         {
            dropAlgCfg.dropAlgorithm = TMCTL_DROP_RED;
         }
         else if (cmsUtl_strcmp(qInfo->dropAlgorithm, MDMVS_WRED) == 0)
         {
            dropAlgCfg.dropAlgorithm = TMCTL_DROP_WRED;
         }
         else
         {
            dropAlgCfg.dropAlgorithm = TMCTL_DROP_DT;
         }
   
         /* Unit of RED threshold is packet */
         dropAlgCfg.dropAlgLo.redMinThreshold = (qInfo->loMinThreshold * qcfg.qsize) / 100;
         dropAlgCfg.dropAlgLo.redMaxThreshold = (qInfo->loMaxThreshold * qcfg.qsize) / 100;
         dropAlgCfg.dropAlgHi.redMinThreshold = (qInfo->hiMinThreshold * qcfg.qsize) / 100;
         dropAlgCfg.dropAlgHi.redMaxThreshold = (qInfo->hiMaxThreshold * qcfg.qsize) / 100;
         dropAlgCfg.dropAlgLo.redPercentage = dropAlgCfg.dropAlgHi.redPercentage = 100;
         /* FIXME!! prio_mask? default to 0 now */
         dropAlgCfg.priorityMask0 = dropAlgCfg.priorityMask1 = 0;

         if (rutcwmp_tmctl_setQueueDropAlg(devType, &intf, qInfo->queueId - 1,
                                           &dropAlgCfg) == TMCTL_ERROR)
         {
            cmsLog_error("rutcwmp_tmctl_setQueueDropAlgExt returns error");
            return CMSRET_INTERNAL_ERROR;
         }
      ...
      {
         /* Delete queue */
         if (rutcwmp_tmctl_delQueueCfg(devType, &intf, qcfg.qid) == TMCTL_ERROR)
         {
            cmsLog_error("Add TM queue. ifname=%s qid=%d priority=%d schedMode=%d qWt=%d shapingRate=%d bestEffort=%d",
                         qInfo->intfName, qInfo->queueId - 1, priority, schedMode, qInfo->queueWeight, qInfo->shapingRate, bestEffort ? 1 : 0);
            return CMSRET_INTERNAL_ERROR;
         }
      }
   }
...
void rutQos_tmQueueConfig_pre(const char* l2ifname)
{
   /*
    1, Make sure no new flow is learned
      ebtables --concurrent -t nat -I POSTROUTING -o eth0+ -j SKIPLOG
      iptables -w -t mangle -I POSTROUTING -o eth0+ -j SKIPLOG
    2, Flush all existed flows
      fc flush --if eth0
   */
   char cmd[BUFLEN_256];

   if (!rutQos_tmIsOwnerFE())
      return;

   snprintf(cmd, sizeof(cmd), "ebtables --concurrent -t nat -I POSTROUTING -o %s+ -j SKIPLOG", l2ifname);
   rut_doSystemAction("rutQos_tmQueueConfig_pre", cmd);

   rutQos_flushFlows(l2ifname);

   return;
}

void rutQos_tmQueueConfig_post(const char* l2ifname)
{
   /*
    ebtables --concurrent -t nat -D POSTROUTING -o eth0+ -j SKIPLOG
    iptables -w -t mangle -D POSTROUTING -o eth0+ -j SKIPLOG
   */
   char cmd[BUFLEN_256];
   ...
   snprintf(cmd, sizeof(cmd), "ebtables --concurrent -t nat -D POSTROUTING -o %s+ -j SKIPLOG", l2ifname);
   rut_doSystemAction("rutQos_tmQueueConfig_post", cmd);
...
void rutQos_tmQueueConfig_pre(const char* l2ifname __attribute__((unused)))
{
   cmsLog_debug("Can not do tmQueueConfigPre if TMCTL is not supported.");
}

void rutQos_tmQueueConfig_post(const char* l2ifname __attribute__((unused)))
{
   cmsLog_debug("Can not do tmQueueConfigPost if TMCTL is not supported.");
}
