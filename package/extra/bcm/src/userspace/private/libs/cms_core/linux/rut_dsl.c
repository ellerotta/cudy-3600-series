...
void xdslUtil_CfgProfileInit(adslCfgProfile * pAdslCfg,  WanDslIntfCfgObject * pDlIntfCfg)
{
    ...
    if( dslCfgParam & kDslCfgModVdsl2Only ){
        ...
        if(pDlIntfCfg->X_BROADCOM_COM_VDSL_35b)
...
#ifdef CONFIG_VDSLBRCMPRIV1_SUPPORT
    pDlIntfCfg->X_BROADCOM_COM_VDSL_35b = ((pAdslCfg->vdslParam & kVdslProfile35b) != 0);
#endif
#ifdef CONFIG_VDSLBRCMPRIV2_SUPPORT
      ...
      if (NULL != fs) {
         if(fgets(line, 64, fs) != NULL) {
            ...
            if((afeIds[0] != 0) && (afeIds[1] != 0)) {
               ret = xdslCtl_DiagProcessDbgCommand(0,DIAG_DEBUG_CMD_OVERRIDE_2ND_AFEIDS,0,afeIds[0],afeIds[1]);
...
*/
    snprintf(cmdLine, sizeof(cmdLine), "nodetach plugin /lib/pppoatm.so %s.%d.%d.%d linkname %s user %s password %s %s %s nodeflate",
      l2IfName, portId, vpi, vci, newObj->X_BROADCOM_COM_IfName,
...
CmsRet rutWan_clearAdslTotalStats(UINT32 lineId)
{
   ...
   if ((ret = (CmsRet)xdslCtl_ResetStatCounters(lineId)) != CMSRET_SUCCESS)
   {
         ...
         if ( (kVdslModVdsl2 != adslMib.adslConnection.modType)
...

            {
               ...
               if (data != NULL)
               {
                  ...
                  xdslCtl_GetObjectValue(0, oidStr2, sizeof(oidStr2), (char *)data, &dataLen);
                  ret = cmsAdsl_formatSnrmUsString(&usNegBandPlanDiscPresentation,
                                                   data,
                  ...
                  memset(data, 0, dataLen);
                  xdslCtl_GetObjectValue(0, oidStr2, sizeof(oidStr2), (char *)data, &dataLen);
                  ret = cmsAdsl_formatSnrmDsString(&dsNegBandPlanDiscPresentation,
                                                   data,
                                                   dataStr, sizeof(dataStr));
                  if (ret == CMSRET_SUCCESS)
                  {
               ...
               {
                  cmsLog_error("failed to allocate % bytes",
...
#if defined(DMP_DSLDIAGNOSTICS_1) && !defined(DMP_DEVICE2_DSLDIAGNOSTICS_1)
                               ...
                               int lineId __attribute__((unused)))
{
  ...
  ret = xdslCtl_GetObjectValue(lineId, NULL, 0, (char *) &adslMib, &adslMibSize);
  if (ret == CMSRET_SUCCESS)
  {
    if (adslMib.adslConnection.modType == kVdslModVdsl2)
    {
...
CmsRet rutWan_getSeltStatus(void *obj)
{
   ...
   ret = (CmsRet) xdslCtl_isSeltTestComplete(seltCfgObj->lineNumber, &testCompleted);
