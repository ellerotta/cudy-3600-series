                              ...
                              UBOOL8 isIpObj __attribute__((unused)))
{
   ...
   if ( isIpObj )
   {
      ...
      NPTv6Enabled = currObj->X_BROADCOM_COM_Dhcp6cForNPTv6Prefix;
   }
   else
   {
   ...
   if (NPTv6Enabled)
   {
      ret = rutWan_updateNptv6Rule(pObj, isIpObj, FALSE);
      return ret;
   }
   ...
   if (wanPrefix)
   {
      ...
      rutWan_updateRuleForAllNptv6(wanPrefix, FALSE);
   }
   ...
   if (wanPrefix)
   {
      ...
      {
         if (0 > snprintf(prefix, CMS_IPADDR_LENGTH, "%s/64", addr))
         {
            cmsLog_error("snprintf failed to write '%s/64' into prefix", addr);
            prefix[0] = 0;
         }
...
void rutWan_updateRuleForAllNptv6(char *prefix, UBOOL8 isAdd)
{
   InstanceIdStack iidStack = EMPTY_INSTANCE_ID_STACK;
   _WanIpConnObject *ipConnObj=NULL;
   _WanPppConnObject *pppConnObj=NULL;
   
   while (cmsObj_getNextFlags(MDMOID_WAN_IP_CONN, &iidStack, OGF_NO_VALUE_UPDATE, (void **) &ipConnObj) == CMSRET_SUCCESS)
   {
      if ((ipConnObj->X_BROADCOM_COM_Dhcp6cForNPTv6Prefix) &&
          !IS_EMPTY_STRING(ipConnObj->X_BROADCOM_COM_IPv6SitePrefix) &&
          !cmsUtl_strcmp(ipConnObj->X_BROADCOM_COM_IPv6ConnStatus, MDMVS_CONNECTED))
      {
         rutIpt_updateNPTv6Rules(isAdd, ipConnObj->X_BROADCOM_COM_IfName, prefix, ipConnObj->X_BROADCOM_COM_IPv6SitePrefix);
      }
   ...
   while (cmsObj_getNextFlags(MDMOID_WAN_PPP_CONN, &iidStack, OGF_NO_VALUE_UPDATE, (void **) &pppConnObj) == CMSRET_SUCCESS)
   {
      if ((pppConnObj->X_BROADCOM_COM_Dhcp6cForNPTv6Prefix) &&
          !IS_EMPTY_STRING(pppConnObj->X_BROADCOM_COM_IPv6SitePrefix) &&
          !cmsUtl_strcmp(pppConnObj->X_BROADCOM_COM_IPv6ConnStatus, MDMVS_CONNECTED))
      {
          rutIpt_updateNPTv6Rules(isAdd, pppConnObj->X_BROADCOM_COM_IfName, prefix, pppConnObj->X_BROADCOM_COM_IPv6SitePrefix);
      }
...
CmsRet rutWan_updateNptv6Rule(const void *pObj __attribute__((unused)),
                              UBOOL8 isIpObj, UBOOL8 isAdd)
{
   CmsRet ret     = CMSRET_SUCCESS;
   char *ifName   = NULL;
   char *addr     = NULL;
   ...
   if (isIpObj)
   {
      ...
      sitePrefix = currObj->X_BROADCOM_COM_IPv6SitePrefix;
   ...
   while ((ret = cmsObj_getNextFlags(MDMOID_PREFIX_INFO, &iidStackPrefix, OGF_NO_VALUE_UPDATE, (void **)&prefixObj)) == CMSRET_SUCCESS)
   {
      if (!cmsUtl_strcmp(prefixObj->mode, MDMVS_WANDELEGATED) &&
           cmsUtl_strcmp(prefixObj->delegatedConnection, ifName))
      {
         if (rut_isWanInterfaceUp(prefixObj->delegatedConnection, FALSE))
         {
            rutIpt_updateNPTv6Rules(isAdd, ifName, prefixObj->prefix, sitePrefix);
         }
      }
                                           ...
                                           UBOOL8 isIpObj __attribute__((unused)))
{
   ...
   cmsLog_debug("Enter: ifname=%s, ipv6Address=%s \n firewall=%d, pdflag=%d, unnumbered=%d, iana=%d, mld=%d", 
     ...
     if (NPTv6Enabled)
     {
        ret = rutWan_updateNptv6Rule(pObj, isIpObj, TRUE);
        return ret;
     }
...
//   {
      ...
      if (!IS_EMPTY_STRING(addr) || unnumbered || !dhcp6cIana)
      {
         ...
         if (MFlag && !LFlag)
         {
            ...
            char           wanPrefixAddr[CMS_IPADDR_LENGTH + BUFLEN_8] = {0};
      ...
      if (pdEnabled)
      {
         ...
         if (wanPrefix)
         {
            ...
            {
               if (0 > snprintf(prefix, CMS_IPADDR_LENGTH, "%s/64", address))
               {
                  cmsLog_error("snprintf failed to write '%s/64' into prefix", address);
                  prefix[0] = 0;
               }
            ...
            rutWan_updateRuleForAllNptv6(wanPrefix, TRUE);
         }
      ...
      if (isDnsWan)
      {
         ...
         {
            if (cmsUtl_strcmp(radvdOtherObj->recursiveDns, dnsServers) ||
                 cmsUtl_strcmp(radvdOtherObj->dnssSearchList, domainName))
            {
                            ...
                            UBOOL8 pdEnabled, UBOOL8 aftrName, UBOOL8 mapt, UBOOL8 mape)
{
   ...
   char iapdAssoc[BUFLEN_128];
   FILE *fp;

   const char *dhcp6cConf = DHCP6C_CONF_TEMPLATE;
   const char *iapd = DHCP6C_IAPD;
   const char *iana = DHCP6C_IANA;
   const char *ianaAssoc = DHCP6C_IANA_ASSOC;
   const char *aftrReq = "request aftr;";
   const char *maptReq = "request mapt;";
   const char *mapeReq = "request mape;";

   /* pass cdrouter v6_cpe_1_7 */
   sprintf(iapdAssoc, DHCP6C_IAPD_ASSOC, DHCP6C_PREFIX_LEN_HINT);
...
//   rutIpt_setupFirewallForDHCPv6(TRUE, ifName);
   ...
   {
      rutDhcp6_createOption17(WAN_PKTCBL_EMTA, ifName);
   ...
   {
      rutDhcp6_createOption17(WAN_EPON_EPTA, ifName);
      snprintf(&cmdLine[strlen(cmdLine)], sizeof(cmdLine), " -P");
   ...
   snprintf(&cmdLine[strlen(cmdLine)], sizeof(cmdLine)-1, " %s", ifName_info);
...
CmsRet rutWan_stopDhcp6c(const char *ifName, UINT32 pid)
{
   ...
   ret = remove(dhcp6cConfFile);
   if (ret != 0)
   {
      cmsLog_error("unable to remove dhcp6cConfFile");
   }
//   rutIpt_setupFirewallForDHCPv6(FALSE, ifName);
   return CMSRET_SUCCESS;
