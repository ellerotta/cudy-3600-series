...
static void rutIP_configPrefixBridgeRule(const char *ifname, const char *addr, UBOOL8 add)
{
   char bridgeName[BUFLEN_128];
   char prefixAddr[BUFLEN_128];
   char prefix[BUFLEN_256];
   UINT32 plen;
   
   memset(bridgeName, 0, BUFLEN_128);
   qdmIntf_getBridgeNameByIntfName_dev2(ifname, bridgeName);
   
   if (!IS_EMPTY_STRING(bridgeName))
   {
       if (cmsUtl_parsePrefixAddress(addr, prefixAddr, &plen) != CMSRET_SUCCESS)
       {
       ...
       cmsNet_subnetIp6SitePrefix(addr, 0, plen, prefixAddr);
       sprintf(prefix, "%s/%d", prefixAddr, plen);
       rutIpt_configRoutingChain6(prefix, bridgeName, add);
       rutEbt_configIPv6Prefix(prefix, add);
   }
}
                             ...
                             const char *prefix  __attribute__((unused)))
{
   ...
   if (cmsUtl_isValidIpAddress(AF_INET6, addr) == TRUE)
   {
      ...
      snprintf(cmdStr, sizeof(cmdStr), "ip -6 addr add %s/64 dev %s", 
               addr, ifname);
      rutIP_configPrefixBridgeRule(ifname, addr, TRUE);
                                      ...
                                      const char *ipIntfName)
{
   ...
   {
      /* XXX fill in */
      rutIP_configPrefixBridgeRule(ipIntfName, ipv6AddrObj->IPAddress, FALSE);
      cmsObj_free((void **) &ipv6AddrObj);
   }
...
void rutIp_unconfigureIpv6Addr(const char *ipIntfName, const char *addr)
{
   ...
   rutIP_configPrefixBridgeRule(ipIntfName, addr, FALSE);
   rut_doSystemAction("rut2_ip6", cmdStr);
