 ...
 *
 */
CmsRet rutWan_moveEthLanToWan(const char *ifName);
 ...
 *
 */
CmsRet rutWan_moveEthWanToLan(const char *ifName);
...
/** check if the WAN interface is a ppp on demand connection.
 *
 * @param ifName     (IN) the inteface name of the WAN connection
 *
 * @return  TRUE     if this WAN interface is a ppp on demand.
 */
 ...
 * @param L2IfName         (IN) the input interface name (eg. ptm0)
 * @param baseL3IfName     (IN) the input interface name (eg. ptm0.1)
 * @param enablePppoErelay (IN) if TRUE, start pppoe-relay on this interace.
 * @param vlanMuxId        (IN) vlanMuxId (for pppoe-relay usage)
 * @param vlanMuxPr        (IN) vlanMuxPr (for pppoe-relay usage)
 * @param vlanTpid         (IN) vlanTpid (for pppoe-relay usage)
...
/** Start the pppoe-relay for pppoe server on baseL3IfName and pppoe client on br0.
 *»
