                        ...
                        SINT32 classRate)
{
   ...
   if (cmdType == QOS_COMMAND_CONFIG)
   {
      ...
      snprintf(cmd, sizeof(cmd), "tc qdisc add dev %s root handle 100: htb 2>/dev/null", queueIntfName);
      rut_doSystemAction("rutQos_rateLimit", cmd);

      /* add the individual rate limiter */
      snprintf(cmd, sizeof(cmd), "tc class add dev %s parent 100: classid 100:%d htb rate %dkbit ceil %dkbit 2>/dev/null",
              queueIntfName, clsKey, classRate, classRate);
      rut_doSystemAction("rutQos_rateLimit", cmd);
      snprintf(cmd, sizeof(cmd), "tc qdisc add dev %s parent 100:%d handle %d: pfifo limit 10 2>/dev/null",
              queueIntfName, clsKey, 100+clsKey);
      rut_doSystemAction("rutQos_rateLimit", cmd);
      snprintf(cmd, sizeof(cmd), "tc filter add dev %s parent 100: protocol 0x%x prio 1 handle 0x%x/0x%x fw classid 100:%d 2>/dev/null",
   ...
   else
   {
      ...
      snprintf(cmd, sizeof(cmd), "tc filter del dev %s parent 100: protocol 0x%x prio 1 handle 0x%x/0x%x fw classid 100:%d 2>/dev/null",
              queueIntfName, ETH_P_ALL, clsKeyMark, SKBMARK_FLOW_ID_M, clsKey);
      rut_doSystemAction("rutQos_rateLimit", cmd);
      snprintf(cmd, sizeof(cmd), "tc qdisc del dev %s parent 100:%d handle %d: pfifo limit 10 2>/dev/null",
              queueIntfName, clsKey, 100+clsKey);
      rut_doSystemAction("rutQos_rateLimit", cmd);
      snprintf(cmd, sizeof(cmd), "tc class del dev %s parent 100: classid 100:%d htb rate %dkbit ceil %dkbit 2>/dev/null",
              queueIntfName, clsKey, classRate, classRate);
      rut_doSystemAction("rutQos_rateLimit", cmd);
      ...
      {
         snprintf(cmd, sizeof(cmd), "tc qdisc del dev %s root handle 100: htb 2>/dev/null", queueIntfName);
         rut_doSystemAction("rutQos_rateLimit", cmd);
      }
   }
