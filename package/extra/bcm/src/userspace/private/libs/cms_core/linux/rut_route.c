...
CmsRet rutRt_configActiveDefaultGateway(const char *gwIfName)
{
   ...
   snprintf(cmd, sizeof(cmd), "route del default dev %s 2>/dev/null", gwIfName);
...
CmsRet rutRt_doSystemDefaultGateway(UBOOL8 isIPv4)
{
   ...
   if ( isIPv4 )
   {
      ...
      /* 3 cases to change the system default gateway: 
      *
      ...
      *
      * 3). When the current active default gateway is same as actual system defaultGateway but if it is a ppp on demand connection
      *     where the external Ip Address of the ppp might be changed and the same default gateway need to be removed and then
      *     add back on.
      */
      if ((cmsUtl_strcmp(L3ForwadingObj->X_BROADCOM_COM_ActiveDefaultGateway, defaultGateway)) || /* case 1 */
          ...
          (rutWan_isPppOnDemand(defaultGateway)))                                                 /* case 3 */   
      {
                                  ...
                                  SINT32 forwardingMetric)
{
   ...
   if (cmsUtl_isValidIpAddress(AF_INET, destIPAddress) == FALSE)
   {
      cmsLog_notice("Invalid destination IP address");
      return CMSRET_INVALID_ARGUMENTS;		
   }
   
   if (cmsUtl_isValidIpAddress(AF_INET, destSubnetMask) == FALSE)
   {
   ...
   if ( cmsUtl_strncmp(interface, "ppp", 3) &&  cmsUtl_strncmp(interface, "ipoa", 4) &&
        ...
        cmsUtl_strncmp(interface, GRE_IFC_STR, strlen(GRE_IFC_STR)) &&
#endif
...
#ifdef SUPPORT_GRE_TUNNEL_TR181
       ((strncmp(interface, GRE_IFC_STR, strlen(GRE_IFC_STR)) == 0)
        && rut_isGreTunnelInterfaceUpLocked(interface)) ||
#endif
       qdmIpIntf_isWanInterfaceUpLocked(interface, TRUE))
   {
      if ( forwardingMetric == -1 )
      {
         snprintf(cmd, sizeof(cmd), "route add -net %s netmask %s ", destIPAddress, destSubnetMask);
      }
      else
      {
         snprintf(cmd, sizeof(cmd), "route add -net %s netmask %s metric %d", destIPAddress, destSubnetMask, forwardingMetric);
      }
      ...
      {
         strcat(cmd, " gw ");
         strncat(cmd, gatewayIPAddress, sizeof(cmd)-1);
         cmd[sizeof(cmd)-1] = '\0';         
      }
      if ( !IS_EMPTY_STRING(interface)) 
      {
         strcat(cmd, " dev ");
         strncat(cmd, interface, sizeof(cmd)-1);
         cmd[sizeof(cmd)-1] = '\0';          
      }
                                     ...
                                     const char *interface)
{
   ...
   snprintf(cmd, sizeof(cmd), "route del -net %s netmask %s ", destIPAddress, destSubnetMask);
   if ( cmsUtl_isValidIpAddress(AF_INET, gatewayIPAddress) == TRUE &&
      cmsUtl_strcmp(gatewayIPAddress, "0.0.0.0") != 0 ) 
   {
      strcat(cmd, " gw ");
                              ...
                              const char *outIfName, SINT16 idx, UBOOL8 add )
{
   ...
   char wan_netmask[BUFLEN_32]={0};
   UBOOL8 subnet_found = TRUE;
   char num[BUFLEN_16];
   UINT32 flowId = 0;
   ...
   if (add)
      ...
      snprintf(eb_cmd, sizeof( eb_cmd), "ebtables --concurrent -t broute -I BROUTING 1 ");
   else
      snprintf(eb_cmd, sizeof( eb_cmd), "ebtables --concurrent -t broute -D BROUTING ");
   ...
   if (!rut_getIfSubnet(outIfName, wan_subnet))
   {
      cmsLog_debug("%s: No subnet info of outIfName<%s>\n", __FUNCTION__, outIfName);
      subnet_found = FALSE;
   }
   
   if (!rut_getIfMask(outIfName, wan_netmask))
   {
      cmsLog_debug("%s: No netmask info of outIfName<%s>\n", __FUNCTION__, outIfName);
      subnet_found = FALSE;
   }
   ...
   if(add)
   {
       rut_configIpv4RpFilter(outIfName, 0); // 0: disabled
   }
                                        ...
                                        char *outIfName)
{
   ...
   cmsUtl_strncpy(ruleName, &prefixedRuleName[strlen(POLICY_ROUTING_PREFIX)], sizeof(ruleName));
   ...
   */
   while((entryCt  < RT_TABLE_MAX)  && fgets(line, sizeof(line), fs))
   {
      ...
      else
      {
         ...
         cmsUtl_strncpy(entryToAddBackBuf[entryCt], line, BUFLEN_32);
         entryCt++;
      }         
   }
