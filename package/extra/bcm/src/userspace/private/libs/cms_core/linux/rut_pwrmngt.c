...
#if defined(SUPPORT_HOSTCPU_PWRSAVE)
   char speed[16];
   snprintf(speed, sizeof(speed), "%u", pwrMngtObj->CPUSpeed);
   if ((ret = pwr_api_cpu_speed_set(speed, 1)))
      return ret;
#endif

#if defined(SUPPORT_DDR_SELF_REFRESH_PWRSAVE)
   if ((ret = pwr_api_dram_sr_set(NULL, pwrMngtObj->DRAMSelfRefreshEn)))
      return ret;
#endif

   if ((ret = pwr_api_cpu_wait_set(NULL, pwrMngtObj->CPUr4kWaitEn)))
      return ret;
