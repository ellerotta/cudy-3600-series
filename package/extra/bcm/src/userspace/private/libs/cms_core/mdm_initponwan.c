...
CmsRet mdm_addDefaultWanGponObjects(void)
{
   ...
   {
      cmsLog_error("Could not add Gpon WANDevice at %s, ret=%d", cmsMdm_dumpIidStack(&pathDesc.iidStack), ret);
      return ret;
   }
...
CmsRet mdm_addDefaultWanEponObject(void)
{
   ...
   CMSMEM_REPLACE_STRING_FLAGS(eponLinkCfg->connectionMode, MDMVS_VLANMUXMODE, mdmLibCtx.allocFlags); 
