      ...
      char *sourceIntfNames, UINT32 sourceIntfNamesLen __attribute__((unused)))
{
                                    ...
                                    (void **) &ipIntfObj) == CMSRET_SUCCESS)
   {
      ...
      if (!isMld)
      {
         if (ipIntfObj->X_BROADCOM_COM_IGMPEnabled && ipIntfObj->name)
         {
         ...
         if (ipIntfObj->X_BROADCOM_COM_IGMP_SOURCEEnabled && ipIntfObj->name)
         {
      ...
      if (isMld)
      {
         if (ipIntfObj->X_BROADCOM_COM_MLDEnabled && ipIntfObj->name)
         {
         ...
         if (ipIntfObj->X_BROADCOM_COM_MLD_SOURCEEnabled && ipIntfObj->name)
         {
...
void rutMulti_createWanAssocList_dev2(char *strictWanAssocList, int strictWanAssocListLen)
{
   Dev2IpInterfaceObject *ipIntfObj = NULL;
   InstanceIdStack iidStack = EMPTY_INSTANCE_ID_STACK;
   CmsRet ret;
                               ...
                               (void **)&ipIntfObj)) == CMSRET_SUCCESS)
   {
      ...
      {
         /* only check the NOT upstream IP interfaces which might be bridge groups */
         cmsObj_free((void **) &ipIntfObj);
         continue;
      }
      if (0 == cmsUtl_strncmp(ipIntfObj->name, "br", 2))
      {
                               ...
                               (void **)&wanIpIntfObj) == CMSRET_SUCCESS)
         {
            if (wanIpIntfObj->X_BROADCOM_COM_Upstream)
            {
                    ...
                    !cmsUtl_strcmp(ipIntfObj->X_BROADCOM_COM_GroupName, "Default")))
               {
                  ...
                  cmsUtl_strncat(strictWanAssocList, strictWanAssocListLen, ipIntfObj->name);
                  cmsUtl_strncat(strictWanAssocList, strictWanAssocListLen, " ");
                  cmsUtl_strncat(strictWanAssocList, strictWanAssocListLen, wanIpIntfObj->name);
               }
            }
