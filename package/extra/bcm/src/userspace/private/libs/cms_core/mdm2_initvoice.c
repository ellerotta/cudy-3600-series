...
#if defined(DMP_X_BROADCOM_COM_EPONWAN_1) && defined(BRCM_PKTCBL_SUPPORT)
/*****************************************************************************
**  FUNCTION:       clearBoundIfName
**
**  PURPOSE:        Clear boundIfName in the voice object first created
**
**  INPUT PARMS:    None.
**
**  OUTPUT PARMS:   None.
...
**
*****************************************************************************/
static void clearBoundIfName(void)
{
...
CmsRet mdm_adjustForVoiceHardware(void)
{
   ...
   int maxPhysEndpt, numFxoEndpt, numFxsEndpt;
   ...
   cmsLog_debug("mdm_adjustForVoiceHardware 1: maxPhysEndpt %d, numFxoEndpt %d, numFxsEndpt %d\n",
                maxPhysEndpt, numFxoEndpt, numFxsEndpt);
   ...
   if (cmsMdm_isDataModelDevice2())
   {
      ...
      servicesObj->voiceServiceNumberOfEntries++;
      cmsLog_debug("Incremented voiceServiceNumberOfEntries");
   ...
   /* VoiceService.{i}.Capabilities.Codecs.{i}. - Will be configured by voice app */
