         ...
         UBOOL8 isWan, UBOOL8 isBridge)
{
   ...
   if (!cmsUtl_strcmp(newStatus, MDMVS_SERVICESTARTING))   /*** NEW STATE ***/
   {
      ...
      else if (!cmsUtl_strcmp(currStatus, MDMVS_SERVICEUP))
      {
          ...
          */
         if (isWan)
            bcmuLog_knotice("partially deactivating services on %s...", ifname);

         rutIpv4Service_teardown(ipIntfFullPath, ifname, isWan, isBridge);

         if (isWan)
            bcmuLog_knotice("Services on %s are partially deactivated.", ifname);
      }
      else
      {
   ...
   else if (!cmsUtl_strcmp(newStatus, MDMVS_SERVICEUP))  /*** NEW STATE ***/
   {
      if (!cmsUtl_strcmp(currStatus, MDMVS_SERVICESTARTING))
      {
          ...
          */
         if (isWan)
            bcmuLog_knotice("Activating services on %s...", ifname);

         rutIpv4Service_setup(ipIntfFullPath, ifname, isWan, isBridge);

         if (isWan)
            bcmuLog_knotice("All services on %s are activated.", ifname);
      }
      else
      {
         cmsLog_error("illegal status transition: newStatus=%s currStatus=%s",
   ...
   {
      if (isWan)
         bcmuLog_knotice("Deactivating services on %s...", ifname);
      ...
      if (isWan)
         bcmuLog_knotice("All services on %s have been deactivated.", ifname);
   }
   else
   {
                          ...
                          UBOOL8 isWan, UBOOL8 isBridge)
{
   ...
   */
   if (!isBridge)
   {
      ...
      {
         UINT32 pppIdleDisconnectTime=0;

         rutIpt_initNatAndFirewallOnWanConnection(ifname, 
                    pppIdleDisconnectTime, isNatEnabled, isFirewallEnabled);
      }
      else if(!isWan) //For LAN side firewall
      {
          ...
          {
              rutIpt_initNatAndFirewallOnWanConnection(ifname,
                         0, isNatEnabled, isFirewallEnabled);
          }else
          {
                          ...
                          UBOOL8 isWan, UBOOL8 isBridge)
{
   ...
   if (isWan)
   {
      ...
      else
      {
	    ...
	    /* Multicast traffic from a WAN service will be dropped when
             * (1) that WAN service enables firewall feature
             * (2) the default route does not exist for that WAN service
             * This can happen when there are 2 wan services created on a wan interface
             * one for unicast and the other for multicast and the default route is set
             * for the unicast wan interface.
             *
             * The reason of blocking traffic is the RP filter on the WAN service is turned on
             * due to firewall enabled. See rutIpt_activateFirewallAntiIPspoofing(),
             * the "/proc/sys/net/ipv4/conf/<wan>/rp_filter" will be set to "1".
             *
             * So we add some checks here, once hit the above two conditions and igmp source
             * is enabled also, then change the rp_filter for the WAN service from "1" (strict mode)
             * to "2" (loose mode) which should be able to prevent multicast traffic from blocking.
             */
	    if (isFirewallEnabled)
            {
               char actDftGwIfName[CMS_IFNAME_LENGTH] = {0};

               if (qdmRt_getActiveDefaultGatewayLocked(actDftGwIfName) == CMSRET_SUCCESS && 
                   cmsUtl_strcmp(ifname, actDftGwIfName))
               {
                  rut_configIpv4RpFilter(ifname, 2); // 2: Loose mode
               }
            }
         }
         
         /* IgmpProxyIntfList must be updated for mcpd.conf */
...
#ifdef SUPPORT_VXLAN_TUNNEL_TR181
      /* no gateway config on VxLan IP interface */
                         ...
                         UBOOL8 isWan, UBOOL8 isBridge)
{
   ...
   {
      if( (rutOpenVS_isEnabled()) && (rutOpenVS_isOpenVSPorts(ifname)))
         rutOpenVS_shutdownOpenVSport(ifname, OVS_BRIDGE);
   }
#endif
