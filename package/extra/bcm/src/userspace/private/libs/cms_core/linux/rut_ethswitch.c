...
static void rut_pmapCreateDataDirectory(UINT16 lanId) {
    ...
    snprintf(file, sizeof(file), "%s/%s", LAN_DATA_PATH, interface);
    if (!access(file, F_OK))
        return;
    ...
    // init
    snprintf(file, sizeof(file), "%s/%s/linkstate", LAN_DATA_PATH, interface);
    snprintf(cmd, sizeof(cmd), "echo 00 > %s", file);
    rut_doSystemAction("rut_pmapCreateDataDirectory", cmd);
}
...
UINT32 rutEsw_getNumberOfVirtualPorts(const char *interfaceName)
{
   ...
   snprintf(cmd, sizeof(cmd), "ethctl %s vport query 2>/var/vcfgerr\n", interfaceName);
...
void rutEsw_updateRealHwSwitchingSetting(UINT32 excludeClassKey __attribute__((unused)))
{
#ifdef SUPPORT_CMS_ALLOW_REAL_HW_SWITCHING
#define	EOPNOTSUPP	95
   UBOOL8 allowed=FALSE;
   int switchingStatus = 0;
   int rc = -1;
   ...
   if((rc = ethswctl_get_switching_status(&switchingStatus)))
   {
      if(rc == EOPNOTSUPP)
         cmsLog_debug("No switch found or hw-switching operation is not supported");      
      else 
         cmsLog_error("Could not get switching status through ethswctl.");
      return;      
   }
