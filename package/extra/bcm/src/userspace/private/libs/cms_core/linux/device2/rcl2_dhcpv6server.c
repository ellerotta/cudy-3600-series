                ...
                CmsRet *errorCode __attribute__((unused)))
{
   ...
   if (newObj != NULL)
   {
      ...
      if (!currObj)
      {
         rutUtil_modifyNumDHCPv6ServerPool(iidStack, 1);
         return ret;
      }

      ret = qdmIntf_fullPathToIntfnameLocked_dev2(newObj->interface, ifName);
      if (ret != CMSRET_SUCCESS)
      {
         cmsLog_error("qdmIntf_fullPathToIntfnameLocked_dev2 failed. ret=%d interface=%s", ret, newObj->interface);
         return ret;
      }      
      ...
      if (newObj->enable)
      {
         if (qdmIpIntf_isAssociatedWanInterfaceUpLocked_dev2(newObj->interface, CMS_AF_SELECT_IPV6))
         {
            ...
            while (!found && (cmsObj_getNextFlags(MDMOID_DEV2_DHCP6C_RCV_OPTION, &iidStack,
                          OGF_NO_VALUE_UPDATE, (void **)&obj) == CMSRET_SUCCESS))
            {
               if (!cmsUtl_strcmp(obj->prefix, prefix))
               {
                  found = TRUE;
                  domainName = cmsMem_strdup(obj->domainName);
               }

               cmsObj_free((void **)&obj);
            }

            ret = rut_restartDhcp6s(ifName,
                                    &(newObj->X_BROADCOM_COM_Pid), ipAddr, domainName,
                                    newObj->IANAEnable,
         ...
         else
         {
            ...
            {
               ret = rut_restartDhcp6s(ifName,
                                       &(newObj->X_BROADCOM_COM_Pid),
                                       NULL, NULL, newObj->IANAEnable,
                                       newObj->X_BROADCOM_COM_MinInterfaceID,
                                       newObj->X_BROADCOM_COM_MaxInterfaceID,
                                       newObj->X_BROADCOM_COM_IANALeaseTime,
                                       prefix);
               if (ret == CMSRET_SUCCESS)
               {
                  CMSMEM_REPLACE_STRING_FLAGS(newObj->status,
                                              newObj->X_BROADCOM_COM_Pid == CMS_INVALID_PID ? MDMVS_DISABLED : MDMVS_ENABLED,
                                              mdmLibCtx.allocFlags);
               }
            }
            else
