...
/** Macro to determine if an MdmObject status is changed.
 */
#define STATUS_CHANGE(n, c) \
   (((n) != NULL && (c) == NULL) || \
    ((n) != NULL && (c) != NULL && cmsUtl_strcmp((c)->status, (n)->status)))
...
/** Send a message to smd to start PPPoA
 */
CmsRet rutDsl_configPPPoA(const char *cmdLineIn, const char *baseIfName,
                          SINT32 *pppPid);
...
void rutUtil_modifyNumCdPerInterfaceResult(const InstanceIdStack *iidStack, SINT32 delta);
void rutUtil_modifyNumCdPerInterfaceResult_dev2(const InstanceIdStack *iidStack, SINT32 delta);
void rutUtil_modifyNumCdPerInterfaceResult_igd(const InstanceIdStack *iidStack, SINT32 delta);

#if defined(SUPPORT_DM_LEGACY98)
#define rutUtil_modifyNumCdPerInterfaceResult(a,b)  rutUtil_modifyNumCdPerInterfaceResult_igd((a),(b))
#elif defined(SUPPORT_DM_HYBRID)
#define rutUtil_modifyNumCdPerInterfaceResult(a,b)  rutUtil_modifyNumCdPerInterfaceResult_igd((a),(b))
#elif defined(SUPPORT_DM_PURE181)
#define rutUtil_modifyNumCdPerInterfaceResult(a,b)  rutUtil_modifyNumCdPerInterfaceResult_dev2((a),(b))
#elif defined(SUPPORT_DM_DETECT)
#define rutUtil_modifyNumCdPerInterfaceResult(a,b)  (cmsMdm_isDataModelDevice2() ? \
                                                   rutUtil_modifyNumCdPerInterfaceResult_dev2((a),(b)) : \
                                                   rutUtil_modifyNumCdPerInterfaceResult_igd((a),(b)))
#endif
...
void rutUtil_modifyNumCwmpAgentRequestEntry_dev2(const InstanceIdStack *iidStack, SINT32 delta);
...
void rutUtil_modifyNumCwmpBootParameterEntry_dev2(const InstanceIdStack *iidStack, SINT32 delta);
void rutUtil_modifyNumCwmpSubscriptionEntry_dev2(const InstanceIdStack *iidStack, SINT32 delta);
...
/** check a GRE tunnel interface up.
 *
...
/** This function takes the full path of a parameter and sets the its attribute to active notification.
 *
 * @param (IN) fullpath 
 * @return CmsRet 
 */
...
/* check if the ppp interface was existed in virtual netdev folder */
