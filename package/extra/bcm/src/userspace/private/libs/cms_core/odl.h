...
/** Start remote_objd for specified component.
 *
 * @param compName (IN) The name of component in BDK.
 * @param shmId (IN) shmId to attach to.
 * @return CmsRet enum.
 */
...
/** Set (write) an array of parameter values with flags.
 *
 ...
 * @param flags           (IN) The flags for set action.
 * @return CmsRet enum.
 */
...
/** Set a new MdmObject.
 *
 * This is used by the object layer to change an object.
 *
 ...
 * @param setFlag    (IN) The flags for set action.
 * @return CmsRet enum.
 */
CmsRet odl_setObject(const void *mdmObj, const InstanceIdStack *iidStack,
                     UINT32 setFlags);
...
/** Same as original version, but with optional args of fullpath and alias.
 *
 * @param fullpath (IN) fullpath to object and may include alias. e.g.
 ...
 *
 * The object may be remote or local.
...
// This function is for local objects only.
CmsRet odl_clearStatisticsLocal(MdmObjectId oid,
                                const InstanceIdStack *iidStack);

// The fullpath may be a remote or local object.
// The only flag supported right now is OGF_LOCAL_MDM_ONLY
