...
// Send QoS params from sysmgmt component to sys_directory to synchronize
// important global QoS settings.  Currently, the only param that needs to be
// sync'd is Enable.  (If more than 1, separate by commas).
// Wifi and DSL components will subscribe to this key and update their copies
// of these params in their local copies of the Device.QoS object.  Note that
// in a full BDK system, only the sysmgmt component should set the
// Device.Qos.Enable param.  The other components hold read only copies, which
// is sync'd via sys_directory.
static void sendQoSParamsToSysDirectory(const  _Dev2QosObject *newObj)
{
   char valueBuf[32]={0};
   snprintf(valueBuf, sizeof(valueBuf), "%d,%d",
            newObj->X_BROADCOM_COM_Enable,
            newObj->X_BROADCOM_COM_EnableStateChanged);
   rut_sendKeyValueEventToSysDirectory(BDK_KEY_DEVICE_QOS_PARAMS, valueBuf);
}
                      ...
                      CmsRet *errorCode __attribute__((unused)))
{
   ...
   if (ADD_NEW(newObj, currObj))
   {
      ...
      {
         // System startup, send params to sys_directory
         sendQoSParamsToSysDirectory(newObj);
      }
   }
    ...
    * DSCP is not configured by any remote components (e.g. DSL or Wifi), so
    * only set DSCP if BDK sysmgmt or CMS Classic.
    */

   if (newObj && currObj &&
       (newObj->defaultDSCPMark != currObj->defaultDSCPMark) &&
       (cmsMdm_isCmsClassic() || cmsMdm_isBdkSysmgmt()))
   {
   ...
   if (newObj && currObj)
   {
      // Modify
      if (cmsMdm_isBdkSysmgmt() &&
          ((newObj->X_BROADCOM_COM_Enable != currObj->X_BROADCOM_COM_Enable) ||
           (newObj->X_BROADCOM_COM_EnableStateChanged != currObj->X_BROADCOM_COM_EnableStateChanged)))
      {
         // Something important changed, send to sys_directory
         sendQoSParamsToSysDirectory(newObj);
      }
   }
                             ...
                             CmsRet *errorCode __attribute__((unused)))
{
   ...
   {
      /* copy info from newObj to pInfo struct */
      pInfo->enable = newObj->enable;
      ...
      {
          ret = rutQos_createPolicer(pInfo);
          newObj->X_BROADCOM_COM_PolicerInfo = pInfo->policerInfo;
      }
      else if (newObj && currObj && newObj->enable &&
          rutQos_isPolicerChanged_dev2(newObj, currObj))
      {
         /* Update the qos.
          * No need to update the classification.
          */
         ret = rutQos_updatePolicer(pInfo);
      }
   }
   ...
   if (DELETE_OR_DISABLE_EXISTING(newObj, currObj))
   {
      ...
      cmsLog_debug("Delete or disable the policer");
       ...
       */
      cmsLog_debug("call rutQos_deletePolicer with 0x%x", currObj->X_BROADCOM_COM_PolicerInfo);
      ret = rutQos_deletePolicer(currObj->X_BROADCOM_COM_PolicerInfo);
...
static UBOOL8 isInterfaceUpNeeded(char *intfName)
{
   if((!cmsUtl_strncmp(intfName, ATM_IFC_STR, strlen(ATM_IFC_STR))) ||
      (!cmsUtl_strncmp(intfName, PTM_IFC_STR, strlen(PTM_IFC_STR))) ||
      ((cmsUtl_strstr(intfName, "GPON")) != NULL) ||
      ((cmsUtl_strstr(intfName, "Epon")) != NULL) ||
      ((cmsUtl_strstr(intfName, "EPON")) != NULL) ||
      (!cmsUtl_strncmp(intfName, EPON_IFC_STR, strlen(EPON_IFC_STR))) ||
      (!cmsUtl_strncmp(intfName, GPON_IFC_STR, strlen(EPON_IFC_STR))))
      return TRUE;
   else
      return FALSE;
}
                           ...
                           CmsRet *errorCode __attribute__((unused)))
{
   ...
   cmsLog_debug("Entered: %s", cmsMdm_dumpIidStack(iidStack));
   ...
   if (newObj != NULL && currObj != NULL)
   {
      UBOOL8 enableStateChanged = TRUE;
      {
         ...
         ret = cmsObj_get(MDMOID_DEV2_QOS, &tmpIidStack, OGF_NO_VALUE_UPDATE, (void **)&qosObj);
         if (ret != CMSRET_SUCCESS)
         {
      ...
      if (!cmsUtl_strcmp(currObj->status, MDMVS_ENABLED))
      {
         ...
         if (((newObj->enable == FALSE && currObj->enable == TRUE) ||
              (newObj->X_BROADCOM_COM_TcpPureAckQueue == FALSE && currObj->X_BROADCOM_COM_TcpPureAckQueue == TRUE)) &&
              !enableStateChanged)
         {
            /*
            ...
            /* TCP Pure-ACK: Configure TCP Pure-ACK Netfilter rules for this L2 interface. */
            rutQos_sendTcpPureAckConfigChanged_dev2(newObj->interface);
         }
      ...
      if (newObj->enable)
      {
         ...
         char nameBuf[BUFLEN_16] = {0};
         CmsQosQueueInfo qInfo;
         UBOOL8 isItfUpNeeded = FALSE;

         /* Check if this is a XTM queue. */
         qdmIntf_getIntfnameFromFullPathLocked_dev2(newObj->interface, nameBuf, sizeof(nameBuf));
         isItfUpNeeded = isInterfaceUpNeeded(nameBuf);
         ...
         if (isItfUpNeeded)
         {
            ...
            cmsLog_notice("intf %s status=%s isItfUpNeeded=%d", newObj->interface, statusBuf, isItfUpNeeded);
         }
         /* XTM/PON queues could be configured only when the link is up. */
         if (!isItfUpNeeded || !cmsUtl_strcmp(statusBuf, MDMVS_UP))
         {
            ...
            if (((newObj->enable == TRUE && currObj->enable == FALSE) ||
                 (newObj->X_BROADCOM_COM_TcpPureAckQueue == TRUE && currObj->X_BROADCOM_COM_TcpPureAckQueue == FALSE)) &&
                 !enableStateChanged)
            {
               /*
   ...
   if (DELETE_EXISTING(newObj, currObj))
   {
      ...
      if (!cmsUtl_strcmp(currObj->status, MDMVS_ENABLED))
      {
         ...
         /* TCP Pure-ACK: Configure TCP Pure-ACK Netfilter rules for this L2 interface. */
         rutQos_sendTcpPureAckConfigChanged_dev2(currObj->interface);
      }
                            ...
                            CmsRet *errorCode __attribute__((unused)))
{
   ...
   if (newObj != NULL)
   {
      ...
      // On 63138/63148/63158, setting port shaper
