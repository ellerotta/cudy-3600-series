...
#define log_local_file_size  48    //48KB
/* Have two log files: /var/log/messages and /var/log/messages.0 */
#define rotated_log_files    1     
                          ...
                          const char *cp_user __attribute__((unused)))
{
   ...
   strncpy(tmp_file, "/var/passwdXXXXXX", sizeof(tmp_file)-1); 
   ...
   */
   ...
   snprintf(cmd, sizeof(cmd), "cp -f %s %s", tmp_file, loginPasswdFile);
   if (system(cmd) != 0)
   {
...
#ifdef SUPPORT_SGS
   fputs("avdefs:x:1002:root\n", tmp_fp);
#endif
#ifdef SUPPORT_NETAPP_RESTRICT
   fputs("netgroup::1000:netuser\n", tmp_fp);
#endif
   ...
   */
   ...
   snprintf(cmd, sizeof(cmd), "cp -f %s %s", tmp_file, loginGroupFile);
   if (system(cmd) != 0)
   {
...
CmsRet rut_restartsysklogd(const _SyslogCfgObject *syslogCfg)
{
   ...
   /* Must first stop syslogd, then remove local files */
   sprintf(args, "rm -rf %s", LOG_LOCAL_FILES);
   if (system(args) != 0)
   {
      //rm failed,won't affect anything.
      cmsLog_debug("Execute cmd: %s failed", args);
   }
   ...
   {
      snprintf(args, BUFLEN_128, "-L -l %d -s %d -b %d",   //limit size of /var/log/messages as 48KB
               cmsUtl_syslogLevelToNum(syslogCfg->localLogLevel), 
               log_local_file_size,
               rotated_log_files);
   }
   else if (!strcmp(syslogCfg->option, MDMVS_LOCAL_FILE_AND_REMOTE))
   {
      snprintf(args, BUFLEN_128, "-L -l %d -s %d -b %d -R %s:%i -r %d",
               cmsUtl_syslogLevelToNum(syslogCfg->localLogLevel),
               log_local_file_size,
               rotated_log_files,
               syslogCfg->serverIPAddress, syslogCfg->serverPortNumber, 
...
// CMS_MSG_SET_LOG_LEVEL
// CMS_MSG_SET_LOG_DESTINATION
// CMS_MSG_SET_LOG_DESTINATION_MASK
// to a single threaded (non-multi-thread aware) app.
static void sendLogConfigMsg(CmsEntityId destEid, UINT32 msgType, UINT32 data)
{
   CmsMsgHeader msg = EMPTY_MSG_HEADER;
   void *msgHandle = cmsMdm_getThreadMsgHandle();
   CmsRet ret;

   cmsLog_debug("Sending 0x%x to %d data=0x%x", msgType, destEid, data);
   ...
   ret = cmsMsg_sendAndGetReplyWithTimeout(msgHandle, &msg, CMSLCK_MAX_HOLDTIME);
   if (ret != CMSRET_SUCCESS && ret != CMSRET_MSG_BOUNCED)
   {
      cmsLog_error("send of log cfg 0x%x to %d failed, ret=%d",
                   msgType, destEid, ret);
   }
...
// Use this function to send a
// CMS_MSG_SET_LOG_LEVEL_EX
// CMS_MSG_SET_LOG_DESTINATION_MASK_EX
// to a multi-thread aware app
static void sendLogConfigMsgEx(CmsEntityId destEid, UINT32 msgType, UINT32 data)
{
   char buf[sizeof(CmsMsgHeader) + sizeof(BcmUlogExMsgBody)] = {0};
   CmsMsgHeader *msg = (CmsMsgHeader *) buf;
   BcmUlogExMsgBody *body = (BcmUlogExMsgBody *) (msg+1);
   void *msgHandle = cmsMdm_getThreadMsgHandle();
   CmsRet ret;

   cmsLog_debug("Sending 0x%x to %d from %d data=0x%x",
                 msgType, destEid, cmsMsg_getHandleEid(msgHandle), data);
   ...
   msg->flags_bounceIfNotRunning = 1;
   msg->wordData = data;
   msg->dataLength = sizeof(BcmUlogExMsgBody);
...
// Use this function to configure single threaded apps, or multi-threaded apps
// which have an app-wide log setting and uses only one thread to access
// the MDM.
void rut_updateLogLevel(CmsEntityId destEid, const char *logLevel)
{
   void *msgHandle = cmsMdm_getThreadMsgHandle();
   UINT32 myEid = (UINT32) cmsMsg_getHandleGenericEid(msgHandle);

   cmsLog_debug("destEid=%d (myEid=%d) logLevel=%s",
                destEid, myEid, logLevel);
   ...
   {
      UINT32 data = cmsUtl_logLevelStringToEnum(logLevel);
      sendLogConfigMsg(destEid, CMS_MSG_SET_LOG_LEVEL, data);
   }
}

// Use this function to configure the log settings of multi-threaded apps
// which have configured their logs for multi-threads.  Note that in this 
// context, "multi-threads" means different log levels for different threads,
// but has nothing to do with a multi-threaded app which has 
// multiple msgHandle's.  In fact, the callers of this function only passes
// a generic EID into this function via destEid.  The callers do not know about
// the destination thread id's, which is needed to create a specificEid.
void rut_updateLogLevelEx(CmsEntityId destEid, const char *logLevel)
{
   UINT32 data;
   void *msgHandle = cmsMdm_getThreadMsgHandle();
   UINT32 myEid = (UINT32) cmsMsg_getHandleGenericEid(msgHandle);
   ...
   data = cmsUtl_logLevelStringToEnum(logLevel);

   if (destEid == myEid)
   {
      // don't send msg to myself, just set my thread log level directly.
      // CMS log is actually implemented by BCM uLog, and this new API is
      // only available from BCM uLog.
      SINT32 threadId = sysUtl_getThreadId();
      bcmuLog_setLevelEx(data, threadId, destEid);
   }
   else
   {
      sendLogConfigMsgEx(destEid, CMS_MSG_SET_LOG_LEVEL_EX, data);
   }
}
...
void rut_updateLogDestination(CmsEntityId destEid, const char *logDest)
{
   ...
   cmsLog_debug("destEid=%d (myEid=%d) new log dest=%s",
                destEid, myEid, logDest);
   ...
   {
      UINT32 data = cmsUtl_logDestinationStringToEnum(logDest);
      sendLogConfigMsg(destEid, CMS_MSG_SET_LOG_DESTINATION, data);
   }
}
...
// which have an app-wide log setting and uses only one thread to access
// the MDM.
void rut_updateLogDestinationMask(CmsEntityId destEid, UINT32 logDestMask)
{
   void *msgHandle = cmsMdm_getThreadMsgHandle();
   UINT32 myEid = (UINT32) cmsMsg_getHandleGenericEid(msgHandle);

   cmsLog_debug("destEid=%d (myEid=%d) new log dest mask=0x%x",
                destEid, myEid, logDestMask);

   if (destEid == myEid)
   {
      /* don't send a message to myself, just set log dest directly */
      bcmuLog_setDestMask(logDestMask);
   }
   else if (destEid == EID_SSHD || destEid == EID_TELNETD || destEid == EID_CONSOLED)
   {
   ...
   {
      sendLogConfigMsg(destEid, CMS_MSG_SET_LOG_DESTINATION_MASK, logDestMask);
   }
}

// Use this function to configure the log settings of multi-threaded apps
// which have configured their logs for multi-threads.  See comments in
// rut_updateLevelEx.
void rut_updateLogDestinationMaskEx(CmsEntityId destEid, UINT32 logDestMask)
{
   void *msgHandle = cmsMdm_getThreadMsgHandle();
   UINT32 myEid = (UINT32) cmsMsg_getHandleGenericEid(msgHandle);
   ...
   if (destEid == myEid)
   {
      // don't send msg to myself, just set my per thread dest mask directly.
      // CMS log is actually implemented by BCM uLog, and this new API is
      // only available from BCM uLog.
      SINT32 threadId = sysUtl_getThreadId();
      bcmuLog_setDestMaskEx(logDestMask, threadId, destEid);
   }
   else
   {
      sendLogConfigMsgEx(destEid, CMS_MSG_SET_LOG_DESTINATION_MASK_EX, logDestMask);
   }
}


/** Send a changed notification message.
 */
void rut_sendChangeNotification(void)
{
   CmsMsgHeader msg;
   void *msgHandle = cmsMdm_getThreadMsgHandle();
   CmsRet ret;
   ...
   if (ret != CMSRET_SUCCESS)
   {
      cmsLog_error("send of ACTIVE_NOTIFICATION msg failed, ret=%d", ret);
   }
   else
   {
      cmsLog_debug("ACTIVE_NOTIFICATION msg sent");
   }
   return;
}

/** Send a alt change notification message
 */
void rut_sendAltChangeNotification(void)
{
   CmsMsgHeader msg;
   void *msgHandle = cmsMdm_getThreadMsgHandle();
   CmsRet ret;
   ...
   memset(&msg, 0, sizeof(CmsMsgHeader));
   msg.type = CMS_MSG_ALT_NOTIFICATION;
   msg.src = cmsMsg_getHandleEid(msgHandle);
   msg.dst = EID_SMD;
   msg.flags_event = 1;
   ...
   if (ret != CMSRET_SUCCESS)
   {
      cmsLog_error("send of ALT_NOTIFICATION msg failed, ret=%d", ret);
   }
   else
   {
      cmsLog_debug("ALT_NOTIFICATION msg sent");
   }
   return;
}

UBOOL8 rut_isValueChangeNotificationEid()
{
   void *msgHandle = cmsMdm_getThreadMsgHandle();
   CmsEntityId myEid = cmsMsg_getHandleGenericEid(msgHandle);
   /*
    * Don't set the valueChanged bit and don't send a notification
    * message if TR69c or TR69c_2 is the one doing the changes.
    */
   //TODO: do we need to check the eid of apibdk_sd?
   if ((myEid == EID_TR69C) ||
       (myEid == EID_TR69C_2))
   {
      return FALSE;
...
void rut_clearIntfStats(const char *devname)
{
   ...
   if ( (socketFd = socket(AF_INET, SOCK_DGRAM, 0)) >= 0) 
   {
      ...
      err = ioctl(socketFd, SIOCSCLEARMIBCNTR, &intf);
      if(err != 0)
      {
         cmsLog_error("ioctl(SIOCSCLEARMIBCNTR) fail!");
      }
      close(socketFd);
   }
   else
