...
{
    void *objVoice = NULL, *objLine = NULL;
    UBOOL8 found = FALSE;
    ...
    ret = cmsObj_getNext(MDMOID_VOICE, &iidVoice, (void **)&objVoice);
    ...
    (const char *ifName)
{
    ...
    debugPrint("===> setVoipBoundIfNameAddress, ifName=%s, ret=%d\n", ifName, ret);
...
CmsRet SendUploadComplete(void)
{
  ...
  /* Trigger send upload complete in voice */
  {
     ...
     cmsObj_getNextFlags(MDMOID_VOICE_MSG_REQ, &iidStack, OGF_NO_VALUE_UPDATE,
                         (void **) &pObj);

     if (pObj)
     {
        pObj->X_BROADCOM_COM_OmciCfgUpldComplete = TRUE;
        if ((ret = cmsObj_set(pObj, &iidStack)) != CMSRET_SUCCESS)
        {
            cmsLog_error("Can't set voice object, ret = %d\n", ret);
        }
...
#ifdef DMP_X_ITU_ORG_VOICE_SIP_1
static UINT32 Form32BitValueFromByteArray(UINT8* charArrayPtr, int size)
{
  // Setup 32-bit value from byte array which may not be word-aligned.
  if(size == 1)
     return (charArrayPtr[0] << 24);
  else if(size == 2)
     return ((charArrayPtr[0] << 24) | (charArrayPtr[1] << 16));
  else
...
static CmsRet GetPotsPortStatus(int portIndex, UINT32* resultPtr)
{
  ...
  VoiceObject* voiceObjPtr = NULL;
#ifdef DMP_VOICE_SERVICE_1
  VoiceLineObject* voiceLineObjPtr = NULL;
  UBOOL8 stateFound = FALSE;
#else
  SipClientObject* voiceLineObjPtr = NULL;
#endif
  ...
  if (cmsObj_getNext(MDMOID_VOICE, &iidVoice, (void **)&voiceObjPtr) == CMSRET_SUCCESS)
  {
#ifdef DMP_VOICE_SERVICE_1
    // Get MDMOID_VOICE_LINE object.
...
CmsRet SetSipConfig(int portIndex, sipConfigRec* recPtr)
{
  ...
  // Setup 32-bit value from byte array which may not be word-aligned.
  temp32 = Form32BitValueFromByteArray(recPtr->sipProxyComPort, sizeof(recPtr->sipProxyComPort));
  ...
  if (SetupSipProxyServerAddress(portIndex, tempStr, temp32) == CMSRET_SUCCESS)
  {
    ...
    // Setup 32-bit value from byte array which may not be word-aligned.
    temp32 = Form32BitValueFromByteArray(recPtr->sipRegistrarComPort, sizeof(recPtr->sipRegistrarComPort));
    ...
    if (SetupSipRegistrarAddress(portIndex, tempStr, temp32) == CMSRET_SUCCESS)
    {
      ...
      // Setup 32-bit value from byte array which may not be word-aligned.
      temp32 = Form32BitValueFromByteArray(recPtr->outboundServerComPort, sizeof(recPtr->outboundServerComPort));

      // Configure Outbound server and port.
      if (SetupSipOutboundProxyAddress(portIndex, tempStr, temp32) == CMSRET_SUCCESS)
      {
        // Setup 32-bit value from byte array which may not be word-aligned.
        temp32 = Form32BitValueFromByteArray(recPtr->regRefreshCycle, sizeof(recPtr->regRefreshCycle));
...
CmsRet SendRebootVoice(void)
{
   ...
   msgHdr.flags_event = 1;
   msgHdr.type = CMS_MSG_VOICE_RESTART;
