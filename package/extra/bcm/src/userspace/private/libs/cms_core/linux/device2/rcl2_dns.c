                ...
                CmsRet *errorCode __attribute__((unused)))
{
   ...
   {
      newObj->enable = TRUE;
      // BDK sysmgmt mode only: publish static DNS server to sys_directory on
      // system startup so that the key will be there even if no static DNS
      // servers are configured (value will be empty string).
