                             ...
                             CmsRet *errorCode  __attribute__((unused)))
{
   ...
   if (newObj != NULL && currObj != NULL)
   {
      ...
      if ( cmsUtl_strcmp(newObj->connStatus, MDMVS_CONNECTED) == 0 )
      {
         ...
         {
            ret = rutTunnel_6rdConfig(wanIntf, wanIp, currObj->prefix, currObj->borderRelayAddress, currObj->ipv4MaskLen, tunnelName, lanIntf, FALSE);
            ret = rutTunnel_6rdConfig(wanIntf, wanIp, newObj->prefix, newObj->borderRelayAddress, newObj->ipv4MaskLen, tunnelName, lanIntf, TRUE);
         }
         else
         {
            ret = rutTunnel_6rdConfig(wanIntf, wanIp, newObj->prefix, newObj->borderRelayAddress, newObj->ipv4MaskLen, tunnelName, lanIntf, TRUE);
   ...
   else if ( DELETE_EXISTING(newObj, currObj) )
   {
      ...
      ret = rutTunnel_6rdConfig(NULL, NULL, currObj->prefix, currObj->borderRelayAddress, currObj->ipv4MaskLen, tunnelName, lanIntf, FALSE);
   }   
                             ...
                             CmsRet *errorCode  __attribute__((unused)))
{
   ...
   if (newObj != NULL && currObj != NULL)
   {
      ...
      if ( cmsUtl_strcmp(newObj->connStatus, MDMVS_CONNECTED) == 0 )
      {
         ...
         if ( cmsUtl_strcmp(currObj->connStatus, MDMVS_CONNECTED) == 0 )
         {
             ...
             */
            ret = rutTunnel_4in6Config(wanIntf, wanIp, currObj->remoteIpv6Address, FALSE);
            ret = rutTunnel_4in6Config(wanIntf, wanIp, ipstr, TRUE);
         }
         else
         {
            ret = rutTunnel_4in6Config(wanIntf, wanIp, ipstr, TRUE);
   ...
   else if ( newObj == NULL )
   {
      ...
      ret = rutTunnel_4in6Config(NULL, NULL, currObj->remoteIpv6Address, FALSE);
   }   
