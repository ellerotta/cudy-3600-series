...
#ifdef DMP_X_BROADCOM_COM_ETHERNETOAM_1
#define TMS_CONFIG_FILE "/tmp/tmsctl.conf"

void rutEthOam_setService(UBOOL8 enabled, const char* ifname)
{
   char cmd[BUFLEN_1024] = {0};
   char configCmd[BUFLEN_1024] = {0};
   char ifNameBuf[BUFLEN_64] = {0};
   FILE *f = NULL;

   f = fopen(TMS_CONFIG_FILE, "r");
   if (f != NULL)
   {
      if (fgets(configCmd, BUFLEN_1024, f) != NULL)
      {
         /* To avoid OAM service disruption when DSL interface link down/up, check the interface. */
         if(ifname)
         {
            snprintf(ifNameBuf, sizeof(ifNameBuf), "-i %s", ifname);
            if(strstr(configCmd, ifNameBuf) == NULL)
            {
               fclose(f);
               return;
            }
   ...
   if(enabled && (configCmd[0] != '\0'))
   {
      rut_doSystemAction( __FUNCTION__, configCmd);
   }
   else
   {
      snprintf(cmd, BUFLEN_1024, "killall -9 tmsctl 2> /dev/null");
      rut_doSystemAction( __FUNCTION__, cmd);
   }
   return;
}

void rutEthOam_prepareConfig()
{
   InstanceIdStack ieee8023ahCfgIidStack;
   Ieee8023ahCfgObject *ieee8023ahCfg = NULL;
   char cmd3ah[BUFLEN_256];
   ...
   cmd3ah[0] = ifNameBuf[0] = oamIdBuf[0] = variableRetrievalBuf[0] = '\0';
   linkEventsBuf[0] = remoteLoopbackBuf[0] = activeModeBuf[0] = autoEventBuf[0] = '\0';

   cmd1ag[0] = mdIdBuf[0] = mdLvlBuf[0] = ccmIntervalBuf[0] = locIntfBuf[0] = '\0';
   ...
   if(ieee8023ahCfg->enabled)
   {
      ...
      snprintf(cmd3ah, sizeof(cmd3ah), "3ah start%s%s%s -f %s%s%s%s",
   ...
   if(enbl1ag)
   {
      if(enbl1731)
      {
         snprintf(cmd1ag, sizeof(cmd1ag), "1731 start%s%s%s%s%s%s%s",
            locIntfBuf, megIdBuf, mdLvlBuf, locMepIdBuf,
            locVlanIdBuf, locCcmEnblBuf, remMepIdBuf);
      }
      else
      {
         snprintf(cmd1ag, sizeof(cmd1ag), "1ag start%s%s%s%s%s%s%s%s",
            locIntfBuf, mdIdBuf, mdLvlBuf, maIdBuf, locMepIdBuf,
            locVlanIdBuf, locCcmEnblBuf, remMepIdBuf);
      }
   }

   if((cmd3ah[0] != '\0') || (cmd1ag[0] != '\0'))
   {
      confFile = fopen(TMS_CONFIG_FILE, "w");
      if (confFile != NULL)
      {
         fprintf(confFile, "tmsctl %s %s &", cmd3ah, cmd1ag);
         fclose(confFile);
      }
   }
   else
   {
      snprintf(cmd, BUFLEN_1024, "rm -f %s", TMS_CONFIG_FILE);
      rut_doSystemAction( __FUNCTION__, cmd);
   }
   return;
}
