   ...
   while (cmsObj_getNext(MDMOID_WAN_IP_CONN, &iidStack1, (void **) &wan_ip_con) == CMSRET_SUCCESS)
   {
      ...
      if ( wan_ip_con->X_BROADCOM_COM_IfName[0] != '\0' ) 
      {
         ...
         if ( init_process == FALSE )
         {
            ...
            {
               snprintf(cmd, sizeof(cmd), "iptables -w -t nat -D PREROUTING -i %s -j DNAT --to-destination %s",
                       wan_ip_con->X_BROADCOM_COM_IfName, currObj->IPAddress);
               rut_doSystemAction("rcl_secDmzHostCfgObject", cmd);
               snprintf(cmd, sizeof(cmd), "iptables -w -D FORWARD -i %s -d %s -j ACCEPT",
                       wan_ip_con->X_BROADCOM_COM_IfName, currObj->IPAddress);
               rut_doSystemAction("rcl_secDmzHostCfgObject", cmd);
               
               snprintf(cmd, sizeof(cmd), "iptables -w -t nat -D PREROUTING -i %s -p udp --dport 67:68 -j RETURN", wan_ip_con->X_BROADCOM_COM_IfName); 
               rut_doSystemAction("rcl_secDmzHostCfgObject", cmd);
               snprintf(cmd, sizeof(cmd), "iptables -w -D FORWARD -i %s -p udp --dport 67:68 -j RETURN", wan_ip_con->X_BROADCOM_COM_IfName);
               rut_doSystemAction("rcl_secDmzHostCfgObject", cmd);
            }
         ...
         {
            snprintf(cmd, sizeof(cmd), "iptables -w -t nat -A PREROUTING -i %s -j DNAT --to-destination %s",
                    wan_ip_con->X_BROADCOM_COM_IfName, newObj->IPAddress);
            rut_doSystemAction("rcl_secDmzHostCfgObject", cmd);
            
            /* Don't preroute DHCP Offer to LAN Host */
            snprintf(cmd, sizeof(cmd), "iptables -w -t nat -I PREROUTING 1 -i %s -p udp --dport 67:68 -j RETURN", wan_ip_con->X_BROADCOM_COM_IfName);
            rut_doSystemAction("rcl_secDmzHostCfgObject", cmd);
            
            snprintf(cmd, sizeof(cmd), "iptables -w -I FORWARD 1 -i %s -d %s -j ACCEPT",
                    wan_ip_con->X_BROADCOM_COM_IfName, newObj->IPAddress);
            rut_doSystemAction("rcl_secDmzHostCfgObject", cmd);
 
            /* Don't forward DHCP Offer to LAN Host */
            snprintf(cmd, sizeof(cmd), "iptables -w -I FORWARD 1 -i %s -p udp --dport 67:68 -j RETURN", wan_ip_con->X_BROADCOM_COM_IfName);
      ...
      if ( wan_ppp_con->X_BROADCOM_COM_IfName[0] != '\0' ) 
      {
         ...
         if ( init_process == FALSE )
         {
            if (cmsUtl_isValidIpAddress(AF_INET, currObj->IPAddress))
            {
               ...
               snprintf(cmd, sizeof(cmd), "iptables -w -t nat -D PREROUTING -i %s -p udp --dport 67:68 -j RETURN", wan_ppp_con->X_BROADCOM_COM_IfName);
               rut_doSystemAction("rcl_secDmzHostCfgObject", cmd);
               snprintf(cmd, sizeof(cmd), "iptables -w -D FORWARD -i %s -p udp --dport 67:68 -j RETURN", wan_ppp_con->X_BROADCOM_COM_IfName); 
               rut_doSystemAction("rcl_secDmzHostCfgObject", cmd);
            }
         }
         /* add the new DMZ host */
         if (cmsUtl_isValidIpAddress(AF_INET, newObj->IPAddress))
         {
            ...
            /* Don't preroute DHCP Offer to LAN Host */
            snprintf(cmd, sizeof(cmd), "iptables -w -t nat -I PREROUTING 1 -i %s -p udp --dport 67:68 -j RETURN", wan_ppp_con->X_BROADCOM_COM_IfName);
            rut_doSystemAction("rcl_secDmzHostCfgObject", cmd);
            ...
            /* Don't forward DHCP Offer to LAN Host */
            snprintf(cmd, sizeof(cmd), "iptables -w -I FORWARD 1 -i %s -p udp --dport 67:68 -j RETURN", wan_ppp_con->X_BROADCOM_COM_IfName);
            rut_doSystemAction("rutIpt_vrtsrvCfg", cmd);  
         }
      }
      cmsObj_free((void **) &wan_ppp_con);		 
