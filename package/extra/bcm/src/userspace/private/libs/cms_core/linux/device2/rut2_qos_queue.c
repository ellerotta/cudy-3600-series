...
UINT32 rutQos_getQueueNumberOfEntries(void)
{
   ...
   num = qosObj->queueNumberOfEntries;
   cmsObj_free((void **)&qosObj);
   return num;
}

void rutQos_setQueueNumberOfEntries(UINT32 num)
{
   Dev2QosObject *qosObj=NULL;
   InstanceIdStack iidStack=EMPTY_INSTANCE_ID_STACK;
   CmsRet ret;
   ...
   qosObj->queueNumberOfEntries = num;
   ret = cmsObj_set(qosObj, &iidStack);
   if (ret != CMSRET_SUCCESS)
   {
      cmsLog_error("Could not set queueNumberOfEntries to %d, ret=%d",
                   num, ret);
   }
   cmsObj_free((void **)&qosObj);
   return;
...
CmsRet rutQos_setDefaultEthQueues_dev2(const char *ifname, UBOOL8 isWan)
{
   ...
   UINT32 precedence;
   UINT32 maxQueues;
   UINT32 queueCountBefore = 0;
   UINT32 queueCountAfter = 0;
   UINT32 qid;
   ...
   cmsLog_notice("Creating %d queues for %s (isWan=%d)",
                 maxQueues, ifname, isWan);

   // First store the queueCount before we create
   queueCountBefore = rutQos_getQueueNumberOfEntries();
   ...
   for (precedence = 1; precedence <= maxQueues; precedence++)
   {
      ...
      /* TCP Pure-ACK: For Ethernet WAN, set second-lowest priority as TCP Pure-ACK queue by default. */
      if(isWan && precedence == (ETHWAN_QOS_LEVELS-1))
      {
         qObj->X_BROADCOM_COM_TcpPureAckQueue = TRUE;
      }      
   ...
   // If queueCount has not increased, that means we are in mdmInit and
   // we have to set the number manually.
   queueCountAfter = rutQos_getQueueNumberOfEntries();
   if (queueCountBefore == queueCountAfter)
      rutQos_setQueueNumberOfEntries(queueCountBefore + maxQueues);
