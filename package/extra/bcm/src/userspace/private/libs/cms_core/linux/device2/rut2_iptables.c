...
CmsRet rutIpt_AddfwExceptionforIPDevice_dev2(char *ipIntfFullPath)
{
   ...
   {
      INIT_INSTANCE_ID_STACK(&iidStack);
      ret = cmsObj_addInstance(MDMOID_DEV2_FIREWALL_EXCEPTION,&iidStack);
      if (ret != CMSRET_SUCCESS)
      {
         cmsLog_error("Could not add MDMOID_DEV2_FIREWALL_EXCEPTION instance, ret=%d", ret);
         return ret;
      }  
                            ...
                            (void **) &fwExObj) == CMSRET_SUCCESS)
      {
          if (cmsUtl_strcmp(fwExObj->IPInterface, ipIntfFullPath) == 0 )
          {
                                    ...
                                    (void **) &fwExRuleObj) == CMSRET_SUCCESS)
             {
               ...
               ret = cmsObj_set((void *)fwExRuleObj, &iidStack1);
               if (ret != CMSRET_SUCCESS)
               {
                  cmsLog_error("Failed to set Dev2FirewallExceptionRuleObject, ret=%d", ret);
               }    
...
CmsRet rutIpt_vrtsrvCfg_dev2(const _Dev2NatPortMappingObject *portmapObj, const UBOOL8 add ,char *portmap_ifName)
{
   ...
   do{
         ...
         if( externalPort==0 && internalPort==0)
         {
             ...
             {
                 snprintf(cmd, sizeof(cmd), "iptables -w -t nat -D PREROUTING -i %s -j DNAT --to-destination %s 2>/dev/null", interface, internalClient);
                 rut_doSystemAction("rutIpt_vrtsrvCfg", cmd);
                 snprintf(cmd, sizeof(cmd), "iptables -w -D FORWARD -i %s -d %s -j ACCEPT  2>/dev/null", interface, internalClient);
                 rut_doSystemAction("rutIpt_vrtsrvCfg", cmd);   
                 ...
                 if( action == 'A')   
                 {
                    snprintf(cmd, sizeof(cmd), "iptables -w -t nat -A PREROUTING -i %s -j DNAT --to-destination %s 2>/dev/null",interface, internalClient);
                    rut_doSystemAction("rutIpt_vrtsrvCfg", cmd);
                    ...
                    snprintf(cmd, sizeof(cmd), "iptables -w -I FORWARD 1 -i %s -d %s -j ACCEPT 2>/dev/null", interface, internalClient);
                    rut_doSystemAction("rutIpt_vrtsrvCfg", cmd); 
         ...
         }else
         {
            ...
            while( entry_ptr->name != NULL )
            {
                ...
                {
                    snprintf(cmd, sizeof(cmd), "iptables -w -t nat -%c PREROUTING -i %s -p %s --dport %d -j REDIRECT --to-ports %d 2>/dev/null"
                            ,action
                            ,interface
      ...
      if(add)
      {
               ...
               (MDMOID_DEV2_NAT_PORT_MAPPING, &iidStack_2, (void **) &DMZ_portmapObj)) == CMSRET_SUCCESS)
         {
            ...
            {
                strncpy(dmzHost, DMZ_portmapObj->internalClient, sizeof(dmzHost)-1);
...
void rutIpt_configNatForIntfGroup_dev2(UBOOL8 isAdd, const char *ifName, const char *bridgeIfName)
{
   ...
   cmsLog_debug("wan isAdd=%d, ifName=%s bridgeIfName=%s", 
                isAdd, ifName, bridgeIfName);
...
#endif
   {
      ...
      {
         snprintf(cmd, sizeof(cmd), "iptables -w -t nat -%c PREROUTING -i %s -d %s -p udp --dport 53 -j DNAT --to %s", 
