   ...
   if (newObj && currObj && newObj->enable)
   {
      ...
      if (!IS_EMPTY_STRING(defaultGateway) &&
          ((cmsUtl_strcmp(currObj->X_BROADCOM_COM_ActiveDefaultGateway, defaultGateway)) ||
           ...
           (rutWan_isPppOnDemand(defaultGateway))))
      {
         char *oldActDftGW = NULL;
         char *newActDftGW = NULL;

         REPLACE_STRING_IF_NOT_EQUAL_FLAGS(newObj->X_BROADCOM_COM_ActiveDefaultGateway,
                                     defaultGateway, mdmLibCtx.allocFlags);

         oldActDftGW = currObj->X_BROADCOM_COM_ActiveDefaultGateway;
         newActDftGW = newObj->X_BROADCOM_COM_ActiveDefaultGateway;

         if (!IS_EMPTY_STRING(newActDftGW))
         {
            CmsRet r2;

            r2 = rutRt_configActiveDefaultGateway(newActDftGW);
            if (r2 != CMSRET_SUCCESS)
            {
               cmsLog_error("failed to set default gateway of %s", newActDftGW);
            ...
            {
               r2 = rut_restartUpnpWithWanIfc(newActDftGW);
               if (r2 != CMSRET_SUCCESS)
               {
         ...
         /* When detecting activeDefaultGateway changes, we should adjust the rp_filter 
          * for the "old' and "new" gw interfaces.
          * Please see the comments in the multicast related section of the rutIpv4Service_setup()
          * to know more about the logic to setup the rp_filter
          */
         if (cmsUtl_strcmp(oldActDftGW, newActDftGW))
         {
            if (!IS_EMPTY_STRING(oldActDftGW) && 
                qdmIpIntf_isFirewallEnabledOnIntfnameLocked_dev2(oldActDftGW))
            {
               if (qdmMulti_isIgmpSourceEnabledOnIntfNameLocked(oldActDftGW))
               {
                  // since the oldActDftGW is not the default gateway now, set the rp_filter to "2" for it
                  rut_configIpv4RpFilter(oldActDftGW, 2); // 2: Loose mode
               }
            }

            if (!IS_EMPTY_STRING(newActDftGW) && 
                qdmIpIntf_isFirewallEnabledOnIntfnameLocked_dev2(newActDftGW))
            {
               // just set the rp_filter back to "1" for the present default gateway
               rutIpt_activateFirewallAntiIPspoofing(newActDftGW);
            }
         }
      }
                ...
                CmsRet *errorCode __attribute__((unused)))
{
        ...
        (newObj != NULL && newObj->enable && currObj != NULL && currObj->enable))  /* reactivate an existing route */
   {
      ...
      {
         snprintf(cmd, sizeof(cmd), "ip -6 ro add %s ", newObj->destIPPrefix);
      }
      else
      {
         snprintf(cmd, sizeof(cmd), "ip -6 ro add %s metric %d", newObj->destIPPrefix, 
      ...
      {
         strcat(cmd, " via ");
         strncat(cmd, newObj->nextHop, sizeof(cmd)-1);
         cmd[sizeof(cmd)-1] = '\0';         
      }
      if (!IS_EMPTY_STRING(newObj->interface))
      {
         ...
         {
            strcat(cmd, " dev ");
            strncat(cmd, l3IntfNameBuf, sizeof(cmd)-1);
            cmd[sizeof(cmd)-1] = '\0';             
         }

#ifdef SUPPORT_GRE_TUNNEL_TR181
         if (!strncmp(l3IntfNameBuf, GRE_IFC_STR, strlen(GRE_IFC_STR))
             && !rut_isGreTunnelInterfaceUpLocked(l3IntfNameBuf))
         {
            cmsLog_debug("l3IntfNameBuf:%s, not up", l3IntfNameBuf);
            return ret;
         }
#endif
      }
   ...
   else if (DELETE_OR_DISABLE_EXISTING(newObj, currObj))
   {
      ...
      snprintf(cmd, sizeof(cmd), "ip -6 ro del %s 2> /var/rmrt", currObj->destIPPrefix);
      rut_doSystemAction("rcl_dev2Ipv6ForwardingObject", cmd);
   }
