...
#endif
{
    ...
    else if (rut_tmctl_isUserTMCtlOwner(user) == TRUE)
    {
        ...
        {
            BOOL tmConfigured;
            tmctl_getPortTmState(devType, if_p, &tmConfigured);
            
#ifdef REMOVE_WAN_QUEUE_ON_LINKDOWN
            if (getPonLinkStatusFlag() == FALSE)
#else
#ifdef BUILD_CUSTOMER
            if ((tmConfigured == FALSE) && (getPonWanStatusFlag() == FALSE))
#else
            if ((tmConfigured == FALSE) && (getPonLinkStatusFlag() == FALSE))
#endif
  ...
  tmctl_devType_e devType, tmctl_if_t *if_p, tmctl_queueCfg_t *qcfg_p)
{
    ...
    {
        tmctl_adapt_prio(devType, if_p, &qcfg_p->priority);
...
tmctl_ret_e rutwrap_tmctl_setPortState(eTmctlUser user,
    tmctl_devType_e devType, tmctl_if_t *if_p, BOOL state)
{
    if (applyTmctlCfg(user, devType, if_p) == FALSE)    
    {
        cmsLog_debug("Skip TMCtl configuration, user=%d", user);
    ...
    {
        return tmctl_setPortState(devType, if_p, state);
    }
}
...
tmctl_ret_e rutwrap_tmctl_deletePolicer(eTmctlUser user __attribute__((unused)), int policerId)
{
	return tmctl_deletePolicer(policerId);
}

tmctl_ret_e rutwrap_tmctl_addMirror(eTmctlUser user __attribute__((unused)), tmctl_devType_e devType,
  tmctl_if_t* if_p, tmctl_mirror_op_e op, tmctl_ethIf_t destIf)
{
    return tmctl_addMirror(devType, if_p, op, destIf, 0);
}

tmctl_ret_e rutwrap_tmctl_delMirror(eTmctlUser user __attribute__((unused)), tmctl_devType_e devType,
  tmctl_mirror_op_e op, tmctl_if_t* if_p)
{
    return tmctl_delMirror(devType, op, if_p);
}

CmsRet rutwrap_tmctl_GetDevInfoByIfName(const char *l2ifname, tmctl_devType_e *devType_p, tmctl_if_t *if_p)
{
