...
static void modifyNumInterfaceSetting(const InstanceIdStack *iidStack,
                                        SINT32 delta)
{
   rutUtil_modifyNumGeneric_dev2(MDMOID_DEV2_IPV6RD,
                                 MDMOID_DEV2_IPV6RD_INTERFACE_SETTING, iidStack, delta);
}
                ...
                CmsRet *errorCode __attribute__((unused)))
{
   ...
   if ( ADD_NEW(newObj, currObj) )
   {
      ...
      modifyNumInterfaceSetting(iidStack, 1);
...
//      char ipstr[CMS_IPADDR_LENGTH];
      ...
      if (cmsUtl_isValidIpAddress(AF_INET, wanIp) == TRUE)
      {
         ...
         if (cmsUtl_strcmp(currObj->status, MDMVS_ENABLED) == 0) 
         {
             ...
             */
            ret = rutTunnel_6rdConfig(ifname, wanIp, currObj->SPIPv6Prefix,
         ...
         ret = rutTunnel_6rdConfig(ifname, wanIp, newObj->SPIPv6Prefix,
...
//            rutIpt_TCPMSSforIPTunnel(ifname, TRUE); //FIXME AAA!!!
   ...
   else if (DELETE_OR_DISABLE_EXISTING(newObj, currObj))
   {
      ...
      if (cmsUtl_strcmp(currObj->status, MDMVS_ENABLED) == 0)
      {
         ret = rutTunnel_6rdConfig(NULL, NULL, currObj->SPIPv6Prefix,
                                   currObj->borderRelayIPv4Addresses,
                                   currObj->IPv4MaskLength,
                                   currObj->X_BROADCOM_COM_TunnelName, 
                                   currObj->X_BROADCOM_COM_TunnelInterface,
