...
#ifdef INIT_WAN_QUEUE_ON_LINKUP
static BOOL isTmConfigured(const char *ifName)
{
    UBOOL8 tmConfigured = FALSE; 
    tmctl_if_t intf;
    ...
    /* ignoring return value if something is wrong no action to take */
    (void)rutwrap_tmctl_GetDevInfoByIfName(ifName, &devType, &intf);

    tmctl_getPortTmState(devType, &intf, &tmConfigured);
Exit:
   return tmConfigured;
}
#endif
...

{
   ...
   if (ENABLE_NEW_OR_ENABLE_EXISTING(newObj, currObj))
   {
      ...
      {
         portInit = !isTmConfigured(newObj->ifName);
      }
   }
   ...
   {
       rdpaCtl_set_sys_car_mode(rut_tmctl_getQueueOwner() != TMCTL_OWNER_BH);
   }
   ...
   if (portInit == TRUE)
   {
      /* remove old configuration before applying new one */ 
