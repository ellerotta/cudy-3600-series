                                    ...
                                    _WanPppConnObject *newObj)
{
   ...
   if (ret == CMSRET_SUCCESS && newObj->idleDisconnectTime > 0)
   {
      ...
      rutIpt_initNatForPppOnDemand(newObj->X_BROADCOM_COM_IfName);
   }   
                                    ...
                                    _WanPppConnObject *newObj)
{
   ...
   rutIpt_initNatAndFirewallOnWanConnection(newObj->X_BROADCOM_COM_IfName,
                                            newObj->idleDisconnectTime,
                                            newObj->NATEnabled,
                                            newObj->X_BROADCOM_COM_FirewallEnabled);

   /* setup Qos queues associated with this interface */
   if ((ret = rutQos_qMgmtClassReconfig(newObj->X_BROADCOM_COM_IfName, FALSE)) != CMSRET_SUCCESS)
   {
 ...
 */ 
   ...
   */
    ...
    * The iptables has to be in first and 2 ip rules to redirect the lan tranffic
    */
   if (isIPv4 && currObj->idleDisconnectTime > 0)
   {
      rutIpt_initNatForPppOnDemand(currObj->X_BROADCOM_COM_IfName);
   }
   ...
   /* For PPP on demand, need to keep the default gateway if ppp connection is down 
    */
   if (newObj && newObj->idleDisconnectTime > 0)
   {
      cmsLog_debug("On demand and leave default gateway in");
   }
   else
   {
      /* reactivate system default gateway and dns */
                                   ...
                                   const _WanPppConnObject *currObj, UBOOL8 isIPv4)
{
   ...
   */
   else
   {
      ...
      if (rutWl2_isPPPoA(iidStack))
      {
          snprintf(cmd, sizeof(cmd), "kill %d", currObj->X_BROADCOM_COM_PppdPid);
          rut_doSystemAction("DEL PPPOA", cmd);
          sleep(1);
	  }
