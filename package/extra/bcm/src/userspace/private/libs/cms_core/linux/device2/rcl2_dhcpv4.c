   ...
   if (newObj && currObj)
   {
          ...
          !IS_EMPTY_STRING(newObj->interface))
      {
             ...
             !strcmp(serviceStatusBuf, MDMVS_SERVICEUP))
         {
            ...
            if ((ret = qdmIntf_fullPathToIntfnameLocked(newObj->interface, intfNameBuf)) != CMSRET_SUCCESS)
            {
                ...
                CmsRet *errorCode __attribute((unused)))
{
   ...
   if (ENABLE_NEW_OR_ENABLE_EXISTING(newObj, currObj))
   {
      ...
      {
         CMSMEM_REPLACE_STRING_FLAGS(newObj->status, MDMVS_ERROR_MISCONFIGURED, mdmLibCtx.allocFlags);
      }
   }
            ...
            rutLan_isDhcpv4ServerPoolChanged_dev2(newObj, currObj))
   {
      ...
      /* if interface is empty, mark the table Misconfigured */
      if (!IS_EMPTY_STRING(newObj->interface))
      {
         if (qdmIntf_fullPathToIntfnameLocked_dev2(newObj->interface, ifName) != CMSRET_SUCCESS)
         {
            error = TRUE;
         }
      }
...
#endif  /* DMP_DEVICE2_DHCPV4SERVER_1 */
