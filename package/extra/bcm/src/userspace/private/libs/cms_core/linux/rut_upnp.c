         ...
         *pcpMode = obj->X_BROADCOM_COM_PCPMode;
         ...
         if (*pcpMode != PCP_MODE_DISABLE) 
         {
            if (*pcpMode == PCP_MODE_DSLITE)
            {
               ...
               ret = qdmIpIntf_getIpv6AddressByNameLocked_dev2(obj->name, addrtmp);
               if (ret == CMSRET_SUCCESS)
               {
                  cmsLog_debug("Get IPv6 address by obj->name %s", obj->name);
               }
            ...
            else /* PCP_MODE_NAT444 */
            {
               ...
               ret = qdmIpIntf_getIpv4AddressByNameLocked_dev2(obj->name, addr);
               if (ret == CMSRET_SUCCESS)
               {
                  cmsLog_debug("get interface %s IPv4 address %s", obj->name, addr);
               }               
               else
               {
                  cmsLog_error("Cannot get IPv4 address from interface %s", obj->name);
               }       
...
CmsRet  rut_startUpnp(char *ifcName, char *excludeIfc, UBOOL8 pcpEnable)
{
   ...
   if (pcpEnable == FALSE)
   {
      ...
      *  2)  if default gateway is not natted just use this natted wan interface.
      ...
      {
         strncpy(upnpIfcName, ifcName, sizeof(upnpIfcName)-1);
         upnpIfcName[sizeof(upnpIfcName)-1] = '\0';
