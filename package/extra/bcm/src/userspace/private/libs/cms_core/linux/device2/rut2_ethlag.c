...
void rutEthLag_configEthIntfOnBridge(const char *brName, const char *ethIfName, UBOOL8 add)
{
   ...
   cmsUtl_strncpy(ifNameBuf, ethIfName, sizeof(ifNameBuf));
...
#ifdef DESKTOP_LINUX
   cmsUtl_strncpy(bond0Intf, DEFAULT_ETHLAG_0, bond0IntfStrLen);
   cmsUtl_strncpy(bond1Intf, DEFAULT_ETHLAG_1, bond1IntfStrLen);   
   return TRUE;
#endif
   ...
   if ( fs != NULL ) 
   {
      while (!found && fgets(line, sizeof(line), fs) )
      {
         ...
         if ((cmsUtl_strcmp(b0, DEFAULT_ETHLAG_0) == 0) && (cmsUtl_strcmp(b1,DEFAULT_ETHLAG_1) == 0)) 
         {
            ...
            if (bond0Intf != NULL)
            {
               cmsUtl_strncpy(bond0Intf, b0, bond0IntfStrLen);
            }
            if (bond1Intf != NULL)
            {
               cmsUtl_strncpy(bond1Intf, b1, bond1IntfStrLen);
            }            
         }
      }
      fclose(fs);
