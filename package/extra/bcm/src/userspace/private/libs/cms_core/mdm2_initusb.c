...
CmsRet mdm_addDefaultUsbHostObject(void)
{
   ...
   for (i=1; i<=MAX_USB_HOSTS; i++)
   {
     ...
     if ((ret = mdm_getObject(pathDesc.oid, &pathDesc.iidStack, (void **) &usbHostObj)) != CMSRET_SUCCESS)
      ...
      {
         /* must be one of OHCI, EHCI, UHCI and xHCI */
         if (strstr(buf, MDMVS_OHCI))
            CMSMEM_REPLACE_STRING_FLAGS(usbHostObj->type, MDMVS_OHCI, mdmLibCtx.allocFlags);
         else if (strstr(buf, MDMVS_EHCI))
            CMSMEM_REPLACE_STRING_FLAGS(usbHostObj->type, MDMVS_EHCI, mdmLibCtx.allocFlags);
         else if (strstr(buf, MDMVS_UHCI))
            CMSMEM_REPLACE_STRING_FLAGS(usbHostObj->type, MDMVS_UHCI, mdmLibCtx.allocFlags);
         else
            CMSMEM_REPLACE_STRING_FLAGS(usbHostObj->type, MDMVS_XHCI, mdmLibCtx.allocFlags);
      }
