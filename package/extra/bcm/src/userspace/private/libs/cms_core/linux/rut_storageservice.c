...
#define _GNU_SOURCE /* for putpwent */
#include <stdlib.h>
#include <sys/vfs.h>
...
CmsRet rutStorage_addUserAccount(char *username, const char *password, char *homeDir)
{
   ...
   snprintf(cmd, sizeof(cmd), "(echo %s; echo %s) | smbpasswd -as %s >/dev/null ",password,password,username);
   rut_doSystemAction("storage:useraccount", cmd);

   /* create a soft link in /var/samba/homes/ */
   snprintf(cmd, sizeof(cmd), "ln -s %s /var/samba/homes/%s",homeDir,username);
   rut_doSystemAction("storage:useraccount", cmd);
...
CmsRet rutStorage_deleteUserAccount(const char *username, const char *homeDir)
{
   ...
   snprintf(cmd, sizeof(cmd), "smbpasswd -x %s",username);
   rut_doSystemAction("storage:useraccount", cmd);

   /*remove users linux account */
   snprintf(cmd, sizeof(cmd), "deluser %s",username);
   rut_doSystemAction("storage:useraccount", cmd);

   /*remove users home dir */
   snprintf(cmd, sizeof(cmd), "rm -rf %s",homeDir);
   rut_doSystemAction("storage:useraccount", cmd);

   /* remove  soft link in /var/samba/homes/ */
   snprintf(cmd, sizeof(cmd), "rm /var/samba/homes/%s",username);
   rut_doSystemAction("storage:useraccount", cmd);
