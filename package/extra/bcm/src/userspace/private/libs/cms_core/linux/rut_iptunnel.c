...
UBOOL8 rutTunnel_containDynamicTunnel( const char *ifName, UBOOL8 is6in4 )
{
   ...
   if (is6in4)
   {
      ...
      ret = qdmIntf_intfnameToFullPathLocked(ifName, isLayer2, &fullPath);
      if (ret != CMSRET_SUCCESS)
      {
         cmsLog_error("Could not convert %s to fullPath", ifName);
         return FALSE;
      }
...
#ifdef SUPPORT_IPV6
CmsRet rutTunnel_4in6Config( const char *wanIntf, const char *wanIp, const char *remoteIp, UBOOL8 add )
{
   CmsRet ret = CMSRET_SUCCESS;
   char cmdStr[BUFLEN_256];

   cmsLog_debug("Enter for %s: wanIntf<%s> wanIp<%s> remoteIp<%s>", add?"ADD":"DEL", wanIntf, wanIp, remoteIp);
   ...
   if ( add )
   {
      ...
      snprintf(cmdStr, sizeof(cmdStr), "ip link set dev ip6tnl1 mtu %d up", mtuSize-40);
      rut_doSystemAction("4in6CfgAdd", cmdStr);
      ...
      snprintf(cmdStr, sizeof(cmdStr), "ethtool -K ip6tnl1 sg off");
      rut_doSystemAction("4in6CfgAdd", cmdStr);
   }
   else
   {
...
CmsRet rutTunnel_get6rdLanAddr(const char *prefixStr, const char *lanIntf, char *addr)
{
...
#elif defined(DMP_X_BROADCOM_COM_DEV2_IPV6_1)
   {
      ...
      ret = qdmIntf_fullPathToIntfnameLocked_dev2(lanIntf, ifname);
      if (ret != CMSRET_SUCCESS)
      {
         cmsLog_error("qdmIntf_fullPathToIntfnameLocked_dev2 failed. ret=%d lanIntf=%s", ret, lanIntf);
      }         
      rutLan_getIntfMacAddr(ifname, macAddr);
   }
#endif
                                          ...
                                          const char *lanIntf)
{
    ...
    * 2. ipv6AddrStr is in the following format: prefix::EUI64/64
...
CmsRet rutTunnel_6rdConfig(const char *wanIntf, const char *wanIp, const char *prefix, const char *brAddr, SINT32 ipv4MaskLen,
                           const char *tunnelName, const char *lanIntf, UBOOL8 add)
{
   ...
   if ( add )
   {
      ...
      char addr6[CMS_IPADDR_LENGTH];
      SINT32 i, plen, mtuSize, tmpMask=0;
      ...
      inet_ntop(AF_INET, &tmpAddr4, ipv4str, INET_ADDRSTRLEN);
      ...
      inet_ntop(AF_INET6, &in6Addr, ipv6str, INET6_ADDRSTRLEN);
      cmsLog_debug("tmpAddr6= %s\n", ipv6str);

      /* always use SLAAC as address instead of ::1 */
      if (rutTunnel_get6rdLanAddr(ipv6str, lanIntf, ipv6AddrStr) == CMSRET_SUCCESS)
      {
         cmsUtl_strcat(ipv6AddrStr, "/64"); /* Address assignment */
      }
      else
      {
      ...
      if ((mtuSize = rutWan_getInterfaceMTU(AF_INET, wanIntf)) <= 0)
      {
         mtuSize = 1500;
      }
      ...
      /* tunnel MTU = WAN MTU - ipv4 header */
      snprintf(cmdStr, sizeof(cmdStr), "ip link set dev sit1 mtu %d up", mtuSize-20);
      rut_doSystemAction("6rdConfigAdd", cmdStr);
