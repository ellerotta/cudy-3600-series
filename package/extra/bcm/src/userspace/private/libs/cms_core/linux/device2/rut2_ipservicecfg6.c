...
{
   cmsLog_notice("%s >>> %s ifname=%s isWan=%d",
   ...
   if (!cmsUtl_strcmp(newStatus, MDMVS_SERVICESTARTING))   /*** NEW STATE ***/
   {
      ...
      else if (!cmsUtl_strcmp(currStatus, MDMVS_SERVICEUP))
      {
          ...
          */
         if (isWan)
            bcmuLog_knotice("partially deactivating IPv6 services on %s...", ifname);

         rutIpv6Service_teardown(ipIntfFullPath, ifname, isWan);

         if (isWan)
            bcmuLog_knotice("IPv6 services on %s are partially deactivated.", ifname);
      }
   ...
   else if (!cmsUtl_strcmp(newStatus, MDMVS_SERVICEUP))  /*** NEW STATE ***/
   {
      if (!cmsUtl_strcmp(currStatus, MDMVS_SERVICESTARTING))
      {
          ...
          */
         if (isWan)
            bcmuLog_knotice("Activating IPv6 services on %s...", ifname);

         rutIpv6Service_setup(ipIntfFullPath, ifname, isWan);

         if (isWan)
            bcmuLog_knotice("All IPv6 services on %s are activated.", ifname);
      }
      else if (!cmsUtl_strcmp(currStatus, MDMVS_SERVICEUP))
      {
         /*
   ...
   {
     if (isWan)
         bcmuLog_knotice("Deactivating IPv6 services on %s...", ifname);
      ...
      if (isWan)
         bcmuLog_knotice("All IPv6 services on %s have been deactivated.", ifname);
   }
   else
   {
...
void rutIpv6Service_setup(const char *ipIntfFullPath, const char *ifname, UBOOL8 isWan)
{
   ...
   /* TCP Pure-ACK: Configure TCP Pure-ACK Netfilter rules for this L3 interface. */
   ...
   /* In hybrid data model, IPv6 is TR181, but QoS is TR98.
      Reconfigure QoS classification. */
   if (isWan)
   {
      if ((ret = rutQos_qMgmtClassReconfig(ifname, FALSE)) != CMSRET_SUCCESS)
      {
         cmsLog_error("rutQos_qMgmtClassReconfig returns error. ret=%d", ret);
         return ;
      }
   }
#endif
   ...
   {
      /*
       * To VeriWave between LAN and WLAN, we can do the following on WebUI.
       * 1. set no WAN service
       * 2. set static LAN IPv6
       * 3. set stateful DHCP server
       * 4. set ULA prefix and its lifetime
       */
      cmsLog_debug("(LAN side) start DHCP server for %s ?", ifname);
      rutDhcpv6_updateServerPoolObj(ipIntfFullPath);
      return;
   }
      ...
      if (found)
      {
                    ...
                    OGF_NO_VALUE_UPDATE, (void **)&prefixObjChild) == CMSRET_SUCCESS)
         {
            if (!cmsUtl_strcmp(prefixObjChild->parentPrefix, prefixFullPath))
            {
                ...
                ret = cmsMdm_pathDescriptorToFullPathNoEndDot(&pathDescPrefixChild, &fullPathPrefixChild);
                if (ret != CMSRET_SUCCESS)
                {
                ...
                else
                {
                   SINT32 subnetId;
#ifdef DMP_DEVICE2_BRIDGE_1
                   rutBridge_getBridgeIndexByName_dev2(ipIntfObj->name, &subnetId);
#else
                   subnetId = 0;
#endif
                   ...
                   if (subnetId < 0)
                   {
                       cmsLog_error("Invalid bridge index: %d", subnetId);
                   }
                   cmsLog_debug("bridge name=%s, subnetId=%d", ipIntfObj->name, subnetId);

                   if (cmsNet_subnetIp6SitePrefix(prefix, (UINT8)subnetId, 64, networkPrefix) != CMSRET_SUCCESS)
                   {
                   ...
                   else
                   {
                      ...
                      rutEbt_configIPv6Prefix(networkPrefix, TRUE);

                      /* Set address (from the delegated prefix) to brX, construct in Modified EUI-64 format!! */
                      cmsUtl_genEui64AddrByPrefix(networkPrefix, ipIntfObj->name, lanIntfAddress, &prefixLen);
                      
                      cmsLog_notice("set address<%s> to %s with iid<%s>", lanIntfAddress,
                                                                      ipIntfObj->name,
                                                                      cmsMdm_dumpIidStack(&iidStackIpIntf));
                   
                      if (!rutIp_findIpv6Addr(&iidStackIpIntf,
                                              lanIntfAddress,
                                              MDMVS_AUTOCONFIGURED,
                                              &iidStack_ipv6Addr))
                      {
                         rutIp_addIpv6Addr(&iidStackIpIntf,
                                           lanIntfAddress,
                                           MDMVS_AUTOCONFIGURED,
                                           fullPathPrefixChild, plt, vlt);
                      }

                      /* get fullpath of child intf obj for RAObj */
                      INIT_PATH_DESCRIPTOR(&pathDescTmp);
                      pathDescTmp.oid = MDMOID_DEV2_IP_INTERFACE;
                      pathDescTmp.iidStack = iidStackIpIntf;
                      ...
                      else
                      {
                         ...
                         /* update dhcp6s */
                         rutDhcpv6_updateServerPoolObj(childIntfFullPath);
...
#ifdef SUPPORT_VXLAN_TUNNEL_TR181
      rutVxlan_activateTunnelIf_dev2(ifname, TRUE);
#endif

#ifdef SUPPORT_GRE_TUNNEL_TR181
      rutGre_activateTunnelIf_dev2(ifname, TRUE);
...
static void rutIpv6Service_teardownChildPD(InstanceIdStack iidStackChild, Dev2Ipv6PrefixObject *prefixObjChild)
{
   char networkPrefix[CMS_IPADDR_LENGTH];
   char prefix[CMS_IPADDR_LENGTH];
   char lanIntfAddress[CMS_IPADDR_LENGTH];
   ...
   cmsUtl_strncpy(prefix, prefixObjChild->prefix, CMS_IPADDR_LENGTH);
   ...
   else
   {
      ...
      if (cmsNet_subnetIp6SitePrefix(prefix, (UINT8)subnetId,
                                     64, networkPrefix) != CMSRET_SUCCESS)
      {
         cmsLog_error("cmsNet_subnetIp6SitePrefix returns error");
      }
      else
      {
         ...
         /* Delete rules for source IP range of PD */
         rutIpt_configRoutingChain6(networkPrefix, ipIntfObj->name, FALSE);
         rutEbt_configIPv6Prefix(networkPrefix, FALSE);

         /* delete corresponding address object at brX */
         cmsUtl_genEui64AddrByPrefix(networkPrefix, ipIntfObj->name, lanIntfAddress, &prefixLen);

         cmsLog_debug("delete address<%s> to %s\n", lanIntfAddress,ipIntfObj->name);


         rutIp_deleteIpv6Addr(&iidStackChild, lanIntfAddress, MDMVS_AUTOCONFIGURED);
...
void rutIpv6Service_teardown(const char *ipIntfFullPath, const char *ifname, UBOOL8 isWan)
{
    ...
    */
   {
      ...
      rutQos_reconfigTcpPureAckRulesLayer3_dev2(ipIntfFullPath, QOS_COMMAND_UNCONFIG);
   ...
   if (found)
   {
                 ...
                 OGF_NO_VALUE_UPDATE, (void **)&prefixObjChild) == CMSRET_SUCCESS))
      {
         if (!cmsUtl_strcmp(prefixObjChild->parentPrefix, prefixFullPath))
         {
            rutIpv6Service_teardownChildPD(iidStackChild, prefixObjChild);
         }
         cmsObj_free((void **)&prefixObjChild);
      }
   }
...
#ifdef DMP_DEVICE2_IPV6ROUTING_1
#ifdef SUPPORT_VXLAN_TUNNEL_TR181
   if (cmsUtl_strncmp(ifname, VXLAN_IFC_STR, strlen(VXLAN_IFC_STR)))   
#endif
   {
...
#ifdef SUPPORT_VXLAN_TUNNEL_TR181
      rutVxlan_activateTunnelIf_dev2(ifname, FALSE);
#endif
#ifdef SUPPORT_GRE_TUNNEL_TR181
      rutGre_activateTunnelIf_dev2(ifname, FALSE);
#endif  //SUPPORT_GRE_TUNNEL_TR181
...
void rutIpv6Service_stop(const char *ipIntfFullPath, const char *ifname, UBOOL8 isWan)
{
   ...
   /*
    *There no MDMVS_SERVICEUP state if WAN at ipv6 Auto Configured mode without dhcpv6/static set or
    *Dhcpv6 have not completed negotiation.
    *When IPv6 WAN is down, state will be MDMVS_SERVICESTARTING -> MDMVS_SERVICEDOWN. 
    *Here should clear address and prefix which set by Router Advertisement like rutIpv6Service_teardown do.
    */  
   CmsRet ret=CMSRET_SUCCESS;
   char prefix[CMS_IPADDR_LENGTH];
   MdmPathDescriptor pathDesc, prefixPathDesc;
   ...
   /*Delete the address which auto configured by Router Advertisement*/
   cmsLog_debug("Delete address which set by RA!");
   ...
   while (cmsObj_getNextInSubTreeFlags(MDMOID_DEV2_IPV6_ADDRESS,
                                    ...
                                    (void **)&addrObj) == CMSRET_SUCCESS)
   {
      /*Address which auto configured by Router Advertisement*/
      if (!cmsUtl_strcmp(addrObj->origin, MDMVS_AUTOCONFIGURED))
      {
         if ((ret = cmsObj_deleteInstance(MDMOID_DEV2_IPV6_ADDRESS, &iidStack)) != CMSRET_SUCCESS)
         {
            cmsLog_error("Failed to delete ipv6AddrObj. ret=%d", ret);
   ...
   /*Delete prefixes which got from Router Advertisement*/
   cmsLog_debug("delete prefixes which got from RA!");
   ...
   while (cmsObj_getNextInSubTreeFlags(MDMOID_DEV2_IPV6_PREFIX,
                                    ...
                                    (void **)&prefixObj) == CMSRET_SUCCESS)
   {
      /* prefix got from Router Advertisement */
      if (!cmsUtl_strcmp(prefixObj->origin, MDMVS_ROUTERADVERTISEMENT))
      {
         if ((ret = cmsObj_deleteInstance(MDMOID_DEV2_IPV6_PREFIX, &iidStack)) != CMSRET_SUCCESS)
         {
            cmsLog_error("Failed to delete ipv6PrefixObj. ret=%d", ret);
