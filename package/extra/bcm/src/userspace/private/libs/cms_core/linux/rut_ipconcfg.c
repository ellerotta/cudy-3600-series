                                   ...
                                   _WanIpConnObject *newObj)
{
   ...
   if (!cmsUtl_strcmp(newObj->connectionType, MDMVS_IP_BRIDGED))
   {
      ...
      {
         rutOpenVS_startupOpenVSport(newObj->X_BROADCOM_COM_IfName, OVS_BRIDGE);
   ...
   else
   {
      ...
      /* setup QoS classification associated with this interface */
      if ((ret = rutQos_qMgmtClassReconfig(newObj->X_BROADCOM_COM_IfName, FALSE)) != CMSRET_SUCCESS)
      {
         cmsLog_error("rutQos_qMgmtClassReconfig returns error. ret=%d", ret);
         return ret;
               ...
               
{
   ...
   if (!cmsUtl_strcmp(currObj->connectionType, MDMVS_IP_BRIDGED))
   {
      ...
      {
         rutOpenVS_shutdownOpenVSport(currObj->X_BROADCOM_COM_IfName, OVS_BRIDGE);
         return CMSRET_SUCCESS;
      }
      else
      {
   ...
   if (!rutWl2_isIPoA(iidStack))
   {
      ...
      if (cmsUtl_strcmp(currObj->connectionType, MDMVS_IP_BRIDGED))
      {
         /* Skip to stop the IPoE interface, leave it to be done in the interface stack */
      }
      else
#endif
