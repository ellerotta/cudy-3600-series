 ...
 */
void rutIpt_initNat(const char *ifName);
 ...
 *
 * @param ifName (IN) The WAN connection interface name.
 * @param pppIdleDisconnection (IN) For ppp connections only, the idle disconnect time. 
...
#define CWMP_CHECK_CHAIN "cwmp_chk"
/** Initialize the CWMP_CHECK_CHAIN
 *
 * @param domain        (IN) Communication domain, either PF_INET or PF_INET6.
 * @param enable        (IN) This indicates the CWMP is enabled or disabled.
 */
...
/**  Execute commands to setup Default Gateway for PPP on demand
 * @param ifcName       (IN) This is the wan interface name string 
 */
...
#ifdef SUPPORT_MAP
#include "device2/rut2_map.h"
/** iptables rules for SNAT MAP
 *
 * @param (IN) add or not
 * @param (IN) callerStr
 * @param (IN) tunintf
