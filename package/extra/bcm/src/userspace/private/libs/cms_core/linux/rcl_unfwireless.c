...
extern void rut2_sendWifiChange(void);
#endif
                         ...
                         CmsRet *errorCode __attribute__((unused)))
{
    ...
    if ((newObj != NULL) && (currObj != NULL))
    {
        ...
        {
            if (0 != strncmp(cfgObj->wlCountry, newObj->regulatoryDomain, 2))
            {
        ...
        {
            char strBuf[BUFLEN_64]={};
            char tmpBuf[BUFLEN_64]={};
            char *splitAuthMode = NULL;
            char *savePtr = NULL;
            ...
            if (wepEnabled)
                ...
                CMSMEM_REPLACE_STRING_FLAGS(virtObj->wlWep, "disabled", mdmLibCtx.allocFlags);
            ...
            /* We have to keep non-TR98 data models */
            cmsUtl_strncpy(strBuf, virtObj->wlAuthMode, sizeof(strBuf));
            memset(tmpBuf, 0, sizeof(tmpBuf));
            splitAuthMode = strtok_r(strBuf, " ", &savePtr);
            if (splitAuthMode == NULL)
            {
                if(!rutWlan_isValidTr98WlAuthMode(strBuf))
                {
                strncpy(tmpBuf, strBuf, sizeof(strBuf)-2);
                strcat(tmpBuf, " ");					
                }
            }
            else
            {
                strcpy(tmpBuf, "");
                while (splitAuthMode != NULL)
                {
                    if(!rutWlan_isValidTr98WlAuthMode(splitAuthMode))
                    {
                        strcat(tmpBuf, splitAuthMode);
                        strcat(tmpBuf, " ");
                    }
                splitAuthMode = strtok_r(NULL," ", &savePtr);					
                }
            }
            
            if(strcmp(wlAuthMode, "open"))
                strcat(tmpBuf, wlAuthMode);
            else if(strlen(tmpBuf) == 0)
                strncpy(tmpBuf, wlAuthMode, sizeof(tmpBuf)-1);
            else if(tmpBuf[strlen(tmpBuf)-1] == ' ')
            {
                tmpBuf[strlen(tmpBuf)-1] = '\0';
            }

            CMSMEM_REPLACE_STRING_FLAGS(virtObj->wlAuthMode, tmpBuf, mdmLibCtx.allocFlags);
                                ...
                                CmsRet *errorCode __attribute__((unused)))
{
    ...
    if (newObj != NULL)
    {
        if ((currObj != NULL) && (currObj->wlEnblSsid != newObj->wlEnblSsid))
        {
            if (newObj->wlEnblSsid && (strlen(newObj->wlBrName) >= 3))
            {
                rutWlan_enableVirtIntfFilter(newObj->wlIfcname, atoi(&(newObj->wlBrName[2])));
            }
            else
            {
                rutWlan_disableVirtIntfFilter(newObj->wlIfcname);
            }
        }
        ...
        if (bssid_index == 1)
        {
            ...
            if ((currObj == NULL) || (0 != cmsUtl_strcmp(currObj->wlAuthMode, newObj->wlAuthMode)))
            {
                ...
                char authModeBuf[BUFLEN_64];

                rutWlan_WlAuthModeStripNonTr98(newObj->wlAuthMode, authModeBuf, sizeof(authModeBuf));
                beaconType = rutWlan_getBeaconType(authModeBuf);
