...
UBOOL8 rutLan_isDhcpdEnabled_dev2(void)
{
   ...
   {
      dhcpdEnabled = dhcpv4ServerObj->enable;
      cmsObj_free((void **)&dhcpv4ServerObj);

      /* if the global dhcpserver setting is enable, then find out if any pool is enabled */
      if (dhcpdEnabled == TRUE)
      {
         dhcpdEnabled = FALSE;
         INIT_INSTANCE_ID_STACK(&iidStack);
         while ((dhcpdEnabled == FALSE) &&
                (cmsObj_getNextFlags(MDMOID_DEV2_DHCPV4_SERVER_POOL, &iidStack, OGF_NO_VALUE_UPDATE, (void **)&dhcpv4ServerPoolObj) == CMSRET_SUCCESS))
         {
...
#ifdef DMP_DEVICE2_DHCPV4RELAY_1
   {
      ...
      INIT_INSTANCE_ID_STACK(&iidStack);
      while (dhcpdEnabled == FALSE &&
             cmsObj_getNextFlags(MDMOID_DEV2_DHCPV4_RELAY_FORWARDING, &iidStack, OGF_NO_VALUE_UPDATE, (void **) &forwardingObj) == CMSRET_SUCCESS)
      {
...
void rutLan_createDhcpdCfg_dev2(void)
{
   ...
   while (cmsObj_getNext(MDMOID_DEV2_DHCPV4_SERVER_POOL, &iidStack, (void **)&dhcpv4ServerPoolObj) == CMSRET_SUCCESS)
   {
          ...
          qdmIntf_fullPathToIntfnameLocked_dev2(dhcpv4ServerPoolObj->interface, ifName) == CMSRET_SUCCESS)
      {
...
#ifdef DMP_DEVICE2_BRIDGE_1 /* SUPPORT_PORT_MAP */
         /* check interface group vendor id */
         /* Only apply the vendor id setting to br0 interface. */
         if (!cmsUtl_strcmp(ifName, "br0"))
         {
             Dev2BridgeFilterObject *filterObj = NULL;
             InstanceIdStack filterIidStack = EMPTY_INSTANCE_ID_STACK;
             char *vendorIdString = NULL;

             while(cmsObj_getNext(MDMOID_DEV2_BRIDGE_FILTER, &filterIidStack, (void**) &filterObj) == CMSRET_SUCCESS)
             {
         ...
         if ( !IS_EMPTY_STRING(dhcpv4ServerPoolObj->subnetMask) )
         {
            ...
            netmask = cmsUtl_strsplit(dhcpv4ServerPoolObj->subnetMask, ".", &size);
            if (netmask != NULL)
            {
                /* We assume the subnetMask string in MDM is an valid netmask string.
                 * So netmask should always have 4 substrings.
                 */
                if (size == 4)
                {
                    if (cmsUtl_strcmp(netmask[2], "0") == 0 &&
                        cmsUtl_strcmp(netmask[3], "0") == 0)
                    {
                        /* The netmask is CLASS B or higher.
                         * Restrict the leases poll size to 512.
                         */
                        snprintf(cmdStr, sizeof(cmdStr), "max_leases 512\n");
                    }
                    else
                    {
                        /* 255.255.255.0 or even smaller subnet */
                        snprintf(cmdStr, sizeof(cmdStr), "max_leases 254\n");
                    }
                    fputs(cmdStr, fs);
                }

                idx = 0;
                while (netmask[idx] != NULL)
                {
                    cmsMem_free(netmask[idx]);
                    idx++;
                }
                cmsMem_free(netmask);
            }
         ...
         if ( !IS_EMPTY_STRING(dhcpv4ServerPoolObj->IPRouters) )
         {
            ...
            strncpy(buff, dhcpv4ServerPoolObj->IPRouters, sizeof(buff)-1);
   ...
   /* 
    * Server config of udhcpd, which are not related to a particular
   ...
   /* disable regular write of leases file since leases file is kept persisten on flash,
    * which has a limited write cycle.
    */
   snprintf(cmdStr, sizeof(cmdStr), "auto_time 0\n");
   fputs(cmdStr, fs);
