...
static void updateLastChange(void)
{
   CmsRet ret;
   InstanceIdStack iidStack = EMPTY_INSTANCE_ID_STACK;
   Dev2FirewallObject *firewallobj = NULL;

   if ((ret = cmsObj_get(MDMOID_DEV2_FIREWALL, &iidStack, 0,
                         (void **)&firewallobj)) != CMSRET_SUCCESS)
   {
      cmsLog_error("Failed to get firewall object. ret=%d", ret);
   }
   else
   {
      ...
      CMSMEM_REPLACE_STRING_FLAGS(firewallobj->lastChange, dateTimeBuf, mdmLibCtx.allocFlags);

      if ((ret = cmsObj_set(firewallobj, &iidStack)) != CMSRET_SUCCESS)
      {
         cmsLog_error("Failed to set firewall object. ret=%d", ret);
      }
...
static int isChainForFirewallService(const Dev2FirewallChainObject *chainObj, 
                                     ...
                                     int *ipver, char *table, int tableLen, 
                                     char *chain, int chainLen)
{
   ...
   strncpy(chainObjName, chainObj->name, sizeof(chainObjName)-1);
   chainObjName[sizeof(chainObjName)-1] = '\0';   

   while ((string != NULL) && (idx < 5))
   {
      token = strsep(&string, "~");
   ...
   cmsLog_debug("isFWS<%d> container<%s> table<%s> chain<%s> ipver<%d>", 
                isFWS, containerName, table, chain, *ipver);
                                    ...
                                    char *chain, int chainLen)
{
   ...
   forFWS = isChainForFirewallService(chainObj, containerName,
                                      CONT_CONTNAME_LEN_MAX, &ipver,
                                      tableName, BUFLEN_8, chainName,BUFLEN_32);
                                       ...
                                       UBOOL8 isEnable)
{
   ...
   {
      INIT_PATH_DESCRIPTOR(&intfPathDesc);
      if (cmsMdm_fullPathToPathDescriptor(obj->sourceInterface, &intfPathDesc) != CMSRET_SUCCESS)
      {
         cmsLog_error("cmsMdm_fullPathToPathDescriptor fails for %s", obj->sourceInterface);
      }
      else
      {
         intfobj = NULL;
         if (cmsObj_get(intfPathDesc.oid, &intfPathDesc.iidStack, 0, (void **)&intfobj) != CMSRET_SUCCESS)
         {
            cmsLog_error("cannot get intfobj");
         }
         else
         {
            if (!IS_EMPTY_STRING(intfobj->name))
            {
               sprintf(srcIf, "-i %s", intfobj->name);
            }
   ...
   {
      INIT_PATH_DESCRIPTOR(&intfPathDesc);
      if (cmsMdm_fullPathToPathDescriptor(obj->destInterface, &intfPathDesc) != CMSRET_SUCCESS)
      {
         cmsLog_error("cmsMdm_fullPathToPathDescriptor fails for %s", obj->destInterface);
      }
      else
      {
         intfobj = NULL;
         ...
         else
         {
            if (!IS_EMPTY_STRING(intfobj->name))
            {
               sprintf(srcIf, "-o %s", intfobj->name);
            }
   ...
   if (isFWS)
   {
      snprintf(cmd, sizeof(cmd), "%s -w -t %s -%c %s %s %s %s %s %s %s %s -j %s",
   ...
   else
   {
      if (isEnable)
      {
         /* iptables does not check for duplicated rule. To avoid duplication,
          * the same rule is deleted before adding. It is ok if the rule does
          * not exist.
          */
         snprintf(cmd, sizeof(cmd), "%s -w -D FORWARD %s %s %s %s %s %s %s -j %s 2>/dev/null",
                 (ipver==4)?"iptables":"ip6tables",
                 proto, srcIP, srcPort, dstIP, dstPort, srcIf, dstIf, target);
         cmsLog_debug("cmd=%s", cmd);
         rut_doSystemAction("doFirewallChainRule", cmd);
      }
      snprintf(cmd, sizeof(cmd), "%s -w -%c FORWARD %s %s %s %s %s %s %s -j %s",
...
int rutfirewall_setDefaultPolicy_dev2(const char *policy, UBOOL8 isEnable)
{
   ...
   {
      snprintf(cmd, sizeof(cmd), "iptables -w -%c FORWARD -j %s", isEnable?'A':'D', "DROP");
   }
   else if (!cmsUtl_strcmp(policy, MDMVS_ACCEPT))
   {
      snprintf(cmd, sizeof(cmd), "iptables -w -%c FORWARD -j %s", isEnable?'A':'D', "ACCEPT");
   }
   else
   {
                                 ...
                                 UBOOL8 enable, int ipver)
{
   ...
   if (enable)
   {
      snprintf(cmd, sizeof(cmd), "%s -w -t %s -N %s", (ipver==4)?"iptables":"ip6tables",
              table, chainName);
      cmsLog_debug("cmd<%s>", cmd);
      ret = runCmd(cmd);
      ...
      {
         /* FIXME: order */
         snprintf(cmd, sizeof(cmd), "%s -w -t %s -I %s -j %s",
                 (ipver==4)?"iptables":"ip6tables", table, chain, chainName);
         cmsLog_debug("cmd<%s>", cmd);
         ret = runCmd(cmd);
         if (ret != CMSRET_SUCCESS)
         {
   ...
   else
   {
      if (isStandardChain)
      {
         snprintf(cmd, sizeof(cmd), "%s -w -t %s -D %s -j %s",
                 (ipver==4)?"iptables":"ip6tables", table, chain, chainName);
         cmsLog_debug("cmd<%s>", cmd);
         ret = runCmd(cmd);
         if (ret != CMSRET_SUCCESS)
      ...
      snprintf(cmd, sizeof(cmd), "%s -w -t %s -F %s", (ipver==4)?"iptables":"ip6tables",
              table, chainName);
      cmsLog_debug("cmd<%s>", cmd);
      ret = runCmd(cmd);
      snprintf(cmd, sizeof(cmd), "%s -w -t %s -X %s", (ipver==4)?"iptables":"ip6tables",
              table, chainName);
      cmsLog_debug("cmd<%s>", cmd);
      ret = runCmd(cmd);
   }
...
static UBOOL8 isChainObjActive(const InstanceIdStack *chainiidStack)
{
   ...
   if (firewallobj->enable)
   {
      ...
      if (cmsObj_get(chainPathDesc.oid, &chainPathDesc.iidStack, 0,
                     (void **)&chainObj) != CMSRET_SUCCESS)
      {
         cmsLog_notice("cannot get chainobj");
         goto out;
      }
      ...
      if (!cmsUtl_strcmp(firewallobj->config, MDMVS_ADVANCED))
      {
         ...
         {
            cmsLog_notice("cannot get levelobj");
            CMSMEM_FREE_BUF_AND_NULL_PTR(chainFullPath);
            goto out;
         }
                ...
                CmsRet *errorCode)
{
   ...
   if (newObj)
   {
      ...
      len = cmsUtl_strlen(newObj->advancedLevel);
      if ((len > 0) && (newObj->advancedLevel[len-1] == '.'))
      {
         /* remove the end-dot before saving to mdm. */
         newObj->advancedLevel[len-1] = '\0';
      }
   }

   if (newObj && IS_EMPTY_STRING(newObj->lastChange))
   {
      char dateTimeBuf[BUFLEN_64]={0};

      /* set lastChange to start-up time */
      cmsTms_getXSIDateTime(0, dateTimeBuf, sizeof(dateTimeBuf));
      CMSMEM_REPLACE_STRING_FLAGS(newObj->lastChange, dateTimeBuf, mdmLibCtx.allocFlags);
   }
   ...
   {
      if (newObj && currObj && (currObj->enable == TRUE))
      {
   ...
   if (isChainObjActive(iidStack))
   {
      ...
      if (ENABLE_NEW_OR_ENABLE_EXISTING(newObj, currObj))
      {
         forFWS = isChainForFirewallService(newObj, containerName,
                                            CONT_CONTNAME_LEN_MAX, &ipver,
                                            table, BUFLEN_8, chain, BUFLEN_32);
         if (forFWS)
         {
            char chainName[BUFLEN_32]={0};
