...
static CmsRet startL3VlanInterface(_Dev2VlanTerminationObject *newObj)
{
   CmsRet ret=CMSRET_SUCCESS;
   char l2IntfNameBuf[CMS_IFNAME_LENGTH]={0};
   UBOOL8 isBridge=TRUE;

   if( !newObj ) return ret;

   cmsLog_debug("newObj->lowerLayers %s", newObj->lowerLayers);
   if ((ret = qdmIntf_fullPathToIntfnameLocked_dev2(newObj->lowerLayers,
              l2IntfNameBuf)) != CMSRET_SUCCESS)
   {
      cmsLog_error("qdmIntf_getIntfnameFromFullPathLocked failed. ret %d",
         ret);
      return ret;
   }
...
static void stopL3VlanInterface(const _Dev2VlanTerminationObject *currObj)
{
   if( !currObj ) return;
   /*
...
#if defined(SUPPORT_WANVLANMUX)
#if defined(SUPPORT_VLANSUBIF)
                ...
                CmsRet *errorCode __attribute__((unused)))
{
   ...
   if (ENABLE_NEW_OR_ENABLE_EXISTING(newObj, currObj))
   {
          ...
          IS_EMPTY_STRING(newObj->name))
      {
         ...
         */
         ...
         {
            char vlanIfName[CMS_IFNAME_LENGTH*2]={0};
         ...
         /*
         * If status is "Up", need to config Vlan interface
         */
         if(!cmsUtl_strcmp(newObj->status, MDMVS_UP))
         {
            ret = startL3VlanInterface(newObj);
            if (ret != CMSRET_SUCCESS)
            {
               cmsLog_error("startL3VlanInterface failed. ret %d", ret);
            }
         }
      }
   }
   else if (POTENTIAL_CHANGE_OF_EXISTING(newObj, currObj))
   {
      ...
      if (!cmsMdm_isDataModelDevice2())
      {
         if( cmsUtl_strstr(newObj->name, PPP_IFC_STR) ) return ret;
      ...
      /*
       * If new status is "Up", need to config Vlan interface
       */
      if (!cmsUtl_strcmp(newObj->status, MDMVS_UP) &&
          cmsUtl_strcmp(currObj->status, MDMVS_UP))
      {
