                           ...
                           CmsRet *errorCode __attribute__((unused)))
{
       ...
       rutLan_isIpv6IntfChanged(newObj, currObj))
   {
          ...
          (cmsUtl_strcmp(newObj->X_BROADCOM_COM_DhcpConnectionStatus, MDMVS_CONNECTED))))
      {
         ...
         CMSMEM_REPLACE_STRING_FLAGS(newObj->X_BROADCOM_COM_DhcpDefaultGateway, "0.0.0.0", mdmLibCtx.allocFlags);
         CMSMEM_REPLACE_STRING_FLAGS(newObj->X_BROADCOM_COM_DNSServers, "", mdmLibCtx.allocFlags);
         if (cmsUtl_strcmp(newObj->IPInterfaceAddressingType, MDMVS_DHCP)) {
            CMSMEM_REPLACE_STRING_FLAGS(newObj->X_BROADCOM_COM_DhcpConnectionStatus, MDMVS_UNCONFIGURED, mdmLibCtx.allocFlags);	 
         }
      }
          ...
          (newObj->X_BROADCOM_COM_DhcpcPid == 0))
      {
         ...
         CMSMEM_REPLACE_STRING_FLAGS(newObj->X_BROADCOM_COM_DhcpConnectionStatus, MDMVS_CONNECTING, mdmLibCtx.allocFlags);
	 ...
	 {
#ifdef SUPPORT_DPI
            insertModuleByName("nf_conntrack_netlink"); // needed by dpi commands
#endif
	    if (FirewallOff)
            {
              ...
              cmsUtl_strcmp(currObj->IPInterfaceSubnetMask, newObj->IPInterfaceSubnetMask)))
         {
             ...
             */
            rutLan_reconfigNatForAddressChange(currObj->X_BROADCOM_COM_IfName,
                                               currObj->IPInterfaceIPAddress, currObj->IPInterfaceSubnetMask,
   ...
   if (DELETE_OR_DISABLE_EXISTING(newObj, currObj))
   {
      ...
      rutLan_disableBridge(currObj->X_BROADCOM_COM_IfName, FALSE);
                ...
                CmsRet *errorCode __attribute__((unused)))
{
   ...
   if (ENABLE_NEW_OR_ENABLE_EXISTING(newObj, currObj))
   {
      if ((ret = rutLan_getParentBridgeIfName(MDMOID_LAN_EPON_INTF, iidStack, bridgeIfName)) == CMSRET_SUCCESS)
      {
         ...
         rutLan_addInterfaceToBridge(newObj->ifName, FALSE, bridgeIfName);
      }
   }
