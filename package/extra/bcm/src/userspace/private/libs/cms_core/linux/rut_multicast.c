                                 ...
                                 UINT32 mode, SINT32 lanToLanEnable)
{
   ...
   ret = bcm_mcast_api_set_snooping_cfg(-1, ifi, BCM_MCAST_PROTO_IPV4, mode);
   if(ret < 0)
   {
...
void rutMulti_createWanAssocList_igd(char *strictWanAssocList, int strictWanAssocListLen)
{
   InstanceIdStack iidStack = EMPTY_INSTANCE_ID_STACK;
      ...
      while (cmsObj_getNext(MDMOID_L2_BRIDGING_FILTER, &fltIidStack, (void **)&pBridgeFltObj) == CMSRET_SUCCESS)
      {
            ...
            /* && (cmsUtl_strcmp(pBridgeFltObj->filterInterface, MDMVS_LANINTERFACES)  I dunno about this part*/
         {
            ...
            else
            {
               ...
               {
                   if (strictWanAssocList[0] != '\0')
                   {
                      cmsUtl_strncat(strictWanAssocList, strictWanAssocListLen, "|");
                   }
                   //attach br name and interface name
                   rutPMap_availableInterfaceReferenceToIfName(availIntfObj->interfaceReference, wanIfName);
                   cmsUtl_strncat(strictWanAssocList, strictWanAssocListLen, ipIntfObj->X_BROADCOM_COM_IfName);
                   cmsUtl_strncat(strictWanAssocList, strictWanAssocListLen, " ");
                   cmsUtl_strncat(strictWanAssocList, strictWanAssocListLen, wanIfName);
               }
...
CmsRet rutMulti_CreateMcpdCfg( void )
{
   ...
   if(igmpObj->igmpBridgeIfNames && strlen(igmpObj->igmpBridgeIfNames) > 0)
   {
      fprintf(fp, " %s", igmpObj->igmpBridgeIfNames);
   }

   if(gponObj->mcastBridgeIfNames && strlen(gponObj->mcastBridgeIfNames) > 0)
   {
      fprintf(fp, " %s", gponObj->mcastBridgeIfNames);
   }
   ...
   if(mldObj->mldBridgeIfNames && strlen(mldObj->mldBridgeIfNames) > 0)
   {
      fprintf(fp, " %s", mldObj->mldBridgeIfNames);
   }
   ...
   if(mldObj->mldMcastIfNames && strlen(mldObj->mldMcastIfNames) > 0)
   {
      fprintf(fp, " %s", mldObj->mldMcastIfNames);
   }
...
#if defined(DMP_X_BROADCOM_COM_MCAST_1) && (defined(DMP_X_BROADCOM_COM_IGMP_1) || defined(DMP_X_BROADCOM_COM_MLD_1))
   {
      ...
      if (bStrict) {
        rutMulti_createWanAssocList(strictWanAssoc, BUFLEN_256);
...
#ifdef BUILD_CUSTOMER
    char tmp[BUFLEN_512] = {0};
    rutWan_getMulticastVlans(tmp);
    fprintf(fp, "#\n");
    fprintf(fp, "#Begin mcast vlan configuration\n");
    fprintf(fp, "#\n");
    fprintf(fp, "mcpd-mcast-vlan %s\n", tmp);
    fprintf(fp, "#\n");
    fprintf(fp, "#End mcast vlan configuration\n");
    fprintf(fp, "#\n");
#endif
...
CmsRet rutMulti_reloadMcpdWithType(int protoType __attribute__((unused)))
{
   ...
   cmsLog_notice("Entered: protoType=%d", protoType);
   ...
   if (protoType == BCM_MCAST_PROTO_ALL)
   {
      cmsLog_notice("mpctl reload (all)");
      rc = system("mcpctl reload");
   }
   else if (protoType == BCM_MCAST_PROTO_IPV4)
   {
      // Must be able to reload ipv4 and ipv6 separately.  See SWBCACPE-38063.
      cmsLog_notice("mpctl reloadipv4");
      rc = system("mcpctl reloadipv4");
   }
   else if (protoType == BCM_MCAST_PROTO_IPV6)
   {
      cmsLog_notice("mpctl reloadipv6");
      rc = system("mcpctl reloadipv6");
   }
   else
   {
      cmsLog_error("Unknown/unsupported protoType %d", protoType);
      ret = CMSRET_INTERNAL_ERROR;
   }

   if (ret == CMSRET_SUCCESS && rc < 0)
   {
      cmsLog_error("mcpctl reload (%d) failed, rc=%d", protoType, rc);
      ret = CMSRET_INTERNAL_ERROR;
   }

   cmsLog_notice("Exit: ret=%d rc=%d protoType=%d", ret, rc, protoType);

#else
   cmsLog_notice("Exit: multicast not supported(?)!");
...
{
   return (rutMulti_reloadMcpdWithType(BCM_MCAST_PROTO_ALL));
}
...
#if defined(DMP_X_BROADCOM_COM_IGMP_1) || defined(DMP_X_BROADCOM_COM_MLD_1)
   // CMS msg to mcpd not supported anymore.  Use mcpctl.
   int rc;
   rc = system("mcpctl reset");
   if (rc < 0)
   {
      cmsLog_error("mcpctl reset failed, rc=%d", rc);
      ret = CMSRET_INTERNAL_ERROR;
   }
#endif /* DMP_X_BROADCOM_COM_IGMP_1 || DMP_X_BROADCOM_COM_IGMP_1 */
