...
// Store the original keys used to get the semaphores for debug purposes.
static key_t zoneKey;
static key_t metaKey;
...
CmsRet oalLck_init(UBOOL8 attachExisting, SINT32 keyOffset)
{
   UINT32 flags;
   UINT32 i;

   zoneKey = mdm_getZoneLockSemaphoreKey();
   metaKey = mdm_getMetaLockSemaphoreKey();

   cmsLog_notice("Entered: attachExisting=%d keyOffset=%d zoneKey=0x%x metaKey=0x%x",
                attachExisting, keyOffset, zoneKey, metaKey);

   // would be nice to use IPC_EXCL, but that means semid must be cleaned up
   // correctly (no killall or crashes).
   flags = (attachExisting) ? 0 : IPC_CREAT;

   /* Create or attach the MDM zone lock array */
   if ((zone_semid = semget(zoneKey+keyOffset,
                            MDM_MAX_LOCK_ZONES, flags|0666)) == -1)
   {
      cmsLog_error("semget failed, zoneKey+offset=%d flags=0x%x errno=%d",
                   zoneKey+keyOffset, flags, errno);
      return CMSRET_INTERNAL_ERROR;
   }

   /* Create or attach the meta-lock array (for internal use only) */
   if ((meta_semid = semget(metaKey+keyOffset,
                            MDM_MAX_META_LOCKS, flags|0666)) == -1)
   {
      cmsLog_error("semget failed, metaKey+offset=%d flags=0x%x errno=%d",
                   metaKey+keyOffset, flags, errno);
      return CMSRET_INTERNAL_ERROR;
   }
   ...
   if (attachExisting)
   {
      cmsLog_notice("attach existing done, offset=%d zoneKey=0x%x zone_semid=0x%x metaKey=0x%x meta_semid=0x%x",
                    keyOffset, zoneKey, zone_semid, metaKey, meta_semid);
   ...
   for (i=0; i < MDM_MAX_META_LOCKS; i++)
   {
      if(semctl(meta_semid, i, SETVAL, 0) == -1)
      {
   ...
   cmsLog_notice("attach new done: offset=%d zoneKey=0x%x zone_semid=0x%x metaKey=0x%x meta_semid=0x%x",
                 keyOffset, zoneKey, zone_semid, metaKey, meta_semid);
   return CMSRET_SUCCESS;
}
...
CmsRet oal_lock_meta(void)
{
   // This is the old name of the meta lock, so assume meta lock 0.
   // Getting meta-info should be fast, so don't need timeout.
   return (lock_generic(meta_semid, 0, NULL));
}

CmsRet oal_lock_meta_rbt(void)
{
   return (lock_generic(meta_semid, 1, NULL));
}
...
CmsRet lock_generic(SINT32 semid, UINT32 semIndex, const UINT32 *timeoutMs)
{
   ...
   if (rc != 0)
   {
      ...
      {
         cmsLog_error("lock failed: zonekey=0x%x metaKey=0x%x semid=0x%x index=%d errno=%d rc=%d",
                      zoneKey, metaKey, semid, semIndex, errno, rc);
...
void oal_unlock_meta(void)
{
   // This is the old name of the meta lock, so assume meta lock 0.
   unlock_generic(meta_semid, 0);
   return;
}

void oal_unlock_meta_rbt(void)
{
   unlock_generic(meta_semid, 1);
   return;
}
