 ...
 *
 * @param config (IN)   MDM config params.
 * @param shmSize (IN)   Size of the shared memory region to create.  Use this
 *                       shared mem size instead of what is in config.
 ...
 * @param keyOffset (IN) offset to lock semaphore key.
 * @return CmsRet enum.
 */
...
/** There are now MDM_MAX_META_LOCKS(4) meta locks to protect low level MDM
 *  operations.  Define helpful names to describe them.  Same restrictions as
 *  the original single meta lock (should be very fast, non-blocking).
 *  meta lock 0: protect MDM locking structures.
 *  meta lock 1: protect MDM RBTree operations.
 *  meta lock 2: unused.
 *  meta lock 3: unused.
 */
