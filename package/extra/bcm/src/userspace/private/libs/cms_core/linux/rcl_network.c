                ...
                CmsRet *errorCode __attribute__((unused)))
{
   ...
   */
   ...
   //  TODO: Nothing should have to happen here.  The interface change should trigger an
   //        WAN/LAN up/down event and voice app will decide if a restart is necessary.
   if (currObj != NULL && bRestartVoice )
   {
      ret = rut_sendMsgCommon(EID_SSK, CMS_MSG_VOICE_CALLMGR_RESTART, 0,
                              TRUE, FALSE, FALSE,
                              NULL, 0);
      if (ret != CMSRET_SUCCESS)
      {
         cmsLog_error("could not send voice START/RESTART msg to voice, ret=%d", ret);
      }
   }
#endif
