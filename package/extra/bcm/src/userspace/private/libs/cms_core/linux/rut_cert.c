...
void rutCert_setCACertLinks(void)
{
         ...
         (MDMOID_CERTIFICATE_CFG, &iidStack, (void **) &certCfg)) == CMSRET_SUCCESS)
   {
      ...
      {
         snprintf(cmd, sizeof(cmd), "ln -s '%s%s.cacert' %s`%s x509 -noout -hash < '%s%s.cacert'`.0",
...
static void deleteCertConf(const char *confFileName) 
{
   ...
   snprintf(cmd, sizeof(cmd), "rm -f %s 2>/dev/null", confFileName);
   rut_doSystemAction(__FUNCTION__, cmd); 
}
...
CmsRet rutCert_createCertReq(PCERT_ADD_INFO pAddInfo, CertificateCfgObject *certCfg) 
{
   ...
   snprintf(cmd, sizeof(cmd), "%s genrsa -out %s.key %d 2> %s", sslApp, certTmpName, CERT_KEY_SIZE, certLogName);
   rut_doSystemAction(__FUNCTION__, cmd); 
   data[0] = '\0';
   ...
   if (errFs != NULL) 
   {
      ...
      snprintf(cmd, sizeof(cmd), "rm %s 2>/dev/null", certLogName);
      rut_doSystemAction(__FUNCTION__, cmd); 

      if (cmsUtl_strstr(data, "(0x010001)")) 
      {
         SINT32 keyFileSize;
   ...
   // For developers and SQA who need to sign this certificate request,
   // you will need to create your own local Certificate Authority on your
   // Linux host system:
   // openssl genrsa -out ca_privkey.pem 2048
   // openssl req -new -x509 -key ca_privkey.pem -out ca_cert.pem -days 356
   // Then create a serial number file (start with 00 and then any digits) 
   // echo "0059" > file.srl
   // Sign the client.csr (Copy and paste from WebUI to client.csr):
   // openssl x509 -req -in client.csr -CA ca_cert.pem -CAkey ca_privkey.pem -CAserial file.srl -out client_cert.pem
   // Copy and paste client_cert.pem back to WebUI.
   if (ret == CMSRET_SUCCESS) 
   {
      SINT32 reqFileSize;

      snprintf(cmd, sizeof(cmd), "%s req -new -key %s.key -out %s.csr -batch -config %s", 
   ...
   if (ret != CMSRET_SUCCESS)
   {
      ...
      snprintf(cmd, sizeof(cmd), "rm -f %s.key 2>/dev/null", certTmpName);
      rut_doSystemAction(__FUNCTION__, cmd); 

      snprintf(cmd, sizeof(cmd), "rm -f %s.csr 2>/dev/null", certTmpName);
      rut_doSystemAction(__FUNCTION__, cmd); 
   }
...
CmsRet rutCert_createCertFile(const CertificateCfgObject *certCfg) 
{
   ...
   // Publish certs to Sys Directory is only needed in BDK.  But unconditionally
   // publish here and let higher level logic decide whether msg 
   // should be dropped.
...
CmsRet rutCert_removeCertFile(const CertificateCfgObject *certCfg) 
{
   ...
   if (cmsUtl_strcmp(certCfg->type, CERT_TYPE_CA) == 0)
   {
      snprintf(fn, sizeof(fn), "%s%s.cacert", certPath, certCfg->name);
      snprintf(cmd, sizeof(cmd), "rm %s 2>/dev/null", fn);
      rut_doSystemAction(__FUNCTION__, cmd); 
      // reset CA certificate links
      rutCert_setCACertLinks();
   }
   ...
   {
      snprintf(fn, sizeof(fn), "%s%s.cert", certPath, certCfg->name);
      snprintf(cmd, sizeof(cmd), "rm %s 2>/dev/null", fn);
      rut_doSystemAction(__FUNCTION__, cmd); 

      snprintf(fn, sizeof(fn), "%s%s.req", certPath, certCfg->name);
      snprintf(cmd, sizeof(cmd), "rm %s 2>/dev/null", fn);
      rut_doSystemAction(__FUNCTION__, cmd); 

      snprintf(fn, sizeof(fn), "%s%s.priv", certPath, certCfg->name);
      snprintf(cmd, sizeof(cmd), "rm %s 2>/dev/null", fn);
      rut_doSystemAction(__FUNCTION__, cmd); 
   }
   ...
   // Update certs in Sys Directory.  Only needed in BDK.  But unconditionally
   // update here and let higher level logic decide whether msg 
   // should be dropped.
   rutJson_encodeCertsAndPublish();
...
CmsRet rutCert_verifyCertReq(CertificateCfgObject *certCfg) 
{
   ...
   makePathToCert(certTmpName, BUFLEN_256, CERT_TEMP_NAME);
   makePathToSslApp(sslApp, BUFLEN_40);
   ...
   snprintf(cmd, sizeof(cmd), "%s x509 -pubkey -in %s -noout > %s", sslApp, certn, outn1);
   rut_doSystemAction(__FUNCTION__, cmd);
   snprintf(cmd, sizeof(cmd), "%s req -pubkey -in %s  -noout > %s", sslApp, reqn, outn2);
   rut_doSystemAction(__FUNCTION__, cmd);

   if (cmsFil_compareFile(outn1, outn2) == FALSE) 
   {
      ret = CMSRET_INVALID_CERT_REQ; 
   }
   
   snprintf(cmd, sizeof(cmd), "rm %s 2>/dev/null", certn);
   rut_doSystemAction(__FUNCTION__, cmd);
   snprintf(cmd, sizeof(cmd), "rm %s 2>/dev/null", reqn);
   rut_doSystemAction(__FUNCTION__, cmd);
   snprintf(cmd, sizeof(cmd), "rm %s 2>/dev/null", outn1);
   rut_doSystemAction(__FUNCTION__, cmd);
   snprintf(cmd, sizeof(cmd), "rm %s 2>/dev/null", outn2);
   rut_doSystemAction(__FUNCTION__, cmd);
...
CmsRet rutCert_retrieveSubject(CertificateCfgObject *certCfg) 
{
   ...
   if (cmsUtl_strcmp(certCfg->type, CERT_TYPE_SIGNING_REQ) == 0) 
   {
       ...
       snprintf(cmd, sizeof(cmd), "%s req -subject -in %s -noout > %s 2>/dev/null", sslApp, certn, outn);
       rut_doSystemAction(__FUNCTION__, cmd);
   }
   else 
   {
       ...
       snprintf(cmd, sizeof(cmd), "%s x509 -subject -in %s -noout > %s 2>/dev/null", sslApp, certn, outn);
       rut_doSystemAction(__FUNCTION__, cmd);
   }
   ...
   snprintf(cmd, sizeof(cmd), "rm %s 2>/dev/null", certn);
   rut_doSystemAction(__FUNCTION__, cmd);
   snprintf(cmd, sizeof(cmd), "rm %s 2>/dev/null", outn);
   rut_doSystemAction(__FUNCTION__, cmd);
    ...
    char *subject) 
{
   ...
   snprintf(cmd, sizeof(cmd), "%s x509 -text -in %s -noout > %s 2>/dev/null", sslApp, certn, outn);
   rut_doSystemAction(__FUNCTION__, cmd);
...
CmsRet rutCert_processImportedCert(CertificateCfgObject *certCfg) 
{
   ...
   if (cmsUtl_strcmp(certCfg->type, CERT_TYPE_CA) !=0) 
   {
      ...
      sprintf(outn, "%s.out", certTmpName);
      snprintf(cmd, sizeof(cmd), "%s rsa -check -in %s -noout -out %s", sslApp, privkeyn, outn);
      rut_doSystemAction(__FUNCTION__, cmd);
      ...
      snprintf(cmd, sizeof(cmd), "rm %s 2>/dev/null", outn);
      rut_doSystemAction(__FUNCTION__, cmd);
      snprintf(cmd, sizeof(cmd), "rm %s 2>/dev/null", privkeyn);
      rut_doSystemAction(__FUNCTION__, cmd);
   }   
