   ...
   if (gatewayList != NULL)
   {
      ...
      while (!found && (ptr != NULL) && (count < CMS_MAX_DEFAULT_GATEWAY))
      {
         ...
         if (*ptr != 0)
         {
            ...
            /* Look for both WAN and LAN side since some devices may get
...
void rutRt_deactivateIpv4Routing_dev2(const char *ipIntfFullPath)
{
    ...
    */
   ret = cmsObj_get(MDMOID_DEV2_ROUTER, &routerIidStack, 0, (void **)&routerObj);
   if (ret != CMSRET_SUCCESS)
   {
      cmsLog_error("could not get MDMOID_DEV2_ROUTER router obj, ret=%d", ret);
   }
