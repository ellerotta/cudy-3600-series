                ...
                CmsRet *errorCode __attribute__((unused)))
{
   ...
   if (cmsUtl_isValidIpAddress(AF_INET, newObj->serverIPAddress) == FALSE)
   {
   ...
   else if ((currObj != NULL) && (!strcmp(currObj->status, MDMVS_ENABLED)) && (!strcmp(newObj->status, MDMVS_DISABLED)))
   {
      ...
      if ((!strcmp(currObj->option, MDMVS_LOCAL_FILE)) || (!strcmp(currObj->option, MDMVS_LOCAL_FILE_AND_REMOTE)))
      {
         sprintf(cmd, "rm -rf %s", LOG_LOCAL_FILES);
         if(system(cmd) != 0)
            cmsLog_debug("Execute cmd: %s failed",cmd);
      }
   }
   else if ((currObj != NULL) && (!strcmp(newObj->status, MDMVS_ENABLED)))
   {
      /* only consider about restarting when it's enabled, otherwise, just update the new parameter */
      if (rut_isSyslogCfgChanged(newObj, currObj))
      {
                ...
                CmsRet *errorCode __attribute__((unused)))
{
   ...
   if (newObj != NULL && currObj != NULL)
   {
      // Sometimes these config messages can cause lock errors when tr69c
      // is re-launched.  Avoid sending them if possible
      if (cmsUtl_strcmp(currObj->loggingLevel, newObj->loggingLevel))
      {
         rut_updateLogLevel(EID_TR69C, newObj->loggingLevel);
      }
      if (cmsUtl_strcmp(currObj->loggingDestination, newObj->loggingDestination))
      {
                ...
                CmsRet *errorCode __attribute__((unused)))
{
   ...
   else if  (newObj && currObj && (currObj->enable == FALSE) && (newObj->enable == TRUE)) /* change from disabled to enabled */
   {
      /* just restart upnp */
                ...
                CmsRet *errorCode __attribute__((unused)))
{
   ...
   {
      /* was running, now need to stop it */
      if ((ret = rut_sendMsgToSmd(CMS_MSG_STOP_APP, EID_DNSMASQ, NULL,0)) != CMSRET_SUCCESS)
      {
         cmsLog_error("failed to stop dnsmasq.");
      }
...
// TODO: move this rcl and associaed stl into mdm_cbk_gpon
