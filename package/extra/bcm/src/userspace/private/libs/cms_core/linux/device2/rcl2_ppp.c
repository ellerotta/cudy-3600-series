...
static UBOOL8 isIpcpAddrConfigured(const char *ipIntfFullPath, const char *addr)
{
   UBOOL8 isConfigured = FALSE;
   ...
   while (!isConfigured && cmsObj_getNextInSubTree(MDMOID_DEV2_IPV4_ADDRESS,
                                       &ipIntfPathDesc.iidStack, &iidStackChild,
                                       (void **) &ipv4Addr) == CMSRET_SUCCESS)
   {
      if (!cmsUtl_strcmp(addr, ipv4Addr->IPAddress))
      {
         isConfigured = TRUE;
      }
                ...
                CmsRet *errorCode __attribute__((unused)))
{
   ...
   {
      cmsLog_debug("POTENTIAL_CHANGE_OF_EXISTING ConnectionStatus %s ==> %s "
                   "new->LastConnError=%s",
                   currObj->connectionStatus, newObj->connectionStatus,
                   newObj->lastConnectionError);
      /* Note this RCL function does not care about changes in status.
       * connectionStatus drives the ppp state machine.
       */
      cmsLog_debug("   status=%s ==> status=%s", currObj->status, newObj->status);
      cmsLog_debug("   idleDisconnectTime %d => %d", currObj->idleDisconnectTime, newObj->idleDisconnectTime);
      ...
      /*
       * The PPP connectionStatus state machine has the following states:
       * -- Unconfigured
       * -- Connecting
       * -- Authenticating
       * -- Connected
       ...
       * -- Disconnected
       *
       * Only 3 state changes need to be handled by RCL handler function:
       * 1).   ==> "Connecting"
       * 2).   ==> "Connected"
       * 3).   ==> "Disconnected"
       */
      ...
      if (!cmsUtl_strcmp(newObj->connectionStatus, MDMVS_CONNECTING))
      {
             ...
             !cmsUtl_strcmp(currObj->connectionStatus, MDMVS_DISCONNECTED))
         {
            ...
            */
            ...
            {
               /* Start PPPoE */
               ret = rutWan_initPPPoE(iidStack, newObj);
               if (ret != CMSRET_SUCCESS)
               {
            ...
            /* for on demand, need to redirect lan traffic to ppp interface.
             * The iptables has to be in first and 2 ip rules to redirect the lan tranffic
             */
            if (ret == CMSRET_SUCCESS && newObj->idleDisconnectTime > 0)
            {
               rutIpt_insertIpModules();
               rutIpt_setDefaultGwForPppOnDemand(newObj->name);
            }
         }
      }
      ...
      */
      else if (!cmsUtl_strcmp(newObj->connectionStatus, MDMVS_CONNECTED))
      {
         ...
         */
         ...
         else if (!pppIntfReset && !cmsUtl_strcmp(currObj->connectionStatus, MDMVS_CONNECTED))
         {
            ...
            */
            ...
            if (!cmsUtl_isZeroIpvxAddress(CMS_AF_SELECT_IPV4, 
                ...
                !isIpcpAddrConfigured(ipIntfFullPath, ipcpObj->localIPAddress))
            {
         ...
         */
             ...
             !cmsUtl_strcmp(newObj->X_BROADCOM_COM_UserRequest, MDMVS_UP))
         {
            ...
            CMSMEM_REPLACE_STRING_FLAGS(newObj->X_BROADCOM_COM_UserRequest, MDMVS_NONE, mdmLibCtx.allocFlags);
                  ...
                  !cmsUtl_strcmp(currObj->connectionStatus, MDMVS_DISCONNECTED))
         {
            cmsLog_debug("case 3: connstatus %s => DISCONNECTED",
            ...
            */
            ...
            else if (newObj->idleDisconnectTime == 0)
            {
               /* Not in ppp on demand mode, so indicate WAN disconnected.
                * (if we were in on demand mode, would leave the WAN light on/green)
                */
            ...
            * Otherwise, keep pppd running when ConnectionStatus = "Disconnected" for following 2 cases:
            * 1). ppp connection is down - lastConnectionError = "ERROR_UNKNOWN",
            * 2). ppp idle time out - lastConnectionError is "ERROR_IDLE_DISCONNECTED",
            *
            ...
            */
            {
               ...
               {
                  /* Lower layer is up, do not kill pppd */

                  /* also check for on-demand stuff */
                  if (newObj->idleDisconnectTime > 0)
                  {
                     /* for on demand, need to redirect lan traffic to ppp interface.
                      * The iptables has to be in first and 2 ip rules to redirect the lan tranffic
                      */
                     if( !cmsUtl_strcmp(currObj->status, MDMVS_LOWERLAYERDOWN) || !cmsUtl_strcmp(currObj->status, MDMVS_DOWN))
                        cmsLog_debug("current status is Down");
                     else
                        rutIpt_setDefaultGwForPppOnDemand(newObj->name);
#ifdef SUPPORT_FCCTL
                     char cmd[BUFLEN_64];
                     snprintf(cmd, sizeof(cmd), "fcctl flush --if %s --silent ", newObj->name);
                     rut_doSystemAction(__FUNCTION__, cmd);
#endif
                  }

                  /*
                   * dsl_md may not update ATM.Link.i.Status promptly.
                   * For PPPoA case, verify newObj->status to double check
                   */
                  if (cmsUtl_strstr(newObj->name, PPPOA_IFC_STR) && 
                      !cmsUtl_strcmp(newObj->status, MDMVS_LOWERLAYERDOWN))
                  {
                     cmsLog_debug("kill pppd for PPPoA with status lowerlayerdown");
                     if (currObj->X_BROADCOM_COM_Pid != CMS_INVALID_PID)
                     {
                        rutPpp_stopPppd_dev2(currObj->X_BROADCOM_COM_Pid,
                                             currObj->lowerLayers);
               ...
               {
                  /* lower layer is down --> kill pppd if it is running */
                  if (currObj->X_BROADCOM_COM_Pid != CMS_INVALID_PID)
                  {
                     rutPpp_stopPppd_dev2(currObj->X_BROADCOM_COM_Pid,
                                          currObj->lowerLayers);
