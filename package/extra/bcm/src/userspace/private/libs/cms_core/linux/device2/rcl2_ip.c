...
{
   // TODO: the Device.IP. object is in both sysmgmt and diag components.
   // The read-only params: IPv4Capable, IPv4Status, Ipv6Capable, IPv6Status
   // should be the same in both components.
   // But if we get a SetParameterValues from tr69c for Device.IP.IPv4Enable
   // or IPv6Enable, remote_objd will send it to sysmgmt component because diag
   // component only claims the Device.IP.Diagnostics. namespace, but then the
   // new IPv4/IPv6 status needs to be reflected in the diag component.
   // Same issue goes for ULAPrefix.  (for ULAPrefix, probably ssk in sysmgmt
   // will set it in the sysmgmt component).
   // Fortunately, it is unlikely the ACS will try to enable/disable the entire
   // IPv4 and IPv6 stack.  Not sure how common ULAPrefix will change or if we
   // even set it in Device.IP.
   // Currently the values are all messed up (e.g. IPv4Status == Disabled).
   // Probably broken since the beginning, but would be good to fix.
   // InterfaceNumberOfEntries and ActivePortNumberOfEntries are ok because
   // they are added in phl_merge.c.
                ...
                CmsRet *errorCode __attribute__((unused)))
{
   ...
   if (ENABLE_NEW_OR_ENABLE_EXISTING(newObj, currObj))
   {
	  ...
	  */
      ...
      if (newObj->X_BROADCOM_COM_Upstream)
      {
         ...
         rutEbt_IPv6PrefixExceptionIf(newObj->name, TRUE);
#endif
      }
   ...
   */
   ...
   else if (DELETE_OR_DISABLE_EXISTING(newObj, currObj))
   {
      ...
      if (currObj->X_BROADCOM_COM_Upstream)
         rutEbt_IPv6PrefixExceptionIf(currObj->name, FALSE);
#endif
                ...
                CmsRet *errorCode __attribute__((unused)))
{
   ...
   if (newObj && newObj->enable)
   {
          ...
          (cmsUtl_strcmp(newObj->status, MDMVS_ENABLED)))
      {
         ...
         {
            /* In some unusual configs, a Bridged WAN service
   ...
   {
      /* we only unconfigure this IP if this is the active IP address */
      if (currObj->enable)
      {
         rutIp_unconfigureIpv4Addr(ipIntfObj->name);
      }
                ...
                CmsRet *errorCode __attribute__((unused)))
{
   ...
   if (ADD_NEW(newObj, currObj))
   {
      rutUtil_modifyNumIpv6Address(iidStack, 1);
   }
   ...
   else if (DELETE_OR_DISABLE_EXISTING(newObj, currObj))
   {
      ...
      else if (!cmsUtl_strcmp(currObj->origin, MDMVS_STATIC))
      {
         /*
          * Notify ssk so it can update IP.Interface.Status and IPv6
          * Service state machine
                ...
                CmsRet *errorCode __attribute__((unused)))
{
   ...
   if (ADD_NEW(newObj, currObj))
   {
      rutUtil_modifyNumIpv6Prefix(iidStack, 1);
   }
   ...
   if (ENABLE_NEW_OR_ENABLE_EXISTING(newObj, currObj))
   {
      ...
      if (!ipIntfObj->X_BROADCOM_COM_Upstream && !IS_EMPTY_STRING(newObj->prefix))
      {
        rutIpt_configRoutingChain6(newObj->prefix, ipIntfObj->name, TRUE);
        rutEbt_configIPv6Prefix(newObj->prefix, TRUE);
      }
   }
   else if (POTENTIAL_CHANGE_OF_EXISTING(newObj, currObj))
   {
      if (!IS_EMPTY_STRING(newObj->prefix))
      {
         if (cmsUtl_strcmp(newObj->prefix, currObj->prefix))
         {
            rutEbt_configIPv6Prefix(newObj->prefix, TRUE);
            rutEbt_configIPv6Prefix(currObj->prefix, FALSE);
         }
      }
      else
      {
   ...
   else if (DELETE_OR_DISABLE_EXISTING(newObj, currObj))
   {
      ...
      if (!ipIntfObj->X_BROADCOM_COM_Upstream && !IS_EMPTY_STRING(currObj->prefix))
      {
        rutIpt_configRoutingChain6(currObj->prefix, ipIntfObj->name, FALSE);
        rutEbt_configIPv6Prefix(currObj->prefix, FALSE);
      }
