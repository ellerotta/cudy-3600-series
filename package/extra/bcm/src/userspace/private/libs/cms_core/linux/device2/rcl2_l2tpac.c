                ...
                CmsRet *errorCode __attribute__((unused)))
{
   ...
   if (ENABLE_NEW_OR_ENABLE_EXISTING(newObj, currObj))
   {
      ...
      {
          strncpy(l2tpAcUsername, pppIntfObj->username, sizeof(l2tpAcUsername)-1);
          l2tpAcUsername[sizeof(l2tpAcUsername)-1] = '\0';	
          	  
          strncpy(l2tpAcPassword, pppIntfObj->password, sizeof(l2tpAcPassword)-1);
          l2tpAcPassword[sizeof(l2tpAcPassword)-1] = '\0';	
          ppp_debug = pppIntfObj->X_BROADCOM_COM_Enable_Debug;        
          cmsObj_free((void **) &pppIntfObj);
          if ((ret = rutL2tpAC_start_dev2(newObj->tunnelName, newObj->lnsIpAddress, l2tpAcUsername, l2tpAcPassword, ppp_debug)) != CMSRET_SUCCESS)
          {
   ...
   {
      rutL2tpAC_stop_dev2(currObj->tunnelName);
   }
