...
//   REPLACE_STRING_IF_NOT_EQUAL_FLAGS(newObj->X_BROADCOM_COM_DefaultIPv6Gateway, "", mdmLibCtx.allocFlags);
    ...
    */
   if (0 > snprintf(intf_info, sizeof(intf_info), "%s__%s", newObj->X_BROADCOM_COM_IfName, l2IfName))
   {
      cmsLog_error("snprintf failed to write '%s__%s' into intf_info", newObj->X_BROADCOM_COM_IfName, l2IfName);
