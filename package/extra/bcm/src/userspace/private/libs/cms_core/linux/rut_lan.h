 ...
 * @param isNewBridge (IN) True if this a new bridge that needs to be created.
 * @param isOVS (IN) True if this is an OVS bridge, otherwise it is a Linux bridge.
 ...
 * @return CmsRet enum.
 */
CmsRet rutLan_enableBridge(const char *bridgeIfName,
                         UBOOL8 isNewBridge,
                         UBOOL8 isOVS,
                         const char *ipAddr,
                         const char *netmask,
...
/** Add the specified LAN or WAN interface to the specified Linux bridge
 * 
 ...
 * @param isWanIntf    (IN) is the first interface a WAN interface? 
 * @param bridgeIfName (IN) bridge interface name
 * @param isOVS        (IN) is the bridge an OVS bridge?
 *
 */
void rutLan_addInterfaceToBridgeEx(const char *lanIfName, UBOOL8 isWanIntf, const char *bridgeIfName, UBOOL8 isOVS);


/** Add the specified LAN dynamic interface to the specified bridge
 *
 * @param lanIfName    (IN) interface name
 * @param bridgeIfName (IN) bridge interface name
 * @param isOVS        (IN) is the bridge an OVS bridge?
 *
 */
...
/** Remove the specified LAN interface from the specified Linux bridge.
 *
 * @param lanIfName   (IN) lan interface name
 * @param bridgeIfName (IN) bridge interface name
 *
 ...
 * 
 */
void rutLan_removeInterfaceFromBridgeEx(const char *lanIfName, const char *bridgeIfName, UBOOL8 isOVS);


/** Remove the specified LAN dynamic interface from the specified bridge.
 *
 * @param lanIfName    (IN) lan interface name
 * @param bridgeIfName (IN) bridge interface name
 * @param isOVS        (IN) is the bridge an OVS bridge?
 ...
 *  and is in the Connected state.  E.g for in homeplug devices.
 ...
 *
 * @param bridgeIfName (IN) The LAN device bridge name.
 * @param oldIpAddr (IN) The previous brx IP Address.
 * @param oldMask   (IN) The previous brx netmask;
 * @param newIpAddr (IN) The new brx IP address.
 * @param newMask   (IN) The new brx netmask;
 */
...
/** Restart the dhcpv6 server on the specified interface. If ifName is NULL then 
 * use the default interface "br0".
 * 
 * @param dns6Servers (IN) The dns server addresses to be advertise.
 * @param ipv6LanCfgObj (IN) 
 * 
 * @returns pid of the newly created dhcp6s instance and the error code.
 */
...
/** Stop the dhcpv6 server on the specified interface.
 * if ifName is NULL then use the default interface "br0".
 * 
 * @return CmsRet enum.
 */
...
/** Add Dynamice interface to a bridge.
 *
 ...
 * @return CmsRet
 */
void rutLan_addDynIntfToBridge(const char *intfName, const char *brIntfName);


/** Remove the specified interface from bridge.
 *
 * @param intfName   (IN) name of the interface to remove from bridge
 * @param brIntfName (IN) name of the bridge
 *
 */
void rutLan_deleteDynIntfFromBridge(const char *intfName, const char *brIntfName);
