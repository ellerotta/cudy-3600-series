...
CmsRet rutIPSec_config_dev2(void)
{
                    ...
                    OGF_NO_VALUE_UPDATE, (void **)&filterObj) == CMSRET_SUCCESS)
   {
      ...
      if(!cmsUtl_strcmp(profileObj->X_BROADCOM_COM_KeyExchangeMode, "auto"))
      {
         ...
         fprintf(fp_conf, "\treplay_window=%d\n", ikeObj->phase2AntiReplayWindow);
         ...
         {
            fprintf(fp_conf, "-%s", ikeObj->phase2AllowedDiffieHellmanGroupTransforms);
         }
      ...
      else if(!cmsUtl_strcmp(profileObj->X_BROADCOM_COM_KeyExchangeMode, "autov2"))
      {
         Ikev2CfgObject *ike2Obj = NULL;

         if (cmsObj_get(MDMOID_IKEV2_CFG, &pathDesc.iidStack,
                     OGF_NO_VALUE_UPDATE, (void *)&ike2Obj) != CMSRET_SUCCESS)
         {
            cmsLog_error("cannot get ikev2Obj");
            cmsObj_free((void **)&filterObj);
            cmsObj_free((void **)&profileObj);
            continue;
         }
         //This is for this specific connection
         fprintf(fp_conf, "conn \"%s\"\n", filterObj->X_BROADCOM_COM_TunnelName);
         fprintf(fp_conf, "\tkeyexchange=ikev2\n");
         fprintf(fp_conf, "\tleft=%s\n", wanIP);
         fprintf(fp_conf, "\tleftsubnet=%s/%d\n",
                 filterObj->sourceIP, local_prefixlen);
         fprintf(fp_conf, "\tright=%s\n", profileObj->remoteEndpoints);
         ...
         if (!cmsUtl_strcmp(ike2Obj->authenticationMethod, "pre_shared_key")) 
         {
            fprintf(fp_conf, "\tauthby=secret\n");
            fprintf(fp_secret, "%s : PSK \"%s\"\n",
                    profileObj->remoteEndpoints, ike2Obj->preSharedKey);
         } 
         else if (!cmsUtl_strcmp(ike2Obj->authenticationMethod, "certificate"))
         {
            fprintf(fp_conf, "\tauthby=rsasig\n");
            fprintf(fp_conf, "\tleftcert=/var/cert/%s.cert\n",
                    ike2Obj->certificateName);
            fprintf(fp_conf, "\tleftsendcert=always\n");  //options: ifasked, never, always
            fprintf(fp_secret, ": RSA /var/cert/%s.priv\n",
                    ike2Obj->certificateName);
         } /* unknown type */

         /* Phase 1 Settings */
         fprintf(fp_conf, "\tikelifetime=%ds\n", ike2Obj->phase1KeyTime);
         fprintf(fp_conf, "\tike=%s-%s-%s\n", ike2Obj->phase1AllowedEncryptionAlgorithms,
                 ike2Obj->phase1AllowedIntegrityAlgorithms,
                 ike2Obj->phase1AllowedDiffieHellmanGroupTransforms);

         /* Phase 2 Settings */
         fprintf(fp_conf, "\tlifetime=%ds\n", ike2Obj->phase2KeyTime);
         fprintf(fp_conf, "\treplay_window=%d\n", ike2Obj->phase2AntiReplayWindow);
         /* Adjust algo name for backward compatibility */
         adj_int_name = ike2Obj->phase2AllowedIntegrityAlgorithms;
         if (strncmp(adj_int_name, old_algo_hmac_prefix, 
                     strlen(old_algo_hmac_prefix)) == 0)
         {
            adj_int_name += strlen(old_algo_hmac_prefix)+1;
         }
         adj_enc_name = ike2Obj->phase2AllowedEncryptionAlgorithms;
         if (0 == strcmp(adj_enc_name, "null_enc")) {
            adj_enc_name = "null";
         }
         if (!cmsUtl_strcmp(profileObj->protocol, "ESP"))
         ...
         /*PFS guarantees that the encryption keys for IPsec SA negotiations are created 
         * separately for each negotiation. if PFS is not turned on then the current keying 
         * material already established at phase 1 will be used again to generate phase 2 SA
         */
         if (ike2Obj->perfectFSEn) 
         {
            fprintf(fp_conf, "-%s", ike2Obj->phase2AllowedDiffieHellmanGroupTransforms);
         }

         if(ike2Obj->phase2ESN)
            fprintf(fp_conf, "-esn");      
    
         fprintf(fp_conf, "\n");       

         cmsObj_free((void **)&ike2Obj);
      }
      ...
      {
          char cmd[BUFLEN_256];
          snprintf(cmd, sizeof(cmd), "iptables -w -t nat -I POSTROUTING 1 -s %s/%d -o %s -d %s/%d -j ACCEPT",
                       filterObj->sourceIP, local_prefixlen,
      ...
      if (filterObj->X_BROADCOM_COM_L2TP_over_IPSec == 1)
      {
         fprintf(fp_conf, "\ttype=transport\n");
         if(filterObj->X_BROADCOM_COM_LNS_L2TP_PORT == 1)
         {
            fprintf(fp_conf, "\tleftprotoport=17/%d\n", DEFAULT_L2TP_PORT);
            fprintf(fp_conf, "\trightprotoport=17/%%any\n");
         }
         else
         {
            fprintf(fp_conf, "\tleftprotoport=17/%%any\n");
            fprintf(fp_conf, "\trightprotoport=17/%d\n", DEFAULT_L2TP_PORT);
         }
      }
