...
CmsRet mdm_serializeToBuf(char *buf, UINT32 *len)
{
   ...
   attr.altNotif = DEFAULT_ALTNOTIFICATION_VALUE;
                      ...
                      UINT32 max)
{
       ...
       (IS_INDIRECT2(objNode) && (DEPTH_OF_IIDSTACK(iidStack) == objNode->instanceDepth)))
   {
          ...
          (hasAnyParamsOrAttrsChanged(objNode, iidStack, parentAttr) && !(objNode->flags & OBN_PRUNE_WRITE_TO_CONFIG_FILE)))
      {
         ...
         if (ret != CMSRET_SUCCESS || (currAttr == NULL))
         {
         ...
         if (indentBuf2 != NULL) {
            ...
            *idx += (wrote < max - (*idx)) ? wrote : max - (*idx);
            ...
            /* check for alternative changed notification settings */
            if (currAttr->altNotif!= parentAttr->altNotif)
            {
               wrote = snprintf(&buf[*idx], max - (*idx), " %s=\"%d\"", CONFIG_FILE_ATTR_ALTNOTIFICATION, currAttr->altNotif);
               *idx += (wrote < max - (*idx)) ? wrote : max - (*idx);
            }
   ...
   else if (IS_INDIRECT2(objNode) && (DEPTH_OF_IIDSTACK(iidStack)+1 == objNode->instanceDepth))
   {
      ...
      if ((instHead = mdm_getInstanceHead(objNode, iidStack)) == NULL)
      {
         cmsLog_error("could not find instance head for %s",
                      cmsMdm_dumpIidStack(iidStack));
      }
      else      
                                        ...
                                        void *cbContext)
{
   ...
   if ((!mdmLibCtx.dumpAll) &&
       ((paramNode->flags & PRN_NEVER_WRITE_TO_CONFIG_FILE) ||
        (((paramNode->flags & (PRN_ALWAYS_WRITE_TO_CONFIG_FILE|PRN_COUNT_PERSISTENT_INSTANCE)) == 0) && (!IS_PARAM_WRITABLE(paramNode)))))
   {
      /*
      ...
      { 
         ...
         if (paramNode->type == MPT_INTEGER || 
             ...
             paramNode->type == MPT_STATS_COUNTER32 ||
             paramNode->type == MPT_LONG64 ||
             paramNode->type == MPT_UNSIGNED_LONG64 ||
             paramNode->type == MPT_STATS_COUNTER64 ||
             paramNode->type == MPT_DECIMAL) 
         {
            sprintf(valBuf, "0");
         }
   ...
   if (mdmLibCtx.dumpAll || ctx->changed)
   {
      if (ctx->buf != NULL)
      {
         ...
         /* Is there an attrDesc for this param and is it different from parent obj node? */
         if ((attrDesc != NULL) &&
             (attrDesc->nodeAttr.altNotif!= ctx->nodeAttr->altNotif))
         {
            wrote = snprintf(&(ctx->buf[ctx->idx]), ctx->max - ctx->idx, " %s=\"%d\"", CONFIG_FILE_ATTR_ALTNOTIFICATION, attrDesc->nodeAttr.altNotif);
            ctx->idx += (wrote < ctx->max - ctx->idx) ? wrote : ctx->max - ctx->idx;
         }
         ...
         {
            if (paramNode->flags & PRN_CONFIG_PASSWORD && currStr != NULL)
            {
