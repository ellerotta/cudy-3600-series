                                ...
                                CmsRet *errorCode  __attribute__((unused)))
{
   ...
   if (newObj != NULL && currObj == NULL)
   {
      ...
      */
      if (strcmp(newObj->IPv6DNSConfigType, MDMVS_STATIC) == 0)
      {
         ...
         {
            ret = rut_restartDhcp6s(NULL, &(newObj->X_BROADCOM_COM_Dhcp6sPid),
                                    newObj->IPv6DNSServers, newObj->IPv6DomainName, 
                                    newObj->statefulDHCPv6Server, newObj->minInterfaceID, 
                                    newObj->maxInterfaceID, newObj->DHCPv6LeaseTime, NULL);
            if (ret != CMSRET_SUCCESS)
            {
               cmsLog_error("rut_restartDhcp6s returns error");
               return ret;
            }
         }
      }
   ...
   else if (newObj != NULL && currObj != NULL)
   {
      ...
      */
      ...
      {
         rut_stopDhcp6s(NULL, newObj->X_BROADCOM_COM_Dhcp6sPid);
         newObj->X_BROADCOM_COM_Dhcp6sPid = 0;
      }
      ...
      if (newObj->DHCPv6ServerEnable)
      {
         if (strcmp(newObj->IPv6DNSConfigType, MDMVS_STATIC) != 0)
         {
            ...
            if (rut_isWanInterfaceUp(newObj->IPv6DNSWANConnection, FALSE))
            {
               ...
               if ((ret = rut_restartDhcp6s(NULL, &(newObj->X_BROADCOM_COM_Dhcp6sPid),
                                           dnsServer, domainName, newObj->statefulDHCPv6Server,
                                           newObj->minInterfaceID, newObj->maxInterfaceID,
                                           newObj->DHCPv6LeaseTime, sitePrefix)) != CMSRET_SUCCESS)
               {
                  cmsLog_error("rut_restartDhcp6s returns error.");
               }
            }
            ...
            {
               rut_stopDhcp6s(NULL, newObj->X_BROADCOM_COM_Dhcp6sPid);
               newObj->X_BROADCOM_COM_Dhcp6sPid = CMS_INVALID_PID;
            }
         }
         else
         {
