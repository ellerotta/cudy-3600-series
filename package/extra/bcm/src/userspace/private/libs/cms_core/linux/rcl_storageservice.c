                                 ...
                                 CmsRet *errorCode __attribute__((unused)))
{
   ...
   if(ENABLE_NEW_OR_ENABLE_EXISTING(newObj,currObj))
   {
      ...
      snprintf(cmd, sizeof(cmd), "ln -s %s /var/samba/share/%s_share",tmpBuf,newObj->name);
   ...
   {
      snprintf(cmd, sizeof(cmd), "rm /var/samba/share/%s_share",currObj->name);
      rut_doSystemAction("storage:samba", cmd);
   }
#endif
                                 ...
                                 CmsRet *errorCode __attribute__((unused)))
{
   ...
   if (ENABLE_NEW_OR_ENABLE_EXISTING(newObj, currObj))
   {
      sprintf(volumeName, "%s", newObj->X_BROADCOM_volumeName);
      /* Replace the invalid character with '_' */
      for ( i = 0; i < strlen(volumeName); i++ )
      {
          if ( volumeName[i] != '-' && volumeName[i] != '_' && volumeName[i] != '.' && !isalpha(volumeName[i]) && !isdigit(volumeName[i]) )
          {
             volumeName[i]= '_'; 
          }
      }
      sprintf(userName, "%s", newObj->username);
      /* Replace the invalid character with '_' */
      for ( i = 0; i < strlen(userName); i++ )
      {
          if ( userName[i] != '-' && userName[i] != '_' && userName[i] != '.' && !isalpha(userName[i]) && !isdigit(userName[i]) )
          {
             userName[i]= '_'; 
          }
      }
      sprintf(homeDir, "/mnt/%s/%s", volumeName, userName);

      if((ret=rutStorage_addUserAccount(userName, newObj->password, homeDir))!= CMSRET_SUCCESS)
      {
         return ret;
      }
   ...
   {
      sprintf(volumeName, "%s", currObj->X_BROADCOM_volumeName);
      /* Replace the invalid character with '_' */
      for ( i = 0; i < strlen(volumeName); i++ )
      {
          if ( volumeName[i] != '-' && volumeName[i] != '_' && volumeName[i] != '.' && !isalpha(volumeName[i]) && !isdigit(volumeName[i]) )
      ...
      sprintf(userName, "%s", currObj->username);
      /* Replace the invalid character with '_' */
      for ( i = 0; i < strlen(userName); i++ )
      {
          if ( userName[i] != '-' && userName[i] != '_' && userName[i] != '.' && !isalpha(userName[i]) && !isdigit(userName[i]) )
      ...
      if((ret=rutStorage_deleteUserAccount(userName, homeDir))!= CMSRET_SUCCESS)
      {
         return ret;
      }
   }
