...
#ifdef DMP_X_BROADCOM_COM_RPSRFS_1
            rutRpsRfs_checkSet(obj->X_BROADCOM_COM_IfName);
#endif
         }
...
//         }
   ...
   if (obj->X_BROADCOM_COM_IfName)
   {
                               ...
                               ipAddr, &ifIndex, &prefixLen, &scope, &ifaFlags) == CMSRET_SUCCESS)
      {
         ...
         if (0 > snprintf(address, CMS_IPADDR_LENGTH, "%s/%d", ipAddr, prefixLen))
         {
            cmsLog_debug("snprintf failed to write '%s/%d' into address", ipAddr, prefixLen);
            address[0] = 0;
         }
