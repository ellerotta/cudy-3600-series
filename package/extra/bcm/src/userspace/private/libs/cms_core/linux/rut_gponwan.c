                                     ...
                                     char *pL2Ifname)
{
   ...
   InstanceIdStack wanConnIid = EMPTY_INSTANCE_ID_STACK;
   ...
   if (!IS_EMPTY_STRING(l2LinkCfgObj->ifName))
   {
   ...
   switch (oid)
   {
      case MDMOID_WAN_IP_CONN:
         ...
         else
         {
            ...
            cmsLog_debug("IP-CONN:<%d:%d> <%d %d %d>", pServiceParams->serviceStatus,ipConn->enable,
                         pServiceParams->pbits,pServiceParams->vlanId,pServiceParams->igmpEnabled);
...
#endif
            cmsLog_debug("PPP-CONN:<%d:%d> <%d %d %d>", pServiceParams->serviceStatus,pppConn->enable,
                         pServiceParams->pbits,pServiceParams->vlanId,pServiceParams->igmpEnabled);
            /* Free the PPP object */
            cmsObj_free((void**)&pppConn);
         }
