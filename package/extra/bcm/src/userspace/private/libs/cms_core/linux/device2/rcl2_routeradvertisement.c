                ...
                CmsRet *errorCode __attribute__((unused)))
{
   ...
   static UBOOL8 rastatus6Started = FALSE;

   if (ADD_NEW(newObj, currObj))
   {
      UINT32 pid;

      rutUtil_modifyNumRaInterfaceSetting(iidStack, 1);

      if (mdmShmCtx->inMdmInit && rastatus6Started == FALSE)
      {
         /* launch rastatus6 here at system bootup time */
         cmsLog_notice("starting rastatus6...");
         if ((pid = rut_sendMsgToSmd(CMS_MSG_START_APP, EID_RASTATUS6,  NULL, 0)) == CMS_INVALID_PID)
         {
            cmsLog_error("failed to start rastatus6");
            ret = CMSRET_INTERNAL_ERROR;
         }
         else
         {
            cmsLog_notice("rastatus6 started with pid=%d", pid);
            rastatus6Started = TRUE;
         }
      }
      ...
      if (newObj->enable)
      {
         ...
         cmsLog_notice("creating ra prefixes");
         rutRa_updatePrefixes(newObj->interface, &(newObj->prefixes), &foundPD, &isIPv6Rd);
         cmsLog_notice("updating radvd conf");
         rutRa_createRadvdConf(foundPD, isIPv6Rd);
         if (isIntfHasAddr6(radvdInterface) == CMSRET_SUCCESS)
            rut_restartRadvd();
      }
   }
