...
void rutNtpd_restart_dev2(void)
{
   ...
   if (cmsObj_get(MDMOID_DEV2_TIME, &iidStack, 0, (void **) &timeObj) == CMSRET_SUCCESS)
   {
      ...
      if(timeObj->NTPServer1 != NULL && timeObj->NTPServer1[0] != '\0' )
      {
         ...
         {
            strcat(servers, " -p " );
            strncat(servers, timeObj->NTPServer2, sizeof(servers)-1);
            servers[sizeof(servers)-1] = '\0';            
         }
         if(timeObj->NTPServer3 != NULL && timeObj->NTPServer3[0] != '\0' )
         {
            strcat(servers, " -p " );
            strncat(servers, timeObj->NTPServer3, sizeof(servers)-1);
            servers[sizeof(servers)-1] = '\0';             
         }
         if(timeObj->NTPServer4 != NULL && timeObj->NTPServer4[0] != '\0' )
         {
            strcat(servers, " -p " );
            strncat(servers, timeObj->NTPServer4, sizeof(servers)-1);
            servers[sizeof(servers)-1] = '\0';              
         }
         if(timeObj->NTPServer5 != NULL && timeObj->NTPServer5[0] != '\0' )
         {
            strcat(servers, " -p " );
            strncat(servers, timeObj->NTPServer5, sizeof(servers)-1);
            servers[sizeof(servers)-1] = '\0';               
         }

         snprintf(cmd, sizeof(cmd), "-n %s", servers);
      ...
      if (timeObj->X_BROADCOM_COM_LocalTimeZoneName)
      {
         ...
         iso8601TimeZoneName = rutTimezone_lookupIso8601(fileName);
      }
      else if (timeObj->localTimeZone)
      {
         iso8601TimeZoneName = rutTimezone_lookupIso8601(timeObj->localTimeZone);
      }
      if (iso8601TimeZoneName) {
         rutTimezone_writeTzFiles(iso8601TimeZoneName);
      }
