...
#define MDM_BBF_PROTO_LEN      8
#define MDM_BBF_PROTO_COMMON   "common"
#define MDM_BBF_PROTO_USP      "usp"
#define MDM_BBF_PROTO_CWMP     "cwmp"

/** Get bbfProtocol of object/parameter.
 *
 * We only want to send CWMP parameters to an ACS, and USP ones to an USP controller.
 * This entry point allows other parameter walking functions,
 * (e.g. phl_getNextPath, mdm_getNextChildObjPathDesc, etc). to determine
 * the protocol of this object or parameter: common, usp or cwmp.
 * From an architectural perspective, tr69c/USP should do the
 * filtering by protocol.  For USP, we pick only USP parameters and register them to the 
 * obuspa agent.   Any query will contain only registered parameters so there is no need
 * for filtering out cwmp ones.  But for TR69c, this filtering is done the same way as 
 * hidden from ACS attribute mechanism 
 *
 * @param      (IN)  Path descriptor pointing to the object or parameter in question.
 * @param      (IN)  String to hold the returned protocol string: common, usp or cwmp
 * @param      (IN)  Max len of string 
 *
 * @return CmsRet enum.
 */
CmsRet mdm_getPathDescProto(const MdmPathDescriptor *pathDesc, char *proto, int len);
...
CmsRet getObjectWritable(const MdmObjectNode *objNode, UBOOL8 *writable);
CmsRet getParamWritable(const MdmParamNode *paramNode, UBOOL8 *writable);
...
/** Get number of parameters whose value has changed and who has
 *  altchange notification attribute.
 *
 * @return The total number of parameter value changes.
 */
UINT32 mdm_getNumberOfAltParamValueChanges(void);
...
/** Clear alternative Parameter Value Change status for all values in the MDM.
 *
 * This function clears the status of all the alt-parameter value changes.
 */
void mdm_clearAllAltParamValueChanges(void);
...
/** Return the local MdmObjectNode with the given oid.
 *
...
/** Return the local root object node
 *
 * @return pointer to MdmObjectNode
 */
MdmObjectNode *mdm_getRootObjectNode(void);


#define GET_OBJNODE_LOCAL   0x01
#define GET_OBJNODE_REMOTE  0x02
#define GET_OBJNODE_ANY     (GET_OBJNODE_LOCAL | GET_OBJNODE_REMOTE)

/** Return the MdmObjectNode with the given oid and getFlags.
 *
 * @param oid (IN) MdmObjectId.
 * @param getFlags (IN) Bitmask of GET_OBJNODE_FLAGS
 * @param actualFlags (OUT) if not NULL, will show where objnode was found.
 *
 * @return pointer to MdmObjectNode or NULL if oid is invalid.
 */
...
/** Get the MdmObjectNode from the local OID table in mdmShmCtx. */
MdmObjectNode *mdm_getLocalObjNode(MdmObjectId oid);

/** Get the MdmObjectNode from the remote OID table in mdmShmCtx. */
...
// Allows customers to customize the semaphore keys used by binary only libmdm_db.so.
key_t mdm_getZoneLockSemaphoreKey(void);
key_t mdm_getMetaLockSemaphoreKey(void);
...
// Return TRUE if this fullpath DEFINITELY points to a local object.
// Prefer to use the local version of this function because we know for sure
// which objects are local.  A "hint" can be included in the alias portion of
// the fullpath, and the hint tells MDM which component this fullpath should
// be sent to.  (currently, only the last instance/alias is considered).
// This is currently used by httpd and CMS DAL code (in sysmgmt) to create
// remote QoS.Queue objects in DSL component.
// Example of hint (component name is somewhere in alias):
// Device.QoS.Queue.[DSL-qos-queue1].
// If the alias does not contain a component name, then we assume it is local.
UBOOL8 mdm_isLocalFullpath(const char *fullpath);

// Return TRUE if this fullpath is possibly a remote object.
UBOOL8 mdm_isPossibleRemoteFullpath(const char *fullpath);


// Return TRUE if this OID is a table that spans multiple components for
// a cmsObj_getNext.  See additional comments in mdm_binaryHelper.c.
...
#define IS_EXTERNAL_CALLER (lck_getTotalZoneEntryCounts() <= 1)


/** Evaluates to true if pointer was allocated from shared memory. */
#define IS_IN_SHARED_MEM(p) ((((void *)(p)) >= mdmShmCtx->mallocStart) && (((char *)(p)) < mdmShmCtx->shmEnd))

/** This component is able to do remote MDM access (only sysmgmt can). */
#define IS_REMOTE_OBJ_CAPABLE  (mdmShmCtx->isRemoteCapable)
...
/** This structure maps an OID to a objNode in the shared memory region. */
typedef struct
{
   MdmObjectId    oid;
...
typedef struct
{
   OidNodeEntry *localOidNodeTableBegin; /**< sparse ordered array of OidNodeEntry's */
   OidNodeEntry *localOidNodeTableEnd;
   OidNodeEntry *remoteOidNodeTableBegin;  /**< sparse ordered array of OidNodeEntry's.  This ptr may be null. */
   OidNodeEntry *remoteOidNodeTableEnd;    /**< this ptr may be null. */
   MdmObjectNode *rootObjNode;  /**< The root node of the MDM (either InternetGatewayDevice. or Device )*/
   ...
   UINT32 numValueChanges;  /**< Number of parameters with active or passive notification 
                             ...
                             *   other than tr69c.
                             */
   UINT32 numAltValueChanges;  /**< Number of parameters with altchange notification 
                                *   that has had their value changed by a management entity
                                */
   ...
   UBOOL8      isRemoteCapable; /**< This distributed MDM is able to do remote obj access. */
   CmsEntityId    intfStackEid; /**< Optional: who gets intfStack msgs.  Set by initializer of MDM. */
   SINT32        lockKeyOffset; /**< Set by initializer of MDM. */
   UINT32 multiCompFirstInstanceId; /**< Set by initializer of MDM (only used if object has OBN_MULTI_COMP_OBJ) */
   UINT32 multiCompLastInstanceId;  /**< same as above */
   MdmLockMetaInfo *lockMeta;    /**< MDM zone lock meta-info table */
   char compName[BDK_COMP_NAME_LEN+1]; /**< Distributed MDM component name (BDK only) */
   rbtree_t typesRBT;
   rbtree_t profilesRBT;
   rbnode_t nullNode;   
   UINT32   flags;
} MdmSharedMemContext;
...
// Per-thread info, used in MdmLibraryContext
typedef struct
{
   SINT32   threadId;  /**< this is used as the lookup key */
   void    *msgHandle; /**< to be used by this thread when sending/receiving messages from MDM. */
} MdmLibraryThreadContext;

#define MDM_LIB_CTX_NUM_THREAD_SLOTS   4
...
typedef struct
{
   ...
   void *msgHandle; /**< Message Handle of the app.  New code should get the
                     *   msgHandle by calling cmsMdm_getThreadMsgHandle(). */
   MdmLibraryThreadContext threadCtx[MDM_LIB_CTX_NUM_THREAD_SLOTS];  /**< new code should call cmsMdm_getThreadMsgHandle() which gets the correct msgHandle from this array, but old code can still use msgHandle above.  If only a single thread accesses the MDM, they are the same. */
...
/** Per process data for the cms_core lib.  */
extern MdmLibraryContext mdmLibCtx;

/** Global data for cms_core and shared mem, visible and shared by all apps attached to this shared mem region. */
