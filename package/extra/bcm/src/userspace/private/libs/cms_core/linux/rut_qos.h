 ...
 *  It configures all types of queues (XTM, ETH, PON, ...).
 ...
 *  changes.  It contains high level logic for reconfiguring queues.
...
/* Only TR98 code call rutQos_qMgmtQueueReconfig() */
 ...
 *  logic for QoS classifications.  (Classifications includes policing and rate limiter).
 *
 * @param ifname     (IN) name of interface that changed link status
 * @param isLayer2 (IN) TRUE if ifname is layer2 interface name. Otherwise, FALSE.
 * @return CmsRet         enum.
 */
CmsRet rutQos_qMgmtClassReconfig(char *ifname, UBOOL8 isLayer2);
CmsRet rutQos_qMgmtClassReconfig_igd(char *ifname, UBOOL8 isLayer2);

/* No data model switching macro needed for rutQos_qMgmtClassReconfig */
/* Only TR98 code call rutQos_qMgmtClassReconfig() */
#define rutQos_qMgmtClassReconfig(i, b)  rutQos_qMgmtClassReconfig_igd((i), (b))
...
/** This API include the actions target to avoid hardware flow 
 * pointing to an queue whose change is under change.
 */
void rutQos_tmQueueConfig_pre(const char* l2ifname);

/** This API target to undo the actions done in API rutQos_tmQueueConfig_pre.
 */
...
/** Flush accelerated flows.
 * If ifname not specified, don't care which interface, flush all;
 * If ifname specified, flush flows related.
 */
void rutQos_flushFlows(const char* ifname);
...
/** TCP Pure-ACK: Configure TCP Pure-ACK Netfilter rules for this L2 / L3 interface.
 *  These functions will get the priority level of the TCP Pure-ACK queue given L2 / L3 interface.
 *  And set the skb->mark of TCP Pure-ACK egress to this L2 / L3 interface with this priority level.
*/
