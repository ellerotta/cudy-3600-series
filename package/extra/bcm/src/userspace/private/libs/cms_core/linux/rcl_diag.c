   ...
   if (cmsUtl_strcmp(newObj->diagnosticsState,MDMVS_REQUESTED) == 0)
   {
      ...
      if (mdmLibCtx.eid != EID_TR69C)
      {
         if (cmsUtl_strlen(ifName) == 0)
            snprintf(cmdLine, sizeof(cmdLine), "-c %d -s %d -q -m %s", (int)newObj->numberOfRepetitions,
                    newObj->dataBlockSize, newObj->host);
         else
            snprintf(cmdLine, sizeof(cmdLine), "-c %d -s %d -q -m %s -I %s", (int)newObj->numberOfRepetitions,
      ...
      else
      {
         ...
         if (cmsUtl_strlen(ifName) == 0)
            snprintf(cmdLine, sizeof(cmdLine), "-c %d -s %d -q -m -d %d %s", (int)newObj->numberOfRepetitions,
                    newObj->dataBlockSize, mdmLibCtx.eid, newObj->host);
         else
            snprintf(cmdLine, sizeof(cmdLine), "-c %d -s %d -q -m -d %d %s -I %s", (int)newObj->numberOfRepetitions,
                ...
                CmsRet *errorCode __attribute__((unused)))
{
   ...
   /* To avoid restarting the Ethernet OAM service twice, leave the ADD_NEW
      case for rcl_ieee8021agCfgObject. */
   if(newObj != NULL && currObj != NULL)
   {
      rutEthOam_setService(FALSE, NULL);
      cmsLog_debug("Create or change 3ah settings");
      rutEthOam_prepareConfig();
      rutEthOam_setService(TRUE, NULL);
   }
   return ret;
}
                ...
                CmsRet *errorCode __attribute__((unused)))
{
   ...
   if (ADD_NEW(newObj, currObj) ||
       (newObj != NULL && currObj != NULL))
   {
      rutEthOam_setService(FALSE, NULL);
      cmsLog_debug("Create or change 1ag settings");
      rutEthOam_prepareConfig();
      rutEthOam_setService(TRUE, NULL);   
   }
   return ret;
...
#if defined(DMP_DOWNLOAD_1) || defined(DMP_UPLOAD_1)
static int fetchFirstWanIpInterface(char *wanIntfName)
{
   ...
   while (!found && cmsObj_getNextFlags(MDMOID_WAN_IP_CONN, &iidStack, OGF_NO_VALUE_UPDATE, (void **)&wanIpConnObj) == CMSRET_SUCCESS)
   {
      sprintf(wanIntfName, "%s", wanIpConnObj->X_BROADCOM_COM_IfName);
      found = TRUE;
   ...
   while (!found && cmsObj_getNextFlags(MDMOID_WAN_PPP_CONN, &iidStack, OGF_NO_VALUE_UPDATE, (void **)&wanPppConnObj) == CMSRET_SUCCESS)
   {
      sprintf(wanIntfName, "%s", wanPppConnObj->X_BROADCOM_COM_IfName);
      found = TRUE;
      ...
      CmsRet *errorCode __attribute__((unused)))
{
   ...
   if (!strcmp(newObj->diagnosticsState, MDMVS_REQUESTED)) 
   {
      ...
      else if (fetchFirstWanIpInterface(&if_name_cmd[3]) == FALSE)
         if_name_cmd[0] = '\0';
...
#ifdef BRCM_SPDTEST
      snprintf(buf, sizeof(buf), "%s -d %d -u %s -H", if_name_cmd, newObj->DSCP, newObj->downloadURL);
#else
...
#ifdef BRCM_SPDTEST
      snprintf(buf, sizeof(buf), "%s -d %d -l %d -u %s -H", if_name_cmd, newObj->DSCP, newObj->testFileLength, newObj->uploadURL);
#else
      ...
      CmsRet *errorCode __attribute__((unused)))
{
   ...
   if (buf[0])
   {
      ...
      if (qdmIntf_fullPathToIntfnameLocked(buf, if_name_cmd) == CMSRET_SUCCESS)
         snprintf(buf, sizeof(buf), "iptables -w -D INPUT -i %s -p udp --dport %d -j ACCEPT", if_name_cmd, currObj->UDPPort);
   }
   else
      snprintf(buf, sizeof(buf), "iptables -w -D INPUT -p udp --dport %d -j ACCEPT", currObj->UDPPort);
   ...
   if (newObj->enable)
   {
      ...
      if (buf[0])
      {
         ...
         if (qdmIntf_fullPathToIntfnameLocked(buf, if_name_cmd) == CMSRET_SUCCESS)
            snprintf(buf, sizeof(buf), "iptables -w -A INPUT -i %s -p udp --dport %d -j ACCEPT", if_name_cmd, newObj->UDPPort);
      }
      else
         snprintf(buf, sizeof(buf), "iptables -w -A INPUT -p udp --dport %d -j ACCEPT", newObj->UDPPort);
