...
typedef struct
{
   ...
   UBOOL8 (*isHiddenFn)(const MdmPathDescriptor *pathDesc, UINT32 flags); /* the callback to further determine if needs to be hidden */
} eneHiddenFromAcsBlacklist;

void getIfnameByAlias(char *alias, char *ifname, int size)
{
   ...
   if (alias == NULL || ifname == NULL)
      return;

   while (!found && cmsObj_getNext(MDMOID_DEV2_IP_INTERFACE, 
                                &iidStack,
                                (void **)&ipIntfObj) == CMSRET_SUCCESS)
   {
      if (!cmsUtl_strcmp(ipIntfObj->alias, alias))
      {
         cmsUtl_strncpy(ifname, ipIntfObj->name, size);
         found = TRUE;
      }
...
{
   static char voiceIfname[BUFLEN_32] = {0};

   if (strlen(voiceIfname) == 0)
   {
      getIfnameByAlias(EPON_EMTA_IF_ALIAS, voiceIfname, sizeof(voiceIfname));
   }
...
{
   static char eptaIfname[BUFLEN_32] = {0};

   if (strlen(eptaIfname) == 0)
   {
      getIfnameByAlias(EPON_EPTA_IF_ALIAS, eptaIfname, sizeof(eptaIfname));
   }
 ...
 */
static char* pktcbl_getHiddenWanIfName(UINT32 flags)
{
   if ((flags & OGF_EID_TR69C_1) == OGF_EID_TR69C_1)
   {
      return epon_epta_wan_name();
   }
   else if ((flags & OGF_EID_TR69C_2) == OGF_EID_TR69C_2)
   {
      return epon_voice_wan_name();
   }
 ...
 *  hide EPTA Wan(veip0.2) from ACS_1 
 */
static UBOOL8 pktcbl_isIpInterfaceHidden(const MdmPathDescriptor *pathDesc, UINT32 flags)
{
   char *IfName_hidden = NULL;
   Dev2IpInterfaceObject *ipIntfObj = NULL;
   UBOOL8 isHidden = FALSE;

   IfName_hidden = pktcbl_getHiddenWanIfName(flags);
 ...
 */
static UBOOL8 pktcbl_isDhcpv4ClientHidden(const MdmPathDescriptor *pathDesc, UINT32 flags)
{
   char *IfName_hidden = NULL;
 ...
 */
static UBOOL8 pktcbl_isDhcpv6ClientHidden(const MdmPathDescriptor *pathDesc, UINT32 flags)
{
   char *IfName_hidden = NULL;
...
CmsRet ene_getPathDescHiddenFromAcs(const MdmPathDescriptor *pathDesc __attribute__((unused)),
                                              UBOOL8 *hidden, UINT32 flags __attribute__((unused)))
{
#if (DMP_X_BROADCOM_COM_MULTIPLE_TR69C_SUPPORT_1 == 2)
   ...
   while(INVALIDE_MDM_MAX_OID != hiddenBL[i].oid)
   {
      if (pathDesc->oid == hiddenBL[i].oid)
      {
         ...
         else
         {
            ...
            *hidden = hiddenBL[i].isHiddenFn(pathDesc, flags);
         }
...
#else
   /* 
    * We come in here if we are NOT in the dual-TR69 mode.
    */
   if (hidden == NULL)
   {
      return CMSRET_INVALID_ARGUMENTS;
