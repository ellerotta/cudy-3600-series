                ...
                CmsRet *errorCode __attribute__((unused)))
{
   ...
   if ( ADD_NEW(newObj, currObj) )
   {
      ...
      if (newObj->endpointAddress)
      {
         ...
         REPLACE_STRING_IF_NOT_EQUAL_FLAGS(newObj->endpointAddressInUse,
                                    newObj->endpointAddress, mdmLibCtx.allocFlags);
      }
   ...
   if (newObj != NULL && newObj->enable)
   {
      ...
      if (cmsUtl_isValidIpAddress(AF_INET6, wanIp) == TRUE)
      {
         ...
         if (cmsUtl_strcmp(currObj->status, MDMVS_ENABLED) == 0) 
         {
             ...
             */
            ret = rutTunnel_4in6Config(ifname, wanIp, currObj->endpointAddressInUse, FALSE);
         }

         ret = rutTunnel_4in6Config(ifname, wanIp, ipstr, TRUE);
         CMSMEM_REPLACE_STRING_FLAGS(newObj->status, MDMVS_ENABLED, mdmLibCtx.allocFlags);
         REPLACE_STRING_IF_NOT_EQUAL_FLAGS(newObj->endpointAddressInUse, ipstr, mdmLibCtx.allocFlags);
...
//            rutIpt_TCPMSSforIPTunnel(ifname, FALSE); //FIXME AAA!!!
   ...
   else if (DELETE_OR_DISABLE_EXISTING(newObj, currObj))
   {
      ...
      if (cmsUtl_strcmp(currObj->status, MDMVS_ENABLED) == 0)
      {
         ret = rutTunnel_4in6Config(NULL, NULL, currObj->endpointAddressInUse, FALSE);
      }
      ...
      else /* case of delete */
      {
         ...
         ret = cmsMdm_fullPathToPathDescriptor(currObj->tunnelInterface, &ipintf);
         if (ret != CMSRET_SUCCESS)
         {
            cmsLog_error("Could not convert currObj->tunnelInterface %s", currObj->tunnelInterface);
            return ret;
         }
         ...
         ret = cmsMdm_fullPathToPathDescriptor(currObj->tunneledInterface, &ipintf);
         if (ret != CMSRET_SUCCESS)
         {
            cmsLog_error("Could not convert currObj->tunneledInterface %s", currObj->tunneledInterface);
            return ret;
         }
