...
CmsRet mdm_adjustForVoiceHardware(void)
{
   ...
   cmsLog_notice("mdm_adjustForVoiceHardware: maxPhysEndpt %d, numFxoEndpt %d\n",
                 maxPhysEndpt, numFxoEndpt);
   ...
   if (cmsMdm_isDataModelDevice2())
   {
      ...
      cmsLog_error("Incremented voiceServiceNumberOfEntries");
   ...
   /* VoiceService.{i}.Capabilities.Codecs.{i}. - Will be configured by the voice app */
   ...
   /* VoiceService.{i}.VoiceProfile.{i}.Line.{i}.Codec.List.{i}. - Will be configured by the voice app */
