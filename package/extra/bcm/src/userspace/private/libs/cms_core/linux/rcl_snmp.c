                ...
                CmsRet *errorCode __attribute__((unused)))
{
   ...
   if (newObj != NULL && currObj == NULL)
   {
      ...
      /* Cannot send message in BDK because SMD handler (voice_cb_thread) has
       * not started yet. */
      if (cmsUtl_strcmp(newObj->status, MDMVS_ENABLED) == 0)
      {
         ret = rut_sendMsgToSmd(CMS_MSG_START_APP, EID_SNMPD, NULL, 0);
         ...
         {
            cmsLog_debug("started SNMPD at bootup, ret=%d", ret);
         }
      }
#endif
   }
   else if (newObj != NULL && currObj != NULL)
   {
      ...
      if (cmsUtl_strcmp(newObj->status, currObj->status))
      {
         ...
         {
            ret = rut_sendMsgToSmd(CMS_MSG_STOP_APP, EID_SNMPD, NULL, 0);
            if (ret != CMSRET_SUCCESS)
            {
               cmsLog_error("failed to stop SNMPD, ret=%d", ret);
            }
            else
            {
      ...
      {
         ret = rut_sendMsgToSmd(CMS_MSG_RESTART_APP, EID_SNMPD, NULL, 0);
         if (ret != CMSRET_SUCCESS)
         {
            cmsLog_error("failed to start or restart SNMPD, ret=%d", ret);
         }
         else
         {
            cmsLog_debug("restarting SNMPD");
         }
