                           ...
                           CmsRet *errorCode __attribute__((unused)))
{
   ...
   if (ENABLE_NEW_OR_ENABLE_EXISTING(newObj, currObj))
   {
      ...
      rutEbt_IPv6PrefixExceptionIf(newObj->X_BROADCOM_COM_IfName, TRUE);
#endif
      return CMSRET_SUCCESS;
   }
   ...
   {   
      ...
      rutEbt_IPv6PrefixExceptionIf(currObj->X_BROADCOM_COM_IfName, FALSE);
#endif
                ...
                CmsRet *errorCode __attribute__((unused)))
{
            ...
            */
                     ...
                     !cmsUtl_strcmp(currObj->connectionStatus, MDMVS_DISCONNECTED))
            {
               ...
               */
               ...
               {
                  /* For ppp on demand, need to have the green light on */
                  cmsLed_setWanDisconnected();
               }
...
#ifdef SUPPORT_FCCTL
                   char cmd[BUFLEN_64];
                   snprintf(cmd, sizeof(cmd), "fcctl flush --if %s --silent ", newObj->X_BROADCOM_COM_IfName);
   ...
   {   
          ...
          !cmsUtl_strcmp(currObj->connectionStatus, MDMVS_DISCONNECTED))
      {
         ...
         /* remove the vlanctl rules for pppoe-relay */
         if(currObj != NULL)
         {
            wanConnOid = GET_MDM_OBJECT_ID(currObj);
            ret = rutWl2_getL2IfName(wanConnOid, iidStack, l2IfName);     
            if (ret != CMSRET_SUCCESS)
            {
               cmsLog_error("rutWl2_getL2IfName for pppoe-relay failed, ret=%d", ret);
               return ret;
            }
            else
            {
               ...
               snprintf(sufix, sizeof(sufix), ".%d.pt", currObj->X_BROADCOM_COM_ConnectionId);
               strcpy(l3IfName_pppoe_pt, l2IfName);
               strncat(l3IfName_pppoe_pt, sufix, sizeof(l3IfName_pppoe_pt)-1);
               cmsLog_debug("l3IfName_pppoe_pt = %s\n", l3IfName_pppoe_pt);
                ...
                CmsRet *errorCode __attribute__((unused)))
{
   ...
   if ( newObj->moveToWANSide )
   {
      ...
      {
         if ((ret = rutWan_moveEthLanToWan(newObj->ifName)) != CMSRET_SUCCESS)
             return ret;
      }
#endif
   ...
   else if ( newObj->moveToLANSide )
   {
      ...
      if (!cmsUtl_strncmp(newObj->ifName, ETH_IFC_STR, strlen(ETH_IFC_STR)))
      {
         if ((ret = rutWan_moveEthWanToLan(newObj->ifName)) != CMSRET_SUCCESS)
             return ret;
      }
#endif
#if defined(DMP_X_BROADCOM_COM_WIFIWAN_1) || defined(DMP_X_BROADCOM_COM_DEV2_WIFIWAN_1)
                ...
                CmsRet *errorCode __attribute__((unused)))
{
   ...
   if (ENABLE_NEW_OR_ENABLE_EXISTING(newObj, currObj))
   {
      ...
      if (newObj->X_BROADCOM_COM_IfName)
      {
         ...
         if ((ret = rutQos_tmPortInit(newObj->X_BROADCOM_COM_IfName, TRUE)) != CMSRET_SUCCESS)
         {
            cmsLog_error("rutQos_tmPortInit() failed. if=%s, ret=%d",
            newObj->X_BROADCOM_COM_IfName, ret);
         }

         rutLan_enableInterface(newObj->X_BROADCOM_COM_IfName);

#ifdef DMP_X_BROADCOM_COM_RPSRFS_1
         rutRpsRfs_checkSet(newObj->X_BROADCOM_COM_IfName);
#endif
      }
      else
      {
