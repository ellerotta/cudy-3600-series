...
// allow oal_mdm.c to use this func.
void lck_clearDeadThreadByTid(SINT32 tid);
...
static UINT32 lck_numEntriesForLockType(char lockType)
{
   if (lockType == 'g')
   {
      return GLOBAL_LOCK_TRACE_ENTRIES;
   }
   else if (lockType == 'z')
   {
      return ZONE_LOCK_TRACE_ENTRIES;
   }
   else if (lockType == 'e')
   {
      return ZONE_ENTRY_TRACE_ENTRIES;
   }
   else
   {
      cmsLog_error("Unknown lock type %c", lockType);
      return 0;
   }
}

static void lck_traceLock(LockTraceEntry *lockTraceArray,
                          char lockType, UINT8 lockCount, UINT8 lockZone,
                          MdmObjectId oid, const char *funcName)
{
   UINT32 i, max;

   max = lck_numEntriesForLockType(lockType);
   for (i=0; i < max; i++)
   {
      // lockCount == 0 means free/empty slot
      if (lockTraceArray[i].lockCount == 0)
      {
         lockTraceArray[i].lockCount = lockCount;
         lockTraceArray[i].lockZone = lockZone;
         lockTraceArray[i].lockType = lockType;
         lockTraceArray[i].oid = oid;
         cmsTms_get(&(lockTraceArray[i].ts));
         cmsUtl_strncpy(lockTraceArray[i].funcName, funcName,
                        sizeof(lockTraceArray[i].funcName));
         return;
      }
   }
   // It is ok if we run out of entries.  It means lock tracing will not
   // work correctly anymore, but the basic locking algorithm will still work.
   return;
}

static void lck_traceUnlock(LockTraceEntry *lockTraceArray,
                            char lockType, const char *funcName)
{
   SINT32 i;  // this needs to be signed to catch going past 0
   SINT32 max = (SINT32) lck_numEntriesForLockType(lockType);

   // search backwards for matching funcName.  Locks should be released in the
   // opposite order of when it was acquired.  We assume the lock is released
   // in the same function as where the lock was acquired (entries are matched
   // based on function name).  So if this assumption is violated, then the
   // tracing algorithm will not work.
   for (i = max-1; i >= 0; i--)
   {
      if ((lockTraceArray[i].lockCount != 0) &&
          (lockTraceArray[i].lockType == lockType) &&
          (!cmsUtl_strncmp(lockTraceArray[i].funcName, funcName,
                           sizeof(lockTraceArray[i].funcName)-1)))
      {
         // Mark this slot as free.
         lockTraceArray[i].lockCount = 0;
         return;
      }
   }
   return;
...
static void lck_dumpTraceArray(const LockTraceEntry *lockTraceArray,
                               char lockType)
{
   UINT32 i, max;
   UINT32 deltaMs;
   ...
   for (i=0; i < max; i++)
   {
      if (lockTraceArray[i].lockCount != 0)
      {
         deltaMs = cmsTms_deltaInMilliSeconds(&nowTs, &(lockTraceArray[i].ts));
         printf("  [%d] %d ms ago at %d.%03d "
                "type=%c zone=%d lockCount=%d oid=%d func=%s\n",
                i, deltaMs,
                lockTraceArray[i].ts.sec, lockTraceArray[i].ts.nsec/1000000,
                lockTraceArray[i].lockType, lockTraceArray[i].lockZone,
                lockTraceArray[i].lockCount, lockTraceArray[i].oid,
                lockTraceArray[i].funcName);
      }
   }
   printf("\n");
   return;
}
#endif  /* MDM_LOCK_TRACING */
...
#ifdef MDM_LOCK_TRACING
   lck_traceLock(thread->globalLockTrace,
                 'g', thread->globalLockCount, 0, 0, callerFuncName);
#endif
...
#ifdef MDM_LOCK_TRACING
   lck_traceUnlock(thread->globalLockTrace, 'g', callerFuncName);
#endif
   if (thread->globalLockCount > 0)
   {
      return;
...
#ifdef MDM_LOCK_TRACING
         lck_traceLock(thread->zoneLockTrace,
                       'z', thread->zoneLockCounts[i], i, 0, callerFuncName);
#endif
      }
   }
...
#ifdef MDM_LOCK_TRACING
               lck_traceUnlock(thread->zoneLockTrace, 'z', callerFuncName);
#endif
            }
         }
         thread->wantZone = MDM_INVALID_LOCK_ZONE;
         // This thread was not able to get the lock, so it might not
         // be holding any locks.  See if we can clear the lock entry.
         condFreeLockThread(thread);
         return ret;
      }
...
{
   return (cmsLck_getLockZoneFlags(oid, GET_OBJNODE_LOCAL, NULL));
}

UINT8 cmsLck_getLockZoneFlags(MdmObjectId oid, UINT32 getFlags,
                              UINT32 *actualFlags)
{
   MdmObjectNode *objNode;

   if ((objNode = mdm_getObjectNodeFlags(oid, getFlags, actualFlags)) == NULL)
   {
...
void cmsLck_dumpInfo()
{
   ...
   CmsLockThreadInfo *threads = NULL;
   CmsLockStats stats;
   UINT32 i, active=0;
   ...
   printf("===== Thread slots =====\n");
   threads = cmsLck_getLockThreadInfo();
   for (i=0; (threads != NULL) && (i < MDM_MAX_LOCK_THREADS); i++)
   {
   ...
   cmsLck_freeLockThreadInfo(&threads);
...
CmsLockThreadInfo *cmsLck_getLockThreadInfo()
{
   UINT32 i, j;
   CmsLockThreadInfo *threads;

   threads = cmsMem_alloc(MDM_MAX_LOCK_THREADS * sizeof(CmsLockThreadInfo), ALLOC_ZEROIZE);
   if (threads == NULL)
   {
      cmsLog_error("Cannot allocate LockThreadInfo array");
      return NULL;
   }

   oal_lock_meta();

   /* do shallow copy first, then fix the array pointers */
   memcpy(threads, mdmShmCtx->lockMeta->threads, MDM_MAX_LOCK_THREADS * sizeof(CmsLockThreadInfo));

   for (i=0; i < MDM_MAX_LOCK_THREADS; i++)
   {
      for (j=0; j < MDM_MAX_LOCK_ZONES; j++)
      {
         // one app cannot access the mdmObj ptrs of another app (could be in
         // heap memory), so null it out here, don't pass back.
         threads[i].tracked[j].single = NULL;
         threads[i].tracked[j].array = NULL;
         // Do deep copy of the OID array.
         if (threads[i].tracked[j].arrayOids != NULL)
         {
            MdmObjectId *oidArray = cmsMem_alloc(MAX_TRACKED_MDMOBJS * sizeof(MdmObjectId),
                                                 ALLOC_ZEROIZE);
            if (oidArray != NULL)
            {
               memcpy(oidArray, threads[i].tracked[j].arrayOids,
                      MAX_TRACKED_MDMOBJS * sizeof(MdmObjectId));
            }
            else
            {
               // too hard to undo now, this is just debug code anyways.
               cmsLog_error("Cannot allocate trackedOid array");
            }
            threads[i].tracked[j].arrayOids = oidArray;
         }
      }
   }
...
void cmsLck_freeLockThreadInfo(CmsLockThreadInfo **threadsPtr)
{
   UINT32 i, j;
   CmsLockThreadInfo *threads;
   ...
   for (i=0; (threads != NULL) && (i < MDM_MAX_LOCK_THREADS); i++)
   {
      for (j=0; j < MDM_MAX_LOCK_ZONES; j++)
      {
         // cmsMem_free can handle NULL input, so just free here
         CMSMEM_FREE_BUF_AND_NULL_PTR(threads[i].tracked[j].arrayOids);
      }
   }
...
void cmsLck_dumpLockThreadInfo(const CmsLockThreadInfo *thread)
{
   ...
   if (thread->globalLockCount > 0)
   {
      lck_dumpTraceArray(thread->globalLockTrace, 'g');
   }
#endif
...
#ifdef MDM_LOCK_TRACING
   if (getTotalZoneLockCounts(thread) > 0)
   {
      printf("  zone lock trace:\n");
      lck_dumpTraceArray(thread->zoneLockTrace, 'z');
   }
#endif
   ...
   {
      printf("  zone entry trace:\n");
      lck_dumpTraceArray(thread->zoneEntryTrace, 'e');
   }
#endif
   ...
   if (thread->totalTrackedMdmObjs > 0)
   {
      for (i=0; i < MDM_MAX_LOCK_ZONES; i++)
      {
         const TrackedMdmObjs *trackedZone = &(thread->tracked[i]);
         if (trackedZone->count > 0)
         {
            printf("  trackedZone[%d].count=%d max=%d\n",
                   i, trackedZone->count, trackedZone->max);
            if (trackedZone->singleOid != 0)
            {
               printf("    [single] oid=%d", trackedZone->singleOid);
            }
            printf("\n");
            if (trackedZone->arrayOids != NULL)
            {
               int j;
               for (j=0; j < MAX_TRACKED_MDMOBJS; j++)
               {
                  if (trackedZone->arrayOids[j] != 0)
                  {
                     printf("    [%d] oid=%d\n", j, trackedZone->arrayOids[j]);
                  }
               }
               printf("\n");
            }
...
void dumpLockTimeoutDiag(UINT8 zone, UINT32 actualTimeoutMs)
{
   ...
   else
   {
      ...
      cmsLog_notice("timed out after %d ms waiting for zone lock %d held by "
...
CmsRet lck_autoLockZone(MdmObjectId oid, const char *where)
{
   ...
   zone = cmsLck_getLockZoneFlags(oid, GET_OBJNODE_ANY, &actualFlags);
   if (zone == MDM_INVALID_LOCK_ZONE)
   {
      cmsLog_error("invalid access to oid %d", oid);
      return CMSRET_INVALID_ARGUMENTS;
   }

   if (actualFlags & GET_OBJNODE_REMOTE)
   {
...
#ifdef MDM_LOCK_TRACING
         lck_traceLock(thread->zoneEntryTrace,
                       'e', thread->zoneEntryCounts[i], i, oid, where);
#endif
      }
   }
...
#ifdef MDM_LOCK_TRACING
               lck_traceUnlock(thread->zoneEntryTrace, 'e', where);
#endif
            }
         }
         thread->wantZone = MDM_INVALID_LOCK_ZONE;
...
void lck_autoTrackMdmObj(const void *mdmObj)
{
   ...
   else
   {
      ...
      if (trackedZone->array == NULL)
      {
         ...
         trackedZone->arrayOids = cmsMem_alloc(
                                     MAX_TRACKED_MDMOBJS * sizeof(MdmObjectId),
                                     mdmLibCtx.allocFlags);
         if ((trackedZone->array == NULL) ||
             (trackedZone->arrayOids == NULL))
         {
            ...
            // Free array and arrayOids in case we succeeded in allocating one
            // but not the other.  cmsMem_free can handle NULL input.
            CMSMEM_FREE_BUF_AND_NULL_PTR(trackedZone->array);
            CMSMEM_FREE_BUF_AND_NULL_PTR(trackedZone->arrayOids);
            return;
         }
      }
      // find empty slot
                     ...
                     MdmObjectId oid, const char *funcName)
{
   ...
   if (owner->tid != CMS_INVALID_PID)
   {
      ...
      {
         ProcThreadInfo tInfo;
	 int dead_process_found = 0;
         if ((sysUtl_getThreadInfoFromProc(owner->tid, &tInfo) == -1) ||
             IS_PROC_THREAD_INFO_ZOMBIE(&tInfo))
         {
            dead_process_found = 1;
         }
         else
         {
            /* Here might have a timing issue with regard to releasing the locks and updating the process status.
             * The locks are released first, and then the /proc entry is updated later, so try again after a short sleep.
             */
...
UINT32 lck_getTotalZoneEntryCounts(void)
{
   CmsLockThreadInfo *thread;
   SINT32 tid;
   UINT32 count=0;
...
{
   oal_lock_meta();
   clearDeadThreadByTidLocked(tid);
   oal_unlock_meta();
      ...
      else
      {
         ...
         for (i=0; i < MDM_MAX_LOCK_ZONES; i++)
         {
            if (thread->tracked[i].array != NULL)
            {
               ...
               CMSMEM_FREE_BUF_AND_NULL_PTR(thread->tracked[i].arrayOids);
            }
         }
