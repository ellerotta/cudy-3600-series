...
/** Given the name of a bridge, return its index in MDM.
 *
 * @param brIntfName         (IN) The name of the bridge
 * @param bridgeIndex        (OUT) The internal bridge index in MDM
 *
 * @return CmsRet
 */
CmsRet rutBridge_getBridgeIndexByName_dev2(const char *brIntfName, SINT32 *index);

/** Given the index of a bridge, return its name in MDM.
 *
 * @param index             (IN) The index of the bridge
 * @param brIntfName        (OUT) The bridge name in MDM
 * @param size              (IN) The size of buffer to hold the bridge name
 *
 * @return CmsRet
 */
CmsRet rutBridge_getBridgeNameByIndex_dev2(SINT32 index, char *brIntfName, UINT32 size);

/** Given the iidStack of a bridge port, return the type of
 *  the containing bridge.
 *
 * @param iidStack         (IN) iidStack of the port object
 * @param isOVS            (OUT) The type of the bridge. If set to TRUE, the bridge
 *                         is an OVS bridge
 * @return CmsRet
 */
...
void rutBridge_addDynIntfToBridge_dev2(const char *intfName, const char *brIntfName);
...
void rutBridge_deleteDynIntfFromBridge_dev2(const char *intfName, const char *brIntfName);
