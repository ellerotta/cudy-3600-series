...
/* 
    ...
    Output: *speed: current link speed in Mbps unit, if 0, link is down.
            ...
            *phycfg: PHY_CAP_XXX, defined in bcmswapitypes.h 
            *subport: subport with highest current link up speed.
*/
int bcm_get_linkspeed(char *ifname, int *speed, int *duplex, int *phycfg, int *subport)
{
    struct ifreq ifr;
    union {
    ...
    memset( &ifrdata, 0, sizeof(ifrdata));
...
int ethernet_cable_diagnostics(char *ifname, ethcd_t *ethcd)
{
    struct ifreq ifr;
    struct ethctl_data _ethctl, *ethctl = &_ethctl;
    int skfd, err;

    memset(&ifr, 0, sizeof(ifr));
    if ((err = sock_init(ifname, &skfd, &ifr)) < 0)
        goto error;

    strncpy(ifr.ifr_name, ifname, sizeof(ifr.ifr_name)-1);
    ifr.ifr_name[sizeof(ifr.ifr_name)-1] = '\0';
    ifr.ifr_data = ethctl;
    memcpy(&ethctl->ethcd, ethcd, sizeof(*ethcd));
    ethctl->op = ETHCDCTL;
    err = ioctl(skfd, SIOCETHCTLOPS, &ifr);
    if (!err)
        memcpy(ethcd, &ethctl->ethcd, sizeof(*ethcd));
error:
    close(skfd);
    return err;
}
