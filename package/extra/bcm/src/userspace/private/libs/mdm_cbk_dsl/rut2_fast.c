...
void rutUtil_modifyNumFastLine(SINT32 delta)
{
   InstanceIdStack iidStack = EMPTY_INSTANCE_ID_STACK;
   Dev2FastObject *fastObj = NULL;
   ...
   if (mdmShmCtx->inMdmInit)
   {
       ...
       */
      cmsLog_debug("don't update FAST link count (delta=%d)",delta);
      return;
   }

   if ((cmsObj_get(MDMOID_DEV2_FAST, &iidStack, 0, (void *) &fastObj)) == CMSRET_SUCCESS)
   {
      fastObj->lineNumberOfEntries += delta;
      cmsObj_set(fastObj, &iidStack);
      cmsObj_free((void **) &fastObj);
   }
}

CmsRet rutfast_getLineIdByLineIidStack_dev2(const InstanceIdStack *iidStack, UINT32 *lineId)
{
   CmsRet ret = CMSRET_INTERNAL_ERROR;
   Dev2FastLineObject *fastLineObj = NULL;

   if ((ret=cmsObj_get(MDMOID_DEV2_FAST_LINE, iidStack, OGF_NO_VALUE_UPDATE, (void **)&fastLineObj)) == CMSRET_SUCCESS)
   {
      *lineId = fastLineObj->X_BROADCOM_COM_BondingLineNumber;
      cmsObj_free((void **)&fastLineObj);
   }
   else
   {
      cmsLog_error("Fail to get cmsObj_get(MDMOID_DEV2_FAST_LINE). ret=%d", ret);
      *lineId = 0;
   }
...
CmsRet rutfast_getShowTimeStats_dev2(Dev2FastLineStatsShowtimeObject *obj)
{
   ...
   ret = cmsAdsl_getAdslMib(&adslMib);
...
} /* rutfast_getAdslCurrentDayStats_dev2 */
...
CmsRet rutfast_getTestParamsInfo_dev2(void *obj)
{
   ...
   ret = cmsAdsl_getAdslMib(&adslMib);
   if (ret == (CmsRet) BCMADSL_STATUS_SUCCESS)
   {
      ...
      gFactor = 1;
      len = 1;
      ret = cmsAdsl_setAdslMibObject(oidStr1, sizeof(oidStr1), (char *)&gFactor, &len);
      if(ret == (CmsRet) BCMADSL_STATUS_SUCCESS)
      {
         /* get SNR */
   ...
   return ret;
} /* rutfast_getAdslTestParamsInfo_dev2 */
...
CmsRet rutfast_getTotalStats_dev2(Dev2FastLineStatsTotalObject *obj)
{
   ...
   return (CMSRET_SUCCESS);
} /* rutfast_getTotalStats_dev2 */

CmsRet rutfast_getLineStats_dev2(Dev2FastLineStatsObject *obj  __attribute__((unused)),
                                 const InstanceIdStack *iidStack  __attribute__((unused)))
{
   ...
   rutfast_getLineIdByLineIidStack_dev2(iidStack, &lineId);
...
CmsRet rutfast_getLineInfo_dev2(Dev2FastLineObject *obj, const InstanceIdStack *iidStack)
{
   ...
   if (xdslStatus == BCM_ADSL_LINK_UP)
   {
      ...
      ret = xdslCtl_GetObjectValue(obj->X_BROADCOM_COM_BondingLineNumber, NULL, 0, (char *) &adslMib, &adslMibSize);
      ...
      {
         /* this is the case when the xdsl link had been trained and UP in FAST mode, and now UP in non-FAST mode.
          * We need to change the status of the FAST link to DOWN.
          */
         REPLACE_STRING_IF_NOT_EQUAL_FLAGS(obj->status, MDMVS_DOWN, mdmLibCtx.allocFlags);
         REPLACE_STRING_IF_NOT_EQUAL_FLAGS(obj->linkStatus, MDMVS_NOSIGNAL, mdmLibCtx.allocFlags);         
         return CMSRET_SUCCESS;
        ...
        )
      {
         ...
         case kMgfastProfileQ424d:
            strcpy(value,"Q424d");
            break;
         case kMgfastProfileP424amp:
            strcpy(value,"P424amp");
            break;
         case kMgfastProfileP424dmp:
            strcpy(value,"P424dmp");
            break;
         case kMgfastProfileQ424cmp:
            strcpy(value,"Q424cmp");
            break;
         case kMgfastProfileQ424dmp:
            strcpy(value,"Q424dmp");
            break;
#endif
         default:
...
#ifdef CONFIG_MGFAST_SUPPORT
         {
         int vdslParm1 = adslCfg.vdslParam1 << 16;
         if (0 == (vdslParm1 & kMgfastProfileP424aDisable))
         {
            strcat(value,"P424a,");
         }
         if (0 == (vdslParm1 & kMgfastProfileP424dDisable))
         {
            strcat(value,"P424d,");
         }
         if (0 == (vdslParm1 & kMgfastProfileQ424cDisable))
         {
            strcat(value,"Q424c,");
         }
         if (0 == (vdslParm1 & kMgfastProfileQ424dDisable))
         {
            strcat(value,"Q424d,");
         }
         if (0 == (vdslParm1 & kMgfastProfileP424ampDisable))
         {
            strcat(value,"P424amp,");
         }
         if (0 == (vdslParm1 & kMgfastProfileP424dmpDisable))
         {
            strcat(value,"P424dmp,");
         }
         if (0 == (vdslParm1 & kMgfastProfileQ424cmpDisable))
         {
            strcat(value,"Q424cmp,");
         }
         if (0 == (vdslParm1 & kMgfastProfileQ424dmpDisable))
         {
            strcat(value,"Q424dmp,");
         }
...
#ifdef CONFIG_MGFAST_SUPPORT
      || (newObj->X_BROADCOM_COM_FAST_P424a != currObj->X_BROADCOM_COM_FAST_P424a)
      || (newObj->X_BROADCOM_COM_FAST_P424d != currObj->X_BROADCOM_COM_FAST_P424d)
      || (newObj->X_BROADCOM_COM_FAST_Q424c != currObj->X_BROADCOM_COM_FAST_Q424c)
      || (newObj->X_BROADCOM_COM_FAST_Q424d != currObj->X_BROADCOM_COM_FAST_Q424d)
      || (newObj->X_BROADCOM_COM_FAST_P424amp != currObj->X_BROADCOM_COM_FAST_P424amp)
      || (newObj->X_BROADCOM_COM_FAST_P424dmp != currObj->X_BROADCOM_COM_FAST_P424dmp)
      || (newObj->X_BROADCOM_COM_FAST_Q424cmp != currObj->X_BROADCOM_COM_FAST_Q424cmp)
      || (newObj->X_BROADCOM_COM_FAST_Q424dmp != currObj->X_BROADCOM_COM_FAST_Q424dmp)
#endif
       )
   {
      changed = TRUE;
...
#ifdef CONFIG_MGFAST_SUPPORT
   if(!pFastLineObj->X_BROADCOM_COM_FAST_P424a)
   {
      pAdslCfg->vdslParam1 |= (kMgfastProfileP424aDisable >> 16);
   }
   if(!pFastLineObj->X_BROADCOM_COM_FAST_P424d)
   {
      pAdslCfg->vdslParam1 |= (kMgfastProfileP424dDisable >> 16);
   }
   if(!pFastLineObj->X_BROADCOM_COM_FAST_Q424c)
   {
      pAdslCfg->vdslParam1 |= (kMgfastProfileQ424cDisable >> 16);
   }
   if(!pFastLineObj->X_BROADCOM_COM_FAST_Q424d)
   {
      pAdslCfg->vdslParam1 |= (kMgfastProfileQ424dDisable >> 16);
   }
   if(!pFastLineObj->X_BROADCOM_COM_FAST_P424amp)
   {
      pAdslCfg->vdslParam1 |= (kMgfastProfileP424ampDisable >> 16);
   }
   if(!pFastLineObj->X_BROADCOM_COM_FAST_P424dmp)
   {
      pAdslCfg->vdslParam1 |= (kMgfastProfileP424dmpDisable >> 16);
   }
   if(!pFastLineObj->X_BROADCOM_COM_FAST_Q424cmp)
   {
      pAdslCfg->vdslParam1 |= (kMgfastProfileQ424cmpDisable >> 16);
   }
   if(!pFastLineObj->X_BROADCOM_COM_FAST_Q424dmp)
   {
      pAdslCfg->vdslParam1 |= (kMgfastProfileQ424dmpDisable >> 16);
   }
#endif
   cmsObj_free((void **) &pFastLineObj);
}
...
#ifdef CONFIG_MGFAST_SUPPORT
   {
   int vdslParam1 = (pAdslCfg->vdslParam1 << 16);
   pFastLineObj->X_BROADCOM_COM_FAST_P424a = ((vdslParam1 & kMgfastProfileP424aDisable) == 0);
   pFastLineObj->X_BROADCOM_COM_FAST_P424d = ((vdslParam1 & kMgfastProfileP424dDisable) == 0);
   pFastLineObj->X_BROADCOM_COM_FAST_Q424c = ((vdslParam1 & kMgfastProfileQ424cDisable) == 0);
   pFastLineObj->X_BROADCOM_COM_FAST_Q424d = ((vdslParam1 & kMgfastProfileQ424dDisable) == 0);
   pFastLineObj->X_BROADCOM_COM_FAST_P424amp = ((vdslParam1 & kMgfastProfileP424ampDisable) == 0);
   pFastLineObj->X_BROADCOM_COM_FAST_P424dmp = ((vdslParam1 & kMgfastProfileP424dmpDisable) == 0);
   pFastLineObj->X_BROADCOM_COM_FAST_Q424cmp = ((vdslParam1 & kMgfastProfileQ424cmpDisable) == 0);
   pFastLineObj->X_BROADCOM_COM_FAST_Q424dmp = ((vdslParam1 & kMgfastProfileQ424dmpDisable) == 0);
   }
#endif
}
