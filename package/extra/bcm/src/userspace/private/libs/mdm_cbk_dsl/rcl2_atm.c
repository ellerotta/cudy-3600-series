                              ...
                              CmsRet *errorCode __attribute__((unused)))
{
   ...
   {
      cmsLog_debug("newObj->status=%s ==> new status=%s",
      ...
      // atm interface up.  Also make sure object is enabled since SQA likes
   ...
   if (addIntf)
   {
      ...
      if (enableExisting)
      {
         ...
         rutIp_sendPropgateStatusExMsg(destEid, MDMOID_DEV2_ATM_LINK, iidStack,
                                       newObj->lowerLayers, MDMVS_UP);
         return CMSRET_SUCCESS;
      }

      // This section actually configures the ATM interface.
...

      {
         ...
         {
            REPLACE_STRING_IF_NOT_EQUAL_FLAGS(newObj->X_BROADCOM_COM_MacAddress, hwAddr, mdmLibCtx.allocFlags);
         }
      }

#ifdef BRCM_CMS_BUILD
      /* these 2 objects are not in distributed DSL MDM yet */
   ...
   else if (deleteIntf && currObj->enable)
   {
      ...
      // (b) the ATM link object is being deleted (isRealDel)
      ...
      // We are not really deleting this ATM link object, just config it DOWN.
      if (!isRealDel)
      {
