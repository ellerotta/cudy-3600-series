                               ...
                               CmsRet *errorCode __attribute__((unused)))
{
   ...
   else if (DISABLE_EXISTING(newObj, currObj))
   {
      ...
      if (!found)
      {
         cmsLog_error("Cannot get line object for disabling, ret=%d", ret);
         return ret;
      }
      rutdsl_configDown_dev2(dslLineObj);
      cmsObj_free((void **) &dslLineObj);
   }
...
CmsRet rcl_dev2FastLineStatsShowtimeObject( _Dev2FastLineStatsShowtimeObject *newObj __attribute__((unused)),
                                            const _Dev2FastLineStatsShowtimeObject *currObj __attribute__((unused)),
                                            const InstanceIdStack *iidStack __attribute__((unused)),
                                            char **errorParam __attribute__((unused)),
                                            CmsRet *errorCode __attribute__((unused)))
{
