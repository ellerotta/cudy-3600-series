 ...
 *  It will only be called in a distributed MDM build in the DSL component.
 */
...
CmsRet mdm_adjustForHardware_dev2()
{
   ...
   cmsLog_notice("Distributed MDM build: adjustForHardware for DSL only");
...
CmsRet mdm_addDslLineAndChannel(int lineNumber)
{
   ...
   char lineNameStr[32]={0};
   char channelAliasStr[64]={0};
   ...
   // In BDK, we automatically enable the DSL line.
   dslLineObj->enable = TRUE;
#ifdef DMP_X_RDK_DSL_1
   // In RDK, we leave DSL line disabled so the xdslManager can set some OEM
   // params (XTURSystemVendorId, XTURVersionNumber, and XTURSerialNumber) and
   // then enable the line.  TODO: since xdslManager does not enable the line yet,
   // leave it enabled by default until xdslManager is updated (after 2023q1).
   // dslLineObj->enable = FALSE;
#endif

   dslLineObj->X_BROADCOM_COM_BondingLineNumber = lineNumber;
   // Data model line number is 1 based, our numbering is 0 based.
   dslLineObj->lineNumber = lineNumber+1;

   /* initialize XTSE to all the standards we support.  XTSE is hex binary */
   char XTSE[24]={0};
   
   snprintf(XTSE, sizeof(XTSE),"%02X%02X%02X%02X%02X%02X%02X%02X",
            (unsigned char)(XTSE_REGIONAL_T1413|XTSE_G992_1_ANNEX_A_NONOVERLAPPED|XTSE_G992_1_ANNEX_B_NONOVERLAPPED|XTSE_G992_1_ANNEX_C_NONOVERLAPPED),
            (unsigned char)(XTSE_G992_2_ANNEX_A_NONOVERLAPPED|XTSE_G992_2_ANNEX_C_NONOVERLAPPED),
            (unsigned char)(XTSE_G992_3_ANNEX_A_NONOVERLAPPED|XTSE_G992_3_ANNEX_B_NONOVERLAPPED| XTSE_G992_3_ANNEX_C_NONOVERLAPPED),
            (unsigned char)(XTSE_G992_3_ANNEX_I_NONOVERLAPPED|XTSE_G992_3_ANNEX_J_NONOVERLAPPED),
            (unsigned char)(XTSE_G992_3_ANNEX_L_MODE1|XTSE_G992_3_ANNEX_L_MODE2|XTSE_G992_3_ANNEX_M_NONOVERLAPPED),
            (unsigned char)(XTSE_G992_5_ANNEX_A_NONOVERLAPPED|XTSE_G992_5_ANNEX_B_NONOVERLAPPED|XTSE_G992_5_ANNEX_C_NONOVERLAPPED|XTSE_G992_5_ANNEX_I_NONOVERLAPPED),
            (unsigned char)(XTSE_G992_5_ANNEX_J_NONOVERLAPPED|XTSE_G992_5_ANNEX_M_NONOVERLAPPED),
            (unsigned char)(XTSE_G993_2_REGION_A|XTSE_G993_2_REGION_B|XTSE_G993_2_REGION_C)
            );
   CMSMEM_REPLACE_STRING_FLAGS(dslLineObj->XTSE, XTSE, mdmLibCtx.allocFlags);
   CMSMEM_REPLACE_STRING_FLAGS(dslLineObj->status, MDMVS_DOWN, mdmLibCtx.allocFlags);
   
   snprintf(lineNameStr, sizeof(lineNameStr), "dsl%d",lineNumber);
   ...
   for (i=0; i < 2; i++)
   {
      ...
      if (i == 0)
      {
         ...
         snprintf(channelAliasStr, sizeof(channelAliasStr), "cpe-%s-atm-channel",lineNameStr);
      }
      else
      {
         CMSMEM_REPLACE_STRING_FLAGS(dslChannelObj->linkEncapsulationUsed, MDMVS_G_993_2_ANNEX_K_PTM, mdmLibCtx.allocFlags);
         snprintf(channelAliasStr, sizeof(channelAliasStr), "cpe-%s-ptm-channel",lineNameStr);
      }
...
CmsRet mdm_addDslBondingGroups(void)
{
   ...
   for (group=0; group < 2; group++)
   {
      ...
      if (group == 0)
      {
         CMSMEM_REPLACE_STRING_FLAGS(dslBondingObj->name,BONDING_GROUP_ATM_NAME, mdmLibCtx.allocFlags);
      ...
      {
         CMSMEM_REPLACE_STRING_FLAGS(dslBondingObj->name,BONDING_GROUP_PTM_NAME, mdmLibCtx.allocFlags);
...
CmsRet mdm_addFastBondingGroup(void)
{
   ...
   CMSMEM_REPLACE_STRING_FLAGS(dslBondingObj->name,BONDING_GROUP_FAST_NAME, mdmLibCtx.allocFlags);
