...
static void startWanConnection(void *appMsgHandle)
{
   WanConnInfo *wanConnInfo=NULL;
   ...
   dlist_for_each_entry(wanConnInfo, &wanConnInfoHead, dlist)
   {
      ...
      if (wanConnInfo->isUsed)
      {
         if (getWanConnObjFromWanConnInfo(wanConnInfo, &wanConnObj) == CMSRET_SUCCESS)
         {
            ...
            updateSingleWanConnStatusLocked(appMsgHandle,
                                            &(wanConnInfo->iidStack),
                                            wanConnObj, isWanLinkUp);
...
static void stopWanConnection(void *appMsgHandle)
{
   WanConnInfo *wanConnInfo=NULL;
   void *wanConnObj=NULL;
...
void  stopAllWanConn(void *appMsgHandle, const InstanceIdStack *parentIidStack, UBOOL8 oldAutoDetectEnabled)
{
   ...
   {
      updateSingleWanConnStatusLocked(appMsgHandle, &iidStack, (void *) ipConn, FALSE);
      cmsObj_free((void **) &ipConn);
   }
   ...
   {
      updateSingleWanConnStatusLocked(appMsgHandle, &iidStack,  (void *) pppConn, FALSE);
      cmsObj_free((void **) &pppConn);
   }
...
void processAutoDetectTask(void *appMsgHandle)
{
   ...
   else 
   {
      ...
      else
      {
         ...
         startWanConnection(appMsgHandle);
      }
   }
...
void updateWanConnStatusInSubtreeLocked_n(void *appMsgHandle, const InstanceIdStack *parentIidStack, UBOOL8 isLinkUp, UBOOL8 startNewConnection)
{
   InstanceIdStack iidStack=EMPTY_INSTANCE_ID_STACK;
   WanIpConnObject *ipConn=NULL;
   WanPppConnObject *pppConn=NULL;
   ...
   {
      stopWanConnection(appMsgHandle);
