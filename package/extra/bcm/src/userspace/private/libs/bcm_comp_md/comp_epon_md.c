...
/*!\file comp_epon_md.c
 * \brief High level functions used by devinfo_md and bcm_epon_hal init thread.
 */
...
// Northbound is used by epon_md to interface with ZBus.
// Some non-BDK environments do not need this part.
BcmRet compMd_initEponNorth(const char **myNamespaces,
                            void *msgHandle)
{
   BcmRet ret;
...
// Southbound is used by the epon_hal_thread to initialize the "EPON Stack":
// which includes eponapp, MDM, bcm_msgd, msgHandle (bus manager)
// Southbound is initialized before Northbound.
BcmRet compMd_initEponSouth(void **msgHandle, SINT32 *shmId)
{
   CmsEntityId eid=EID_EPON_HAL_THREAD;
   const char *busName=EPON_MSG_BUS;
   UBOOL8 isBusManager=TRUE;
   const char *args="-c epon";
   MdmInitConfig mdmConfig;
   BcmRet ret;

   myCompName = BDK_COMP_EPON;
   ...
   mdmConfig.mdmLibName = "libmdm2_epon.so";
   mdmConfig.shmSize = MAX_MDM_SHM_SIZE_EPON;  // probably don't need this much.
   mdmConfig.shmAttachAddr = (void *) MDM_SHM_ATTACH_ADDR_EPON;
   mdmConfig.lockKeyOffset = EPON_KEY_OFFSET;
   mdmConfig.eid = eid;
...
BcmRet compMd_launchEponapp(SINT32 shmId)
{
   ...
   const char *prefix = "/userspace/private/apps/eponapp/objs";
#else
   const char *prefix = "/bin";
#endif
   char eponappArgs[512]={0};
   BcmRet ret;

   snprintf(eponappArgs, sizeof(eponappArgs), "-m %d -n", shmId);
   ret = compMd_launchApp(prefix, "eponapp", eponappArgs, NULL);
   return ret;
}
