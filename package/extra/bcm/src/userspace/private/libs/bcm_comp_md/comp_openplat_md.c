...
/*!\file comp_openplat_md.c
 * \brief High level functions used by openplat_md init thread.
 */
...
{
   CmsEntityId eid=EID_OPENPLAT_MD;
   const char *busName=OPENPLAT_MSG_BUS;
   UBOOL8 isBusManager=TRUE;
   const char *args="-c openplat";
   MdmInitConfig mdmConfig;
   BcmRet ret;

   myCompName = BDK_COMP_OPENPLAT;
   ...
   mdmConfig.mdmLibName = "libmdm2_openplat.so";
   mdmConfig.shmSize = MAX_MDM_SHM_SIZE_OPENPLAT;  // probably don't need this much.
   mdmConfig.shmAttachAddr = (void *) MDM_SHM_ATTACH_ADDR_OPENPLAT;
   mdmConfig.lockKeyOffset = OPENPLAT_KEY_OFFSET;
