...
int bcm_mcast_api_ipv4_purge_grp_reporter(int sock_nl, int ifi, int dstdev_ifi, struct in_addr *grp, struct in_addr *rep)
{
   t_BCM_MCAST_IGMP_PURGE_GRP_REPORTER msg;

   msg.parent_ifi = ifi;
   msg.dstdev_ifi = dstdev_ifi;
   msg.rep.s_addr = rep->s_addr;
   msg.grp.s_addr = grp->s_addr;

   return bcm_mcast_api_nl_tx(sock_nl, BCM_MCAST_MSG_IGMP_PURGE_GRP_REPORTER, (unsigned char *)&msg, sizeof(msg), NULL, 0, 1);
} /* bcm_mcast_api_ipv4_purge_grp_reporter */

int bcm_mcast_api_ipv6_purge_grp_reporter(int sock_nl, int ifi, int dstdev_ifi, struct in6_addr *grp, struct in6_addr *rep)
{
   t_BCM_MCAST_MLD_PURGE_GRP_REPORTER msg;
   ...
   msg.rep.s6_addr32[2] = rep->s6_addr32[2];
   msg.rep.s6_addr32[3] = rep->s6_addr32[3];
   msg.grp.s6_addr32[0] = grp->s6_addr32[0];
   msg.grp.s6_addr32[1] = grp->s6_addr32[1];
   msg.grp.s6_addr32[2] = grp->s6_addr32[2];
   msg.grp.s6_addr32[3] = grp->s6_addr32[3];

   return bcm_mcast_api_nl_tx(sock_nl, BCM_MCAST_MSG_MLD_PURGE_GRP_REPORTER, (unsigned char *)&msg, sizeof(msg), NULL, 0, 1);
...
int bcm_mcast_api_set_snooping_cfg(int sock_nl, int ifi, int proto, int mode)
{
...
int bcm_mcast_api_set_hostctl_cfg(int sock_nl, int ifi, int proto, int enable)
{
   t_BCM_MCAST_HOSTCTL_CFG msg;

   msg.ifi = ifi;
   msg.proto = proto;
   msg.enable = enable;
   return bcm_mcast_api_nl_tx(sock_nl, BCM_MCAST_MSG_HOSTCTL_CFG, (unsigned char *)&msg, sizeof(msg), NULL, 0, 1);
}
...
int bcm_mcast_api_update_igmp_host_client(int socknl,
                                          const struct in_addr *grp,
                                          const struct in_addr *src,
                                          int isAdd,
                                          t_BCM_MCAST_WAN_INFO_ARRAY *wan_info)
{
   t_BCM_MCAST_HOST_CLIENT_ENTRY hc_entry_msg;
   ...
   memset(&hc_entry_msg, 0, sizeof(hc_entry_msg));

   hc_entry_msg.grp.is_ipv4 = 1;
   hc_entry_msg.grp.ipv4_addr.s_addr = grp->s_addr;
   hc_entry_msg.src.is_ipv4 = 1;
   if (src)
   {
       hc_entry_msg.src.ipv4_addr.s_addr = src->s_addr;
   }
   else
   {
       hc_entry_msg.src.ipv4_addr.s_addr = 0;
   }
   hc_entry_msg.isAdd = isAdd;
   memcpy(&hc_entry_msg.wan_info, wan_info, sizeof(t_BCM_MCAST_WAN_INFO_ARRAY));
   return bcm_mcast_api_nl_tx(socknl, BCM_MCAST_MSG_HOST_CLIENT_ENTRY_UPDATE,
                              (unsigned char *)&hc_entry_msg,
                              sizeof(hc_entry_msg), NULL, 0, 1);
}

int bcm_mcast_api_update_mld_host_client(int socknl,
                                         const struct in6_addr *grp,
                                         const struct in6_addr *src,
                                         int isAdd,
                                         t_BCM_MCAST_WAN_INFO_ARRAY *wan_info)
{
   ...
   hc_entry_msg.grp.is_ipv4 = 0;
   memcpy(&hc_entry_msg.grp.ipv6_addr, grp, sizeof(struct in6_addr));
   hc_entry_msg.src.is_ipv4 = 0;
   if (src)
   {
       memcpy(&hc_entry_msg.src.ipv6_addr, src, sizeof(struct in6_addr));
   }
   else
   {
       memset(&hc_entry_msg.src.ipv6_addr, 0, sizeof(struct in6_addr));
   }
   hc_entry_msg.isAdd = isAdd;
   memcpy(&hc_entry_msg.wan_info, wan_info, sizeof(t_BCM_MCAST_WAN_INFO_ARRAY));
   return bcm_mcast_api_nl_tx(socknl, BCM_MCAST_MSG_HOST_CLIENT_ENTRY_UPDATE,
...
int bcm_mcast_api_get_snooping_cfg(int ifi, int proto, int *mode)
{
   t_BCM_MCAST_SNOOP_CFG  msg;
...
int bcm_mcast_api_get_hostctl_cfg(int ifi, int proto, int *enable)
{
   t_BCM_MCAST_HOSTCTL_CFG  msg;
   t_BCM_MCAST_HOSTCTL_CFG *prsp = NULL;
   unsigned int           rsp_len = 0;
   unsigned char          buf[NLMSG_SPACE(sizeof(*prsp)) + sizeof(struct nlmsgerr)];
   int                    ret;
   int                    nl_sock;
   ...
   msg.proto = proto;
   msg.ifi = ifi;
   ret = bcm_mcast_api_nl_tx(nl_sock, BCM_MCAST_MSG_HOSTCTL_CFG, (unsigned char *)&msg, sizeof(msg), NULL, 0, 0);
   if ( ret < 0 )
   {
      close(nl_sock);
      return ret;
   ...
   ret = bcm_mcast_api_nl_wait_rsp(nl_sock, BCM_MCAST_MSG_HOSTCTL_CFG, &buf[0], sizeof(buf));
   if ( ret < 0 )
   {
      close(nl_sock);
      return ret;
   ...
   nl_msgHdr = (struct nlmsghdr *)&buf[0];
   prsp = (t_BCM_MCAST_HOSTCTL_CFG *)NLMSG_DATA(nl_msgHdr);
   rsp_len = nl_msgHdr->nlmsg_len;
   close(nl_sock);
   if ( rsp_len < NLMSG_SPACE(sizeof(*prsp)) )
   {
...
/* Return failure if the socket buffer available for write is less than
   2% of socket send buffer size. 2% is chosen based on lab tests. This
   value can be tweaked if needed */
#define BCM_MCAST_STREAM_SOCK_AVAIL_BYTES_THRESHOLD_PERCENT 2
int bcm_mcast_api_stream_sock_send(int sd, void *data, size_t datalen) 
{
    struct msghdr msg = { 0 };
    struct iovec iov;
    int rc;
    int unsent_data_bytes=0;
    int sendbuff_size=0;
    socklen_t optlen;

    // Get buffer size
    optlen = sizeof(sendbuff_size);
    rc = getsockopt(sd, SOL_SOCKET, SO_SNDBUF, &sendbuff_size, &optlen);

    if(rc == -1)
    {
        printf("%s Error getsockopt %d:%s\n", __func__, errno, strerror(errno));
        return -1;
    }

    if ( ioctl(sd, SIOCOUTQ, &unsent_data_bytes) < 0 )
    {
        printf("%s ioctl returned error %d:%s\n", __func__, errno, strerror(errno));
        return -1;
    }

    if ( (sendbuff_size - unsent_data_bytes) < 
         ((BCM_MCAST_STREAM_SOCK_AVAIL_BYTES_THRESHOLD_PERCENT * sendbuff_size) / 100)  )
    {
        return -1;
    }
...
int bcm_mcast_api_ifname_to_idx(char *ifname, int *ifIndex)
{
    ...
    strncpy(ifr.ifr_name, ifname, MIN(IFNAMSIZ-1,strlen(ifname)));
