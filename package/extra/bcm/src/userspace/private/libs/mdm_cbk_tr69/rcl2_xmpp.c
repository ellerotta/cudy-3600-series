                ...
                CmsRet *errorCode __attribute__((unused)))
{
    ...
    else if (DELETE_EXISTING(newObj, currObj))
    {
       ...
       // send this to xmpp directly.
       rut_sendMsgCommon(EID_XMPPC, CMS_MSG_XMPP_CONNECTION_DELETE, 0,
                         1, 0, 0,
                         (void *) &msgData, sizeof(msgData));
       rutUtil_modifyNumXmppConn(iidStack, -1);
    }
    else
    {
        // Either connection enable has changed, or
        // the connection is enabled but keepAliveInterval has changed.
             ...
             (newObj->keepAliveInterval != currObj->keepAliveInterval)))
        {
           ...
           if (newObj->enable && !IS_EMPTY_STRING(newObj->username))
           {
              char jabberID[BUFLEN_1024] = {0};
              cmsUtl_generateJabberId(newObj->username, newObj->domain, newObj->resource,
                                      jabberID);
              CMSMEM_REPLACE_STRING_FLAGS(newObj->jabberID, jabberID, mdmLibCtx.allocFlags);
           }
           ...
           {
              if (newObj->serverNumberOfEntries == 0)
              {
                 cmsLog_error("Connection algo is ServerTable, but 0 servers configured!");
              }
              else
              {
                 rutXmpp_get_server_info(iidStack,
           ...
           if (newObj->enable != currObj->enable)
           {
               rut_sendMsgCommon(EID_XMPPC, CMS_MSG_XMPP_CONNECTION_ENABLE, 0,
                                 1, 0, 0,
                                 (void *) &msgData, sizeof(msgData));
           }
           else
           {
               rut_sendMsgCommon(EID_XMPPC, CMS_MSG_XMPP_CONNECTION_UPDATE, 0,
                                 1, 0, 0,
                                 (void *) &msgData, sizeof(msgData));
           }
        }
