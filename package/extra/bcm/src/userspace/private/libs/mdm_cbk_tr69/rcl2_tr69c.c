                                       ...
                                       CmsRet *errorCode __attribute((unused)))
{
   ...
   {
      UBOOL8 b = ((newObj->enableCWMP != currObj->enableCWMP) ||
                  (newObj->periodicInformEnable != currObj->periodicInformEnable) ||
                  (newObj->periodicInformInterval != currObj->periodicInformInterval) ||
                  cmsUtl_strcmp(newObj->periodicInformTime, currObj->periodicInformTime) ||
                  cmsUtl_strcmp(newObj->URL, currObj->URL) ||
      ...
      // When turning on or off CWMP, reset the iptables rule of CWMP_CHAIN 
      if (newObj->enableCWMP != currObj->enableCWMP) 
      {
         rutIpt_initCWMPChk(PF_INET, newObj->enableCWMP);
#ifdef SUPPORT_IPV6
         rutIpt_initCWMPChk(PF_INET6, newObj->enableCWMP);
#endif
      }
...
void rut_modifyNumManageableDevices_dev2(SINT32 delta)
{
   ...
   if (mdmShmCtx->inMdmInit)
   {
      ...
      cmsLog_debug("don't update count in oid %d (delta=%d)",
                   MDMOID_DEV2_MANAGEMENT_SERVER_MANAGEABLE_DEVICE, delta);
      return;
   }

   if ((cmsObj_get(MDMOID_DEV2_MANAGEMENT_SERVER, &iidStack, 0, (void *) &obj)) == CMSRET_SUCCESS)
   {
      obj->manageableDeviceNumberOfEntries += delta;
                                                      ...
                                                      CmsRet *errorCode __attribute((unused)))
{
   if (ADD_NEW(newObj, currObj))
   {
      rut_modifyNumManageableDevices_dev2(1);
   }
...
CmsRet rcl_dev2ManagementServerObject( _Dev2ManagementServerObject *newObj __attribute((unused)),
                                       const _Dev2ManagementServerObject *currObj __attribute((unused)),
                                       const InstanceIdStack *iidStack __attribute((unused)),
                                       char **errorParam __attribute((unused)),
                                       CmsRet *errorCode __attribute((unused)))
{
...
#ifdef DMP_DEVICE2_DEVICEASSOCIATION_1
CmsRet rcl_dev2ManagementServerManageableDeviceObject(_Dev2ManagementServerManageableDeviceObject *newObj __attribute((unused)),
                                                      const _Dev2ManagementServerManageableDeviceObject *currObj __attribute((unused)),
                                                      const InstanceIdStack *iidStack __attribute((unused)),
                                                      char **errorParam __attribute((unused)),
                                                      CmsRet *errorCode __attribute((unused)))
{
