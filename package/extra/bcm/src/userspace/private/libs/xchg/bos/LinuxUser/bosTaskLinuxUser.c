...
BOS_STATUS bosTaskProceedToShutdown( BOS_TASK_ID *taskid )
{
   int counter, status;
   counter =3;
   bosLogInfo("Canceling task %d", *(pthread_t*) taskid);
   while (counter--)
   {
      status = pthread_cancel( *(pthread_t*) taskid);
      if (status)
      {
         bosLogErr("Canceling task %d failed. Status=%d, Iteration=%d", *(pthread_t*) taskid, status, counter );
         bosSleep(100);
      }
      else
         break;
...
BOS_STATUS bosTaskWaitForDone( BOS_TASK_ID *taskid )
{
   ...
   while (counter--)
   {
      ...
      if (status)
      {
         bosLogErr("Wait for task %d failed. Status=%d, Iteration=%d", *(pthread_t*) taskid, status, counter );
         bosSleep(100);
      }
      else
         break;
