...
int fcCtlConfig(int option, int arg1)
{
    ...
    switch (option)
    {
        ...
        case FCACHE_CONFIG_OPT_LOW_PKT_RATE:
            ret = fcDevIoctl( FCACHE_IOCTL_LOW_PKT_RATE, -1, arg1 );
            break;
        ...
        case FCACHE_CONFIG_OPT_TX_THREAD:
            {
                ret = fcDevIoctl( FCACHE_IOCTL_SET_TX_THREAD, -1, arg1 );
            }
            break;

        case FCACHE_CONFIG_OPT_TX_THREAD_IDELAY:
            {
                ret = fcDevIoctl( FCACHE_IOCTL_SET_TX_THREAD_IDELAY, -1, arg1 );
            }
            break;

        case FCACHE_CONFIG_OPT_NOTIFY_PROC_MODE:
            {
                ret = fcDevIoctl( FCACHE_IOCTL_SET_NOTIFY_PROC_MODE, -1, arg1 );
            }
            break;
        ...
        case FCACHE_CONFIG_OPT_TOS_MFLOWS:
            ret = fcDevIoctl( FCACHE_IOCTL_TOS_MFLOWS, -1, arg1 );
            break;

        case FCACHE_CONFIG_OPT_UNKNOWN_UCAST:
            ret = fcDevIoctl( FCACHE_IOCTL_UNKNOWN_UCAST, -1, arg1 );
            break;

        case FCACHE_CONFIG_OPT_IDLE_TIMER_TCP:
            ret = fcDevIoctl( FCACHE_IOCTL_IDLE_TIMER_TCP, -1, arg1 );
            break;

        case FCACHE_CONFIG_OPT_IDLE_TIMER_UDP:
            ret = fcDevIoctl( FCACHE_IOCTL_IDLE_TIMER_UDP, -1, arg1 );
            break;

        case FCACHE_CONFIG_OPT_IDLE_TIMER_L2:
            ret = fcDevIoctl( FCACHE_IOCTL_IDLE_TIMER_L2, -1, arg1 );
            break;

        case FCACHE_CONFIG_OPT_IDLE_TIMER_MCAST:
            ret = fcDevIoctl( FCACHE_IOCTL_IDLE_TIMER_MCAST, -1, arg1 );
            break;

        case FCACHE_CONFIG_OPT_IDLE_TIMER_UNKNOWN_UCAST:
            ret = fcDevIoctl( FCACHE_IOCTL_IDLE_TIMER_UNKNOWN_UCAST, -1, arg1 );
            break;

        case FCACHE_CONFIG_OPT_IDLE_TIMER_OTHER:
            ret = fcDevIoctl( FCACHE_IOCTL_IDLE_TIMER_OTHER, -1, arg1 );
            break;

        case FCACHE_CONFIG_OPT_PURE_LLC:
            ret = fcDevIoctl( FCACHE_IOCTL_PURE_LLC, -1, arg1 );
            break;

        case FCACHE_CONFIG_OPT_RX_PKTLEN_MODE:
            ret = fcDevIoctl( FCACHE_IOCTL_RX_PKTLEN_MODE, -1, arg1 );
            break;
 ...
 *------------------------------------------------------------------------------
 */
int fcCtlDump(int option, int arg1, int arg2, int arg3)
{
    int ret = FCACHE_ERROR;
    FcDumpInfoParams_t params = {};
    ...
    switch (option)
    {
        case FCACHE_DUMP_OPT_FLOW_INFO:
            params.flow.arg = arg1;
            break;

        case FCACHE_DUMP_OPT_BLOG_INFO:
            params.blog.arg = arg1;
            break;

        case FCACHE_DUMP_OPT_MCAST_BM_INFO:
            params.mcast_bm.num_of_bm = arg1;
            params.mcast_bm.bm_start_idx = arg2;
            params.mcast_bm.only_in_use_bm = arg3;
            break;

        case FCACHE_DUMP_OPT_UNKNOWN_UCAST_BM_INFO:
            params.unknown_ucast_bm.num_of_bm = arg1;
            params.unknown_ucast_bm.bm_start_idx = arg2;
            params.unknown_ucast_bm.only_in_use_bm = arg3;
            break;
    ...
    ret = fcDevIoctl(FCACHE_IOCTL_DUMP_DEBUG_INFO, -1, (uintptr_t)&params);
...
int fcCtlReserveNfFlwStatsQuery(FlwStatsQueryInfo_t *queryInfo_p)
{
    int retVal=0;
    
    /* Issue IOCTL to create a new flowstats query */
    retVal = fcDevIoctl( FCACHE_IOCTL_RESERVE_NFFLWSTATS, -1, (uintptr_t)queryInfo_p);
...
int fcCtlGetNfFlwStatsQuery(FlwStatsQueryInfo_t *queryInfo_p)
{
    int retVal = 0;
    
    /* Issue IOCTL to run a flowstats query */
    retVal = fcDevIoctl( FCACHE_IOCTL_GET_NFFLWSTATS, -1, (uintptr_t)queryInfo_p);
...
int fcCtlDeleteNfFlwStatsQuery(FlwStatsQueryInfo_t *queryInfo_p)
{
    /* Issue IOCTL to delete a flowstats query */
    return fcDevIoctl( FCACHE_IOCTL_DELETE_NFFLWSTATS, -1, (uintptr_t)queryInfo_p); 
}
...
{
    return fcDevIoctl(FCACHE_IOCTL_GET_FLOWSTATS_POLL_PARAMS, -1, (uintptr_t)(pollparams));	
}

int fcCtlSetFlwPollParams(FlwStatsPollParams_t *pollparams)
{
    return fcDevIoctl(FCACHE_IOCTL_SET_FLOWSTATS_POLL_PARAMS, -1, (uintptr_t)(pollparams));	
}
...
int fcCtlClearNfFlwStatsQuery(FlwStatsQueryInfo_t *queryInfo_p)
{
    /* Issue IOCTL to clear a flowstats query */
    return fcDevIoctl(FCACHE_IOCTL_CLEAR_NFFLWSTATS, -1, (uintptr_t)queryInfo_p);
}
...
int fcCtlGetQueryEntryNumberNF(void)
{
    /* Issue IOCTL to get number of queries in fc */
    return fcDevIoctl( FCACHE_IOCTL_GET_NFFLWSTATS_NUM, -1, 0);
}
...
int fcCtlDumpNfFlwStats(NfFlwStatsDumpInfo_t *flwDumpInfo_p)
{
    /* Issue IOCTL to delete a flowstats query */
    return fcDevIoctl( FCACHE_IOCTL_DUMP_NFFLWSTATS, -1, (uintptr_t)flwDumpInfo_p);
}
...
{	
	...
	FILL_TUPLE_FIELD(pQuery,
                     rxinnervid,
                     FLWSTATS_FIELDNAME_RXINVID,
                     FLWSTATS_QUERYMASK_RXINVID,
                     strtol(pcValue, NULL, 0))
	FILL_TUPLE_FIELD(pQuery,
                     rxinnerpbits,
                     FLWSTATS_FIELDNAME_RXINPBITS,
                     FLWSTATS_QUERYMASK_RXINPBITS,
                     strtol(pcValue, NULL, 0))
	FILL_TUPLE_FIELD(pQuery,
                     rxoutervid,
                     FLWSTATS_FIELDNAME_RXOUTVID,
                     FLWSTATS_QUERYMASK_RXOUTVID,
                     strtol(pcValue, NULL, 0))
	FILL_TUPLE_FIELD(pQuery,
                     rxouterpbits,
                     FLWSTATS_FIELDNAME_RXOUTPBITS,
                     FLWSTATS_QUERYMASK_RXOUTPBITS,
                     strtol(pcValue, NULL, 0))
	FILL_TUPLE_FIELD(pQuery,
                     txinnervid,
                     FLWSTATS_FIELDNAME_TXINVID,
                     FLWSTATS_QUERYMASK_TXINVID,
                     strtol(pcValue, NULL, 0))
	FILL_TUPLE_FIELD(pQuery,
                     txinnerpbits,
                     FLWSTATS_FIELDNAME_TXINPBITS,
                     FLWSTATS_QUERYMASK_TXINPBITS,
                     strtol(pcValue, NULL, 0))
	FILL_TUPLE_FIELD(pQuery,
                     txoutervid,
                     FLWSTATS_FIELDNAME_TXOUTVID,
                     FLWSTATS_QUERYMASK_TXOUTVID,
                     strtol(pcValue, NULL, 0))
	FILL_TUPLE_FIELD(pQuery,
                     txouterpbits,
                     FLWSTATS_FIELDNAME_TXOUTPBITS,
                     FLWSTATS_QUERYMASK_TXOUTPBITS,
                     strtol(pcValue, NULL, 0))

	FILL_TUPLE_TOS_FIELD(pQuery,
                     FLWSTATS_FIELDNAME_TXTOS,
                     FLWSTATS_QUERYMASK_TXTOS,
                     strtol(pcValue, NULL, 0), tosCount)
	FILL_TUPLE_TOS_FIELD(pQuery,
                     FLWSTATS_FIELDNAME_RXTOS,
                     FLWSTATS_QUERYMASK_RXTOS,
                     strtol(pcValue, NULL, 0), tosCount)
	FILL_TUPLE_TOS_FIELD(pQuery,
                     FLWSTATS_FIELDNAME_TXDSCP,
                     FLWSTATS_QUERYMASK_TXDSCP,
                     strtol(pcValue, NULL, 0), tosCount)
	FILL_TUPLE_TOS_FIELD(pQuery,
                     FLWSTATS_FIELDNAME_RXDSCP,
                     FLWSTATS_QUERYMASK_RXDSCP,
                     strtol(pcValue, NULL, 0), tosCount)

	FILL_TUPLE_FIELD(pQuery,
                     rxchannel,
                     FLWSTATS_FIELDNAME_RXCHANNEL,
                     FLWSTATS_QUERYMASK_RXCHANNEL,
                     strtol(pcValue, NULL, 0))
	/* txchannel field is part of pQuery->mask2. So call
       the FILL_TUPLE_FIELD_2 macro */
	FILL_TUPLE_FIELD_2(pQuery,
                       txchannel,
                       FLWSTATS_FIELDNAME_TXCHANNEL,
                       FLWSTATS_QUERYMASK_TXCHANNEL,
                       strtol(pcValue, NULL, 0))

	if (tosCount > 1)
	{
		printf("ERROR:  only one of rxtos, txtos, rxdscp, txdscp can be specified\n");
	...
	if(!strcasecmp(pcField, FLWSTATS_FIELDNAME_SRCPHY))
	{
		...
		strncpy((char *)pQuery->create.queryTuple.srcphy, pcValue, FLWSTATS_PHY_LEN-1);
                pQuery->create.queryTuple.srcphy[FLWSTATS_PHY_LEN-1] = '\0';
	...
	if(!strcasecmp(pcField, FLWSTATS_FIELDNAME_DSTPHY))
	{
		...
		strncpy((char *)pQuery->create.queryTuple.dstphy, pcValue, FLWSTATS_PHY_LEN-1);
                pQuery->create.queryTuple.dstphy[FLWSTATS_PHY_LEN-1] = '\0';
    ...
    if(pQuery->create.queryTuple.mask == 0 && pQuery->create.queryTuple.mask2 == 0)
    {
        /* Nope. Flag an error. */
