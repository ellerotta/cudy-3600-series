...
CmsRet qdmTr69c_getManagementServerCfgLocked_dev2(char *url, char *connReqUser, char *connReqPwd, char *user, char *pwd)
{
   ...
   {
      cmsUtl_strcpy(url, obj->URL);
      cmsUtl_strcpy(connReqUser,obj->connectionRequestUsername);
      cmsUtl_strcpy(connReqPwd,obj->connectionRequestPassword);
      cmsUtl_strcpy(user,obj->username);
      cmsUtl_strcpy(pwd,obj->password);
...
CmsRet qdmTr69c_getBoundIfNameLocked_dev2(char *ifName)
{
   ...
   cmsUtl_strcpy(ifName, mgmtObj->X_BROADCOM_COM_BoundIfName);
...
CmsRet qdmTr69c_getUrlLocked_dev2(char *url)
{
   Dev2ManagementServerObject *mgmtObj=NULL;
   InstanceIdStack iidStack = EMPTY_INSTANCE_ID_STACK;
   CmsRet ret=CMSRET_SUCCESS;
                                                      ...
                                                      char *transferTypeFilter)
{
   ...
   *enable = policyObj->enable;
   cmsUtl_strcpy(fileTypeFilter, policyObj->fileTypeFilter);
   cmsUtl_strcpy(resultTypeFilter, policyObj->resultTypeFilter);
   cmsUtl_strcpy(transferTypeFilter, policyObj->transferTypeFilter);
                                                  ...
                                                  char *faultCodeFilter)
{
   ...
   *enable = policyObj->enable;
   cmsUtl_strcpy(opTypeFilter, policyObj->operationTypeFilter);
   cmsUtl_strcpy(resultTypeFilter, policyObj->resultTypeFilter);
   cmsUtl_strcpy(faultCodeFilter, policyObj->faultCodeFilter);
