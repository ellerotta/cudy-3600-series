                                                 ...
                                                 CmsQosQueueInfo *qInfo)
{
   ...
   qInfo->tcpPureAckQueue = qObj->X_BROADCOM_COM_TcpPureAckQueue;
                                                 ...
                                                 CmsQosClassInfo *cInfo)
{
   ...
   cInfo->icmpv6Type = cObj->X_BROADCOM_COM_Icmpv6Type;
   cInfo->icmpv6Code = cObj->X_BROADCOM_COM_Icmpv6Code;
   cInfo->icmpv6TypeCodeExclude= cObj->X_BROADCOM_COM_Icmpv6TypeCodeExclude;
...
{
   // It is a bit odd that instance and refInstance are SINT32, but they are
   // actually dealing with instanceId's which are UINT32.  Since we are only
   // comparing for equality, it is ok.  But must be careful to not compare
   // using > or <
                                             ...
                                             CmsQosPolicerInfo *pInfo)
{
   ...
   pInfo->policerInfo = pObj->X_BROADCOM_COM_PolicerInfo;
                    ...
                    OGF_NO_VALUE_UPDATE, (void **)&cObj) == CMSRET_SUCCESS)
   {
          ...
          (cObj->X_BROADCOM_COM_ClassQueue > 0))
      {
         ...
         else
         {
            ...
            {
               if(qdmIntf_isFullPathLayer2Locked_dev2(cObj->interface))
               {
                  ingressUpstream = qdmIntf_isLayer2IntfNameUpstreamLocked(ingressIntfNameBuf);
               }
               else
               {
                  ingressUpstream = qdmIpIntf_isIntfNameUpstreamLocked_dev2(ingressIntfNameBuf);
               }
            }
         }
...
UBOOL8 qdmQos_isAnyInterfaceOfQueuesMatchedLocked_dev2(const void *obj)
{
   Dev2QosQueueObject *qObj = NULL;
   InstanceIdStack iidStack;
   CmsRet ret;
   Dev2QosQueueObject *dev2QObj= (_Dev2QosQueueObject*)obj;
