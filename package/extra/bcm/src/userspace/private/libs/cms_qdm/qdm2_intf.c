                                             ...
                                             char **fullPathBuf)
{
   ...
   if (layer2)
   {
      ...
      if ((strncmp(intfName, GPON_IFC_STR, strlen(GPON_IFC_STR)) == 0) ||
          (strncmp(intfName, EPON_IFC_STR, strlen(EPON_IFC_STR)) == 0))
      {
          while ((!found) &&
      ...
      INIT_INSTANCE_ID_STACK(&iidStack);
      if (strncmp(intfName, PTM_IFC_STR, strlen(PTM_IFC_STR)) == 0)
      {
          while ((!found) &&
      ...
      INIT_INSTANCE_ID_STACK(&iidStack);
      if ((strncmp(intfName, ATM_IFC_STR, strlen(ATM_IFC_STR)) == 0)
          ||  (strncmp(intfName, IPOA_IFC_STR, strlen(IPOA_IFC_STR)) == 0))
      {
          while ((!found) &&
      ...
      INIT_INSTANCE_ID_STACK(&iidStack);
      if (strncmp(intfName, ETH_IFC_STR, strlen(ETH_IFC_STR)) == 0)
      {
          while ((!found) &&
       ...
       */
      INIT_INSTANCE_ID_STACK(&iidStack);
      if (strncmp(intfName, PPP_IFC_STR, strlen(PPP_IFC_STR)) == 0)
      {
          while ((!found) &&
      ...
      INIT_INSTANCE_ID_STACK(&iidStack);
      if ((strncmp(intfName, WLAN_IFC_STR, strlen(WLAN_IFC_STR)) == 0) ||
          (strncmp(intfName, "wds", 3) == 0))
      {
          char wlifname[8] = {0};
          int wl_index = 0;
      ...
      INIT_INSTANCE_ID_STACK(&iidStack);
      if (strncmp(intfName, USB_IFC_STR, strlen(USB_IFC_STR)) == 0)
      {
          while ((!found) &&
      ...
      INIT_INSTANCE_ID_STACK(&iidStack);
      if (strncmp(intfName, ETHLAG_IFC_STR, strlen(ETHLAG_IFC_STR)) == 0)
      {
          while ((!found) &&
      ...
      INIT_INSTANCE_ID_STACK(&iidStack);
      if ((strncmp(intfName, VXLAN_IFC_STR, strlen(VXLAN_IFC_STR)) == 0))
      {
          while ((!found) &&
                 ((ret = cmsObj_getNextFlags(MDMOID_DEV2_VXLAN_TUNNEL_INTERFACE,
                     &iidStack, OGF_NO_VALUE_UPDATE, &mdmObj)) == CMSRET_SUCCESS))
          {
              found = (0 == cmsUtl_strcmp(((Dev2VxlanTunnelInterfaceObject*)mdmObj)->name, intfName));
              if (found)
              {
                  pathDesc.oid = MDMOID_DEV2_VXLAN_TUNNEL_INTERFACE;
                  pathDesc.iidStack = iidStack;
              }
              cmsObj_free(&mdmObj);
          }
   ...
   else
   {
      ...
      if (!found)
      {
          ...
          */
         INIT_INSTANCE_ID_STACK(&iidStack);
         while (cmsObj_getNextFlags(MDMOID_DEV2_ETHERNET_LINK, &iidStack, OGF_NO_VALUE_UPDATE, &mdmObj) == CMSRET_SUCCESS)
         {
            if (0 == cmsUtl_strcmp(((Dev2EthernetLinkObject*)mdmObj)->name, intfName))
            {
               found = TRUE;
      ...
      INIT_INSTANCE_ID_STACK(&iidStack);
      while ((!found) &&
             ((ret = cmsObj_getNextFlags(MDMOID_DEV2_GRE_TUNNEL_INTERFACE, &iidStack,
                                 OGF_NO_VALUE_UPDATE, &mdmObj)) == CMSRET_SUCCESS))
      {
         found = !cmsUtl_strcmp(((Dev2GreTunnelInterfaceObject *)mdmObj)->name, intfName) &&
                   !((Dev2GreTunnelInterfaceObject *)mdmObj)->X_BROADCOM_COM_L2_Mode;
         if (found)
         {
            pathDesc.oid = MDMOID_DEV2_GRE_TUNNEL_INTERFACE;
            pathDesc.iidStack = iidStack;
         }
         cmsObj_free(&mdmObj);
      }
...
#endif
#ifdef SUPPORT_VXLAN_TUNNEL_TR181
   else if (pathDesc.oid == MDMOID_DEV2_VXLAN_TUNNEL_INTERFACE)
   {
      /* these objects do not have an Upstream param, but we know they are upstream */
      isUpstream = TRUE;
   }
...
UBOOL8 qdmIntf_isEnabledOnFullPathLocked_dev2(const char *fullPath)
{
   MdmPathDescriptor pathDesc;
   CmsRet ret;
   ...
   return (qdmIntf_isEnabledOnPathDescLocked_dev2(&pathDesc));
}

UBOOL8 qdmIntf_isEnabledOnPathDescLocked_dev2(const MdmPathDescriptor *pathDescIn)
{
   MdmPathDescriptor pathDesc;
   PhlGetParamValue_t   *pParamValue = NULL;
   CmsRet ret;
