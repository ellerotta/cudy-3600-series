...
UBOOL8 qdmDsl_isAtmConnectionLocked(const char *lowerLayers)
{
   ...
   ret = cmsMdm_fullPathToPathDescriptor(lowerLayers, &pathDesc);
   if (ret != CMSRET_SUCCESS)
   {
      cmsLog_error("qdmDsl_isAtmConnectionLocked : cmsMdm_fullPathToPathDescriptor fail. ret = %d", ret);
      return FALSE;
   }
   ...
   ret = (CmsRet) xdslCtl_GetObjectValue(dslLineObj->X_BROADCOM_COM_BondingLineNumber, NULL, 0, (char *) &adslMib, &adslMibSize);
   if (ret != CMSRET_SUCCESS)
   {
...
void qdmDsl_getDSLTrainedModeLocked_dev2(UBOOL8 *isVdsl, UBOOL8 *isAtm)
{
   ...
   *isAtm = TRUE;
   ...
   if (found)
   {
      ret = (CmsRet) xdslCtl_GetObjectValue(lineNumber, NULL, 0, (char *) &adslMib, &adslMibSize);
      if (ret != CMSRET_SUCCESS)
      {
 ...
 */
void qdmDsl_isDslBondingGroupStatusOperational_dev2(char *name,UBOOL8 *operational)
{
   UBOOL8 found = FALSE;
   Dev2DslBondingGroupObject *dslBondingGroupObj = NULL;
   InstanceIdStack iidStack = EMPTY_INSTANCE_ID_STACK;
   ...
   while (!found && cmsObj_getNext(MDMOID_DEV2_DSL_BONDING_GROUP, &iidStack, (void **) &dslBondingGroupObj) == CMSRET_SUCCESS)
   {
      if (!cmsUtl_strcmp(dslBondingGroupObj->name, name))
      {
         found = TRUE;
...
UBOOL8 qdmDsl_isAnyLowerLayerChannelUpLocked_dev2(char *lowerLayers)
{
   ...
   while (pToken)
   {
      ...
      {
         if (lowerLayerPathDesc.oid == MDMOID_DEV2_FAST_LINE)
         {
            Dev2FastLineObject *fastLineObj=NULL;
            
            if (cmsObj_get(lowerLayerPathDesc.oid,&lowerLayerPathDesc.iidStack,
                           0,(void **)&fastLineObj) == CMSRET_SUCCESS)
            {
         ...
         else if (lowerLayerPathDesc.oid == MDMOID_DEV2_DSL_CHANNEL)
         {
            Dev2DslChannelObject *dslChannelObj=NULL;
         ...
         else if (lowerLayerPathDesc.oid == MDMOID_DEV2_DSL_BONDING_GROUP)
         {
            Dev2DslBondingGroupObject *dslBondingGroupObj=NULL;
            
            if (cmsObj_get(lowerLayerPathDesc.oid,&lowerLayerPathDesc.iidStack,
                           0,(void **)&dslBondingGroupObj) == CMSRET_SUCCESS)
            {
               if (!cmsUtl_strcmp(dslBondingGroupObj->status,MDMVS_UP))
               {
                  found = TRUE;
               }
               cmsObj_free((void **)&dslBondingGroupObj);
         ...
         {
            cmsLog_error("Unexpected lowerlayer %s (oid=%d)", pToken, lowerLayerPathDesc.oid);
         }
