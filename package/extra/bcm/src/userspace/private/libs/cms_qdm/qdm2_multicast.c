                 ...
                 UINT32 *mode, int *lanToLanEnable, UBOOL8 *enabled)
{
   ...
   *enabled = FALSE;
   ...
   if (!isMld)
   {
      ...
      modeRet = qdmMulti_getAssociatedBridgeModeLocked(fullPath, &bridgeMode);
      if (modeRet != CMSRET_SUCCESS)
      {
         cmsLog_error("could not get bridge's mode, ret=%d", modeRet);
                                                   ...
                                                   int *mode)
{
   ...
   *mode = INTFGRP_BR_HOST_MODE;
   ...
   if (ret != CMSRET_SUCCESS)
   {
      cmsLog_notice("Could not get bridge obj, ret=%d", ret);
   }
   else
   {
      *mode = brObj->X_BROADCOM_COM_Mode;
      cmsObj_free((void **)&brObj);
   }
