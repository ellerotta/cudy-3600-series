...
UBOOL8 qdmIpIntf_isLanDhcpcEnabled_dev2(const char *ifName)
{
   CmsRet ret = CMSRET_OBJECT_NOT_FOUND;
   UBOOL8 isWan = FALSE, isStatic = FALSE;
   /* this object is Device.IP.Interface.{i}.IPv4Address.{i}. */

   if ((ret = qdmIpIntf_getIpv4AddrInfoByNameLocked_dev2(ifName, NULL, NULL, &isWan, &isStatic)) != CMSRET_SUCCESS) {
      /* during transiiton from static to dhcp, original static ipv4AddrObj
       * will be deleted, and new ipv4AddrObj will be added by smd after receving
       * message from dhcpc. Consequently, there is no object found during transition
       * period. However, it is not a problem cuz GUI will never be avaialble either.
       * it's fine to ignore not found error in the case.  
       */
      if (ret != CMSRET_OBJECT_NOT_FOUND) {
         cmsLog_error("Failed to qdmIpIntf_getIpv4AddrInfoByNameLocked_dev2, ret=%d", ret);
      }
   }
   return (!isWan && !isStatic);
}
         ...
         else
         {
            if (!cmsUtl_strcmp(ifName, intfName))
            {
               ...
               else
               {
                  ...
                  {
                     found = TRUE;
                     isBridge = FALSE; //Use vlanctl rule to replace pppoe-relay, isBridge should be FALSE
                  }
