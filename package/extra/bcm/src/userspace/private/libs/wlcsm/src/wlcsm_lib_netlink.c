...
char *wlcsm_mngr_get(unsigned int radio_idx,unsigned int sub_idx,char * name, char *value)
{
    ...
    char nv_name[128]={0};
    int i=0;
    fprintf(stderr,"%s:%d: %d:%d and name:%s\n",__FUNCTION__,__LINE__,radio_idx,sub_idx,name);
    do {
        if(strlen(name)==strlen(g_wlcsm_nvram_mngr_mapping[i].mngr_name) &&
                !strncmp(name,g_wlcsm_nvram_mngr_mapping[i].mngr_name,strlen(name))) {
            if(g_wlcsm_nvram_mngr_mapping[i].pos == MNGR_POS_RADIO) 
                snprintf(nv_name,127,"wl%d_%s",radio_idx,g_wlcsm_nvram_mngr_mapping[i].nvram_name);
            else if(sub_idx>0)
                snprintf(nv_name,127,"wl%d.%d_%s",radio_idx,sub_idx,g_wlcsm_nvram_mngr_mapping[i].nvram_name);
            if((nv_value = wlcsm_nvram_get(nv_name))) {
                memcpy(value,nv_value,strlen(nv_value));
                return value;
            } else
                return NULL;
        }
    } while( (++i) < MNGR_NVRAM_MLEN );
    return NULL;
#else
    int current_pos=0;
...
{
#ifdef UNF_CFG
    fprintf(stderr,"%s:%d set %s to %s\n",__FUNCTION__,__LINE__,name,value?value:"NULL");
    return WLCSM_SUCCESS;
#else
