...
// CM has defined MAX radio 3, here to change it
// to 3 to be able to build with RDKWIFI
#define MAX_WLAN_ADAPTER 3
...
typedef struct {
    ...
    char ssid[WL_SSID_SIZE_MAX];
    char ifcName[WL_SM_SIZE_MAX];
    char authenticationState;   /**< AuthenticationState */
    char active;                /**< Active */
    ...
    int snr;
    char operStandard[64];
    unsigned int operBandwidth;
    unsigned int txTotalPkts;
    unsigned int txTotalPktsSent;
    unsigned int rxDataPkts;
    unsigned int txFailures;
    unsigned int txPktsRetries;
    unsigned int txFwPktsRetries;
    unsigned int txPktsRetryExhaused;
    unsigned int txFwPktsRetryExhaused;
...
/* Channel Switch Announcement struct */
typedef struct csa {
    unsigned int channel;			/* channel */
    unsigned int channel_width;			/* channel width */
    unsigned int csa_beacon_count;		/* CSA beacon count */
} csa_t;

typedef enum
{
    CSA_DEAUTH_MODE_NONE = 0,
    CSA_DEAUTH_MODE_UCAST,
    CSA_DEAUTH_MODE_BCAST
} csaDeauthMode_t;


typedef struct _associatedDevRateInfoStats {
        unsigned char nss;                              /* 0 equals legacy protocolss (OFDM, CCK) 1 - n spatial stream (HT, VHT) */
        unsigned char mcs;                              /* 0 - 7 (HT) - 9 (VHT) */
        unsigned short int bw;                          /* 20, 40, 80, 160 ... (to be considered 5 , 10, 80+80) ... */
        unsigned long long int flags;                   /* Flag indicating data validation that HAS_BYTES, HAS_MSDUS, HAS_MPDUS, HAS_PPDUS, HAS_BW_80P80, HAS_RSSI_COMB, HAS_RSSI_ARRAY */
        unsigned long long int bytes;                   /* number of bytes received for given rate */
        unsigned long long int msdus;                   /* number of MSDUs received for given rate */
        unsigned long long int mpdus;                   /* number of MPDUs received for given rate */
        unsigned long long int ppdus;                   /* number of PPDUs received for given rate */
        unsigned long long int retries;                 /* number of retries received for given rate */
        union {
                struct {
                        unsigned char rssi_combined;                    /* Last RSSI received on give rate */
                        unsigned char rssi_array[8][4];                 /* 8=antennas, 4=20+20+40+80 extension rssi */
                } rx;
                struct {
                        unsigned long long int attempts;		/* number of attempts trying transmitt on given rate */
                } tx;
        };
} wifi_associatedDevRateInfoStats_t;
...
#define DCS_SCANRESULTS_BUF_LEN (32*1024)
#define DCS_CHANIM_BUF_LEN		(2*1024)
...
#else
#define WLAN_APPS_LOGS(origin, str)
#endif /* CMWIFI_RDKB */
