...
int wlcsm_nvram_commit_reqd(void)
{
#ifdef BUILD_RDKWIFI
    nvram_set_count=nvram_get_count();
    fprintf(stderr, "%s: %d changed!\n", __FUNCTION__, nvram_set_count);
#endif
    return nvram_set_count ? 1 : 0;
}
...
int wlcsm_nvram_commit (void)
{
    ...
    if(wlcsm_netlink_send_mesg(WLCSM_MSG_NVRAM_COMMIT,NULL,0)== WLCSM_SUCCESS) {
        ...
        if(hdr!=NULL && hdr->type==WLCSM_MSG_NVRAM_COMMIT) {
            ret=WLCSM_SUCCESS;
            nvram_set_count = 0;
        }
    }
