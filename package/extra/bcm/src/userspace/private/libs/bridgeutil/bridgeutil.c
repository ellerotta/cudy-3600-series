...
int br_util_get_bridge_ifindex_from_lower_ifname(const char *ifname)
{
   ...
   snprintf(sfspath, SYSFS_PATH_MAX, "/sys/class/net/%s/brport/bridge/ifindex", ifname);
   fp = fopen(sfspath, "r");
   if (fp != NULL)
   {
      rt = fscanf(fp, "%i", &ifindex);
   ...
   if (dp != NULL)
   {
      while (((ep = readdir(dp)) != NULL) && (wanidx < *num))
      {
         ...
         if (fp != NULL)
         {
            ...
            if ( rt != 0 )
            {
                if (bcmnet_ioctl_iswandev(ep->d_name) == 1)
                {
...
int br_util_read_fdb(const char *ifname, struct __fdb_entry *fdbs, int offset, int num)
{
   ...
   if (f)
   {
      if(fseek(f, offset*sizeof(struct __fdb_entry), SEEK_SET) == 0)
          ...
          printf("br_util_read_fdb : fseek fail\n");
...
int br_util_set_aging(const char *brname, unsigned int age)
{
    ...
    snprintf(sfspath, SYSFS_PATH_MAX, "/sys/class/net/%s/bridge/ageing_time", brname);
