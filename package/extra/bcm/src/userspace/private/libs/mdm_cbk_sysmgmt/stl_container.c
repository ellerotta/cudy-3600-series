...
static CmsRet container_get_info(const char *container_name, PCONTAINER_INFO info)
{
   ...
   {
      if (cmsUtl_strcmp(euObj->X_BROADCOM_COM_ContainerName, container_name) == 0)
      {
         found = TRUE;
   ...
   cmsUtl_strncpy(euContainer, container_name, sizeof(euContainer));
   ...
   msg->type = (CmsMsgType) CMS_MSG_CONTAINER_STATUS_REFRESH;
   msg->src = mdmLibCtx.eid;
#ifdef SUPPORT_MODSWD
   msg->dst = EID_MODSWD;
#else
   msg->dst = EID_BBCD;
#endif
   ...
   cmsUtl_strncpy(reqMsg->eeContainer, eeContainer, sizeof(eeContainer));
   cmsUtl_strncpy(reqMsg->euContainer, euContainer, sizeof(euContainer));
                               ...
                               int instance_id)
{
   ...
   char infoName[MDM_SINGLE_FULLPATH_BUFLEN]={0};
   char *pChar = NULL;
   UBOOL8 found = FALSE;
   InstanceIdStack iidStack = EMPTY_INSTANCE_ID_STACK;
   InstanceIdStack eeIidStack = EMPTY_INSTANCE_ID_STACK;
   ...
   /* Look for EE object that has its X_BROADCOM_COM_ContainerName matched with the given info name */
   while (found == FALSE &&
   ...
   {
      if (cmsUtl_strcmp(info->name, eeObj->X_BROADCOM_COM_ContainerName) == 0)
      {
         found = TRUE;

         /* Update execEnvRef parameter in container info object */
         REPLACE_STRING_IF_NOT_EQUAL(containerInfoObj->execEnvRef, eeObj->parentExecEnv);
      }
   ...
   /* info->name of EU object can be ExampleEE3_spMaster:0, OPS_HOSTEE_dsldiagd_1.2, etc... 
    * euObj->EUID of EU object can be ExampleEE3_spMaster_2.2:1, OPS_HOSTEE_dsldiagd_1.2 */
   cmsUtl_strncpy(infoName, info->name, sizeof(infoName));
   /* remove the last ':' part  */
   pChar = strrchr(infoName, ':');
   if (pChar != NULL)
      *pChar = '\0';

   /* Look for EU object that has its EUID contains infoName */
   while (found == FALSE &&
          cmsObj_getNextFlags(MDMOID_EU,
                              ...
                              (void **) &euObj) == CMSRET_SUCCESS)
   {
      if (strstr(euObj->EUID, infoName) != NULL)
      {
         found = TRUE;
