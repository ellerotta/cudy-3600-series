...
void rutUtil_modifyNumSampleSets_dev2(SINT32 delta)
{
   InstanceIdStack iidStack = EMPTY_INSTANCE_ID_STACK;
   Dev2PeriodicStatObject *obj = NULL;
   ...
   if (mdmShmCtx->inMdmInit)
   {
      ...
      cmsLog_debug("don't update count in oid %d (delta=%d)",
                   MDMOID_DEV2_SAMPLE_SET, delta);
      return;
   }

   if ((cmsObj_get(MDMOID_DEV2_PERIODIC_STAT, &iidStack, 0, (void *) &obj)) == CMSRET_SUCCESS)
   {
      obj->sampleSetNumberOfEntries += delta;
...
void rutUtil_modifyNumParameters_dev2(SINT32 delta)
{
   InstanceIdStack iidStack = EMPTY_INSTANCE_ID_STACK;
   Dev2SampleSetObject *obj = NULL;
   ...
   if (mdmShmCtx->inMdmInit)
   {
       ...
       */
      cmsLog_debug("don't update count in oid %d (delta=%d)",
                   MDMOID_DEV2_SAMPLE_PARAMETER, delta);
      return;
   }

   if ((cmsObj_get(MDMOID_DEV2_SAMPLE_SET, &iidStack, 0, (void *) &obj)) == CMSRET_SUCCESS)
   {
      obj->parameterNumberOfEntries += delta;
      if ((cmsObj_set(obj, &iidStack)) != CMSRET_SUCCESS)
      {
          cmsLog_debug("cmsObj error!");
      }
                ...
                CmsRet *errorCode __attribute((unused)))
{
   ...
   if ( ADD_NEW(newObj, currObj) )
   {
      ...
      {
         cmsLog_debug("Start periodic statistic msg sent, new pid=%d", pid);
      }
   }
   return ret;
}
                ...
                CmsRet *errorCode __attribute((unused)))
{
   ...
   if ( ADD_NEW(newObj, currObj) )
   {
      rutUtil_modifyNumSampleSets_dev2(1);
   }
                ...
                CmsRet *errorCode __attribute((unused)))
{
   ...
   if ( ADD_NEW(newObj, currObj) )
   {
      rutUtil_modifyNumParameters_dev2(1);
   }
