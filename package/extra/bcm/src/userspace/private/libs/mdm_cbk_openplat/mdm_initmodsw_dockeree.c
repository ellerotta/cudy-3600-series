...
CmsRet mdm_addDefaultModSwDockerEeObjects(UINT32 *eeAddedCount)
{
          ...
          (ret = mdm_getNextObject(MDMOID_EXEC_ENV, &iidStack, (void **)&eeObj)) == CMSRET_SUCCESS)
   {
      ...
      {
         int cmp = cmsUtl_strcmp(eeObj->version, DOCKEREE_VERSION);

         if (cmp != 0)
         {
            CMSMEM_REPLACE_STRING_FLAGS(eeObj->version, DOCKEREE_VERSION,
                                        mdmLibCtx.allocFlags);
            ret = mdm_setObject((void **) &eeObj, &iidStack, FALSE);
            if (ret != CMSRET_SUCCESS)
            {
               cmsLog_error("could not set DOCKER EE, ret=%d", ret);
            }
         }

         if (cmp > 0)
         {
            cmsLog_error("DOCKER EE is downgraded from %s to %s. Docker images may not be able to run on current docker engine.",
               eeObj->version, DOCKEREE_VERSION);
         }
   ...
   if (add)
   {
      ...
      snprintf(eeType, sizeof(eeType), "%s %s %s", OPS_EE_TYPE_PREFIX, DOCKEREE_TYPE, eeObj->version);
...
#endif /* DMP_DEVICE2_X_BROADCOM_COM_MODSW_DOCKEREE_1 */
