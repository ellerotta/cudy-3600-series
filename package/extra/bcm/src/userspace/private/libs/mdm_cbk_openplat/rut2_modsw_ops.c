...
void rutModsw_sendMsgTo(CmsEntityId dstEid, CmsModSwMsgType msgType,
                        char *payload, int payloadLen)
{
   ...
   reqMsg = cmsMem_alloc(sizeof(CmsMsgHeader) + payloadLen, ALLOC_ZEROIZE);
   ...
   reqMsg->type = (CmsMsgType) msgType;
   reqMsg->src = mdmLibCtx.eid;
   reqMsg->dst = dstEid;
   reqMsg->flags_event = 1;
   reqMsg->dataLength = payloadLen;

   msgPayload = (char *)(reqMsg + 1);
   memcpy(msgPayload, payload, payloadLen);
