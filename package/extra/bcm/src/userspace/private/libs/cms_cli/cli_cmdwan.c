...
static CmsRet validateLinkType(char *input, char *linkType, int len);
...
CmsRet cmdWanAddAtmIntf(SINT32 argc, char **argv)
{
   ...
   for (i = 1; (i < argc) && (ret == CMSRET_SUCCESS); ++i)
   {
      if (strcasecmp(argv[i], "--linktype") == 0)
      {
         ...
         if ((ret = validateLinkType(argv[i], glbWebVar.linkType, sizeof(glbWebVar.linkType))) != CMSRET_SUCCESS)
         {
            cmdWanHelp("atmintf");
            return ret;
         }
      ...
      else if (strcasecmp(argv[i], "--encap") == 0)
      {
         ...
         {
            strncpy(encap, argv[i], sizeof(encap)-1);
         }
      }
   ...
   /* set drop algorithm of the default queue to DT */
   strncpy(glbWebVar.dropAlgorithm, MDMVS_DT, sizeof(glbWebVar.dropAlgorithm)-1);
...
CmsRet cmdWanAddEthIntf(SINT32 argc, char **argv)
{
   ...
   strncpy(glbWebVar.wanL2IfName, argv[0], sizeof(glbWebVar.wanL2IfName)-1 );
   glbWebVar.connMode = 1; // vlanmux mode is always true.
      ...
      else if (strcasecmp(argv[i], "--protocol") == 0)
      {
         if ((ret = validateProtocol(argv[++i], &glbWebVar.ntwkPrtcl)) == CMSRET_SUCCESS)
         {
            if (glbWebVar.ntwkPrtcl == PROTO_BRIDGE)
            {
               ...
               strncpy(glbWebVar.dnsPrimary, "0.0.0.0", sizeof(glbWebVar.dnsPrimary)-1);
               strncpy(glbWebVar.dnsSecondary, "0.0.0.0", sizeof(glbWebVar.dnsSecondary)-1);
            }
...
//#endif
      ...
      else if (strcasecmp(argv[i], "--ipaddr") == 0)
      {
         ...
         {
            strncpy(&glbWebVar.wanIpAddress[0], argv[i], sizeof(glbWebVar.wanIpAddress)-1);
            if (cli_isIpAddress(argv[++i]))
            {
               strncpy(&glbWebVar.wanSubnetMask[0], argv[i], sizeof(glbWebVar.wanSubnetMask)-1);
            }
            else
            {
      ...
      {
         i++;
         strncpy(glbWebVar.dnsIfcsList, argv[i], sizeof(glbWebVar.dnsIfcsList)-1);
      ...
      {
         i++;
         strncpy(glbWebVar.wanIfName, argv[i], sizeof(glbWebVar.wanIfName)-1);
      }
...
CmsRet cmdWanDeleteInterface(SINT32 argc, char **argv)
{
   ...
   if (!cmsUtl_strcmp(argv[0], "eth"))
   {
      ...
      strncpy(glbWebVar.wanL2IfName, argv[1], sizeof(glbWebVar.wanL2IfName)-1);
...
CmsRet cmdWanDeleteService(SINT32 argc, char **argv)
{
   ...
   strncpy(glbWebVar.wanIfName, argv[0], sizeof(glbWebVar.wanIfName)-1);
 ...
 * Description  : Processes the wan --show command.
 ...
 * Description  : Processes the wan --help command.
 * Returns      : void
 ***************************************************************************/
...
#ifdef SUPPORT_DSL
CmsRet validateLinkType(char *input, char *linkType, int len)
{
   CmsRet nRet = CMSRET_SUCCESS;
   ...
   else
   {
      ...
      {
         strncpy(linkType, MDMVS_IPOA, len-1);
      }
      else if (strcasecmp(input, "pppoa") == 0)
      {
         strncpy(linkType, MDMVS_PPPOA, len-1);
      }
      else if (strcasecmp(input, "EOA") == 0)
      {
         strncpy(linkType, MDMVS_EOA, len-1);
      }
      else
      {
         nRet = CMSRET_INVALID_ARGUMENTS;
...
CmsRet validateUsername(char *username, char *pppUserName)
{
   ...
   {
      if (pppUserName != NULL)
         strncpy(pppUserName, username, sizeof(pppUserName)-1);
   }
   else
   {
      printf("\nInvalid username '%s'\n", username? username : "");
...
CmsRet validatePassword(char *password, char *pppPassword)
{
   ...
   {
      if (pppPassword != NULL)
         strncpy(pppPassword, password, sizeof(pppPassword)-1);
   }
   else
   {
      printf("\nInvalid password '%s'\n", password? password : "");
