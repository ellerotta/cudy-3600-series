...
CmsRet cliCmd_showDhcpServer_dev2(void)
{
    ...
    if (ret != CMSRET_SUCCESS)
    {
        ...
        printf("dhcpserver: unknown status");
        return ret;
    }

    if (dhcpv4ServerObj->enable == FALSE)
    {
        printf("dhcpserver: disable\n");
    ...
    if (found == FALSE)
    {
       cmsLog_error("DHCP server is not running on br0");
...
void processSntpCmd_dev2(char *cmdLine)
{
   ...
   if (pToken != NULL)
   {
      ...
      {
         for (i = 0; i < STNP_SERVER_MAX && pToken && strcmp(pToken, "-s") == 0; i++)
         {
            pToken = strtok(NULL, " ");
            if (pToken == NULL && i == 0)
            {
         ...
         if ((!error) && pToken && (0 == strcmp(pToken, "-t")))
         {
            pToken = strtok(NULL, "\n"); // Rest of string
            if (pToken == NULL)
            {
   ...
   {
      cliCmdSaveNeeded = TRUE;
      configureSntp_dev2(sntpServers, timezone);
   }
