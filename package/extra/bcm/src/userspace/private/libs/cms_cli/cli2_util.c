...
void cli_wanShowInterfaces_dev2(void)
{
   ...
   while(cmsObj_getNext(MDMOID_DEV2_ATM_LINK, &atmLinkIid, (void **) &atmLink) == CMSRET_SUCCESS)
   {
      ...
      printf("ATM\t%d\t", atmLink->X_BROADCOM_COM_ATMInterfaceId);
      cmsUtl_atmVpiVciStrToNum_dev2(atmLink->destinationAddress, &vpi, &vci);
      printf("%d/%d\t", vpi, vci);
      printf("%s\t\t", atmLink->linkType);
      if (cmsObj_get(MDMOID_DEV2_ATM_LINK_QOS, &atmLinkIid, 0, (void **)&atmLinkQos) == CMSRET_SUCCESS)
      {
         printf("%s", atmLinkQos->qoSClass);
         cmsObj_free((void **) &atmLinkQos);
      }
      printf("\t");
      printf("%s\t", atmLink->encapsulation);
      printf("%s\t\t", atmLink->X_BROADCOM_COM_ATMEnbQos ? "Yes" : "No");
      printf("%s\t\t", !cmsUtl_strcmp(atmLink->X_BROADCOM_COM_ConnectionMode, MDMVS_VLANMUXMODE) ? "Yes" : "No");
      printf("\n");
   ...
   while(cmsObj_getNext(MDMOID_DEV2_PTM_LINK, &ptmLinkIid, (void **) &ptmLink) == CMSRET_SUCCESS)
   {
      ...
      printf("PTM\t%d\t", ptmLink->X_BROADCOM_COM_PTMPortId);
      ...
      printf("%s\t\t", ptmLink->X_BROADCOM_COM_PTMEnbQos ? "Yes" : "No");
      printf("%s\t\t", !cmsUtl_strcmp(ptmLink->X_BROADCOM_COM_ConnectionMode, MDMVS_VLANMUXMODE) ? "Yes" : "No");
      printf("\n");
...
UBOOL8 cli_isValidL2IfName_dev2(const char *l2IfName)
{
   ...
   cmsUtl_strncpy(wanL2IfName, l2IfName, CMS_IFNAME_LENGTH);
