LIB := libmdm_cbk_wifi.so

all install: conditional_build


CURR_DIR := $(shell pwd)
BUILD_DIR:=$(subst /userspace, /userspace,$(CURR_DIR))
BUILD_DIR:=$(word 1, $(BUILD_DIR))
include $(BUILD_DIR)/make.common

ifneq ($(strip $(BRCM_VOICE_SUPPORT)),)
include $(BUILD_DIR)/make.voice
endif


ARCH                  := $(PROFILE_ARCH)
LIB_INSTALL_DIR       := $(BCM_FSBUILD_DIR)/private/lib
HEADER_INSTALL_DIR    := $(BCM_FSBUILD_DIR)/private/include
ALLOWED_INCLUDE_PATHS := -I . \
                         -I $(HEADER_INSTALL_DIR) \
                         -I $(BCM_FSBUILD_DIR)/public/include \
                         -I $(BUILD_DIR)/userspace/public/include  \
                         -I $(BUILD_DIR)/userspace/public/include/$(OALDIR) \
                         -I $(BUILD_DIR)/userspace/private/include  \
                         -I $(BUILD_DIR)/userspace/private/include/$(OALDIR) \
                         -I $(INC_BRCMDRIVER_PUB_PATH)/$(BRCM_BOARD)  \
                         -I $(INC_BRCMDRIVER_PRIV_PATH)/$(BRCM_BOARD) \
                         -I $(INC_BRCMSHARED_PUB_PATH)/$(BRCM_BOARD) \
                         -I $(BUILD_DIR)/userspace/private/libs/cms_core \
                         -I $(BUILD_DIR)/userspace/private/libs/cms_core/$(OALDIR) \
                         -I $(BUILD_DIR)/userspace/private/libs/cms_core/$(OALDIR)/device2

ifeq ($(BUILD_RDPA),y)
ALLOWED_INCLUDE_PATHS += $(INC_RDP_FLAGS)
endif


export ARCH CFLAGS LIB_INSTALL_DIR HEADER_INSTALL_DIR


# Final location of LIB for system image.  Only the BRCM build system needs to
# know about this.
FINAL_LIB_INSTALL_DIR := $(INSTALL_DIR)/lib$(BCM_INSTALL_SUFFIX_DIR)


ifneq ($(strip $(BUILD_BRCM_OPENWRT)),)
CFLAGS += -DCONFIG_BRCM_OPENWRT
endif

ifneq ($(strip $(BRCM_DRIVER_WIRELESS)),)
ifneq ($(strip $(BUILD_BRCM_CMS)),)
COND_BUILD_CBK_WIFI := 1
endif
ifneq ($(strip $(BUILD_DISTRIBUTED_MDM)),)
COND_BUILD_CBK_WIFI := 1
CFLAGS += -DBUILD_DISTRIBUTED_MDM
endif

ifneq ($(strip $(BUILD_BRCM_UNFWLCFG)),)
export BUILD_BRCM_UNFWLCFG
endif

ifneq ($(strip $(BUILD_BCM_WLAN_DPDCTL)),)
CFLAGS += -DBUILD_BCM_WLAN_DPDCTL
endif
endif

ifeq ($(strip $(COND_BUILD_CBK_WIFI)),1)

conditional_build:
	mkdir -p objs
	$(MAKE) -C objs -f ../Makefile install
	mkdir -p  $(FINAL_LIB_INSTALL_DIR)
	cp -p $(LIB_INSTALL_DIR)/$(LIB) $(FINAL_LIB_INSTALL_DIR)
	mkdir -p $(BCM_FSBUILD_DIR)/private/include
	-install -m 644 rut2_wifi.h $(BCM_FSBUILD_DIR)/private/include
	-install -m 644 rut2_unfwifi.h $(BCM_FSBUILD_DIR)/private/include

else

conditional_build:
	@echo "skipping $(LIB) (not configured)"

endif


clean:
	rm -f $(FINAL_LIB_INSTALL_DIR)/$(LIB)
	rm -f $(BCM_FSBUILD_DIR)/private/include/rut2_wifi.h
	rm -f $(BCM_FSBUILD_DIR)/private/include/rut2_unfwifi.h
	-mkdir -p objs
	-$(MAKE) -C objs -f ../Makefile clean
	rm -rf objs


CONSUMER_RELEASE_BINARYONLY_PREPARE: binaryonly_prepare

binaryonly_prepare:
	-mkdir -p objs
	-$(MAKE) -C objs -f ../Makefile binaryonly_prepare
	rm -rf objs


shell:
	@echo "Entering makefile debug shell (type exit to exit) >>>"
	@bash -i
	@echo "exiting debug shell."
