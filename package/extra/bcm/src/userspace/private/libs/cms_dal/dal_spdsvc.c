...
void dalSpdsvc_getSpeedTestParams(char *varValue)
{
   ...
   snprintf(buf, 128, "/bin/ip route | grep default");
   p = popen(buf, "r");
   if (NULL != p)
   {
      if (fgets(buf, 128, p) && strstr(buf, "default"))
      {
         sscanf(buf, "default via %s dev", gwIp);
      }
      else
      {
         cmsLog_notice("Failed to get default gateway ip address");
      }
      pclose(p);
   }
   ...
   sprintf(varValue,"%s|%s|%u|%s|%s|%u|%u|%u|%s|%u|%u|%d|%u|%u",
         spdsvcObj->mode, gwIp[0] != '\0' ? gwIp : spdsvcObj->serverIpAddr,
...
void dalSpdsvc_getSpeedTestResults(char *varValue)
{
   ...
   {
      sprintf(results, "%s;%u;%u;%u;%u;%u;%u;%u;%s|",
