                                  ...
                                  MdmPathDescriptor *ipIntfPathDesc)
{
   ...
   isLayer2 = (webVar->ntwkPrtcl == CMS_WAN_TYPE_PPPOA ||
   ...
   {
      cmsUtl_strncpy(ethVlanPathRef, ethLinkPathBuf, sizeof(ethVlanPathRef));
   }
   ...
   */
   cmsUtl_strncpy(ipIntfIfName, ethVlanObj->name, sizeof(ipIntfIfName));
         ...
         if (!cmsUtl_strcmp(pppIntfObj->lowerLayers, ethVlanPathRef))
         {
            ...
            else
            {
               ...
               else
               {
                  ...
                  cmsUtl_strncpy(ipIntfIfName, pppIntfObj->name, sizeof(ipIntfIfName));
               }
...
CmsRet dalWan_addIPv6Service_dev2(const WEB_NTWK_VAR *webVar)
{
   ...
   else
   {
         ...
         )
      {
         ...
         cmsUtl_strncpy(ipIntfLowerLayer, layer2FullPath, sizeof(ipIntfLowerLayer));
   ...
   else 
   {
      ...
      */
      ...
      if (webVar->dhcp6cForPd)
      {
         ...
         {
            /* Add a child prefix object for every available LAN bridge interfaces,
             * and set their parentPrefix to the prefix object which we just created
             * for the WAN IP interface object.
             */   
            ...
            {
               if (FALSE == ipIntfObj->X_BROADCOM_COM_Upstream)
               {
                  if ((ret = dalIp_addIPv6ChildPrefix_dev2(&iidStack, prefixPathRef, "::/64")) != CMSRET_SUCCESS)
                  {
