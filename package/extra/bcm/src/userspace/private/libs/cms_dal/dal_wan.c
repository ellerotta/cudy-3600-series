...
CmsRet dalWan_addService_igd(const WEB_NTWK_VAR *webVar)
{
   ...
   if (webVar->ntwkPrtcl == CMS_WAN_TYPE_BRIDGE ||
       ...
       /* next 3 used by ethWan, no need to put in ifdef */
...
CmsRet addWanPppConnection(const WEB_NTWK_VAR *webVar, InstanceIdStack *iidStack)
{
   ...
   case CMS_WAN_TYPE_PPPOA:
      ...
      {
         wanPppConn->idleDisconnectTime = webVar->pppTimeOut;
      }
      else
      {
   ...
   default:
      cmsLog_error("only pppoe, pppoa are supported, ntwkPrctl=%d", webVar->ntwkPrtcl);
...
CmsRet addWanIpConnection(const WEB_NTWK_VAR *webVar, InstanceIdStack *iidStack)
{
   ...
   if (cmsUtl_strstr(webVar->wanL2IfName, EPON_IFC_STR) && rut_isWanTypeEpon())
   {
      ...
      if ((ret = cmsObj_get(MDMOID_WAN_EPON_LINK_CFG, iidStack, OGF_NO_VALUE_UPDATE, (void **)&eponLinkCfg)) != CMSRET_SUCCESS)
      {
         cmsLog_error("failed to get MDMOID_WAN_EPON_LINK_CFG, ret=%d", ret);
         return ret;
      }
      vlanMuxEnab =  !cmsUtl_strcmp(eponLinkCfg->connectionMode, MDMVS_VLANMUXMODE);
      cmsObj_free((void **) &eponLinkCfg);
   ...
   case CMS_WAN_TYPE_DYNAMIC_IPOE:
      /* used by ethWan, no need to put in ifdef */      
   case CMS_WAN_TYPE_DYNAMIC_ETHERNET_IP:
      cmsMem_free(wanIpConn->connectionType);
...
CmsRet dalWan_editInterface_igd(const WEB_NTWK_VAR *webVar)
{
   ...
   if (isPpp)
   {
      ...
      if (pppConn->idleDisconnectTime != 0 && webVar->enblOnDemand)
      {
         ...
         if (pppConn->idleDisconnectTime != (UINT32) (webVar->pppTimeOut))
         {
            CMSMEM_REPLACE_STRING(pppConn->connectionTrigger, MDMVS_ONDEMAND);
            pppConn->idleDisconnectTime = (UINT32) webVar->pppTimeOut;
            change = TRUE;
         }            
      }
      ...
      if (cmsUtl_strcmp(pppConn->X_BROADCOM_COM_IPv6AddressingType, webVar->wanAddr6Type))
      {
      ...
      {
         if (cmsUtl_strcmp(pppConn->X_BROADCOM_COM_ExternalIPv6Address, webVar->wanAddr6))
         {
            /* Need to set change to TRUE if the externalIPv6Address differs */
            CMSMEM_REPLACE_STRING(pppConn->X_BROADCOM_COM_ExternalIPv6Address , webVar->wanAddr6);
            change = TRUE;
         }
         if (pppConn->X_BROADCOM_COM_Dhcp6cForAddress != FALSE)
         {
            pppConn->X_BROADCOM_COM_Dhcp6cForAddress = FALSE;
            change = TRUE;
         }
      }
      else if (pppConn->X_BROADCOM_COM_Dhcp6cForAddress != webVar->dhcp6cForAddr)
      {
         pppConn->X_BROADCOM_COM_Dhcp6cForAddress = webVar->dhcp6cForAddr;
         change = TRUE;
      }

      if (pppConn->X_BROADCOM_COM_UnnumberedModel != webVar->unnumberedModel)
      {
         pppConn->X_BROADCOM_COM_UnnumberedModel = webVar->unnumberedModel;
         change = TRUE;
      }
      if (pppConn->X_BROADCOM_COM_Dhcp6cForPrefixDelegation != webVar->dhcp6cForPd)
      {
         pppConn->X_BROADCOM_COM_Dhcp6cForPrefixDelegation = webVar->dhcp6cForPd;
         change = TRUE;
      }
#endif
             ...
             (ret = cmsObj_getNext(MDMOID_WAN_PPP_CONN, &conIid, (void **)&pppCon)) == CMSRET_SUCCESS)
      {
         if (cmsUtl_strcmp(pppCon->X_BROADCOM_COM_IfName, serviceIfcName) == 0)
         {
            ...
            *wanConIid = conIid;
            ...
            STR_COPY_OR_NULL_TERMINATE(currIfc, pppCon->X_BROADCOM_COM_IfName);
            found = TRUE;
         }
         cmsObj_free((void **)&pppCon);
      }
             ...
             (ret = cmsObj_getNext(MDMOID_WAN_IP_CONN, &conIid, (void **)&ipCon)) == CMSRET_SUCCESS)
      {
         if (cmsUtl_strcmp(ipCon->X_BROADCOM_COM_IfName, serviceIfcName) == 0)
         {
            ...
            *wanConIid = conIid;
            ...
            STR_COPY_OR_NULL_TERMINATE(currIfc, ipCon->X_BROADCOM_COM_IfName);
            found = TRUE;
         }
         cmsObj_free((void **)&ipCon);
      }
...
CmsRet dalWan_getWanConInfoForEdit_igd(WEB_NTWK_VAR *webVar)
{
   ...
   if (!isPpp)
   {
      ...
      if (ipConn->name != NULL)
      {
         STR_COPY_OR_NULL_TERMINATE(webVar->serviceName, ipConn->name);
      }         
   ...
   else 
   {
      ...
      if (pppConn->idleDisconnectTime != 0)
      {
         ...
         webVar->pppTimeOut = pppConn->idleDisconnectTime;
      }
      else
      {
      ...
      if (!cmsUtl_strcmp(pppConn->X_BROADCOM_COM_IPv6AddressingType, MDMVS_STATIC))
      {
         strcpy(webVar->wanAddr6Type, pppConn->X_BROADCOM_COM_IPv6AddressingType);
         strcpy(webVar->wanAddr6, pppConn->X_BROADCOM_COM_ExternalIPv6Address);
      }
      webVar->unnumberedModel = pppConn->X_BROADCOM_COM_UnnumberedModel;
      webVar->dhcp6cForAddr = pppConn->X_BROADCOM_COM_Dhcp6cForAddress;
      webVar->dhcp6cForPd = pppConn->X_BROADCOM_COM_Dhcp6cForPrefixDelegation;
#endif
   }
   ...
   if (qdmIpIntf_isIpv6EnabledOnIntfNameLocked(webVar->wanIfName))
   {
      ret = dalWan_getWanConInfoForEdit_dev2(webVar);
   }
#endif
...
CmsRet  dalWan_setWanConByIntfName_igd(const char *ifName, const char *alias, UBOOL8 *isBridge, 
                     UBOOL8 *ipv4Enable, UBOOL8 *ipv6Enable)
{
   UBOOL8 isPpp = FALSE;
   void *wanConnObj=NULL;
   InstanceIdStack iidStack=EMPTY_INSTANCE_ID_STACK;
   ...
   else
   {
      ...
      if (ipv4Enable && ipConn->X_BROADCOM_COM_IPv4Enabled != *ipv4Enable) 
      {
         ipConn->X_BROADCOM_COM_IPv4Enabled = *ipv4Enable;
         change = TRUE;
      }
#ifdef DMP_X_BROADCOM_COM_IPV6_1
      if (ipv6Enable && ipConn->X_BROADCOM_COM_IPv6Enabled != *ipv6Enable) 
      {
         ipConn->X_BROADCOM_COM_IPv6Enabled = *ipv6Enable;
         change = TRUE;
      }
#endif
      if (!IS_EMPTY_STRING(alias))
      {
         CMSMEM_REPLACE_STRING(ipConn->name, alias);
         change = TRUE;
      }

      if (change && isBridge && !(*isBridge))
      {
         /* Need to disable the WAN connection first in the cmsObj_set below */
         ipConn->enable = FALSE;
      }      
   ...
   if (change)
   {
      ...
      */
      ...
      {
         if ((ipv4Enable && *ipv4Enable) || (ipv6Enable && *ipv6Enable))
         {             
            /* Reactivate this WAN connection */
...
CmsRet  dalWan_getWanConByIntfName_igd(const char *ifName, UBOOL8 *isBridge, UBOOL8 *ipv4Enable, UBOOL8 *ipv6Enable)
{
   UBOOL8 isPpp = FALSE;
   void *wanConnObj=NULL;
   InstanceIdStack iidStack=EMPTY_INSTANCE_ID_STACK;
   ...
   else
   {
      ...
      if (ipv4Enable)
         *ipv4Enable = ipConn->X_BROADCOM_COM_IPv4Enabled;

      if (ipv6Enable)
      {
#ifdef DMP_X_BROADCOM_COM_IPV6_1
         *ipv6Enable = ipConn->X_BROADCOM_COM_IPv6Enabled;
#else
         *ipv6Enable = 0;
#endif
      }
      ...
      if (isBridge)
         *isBridge = !cmsUtl_strcmp(ipConn->connectionType, MDMVS_IP_BRIDGED);
   }
