...
CmsRet fillAccessTimeRestriction(const char *username, const char *mac, const unsigned char days, const unsigned short int starttime, const unsigned short int endtime, AccessTimeRestrictionObject *todCfg)
{
   ...
   if( days != 127 ) 
   {
      if( days & 1 )
      {
         strncat( days_str, "Mon,", sizeof( days_str )-1 );
      }
      if( days & 2 )
      {
         strncat( days_str, "Tue,", sizeof( days_str )-1 );
      }
      if( days & 4 )
      {
         strncat( days_str, "Wed,", sizeof( days_str )-1 );
      }
      if( days & 8 )
      {
         strncat( days_str, "Thu,", sizeof( days_str )-1 );
      }
      if( days & 16 )
      {
         strncat( days_str, "Fri,", sizeof( days_str )-1 );
      }
      if( days & 32 )
      {
         strncat( days_str, "Sat,", sizeof( days_str )-1 );
      }
      if( days & 64 )
      {
         strncat( days_str, "Sun ", sizeof( days_str )-1 );	 
      }
   }
   else
   {
      strncat( days_str, "Mon,Tue,Wed,Thu,Fri,Sat,Sun ", sizeof( days_str )-1 );	
   }
