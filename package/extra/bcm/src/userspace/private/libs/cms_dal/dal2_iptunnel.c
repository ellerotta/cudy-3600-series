                                    ...
                                    UBOOL8 *wanUp, UBOOL8 is6rd)
{
   ...
   if (!foundWan || !foundLan)
   {
      if (!cmsMdm_isDataModelDevice2() && is6rd && !foundWan)
      {
          ...
          */
         ret = qdmIntf_intfnameToFullPathLocked(wanIntf, islayer2, wanfullPath);
         if (ret != CMSRET_SUCCESS)
            cmsLog_error("qdmIntf_intfnameToFullPathLocked returns error. ret=%d", ret);
...
CmsRet dalTunnel_delete_dev2(const char* tunnelName, const char *mode)
{
   ...
   if (!cmsUtl_strcmp(mode, MDMVS_IPV4INIPV6))
   {
      ...
      while (!found && cmsObj_getNextFlags(MDMOID_DEV2_DSLITE_INTERFACE_SETTING, &iidStack, OGF_NO_VALUE_UPDATE, (void **)&dsLiteObj) == CMSRET_SUCCESS)
      {
   ...
   else
   {
      ...
      while (!found && cmsObj_getNextFlags(MDMOID_DEV2_IPV6RD_INTERFACE_SETTING, &iidStack, OGF_NO_VALUE_UPDATE, (void **)&ipv6rdObj) == CMSRET_SUCCESS)
      {
                       ...
                       UINT32 psidOffset, UINT32 psidLen, UINT32 psid)
{
   ...
   else
   {
      ...
      if ( dynamic == FALSE )
      {
         rule->EABitsLength = psidLen + 32 - ipv4MaskLen;
