...
CmsRet cmsDal_setTr69cCfg_dev2(const WEB_NTWK_VAR *webVar)
{
   ...
   if (webVar->tr69cEnable)
   {
      // tr69c is enabled, and we are configuring the parameters.
      // Configure the MANAGMENT_SERVER first to ensure tr69c is started (via enableCWMP).
      cmsLog_notice("Configure tr69 params...");
      ...
      acsCfg->enableCWMP = webVar->tr69cEnable;
      ...
      cmsLog_debug("set DEV2_MANAGEMENT_SERVER");
      ...
      // TODO: instead of doing a cmsObj_get and then set, it would be even
      // more efficient if we set these 2 params via the BCM generic API.
   ...
   else
   {
      ...
      cmsLog_notice("Disabling tr69c...");

      // Special handling for disable tr69c.  Just set enableCWMP to FALSE, and
      // do not bother setting any other params.
      memset(&paramInfo, 0, sizeof(paramInfo));
      paramInfo.fullpath = "Device.ManagementServer.EnableCWMP";
      paramInfo.type = "boolean";
      paramInfo.value = "0";
      paramInfo.profile = "Unspecified";
      ret = bcmGeneric_setParameterValuesFlags(&paramInfo, 1, 0);
      if (ret != CMSRET_SUCCESS)
      {
         cmsLog_error("set of %s to %s failed, ret=%d",
                      paramInfo.fullpath, paramInfo.value, ret);
         return ret;
      }
   }

   /* recreate /var/dnsinfo for updating the specific wan interface dns  for resolve needs */
   // Not clear why we need to reconfig DNSproxy when we change tr69c config,
...
void cmsDal_getTr69cCfg_dev2(WEB_NTWK_VAR *webVar  __attribute__((unused)))
{
   ...
   webVar->tr69cEnable = acsCfg->enableCWMP;
