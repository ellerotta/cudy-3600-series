...
CmsRet dalStorage_addUserAccount(const WEB_NTWK_VAR *webVar, char**errorStr)
{
   ...
   if(storageServiceObj->userAccountNumberOfEntries)
   {
       SINT32 pid;
       UBOOL8 isActive;
   ...
   while(( ret =cmsObj_getNextInSubTree(MDMOID_USER_ACCOUNT, &iidStack, &childIidStack, (void **) &userAccountObj)) == CMSRET_SUCCESS)
   {
      ...
      if(cmsUtl_strcmp(userName,userAccountObj->username) ==0)
      {
         ...
         else
         {
            ...
            else if(isActive && storageServiceObj->userAccountNumberOfEntries == 0)
            {
                /* smbd is already there and no user account existed, delete smbd now */
                if((rut_sendMsgToSmd(CMS_MSG_STOP_APP, EID_SAMBA, NULL,0)) != CMSRET_SUCCESS)
                   cmsLog_error("failed to stop smbd.");		        
            }    
