                                      ...
                                      UINT32 pathLen)
{
   ...
   ptr = strchr(webVar->wanAddr6, '/');
   if (!ptr)
   {
      len = 64;
   }
   else if ((len = atoi(++ptr)) % 8)
   {
      /* FIXME: len is at 8 bit boundary */
      len = (len/8 + 1) * 8;
   }
   cmsNet_subnetIp6SitePrefix(webVar->wanAddr6, 0, len, addrtmp);
...
CmsRet dalIp_addIPv6ChildPrefix_dev2(const InstanceIdStack *ipIntfIidStack,
                                     const char *parentRef, const char *childPrefixBits)
{
   ...
   cmsLog_debug("parent/child = %s/%s", parentRef, childPrefixBits);

   if (IS_EMPTY_STRING(parentRef) ||
   ...
   memcpy(&iidStack, ipIntfIidStack, sizeof(InstanceIdStack));
                                       ...
                                       char * prefixPathRef)
{
   ...
   cmsUtl_strncpy(addrtmp, webVar->wanAddr6, sizeof(addrtmp));
   tmp = strchr(addrtmp, '/');
   *tmp = '\0';
