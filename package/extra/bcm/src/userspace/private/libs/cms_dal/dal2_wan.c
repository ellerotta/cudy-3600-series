...
CmsRet dalWan_addIPv4Service_dev2(const WEB_NTWK_VAR *webVar)
{
   ...
   /* XXX TODO: wifi.SSID has mac address already, so do we
   ...
   else
   {
	 ...
	 if (ret != CMSRET_SUCCESS)
         {
            cmsLog_error("dalBridge_addPortToBridge_dev2 failed, ret = %d", ret);
            return ret;
         }
      }
      else
   ...
   */ 
   ...
   {
      if ((ret = dalNat_set_dev2(ipIntfFullPathBuf,webVar->enblNat)) != CMSRET_SUCCESS)
      {
...
*/
      ...
      else if (cmsMdm_isDataModelDevice2())
      {
         webVar->enblFirewall = ipIntfObj->X_BROADCOM_COM_FirewallEnabled;
      ...
      if (pppIntfObj->idleDisconnectTime != 0)
      {
         ...
         webVar->pppTimeOut = pppIntfObj->idleDisconnectTime;
      }
      else
      {
      ...
      while(!found &&
            (cmsObj_getNextFlags(MDMOID_DEV2_DHCPV6_CLIENT, &iidStack,
                                 OGF_NO_VALUE_UPDATE,
                                 (void **)&dhcpClientObj) == CMSRET_SUCCESS))
      {
         if (!cmsUtl_strcmp(dhcpClientObj->interface, ipIntfFullPathBuf))
         {
            found = TRUE;
            webVar->dhcp6cForAddr = dhcpClientObj->requestAddresses;
            webVar->dhcp6cForPd = dhcpClientObj->requestPrefixes;
            webVar->unnumberedModel = dhcpClientObj->X_BROADCOM_COM_UnnumberedModel;
         }
...
*/
      ...
      if (!webVar->enblOnDemand)
      {
         ...
         CMSMEM_REPLACE_STRING(pppIntfObj->connectionTrigger, MDMVS_ALWAYSON);
         change = TRUE;         
      }
      if (pppIntfObj->idleDisconnectTime == 0 && webVar->enblOnDemand)
      {
         pppIntfObj->idleDisconnectTime = webVar->pppTimeOut;
         CMSMEM_REPLACE_STRING(pppIntfObj->connectionTrigger, MDMVS_ONDEMAND);
         change = TRUE;
      }
      if (pppIntfObj->idleDisconnectTime != 0 && webVar->enblOnDemand)
      {
         /* Need to set change to TRUE if the idleDisconnectTim differs */
         if (pppIntfObj->idleDisconnectTime != (UINT32) (webVar->pppTimeOut))
         {
            CMSMEM_REPLACE_STRING(pppIntfObj->connectionTrigger, MDMVS_ONDEMAND);
            pppIntfObj->idleDisconnectTime = (UINT32) webVar->pppTimeOut;
            change = TRUE;
         }            
      }
...
*/    
...
#ifdef DMP_X_BROADCOM_COM_DEV2_IPV6_1
      while(!foundDhcp6c &&
            (cmsObj_getNextFlags(MDMOID_DEV2_DHCPV6_CLIENT, &dhcpClientIidStack,
                                 OGF_NO_VALUE_UPDATE,
                                 (void **)&dhcpClientObj) == CMSRET_SUCCESS))
      {
         if (!cmsUtl_strcmp(dhcpClientObj->interface, ipIntfFullPathBuf))
      ...
      if (foundDhcp6c)
      {
         if (dhcpClientObj->requestAddresses != webVar->dhcp6cForAddr)
         {
            dhcpClientObj->requestAddresses = webVar->dhcp6cForAddr;
            changeDhcp6c = TRUE;
         }
         if (dhcpClientObj->requestPrefixes != webVar->dhcp6cForPd)
         {
            dhcpClientObj->requestPrefixes = webVar->dhcp6cForPd;
            changeDhcp6c = TRUE;
         }
         if (dhcpClientObj->X_BROADCOM_COM_UnnumberedModel != webVar->unnumberedModel)
         {
            dhcpClientObj->X_BROADCOM_COM_UnnumberedModel = webVar->unnumberedModel;
            changeDhcp6c = TRUE;
         }
         if (changeDhcp6c)
         {
            if ((ret = cmsObj_set(dhcpClientObj, &dhcpClientIidStack)) != CMSRET_SUCCESS)
            {
               ...
               cmsLog_error("Failed to set dhcpClient obj. ret %d", ret);
            }
         }
   ...
   else
   {
      ...
      if (cmsMdm_isDataModelDevice2())
      {
         ...
         if ( (isCurrNatEnabled != webVar->enblNat) ) 
         {
            if ((ret = dalNat_set_dev2(ipIntfFullPathBuf,webVar->enblNat)) != CMSRET_SUCCESS)
            {
               /* complain but continue */
...
#ifdef DMP_DEVICE2_ETHERNETINTERFACE_1
      {
                                    ...
                                    (void **) &ethIntfObj) == CMSRET_SUCCESS)
         {
            if (ethIntfObj->upstream && ethIntfObj->enable)
            {
               ...
               STR_COPY_OR_NULL_TERMINATE(name, ethIntfObj->name);
...
CmsRet dalWan_getAdslFlags_dev2(UINT32 *adslFlags)
{
   ...
   *adslFlags = 0;
   strncpy(cfgModType,dslLineObj->X_BROADCOM_COM_AdslModulationCfg,BUFLEN_128-1);
   cfgModType[BUFLEN_128-1] = '\0';
...
static CmsRet dalWan_getInterfaceByIntfName(const char *ifName, Dev2IpInterfaceObject **ipIntfObj, InstanceIdStack *iidStack)
{
   CmsRet ret = CMSRET_OBJECT_NOT_FOUND;
   UBOOL8 found = FALSE;
   ...
   if (!found)
   {
      cmsLog_debug("Cannot find WAN IP interface %s for editing?", ifName);
      return CMSRET_INTERNAL_ERROR;
   }
...
CmsRet dalWan_setWanConByIntfName_dev2(const char *ifName, const char *alias, UBOOL8 *isBridge,
               UBOOL8 *ipv4Enable, UBOOL8 *ipv6Enable)
{
   ...
   cmsLog_debug("Enter %s, %s, isBridge:%d, IPv4Enable %d, IPv6Enable %d", ifName, alias?alias:"NULL", 
        isBridge?*isBridge:-1, ipv4Enable?*ipv4Enable:-1, ipv6Enable?*ipv6Enable:-1);
   if (CMSRET_SUCCESS != dalWan_getInterfaceByIntfName(ifName, &ipIntfObj, &iidStack))
   {
   ...
   if (ipv4Enable && ipIntfObj->IPv4Enable != *ipv4Enable)
   {
...
#ifdef DMP_DEVICE2_IPV6INTERFACE_1
   if (ipv6Enable && ipIntfObj->IPv6Enable != *ipv6Enable)
   {
   ...
   {
      CMSMEM_REPLACE_STRING(ipIntfObj->alias, alias);
      change = TRUE;          
   }
...
CmsRet  dalWan_getWanConByIntfName_dev2(const char *ifName, UBOOL8 *isBridge,
                        UBOOL8 *ipv4Enable, UBOOL8 *ipv6Enable)
{
   Dev2IpInterfaceObject *ipIntfObj = NULL;
   InstanceIdStack iidStack = EMPTY_INSTANCE_ID_STACK;
   ...
   if (CMSRET_SUCCESS != dalWan_getInterfaceByIntfName(ifName, &ipIntfObj, &iidStack))
   {
      cmsLog_debug("cannot find obj for %s", ifName);
      return CMSRET_INTERNAL_ERROR;
   }
   if(isBridge)
