...
{
   DAL_VOICE_PARMS locParms = { 0 };

   cmsLog_debug( "Voice - %s DECT Registration Window",
                 ( open ? "Open" : "Close" ) );

   dalVoice_mapSpNumToVpInst( 0, &locParms.op[0] );
   dalVoice_SetDectMsgReq(&locParms, open ? CMS_MSG_VOICE_DECT_OPEN_REG_WND
...
CmsRet dalVoice_dectCtlDelHset( unsigned int hset, void* msgHandleArg )
{
   ...
   dalVoice_mapSpNumToVpInst( 0, &locParms.op[0] );
   snprintf(msgData, sizeof(msgData), "%u", hset);
   dalVoice_SetMsgData(&locParms, msgData);
   dalVoice_SetDectMsgReq(&locParms, CMS_MSG_VOICE_DECT_HS_DELETE);
...
CmsRet dalVoice_dectCtlPingHset( unsigned int hset, void* msgHandleArg )
{
   ...
   snprintf(msgData, sizeof(msgData), "%u", hset);
   dalVoice_SetMsgData(&locParms, msgData);
   dalVoice_SetDectMsgReq(&locParms, CMS_MSG_VOICE_DECT_HS_PING);
...
CmsRet dalVoice_SetDectAc( DAL_VOICE_PARMS *parms, char *value )
{
   ...
   dalVoice_mapSpNumToVpInst( 0, &locParms.op[0] );
   dalVoice_SetMsgData(&locParms, value);
   dalVoice_SetDectMsgReq(&locParms, CMS_MSG_VOICE_DECT_AC_SET);
...
CmsRet dalVoice_SetDectMsgReq ( DAL_VOICE_PARMS *parms, UINT32 msgType )
{
   ...
   {
      voiceMsgReqObj->X_BROADCOM_COM_DectMsgType = msgType;
