...
extern CmsRet rutLan_setLanDhcpClient_dev2(const char *ifName, UBOOL8 enable);
extern CmsRet rutIp_deactivateIpv4Interface_dev2(const char *ifName);
    ...
    char *maxAddress, SINT32 *leaseTime, char *relaySrv)
{
   ...
   ret = cmsObj_get(MDMOID_DEV2_DHCPV4_SERVER,
                    &iidStack,
                    OGF_NO_VALUE_UPDATE,
                    (void **) &dhcpv4ServerObj);
   ...
   *enabled = (SINT32) dhcpv4ServerObj->enable;

   cmsObj_free((void **)&dhcpv4ServerObj);
...
CmsRet dalLan_setLanDevice_dev2(const WEB_NTWK_VAR *webVar)
{
   ...
   /* Firewall */
   if((ret = qdmIntf_getPathDescFromIntfnameLocked_dev2(bridgeIfName,FALSE,&pathDesc))==CMSRET_SUCCESS)
   {
      if(cmsObj_get(pathDesc.oid,&pathDesc.iidStack,OGF_NORMAL_UPDATE,(void *)&ipIntfObj)==CMSRET_SUCCESS)
      {
         ...
         ret = cmsObj_set(ipIntfObj,&pathDesc.iidStack);			
         if (ret  != CMSRET_SUCCESS)
      ...
      cmsLog_error("fail to get pathDesc for bridgeIfName %s, ret=%d\n", bridgeIfName, ret);
      return ret;
   }

   if (cmsObj_get(MDMOID_DEV2_DHCPV4_SERVER, &iidStack, 0, (void **)&dhcpv4ServerObj) == CMSRET_SUCCESS)
   {
      ...
      ret = cmsObj_set(dhcpv4ServerObj, &iidStack);
      if (ret  != CMSRET_SUCCESS)
         cmsLog_error("cmsObj_set returns error. ret=%d", ret);
      cmsObj_free((void **)&dhcpv4ServerObj);
   }
                         ...
                         (void **)&dhcpv4ServerPoolObj) == CMSRET_SUCCESS)
   {
          ...
          cmsUtl_strcmp(ifName, bridgeIfName) == 0)
      {
         ...
         if (webVar->enblDhcpc) {
            if ((ret = rutLan_setLanDhcpClient_dev2(bridgeIfName, TRUE)) != CMSRET_SUCCESS) {
                cmsLog_error("Failed to enable DHCPv4 Client, ret=%d", ret);
            }	
         }
		 else { 
            if ((ret = rutLan_setLanDhcpClient_dev2(bridgeIfName, FALSE)) != CMSRET_SUCCESS) {
                cmsLog_error("Failed to disable DHCPv4 Client, ret=%d", ret);
            }
            if ((ret = rutIp_deactivateIpv4Interface_dev2(bridgeIfName)) != CMSRET_SUCCESS) {
                cmsLog_error("Failed to deactivate Dynamic IPv4 Address");
            }
                               ...
                               UBOOL8 *dhcpcEnabled)
{
   ...
   if (dhcpcEnabled != NULL) {
   	  *dhcpcEnabled = qdmIpIntf_isLanDhcpcEnabled_dev2(ifName);
   }
          ...
          cmsUtl_strcmp(ifName, buff) == 0)
      {
                                        ...
                                        (void **) &dhcpv4StaticAddressObj) == CMSRET_SUCCESS)
         {
            if (dhcpv4StaticAddressObj->enable == TRUE) 
            {
               ...
               /* The size of varValue is WEB_BIG_BUF_SIZE_MAX (13000) defined in cgi_main.h.
                * Here we limit the size to 2048, which should be enough for max 32 static
                * dhcp4 static addresses.
                */
               cmsUtl_strncat(varValue, (BUFLEN_1024 * 2), buff);
            }
          ...
          cmsUtl_strcmp(ifName, buff) == 0)
      {
                                        ...
                                        (void **) &dhcpv4StaticAddressObj) == CMSRET_SUCCESS)
         {
                ...
                cmsUtl_strcmp(mac, dhcpv4StaticAddressObj->chaddr) == 0) 
            {
               ...
               ret = cmsObj_set(dhcpv4StaticAddressObj, &iidStackChild);
               if (ret  != CMSRET_SUCCESS)
                  cmsLog_error("cmsObj_set returns error. ret=%d", ret);
               cmsObj_free((void **) &dhcpv4StaticAddressObj);
...
void dalLan_getLanVlan_dev2(WEB_NTWK_VAR *webVar) 
{
...
#ifdef DMP_DEVICE2_VLANTERMINATION_1
   {
      ...
      {
         if (cmsUtl_findInList(vlanTerminationObj->lowerLayers, lanFullPath) != NULL)
         {
            if (strlen(webVar->lanVlanTagList))
               ...
               snprintf(strVlanTag, BUFLEN_16, "%d/%d", vlanTerminationObj->VLANID,
                        vlanTerminationObj->X_BROADCOM_COM_Vlan8021p);
            cmsUtl_strncat(webVar->lanVlanTagList, BUFLEN_128, strVlanTag);
...
#ifdef DMP_DEVICE2_VLANBRIDGE_1
   {
                                 ...
                                 (void **)&brPortObj) == CMSRET_SUCCESS)
      {
         if (!brPortObj->managementPort &&
             cmsUtl_findInList(brPortObj->lowerLayers, lanFullPath) != NULL &&
...
void dalLan_setLanVlan_dev2(WEB_NTWK_VAR *webVar) 
{
...
#endif
   InstanceIdStack iidStack = EMPTY_INSTANCE_ID_STACK;
   InstanceIdStack iidStackTemp = EMPTY_INSTANCE_ID_STACK;
   MdmPathDescriptor pathDesc = EMPTY_PATH_DESCRIPTOR;
   char *lanFullPath = NULL;
   CmsRet ret;
   ...
   cmsLog_debug("FullPath of LAN is %s", lanFullPath);
   cmsLog_debug("lanVlanTagList is %s", webVar->lanVlanTagList);
...
//         dalBridge_addIntfNameToBridge_dev2(vlanName, "br0");
   ...
   INIT_INSTANCE_ID_STACK(&iidStack);
   INIT_INSTANCE_ID_STACK(&iidStackTemp);
                         ...
                         (void **)&vlanTerminationObj) == CMSRET_SUCCESS)
   {
      if (cmsUtl_findInList(vlanTerminationObj->lowerLayers, lanFullPath) != NULL)
      {
         ...
         if (!found)
         {
            ...
            if (ret != CMSRET_SUCCESS)
            {
               cmsLog_error("cmsObj_deleteInstance(vid = %d) failed. ret=%d",
                   vlanTerminationObj->VLANID, ret);
            }
            iidStack = iidStackTemp;
         }
         else
         {
            iidStackTemp = iidStack;
         }
      }
...
static CmsRet dal_enableVlanTermination_dev2(const char *fullPath, UINT32 enable)
{
   ...
   if (ethVlanObj->enable != enable)
   {
      ...
      if ((ret = cmsObj_set(ethVlanObj, &(pathDesc.iidStack))) != CMSRET_SUCCESS)
      {
