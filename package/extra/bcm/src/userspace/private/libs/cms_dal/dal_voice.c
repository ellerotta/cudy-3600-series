...
static void dumpCmdParamUInt( DAL_VOICE_PARMS *parms, unsigned int value );
...
CmsRet dalVoice_SetDefaults( DAL_VOICE_PARMS *parms, char *value )
{
   ...
   /* Logging configurations are not reset to preserve continuity when debugging. */
   ...
   for ( i = 0; i<spNum; i++ )
   {
      ...
      dalVoice_SetRtcpXrConfig( &parmsList, "00");
      ...
      dalVoice_SetSipRegistrationPeriod( &parmsList, "3600" );
      dalVoice_SetSimServsXmlFeatureEnable( &parmsList, MDMVS_OFF);
      ...
      for( j = 0; j < numAcc; j++ )
      {
         ...
         dalVoice_SetVlCFFeatureStarted ( &parmsList, "0" );
         ...
         dalVoice_SetVlCFFeatureEnabled ( &parmsList, "1" );

         dalVoice_SetHookFlashEnable( &parmsList, "yes" );
   ...
   dalVoice_SetUseSipStackDnsResolver( &parmsList, MDMVS_ON );
...
CmsRet dalVoice_SetLoggingDestination( DAL_VOICE_PARMS *parms, char * setVal )
{
   ...
   if (     cmsUtl_strcmp( setVal,MDMVS_STANDARD_ERROR  )
         && cmsUtl_strcmp( setVal,MDMVS_SYSLOG )
         && cmsUtl_strcmp( setVal,MDMVS_TELNET  )   )
   {
      cmsLog_error( "Invalid log level value: %s\n", setVal);
   ...
   {
      REPLACE_STRING_IF_NOT_EQUAL(obj->X_BROADCOM_COM_LoggingDestination, setVal);
...
**
**  PUROPOSE:       Use the built-in SIP stack DNS resolver
...
CmsRet dalVoice_SetUseSipStackDnsResolver ( DAL_VOICE_PARMS *parms, char *setVal )
{
   ...
   {
      voiceObj->X_BROADCOM_COM_UseSipStackDnsResolver = boolVal;
      if ( (ret = cmsObj_set(voiceObj, &iidStack)) != CMSRET_SUCCESS )
      {
         cmsLog_error( "could not set voice obj, ret=%d", ret);
      }
...
CmsRet dalVoice_SetDTMFMethod( DAL_VOICE_PARMS *parms, char *value )
{
   ...
   if ( strcmp( value, MDMVS_RFC4733 ) && strcmp( value, MDMVS_INBAND ) && strcmp( value, MDMVS_SIPINFO ))
   {
...
/*****************************************************************
**  FUNCTION:       dalVoice_SetVlVoipStatus
**
**  PUROPOSE:       Set VOIP service status.
**
**  INPUT PARMS:    vpInst    - parms->op[0]
**                  lineInst  - parms->op[1]
**                  value     - String of voip service status
**
**  RETURNS:        CMSRET_SUCCESS - Read Success
**                  other failed, check with reason code
**
*******************************************************************/
CmsRet dalVoice_SetVlVoipStatus(DAL_VOICE_PARMS *parms, char *value)
{
   unsigned int        vpInst        = parms->op[0];
   unsigned int        lineInst      = parms->op[1];
   CmsRet              ret;
   ...
   /* Replace Line status value */
   REPLACE_STRING_IF_NOT_EQUAL(voiceLineObj->status, value);

   /* copy new value from local copy to MDM */
   if ( ( ret = cmsObj_set( voiceLineObj,  &iidStack ) ) != CMSRET_SUCCESS )
   {
      cmsLog_error( "Can't set VoiceLineObject ret = %d\n", ret);
   }
...
/*****************************************************************
**  FUNCTION:       dalVoice_SetVlCallStatus
**
**  PUROPOSE:       Set call status.
**
**  INPUT PARMS:    vpInst    - parms->op[0]
**                  lineInst  - parms->op[1]
**                  value     - String of call status
**
**  RETURNS:        CMSRET_SUCCESS - Read Success
**                  other failed, check with reason code
**
*******************************************************************/
CmsRet dalVoice_SetVlCallStatus(DAL_VOICE_PARMS *parms, char *value)
{
   unsigned int        vpInst        = parms->op[0];
   unsigned int        lineInst      = parms->op[1];
   CmsRet              ret;
   ...
   /* Replace Line status value */
   REPLACE_STRING_IF_NOT_EQUAL(voiceLineObj->callState, value);
...
CmsRet dalVoice_SetVlCFCallWaiting( DAL_VOICE_PARMS *parms, char *value )
{
   ...
   /* parse setVal as unsigned int */
   /* Check for correct CLI parameter value */
   if ( !strcasecmp( "on", setVal ) )
   {
      setValInt = 1;
   }
   else if ( !strcasecmp( "off", setVal ) )
   {
      setValInt = 0;
   }
   else
...
* Description  : Sets RTCP-XR config flags
*                VoiceProfile.{i}.RTP.RTCP.X_BROADCOM_COM_RTCPXR_Config = new value
*
* Parameters   : parms->op[0] = vpInst, parms->op[1] = lineInst, value = value to be set
* Returns      : CMSRET_SUCCESS when successful.
****************************************************************************/
CmsRet dalVoice_SetRtcpXrConfig( DAL_VOICE_PARMS *parms, char *value )
{
   CmsRet              ret;
   InstanceIdStack     iidStack = EMPTY_INSTANCE_ID_STACK;
   VoiceProfRtpRtcpObject *voiceProfRtpRtcpObj = NULL;

   unsigned int vpInst = parms->op[0];

   LEVELINFO   info[DAL_VOICE_MAXLEVELS] = { {MDMOID_VOICE_PROF, vpInst, EMPTY_INSTANCE_ID_STACK},
                                   {MDMOID_VOICE_PROF_RTP, ANY_INSTANCE, EMPTY_INSTANCE_ID_STACK},
                                   {MDMOID_VOICE_PROF_RTP_RTCP, ANY_INSTANCE, EMPTY_INSTANCE_ID_STACK},
                                   {0, 0, EMPTY_INSTANCE_ID_STACK}, };

   dumpCmdParam(parms, value); /* Debug: display command parameters passed here */

   /*  Get the Voice Profile object */
   if ( (ret = getObj(info, &iidStack, OGF_NO_VALUE_UPDATE, (void**)&voiceProfRtpRtcpObj)) != CMSRET_SUCCESS )
   {
      cmsLog_error( "Can't retrieve RTP object \n" );
      return ( ret );
   }
   ...
   /* set the new value in local copy */
   REPLACE_STRING_IF_NOT_EQUAL(voiceProfRtpRtcpObj->X_BROADCOM_COM_RTCPXR_Config, value );

   /* copy new value from local copy to MDM */
   if ( ( ret = cmsObj_set( voiceProfRtpRtcpObj,  &iidStack)) != CMSRET_SUCCESS )
   {
      cmsLog_error( "Can't set RTCP XR enable ret = %d\n", ret);
   }

   cmsObj_free((void **) &voiceProfRtpRtcpObj);
...
/***************************************************************************
* Function Name: dalVoice_SetHookFlashEnable
*
* Description  : Set hook flash enable flag
*                VoiceProfile.{i}.Line.{i}.CallingFeatures.X_BROADCOM_COM_HookFlashEnable
*
* Parameters   : parms->op[0] = vpInst
*                parms->op[1] = lineInst
*                enable = value to be set "yes" or "no"
*
* Returns      : CMSRET_SUCCESS when successful.
****************************************************************************/
CmsRet dalVoice_SetHookFlashEnable( DAL_VOICE_PARMS *parms, char *enable )
{
   unsigned int       vpInst      = parms->op[0];
   unsigned int       lineInst    = parms->op[1];
   CmsRet             ret;
   ...
   dumpCmdParam(parms, enable); /* Debug: display command parameters passed here */

   if ( enable != NULL && strlen(enable) )
   {
      if ( !strncasecmp(enable, MDMVS_YES, strlen(MDMVS_YES)))
      {
          setVal = 1;
      }
      else if(!strncasecmp(enable, MDMVS_NO, strlen(MDMVS_NO)))
      {
          setVal = 0;
      }
   }
   ...
   {
      obj->X_BROADCOM_COM_HookFlashEnable = setVal;

      cmsLog_debug( "Hook Flash Enable = %d for [lineInst] = [%u]\n", setVal, lineInst);
...
/***************************************************************************
* Function Name: dalVoice_SetDialTonePattern
*
* Description  : Set the default dial tone pattern override
*                VoiceProfile.{i}.Line.{i}.CallingFeatures.X_BROADCOM_COM_DialTonePattern
*
* Parameters   : parms->op[0] = vpInst
*                parms->op[1] = lineInst
*                value = value to be set:
*                         "StandardDialTone"
*                         "SpecialConditionTone"
*                         "MessageWaitingTone"
*
* Returns      : CMSRET_SUCCESS when successful.
*                other failed, check with reason code
...
CmsRet dalVoice_SetDialTonePattern( DAL_VOICE_PARMS *parms, char *value )
{
   ...
   /* Only allow specific values */
   if (     cmsUtl_strcmp( value, MDMVS_STANDARDDIALTONE  )
         && cmsUtl_strcmp( value, MDMVS_SPECIALCONDITIONTONE )
         && cmsUtl_strcmp( value, MDMVS_MESSAGEWAITINGTONE ) )
   {
      cmsLog_error( "Invalid log level value: %s\n", value);
      return CMSRET_INVALID_PARAM_VALUE;
   }
   ...
   else
   {
      ...
      /* set the new value in local copy, after freeing old memory */
      REPLACE_STRING_IF_NOT_EQUAL( obj->X_BROADCOM_COM_DialTonePattern, value );

      cmsLog_debug( "Dial Tone Pattern = %s for [lineInst] = [%u]\n", value, lineInst);
...
/***************************************************************************
* Function Name: dalVoice_SetVlStatsPacketsSent
*
* Description  : 
*                VoiceService.{i}.VoiceProfile.{i}.Line.{i}.Stats.PacketsSent
*
* Parameters   : parms->op[0] = vpInst, vparms->op[1] = lineInst
*                val = value to be set
*
* Returns      : CMSRET_SUCCESS when successful.
****************************************************************************/
CmsRet dalVoice_SetVlStatsPacketsSent( DAL_VOICE_PARMS *parms, unsigned int val )
{
   unsigned int       vpInst      = parms->op[0];
   unsigned int       lineInst    = parms->op[1];
   CmsRet             ret;
   ...
   dumpCmdParamUInt(parms, val); /* Debug: display command parameters passed here */
...
/***************************************************************************
* Function Name: dalVoice_SetVlStatsPacketsReceived
*
* Description  : 
*                VoiceService.{i}.VoiceProfile.{i}.Line.{i}.Stats.PacketsReceived
*
* Parameters   : parms->op[0] = vpInst, vparms->op[1] = lineInst
*                val = value to be set
*
* Returns      : CMSRET_SUCCESS when successful.
****************************************************************************/
CmsRet dalVoice_SetVlStatsPacketsReceived( DAL_VOICE_PARMS *parms, unsigned int val )
{
   unsigned int       vpInst      = parms->op[0];
   unsigned int       lineInst    = parms->op[1];
   CmsRet             ret;
...
/***************************************************************************
* Function Name: dalVoice_SetVlStatsBytesSent
*
* Description  : 
*                VoiceService.{i}.VoiceProfile.{i}.Line.{i}.Stats.BytesSent
*
* Parameters   : parms->op[0] = vpInst, vparms->op[1] = lineInst
*                val = value to be set
*
* Returns      : CMSRET_SUCCESS when successful.
****************************************************************************/
CmsRet dalVoice_SetVlStatsBytesSent( DAL_VOICE_PARMS *parms, unsigned int val )
{
   unsigned int       vpInst      = parms->op[0];
   unsigned int       lineInst    = parms->op[1];
   CmsRet             ret;
...
/***************************************************************************
* Function Name: dalVoice_SetVlStatsBytesReceived
*
* Description  : 
*                VoiceService.{i}.VoiceProfile.{i}.Line.{i}.Stats.BytesReceived
*
* Parameters   : parms->op[0] = vpInst, vparms->op[1] = lineInst
*                val = value to be set
*
* Returns      : CMSRET_SUCCESS when successful.
****************************************************************************/
CmsRet dalVoice_SetVlStatsBytesReceived( DAL_VOICE_PARMS *parms, unsigned int val )
{
   unsigned int       vpInst      = parms->op[0];
   unsigned int       lineInst    = parms->op[1];
   CmsRet             ret;
...
/***************************************************************************
* Function Name: dalVoice_SetVlStatsPacketsLost
*
* Description  : 
*                VoiceService.{i}.VoiceProfile.{i}.Line.{i}.Stats.PacketsLost
*
* Parameters   : parms->op[0] = vpInst, vparms->op[1] = lineInst
*                val = value to be set
*
* Returns      : CMSRET_SUCCESS when successful.
****************************************************************************/
CmsRet dalVoice_SetVlStatsPacketsLost( DAL_VOICE_PARMS *parms, unsigned int val )
{
   unsigned int       vpInst      = parms->op[0];
   unsigned int       lineInst    = parms->op[1];
   CmsRet             ret;
...
/***************************************************************************
* Function Name: dalVoice_SetVlStatsIncomingCallsReceived
*
* Description  : 
*                VoiceService.{i}.VoiceProfile.{i}.Line.{i}.Stats.IncomingCallsReceived
*
* Parameters   : parms->op[0] = vpInst, vparms->op[1] = lineInst
*                val = value to be set
*
* Returns      : CMSRET_SUCCESS when successful.
****************************************************************************/
CmsRet dalVoice_SetVlStatsIncomingCallsReceived( DAL_VOICE_PARMS *parms, unsigned int val )
{
   unsigned int       vpInst      = parms->op[0];
   unsigned int       lineInst    = parms->op[1];
   CmsRet             ret;
...
/***************************************************************************
* Function Name: dalVoice_SetVlStatsIncomingCallsAnswered
*
* Description  : 
*                VoiceService.{i}.VoiceProfile.{i}.Line.{i}.Stats.IncomingCallsAnswered
*
* Parameters   : parms->op[0] = vpInst, vparms->op[1] = lineInst
*                val = value to be set
*
* Returns      : CMSRET_SUCCESS when successful.
****************************************************************************/
CmsRet dalVoice_SetVlStatsIncomingCallsAnswered( DAL_VOICE_PARMS *parms, unsigned int val )
{
   unsigned int       vpInst      = parms->op[0];
   unsigned int       lineInst    = parms->op[1];
   CmsRet             ret;
...
/***************************************************************************
* Function Name: dalVoice_SetVlStatsIncomingCallsConnected
*
* Description  : 
*                VoiceService.{i}.VoiceProfile.{i}.Line.{i}.Stats.IncomingCallsConnected
*
* Parameters   : parms->op[0] = vpInst, vparms->op[1] = lineInst
*                val = value to be set
*
* Returns      : CMSRET_SUCCESS when successful.
****************************************************************************/
CmsRet dalVoice_SetVlStatsIncomingCallsConnected( DAL_VOICE_PARMS *parms, unsigned int val )
{
   unsigned int       vpInst      = parms->op[0];
   unsigned int       lineInst    = parms->op[1];
   CmsRet             ret;
   ...
   {
      obj->incomingCallsConnected = val;
...
/***************************************************************************
* Function Name: dalVoice_SetVlStatsIncomingCallsFailed
*
* Description  : 
*                VoiceService.{i}.VoiceProfile.{i}.Line.{i}.Stats.IncomingCallsFailed
*
* Parameters   : parms->op[0] = vpInst, vparms->op[1] = lineInst
*                val = value to be set
*
* Returns      : CMSRET_SUCCESS when successful.
****************************************************************************/
CmsRet dalVoice_SetVlStatsIncomingCallsFailed( DAL_VOICE_PARMS *parms, unsigned int val )
{
   unsigned int       vpInst      = parms->op[0];
   unsigned int       lineInst    = parms->op[1];
   CmsRet             ret;
...
/***************************************************************************
* Function Name: dalVoice_SetVlStatsOutgoingCallsAttempted
*
* Description  : 
*                VoiceService.{i}.VoiceProfile.{i}.Line.{i}.Stats.OutgoingCallsAttempted
*
* Parameters   : parms->op[0] = vpInst, vparms->op[1] = lineInst
*                val = value to be set
*
* Returns      : CMSRET_SUCCESS when successful.
****************************************************************************/
CmsRet dalVoice_SetVlStatsOutgoingCallsAttempted( DAL_VOICE_PARMS *parms, unsigned int val )
{
   unsigned int       vpInst      = parms->op[0];
   unsigned int       lineInst    = parms->op[1];
   CmsRet             ret;
   ...
   {
      obj->outgoingCallsAttempted = val;
...
/***************************************************************************
* Function Name: dalVoice_SetVlStatsOutgoingCallsAnswered
*
* Description  : 
*                VoiceService.{i}.VoiceProfile.{i}.Line.{i}.Stats.OutgoingCallsAnswered
*
* Parameters   : parms->op[0] = vpInst, vparms->op[1] = lineInst
*                val = value to be set
*
* Returns      : CMSRET_SUCCESS when successful.
****************************************************************************/
CmsRet dalVoice_SetVlStatsOutgoingCallsAnswered( DAL_VOICE_PARMS *parms, unsigned int val )
{
   unsigned int       vpInst      = parms->op[0];
   unsigned int       lineInst    = parms->op[1];
   CmsRet             ret;
...
/***************************************************************************
* Function Name: dalVoice_SetVlStatsOutgoingCallsConnected
*
* Description  : 
*                VoiceService.{i}.VoiceProfile.{i}.Line.{i}.Stats.OutgoingCallsConnected
*
* Parameters   : parms->op[0] = vpInst, vparms->op[1] = lineInst
*                val = value to be set
*
* Returns      : CMSRET_SUCCESS when successful.
****************************************************************************/
CmsRet dalVoice_SetVlStatsOutgoingCallsConnected( DAL_VOICE_PARMS *parms, unsigned int val )
{
   unsigned int       vpInst      = parms->op[0];
   unsigned int       lineInst    = parms->op[1];
   CmsRet             ret;
   ...
   {
      obj->outgoingCallsConnected = val;
...
/***************************************************************************
* Function Name: dalVoice_SetVlStatsOutgoingCallsFailed
*
* Description  : 
*                VoiceService.{i}.VoiceProfile.{i}.Line.{i}.Stats.OutgoingCallsFailed
*
* Parameters   : parms->op[0] = vpInst, vparms->op[1] = lineInst
*                val = value to be set
*
* Returns      : CMSRET_SUCCESS when successful.
****************************************************************************/
CmsRet dalVoice_SetVlStatsOutgoingCallsFailed( DAL_VOICE_PARMS *parms, unsigned int val )
{
   unsigned int       vpInst      = parms->op[0];
   unsigned int       lineInst    = parms->op[1];
   CmsRet             ret;
...
/***************************************************************************
* Function Name: dalVoice_SetVlStatsCallsDropped
*
* Description  : 
*                VoiceService.{i}.VoiceProfile.{i}.Line.{i}.Stats.CallsDropped
*
* Parameters   : parms->op[0] = vpInst, vparms->op[1] = lineInst
*                val = value to be set
*
* Returns      : CMSRET_SUCCESS when successful.
****************************************************************************/
CmsRet dalVoice_SetVlStatsCallsDropped( DAL_VOICE_PARMS *parms, unsigned int val )
{
   unsigned int       vpInst      = parms->op[0];
   unsigned int       lineInst    = parms->op[1];
   CmsRet             ret;
...
/*****************************************************************
**  FUNCTION:       dalVoice_GetHookFlashEnable
**
**  PUROPOSE:       Return value of
**                  VoiceProfile.{i}.Line.{i}.CallingFeatures.X_BROADCOM_COM_HookFlashEnable
**
...
**                  length       = Maximum length of output string
**
**  OUTPUT PARMS:   enable - enable hook flash, "Yes" as true or "No" as false
**
**  RETURNS:        CMSRET_SUCCESS - Read Success
**                  other failed, check with reason code
**
*******************************************************************/
CmsRet dalVoice_GetHookFlashEnable ( DAL_VOICE_PARMS *parms, char *enable, unsigned int length )
{
   CmsRet             ret;
   VoiceLineCallingFeaturesObject *obj = NULL;
   ...
   if ( ret != CMSRET_SUCCESS || obj == NULL )
   {
      cmsLog_error( "%s failed get calling feature Object\n",__FUNCTION__);
      return ( ret );
   }

   snprintf( (char *)enable, length, "%u", obj->X_BROADCOM_COM_HookFlashEnable );
   cmsObj_free( (void **) &obj );
...
/*****************************************************************
**  FUNCTION:       dalVoice_GetDialTonePattern
**
**  PUROPOSE:       Return value of
**                  VoiceProfile.{i}.Line.{i}.CallingFeatures.X_BROADCOM_COM_DialTonePattern
**
...
**                  length       = Maximum length of output string
**
**  OUTPUT PARMS:   value - The dial tone pattern override value. Can be:
**                          "StandardDialTone"
**                          "SpecialConditionTone"
**                          "MessageWaitingTone"
**
...
CmsRet dalVoice_GetDialTonePattern ( DAL_VOICE_PARMS *parms, char *value, unsigned int length )
{
   ...
   strncpy( (char *)value, obj->X_BROADCOM_COM_DialTonePattern, length );
   cmsObj_free( (void **) &obj );
...
CmsRet dalVoice_GetRegionVrgCode(DAL_VOICE_PARMS *parms, char *country, unsigned int length )
{
   ...
   {
      cmsLog_debug( "region = %s\n ", profObj->region);
      strncpy( (char *)localeAlpha2, profObj->region, TEMP_CHARBUF_SIZE -1 );
      localeAlpha2[TEMP_CHARBUF_SIZE -1] = '\0';
...
CmsRet dalVoice_GetLoggingDestination( DAL_VOICE_PARMS *parms, char * getVal, unsigned int length)
{
   ...
   {
      snprintf( getVal, length, "%s",obj->X_BROADCOM_COM_LoggingDestination );
   }
...
CmsRet dalVoice_GetBoundIPAddr( DAL_VOICE_PARMS *parms, char * getVal, unsigned int length )
{
   ...
   else
   {
      ...
      {
         cmsLog_notice( "X_BROADCOM_COM_BoundIpAddr is null");
         snprintf( getVal, length, "undefined" );
      }
   }
...
**
**  PUROPOSE:       Get flag to use built-in SIP stack DNS resolver
*                   (X_BROADCOM_COM_UseSipStackDnsResolver)
**
**  INPUT PARMS:    None
**
**  OUTPUT PARMS:   getVal - Use SIP stack DNS resolver ("0" or "1")
**
**  RETURNS:        CMSRET_SUCCESS - Read Success
**                  other failed, check with reason code
**
...
CmsRet dalVoice_GetUseSipStackDnsResolver(DAL_VOICE_PARMS *parms, char *getVal, unsigned int length )
{
   ...
   {
      snprintf( getVal, length, "%u",voiceObj->X_BROADCOM_COM_UseSipStackDnsResolver );
   }
...
CmsRet dalVoice_SetEuroFlashEnable( DAL_VOICE_PARMS *parms, char *value )
{
   ...
   cmsLog_debug( "EuroFlash = %d for [vpInst] = [%u]\n", setVal, vpInst);
   /* copy new value from local copy to MDM */
   if ( ( ret = cmsObj_set( sipObj,  &iidStack)) != CMSRET_SUCCESS )
   {
...
/***************************************************************************
* Function Name: dalVoice_SetSimServsXmlFeatureEnable
* Description  : Enable or disable "application/simservs+xml" feature
*                MDMVS_ON for enable, MDMVS_OFF for disable (case insensitive)
*                VoiceService.{i}.VoiceProfile.{i}.SIP.X_BROADCOM_COM_SimServsXmlFeatureEnable
*
* Parameters   : parms->op[0] = vpInst, value = value to be set
* Returns      : CMSRET_SUCCESS when successful.
****************************************************************************/
CmsRet dalVoice_SetSimServsXmlFeatureEnable( DAL_VOICE_PARMS *parms, char *value )
{
   ...
   sipObj->X_BROADCOM_COM_SimServsXmlFeatureEnable = setVal;

   cmsLog_debug( "SimServsXmlFeatureEnable = %d for [vpInst] = [%u]\n", setVal, vpInst);
   /* copy new value from local copy to MDM */
   if ( ( ret = cmsObj_set( sipObj,  &iidStack)) != CMSRET_SUCCESS )
   {
      cmsLog_error( "Can't set SimServsXmlFeature enable ret = %d\n", ret);
   }
...
CmsRet dalVoice_GetLogServer(DAL_VOICE_PARMS *parms, char *logServer, unsigned int length )
{
   ...
   dalVoice_removeUnprintableCharacters(logServer);
...
CmsRet dalVoice_GetDTMFMethodIntValue(DAL_VOICE_PARMS *parms, char *dtmfRelay, unsigned int length )
{
   ...
   char* dtmfMethods[] = { MDMVS_INBAND, MDMVS_RFC4733, MDMVS_SIPINFO };
...
CmsRet dalVoice_GetSipMusicServer(DAL_VOICE_PARMS *parms, char *musicServer, unsigned int length )
{
   ...
   dalVoice_removeUnprintableCharacters(musicServer);
...
**                  lineInst - parms->op[1]
**
**  OUTPUT PARMS:   VoiceLineStatsObject stats block
**
**  RETURNS:        CMSRET_SUCCESS - Read Success
**                  other failed, check with reason code
**
*******************************************************************/
CmsRet dalVoice_GetVlStats(DAL_VOICE_PARMS *parms, VoiceLineStatsObject *statsBlk )
{
   unsigned int            vpInst     = parms->op[0];
   unsigned int            lineInst   = parms->op[1];
   CmsRet                  ret;
...
**  INPUT PARMS:    vpInst - parms->op[0];
**
**  OUTPUT PARMS:   config - Value of RTCP XR config flags
**
**  RETURNS:        CMSRET_SUCCESS - Read Success
**                  other failed, check with reason code
**
...
CmsRet dalVoice_GetRtcpXrConfig(DAL_VOICE_PARMS *parms, char *config, unsigned int length )
{
   ...
   /* Copy config value */
   if( voiceProfRtpRtcpObj->X_BROADCOM_COM_RTCPXR_Config != NULL )
   {
      strncpy( (char *) config, voiceProfRtpRtcpObj->X_BROADCOM_COM_RTCPXR_Config, length); //X_BROADCOM_COM_RTCPXR_Config, length);
   }
   else
   {
      cmsLog_error( "NULL RTCP-XR config string\n");
      config[0] = '\0';
   }
   cmsObj_free((void **) &voiceProfRtpRtcpObj);
...
CmsRet dalVoice_GetTotalNumLines( int * numTotLines )
{
   ...
   rutVoice_getNumLines( (intptr_t)NULL, &numLines );
...
CmsRet dalVoice_GetNtrAutoOffsetHistory(DAL_VOICE_PARMS *parms, char *autoOffset, unsigned int length )
{
                                       ...
                                       (void **)&obj) )
   {
      ...
      else
      {
         if( obj->autoOffset != NULL )
         {
            snprintf(tmp, sizeof(tmp), "%s", obj->autoOffset);
...
CmsRet dalVoice_GetNtrAutoPpmHistory(DAL_VOICE_PARMS *parms, char *autoPpm, unsigned int length )
{
                                       ...
                                       (void **)&obj) )
   {
      ...
      else
      {
         if( obj->autoPpm != NULL )
         {
            snprintf(tmp, sizeof(tmp), "%s", obj->autoPpm );
            if(strlen(autoPpm))
...
CmsRet dalVoice_GetNtrAutoPhaseErrorHistory(DAL_VOICE_PARMS *parms, char *autoPhaseError, unsigned int length )
{
                                       ...
                                       (void **)&obj) )
   {
      ...
      {
         snprintf(tmp, sizeof(tmp), "%s", obj->autoPhaseError );
...
/*****************************************************************
**  FUNCTION:       dalVoice_GetSimServsXmlFeatureEnable
**
**  PUROPOSE:       Retrieve "application/simservs+xml" feature enable
**
**  INPUT PARMS:    vpInst - parms->op[0]
**                  length - buffer length
**
**  OUTPUT PARMS:
**                  pSimServsXmlEn - SimServs+Xml feature enable
**                                   MDMVS_ON for enabled, MDMVS_OFF for disabled
**
**  RETURNS:        CMSRET_SUCCESS - Read Success
**                  other failed, check with reason code
**
*******************************************************************/
CmsRet dalVoice_GetSimServsXmlFeatureEnable(DAL_VOICE_PARMS *parms, char *pSimServsXmlEn, unsigned int length )
{
   unsigned int        vpInst     = parms->op[0];
   CmsRet              ret;
   InstanceIdStack     iidStack   = EMPTY_INSTANCE_ID_STACK;
   ...
   /* Copy SIP tag matching value */
   cmsLog_debug( "SimServsXmlFeatureEnable = %d\n ", sipObj->X_BROADCOM_COM_SimServsXmlFeatureEnable);
   if (sipObj->X_BROADCOM_COM_SimServsXmlFeatureEnable)
   {
      snprintf( pSimServsXmlEn, length, "%s", MDMVS_ON );
   }
   else
   {
      snprintf( pSimServsXmlEn, length, "%s", MDMVS_OFF );
   }
   cmsObj_free( (void **) &sipObj );
...
{
   VoiceMsgReqObject *voiceMsgReqObj=NULL;
   InstanceIdStack iidStack = EMPTY_INSTANCE_ID_STACK;
   CmsRet ret;
   ...
   if ( (ret = cmsObj_getNext(MDMOID_VOICE_MSG_REQ, &iidStack, (void **) &voiceMsgReqObj))
        != CMSRET_SUCCESS)
   {
      cmsLog_error( "could not get voiceMsgReq object, ret=%d", ret);
   }
   else
   {
      voiceMsgReqObj->X_BROADCOM_COM_OmciMibReset = TRUE;
      if ( (ret = cmsObj_set(voiceMsgReqObj, &iidStack)) != CMSRET_SUCCESS )
      {
         cmsLog_error( "could not set voiceMsgReq obj, ret=%d", ret);
      }

      cmsObj_free((void **) &voiceMsgReqObj);
   }
...
CmsRet dalVoice_SetOmciCfgUpldComplete ( DAL_VOICE_PARMS *parms, char *setVal )
{
   ...
   {
      voiceMsgReqObj->X_BROADCOM_COM_OmciCfgUpldComplete = TRUE;
      if ( (ret = cmsObj_set(voiceMsgReqObj, &iidStack)) != CMSRET_SUCCESS )
      {
         cmsLog_error( "could not set voiceMsgReq obj, ret=%d", ret);
      }
...
CmsRet dalVoice_SetVoiceMsgReq ( DAL_VOICE_PARMS *parms, UINT32 msgType )
{
   ...
   {
      voiceMsgReqObj->X_BROADCOM_COM_MsgType = msgType;
      if ( (ret = cmsObj_set(voiceMsgReqObj, &iidStack)) != CMSRET_SUCCESS )
      {
         cmsLog_error( "could not set voiceMsgReq obj, ret=%d", ret);
      }
...
CmsRet dalVoice_SetMsgData ( DAL_VOICE_PARMS *parms, char *value )
{
   ...
   {
      REPLACE_STRING_IF_NOT_EQUAL(voiceMsgReqObj->X_BROADCOM_COM_MsgData, value );
      if ( (ret = cmsObj_set(voiceMsgReqObj, &iidStack)) != CMSRET_SUCCESS )
      {
         cmsLog_error( "could not set voiceMsgReq obj, ret=%d", ret);
      }
...
CmsRet dalVoice_GetVoiceAppState( DAL_VOICE_PARMS *parms, char *value, unsigned int length )
{
   ...
   else
   {
      ...
      /* Copy the value */
      if( voiceObj->X_BROADCOM_COM_VoiceAppState != NULL )
      {
         strncpy( (char *) value, voiceObj->X_BROADCOM_COM_VoiceAppState, length);
      }
      else
      {
         value[0] = '\0';
...
CmsRet dalVoice_SetVoiceAppState( DAL_VOICE_PARMS *parms, char *value )
{
   ...
   {
      REPLACE_STRING_IF_NOT_EQUAL(voiceObj->X_BROADCOM_COM_VoiceAppState, value);
                               ...
                               const char * setVal )
{
   ...
   strncpy( tokString, setVal, LIST_CHAR_BUFFER - 1 );
   tokString[LIST_CHAR_BUFFER-1] = '\0';
                                         ...
                                         DAL_VOICE_FEATURE_CODE callFeatId, UINT32 setVal)
{
   switch(callFeatId)
   {
      ...
      case DAL_VOICE_FEATURE_CODE_CONFERENCING:
         switch(callType)
         {
            case CFCALLTYPE_ENABLED: cfObject->X_BROADCOM_COM_CallConferenceEnable = (UINT8)setVal; break;
            default: break;
         }
         break;
                                ...
                                DAL_VOICE_FEATURE_CODE callFeatId, int* getVal)
{
    switch(callFeatId)
    {
        ...
        case DAL_VOICE_FEATURE_CODE_CONFERENCING:
           switch(callType)
           {
              case CFCALLTYPE_ENABLED:    *getVal = cfObject->X_BROADCOM_COM_CallConferenceEnable; break;
              default: break;
           }
           break;
...
static CmsRet getVlCLCodecList( unsigned int vpInst, unsigned int lineInst, void *codec, unsigned int length )
{
   ...
   for ( i = 0; i < maxPrefCodecs; i++ )
   {
      ...
      {
         snprintf(tmp, sizeof(tmp), "%s", codecList[i]->codec);
         if(i)
...
static CmsRet dumpAccountParams(int spNum, int accNum)
{
   ...
   if ( dalVoice_GetVlCFFeatureStarted( &parms, objValue, MAX_TR104_OBJ_SIZE ) == CMSRET_SUCCESS )
   {
      printf( "         WarmLine started     : %s\n", atoi(objValue) ? "on" : "off" );
   }
   ...
   parms.op[2] = DAL_VOICE_FEATURE_CODE_CONFERENCING;

   objValue[0] = '\0';

   if ( dalVoice_GetVlCFFeatureEnabled( &parms, objValue, MAX_TR104_OBJ_SIZE ) == CMSRET_SUCCESS )
   {
      printf( "         Conference Calling   : %s\n", atoi(objValue) ? "on" : "off" );
   }
...
static CmsRet dumpVoiceStats( void )
{
   VoiceLineStatsObject       callStats;
   DAL_VOICE_PARMS            parms;
   int                        vpInst;
   int                        lineInst;
   int                        spNum;
   ...
   for ( i = 0; i < spNum; i++ )
   {
      ...
      for ( j = 0; j < numAcc; j++ )
      {
         ...
         else
         {
            ...
            printf( "      CallsDropped                    : %u\n\n", callStats.callsDropped );

            /* Clear call stats block for next account */
            memset( &callStats, 0, sizeof(VoiceLineStatsObject) );
         }
      } /* Voice accounts */
   } /* Voice profiles */
   return CMSRET_SUCCESS;
...
*
* Parameters   : parms (IN) - parameter structure
*              : value (IN) - string value to be set or obtained
* Returns      : CMSRET_SUCCESS when successfule.
****************************************************************************/
static void dumpCmdParam( DAL_VOICE_PARMS *parms, char *value )
{
...
/***************************************************************************
* Function Name: dumpCmdParamUInt
* Description  : Displays the command line parameters individually square-bracketed.
*
* Parameters   : parms (IN) - parameter structure
*              : value (IN) - uint value to be set or obtained
* Returns      : CMSRET_SUCCESS when successfule.
****************************************************************************/
static void dumpCmdParamUInt( DAL_VOICE_PARMS *parms, unsigned int value )
{
