...
CmsRet dalPptpAc_addPptpAcInterface(const WEB_NTWK_VAR *webVar)
{
   ...
   else 
   {
      ...
      cmsObj_free((void **) &PptpAclinkCfg);
   }
...
#ifdef DMP_X_BROADCOM_COM_PPTPAC_2
#define SYSTEM_VIRTUAL_NETDEV_PATH "/sys/devices/virtual/net"
#define SYSTEM_LS_CMD "ls"
#define SYSTEM_ECHO_CMD "echo"
...
CmsRet dalPptpCfg(const char *enable, const char *pptpIpAddress, const char *pptpusertname, const char *pptppassword)
{
   ...
   Dev2PppInterfacePptpObject *pptpCfg = NULL;   
   UBOOL8 found = FALSE;
   
   cmsLog_debug("configure pptp with %s/%s/%s/%s", enable, pptpIpAddress, pptpusertname, pptppassword);
   ...
   {
      if (!cmsUtl_strcmp(pppIntfObj->name, "PptpAc"))
      {
         InstanceIdStack pptpIidStack = EMPTY_INSTANCE_ID_STACK;
               
         if ((ret = cmsObj_getNextInSubTree(MDMOID_DEV2_PPP_INTERFACE_PPTP, &iidStack, 
                                             &pptpIidStack, (void **) &pptpCfg)) != CMSRET_SUCCESS)
         {
            cmsLog_error("Failed to get pptpCfg, error=%d", ret);
            cmsObj_free((void **)&pppIntfObj);
            return ret;
         }
         
         found = TRUE;
         cmsLog_debug("pptpac object found");   
         ...
         if (pptpCfg)
         {
            ...
            {
               pptpCfg->enable = TRUE;
               CMSMEM_REPLACE_STRING(pptpCfg->pptpIpAddress, pptpIpAddress);
               CMSMEM_REPLACE_STRING(pptpCfg->pptpusertname, pptpusertname);
               CMSMEM_REPLACE_STRING(pptpCfg->pptppassword, pptppassword);
            }
            else
            {
               pptpCfg->enable = FALSE;
            }

            ret = cmsObj_set(pptpCfg, &iidStack);
            cmsObj_free((void **) &pptpCfg);
         }
      }
      else
      {
   ...
   else
   {
      ...
      if ((ret = cmsObj_get(MDMOID_DEV2_PPP_INTERFACE_PPTP, &iidStack, 0, (void **) &pptpCfg)) != CMSRET_SUCCESS)
      {
         cmsLog_error("Failed to get Dev2PppInterfacePptpObject, ret=%d", ret);
         cmsObj_deleteInstance(MDMOID_DEV2_PPP_INTERFACE, &iidStack);
         cmsObj_free((void **) &pppIntfObj);   
         return ret;
      }
      ...
      /* fill in service name */
      CMSMEM_REPLACE_STRING(pppIntfObj->name, "PptpAc");   /* todo. just assign some thing for now */
   ...
   if (ret != CMSRET_SUCCESS)
   {
      cmsLog_error("Failed to set Dev2PppInterfacePptpObject, ret = %d", ret);
   }
...
CmsRet dalGetPptpInfo(char *info)
{
   ...
   {
      strcpy(info, "0|||"); //set default value to info string
   }
                               ...
                               (void **)&pppIntfObj) == CMSRET_SUCCESS))
   {
      if (!cmsUtl_strcmp(pppIntfObj->name, "PptpAc"))
      {
         ...
         if (pptpCfg)
         {
             if(pptpCfg->enable)
             {
                if (access(PPTPAC_LINKNAME_FILE, F_OK) == 0)
                {
                   if ((fs = fopen(PPTPAC_LINKNAME_FILE, "r")) == NULL)
                      cmsLog_error("Failed to get file %s", PPTPAC_LINKNAME_FILE);
                   else
                   {
                      if (fread(buf, 1, sizeof(buf), fs) > 0)
                      {
                         buf[strlen(buf)-1]='\0';
                         if(is_PppIntf_exist(buf))
                            activeFlag = 1;
                      }
                      fclose(fs); 
                   }
             ...
             sprintf(info, "%d|%s|%s|%s|%d", pptpCfg->enable, pptpCfg->pptpIpAddress, pptpCfg->pptpusertname, pptpCfg->pptppassword, activeFlag);
             cmsObj_free((void **) &pptpCfg);
         }
      }
      else
