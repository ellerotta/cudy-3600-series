...
static CmsRet dalCert_adjustRefCount(char *name, SINT32 delta)
{
   InstanceIdStack iidStack = EMPTY_INSTANCE_ID_STACK;
   CertificateCfgObject *obj = NULL;
   CmsRet ret = CMSRET_OBJECT_NOT_FOUND;

   while ((ret == CMSRET_OBJECT_NOT_FOUND) &&
          (cmsObj_getNext(MDMOID_CERTIFICATE_CFG, &iidStack, (void **) &obj) == CMSRET_SUCCESS))
   {
      /* only adjust local certificates */
      if ((cmsUtl_strcmp(obj->type, CERT_TYPE_CA) != 0) &&
          (cmsUtl_strcmp(obj->name, name) == 0))
      {
         /* make sure that refcount never goes below zero */
         if ((delta < 0) && ((SINT32)obj->refCount + delta) < 0)
         {
            obj->refCount = 0;
            cmsLog_error("X_BROADCOM_COM_CertificateCfg (%s) refCount reset to zero", name);
         }
         else
         {
            /* set X_BROADCOM_COM_CertificateCfg */
            obj->refCount += delta;
            cmsLog_debug("X_BROADCOM_COM_CertificateCfg (%s) refCount = %d", name, obj->refCount);
         }
         ret = cmsObj_set(obj, &iidStack);
      }
      /* free certCfg object */
...
{
   return dalCert_adjustRefCount(name, 1);
}
