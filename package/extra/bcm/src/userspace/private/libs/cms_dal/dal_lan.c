...
CmsRet dalLan_setLanDevice_igd(const WEB_NTWK_VAR *webVar)
{
   ...
   else
   {
         ...
         if((ret = cmsObj_getNextInSubTree(MDMOID_LAN_IP_INTF, &lanHostIidStack, &lanIpIntfIidStack, (void **) &lanIpObj)) == CMSRET_SUCCESS)
         {
            ...
            {
                doSet = TRUE;
                CMSMEM_REPLACE_STRING(lanIpObj->IPInterfaceAddressingType, MDMVS_DHCP);
            }
            else if (cmsUtl_strcmp(lanIpObj->IPInterfaceIPAddress, webVar->ethIpAddress) ||
                cmsUtl_strcmp(lanIpObj->IPInterfaceSubnetMask, webVar->ethSubnetMask) ||
                !cmsUtl_strcmp(lanIpObj->IPInterfaceAddressingType, MDMVS_DHCP))
            {
                doSet = TRUE;
                          ...
                          UBOOL8 *dhcpcEnabled)
{
   ...
   {
      *dhcpcEnabled = FALSE;
      if (!cmsUtl_strcmp(lanIpIntf->IPInterfaceAddressingType, MDMVS_DHCP))
         *dhcpcEnabled = TRUE;
   }
...
CmsRet dalLan_StaticIPRemove_igd(const char * brName, const char *mac)
{
   ...
   else 
   {
      ...
      ret = cmsObj_set(lanConditionServeObj, &lanConditionServeIidStack);
      if (ret  != CMSRET_SUCCESS)
