...
void dalEthOam_getAllCfg(WEB_NTWK_VAR *pWebVar)
{
   ...
   if(ieee8023ahCfg->ifName){
      STR_COPY_OR_NULL_TERMINATE(pWebVar->eoam3ahIntf, ieee8023ahCfg->ifName);
   }
   ...
   if(ieee8021agCfg->mdId){
      STR_COPY_OR_NULL_TERMINATE(pWebVar->eoam1agMdId, ieee8021agCfg->mdId);
   }
   pWebVar->eoam1agMdLvl = ieee8021agCfg->mdLevel;
   if(ieee8021agCfg->maId){
      STR_COPY_OR_NULL_TERMINATE(pWebVar->eoam1agMaId, ieee8021agCfg->maId);
   }
   ...
   {
      if(localMep->ifName){
         STR_COPY_OR_NULL_TERMINATE(pWebVar->eoam1agLocIntf, localMep->ifName);
      }
