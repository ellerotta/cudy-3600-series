...
static CmsRet getConfiguredEthWanObj_dev2(InstanceIdStack *iidStack, Dev2EthernetInterfaceObject **ethIntfObj, const char *l2Name)
{
   while (cmsObj_getNextFlags(MDMOID_DEV2_ETHERNET_INTERFACE,
                              iidStack,
                              OGF_NO_VALUE_UPDATE,
                              (void **) ethIntfObj) == CMSRET_SUCCESS)
   {
      ...
      */
      if ((*ethIntfObj)->upstream && (*ethIntfObj)->enable &&
          !cmsUtl_strcmp((*ethIntfObj)->name, l2Name))
      {
...
CmsRet dalEth_deleteEthInterface_dev2(const WEB_NTWK_VAR *webVar)
{
    ...
    */
   ret = getConfiguredEthWanObj_dev2(&iidStack, &ethIntfObj, webVar->wanL2IfName);
   if (ret != CMSRET_SUCCESS)
   {
   ...
   isWanOnly = cmsUtl_isSubOptionPresent(pWANOnlyPortList, webVar->wanL2IfName);
   ...
   */
   ...
   if ( !isWanOnly )
   {
      ...
      char lanIfNameBuf[CMS_IFNAME_LENGTH+2]={0};

      if (strstr(webVar->wanL2IfName, ETH_IFC_STR) && (strchr(webVar->wanL2IfName, '.') == NULL))
      {
         snprintf(lanIfNameBuf, sizeof(lanIfNameBuf), "%s.0", webVar->wanL2IfName);
         lanIfName = lanIfNameBuf;
      }
      else
#endif
         lanIfName = webVar->wanL2IfName;
