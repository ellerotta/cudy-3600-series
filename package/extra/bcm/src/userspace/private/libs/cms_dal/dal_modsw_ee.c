...
CmsRet dalModSw_getAvailableEE(NameList **ifList)
{
   ...
   *ifList = NULL;
   ...
   {
      char displayEEName[TR157_DESCRIPTION_STR_LEN+1]={0};
      ...
      snprintf(displayEEName, TR157_DESCRIPTION_STR_LEN, "%s:%s",
               ExecEnv->name, ExecEnv->version);
      /* append to name list */
      cmsLog_debug("adding %s to list of available EE Names", displayEEName);
      nameListElement->name = cmsMem_strdup(displayEEName);
