...
CmsRet dalIPSec_addTunnel_dev2(const PWEB_NTWK_VAR pWebVar)
{
   ...
   else if (!cmsUtl_strcmp(pWebVar->ipsKeyExM, "auto"))
   {
      ...
      ikeObj->phase2AntiReplayWindow = pWebVar->ipsPh2AntiReplayWindow;
      if (!cmsUtl_strcmp(pWebVar->ipsPh2ESN, "enable"))
      {
         ikeObj->phase2ESN = TRUE;
      }
      else
      {
         ikeObj->phase2ESN = FALSE;
      }
   ...
   else if (!cmsUtl_strcmp(pWebVar->ipsKeyExM, "autov2"))
   {
      Ikev2CfgObject *ikev2Obj = NULL;

      if ((ret = cmsObj_get(MDMOID_IKEV2_CFG, &iidStack, 0,
                            (void **) &ikev2Obj)) != CMSRET_SUCCESS)
      {
         cmsLog_error("Failed to get Ikev2CfgObject, ret=%d", ret);
         return ret;
      }

      CMSMEM_REPLACE_STRING(ikev2Obj->authenticationMethod, pWebVar->ipsAuthM);
      CMSMEM_REPLACE_STRING(ikev2Obj->preSharedKey, pWebVar->ipsPSK);
      CMSMEM_REPLACE_STRING(ikev2Obj->certificateName,
                            pWebVar->ipsCertificateName);
      if (!cmsUtl_strcmp(pWebVar->ipsPerfectFSEn, "enable"))
      {
         ikev2Obj->perfectFSEn = TRUE;
      }
      else
      {
         ikev2Obj->perfectFSEn = FALSE;
      }
      CMSMEM_REPLACE_STRING(ikev2Obj->phase1AllowedEncryptionAlgorithms,
                            pWebVar->ipsPh1EncryptionAlgo);
      CMSMEM_REPLACE_STRING(ikev2Obj->phase1AllowedIntegrityAlgorithms,
                            pWebVar->ipsPh1IntegrityAlgo);
      CMSMEM_REPLACE_STRING(ikev2Obj->phase1AllowedDiffieHellmanGroupTransforms,
                            pWebVar->ipsPh1DHGroup);
      ikev2Obj->phase1KeyTime = pWebVar->ipsPh1KeyTime;
      CMSMEM_REPLACE_STRING(ikev2Obj->phase2AllowedEncryptionAlgorithms,
                            pWebVar->ipsPh2EncryptionAlgo);
      CMSMEM_REPLACE_STRING(ikev2Obj->phase2AllowedIntegrityAlgorithms,
                            pWebVar->ipsPh2IntegrityAlgo);
      CMSMEM_REPLACE_STRING(ikev2Obj->phase2AllowedDiffieHellmanGroupTransforms,
                            pWebVar->ipsPh2DHGroup);
      ikev2Obj->phase2KeyTime = pWebVar->ipsPh2KeyTime;
      ikev2Obj->phase2AntiReplayWindow = pWebVar->ipsPh2AntiReplayWindow;
      if (!cmsUtl_strcmp(pWebVar->ipsPh2ESN, "enable"))
      {
         ikev2Obj->phase2ESN = TRUE;
      }
      else
      {
         ikev2Obj->phase2ESN = FALSE;
      }

      if ( (ret = cmsObj_set(ikev2Obj, &iidStack)) != CMSRET_SUCCESS )
      {
         cmsLog_error("Failed to set Ikev1CfgObject");
      }

      cmsObj_free((void **) &ikev2Obj);
   }
   ...
   CMSMEM_FREE_BUF_AND_NULL_PTR(profilePath);
   filterObj->X_BROADCOM_COM_L2TP_over_IPSec = pWebVar->l2tpIpsecEnable;
   filterObj->X_BROADCOM_COM_LNS_L2TP_PORT = pWebVar->isL2tpServerPort;
