...
// Global vars used by calling app.
UBOOL8 loggingSOAP = FALSE;


// External apps should not use these vars directly.  Use API's below to
// set and get.
#define WEBPROTO_MAX_APPCB 4
typedef struct tidAppCBEntry
{
   SINT32 tid;              // threadId
   WebprotoCB *appCB;       // appCB for this thread
} TidAppCBEntry;

TidAppCBEntry appCBTable[WEBPROTO_MAX_APPCB] = {{0, NULL},
                                                {0, NULL},
                                                {0, NULL},
                                                {0, NULL}};
WebprotoCB *globalAppCB = NULL;

// Register a single, global appCB.  All threads in app will use this appCB.
void webproto_setCallbacks(WebprotoCB *cb)
{
   globalAppCB = cb;
   return;
}

void webproto_setCallbacksForThread(WebprotoCB *cb)
{
   SINT32 myTid = cmsEid_getThreadId();
   int i;

   for (i=0; i < WEBPROTO_MAX_APPCB; i++)
   {
      if (appCBTable[i].tid == 0)
      {
         appCBTable[i].tid = myTid;
         appCBTable[i].appCB = cb;
         cmsLog_debug("Storing threadId %d in slot %d", myTid, i);
         return;
      }
   }
   cmsLog_error("No more empty slots in appCBTable!");
   return;
}

void webproto_clearCallbacksForThread()
{
   SINT32 myTid = cmsEid_getThreadId();
   int i;
   ...
   for (i=0; i < WEBPROTO_MAX_APPCB; i++)
   {
      if (appCBTable[i].tid == myTid)
      {
         appCBTable[i].tid = 0;
         appCBTable[i].appCB = NULL;
         cmsLog_debug("delete threadId %d in slot %d", myTid, i);
         return;
      }
   }
   cmsLog_error("Could not find threadId %d in appCBTable!", myTid);
   return;
}

WebprotoCB *webproto_getAppCB()
{
   SINT32 myTid = cmsEid_getThreadId();
   int i;

   // First check if global appCB is set, if so, just use that.  tr69c uses this mechanism.
   if (globalAppCB != NULL)
      return globalAppCB;

   // Look for per-thread appCB.  obuspa uses this mechanism.
   for (i=0; i < WEBPROTO_MAX_APPCB; i++)
   {
      if (myTid == appCBTable[i].tid)
         return appCBTable[i].appCB;
   }

   cmsLog_error("Could not find appCB for threadId=%d, first register with webproto_setCallbacks or webproto_setCallbacksForThread", myTid);
   return NULL;
}
...
static int verify_callback(int ok, X509_STORE_CTX *store __attribute__((unused)))
{
   ...
   int  err, depth;
   UBOOL8 synchronized;
   WebprotoCB *appCB = webproto_getAppCB();

   if (appCB == NULL)
      return 0;

   synchronized = (*appCB->isTimeSynchronized)();
   ...
   if (ok != 0 && depth == 0)
   {
      ...
      if ((pCN = strstr(subject, "CN=")) != NULL)
      {
         ...
         if ((*appCB->getRedirectUrl)() != NULL)
         {
            pURL = (*appCB->getRedirectUrl)();
         }
         else
         {
            pURL = (*appCB->getAcsUrl)();
         }
...
{ 
   ...
   cmsTmr_cancel(appCB->tmrHandle, postShutdownCleanUp, (void*)pc);
   stopListener(pc->fd);
   close(pc->fd);
   if (pc->ssl)
   {
...
void proto_FreeCtx(tProtoCtx *pc)
{
   ...
   {
      case iNormal:
         cmsLog_debug("close fd %d and stop listener", pc->fd);
         stopListener(pc->fd);
         close(pc->fd);
         cmsMem_free(pc);
         break;
      ...
      case iSsl:
      {
         ...
         DBGSSLC((stderr, "%s   proto_FreeCtx()ssl=%p fd=%d\n",
                  getticks(), pc->ssl, pc->fd));
         ret = cmsTmr_set(appCB->tmrHandle, postShutdownCleanUp, (void *)pc, 3000, "postShutdownCleanUp");
...
int proto_ReadWait(tProtoCtx *pc, char *ptr, int nbytes)
{
    ...
    /* turn on synchroneous I/O, this call will block. */
    {
        ...
        if (fcntl(pc->fd, F_SETFL, bflags) < 0)
            cmsLog_debug("ERROR: fcntl remove NONBLOCK flag failed (%d)", errno);
    }
...
char *readLengthMsg(tWget *wg, int readLth, int *mlth, int doFlushStream) {
   int bufCnt = 0, readCnt = 0;
   int bufLth = readLth;
   char *soapBuf = NULL;

   cmsLog_debug("Entered: readlen=%d", readLth);
   ...
   *mlth = 0;
         ...
         else
         {
            if (readCnt == -99) {
               ...
               *mlth = 0;
               ...
               cmsLog_error("download interrupted");
               break;
            }
         }
      }
      cmsLog_debug("soapBuf bufCnt=%d readLth=%d", bufCnt, readLth);
      if(readCnt != -99)
      {
         *mlth = bufCnt;
         soapBuf[bufCnt] = '\0';
   ...
   {
      cmsLog_error("Failed to allocate buf length %d", readLth);
   }

   return soapBuf;
}

char *readChunkedMsg(tWget *wg, int *mlth, int maxSize) {
   char *soapBuf = NULL;
   char chunkedBuf[128];   
   ...
   *mlth = 0;
   ...
   if (proto_Readline(wg->pc, chunkedBuf, sizeof(chunkedBuf)) > 0)
   {
      ...
      while (chunkedSz > 0)
      {
         ...
         if (chunkedSz != readSz)
            cmsLog_error("===> readChunkedMsg, chunked size = %d, read size = %d\n", chunkedSz, readSz);
         if (readBuf == NULL)
         {
         ...
         {
            cmsLog_error("reading more data than maxSize (%d)", maxSize);
            freeMsg(soapBuf, mlth);
...
char *readChunkedImage(tWget *wg, int *mlth)
{
   int imageBufSize;
   int allocSize;
   char *imageBuf = NULL;
   ...
   *mlth = 0;
   ...
   imageBufSize = cmsImg_getImageFlashSize();
   if ((imageBuf = (char *) cmsMem_alloc(imageBufSize, 0)) == NULL)
   {
      ...
      {
         cmsLog_error("Failed to allocate %d bytes for chunked image download", imageBufSize);
      ...
      if (imageBuf == NULL)
      {
         cmsLog_error("Failed to allocate %d bytes for chunked image download", \
                      allocSize);
         return NULL;
      }
      else
   ...
   do {
      ...
      {
         cmsLog_error("next chunk would exceed imageBufSize, %d+%d>%d",
      ...
      else
      {
         ...
         {
            cmsLog_error("===>length mismatch, wanted chunkSize=%d, bytesRead=%d",
                         chunkSize, bytesRead);
            total = 0;
            break;
         }
...
char *readResponse( tWget *wg, int *mlth, int maxBufferSize)
{
   ...
   else if (wg->hdrs->TransferEncoding && !strcasecmp(wg->hdrs->TransferEncoding,"chunked"))
   {
      ...
      {
         cmsLog_error("Don't know what kind of response this is!?!");
      }
   }
    ...
    for (n = 1; n < maxlen; n++) {
        ...
        if (rc == 1) {
            *ptr++ = c;
            ...
            if (n == 1) {
               /* TBD: fix part2, restore flags */
                if (fcntl(pc->fd, F_SETFL, flags) < 0)
                    cmsLog_debug("ERROR: fcntl restore flag failed (%d)", errno);
...
void proto_SendRaw(tProtoCtx *pc, const char *arg, int len)
{
   ...
   if ((*appCB->getFault)() == CMSRET_EAGAIN_ERROR)
   {
   ...
   while (totWlth < len &&
          (*appCB->getFault)() != CMSRET_EAGAIN_ERROR &&
          (*appCB->getFault)() != CMSRET_SOCKET_ERROR)
   {
      ...
      else
      {
         if (errno == EAGAIN)
         {
            ...
            else
            {
               ...
               else
               {
                   ...
                   */
                  (*appCB->setFault)(CMSRET_EAGAIN_ERROR);
         ...
         {
             (*appCB->setFault)(CMSRET_SOCKET_ERROR);
...
int proto_ParseRequest(tProtoCtx *pc, tHttpHdrs *hdrs)
{
    ...
    www_IgnoreParam(path); // ignore '?param=*' in request line
    www_StripTail(path);
...
int proto_ParseResponse(tProtoCtx *pc, tHttpHdrs *hdrs)
{
    ...
    char protocol[BUF_SIZE_MAX] = {0};
    char status[BUF_SIZE_MAX] = {0};
    char message[BUF_SIZE_MAX] = {0};
    int rc = 0;

    /* Parse the first line of the request. */
    rc = proto_Readline(pc, buf, BUF_SIZE_MAX);
    if (rc <= 0) {
        cmsLog_error("readline error, rc=%d", rc);
        return -1;
    }
...
int proto_Skip(tProtoCtx *pc)
{
    ...
    if (fcntl(pc->fd, F_SETFL, nbflags) < 0)
        cmsLog_debug("ERROR: fcntl set NONBLOCK flag failed (%d)", errno);
