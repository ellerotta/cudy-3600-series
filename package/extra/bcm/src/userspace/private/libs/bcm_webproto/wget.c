...
static char *getticks()
{
    ...
    snprintf(timestr, sizeof(timestr), "%04ld.%06ld", now.tv_sec%1000, now.tv_usec);
    return timestr;
}
#endif
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
                          ...
                          const char *msg)
{
   ...
   wg.magicId = WEBPROTO_MAGICID_WGEXT;
   wg.status = status;
   wg.pc = data->pc;
   wg.hdrs = data->hdrs;
   wg.msg = msg;
                            ...
                            const char *uri)
{
   ...
   if ((*appCB->getWanState)() == eWAN_INACTIVE)
   {
      return -1;
   }
   ...
   while (next)
   {
      cmsLog_notice("Send extra header (%s)", next->hdr);
                        ...
                        int datalen, const char *content_type, int content_len)
{
   ...
   snprintf(buf, sizeof(buf), "%d", content_len);
                             ...
                             int content_len)
{
   ...
   if ((appCB == NULL) ||
       (*appCB->getWanState)() == eWAN_INACTIVE)
   {
      return -1;
   }
...
static void do_connect(void *p)
{
   ...
   cmsLog_notice("ENTER:");
   cmsTmr_cancel(appCB->tmrHandle, timer_connect, data);
   stopListener(data->pc->fd);
...
static void do_send_request(void *p, int errorcode)
{
   ...
   cmsLog_notice("Entered: keepConn=%d status=%d request=%d",
                 data->keepConnection, data->status, data->request);
   if (errorcode < 0)
   {
      report_status(data, iWgetStatus_ConnectionError, 
                      "Failed to establish SSL connection");
   ...
   if (res < 0)
   {
      cmsLog_error("send_x_request failed, res=%d request=%d", res, data->request);
      report_status(data, iWgetStatus_ConnectionError, 
   ...
   ret = cmsTmr_set(appCB->tmrHandle, timer_response, data, ACSRESPONSETIME, "timer_response"); /* response timeout is 60 sec */
...
static void do_response(void *p)
{
   ...
   cmsLog_notice("Entered:  data->pc=%p",data->pc);
   cmsTmr_cancel(appCB->tmrHandle, timer_response, data);
...
tWgetInternal *wget_Connect(const char *url, CmsEventHandler callback, void *handle)
{
   ...
   wg->magicId        = WEBPROTO_MAGICID_WGINT;
   wg->request        = eConnect;
   ...
   // dns_lookup returns 1 on success, 0 on failure
   if (dns_lookup(wg->host, &(wg->host_addr)))
   {
      ...
      {
         snprintf(lastErrorMsg, sizeof(lastErrorMsg), "could not get AF_INET/AF_INET6 for %s", wg->host);
         freeData(wg);
         wg = NULL;
      }
      ...
      else if ((res = (*appCB->establishConnection)(wg->host_addr, &fd)) < 0)
      {
         if (res == -1)
         {
...
int wget_Disconnect(tWgetInternal *wio)
{
   ...
   if (appCB == NULL)
    ...
    if (wio!=NULL)
    { 
        cmsTmr_cancel(appCB->tmrHandle, timer_response, wio); /* may be running */
