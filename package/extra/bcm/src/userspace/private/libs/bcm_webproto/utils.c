...
int dns_lookup(const char *name, struct sockaddr_storage *addr)
{
	...
	{
		cmsLog_debug("dns_lookup(%s) failed", name);
...
int findProc(const char* str)
{
	...
	if ((d = opendir("/proc")) != NULL) {
		while ((de = readdir(d))) {
			...
			if ((n = readlink(path, cmd, sizeof(cmd)-1)) >= 0)
				cmd[n] = '\0';
...
int testBoolean(const char *s)
{
	...
	if (strcmp(s,"0")==0)
		...
		r = 1;
                ...
                cmsLog_error("unexpected string ==>%s<==, returning %d", s, r);
	return r;
}
...
static char *makeNonce(void)
{
	...
	snprintf(buf, sizeof(buf), "%8x:%8x:%8x", 
		getRandomNumber(0,UINT32_MAX), getRandomNumber(0,UINT32_MAX), getRandomNumber(0,UINT32_MAX));
	b64_encode(buf, 0, &np);
	return np;
}
...
int parseAuthorizationHdr(char *ahdr, SessionAuth *sa, char *username, char *password)
{
	...
	if ( ahdr && (p=strcasestr(ahdr, "digest"))) {
		...
		getArg(p,"realm", &sa->realm);
   		...
   		{
   			cmsLog_error("Mismatch authorization header realm value =%s", sa->realm);
   			return 0;
  		}
		...
		getArg(p,"opaque", &sa->opaque);
 		...
 		{
 			cmsLog_error("Mismatch authorization header opaque value =%s", sa->opaque);
 			return 0;
 		}
		...
		{
			cmsLog_error("Mismatch authorization header qop value =%s", sa->qop);
			return 0;
		}
...
static void generateCnonce(char **cnonceBuf)
{
	...
	time_t	now;
	now= time(NULL);
	snprintf(buf, 12, "%011lld", (long long) now);
...
static void generateBasicAuth(SessionAuth *sa, char *user, char *pwd)
{
	...
	strncpy(raw, user, sizeof(raw)-1);
	strncat(raw, ":", (sizeof(raw)-1)-strlen(raw));
	strncat(raw, pwd, (sizeof(raw)-1)-strlen(raw));
	dataLen=strlen(raw);
...
uint32_t getRandomNumber(uint32_t from, uint32_t to)
{
   ...
   fRandom = open("/dev/urandom", O_NDELAY);
   if (fRandom >= 0)
   {
      if ((ret = read(fRandom, &num, sizeof(num))) > 0)
