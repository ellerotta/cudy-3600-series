...
void stopListener(int fd)
{
   ...
   for (prev = curr = appCB->listeners; curr != NULL && curr->fd != fd; curr = curr->next)
   {
      prev = curr;
   }
   ...
   {
      // We found a match
      // See if it is head of list
      if (curr == appCB->listeners)
      {
         appCB->listeners = curr->next;
      }
      else
      {
         // matching entry is not at the head of list
...
void closeAllListeners(void)
{
   ...
   for (l = appCB->listeners; l; l = l->next)
   {
      close(l->fd);
   }
}
...
void freeAllListeners(void)
{
   ...
   while (appCB->listeners)
   {
      tmp = appCB->listeners->next;
      cmsMem_free(appCB->listeners);
      appCB->listeners = tmp;
   }
}
...
void setListenerType(int fd, CmsEventHandler func, void* handle, tListenerType type)
{
   ...
   l = cmsMem_alloc((sizeof *l), ALLOC_ZEROIZE);
   l->fd     = fd;
   l->func   = func;
   l->handle = handle;
   l->next   = appCB->listeners;
   l->type   = type;
   appCB->listeners = l;
}
