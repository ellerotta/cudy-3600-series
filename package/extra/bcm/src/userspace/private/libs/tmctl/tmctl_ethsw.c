...
#if defined(HAS_NONBRCM_SW)
#define NONBRCM_SW_TBDP(fmt, arg...) if (unit==1) {tmctl_error(" !BRCM_SW TBD.." fmt, ##arg); return TMCTL_SUCCESS;}
#else
#define NONBRCM_SW_TBDP(fmt, arg...)
#endif
/* ----------------------------------------------------------------------------
                                    ...
                                    tmctl_portQcfg_t* portQcfg_p)
{
   ...
   if (unit == 1)
   {
      /* Do not support queue shaper on external switch port to avoid packet drop in it. */
                                    ...
                                    tmctl_portQcfg_t* portQcfg_p)
{
   ...
   NONBRCM_SW_TBDP("%s #Qs=%d pmap=%x sched=%d #spq=%d wrrType=%d", ifname, portQcfg_p->numQueues, portmap, schedMode, numSpQ, wrrType);
...
/* ----------------------------------------------------------------------------
 * This function gets the Ethernet switch port rx rate configuration.
 *
 * Parameters:
 *    ifname (IN)    Linux interface name.
 *    shaper_p (OUT) The queue shaper configuration.
 ...
 * ----------------------------------------------------------------------------
 */
tmctl_ret_e tmctlEthSw_getPortRxRate(const char*     ifname,
                                     tmctl_shaper_t* shaper_p)
{
   tmctl_ret_e ret = TMCTL_SUCCESS;
   ...
   memset(shaper_p, 0, sizeof(tmctl_shaper_t));
   NONBRCM_SW_TBDP("%s ",ifname);
   
   tmctl_debug("Calling bcm_port_rate_egress_get_X: unit=%d port=%d",
               unit, port);
   rc = bcm_port_rate_ingress_get(unit, port, &shapingRate, &burstSize);
   if (rc)
   {
      tmctl_error("bcm_port_rate_ingress_get returns error %d", rc);
      return TMCTL_ERROR;
   }
...
}  /* End of tmctlEthSw_getPortRxRate() */


/* ----------------------------------------------------------------------------
 * This function sets the Ethernet switch port rx rate configuration.
 *
 * Parameters:
 *    ifname (IN)   Linux interface name.
 *    shaper_p (IN) The port shaper configuration.
 ...
 * ----------------------------------------------------------------------------
 */
tmctl_ret_e tmctlEthSw_setPortRxRate(const char*     ifname,
                                     tmctl_shaper_t* shaper_p)
{
   tmctl_ret_e ret = TMCTL_SUCCESS;
   ...
   while (portmap)
   {
      ...
      {
         /* set port shaping */      
         tmctl_debug("Calling bcm_port_rate_ingress_set: unit=%d port=%d rate=%d burst=%d",
                     unit, port, shaper_p->shapingRate, shaper_p->shapingBurstSize);
         rc = bcm_port_rate_ingress_set(unit, port, shaper_p->shapingRate /* kbps */, (shaper_p->shapingBurstSize /* Bytes */ * 8)/1000);
         if (rc)
         {
            tmctl_error("bcm_port_rate_ingress_set returns error %d", rc);
            return TMCTL_ERROR;
         }
      }
   }
...
}  /* End of tmctlEthSw_setPortRxRate() */
...
/* ----------------------------------------------------------------------------
 * This function sets the Ethernet switch queue state.
 *
 * Parameters:
 *    ifname (IN)   Linux interface name.
 *    qid (IN)      Queue ID.
 ...
 * ----------------------------------------------------------------------------
 */
tmctl_ret_e tmctlEthSw_setQueueState(const char*     ifname,
                                      int             qid,
                                      BOOL            enable)
{
   int        rc;

   tmctl_debug("Enter: ifname=%s qid=%d enable=%d",
               ifname, qid, enable);

   rc = bcm_port_txq_set(ifname, qid, enable);
   if (rc)
   {
      tmctl_error("bcm_port_txq_set returns error %d", rc);
      return TMCTL_ERROR;
   }
   return TMCTL_SUCCESS;
   
}  /* End of tmctlEthSw_setQueueState() */
                                      ...
                                      tmctl_portTmParms_t* tmParms_p)
{
   ...
   if (unit == 1)
   {
       static uint32_t port_map_g=0;
       
       tmParms_p->schedCaps |= TMCTL_SP_CAPABLE;  // setup dummy default values
       tmParms_p->maxQueues = 4;
       tmParms_p->maxSpQueues = 4;
       tmParms_p->queueShaper = 1;
       tmParms_p->portShaper = 1;
       if (!(port_map_g & (1<<port))) {
           port_map_g |= 1<<port;
           NONBRCM_SW_TBDP("%s schedType=0x%x maxQueues=%d maxSpQueues=%d queueShaper=%d portShaper=%d", 
                        ifname, tmParms_p->schedCaps, tmParms_p->maxQueues, tmParms_p->maxSpQueues,
...
static void _print_devtype_if_from_unit_port(int unit, int port, char* prompt)
{
    char buf[80];
    
    if (bcm_ifname_get(unit, port, buf) == 0)
        printf("%s devtype:%d(eth) if:%s\n", prompt, TMCTL_DEV_ETH, buf);
    else
        printf("%s devtype:%d(eth) unit/port:%d/%d\n", prompt, TMCTL_DEV_ETH, unit, port);
}

tmctl_ret_e tmctl_EthSwMirrorPrint(tmctl_mirror_t *mirror_p)
{
    int port;

    printf(" Mirror %s configuration: (unit=%d)\n", mirror_p->op == TMCTL_MIRROR_RX_GET ? "Rx" : "Tx", mirror_p->unit );
    if (mirror_p->srcMap)
    {
        for (port = 0; port <= 8; port++)
        {
            if (mirror_p->srcMap & 1 << port)
                _print_devtype_if_from_unit_port(mirror_p->unit, port, " sourceif");
        }
        _print_devtype_if_from_unit_port(mirror_p->unit, mirror_p->destPort, "   destif");
    }
    return TMCTL_SUCCESS; 
}

tmctl_ret_e tmctl_EthSwGetMirror(tmctl_mirror_t *mirror_p)
{
    int enable, port, ret;
    unsigned int ing_pmap, eg_pmap;
    unsigned int blk_no_mrr;    // not used
    int tx_port, rx_port;       // not used

    ret = bcm_port_mirror_get(mirror_p->unit, &enable, &port, &ing_pmap, &eg_pmap, &blk_no_mrr, &tx_port, &rx_port);
    if (ret) return TMCTL_ERROR;

    mirror_p->srcMap = 0;
    if (enable)
    {
        mirror_p->srcMap = (mirror_p->op == TMCTL_MIRROR_RX_GET) ? ing_pmap : eg_pmap;
        mirror_p->destPort = port;
    }   
    return TMCTL_SUCCESS;
}

tmctl_ret_e tmctl_EthSwClrMirror(tmctl_mirror_t *mirror_p)
{
    int enable, port, ret;
    unsigned int ing_pmap, eg_pmap;
    unsigned int blk_no_mrr;    // not used
    ...
    ret = bcm_port_mirror_get(mirror_p->unit, &enable, &port, &ing_pmap, &eg_pmap, &blk_no_mrr, &tx_port, &rx_port);
    if (ret) return TMCTL_SUCCESS;

    if (enable && ((mirror_p->op == TMCTL_MIRROR_RX_CLR && ing_pmap) ||
                   (mirror_p->op == TMCTL_MIRROR_TX_CLR && eg_pmap)))
    {
        if (mirror_p->op == TMCTL_MIRROR_RX_CLR)
        {
            ing_pmap = 0;
            enable = eg_pmap ? 1 : 0;
        }
        ...
        ret = bcm_port_mirror_set(mirror_p->unit, enable, port, ing_pmap, eg_pmap, blk_no_mrr, tx_port, rx_port);
        if (ret)
        {
            tmctl_error("fail to clear SF2 mirror config\n");
            return TMCTL_ERROR;
        }
    }
    return TMCTL_SUCCESS;
...
tmctl_ret_e tmctl_EthSwAddMirror(tmctl_mirror_t *mirror_p)
{
    int enable, port, ret;
    unsigned int ing_pmap, eg_pmap;
    unsigned int blk_no_mrr;    // not used
    ...
    if (ret)
    {
        tmctl_error("can't get existing SF2 mirror config\n");
        return TMCTL_ERROR;
    }

    if (enable && port != mirror_p->destPort)
    {
        tmctl_error("dest port conflict\n");
        return TMCTL_ERROR;
    }

    if (!enable)
    {
        ing_pmap = eg_pmap = blk_no_mrr = 0;    
    }
    
    enable = 1;
    port = mirror_p->destPort;
    if (mirror_p->op == TMCTL_MIRROR_RX_ADD)
        ing_pmap |= 1 << mirror_p->srcPort;
    else
        eg_pmap |= 1 << mirror_p->srcPort;
        
    ret = bcm_port_mirror_set(mirror_p->unit, enable, port, ing_pmap, eg_pmap, blk_no_mrr, tx_port, rx_port);
    if (ret)
    {
        tmctl_error("fail to add SF2 mirror config\n");
        return TMCTL_ERROR;
    }
    return TMCTL_SUCCESS;
}

tmctl_ret_e tmctl_EthSwDelMirror(tmctl_mirror_t *mirror_p)
{
    int enable, port, ret;
    unsigned int ing_pmap, eg_pmap;
    unsigned int blk_no_mrr;    // not used
    ...
    if (mirror_p->op == TMCTL_MIRROR_RX_DEL)
        ing_pmap &= ~(1 << mirror_p->srcPort);
    else
        eg_pmap &= ~(1 << mirror_p->srcPort);
    ...
    if (ret)
    {
        tmctl_error("fail to del SF2 mirror config\n");
        return TMCTL_ERROR;
    }
    return TMCTL_SUCCESS;
}