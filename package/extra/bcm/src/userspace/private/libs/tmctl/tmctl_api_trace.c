...
#define TMCTL_TRACE_FILE_LOG       "/var/tmctl.traceLog"
#define TMCTL_TRACE_FILE_ENABLE    "/var/tmctl.traceEnable"
#define TMCTL_TRACE_FILE_PRT2LOG   "/var/tmctl.tracePrt2log"
...
#define __print(fmt, arg...)                                            \
    do {                                                                \
        if (!tmctlApiTraceEnable_g &&                                   \
            !access(TMCTL_TRACE_FILE_ENABLE, F_OK))                     \
            tmctlApiTraceEnable_g = 1;                                  \
        if (tmctlApiTraceEnable_g) {                                    \
            if (!tmctlApiTracePrintToFile_g &&                          \
                !access(TMCTL_TRACE_FILE_PRT2LOG, F_OK))                \
                tmctlApiTracePrintToFile_g = 1;                         \
            if (tmctlApiTracePrintToFile_g) {                           \
                FILE *fd = fopen(TMCTL_TRACE_FILE_LOG, "a+");           \
                if (fd == NULL) {                                       \
                    __error("Could not open %s\n", TMCTL_TRACE_FILE_LOG); \
                } else {                                                \
                    fprintf(fd, fmt, ##arg);                            \
                    fclose(fd);                                         \
                }                                                       \
...
{
    if (!tmctlApiTraceEnable_g && !access(TMCTL_TRACE_FILE_ENABLE, F_OK))
        tmctlApiTraceEnable_g = 1;
...
static void __queueCfgTrace(tmctl_queueCfg_t *pQueueCfg)
{
    __print("--qid %d --priority %d --qsize %d --weight %d --minbufs %d --schedmode %d "
            ...
            pQueueCfg->bestEffort, pQueueCfg->l4s);
...
static void __queueDropAlgTrace(tmctl_queueDropAlg_t *pQueueCfg)
{
    ...
    __print("--dropalg %d "
            "--loredminthr %d --loredmaxthr %d --loredpct %d "
...
static void __tcToQTrace(tmctl_tcToQCfg_t* cfg_p)
{
    int i;
...
void tmctl_configTrace(int enable, int printToFile)
{
    ...
    fprintf(stdout, "tmctl trace is %s, print to %s\n", enable ? "enabled" : "disabled",
            (printToFile) ? TMCTL_TRACE_FILE_LOG : "stdout");

    tmctlApiTraceEnable_g = (enable) ? 1 : 0;
    if (enable)
    {
        fd = fopen(TMCTL_TRACE_FILE_ENABLE, "w");
        if (fd) fclose(fd);
    }
    else
    {
        unlink(TMCTL_TRACE_FILE_ENABLE);
    }

    tmctlApiTracePrintToFile_g = (printToFile) ? 1 : 0;
    if (printToFile)
    {
        fd = fopen(TMCTL_TRACE_FILE_PRT2LOG, "w");
        if (fd) fclose(fd);
    }
    else
    {
        unlink(TMCTL_TRACE_FILE_PRT2LOG);
    }
}
...
tmctl_ret_e tmctl_getPortRxRateTrace(tmctl_devType_e devType,
                                     tmctl_if_t *if_p, tmctl_shaper_t *shaper_p)
{
    char ifName[TMCTL_TRACE_IF_NAME_SIZE];
    ...
    __trace(getportrxrate, "--devtype %d --if %s\n", devType, ifName);

    return TMCTL_SUCCESS;
}

tmctl_ret_e tmctl_setPortRxRateTrace(tmctl_devType_e devType,
                                     tmctl_if_t *if_p, tmctl_shaper_t *shaper_p)
{
    char ifName[TMCTL_TRACE_IF_NAME_SIZE];
    ...
    __trace(setportrxrate, "--devtype %d --if %s ", devType, ifName);
...
tmctl_ret_e tmctl_CreateShaperTrace(tmctl_devType_e    devType,
                                     tmctl_if_t*        if_p,
                                     tmctl_shaper_t     *shaper_p,
                                     uint64_t *shaper_obj)
{
    ...
    __trace(CreateSubsetqshaper, "--devtype %d --if %s --minRate %d --shapingRate %d --shapingBurstSize %d --shaper_obj %p\n",
                    devType, ifName, shaper_p->minRate, shaper_p->shapingRate, shaper_p->shapingBurstSize, shaper_obj);

    return TMCTL_SUCCESS;
}

tmctl_ret_e tmctl_DeleteShaperTrace(uint64_t shaper_obj)
{
    __trace(DeleteSubsetqshaper, "--shaper_obj %p\n", shaper_obj);
    return TMCTL_SUCCESS;
}

tmctl_ret_e tmctl_SetShaperQueueTrace(tmctl_devType_e    devType,
                                     tmctl_if_t*        if_p,
                                     int queueId,
                                     uint64_t shaper_obj)
{
    ...
    __trace(SetShaperQueue, "--devtype %d --if %s queueId=%d --shaper_obj %p\n",
                    devType, ifName, queueId, shaper_obj);
...
tmctl_ret_e tmctl_setQueueFlowCtrlTrace(tmctl_devType_e devType,
                                        tmctl_if_t *if_p, int queueId,
                                        tmctl_queueFlowCtrl_t *flowCtrl_p)
{
    ...
    __trace(setqflowctrl, "--devtype %d --if %s --qid %d --enable %d "
            "--pausethr %d --hysteresisthr %d\n", devType, ifName, queueId,
            flowCtrl_p->enable, flowCtrl_p->pauseThr, flowCtrl_p->hystThr);

    return TMCTL_SUCCESS;
}

tmctl_ret_e tmctl_getQueueFlowCtrlTrace(tmctl_devType_e devType,
                                        tmctl_if_t *if_p, int queueId,
                                        tmctl_queueFlowCtrl_t* flowCtrl_p)
{
    char ifName[TMCTL_TRACE_IF_NAME_SIZE];
    ...
    __trace(getqflowctrl, "--devtype %d --if %s --qid %d\n", devType, ifName, queueId);
...
tmctl_ret_e tmctl_setTcToQueueTrace(int table_index,
...
tmctl_ret_e tmctl_createPolicerTrace(tmctl_policer_t *policer_p)
{
  __trace(createpolicer,
      "--pid %d --cir %llu \n", policer_p->policerId, (long long)policer_p->cir);
...
tmctl_ret_e tmctl_deletePolicerTrace(int policerId)
{
...
tmctl_ret_e tmctl_getMirrorTrace(tmctl_mirror_op_e op)
{
    if (op < TMCTL_MIRROR_TX_GET)
         __trace(getMirrorRx, "\n");
    else
         __trace(getMirrorTx, "\n");
...
tmctl_ret_e tmctl_clrMirrorTrace(tmctl_mirror_op_e op)
{
    if (op < TMCTL_MIRROR_TX_CLR)
         __trace(clrMirrorRx, "\n");
    else
         __trace(clrMirrorTx, "\n");
...
tmctl_ret_e tmctl_addMirrorTrace(tmctl_devType_e devType,
                                tmctl_if_t* if_p,
                                ...
                                tmctl_ethIf_t destIf)
{
    ...
    if (op < TMCTL_MIRROR_TX_ADD)
         __trace(addMirrorRx, "--devtype %d --if %s --destif %s \n", devType, ifName, destIf.ifname);
    else
         __trace(addMirrorTx, "--devtype %d --if %s --destif %s \n", devType, ifName, destIf.ifname);

  return TMCTL_SUCCESS;
}

tmctl_ret_e tmctl_delMirrorTrace(tmctl_devType_e devType,
                                tmctl_mirror_op_e   op,
                                tmctl_if_t* if_p)
{
    char ifName[TMCTL_TRACE_IF_NAME_SIZE];
    ...
    if (op < TMCTL_MIRROR_TX_ADD)
         __trace(delMirrorRx, "--devtype %d --if %s \n", devType, ifName);
    else
         __trace(delMirrorTx, "--devtype %d --if %s \n", devType, ifName);
