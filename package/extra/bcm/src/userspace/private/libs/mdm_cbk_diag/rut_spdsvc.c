...
CmsRet rutSpdsvc_runSpeedService( SpeedServiceObject *spdsvcObj )
{
   ...
   // mark command from BDK or CMS and put it into background
   if (cmsMdm_isBdkDiag())
   {
      ptr += sprintf(ptr, " fromBDK&");
   }
   else
   {
      ptr += sprintf(ptr, " fromCMS&");
   }
   cmsLog_debug("final cmdLine=%s", cmd);
...
void rutSpdsvc_addPreviousTestResult(const SpeedServiceObject *currObj,
...
static void rut_checkResultHistory( SpeedServiceObject *spdsvcObj)
{
   ...
   // Prevent result history table from growing too large
   while (spdsvcObj->resultHistoryNumberOfEntries > MAX_SPDSVC_HISTORY_ENTRIES)
   {
      ...
      cmsObj_free((void **) &resultObj);  // don't need it, free it.
      ...
      INIT_INSTANCE_ID_STACK(&iidStack);  // tell getNext to start from beginning of list
   }
}
