...
void modifyDeviceInfoNumEntries(MdmObjectId oid, SINT32 delta)
{
   ...
   if (mdmShmCtx->inMdmInit)
   {
       ...
       */
      cmsLog_debug("don't update count for new object oid %d (delta=%d)",oid,delta);
      return;
   }

   if ((cmsObj_get(MDMOID_DEV2_DEVICE_INFO, &iidStack, 0, (void *) &obj)) == CMSRET_SUCCESS)
   {
      if (oid == MDMOID_DEV2_DEVICE_VENDOR_CONFIG_FILE)
      {
         obj->vendorConfigFileNumberOfEntries += delta;
      }
      else if (oid == MDMOID_DEV2_PROCESS_STATUS)
      {
         obj->processorNumberOfEntries += delta;
      }
#ifdef DMP_DEVICE2_SUPPORTEDDATAMODEL_1
      else if (oid == MDMOID_DEV2_SUPPORTED_DATA_MODEL)
      {
         obj->supportedDataModelNumberOfEntries += delta;
      }
#endif
                                             ...
                                             CmsRet *errorCode __attribute__((unused)))
{
   ...
   {
      modifyDeviceInfoNumEntries(MDMOID_DEV2_DEVICE_VENDOR_CONFIG_FILE, 1);
   }
                                        ...
                                        CmsRet *errorCode __attribute__((unused)))
{
   if (ADD_NEW(newObj, currObj))
   {
      modifyDeviceInfoNumEntries(MDMOID_DEV2_SUPPORTED_DATA_MODEL, 1);
   }
   else if (DELETE_EXISTING(newObj, currObj))
   {
      modifyDeviceInfoNumEntries(MDMOID_DEV2_SUPPORTED_DATA_MODEL, -1);
...
CmsRet rcl_dev2RebootInfoObject( _Dev2RebootInfoObject *newObj __attribute__((unused)),
                const _Dev2RebootInfoObject *currObj __attribute__((unused)),
                const InstanceIdStack *iidStack __attribute__((unused)),
                char **errorParam __attribute__((unused)),
                CmsRet *errorCode __attribute__((unused)))
{
