...
/* TODO: XXX this could be a problem in the standalone component version */
                            ...
                            const InstanceIdStack *iidStack __attribute__((unused)))
{
   ...
   if (obj->additionalHardwareVersion == NULL)
   {
      ...
      ret = devCtl_boardIoctl(BOARD_IOCTL_GET_ID, 0,
                              boardId, sizeof(boardId), 0, NULL);
      ...
      obj->processorNumberOfEntries = sysUtil_getNumCpuThreads();
   ...
   // SoftwareVersion is only updated during bootup.  See comments in
   // mdm_adjustForHardware_dev2_devinfo.  Perhaps we should do the same
   // for SwBuildTimestamp, additionalSoftwareVersion, hardwareVersion,
   // additionalHardwareVersion, serialNumber, and any other params which
   // do not change after bootup so they work better with active notification.
   ...
   if (obj->additionalSoftwareVersion == NULL)
   {
      ...
      ret = (CmsRet) bcmUtl_getBootloaderVersion(finalString, sizeof(finalString));
      if (ret != CMSRET_SUCCESS)
      {
