...
IMPL_setSyslogLevel(vlanCtl);
IMPL_getSyslogLevel(vlanCtl);
IMPL_isSyslogLevelEnabled(vlanCtl);
IMPL_setSyslogMode(vlanCtl);
IMPL_isSyslogEnabled(vlanCtl);
...
{
    VLANCTL_ASSERT(logLevel < VLANCTL_LOG_LEVEL_MAX);
                                ...
                                int isRouted, int isMulticast)
{
    ...
    strncpy(iocCreateVlan.realDevName, realDevName, BCM_VLAN_IFNAMSIZ-1);
    iocCreateVlan.realDevName[BCM_VLAN_IFNAMSIZ-1] = '\0';
    iocCreateVlan.vlanDevId = vlanDevId;
    iocCreateVlan.isRouted = isRouted;
    iocCreateVlan.isMulticast = isMulticast;
    iocCreateVlan.isSwOnly = 0;
                                   ...
                                   vlanCtl_createParams_t *createParamsP)                                    
{
    ...
    vlanCtl_createVlanInterfaceExtTrace(realDevName, vlanDevId, createParamsP);
    ...
    iocCreateVlan.realDevName[BCM_VLAN_IFNAMSIZ-1] = '\0';
    iocCreateVlan.vlanDevId = vlanDevId;
    iocCreateVlan.isRouted = createParamsP->isRouted;
    iocCreateVlan.isMulticast = createParamsP->isMulticast;
    iocCreateVlan.isSwOnly = createParamsP->isSwOnly;
                                      ...
                                      int isRouted, int isMulticast)
{
    ...
    strncpy(iocCreateVlan.realDevName, realDevName, sizeof(iocCreateVlan.realDevName)-1);
    iocCreateVlan.realDevName[sizeof(iocCreateVlan.realDevName)-1] = '\0';
    strncpy(iocCreateVlan.vlanDevName, vlanDevName, sizeof(iocCreateVlan.vlanDevName)-1);
    iocCreateVlan.vlanDevName[sizeof(iocCreateVlan.vlanDevName)-1] = '\0';
    iocCreateVlan.isRouted = isRouted;
    iocCreateVlan.isMulticast = isMulticast;
    iocCreateVlan.isSwOnly = 0;
                                         ...
                                         vlanCtl_createParams_t *createParamsP)
{
    ...
    vlanCtl_createVlanInterfaceByNameExtTrace(realDevName, vlanDevName, createParamsP);
...
int vlanCtl_deleteVlanInterface(char *vlanDevName)
{
    ...
    strncpy(iocDeleteVlan.vlanDevName, vlanDevName, BCM_VLAN_IFNAMSIZ-1);
    iocDeleteVlan.vlanDevName[BCM_VLAN_IFNAMSIZ-1] = '\0';
                          ...
                          unsigned int *tagRuleId)
{
    ...
    if(tableDir == VLANCTL_DIRECTION_TX &&
    ...
    {
        VLANCTL_LOG_ERROR("--filter-rxif and --filter-txif only apply to tx tag rules %s, %s",
          vlanCtl_tagRule.filter.rxRealDevName, vlanCtl_tagRule.filter.txVlanDevName);

        ret = -EINVAL;
        goto out;
    }
    
    strncpy(iocInsertTagRule.ruleTableId.realDevName, realDevName, BCM_VLAN_IFNAMSIZ-1);
    iocInsertTagRule.ruleTableId.realDevName[BCM_VLAN_IFNAMSIZ-1] = '\0';
    ...
    else
    {
        *tagRuleId = iocInsertTagRule.tagRule.id;
        ...
        VLANCTL_LOG_DEBUG("Created new Tag Rule: dev=%s, dir=%d, tags=%d, id=%d\n\n",
                          ...
                          unsigned int nbrOfTags, unsigned int tagRuleId)
{
    ...
    strncpy(iocRemoveTagRule.ruleTableId.realDevName, realDevName, BCM_VLAN_IFNAMSIZ-1);
    iocRemoveTagRule.ruleTableId.realDevName[BCM_VLAN_IFNAMSIZ-1] = '\0';
...
int vlanCtl_removeAllTagRule(char *vlanDevName)
{
    ...
    strncpy(iocRemoveAllTagRule.vlanDevName, vlanDevName, BCM_VLAN_IFNAMSIZ-1);
    iocRemoveAllTagRule.vlanDevName[BCM_VLAN_IFNAMSIZ-1] = '\0';
...
int vlanCtl_dumpRuleTable(char *realDevName, vlanCtl_direction_t tableDir, unsigned int nbrOfTags)
{
    ...
    strncpy(iocDumpRuleTable.ruleTableId.realDevName, realDevName, BCM_VLAN_IFNAMSIZ-1);
    iocDumpRuleTable.ruleTableId.realDevName[BCM_VLAN_IFNAMSIZ-1] = '\0';
...
int vlanCtl_dumpAllRules(void)
{
    ...
    ret = vlanctl_dev_ioctl(vlanCtl_fileDesc, BCM_VLAN_IOC_DUMP_ALL_RULES, NULL);
    if(ret)
    {
        VLANCTL_LOG_ERROR("%s", strerror(errno));
    }
                                 ...
                                 unsigned int nbrOfTags, unsigned int *nbrOfRules)
{
    ...
    strncpy(iocGetNbrOfRulesInTable.ruleTableId.realDevName, realDevName, BCM_VLAN_IFNAMSIZ-1);
    iocGetNbrOfRulesInTable.ruleTableId.realDevName[BCM_VLAN_IFNAMSIZ-1] = '\0';
                              ...
                              unsigned int defaultVid)
{
    ...
    strncpy(iocSetDefaultVlanTag.ruleTableId.realDevName, realDevName, BCM_VLAN_IFNAMSIZ-1);
    iocSetDefaultVlanTag.ruleTableId.realDevName[BCM_VLAN_IFNAMSIZ-1] = '\0';
...
int vlanCtl_setDscpToPbits(char *realDevName, unsigned int dscp, unsigned int pbits)
{
    ...
    strncpy(iocDscpToPbits.realDevName, realDevName, BCM_VLAN_IFNAMSIZ-1);
    iocDscpToPbits.realDevName[BCM_VLAN_IFNAMSIZ-1] = '\0';
...
int vlanCtl_setTpidTable(char *realDevName, unsigned int *tpidTable)
{
    ...
    strncpy(iocSetTpidTable.realDevName, realDevName, BCM_VLAN_IFNAMSIZ-1);
    iocSetTpidTable.realDevName[BCM_VLAN_IFNAMSIZ-1] = '\0';
...
int vlanCtl_dumpTpidTable(char *realDevName)
{
    ...
    strncpy(iocDumpTpidTable.realDevName, realDevName, BCM_VLAN_IFNAMSIZ-1);
    iocDumpTpidTable.realDevName[BCM_VLAN_IFNAMSIZ-1] = '\0';
...
int vlanCtl_dumpLocalStats(char *realDevName)
{
    ...
    strncpy(iocDumpLocalStats.realDevName, realDevName, BCM_VLAN_IFNAMSIZ-1);
    iocDumpLocalStats.realDevName[BCM_VLAN_IFNAMSIZ-1] = '\0';
...
int vlanCtl_setIfSuffix(char *ifSuffix)
{
    ...
    strncpy(iocSetIfSuffix.suffix, ifSuffix, BCM_VLAN_IF_SUFFIX_SIZE-1);
    iocSetIfSuffix.suffix[BCM_VLAN_IF_SUFFIX_SIZE-1] = '\0';
                             ...
                             vlanCtl_defaultAction_t defaultAction, char *defaultRxVlanDevName)
{
    ...
    if(tableDir == VLANCTL_DIRECTION_RX && defaultAction == VLANCTL_ACTION_ACCEPT)
    {
        ...
        strncpy(iocSetDefaultAction.defaultRxVlanDevName, defaultRxVlanDevName, BCM_VLAN_IFNAMSIZ-1);
        iocSetDefaultAction.defaultRxVlanDevName[BCM_VLAN_IFNAMSIZ-1] = '\0';
    }
    else
    {
        /* defaultRxVlanDevName is not needed (may be NULL) */

        strncpy(iocSetDefaultAction.defaultRxVlanDevName, "DONTCARE", BCM_VLAN_IFNAMSIZ-1);
        iocSetDefaultAction.defaultRxVlanDevName[BCM_VLAN_IFNAMSIZ-1] = '\0';
    }

    strncpy(iocSetDefaultAction.ruleTableId.realDevName, realDevName, BCM_VLAN_IFNAMSIZ-1);
    iocSetDefaultAction.ruleTableId.realDevName[BCM_VLAN_IFNAMSIZ-1] = '\0';
...
int vlanCtl_setRealDevMode(char *realDevName, bcmVlan_realDevMode_t mode)
{
    ...
    strncpy(iocSetRealDevMode.realDevName, realDevName, BCM_VLAN_IFNAMSIZ-1);
    iocSetRealDevMode.realDevName[BCM_VLAN_IFNAMSIZ-1] ='\0';
...
int vlanCtl_createVlanFlows(char *rxVlanDevName, char *txVlanDevName)
{
    ...
    strncpy(iocVlanFlows.rxVlanDevName, rxVlanDevName, sizeof(iocVlanFlows.rxVlanDevName)-1);
    iocVlanFlows.rxVlanDevName[sizeof(iocVlanFlows.rxVlanDevName)-1] = '\0';    
    strncpy(iocVlanFlows.txVlanDevName, txVlanDevName, sizeof(iocVlanFlows.txVlanDevName)-1);
    iocVlanFlows.txVlanDevName[sizeof(iocVlanFlows.txVlanDevName)-1] = '\0';
...
int vlanCtl_runTest(unsigned int testNbr, char *rxVlanDevName, char *txVlanDevName)
{
    ...
    strncpy(iocRunTest.rxVlanDevName, rxVlanDevName, BCM_VLAN_IFNAMSIZ-1);
    iocRunTest.rxVlanDevName[BCM_VLAN_IFNAMSIZ-1] = '\0';
    strncpy(iocRunTest.txVlanDevName, txVlanDevName, BCM_VLAN_IFNAMSIZ-1);
    iocRunTest.txVlanDevName[BCM_VLAN_IFNAMSIZ-1] = '\0';
...
int vlanCtl_filterOnRxRealDevice(char *realDevName)
{
    ...
    strncpy(vlanCtl_tagRule.filter.rxRealDevName, realDevName, BCM_VLAN_IFNAMSIZ-1);
    vlanCtl_tagRule.filter.rxRealDevName[BCM_VLAN_IFNAMSIZ-1] = '\0';
...
int vlanCtl_filterOnTxVlanDevice(char *vlanDevName)
{
    ...
    strncpy(vlanCtl_tagRule.filter.txVlanDevName, vlanDevName, BCM_VLAN_IFNAMSIZ-1);
    vlanCtl_tagRule.filter.txVlanDevName[BCM_VLAN_IFNAMSIZ-1] ='\0';
...
int vlanCtl_setReceiveVlanDevice(char *vlanDevName)
{
    ...
    strncpy(vlanCtl_tagRule.rxVlanDevName, vlanDevName, BCM_VLAN_IFNAMSIZ-1);
    vlanCtl_tagRule.rxVlanDevName[BCM_VLAN_IFNAMSIZ-1] = '\0';
