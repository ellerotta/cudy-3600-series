...
/* brcm start */
	fprintf(stderr, "%sFIPS 140-2 %% fail: %2.2f%%\n",
		logprefix, (double)(rng_stats.bad_fips_blocks*100)/(rng_stats.good_fips_blocks+rng_stats.bad_fips_blocks) );
/* brcm end */
	fprintf(stderr, "%s\n", dump_stat_counter(buf, sizeof(buf),
...
/* brcm start */
		if( max_fips_blocks && ((rng_stats.bad_fips_blocks + rng_stats.good_fips_blocks) >= max_fips_blocks) )
			return;
/* brcm end */
	}

/* brcm start */
	max_fips_blocks = rng_stats.bad_fips_blocks + rng_stats.good_fips_blocks;
/* brcm end */
}
...
/* brcm start */
	if( max_fips_blocks ) {
		if( gotsigterm ) {
			fprintf(stderr, "%sFIPS 140-2 tests ending due to sigterm!\n",
				logprefix);
		}
		fprintf(stderr, "%sFIPS 140-2 testruns completed: %d\n",
			logprefix, max_fips_blocks);
	}
/* brcm end */
