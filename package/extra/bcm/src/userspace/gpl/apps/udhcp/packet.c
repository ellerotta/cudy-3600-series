...
static int calc_length(struct dhcpMessage *payload)
{
	int payload_length;
// brcm
	unsigned int options_length = end_option(payload->options) + 1;
	
	payload_length = sizeof(struct dhcpMessage) - MAX_OPTIONS_LEN;
	// Make sure the size never exceeds the buffer size
	if (options_length > MAX_OPTIONS_LEN) {
		DEBUG(LOG_ERR, "%s: ERROR: Options length (%d) exceeds buffer (%d)!",
			__FUNCTION__, options_length, MAX_OPTIONS_LEN);
		options_length = MAX_OPTIONS_LEN;
	}
	payload_length += options_length;
// brcm
	if (payload_length % 2) {
		// Make the size even and add a 0x00 to the buffer
		payload_length++;
