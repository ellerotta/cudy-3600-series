...
// brcm
	if (end + string[OPT_LEN] + 2 + 1 >= (int)MAX_OPTIONS_LEN) {
		for (i = 0; options[i].code && options[i].code != string[OPT_CODE]; i++);
		LOG(LOG_ERR, "Option %s (0x%02x) did not fit into the packet!  Current len (%d), new option len (%d)",
			options[i].code ? options[i].name : "unknown", string[OPT_CODE], end, string[OPT_LEN]);
		return 0;
	}
// brcm
...
// brcm
	/* end position + string length + option code/length + end option */
	if (!length || end + length + 2 + 1 > (int)MAX_OPTIONS_LEN) {
// brcm
...
CmsRet queryDeviceInfo(GetDeviceInfoMsgBody *pDeviceInfo)
{
   ...
   char replyBuf[sizeof(CmsMsgHeader)+sizeof(GetDeviceInfoMsgBody)];
   CmsMsgHeader *replyMsg = (CmsMsgHeader *)replyBuf;
   CmsMsgHeader sendMsg;
   GetDeviceInfoMsgBody *body = NULL;

   memset(replyMsg, 0, sizeof(CmsMsgHeader)+sizeof(GetDeviceInfoMsgBody));

   memset(&sendMsg, 0, sizeof(CmsMsgHeader));
   sendMsg.type = CMS_MSG_GET_DEVICE_INFO;
   sendMsg.src = EID_DHCPD;
   sendMsg.dst = EID_SSK;
   sendMsg.flags_request = 1;

   ret = cmsMsg_sendAndGetReplyBufWithTimeout(msgHandle, &sendMsg, &replyMsg, timeoutMs);
   if (ret != CMSRET_SUCCESS)
   {
      cmsLog_error("SendAndGetReplyBufWithTimeout is failed. ret = %d.", ret);
      goto out;
   }

   if (replyMsg->type != CMS_MSG_GET_DEVICE_INFO)
   {
      cmsLog_error("Invalid message type %x", replyMsg->type);
      ret = CMSRET_INTERNAL_ERROR;
      goto out;
   }

   if (replyMsg->dataLength != sizeof(GetDeviceInfoMsgBody))
   {
   ...
   body = (GetDeviceInfoMsgBody *) (replyMsg + 1);

   cmsLog_debug("===> queryDeviceInfo, oui=<%s>, serialNum=<%s>, productClass=<%s>.",
                body->oui, body->serialNum, body->productClass);

   memcpy(pDeviceInfo, body, sizeof(GetDeviceInfoMsgBody));
...
/* Create OptionX */
int CreateOptionX(int code, char *VIinfo)
{
    int len = 0;
    return ReadOptionX(code, VIinfo, &len);
}
...
int ReadOptionX(int code, char *VIinfo, int *len)
{
    ...
    /* Full name example: /var/udhcpc/veip0.1/option60.in */
    snprintf(file, sizeof(file), "%s/%s/%s%d%s", BRCM_UDHCPC_CONFIG_DIR, session_path, 
                                BRCM_UDHCPC_CONFIG_FILE_BASE_NAME, code,
                                BRCM_UDHCPC_CONFIG_IN_FILE_SUFFIX);
    if (!(in = fopen(file, "r"))) {
        LOG(LOG_ERR, "unable to open option file: %s", file);
        return -1;
    }
    ...
    /* verify */
    if (rlen != (buffer[VENDOR_OPTION_LEN_OFFSET] + VENDOR_OPTION_SUBCODE_OFFSET)) {
        LOG(LOG_ERR, "Invalid option file: %s", file);
        ret = -1;
        goto exit;
    }

    if(cmsLog_getLevel() == LOG_LEVEL_DEBUG) {
        LOG(LOG_DEBUG, "Option%d:\n", code);
        ShowOption(buffer, rlen);
    }

    memcpy(VIinfo, buffer, rlen);
    *len = rlen;
...
int SaveOptionX(int code, const char *option, int len)
{
    ...
    if (fwrite(buffer, wlen, cnt, out) != cnt) {
        LOG(LOG_ERR, "write option file: %s fail", file);
        ret = -1;
        goto exit;
    }
...
/* Save the environment to an env file */
int SaveEnv(char ** envp)
{
    char file[BRCM_UDHCPC_CONFIG_FILE_NAME_LEN] = {0};
    char *buffer;
    FILE *out;
    ...
    i = 0;
    while (envp[i]) {
        sublen = strlen(envp[i]); 
        strcpy(&buffer[offset], envp[i]);
        buffer[offset + sublen] = '\n'; /* replace '\0'  as '\n' */
        offset = offset + sublen + 1; /* 1 for '\n' */
        i++;
    }
    ...
    /* Full name example: /var/udhcpc/veip0.1/option.env */
    snprintf(file, sizeof(file), "%s/%s/%s.env", BRCM_UDHCPC_CONFIG_DIR, session_path, 
                                BRCM_UDHCPC_CONFIG_FILE_BASE_NAME);
    if (!(out = fopen(file, "w"))) {
        LOG(LOG_ERR, "unable to open option file: %s", file);
        ret = -1;
        goto exit;
