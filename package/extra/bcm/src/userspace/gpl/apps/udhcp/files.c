	...
	for(iface = iface_config; iface; iface = iface->next) {
      ...
      if (iface->interface == NULL || strstr(iface->interface, "br") == 0)
		...
		for (retry_count = 0; retry_count < BRCM_RETRY_COUNT;
			...
			sleep(BRCM_RETRY_INTERVAL*(retry_count+1)); // slowly back off
		}
		if (local_rc < 0) {
...
void write_leases(int dummy __attribute__((unused)))
{
	...
	if (server_config.notify_file) {
		...
		if (system(buf) == -1)
			LOG(LOG_ERR, "Unable to execute command %s", buf);
	}
}
...
void send_release_lease(const struct saved_lease *lease)
{
   ...
   snprintf(body->ifName, sizeof(body->ifName), "%s", cur_iface->interface);
   snprintf(body->ipAddr, sizeof(body->ipAddr), "%s",  inet_ntoa(inaddr));
   snprintf(body->hostName, sizeof(body->hostName), "%s", lease->hostname);
...
void read_leases(char *file)
{
	...
	if (!(fp = fopen(file, "r"))) {
		DEBUG(LOG_INFO, "Unable to open %s for reading", file);
		return;
	}

	if (stat(file, &attr) == -1) {
		LOG(LOG_ERR, "Unable to get stat of %s", file);
		return;
	}
		...
		for (iface = iface_config; iface; iface = iface->next) {
			if (ntohl(lease.yiaddr) >= ntohl(iface->start) &&
				...
				iface->leases[iface->cnt_leases].expires =
					ntohl(lease.expires);	
				if (server_config.remaining)
					iface->leases[iface->cnt_leases].expires += last_modify;
...
void send_lease_info(UBOOL8 isDelete, const struct dhcpOfferedAddr *lease)
{
   ...
   snprintf(body->addressSource, sizeof(body->addressSource), CMSVS_DHCP);

   /* is there a way we can tell if we assigned this address to a host on WLAN? */
   snprintf(body->interfaceType, sizeof(body->interfaceType), CMSVS_ETHERNET);

   /* the vendor id is also contained in the lease struct,
    * we could also send that to ssk to put into the host entry. */

   snprintf(body->oui, sizeof(body->oui), "%s", lease->oui);
   snprintf(body->serialNum, sizeof(body->serialNum), "%s", lease->serialNumber);
   snprintf(body->productClass, sizeof(body->productClass), "%s", lease->productClass);
