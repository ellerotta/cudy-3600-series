...
int sendOffer(struct dhcpMessage *oldpacket)
{
	...
	if(!static_lease_ip) {
			...
			ntohl(req_align) <= ntohl(cur_iface->end)) 
		{
			...
			/* requested ip not already taken/offered. Or its taken, but expired */
			if((!(lease = find_lease_by_yiaddr(req_align))) || lease_expired(lease)) {

				packet.yiaddr = req_align; 
			}
			else  {
                		LOG(LOG_WARNING, "Requested IP addresses is in use -- "
				"OFFER abandoned");
                		return -1;
        		}
