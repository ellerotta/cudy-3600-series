...
static unsigned int dst_eid = EID_SSK;
static unsigned int is_bdk = 0;
...
"  -e, --eid=eid                   destination process EID (default: EID_SSK)\n"
"  -B, --bdk                       BDK\n"
...
#endif
{
	...
	while (1) {
		int option_index = 0;
// brcm
		c = getopt_long(argc, argv, "c:fH:hi:np:qr:a:l:Be:s:d:u:v6I:D:O:S:P:Q:R:K", options, &option_index);
		if (c == -1) break;
...
// brcm
   ...
   cmsLog_setLevel(DEFAULT_LOG_LEVEL);
	...
	if (is_bdk) {
		if (dst_eid == EID_OMCID) {
			const char *busName = GPON_MSG_BUS;
			cmsMsg_initOnBus(EID_DHCPC, EIF_MULTIPLE_INSTANCES, busName, &msgHandle);
		}
	}
	else {
...
// brcm
			LOG(LOG_INFO, "Timeout in state %d", state);
// brcm
			switch (state) {
			case INIT_SELECTING:
				// brcm
...
// brcm
			LOG(LOG_INFO, "Received packet in state %d", state);
// brcm
			switch (state) {
			case INIT_SELECTING:
