...
static void init_packet(struct dhcpMessage *packet, char type)
{
	...
	if (is_opt_rep_map_used) {
		int code;
		for (code = DHCP_PADDING; code < DHCP_END; code++) {
			if (opt_rep_map[code]) {
				memset(VSI, 0, sizeof(VSI));
				if (CreateOptionX(code, VSI) == 0) {
					add_option_string(packet->options, (unsigned char *)VSI);
				}
			}
		}
...
static int add_param_req_to_list(char *list, int offset, int code)
{
	if (offset < MAX_DHCP_OPTION_LEN) {
		list[offset++] = code;
		list[OPT_LEN] = offset - 2;
	}
	else {
		LOG(LOG_ERR, "offset(%d) out of range!", offset);
	} 
...
{
	char request_list_base[] = {DHCP_PARAM_REQ, 0, PARM_REQUESTS};
	char request_list[MAX_DHCP_OPTION_LEN] = {0};
	int len=0;

	len = sizeof(request_list_base);
	memcpy(request_list, request_list_base, len);
	request_list[OPT_LEN] = len - 2;

	if (ipv6rd_opt) {
		len = add_param_req_to_list(request_list, len, DHCP_6RD_OPT);
	}
	...
	if (is_opt_req_map_used) {
		...
		for (code = DHCP_PADDING; code < DHCP_END; code++) {
			if (opt_req_map[code]) {
				len = add_param_req_to_list(request_list, len, code);
			}
		}
	}
...
/* Broadcast a DHCP discover packet to the network,
   with optionally requested IP and lease time */
int send_discover(unsigned long xid, unsigned long requested, unsigned long lease)
{
	...
	if (lease)
		add_simple_option(packet.options, DHCP_LEASE_TIME, ntohl(lease));
...
int get_raw_packet(struct dhcpMessage *payload, int fd)
{
    ...
    */
	...
	if (packet.ip.protocol != IPPROTO_UDP || packet.ip.version != IPVERSION ||
       ...
       (sizeof(struct udp_dhcp_packet) - MAX_OPTIONS_LEN) > (unsigned int) bytes) {
