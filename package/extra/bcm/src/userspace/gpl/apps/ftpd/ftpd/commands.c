...
void do_fwUpdate(void)
{
    ...
    for (;;) {
      ...
      if (imgUtilHandle == NULL)
      {
         ...
         if (imgUtilHandle == NULL)
         {
            cmsLog_error("img_util_open failed (not enough memory?)");
            displayMessage(UPLOAD_FAIL_NO_MEM);
            free(buffer);
         ...
         if ((byteRd > CMS_CONFIG_FILE_DETECTION_LENGTH) &&
             cmsImg_isConfigFileLikely(buffer))
         {
            printf("%s: config file detected!\n", appName);
            isConfigFile = 1;
         }
      }

      rc = img_util_write(imgUtilHandle, (UINT8*)buffer, byteRd);
      if (rc < 0)
      {
            cmsLog_error("img_util_write failed.");
            free(buffer);
            img_util_abort(imgUtilHandle);
            imgUtilHandle = NULL;
            displayMessage(UPLOAD_FAIL_NO_MEM);
   ...
   {
      printf("%s: uploaded image size=%u, waiting for quit before writing to flash...\n",
             appName, uploadSize);
...
void command_fwupdate_clean(void)
{
   if (imgUtilHandle != NULL)
   {
      printf("%s: Image write abort.\n", __FUNCTION__);
...
void command_quit(char *params __attribute__((unused)))
{
   ...
   if (isConfigFile)
   {
      printf("%s: writing config file now...\n", appName);
   }
   else
   {
      printf("%s: flashing image now....\n", appName);
   }
   fflush(stdout);
   ...
   if (rc != 0)
   {
      ...
      cmsLog_error("img_util_close failed");
      exit(1);
   }
   else
   {
      if (isConfigFile)
      {
          // If we wrote a config file, do an immediate reset/reboot.
          bcmuLog_knotice("new configfile, reboot!");
          cmsImg_reboot();
          // cmsImg_reboot will not return, so we will not get past this point.
          // But to be safe, exit anyways.
          exit(0);
      }
      else
      {
         /* After a successful write of the image, we need to set bootstate
          * and log a friendly reboot message.
          */
#ifdef SUPPORT_DM_PURE181
         // CMS TR181 and BDK (which is only TR181) will try to boot new image once.
         // TR181 has more sophisticated data model and code which allows us
         // to try new firmware image once, and commit it if we boot successfully.
         // TODO: but the data model will get out of sync, need to write a hint
         // to scratchpad or something.
         bcmuLog_knotice("Firmwareselect: try booting new image once.");
         setBootImageState(BOOT_SET_NEW_IMAGE_ONCE);
         cmsImg_writeBootFirmwareImageHint("ftpd", (getBootPartition() == 1) ? 2 : 1);
#else
         // BASESHELL, CMS98, and CMS Hybrid98+181 will immediately commit
         // the new image.
         setBootImageState(BOOT_SET_NEW_IMAGE);
#endif
         bcmUtl_loggedBusybox_reboot("ftpd", REBOOT_REASON_SOFTWARE_UPGRADE);
         exit(0);
      }
   }
}
