...
void ovs_brcm_notify_fdb_change(const struct eth_addr *mac)
{
    unsigned char cli[64]  = "";
    unsigned char ACT='D';


    sprintf(cli,"echo \"%c=%02X:%02X:%02X:%02X:%02X:%02X\" > %s"
            ,ACT
            ,mac->ea[0]
            ,mac->ea[1]
            ,mac->ea[2]
            ...
            ,mac->ea[4]
            ,mac->ea[5]
            ,"/proc/fdbsync/operate"
        );
    system(cli);

    sprintf(cli,"fc flush --mac %02X:%02X:%02X:%02X:%02X:%02X > /dev/null"
            ,mac->ea[0]
            ,mac->ea[1]
            ,mac->ea[2]
            ,mac->ea[3]
...
#if defined(CONFIG_BCM_OVS)
        if(mac)
            ovs_brcm_notify_fdb_change(&(mac->mac));
#endif
...
#if defined(CONFIG_BCM_OVS)
    ovs_brcm_notify_fdb_change(&(e->mac));
#endif
