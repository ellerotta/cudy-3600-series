...
static void do_setup(struct net_device *netdev)
{
    ...
    bcm_netdev_ext_field_set(netdev, blog_stats_flags, BLOG_DEV_STAT_FLAG_INCLUDE_ALL);
#endif
}
...
static netdev_tx_t internal_dev_recv(struct sk_buff *skb)
{
...
#if defined(CONFIG_BCM_KF_BLOG) && defined(CONFIG_BLOG)
	{
		...
		if (blog_p) {
			blog_lock();
			blog_link(IF_DEVICE, blog_p, (void*)netdev, DIR_RX, skb->len);
			blog_unlock();

			/* No host client present, complete learning */
			/* host client support for OVS bridge may be added in future */
			if (BLOG_RX_MCAST(blog_p) || BLOG_RX_UNKNOWN_UCAST(blog_p)) {
				blog_inc_known_exception_client_count(skb);
