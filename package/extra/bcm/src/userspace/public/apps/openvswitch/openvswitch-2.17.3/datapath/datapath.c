...
#if defined(CONFIG_BCM_OVS_MCAST)
	upcall_info->bcm_pktmetadata.to_acceldev_ifi = -1;
	if (skbuff_bcm_ext_indev_get(skb)) 
	{
		upcall_info->bcm_pktmetadata.to_acceldev_ifi = 
			skbuff_bcm_ext_indev_get(skb)->ifindex;
	}
#endif
...
static int ovs_flow_cmd_new(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (likely(!flow)) {
		...
		input_vport = ovs_vport_ovsl_rcu(dp, new_flow->key.phy.in_port);
		if (input_vport && is_gretap_dev(input_vport->dev))
			new_flow->is_gretap = true;
#endif
