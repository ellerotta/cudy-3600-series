...
int main(int argc, char **argv)
{
   ...
   {
      stop = 1;
      // If we are stopping (shutting down) the system, make sure the
      // isShutdownInProgress hint file is present.  If not, create it.
      // We will hit this path of user types "reboot" in the busybox shell.
      if (!bcmUtl_isShutdownInProgress())
      {
         bcmUtl_declareShutdownInProgressEx("bcm_boot_launcher/busybox shell/unknown src",
                                            "reboot",
                                            BCM_DECLARE_SHUTDOWN_KNOTICE);
      }
   }
   else
   {
