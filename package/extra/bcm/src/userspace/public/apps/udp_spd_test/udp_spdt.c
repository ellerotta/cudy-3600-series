...
{
    uint64_t rate_M = 0, rate_K = 0, rate_B = 0, bits;

    bits = bytes * 8;
    if (usec)
    {
        rate_M = bits / usec;
        rate_K = ((bits - (rate_M * usec)) * 1000) / usec;
    }

    if (usec/1000)
    {
        rate_B = ((bits * 1000) - (rate_M * usec * 1000 + rate_K * usec)) / (usec / 1000);
    }

    printf("Stats: %s packets %lld, %s bytes %lld, usec %lld, (Goodput) Rate = ", prefix, packets, prefix, bytes, usec);

    if (rate_M / GB > 0)
    {
        uint64_t rate_gb = rate_M / GB;
        uint64_t rate_mb = (rate_M - rate_gb * GB);

        printf("%d.%dGbps\n", (int)rate_gb, (int)rate_mb);
    }
    else if (rate_M > 0)
    {
        printf("%d.%dMbps\n", (int)rate_M, (int)(rate_K));
    }
    else
    {
        printf("%d.%dKbps\n", (int)rate_K, (int)rate_B);
    }
}
    ...
    if (is_connected)
    {
        if (is_running)
        {
            if (is_recv)
            {
                ...
                rc = spdt_stats_get(stream_idx, SPDT_STATS_GET_CURRENT, &stats);
                if (!rc)
                {
...
void parse_args(int argc, char *argv[])
{
    ...
    while ((opt = getopt_long_only(argc, argv, "", long_options, &long_index)) != -1)
    {
        ...
        case 's':
            ...
            break;
        case 'a':
            strncpy(ip_addr, optarg, NI_MAXHOST-1);
            break;
        case 'p':
            port = atoi(optarg); 
            break;
        case 'A':
            strncpy(local_ip_addr, optarg, NI_MAXHOST-1);
            break;
        case 'P':
    ...
    if (!is_recv)
    {
        ...
        if (kbps < 100)
        {
            fprintf(stderr, "Rate lower than 100Kbps is not supported");
            goto exit;
        }
