 ...
 * -- USP related messages                           0x10003200-0x100032FF
 *
...
typedef enum 
{
   ...
   CMS_MSG_DNSPROXY_RELOAD	                    = 0x10000270, /**< OBSOLETE: Sent to dnsproxy to force it reload config file without restart */
   CMS_MSG_TIME_STATE_CHANGED 	                = 0x10000271, /**< TIME state changed */
   CMS_MSG_DNSPROXY_IPV6_CHANGED                = 0x10000272, /**< OBSOLETE: Sent to dnsproxy to inform the DProxy IPv6 DNS server address */
   CMS_MSG_DNSPROXY_GET_STATS	                = 0x10000273, /**< OBSOLETE: Sent to dnsproxy to get DNS query error statistic */
   ...
   CMS_MSG_DNSPROXY_DUMP_STATUS                 = 0x100002A1, /* OBSOLETE: Tell dnsproxy to dump its current status */
   CMS_MSG_DNSPROXY_DUMP_STATS                  = 0x100002A2, /* OBSOLETE: Tell dnsproxy to dump its statistics */
   ...
   CMS_MSG_UDPECHO_DIAG_COMPLETE                = 0x10000799, /**< udpecho diag is complete */
   CMS_MSG_OBUDPST_DIAG_COMPLETE                = 0x1000079A, /**< obudpst diag is complete */
   CMS_MSG_ETHCABLE_DIAG_COMPLETE               = 0x1000079B, /**< ethernet cable diag completes */
   CMS_MSG_SERVERSELECTION_DIAG_COMPLETE        = 0x1000079C, /**< server selection diag completes */
   ...
   CMS_MSG_START_OBUDPST                        = 0x100007AA, /**< event msg to ssk to request start obudpst service, no response */
   CMS_MSG_STOP_OBUDPST                         = 0x100007AB, /**< event msg to ssk to request stop obudpst service, no response */
   CMS_MSG_START_UDPECHO_DIAG                   = 0x100007AC, /**< event msg to ssk to request start udpecho diag, no response */
   CMS_MSG_STOP_UDPECHO_DIAG                    = 0x100007AD, /**< event msg to ssk to request stop udpecho diag, no response */
   CMS_MSG_START_SERVERSELECTION_DIAG           = 0x100007AE, /**< event msg to ssk to request start server selection diag, no response */
   CMS_MSG_STOP_SERVERSELECTION_DIAG            = 0x100007AF, /**< event msg to ssk to request stop server selection diag, no response */
   CMS_MSG_BAS_DM_UPDATE                        = 0x100007B0, /**< event msg to update BAS data model */
   ...
   CMS_MSG_DUMP_MSG_CONNECTIONS                 = 0x10000859,  /**< request smd/bcm_msgd to show all of its connection infos */
   ...
   CMS_MSG_DOXTMLOCK                            = 0x10000868,  /**< set doXtmLock to value in wordData, send to ssk only */
   ...
   CMS_MSG_GET_CONFIG_FILE                      = 0x10000872,  /**< ask ssk for a copy of the config file. */
   ...
   CMS_MSG_VOICE_CONFIG_CHANGED                 = 0x10002005, /**< Voice MDM parameter changed event msg (CmsMsgVoiceConfigChanged)
                                                                 *  Also used any time voice-related MDM item has changed, e.g. event reporting */
   ...
   CMS_MSG_VOICE_STATISTICS_RESET               = 0x1000200A, /**< request to reset Voice call statistics */
   CMS_MSG_VOICE_ANNOUNCE_DOWNLOAD              = 0x1000200B, /**< request to download voice announcement */
   CMS_MSG_VOICE_CODEC_PROFILE_DELETE           = 0x1000200C, /**< Notify that codec profile has changed */
   ...
   CMS_MSG_OMCI_IGMP_ADMISSION_CONTROL          = 0x10002220, /**< Obsolete: do not use anymore. */
   CMS_MSG_OMCI_MLD_ADMISSION_CONTROL           = 0x10002221, /**< Obsolete: do not use anymore. */
   ...
   CMS_MSG_GPON_DMOBJ_CHANGED                   = 0x10002303, /**< GPON data model updated notification. */
   ...
   CMS_MSG_MD_PROXY_GET_PARAM           = 0x10002780,  /**< request to the md process to proxy the getParameterValues requests to athother component */
   ...
   CMS_MSG_USP_RESERVED_BEGIN           = 0x10003200, /**< This range of messages are reserved for use by USP (see cms_msg_usp.h) */
   CMS_MSG_USP_RESERVED_END             = 0x100034FF, /**< End of USP reserved range */
   ...
   CMS_MSG_REMOTE_OBJ_CLEAR_STATISTICS  = 0x1000A11A, /**< request to remote_objd to clear stats on the fullpath (for cmsObj_clearStatistics) */
...
typedef struct cms_msg_header
{
   ...
   CmsEntityId src;   /**< CmsEntityId of the sender; the best way is to set
                       *   this with cmsMsg_getHandleEid(msgHandle). */
   CmsEntityId dst;   /**< CmsEntityId of the receiver; for apps that can have
                       *   multiple instances or threads, this must be a
                       *   specificEid, meaning it contains a threadId and genericEid.
                       *   Usually, this is known from the received msg, e.g. msg->src. */
   union {
...
/** Threshold for number of connection entries which will trigger printing
 *  out a possible memory leak warning.
 */
#define BCM_MSG_CONN_WARNING_THRESHOLD   200
 ...
 *
 * The following ERROR_* defines are used in the ppplastconnecterror field.
 * Since this message is sent by pppd, a GPL app, it is better not to have a
 * dependency on mdm_validstrings.h.
 * Define local versions here, which must match mdm_validstrings.h.
 */

#define CMSVS_ERROR_NONE                      "ERROR_NONE"
#define CMSVS_ERROR_AUTHENTICATION_FAILURE    "ERROR_AUTHENTICATION_FAILURE"
#define CMSVS_ERROR_IDLE_DISCONNECT           "ERROR_IDLE_DISCONNECT"
#define CMSVS_ERROR_USER_DISCONNECT           "ERROR_USER_DISCONNECT"
#define CMSVS_ERROR_ISP_TIME_OUT              "ERROR_ISP_TIME_OUT"
#define CMSVS_ERROR_UNKNOWN                   "ERROR_UNKNOWN"
...
typedef struct
{
   ...
   UINT32 sessionId; /**< session id, if pppState==BCM_PPPOE_CLIENT_STATE_UP */
 ...
 *
 * The following 2 defines are used in addressSource.and interfaceType.
 * Since this message is sent by udhcpd, a GPL app, it is better not to have a
 * dependency on mdm_validstrings.h.
 * Define local versions here, which must match mdm_validstrings.h.
 */
#define CMSVS_DHCP             "DHCP"
#define CMSVS_ETHERNET         "Ethernet"
...
/*
 * The following defines are used in ping.diagnosticsState and
 * tracert.diagnosticsState.  They are sent from bcm_busybox, which
 * should not have a dependency on the MDM (MDMVS_* in mdm_validstrings.h).
 * So define a "CMS" version of a few values, which should match the ones MDM
 * uses since they all come from TR-181.
 */
#define CMSVS_REQUESTED                   "Requested"
#define CMSVS_COMPLETE                    "Complete"
#define CMSVS_PING_INPROGRESS             "Ping_InProgress"
#define CMSVS_ERROR_CANNOTRESOLVEHOSTNAME "Error_CannotResolveHostName"
#define CMSVS_ERROR_MAXHOPCOUNTEXCEEDED   "Error_MaxHopCountExceeded"
...
typedef struct
{
  ...
  char deltaLowerLayers[MAX_MDM_MULTI_FULLPATH_BUFLEN+32];  /**< a specially
...
typedef struct
{
  char ipLowerLayerFullPath[MAX_MDM_SINGLE_FULLPATH_BUFLEN]; /**< The lowerlayer fullpath of the ip interface */
...
typedef struct
{
  ...
  char higherLayerFullPath[MAX_MDM_SINGLE_FULLPATH_BUFLEN]; /**< meaning depends on PROPAGATE_FLAG_XXX */
  char lowerLayerFullPath[MAX_MDM_SINGLE_FULLPATH_BUFLEN];  /**< meaning depends on PROPAGATE_FLAG_XXX */
...
typedef struct
{
   ...
   char   wanType[BUFLEN_16];
   char   wanRate[BUFLEN_16];
...
typedef enum {
   ...
   CMSMSGVOICECONFIGCHANGED_LOW_VOICE_QUALITY, /* Low voice quality (CmsMsgLowVoiceQualityEvent) */
...
typedef struct
{
	...
	char destCompName[BUFLEN_128];
	UINT32 flags;
} CmsMsgMdProxyGetParamRequest;
 ...
 *
 * @param eid       (IN)  generic Entity id of the calling process.
 * @param flags     (IN)  Currently, only EIF_MULTIPLE_INSTANCES or EIF_MULTIPLE_THREADS are detected.
...
#define CMS_INIT_ON_BUS_RETRY_TIMEOUT_MS  200

/** Same as cmsMsg_initOnBus, but with timeout and retry.
 *
 *  If timeoutMs is non-zero, will retry once every 200ms up to the specified timeout.
 *  For example, if timeoutMs is 5000, then initOnBusWithTimeout will retry
 *  up to 5000/200=25 times with a 200ms nanosleep in between tries.
 *  Timeout and retry is useful to allow the bcm_msgd to start up before
 *  another app connects to it.  If timeoutMs is less than 200, then
 *  effectively, there will be only 1 try and no retries.
 */
...
/** Send a message to the bcm_msgd to set the given EID as the "bus manager".
 *
 *  This is only needed when there are distributed MDM components. 
 *  In these environments, sometimes the xyz_md is is the bus manager, and
 *  sometimes the xyz_ssk process is the bus manager.
 */
...
/** Get the eid of the creator of this message handle.  Note the returned
 *  eid is the specificEid.  For single instance apps which only creates
 *  a single msgHandle, the specificEid is the same as the genericEid.  But
 *  for multi-instance apps or multi-threaded apps which create multiple
 *  msg handle's, the specificEid is the thread id plus the generic eid.
 *  See cms_eid.h for details.
 * 
...
/** Get the specificEid of the creator of this message handle.
 *
 *  This is the same as cmsMsg_getHandleEid().
 */
CmsEntityId cmsMsg_getHandleSpecificEid(const void *msgHandle);

/** Get the genericEid of the creator of this message handle.
 */
