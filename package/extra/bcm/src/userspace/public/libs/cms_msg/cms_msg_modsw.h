...
typedef enum 
{
   ...
   CMS_MSG_REQUEST_EU_STATUS                    = 0x10002522, /**< ask ee manager to return the status and statistics of an EU */
   CMS_MSG_BOOTUP_START_BAS                     = 0x10002523, /**< start bas EU at bootup */
   CMS_MSG_BOOTUP_START_BAS_CLIENT              = 0x10002524, /**< start bas client EU at bootup */
   CMS_MSG_DU_MAX_EXCEED_EVENT                  = 0x10002525, /**< exceed DU max number event */
   ...
   CMS_MSG_CONTAINER_OOM_EVENT                  = 0x10002571, /**< container out-of-memory event notification */
   CMS_MSG_EU_STATE_CHANGE_EVENT                = 0x10002572, /**< EU state change event notification */
} CmsModSwMsgType;
...
/** Data body for CMS_MSG_REQUEST_EU_STATUS message type.
 *
 */
typedef struct
{
   char eeBusInfo[MAX_MDM_SINGLE_FULLPATH_BUFLEN];  /**< Execution Environment Bus Info */
   char execEnvLabel[BUFLEN_64];                /**< Execution Environment Label */
} EUstatusRequestMsgBody;


/** Data body for CMS_MSG_REQUEST_EU_STATUS message reply.
 *
 */
typedef struct
{
   char status[BUFLEN_32];          /**< EU status: Idle, Starting, Active, Stopping */
   char execFaultCode[BUFLEN_32];   /**< Execution Fault Code: NoFault, FailureOnStart,
                                         FailureOnAutoStart, FailureOnStop,
                                         FailureWhileActive, DependencyFailure,
                                         UnStartable */
   char execFaultMsg[BUFLEN_256];   /**< Execution Fault Message */
   SINT32 diskSpaceInUse;           /**< Disk space in use by EU */
   SINT32 memoryInUse;              /**< Memory in use by EU */
   SINT32 retcode;                  /**< BcmRet code */
} EUstatusReplyMsgBody;
...
typedef struct
{
   ...
   char fullPath[MAX_MDM_SINGLE_FULLPATH_BUFLEN];   /**< CMS full path of EU */
...
/** Data body for CMS_MSG_CONTAINER_OOM_EVENT message type.
 *
 */
typedef struct
{
   char containerName[BUFLEN_64];
} ContainerOomEventMsgBody;


/** Data body for CMS_MSG_DU_MAX_EXCEED_EVENT message type.
 *
 */
typedef struct
{
   SINT32 duMaxNum;
} DUExceedEventMsgBody;
...
/** Data body for CMS_MSG_EU_STATE_CHANGE_EVENT message type.
 *
 */
typedef struct
{
   char name[BUFLEN_32];    /**< name of EU */
   char reason[BUFLEN_32];  /**< reason of the state change */
   char time[BUFLEN_32];    /**< time of the state change */
} EuStateChangedMsgBody;
...
typedef struct {
   ...
   char pid[BUFLEN_32];
   char cpu_use[BUFLEN_64];
   char mem_use[BUFLEN_64];
   char interface[BUFLEN_32];
   char ipv4_addrs[BUFLEN_256+BUFLEN_4];
   char ports[BUFLEN_64];
   char byte_sent[BUFLEN_64];
   char byte_received[BUFLEN_64];
