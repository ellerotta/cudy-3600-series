...
// If you want to log sparse messages, uncomment out the following line
// #define CC_IMGUTIL_NOTICE 1
#if defined(CC_IMGUTIL_NOTICE)
#define imgutil_notice(fmt, arg...) \
  printf("NOTICE[%s.%u]: " fmt "\n", __FUNCTION__, __LINE__, ##arg)
#else
#define imgutil_notice(fmt, arg...)
...
static UBOOL8 matchChipId(const char *strTagChipId, const char *signature2 __attribute__((unused)))
{
    ...
    {
        imgutil_error("Chip Id error.  Image Chip Id = %04x, Board Chip Id = %04x.", tagChipId, chipId);
        match = FALSE;
#ifdef DESKTOP_LINUX
...
CmsRet bcmImg_verifyBroadcomFileTag(FILE_TAG *pTag, UBOOL8 fullImageB, int imageLen)
{
    ...
    imgutil_debug("start of pTag=%p tagversion %02x %02x %02x %02x", pTag,
                  pTag->tagVersion[0],
                  pTag->tagVersion[1],
                  pTag->tagVersion[2],
                  pTag->tagVersion[3]);
    ...
    if (crc != tokenCrc)
    {
         ...
         */
        imgutil_debug("token crc failed, this is not a valid broadcom image");
        imgutil_debug("calculated crc=0x%x tokenCrc=0x%x", crc, tokenCrc);
        return CMSRET_INVALID_IMAGE;
    }
    imgutil_debug("header CRC is OK.");
    ...
    if (tagVer != curVer)
    {
       imgutil_error("Firmware tag version [%d] is not compatible with the current Tag version [%d]", tagVer, curVer);
       return CMSRET_INVALID_IMAGE;
    }

    imgutil_debug("tarVer=%d, curVar=%d", tagVer, curVer);

    if (!matchChipId((char *) pTag->chipId, (char *) pTag->signiture_2))
    {
       imgutil_error("chipid check failed");
       return CMSRET_INVALID_IMAGE;
    }

    imgutil_debug("chipid is OK");

    if (fullImageB == FALSE)
    {
       imgutil_debug("Good broadcom image header");
       return CMSRET_SUCCESS;
    }

    // check imageCrc
    totalImageSize = atoi((char *) pTag->totalImageLen);
    imgutil_debug("totalImageLen=%d, imageLen=%d, TAG_LEN=%d\n", totalImageSize, imageLen, TAG_LEN);

    if (totalImageSize > (imageLen -TAG_LEN))
    {
        imgutil_error("invalid size\n");
        return CMSRET_INVALID_IMAGE;
    }
    crc = CRC_INITIAL_VALUE;
    ...
    if (crc != imageCrc)
    {
         ...
         * is wrong.
         */
        imgutil_error("image crc failed after broadcom header crc succeeded");
        imgutil_error("calculated crc=0x%x imageCrc=0x%x totalImageSize=%d", crc, imageCrc, totalImageSize);
        return CMSRET_INVALID_IMAGE;
    }
    imgutil_debug("image crc is OK");

    imgutil_debug("Good broadcom image");
...
CmsRet bcmImg_flashImage(char *imagePtr, UINT32 imageLen, CmsImageFormat format, UINT32 opts, int * noReboot)
{
    ...
    imgutil_debug("format=%d opts=0x%x noReboot=0x%x", format, opts, *noReboot);

    if (format != CMS_IMAGE_FORMAT_FLASH && format != CMS_IMAGE_FORMAT_BROADCOM)
    {
       imgutil_error("invalid image format %d", format);
       return CMSRET_INVALID_IMAGE;
    }
    ...
    {
        imgutil_notice("Flash whole image...");
        ...
        if ((ret = getFlashInfo(&flags)) == CMSRET_SUCCESS)
        {
           ...
           { // NOR flash device
              ret = (CmsRet) devCtl_boardIoctl(BOARD_IOCTL_FLASH_WRITE,
                                      BCM_IMAGE_WHOLE,
                                      imagePtr,
                                      imageLen-TOKEN_LEN,
                                      *noReboot, 0);
           ...
           { // NAND flash device (assumes flags & FLASH_INFO_FLAG_NAND)
              ret = writeImageToNand( (unsigned char *)imagePtr, imageLen - TOKEN_LEN );
              // XXX Do we need to check for EMMC and VFBIO?
           }
        }

        // see bcm_hwdefs.h (the reboot bits are above 0xFFFF)
        *noReboot &= 0xFFFF;

        if (ret != CMSRET_SUCCESS)
        {
           imgutil_error("Failed to flash whole image");
    ...
    imgutil_debug("cfeSize=%d kernelSize=%d rootfsSize=%d", cfeSize, kernelSize, rootfsSize);
    ...
    if (cfeAddr)
    {
        ...
        if (ret != CMSRET_SUCCESS)
        {
            imgutil_error("Failed to flash CFE");
            return CMSRET_IMAGE_FLASH_FAILED;
        }
    }
    ...
    if (rootfsAddr && kernelAddr)
    {
        ...
        if (ret != CMSRET_SUCCESS)
        {
            imgutil_error("Failed to flash root file system and kernel");
            return CMSRET_IMAGE_FLASH_FAILED;
        }
    }
    ...
    *noReboot &= 0xFFFF;
    ...
    {
       imgutil_error("Possible error: cfe, kernel, rootfs not flashed");
    }
    else
    {
       imgutil_notice("Image flash done.");
    }
...
CmsRet get_mtd_device_size(const char * partition, unsigned int * size)
{
    ...
    while(fgets(line, MAX_MTD_NAME_SIZE*2, fp))
    {
	    ...
	    unsigned int temp2;
            ...
            temp = strtol(sze, NULL, 16);
            getFlashTotalSize(&temp2);

            if ((temp > 0) && (temp <= ((int)temp2 * 1024)))
            {
            ...
            printf("ERROR in %s!!! Could not get mtd partition %s device size\n", __FUNCTION__, partition);
            *size = 0;
...
unsigned int bcmImg_getImageFlashSize(void)
{
   ...
   {
      ret = (CmsRet) devCtl_boardIoctl(BOARD_IOCTL_FLASH_READ,
                              FLASH_SIZE,
                              0, 0, 0, &flashSize);
   }
   else
   {
      ...
      else if (gFlashInfo == FLASH_INFO_FLAG_VFBIO)
      {
         ...
         if( img_idx != -1 )
         {
                  size = vfbioGetAvailLoaderSpace() + vfbioGetAvailImgSpace(img_idx);
         }
         else
         {
                  uint64_t size1 = vfbioGetAvailImgSpace(1);
                  size = vfbioGetAvailImgSpace(2);
                  if(size1 < size)
                           size = size1;
                  size += vfbioGetAvailLoaderSpace();
                  printf("%s: getUpgradePartition failed, estimated available flash size could be less then actually\n", __FUNCTION__);
         }
         // clip if needed as historically it is 32 bits only, 4GB should be enough any case 
         flashSize = (size > ULONG_MAX) ? ULONG_MAX : size; 
         printf("%s: Flash size %" PRIx64 " (clipped %x)\n", __FUNCTION__, size, flashSize);
#else
         flashSize = 0x1000000;
#endif
         ret = CMSRET_SUCCESS;
   ...
   if (ret != CMSRET_SUCCESS)
   {
      imgutil_error("Could not get flash size, return 0");
      flashSize = 0;
   }
...
unsigned int bcmImg_getRealConfigFlashSize(void)
{
...
#endif
         {
            ret = (CmsRet) devCtl_boardIoctl(BOARD_IOCTL_GET_PSI_SIZE, 0, NULL, 0, 0, (void *)&size);
            if (ret != CMSRET_SUCCESS)
            {
               imgutil_error("boardIoctl to get config flash size failed.");
               size = 0;
            }
         }
      }
   ...
   {
      imgutil_error("could not determine NAND or NOR");
   }

   imgutil_debug("returning size=%d", size);
...
unsigned char bcmImg_willFitInFlash(unsigned int imageSize)
{
   ...
   imgutil_debug("flash size is %u bytes, imageSize=%u bytes", flashSize, imageSize);
#endif /* DESKTOP_LINUX */
...
#endif
            {
                ret = (CmsRet) devCtl_boardIoctl(BOARD_IOCTL_GET_BACKUP_PSI_SIZE, 0, NULL,
                                        0, 0, (void *)&size);
                if (ret == CMSRET_SUCCESS)
                {
                    avail = TRUE;
...
{
    // This is a more graceful reboot, just like typing reboot in busybox shell.
    // If possible, use bcmUtl_loggedBusybox_reboot().
    sync();
   if ( system("reboot") != 0 )
      imgutil_error("Failed to run reboot cmd.");
}
...
CmsImageFormat bcmImg_validateImage(const char *imageBuf, UINT32 imageLen)
{
   ...
   if ((imageLen > sizeof(struct fdt_header)) && (fdt_check_header(imageBuf) == 0))
   {
      ...
      imgutil_debug("Broadcom pktb format verified.");
   ...
   else if ((imageLen > sizeof(FILE_TAG)) && (bcmImg_verifyBroadcomFileTag((FILE_TAG *) imageBuf, 1, imageLen) == CMSRET_SUCCESS))
   {
      ...
      /* Found a valid Broadcom defined TAG record at the beginning of the image */
      imgutil_debug("Broadcom format verified.");
      maxLen = bcmImg_getImageFlashSize() + bcmImg_getBroadcomImageTagSize();
      if (imageLen > maxLen)
      {
         imgutil_error("broadcom image is too large for flash, got %u, max %u", imageLen, maxLen);
      }
      else
      {
         result = CMS_IMAGE_FORMAT_BROADCOM;
   ...
   else
   {
      ...
      if (imageLen > TOKEN_LEN)
      {
...
#endif
         {
            ...
            imgutil_debug("Whole flash image format [%d bytes] verified.", imageLen);
            maxLen = bcmImg_getImageFlashSize();
            if (imageLen > maxLen)
            {
               imgutil_error("whole image is too large for flash, got %u, max %u", imageLen, maxLen);
            }
            else
            {
         ...
         else
         {
            ...
            imgutil_debug("Could not determine image format [%d bytes]", imageLen);
#else
            imgutil_error("Could not determine image format [%d bytes]", imageLen);
#endif
            imgutil_debug("calculated crc=0x%x image crc=0x%x", crc, imageCrc);
         }
      }
   }

   imgutil_debug("returning image format %d", result);
...
UBOOL8 bcmImg_isBcmTaggedImage(const char *imageBuf, UINT32 *imageSize)
{
   ...
   if (crc == tokenCrc)
   {
      ...
      *imageSize = (UINT32) atoi((char *)pTag->totalImageLen) + TAG_LEN;
      imgutil_debug("It's a broadcom tagged image with length %d", *imageSize );
   }
   else
   {
      imgutil_debug("token crc failed, this is not a valid broadcom image");
...
CmsImageFormat bcmImg_parseImgHdr(UINT8 *bufP, UINT32 bufLen)
{
   ...
   {
       imgutil_debug("Detected Broadcom image format.");
       result = CMS_IMAGE_FORMAT_BROADCOM;
   }
...
UINT8* bcmImg_allocImgBuf(UINT32 maxBufSize, UINT32 *allocBufSizeP)
{
    ...
    imgutil_debug("bcmImg_allocImgBuf() begin, %d", allocSize);
    ...
    if ((bufP = (UINT8*)malloc(allocSize)) == NULL)
    {
        ...
        /* if (maxBufSize > 0) */
        {
            ...
            {
               allocSize -= 64 * 1024;
               imgutil_debug("Try allocating %d kb", allocSize/1024);
            }
        }

        if (bufP == NULL)
        {
            imgutil_error("Not enough memory to alloc %u bytes.", allocSize);
            *allocBufSizeP = 0;
            return NULL;
        }
    }

    imgutil_debug("bcmImg_allocImgBuf() end, %d", allocSize);
