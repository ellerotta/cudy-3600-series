...
// If you want to log full debug messages, uncomment out the following line
                                ...
                                SINT32 offset, void *data)
{
    ...
    if( action == SCRATCH_PAD )
    {
        ...
        case BOARD_IOCTL_FLASH_WRITE:
            ...
            break;
        default:
            flashutil_error("unhandled BOARD_IOCTL code %d for scratch pad", boardIoctl);
    ...
    else
    {
        ...
        default:
            flashutil_error("unhandled action %d in BOARD_IOCTL code %d",
                            action, boardIoctl);
        ...
        {
            flashutil_debug("reading from fname %s", fname);
            ret = devCtl_flashReadFile(fname, string, strLen);
        }
        else if( boardIoctl == BOARD_IOCTL_FLASH_WRITE )
        {
            flashutil_debug("writing to fname %s (len=%d)", fname, strLen);
            ret = devCtl_flashWriteFile(fname, string, strLen);
            sync();
        }
        else
        {
            flashutil_error("unhandled BOARD_IOCTL code %d", boardIoctl);
            ret = BCMRET_INVALID_ARGUMENTS;
        }
    }
                           ...
                           const char *compName, char *fname, unsigned int len)
{
    ...
    flashutil_debug("%s does not exist, fallback", fname);
    if (action == PERSISTENT)
       snprintf(fname, len, "%s", PSI_FILE_NAME);
    else
       snprintf(fname, len, "%s", PSI_BACKUP_FILE_NAME);
...
int readNvramData(void *nvramData)
{
    ...
    switch( gFlashInfo )
    {
        ...
        case FLASH_INFO_FLAG_VFBIO:
            ret = vfbioReadNvramData(nvramData);
        break;
#endif
...
#ifdef BCM_FLASH_VFBIO
        else if( vfbioIsBootDevice() )
        {
            *flags = FLASH_INFO_FLAG_VFBIO;
        }
#endif
...
#ifdef BCM_FLASH_VFBIO
        case FLASH_INFO_FLAG_VFBIO:
            ret = vfbioGetFlashSize(&value);
        break;
#endif
        case FLASH_INFO_FLAG_NOR:
            value = spinorGetAvailSpace(SPI_NOR_MTD) / 1024;
        break;
        default:
...
int devCtl_getSequenceNumber(int image)
{
    ...
    getFlashInfo(&flag);
    if (flag & (FLASH_INFO_FLAG_NAND | FLASH_INFO_FLAG_EMMC | FLASH_INFO_FLAG_VFBIO))
...
#ifdef BCM_FLASH_VFBIO
        case FLASH_INFO_FLAG_VFBIO:
            seqNumber = vfbioGetSequenceNumber(imageNumber);
        break;
#endif
...
#ifdef DUMP_VER_STR
    printf("%s: Image version tag[%d]:%s\n", __FUNCTION__, status, tag);
#endif

    if (status > keyLen)
    {
        for (i = keyLen; i<(BUFLEN_256) && i<status; i++)
        {
...
int flashDevNodeExists( char * dev_path __attribute__((unused)), int exists __attribute__((unused)))
{
    return 1;
}
...
#ifdef BCM_FLASH_VFBIO
        case FLASH_INFO_FLAG_VFBIO:
            return(vfbioGetAvailImgSpace(update_img_idx));
        break;
#endif
        case FLASH_INFO_FLAG_NOR:
            if( !isLegacyFlashLayout())
...
#ifdef BCM_FLASH_VFBIO
        case FLASH_INFO_FLAG_VFBIO:
            return(vfbioGetAvailLoaderSpace());
        break;
#endif
        case FLASH_INFO_FLAG_NOR:
...
int synchLoaderEnv( char * loader_fname )
{
    ...
    FILE* file_ptr = NULL;
    uint64_t env_size, raw_env_size;
    int ret = -1;
    ...
    /* 1 - Dump current raw environment to temp file */
    if( system("cat /proc/environment/raw > /var/current_env.tmp"))
    {
        printf("%s: Error, Could not read from %s\n", __FUNCTION__, PROC_ENV_RAW);
        ret = -1;
        goto exit_loader_synch;
    }

    /* 2 - Open and read the temp environment into memory */
    file_ptr = fopen("/var/current_env.tmp", "r");
    if( !file_ptr )
    {
        printf("%s: Error, cannot open %s\n", __FUNCTION__, "/var/current_env.tmp");
        ret = -1;
        goto exit_loader_synch;
    }

    /* 3 - Get size of raw environment */
    if( fstat(fileno(file_ptr), &stbuf) )
    {
        printf("%s: Error, failed to fstat %s\n", __FUNCTION__, "/var/current_env.tmp");
        ret = -1;
        goto exit_loader_synch;
    }

    raw_env_size = stbuf.st_size;
    env_size = raw_env_size - sizeof(UBOOT_ENV_HDR);

    /* 4 - Allocate mem for raw environment */
    in_mem_env = malloc( raw_env_size+1 );
    if( !in_mem_env )
    {
    ...
    /* 5 - Read in raw environment */
    bytes = fread(in_mem_env, 1, raw_env_size , file_ptr);
    if( !bytes )
    {
    ...
    {
        size_t rsz;
        rsz = fread((char*)&raw_env_size, 4, 1, file_ptr);
	/* Check if correct no. of elements were read */
        if (rsz != 1)
        {
            printf("%s: bad read value %lld\n", __FUNCTION__, (long long int)rsz);
            ret = -1;
            goto exit_loader_synch;
        }
        if ( fseek( file_ptr, -8, SEEK_CUR ) )
            break;

        if( env_magic == UBOOT_ENV_MAGIC )
        {

            if(raw_env_size > env_hdr->size)
            {
                printf("Image uboot env %lld mismatch with current uboot env size %d\n", (long long int)raw_env_size, env_hdr->size);
            }
            else if(raw_env_size < env_hdr->size)
            {
                printf("Image uboot env %lld smaller than current uboot env size %d\n", (long long int)raw_env_size, env_hdr->size);
                printf("not writing uboot env");
                if ( fseek( file_ptr, BOOT_MAGIC_OFFS, SEEK_CUR ) )
                    break;
                continue;
            }
            printf("Found MAGIC at 0x%08x\n", (unsigned int)ftell(file_ptr));
            ret = fwrite( in_mem_env, 1, env_size+sizeof(UBOOT_ENV_HDR), file_ptr);
            if( !ret )
            {
            ...
            if ( fseek( file_ptr, BOOT_MAGIC_OFFS - ftell(file_ptr)%BOOT_MAGIC_OFFS, SEEK_CUR ) )
                break;
...
#ifdef BCM_FLASH_VFBIO
        case FLASH_INFO_FLAG_VFBIO:
            ret = vfbioGetImageVersion(imagePtr, imageSize, image_name, image_name_len);
        break;
#endif
...
int bcmFlash_getIdent(int part, int *start, int *end, const char *key, char *line, int len)
{
    ...
    else if (gFlashInfo == FLASH_INFO_FLAG_EMMC || gFlashInfo == FLASH_INFO_FLAG_VFBIO)
    {
    ...
    if (gFlashInfo == FLASH_INFO_FLAG_NAND)
    {
        ...
        else
        {
            if ( (part == 1) || ((part == 3) && (boot == 1)) || ((part == 4) && (boot == 2)) )
                ret = get_ubi_device_name("bootfs1", name);
            else
                ret = get_ubi_device_name("bootfs2", name);
...
#ifdef BCM_FLASH_VFBIO
    else if (gFlashInfo == FLASH_INFO_FLAG_VFBIO)
    {
        if ( (part == 1) || ((part == 3) && (boot == 1)) || ((part == 4) && (boot == 2)) )
            snprintf(name, sizeof(name) - 1, VFBIO_DEV_PNAME_FMT_BOOTFS, 1);
        else
            snprintf(name, sizeof(name) - 1, VFBIO_DEV_PNAME_FMT_BOOTFS, 2);
    }
#endif
    ...
    if ((blksize == 0) || ((buf = malloc(blksize)) == 0))
    {
    ...
    if (*start >= 0)
    {
        if ( fseek(fp, blksize * *start, SEEK_SET) )
        {
    ...
    ret = 0;
    /* coverity[string_null_argument] the buffer is exactly the block read size, we are analyzing the block a character at a time up to the block length, does not need to be null terminated */
...
static int validate_metadata(MDATA *mdatap, int * committed, int *valid, int *seq)
{
    ...
    int seq_img_idx[2] = {-1};
    int i;
#ifndef BCM_FLASH_VFBIO
    uint32_t crc;
    char *commitp;
    char *validp;
    char *seqp;

    if (mdatap->size > PKGTB_METADATA_RDWR_SIZE)
    {
        flashutil_error("metadata size %u is insane", mdatap->size);
        goto ERR_RET;
    }
    ...
    {
        flashutil_error("metadata crc failed! exp: 0x%08x  calc: 0x%08x", mdatap->mdata_obj.crc, crc);
        goto ERR_RET;
    }

    /* Metadata seems good */
    commitp = mdatap->mdata_obj.data;
    validp = commitp + strlen(commitp) + 1;
    seqp = validp  + strlen(validp)  + 1;
    ...
    if(!(fd = open("/proc/nvram/COMMITTED", O_RDONLY, 0)) || (read(fd, value, sizeof(value)) <= 0))
    {
        flashutil_error("failed to read COMMITTED value");
        goto ERR_RET;
    }
    close(fd);
    sscanf(value, "%d", &committed_idx);
    memset(value, 0, sizeof(value));
    if(!(fd = open("/proc/nvram/VALID", O_RDONLY, 0)) || (read(fd, value, sizeof(value)) <= 0))
    {
        flashutil_error("failed to read VALID value");
        goto ERR_RET;
    }
    close(fd);
    sscanf(value, "%d,%d", &valid_img_idx[0], &valid_img_idx[1]);
    memset(value, 0, sizeof(value));
    if(!(fd = open("/proc/nvram/SEQ", O_RDONLY, 0)) || (read(fd, value, sizeof(value)) <= 0))
    {
        flashutil_error("failed to read SEQ value");
        goto ERR_RET;
    }
    close(fd);
    sscanf(value, "%d,%d", &seq_img_idx[0], &seq_img_idx[1]);
#endif
    if ((valid_img_idx[0] < 0) || (valid_img_idx[0] > 2) || (valid_img_idx[1] < 0) || (valid_img_idx[1] > 2))
    {
        flashutil_error("bad valid image index [1]=%u [2]=%u", valid_img_idx[0], valid_img_idx[1]);
        goto ERR_RET;
    }
    //FIXME: Magic numbers
    for( i=0; i<2; i++ )
...
static int set_metadata_val( int * committed, int * valid, int * seq )
{
    ...
    sprintf(value, "%d,%d", valid[0], valid[1]);
    if(!(fd = open("/proc/nvram/VALID", O_WRONLY, 0)) || (write(fd, value, strlen(value)) <= 0))
    {
        flashutil_error("failed to write VALID value %d", fd);
        close(fd);
        return -1;
    }
    close(fd);
    sprintf(value, "%d,%d", seq[0], seq[1]);
    if(!(fd = open("/proc/nvram/SEQ", O_WRONLY, 0)) || (write(fd, value, strlen(value)) <= 0))
    {
        flashutil_error("failed to write SEQ value %d", fd);
        close(fd);
        return -1;
    }
    close(fd);
    sprintf(value, "%d",*committed);
    if(!(fd = open("/proc/nvram/COMMITTED", O_WRONLY, 0)) || (write(fd, value, strlen(value)) <= 0))
    {
        flashutil_error("failed to write COMMITTED value %d", fd);
        close(fd);
        return -1;
    }
    close(fd);
...
static int get_metadata_val( int * committed, int * valid, int * seq)
{
    ...
    for( mdata_idx=1; (mdata_idx<=2) && (ret < 0); mdata_idx++ )
    {
        ...
        if( num_bytes_read >=  PKGTB_METADATA_SIZE )
            ...
            fprintf(stderr, "ERROR!!! unable to retrieve metadata%d!\n", mdata_idx);
    }
    free(mdatap);
...
#else
    ret = validate_metadata(NULL, committed, valid, seq);
#endif
...
#ifdef BCM_FLASH_VFBIO
            case FLASH_INFO_FLAG_VFBIO:
                ret = vfbioCommit(partition, string);
            break;
#endif
...
int devCtl_setImageState(int state)
{
   ...
   getFlashInfo(&flag);
   if (flag & (FLASH_INFO_FLAG_NAND | FLASH_INFO_FLAG_EMMC | FLASH_INFO_FLAG_VFBIO)) // NAND/EMMC/VFBIO flash, no need to dig into kernel space
...
int setBootReason( int reason )
{
    ...
    //profile 7 bug workaround, write profile 7 boot reason in case if we are downgrading to < 5.04.03
    snprintf(command, sizeof command, "echo 0x%x > /proc/bootstate/old_reset_reason 2> /dev/null  ", reason);
    system(command);
#endif
...
#define DEV_NODE_CHK_INTRVL_USEC     500000       
int flashDevNodeExists( char * dev_path, int exists)
{
    ...
    if( dev_node_wait_cnt <= 0 )
	    printf("%s: %s %s_wait_usec_left:%d\n", __FUNCTION__, dev_path, (exists?"exist":"notexist"),
        	                         dev_node_wait_cnt);
