    ...
    char* dev_name_prefix, int seg_index, imgif_ctx_t *ctxP )
{
    ...
    {
        printf("%s: Error: GPT partition %s does not exist!\n", __FUNCTION__, cmd);
        ret = -1;
    }
    ...
    char *dev_name_prefix, int seg_index, imgif_ctx_t *ctxP )
{
    ...
    {
        int old_lun_id = -1;
        uint32_t lun_flags = 0;

        if (vfbio_lun_get_id(lun_name, &old_lun_id) == VFBIO_ERROR_OK)
        {
            struct vfbio_lun_descr old_descr;
            ret = vfbio_lun_get_info(old_lun_id, &old_descr);
            if (!ret)
            {
                if (old_descr.read_only)
                    lun_flags |= VFBIO_LUN_CREATE_FLAG_READ_ONLY;
                if (old_descr.encrypted)
                    lun_flags |= VFBIO_LUN_CREATE_FLAG_ENCRYPTED;
            }
        ...
        ret = vfbio_lun_create(lun_name, ctxP->pkgtb_img_segments[seg_index].bytesdone, lun_flags, &lun_id);
        if (ret || !flashDevNodeExists(dev_name, 1))
        {
...
static FILE * init_storage_dev( const char * dev_name_prefix, int seg_index, imgif_ctx_t *ctxP)
{
    ...
    if( ret == 0 )
    {
        switch( flashInfo.flashType )
        {
            ...
            break;
            case FLASH_INFO_FLAG_EMMC:
                sprintf(cmd, "/dev/%s%d", dev_name_prefix, ctxP->update_img_idx);
                if( flashDevNodeExists(cmd, 1) )
                {
...
int check_pkgtb_compat_string( char* compat_str )
{
    ...
    comp_str_buf = malloc(strlen(compat_str)+1);
    if( comp_str_buf )
    {
        memset(comp_str_buf,'\0',strlen(compat_str)+1);
        strncpy(comp_str_buf, compat_str, strlen(compat_str));
...
/*****************************************************************************
*  FUNCTION:  check_image_partitions
*  PURPOSE:   Check if image partitions were created correctly
*  PARAMETERS:
*      ctxP (IN) - IMGIF context pointer.
*      flashtype (IN) - flashtype
*  RETURNS:
*      -1 - failed operation.
*  NOTES:
*      None.
*****************************************************************************/
static int check_image_partitions(UINT32 flashType, imgif_ctx_t *ctxP)
{
    char cmd[128];
    int ret= -1;
    if( ctxP )
    {
        switch( flashType )
        {
            ...
            {
                /* Check rootfs */
                int vol_id = (ctxP->update_img_idx == PKGTB_IMG1_PART_NUM?    
                    PKGTB_NAND_ROOTFS1_VOLID:PKGTB_NAND_ROOTFS2_VOLID);   
                sprintf(cmd, "/dev/ubi0_%d", vol_id) ;
                if( flashDevNodeExists(cmd, 1) )
                {
            ...
            {
                /* Check rootfs */
                sprintf(cmd, "/dev/rootfs%d", ctxP->update_img_idx) ;
                if( flashDevNodeExists(cmd, 1) )
                {
                    /* Check bootfs */
                    sprintf(cmd, "/dev/bootfs%d", ctxP->update_img_idx) ;
                    if( flashDevNodeExists(cmd, 1) )
                        ret = 0;
                }
            }
    ...
    if( ret ) 
    {
        printf("%s: Error: %s %s for image %d does not exist!\n", __FUNCTION__, 
                ((flashType==FLASH_INFO_FLAG_NAND)?"UBI Volume":"GPT partition"),
                   cmd, ctxP->update_img_idx);
    }
    else
    {
        printf("%s: All %ss for image %d are present\n", __FUNCTION__, 
                ((flashType==FLASH_INFO_FLAG_NAND)?"UBI Volume":"GPT partition"),
                   ctxP->update_img_idx);
    }
...
static int finalize_pkgtb_img_write(imgif_ctx_t *ctxP, unsigned char abortFlag)
{
        ...
        for (i=0; i < MAX_FORKED_WRITES; i++)
        {
            if (forked_writes_array[i] != 0)
            {
                ...
                if (child_pid > 0)
                {
                    ...
                    {
                        printf("%s: Child with PID %ld exited with status 0x%x, forked_flash_writes=%d\n",__FUNCTION__, 
                            (long)child_pid, WEXITSTATUS(child_ret), forked_flash_writes-1);
                    }
                    else if (WIFSIGNALED(child_ret)) {
                        printf("%s: Child with PID %ld exited via signal %d, forked_flash_writes=%d\n",__FUNCTION__, 
                            (long)child_pid, WTERMSIG(child_ret),forked_flash_writes-1);
                    }
                }
                else
                {
                    ...
                    write_fails++;
                    printf("%s: Unexpected error, forked_flash_writes=%d\n", __FUNCTION__, forked_flash_writes-1);
                }
    ...
    /* Ensure that all image partitions are present complete before flashing any final delay_commit partitions */
    ret = check_image_partitions(flashInfo.flashType, ctxP);
    if( ret )
    {
        printf("%s: ERROR: Invalid flash image partitions detected after upgrade!\n", __FUNCTION__);
        goto cleanup;
    }
