...
CmsRet cmsUtl_macStrToNum(const char *macStr, UINT8 *macNum) 
{
   ...
   macStrLen = strnlen(macStr, MAC_STR_LEN+1);
...
char** cmsUtl_strsplit(const char *a_str, const char *delim, size_t *size)
{
    char **result = NULL;
    char *buffer;
    size_t count = 0;
    ...
    if (a_str == NULL || delim == NULL || size == NULL)
    {
    ...
    *size = 0;
    ...
    /* If last_comma is not at the end of a_str, we have a trailing substring. */
    if (last_comma < (buffer + strlen(buffer) - 1))
    {
        count++;
    }
    ...
    result = cmsMem_alloc(sizeof(char*) * count, ALLOC_ZEROIZE);
    ...
    if (result)
    {
        ...
        while (token)
        {
            *(result + idx++) = cmsMem_strdup(token);
...
CmsRet cmsUtl_parseUrl(const char *url, UrlProto *proto, char **addr, UINT16 *port, char **path)
{
   size_t n = 0;
   char *p = NULL;
   char protocol[BUFLEN_16] = {0};
   char host[BUFLEN_1024]   = {0};
   ...
   if ((proto == NULL) || (addr == NULL) || (port == NULL) || (path == NULL))
   {
      cmsLog_error("one or more input args is NULL %p/%p/%p/%p",
  ...
  *port = 0;
   ...
   /* host */
   {
      ...
      while (*p && (*p != endChar1) && (*p != endChar2) && (++count < sizeof(host)))
      {
         *pHost++ = *p++;
      }
      *pHost = '\0';
   ...
   if (*p == ':') 
   {
      ...
      p++;
      while (isdigit(*p) && (++count < sizeof(buf)))
      {
         *pBuf++ = *p++;
      }
      *pBuf = '\0';
   ...
   {
      size_t count = 0;
      char uri[BUFLEN_1024] = {0};
      char *pUri = uri;

      while ((*pUri++ = *p++) && (++count < sizeof(uri)));
...
{
   return ((CmsRet) bcmUtl_getBaseDir(pathBuf, pathBufLen));
}
...
char *cmsUtl_strcasestr(const char *s1, const char *s2) 
{
   ...
   if (str1 == NULL)
   {
      // not safe to use emptyStr as strcasestr()'s 1st parameter
      return &emptyStr;
   }
   if (str2 == NULL)
   {
...
void cmsUtl_generateJabberId(const char *username,
                             const char *domain,
                             const char *resource,
                             char *jabberID)
{
   ...
   sprintf(jabberID, "%s", username);
   if (domain != NULL && domain[0] != '\0')
   {
      strcat(jabberID, "@");
      strcat(jabberID, domain);
   }
   if (resource != NULL && resource[0] != '\0')
   {
      strcat(jabberID, "/");
      strcat(jabberID, resource);
   }
   return;
}
...
int cmsUtl_BITSSetBit(char *pszBITS, int bit)
{
    if (!pszBITS)
    {
        return -1;
    ...
    /* Check max bit - example "ABCDEF" = 24 bits represented,
     * so the largest "bit" is 23.
     */
    int maxBit = (strlen(pszBITS) * 4);
    if (0 == maxBit || bit >= maxBit)
    {
        return -1;
    }

    /* bit 0-3 is first character, 4-7 is second character, etc... */
    int index = (bit / 4);
    int offset = (bit % 4);
    unsigned char byte = 0;
    byte = (unsigned char)cmsUtl_byte2nibble(pszBITS[index]);

    unsigned char mask = (0x80 >> (offset+4));

    byte |= mask;

    pszBITS[index] = cmsUtl_nibble2byte(byte);

    return 0;
}

int cmsUtl_BITSClearBit(char *pszBITS, int bit)
{
    if (!pszBITS)
    {
        return -1;
...
int cmsUtl_BITSGetBit(char *pszBITS, int bit)
{
    if (!pszBITS)
    {
        return -1;
...
UBOOL8 cmsUtl_isValidDecimal(const char *decimal)
{
   int i, numi = 0, len, dotNum = 0;

   if (decimal == NULL ||
      (len = strlen(decimal)) == 0)
   {
      return FALSE;
   }
