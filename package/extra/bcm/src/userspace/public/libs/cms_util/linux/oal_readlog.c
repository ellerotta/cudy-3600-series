...
#define LOCAL_FILE      "/var/log/messages"
#define LOCAL_FILE_COPY "/var/log/messages.copy"    
...
static int oal_readLogFromFile(int ptr, char* buffer)
{
   char cmd[64];
   int index = -1;

   if (BCM_SYSLOG_FIRST_READ == ptr)
   {
      if (-1 == access(LOCAL_FILE, F_OK))
      {
         cmsLog_debug("Syslog disabled or  local file disabled\n");
         return BCM_SYSLOG_READ_BUFFER_ERROR;
      }

      sprintf(cmd, "cp -f %s %s", LOCAL_FILE, LOCAL_FILE_COPY);
      if (0 != system(cmd))
      { 
         cmsLog_error("Execute cmd :%s failed\n", cmd);
         return BCM_SYSLOG_READ_BUFFER_ERROR;
      }

      log_file_fd = fopen(LOCAL_FILE_COPY, "r");

      if(NULL == log_file_fd)
      {
         cmsLog_error("Open the copy of local file failed \n");
         return BCM_SYSLOG_READ_BUFFER_ERROR;
      }
   ...
   if(NULL != fgets(buffer, BCM_SYSLOG_MAX_LINE_SIZE, log_file_fd))
   {   
      index++;
   }
   else
   {
      index = BCM_SYSLOG_READ_BUFFER_END;
      buffer[0] = '\0';
      fclose(log_file_fd);
      log_file_fd = NULL;
      sprintf(cmd, "rm -rf %s", LOCAL_FILE_COPY);
      if (0 != system(cmd))
          cmsLog_debug("Execute cmd:%s failed\n",cmd);
   }
...
int oal_readLogPartial(int ptr, char* buffer)
{
  ...
  if ( (log_shmid = shmget(KEY_ID, 0, 0)) == -1) {
    cmsLog_debug("Syslog disabled or log buffer not allocated,try to open local file\n");
    i = oal_readLogFromFile(ptr, buffer);
    goto output_end;
  }
