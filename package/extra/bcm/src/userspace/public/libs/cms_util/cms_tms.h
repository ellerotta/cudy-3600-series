...
/** Add the specified delta tms to the given timestamp.
 *
 * @param tms    (IN/OUT) The timestamp to operate on.
 * @param delta  (IN)     The delta timestamp to be added to tms.
 */
void cmsTms_add(CmsTimestamp *tms, const CmsTimestamp *delta);
 ...
 *  Note the DateTime is in the device's timezone.  See also bcmTms_getXSIDateTimeGmt()
 *  for the same function but in GMT.
 *
 ...
 *  Note the DateTime is in the device's timezone.
 *
 * @param t      (IN) Number of seconds since Jan 1 1970.  If 0, then this
