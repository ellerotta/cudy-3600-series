...
{
   return ((1 == sysUtl_isInterfaceLinkUp(ifname)) ? TRUE : FALSE);
}
...
void cmsNet_inet_cidrton(const char *cp, struct in_addr *ipAddr, struct in_addr *ipMask)
{
   ...
   strncpy(addrStr, cp, sizeof(addrStr)-1);
...
{
   return (sysUtl_getIfindexByIfname(ifname));
}


int cmsNet_getIfnameByIndex(int index, char *intfName)
{
   return (sysUtl_getIfnameByIndex(index, intfName));
}
...
CmsRet cmsNet_getEthLinkStatus(const char *ifname, int port, int *speed, UBOOL8 *fullDuplex, UBOOL8 *linkUp)
{
   ...
   if (emac_ports > 1)
   {
      cmsLog_debug("interface %s is Ethernet Switch, use port argument", ifr.ifr_name);
