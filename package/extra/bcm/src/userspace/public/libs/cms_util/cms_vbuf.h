...
typedef struct {
    void *data;
    UINT32 maxSize;  // size of data buffer
    UINT32 size;     // how much data has been written into data so far.
    UINT32 index;    // how much data has been read from data so far.
} CmsVbuf;
...
/** Get data from arbitrary offset in the vbuf.  Does not advance read index.
 */
