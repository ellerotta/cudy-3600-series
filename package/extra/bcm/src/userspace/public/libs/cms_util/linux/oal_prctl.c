...
/* maximum amount of time to sleep between collect process attempt if timeout was specified. */
#define MAX_COLLECT_WAIT_INTERVAL_MS 16
...
CmsRet oal_spawnProcess(const SpawnProcessInfo *spawnInfo, SpawnedProcessInfo *procInfo)
{
   ...
   if (pid == 0)
   {
      ...
      // If caller used absolute path (starts with /) then just use execv.
      // Otherwise, use execvp to find the exe based on the PATH env var.
#ifdef SUPPORT_NETAPP_RESTRICT
      if(!SecAppLoadExe(spawnInfo->exe, spawnInfo->args))
#else
      if (spawnInfo->exe)
#endif
      {
   ...
   if (spawnInfo->cpuMask)
  {
   ...
   if (spawnInfo->spawnMode == SPAWN_AND_WAIT)
   {
      ...
      if (ret == CMSRET_TIMED_OUT)
      {
         ...
         collectInfo.collectMode = COLLECT_PID_TIMEOUT;
         collectInfo.pid = pid;
         collectInfo.timeout = MAX_COLLECT_WAIT_INTERVAL_MS;
...
CmsRet oal_collectProcess(const CollectProcessInfo *collectInfo, SpawnedProcessInfo *procInfo)
{
   ...
   UINT32 sleepTime;
   UINT32 waitIntervalMS = 1; // it could be variable, starting from 1ms
    ...
    * WNOHANG and then sleeping for a short amount of time between checks,
    * and the time might be variable, i.e., 1->2->4->8ms to the maximum cap at 16ms.
    * The loop is a bit tricky because it has to handle callers
   ...
   while (timeoutRemaining > 0)
   {
      ...
      if (rc == 0)
      {
         ...
         {
            sleepTime = (timeoutRemaining > waitIntervalMS) ?
                         waitIntervalMS : timeoutRemaining - 1;
            usleep(sleepTime * USECS_IN_MSEC);
            timeoutRemaining -= sleepTime;

	    // to multiply waitIntervalMS by 2 as the next interval time, but it should not
            // larger than MAX_COLLECT_WAIT_INTERVAL_MS
            waitIntervalMS = (waitIntervalMS >= MAX_COLLECT_WAIT_INTERVAL_MS) ?
                              MAX_COLLECT_WAIT_INTERVAL_MS : (1 << waitIntervalMS);
         }
         else
         {
            timeoutRemaining = 0;
...
CmsRet parseArgs(const char *cmd, const char *args, char ***argv)
{
   ...
   {
      cmsLog_error("malloc of %d failed", (int) strlen(cmdStr) + 1);
      freeArgs(array);
      return CMSRET_RESOURCE_EXCEEDED;
   }
   else
