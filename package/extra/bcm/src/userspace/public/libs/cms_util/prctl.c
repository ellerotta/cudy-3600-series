...
CmsRet prctl_launchApp(const char *prefix, const char *exe, const char *args, int *pid)
{
   ...
   cmsLog_notice("prefix=%s exe=%s args=%s", prefix, exe, args);
   ...
   {
      size_t exeStrlen = strlen(exeBuf);
      snprintf(&(exeBuf[exeStrlen]), sizeof(exeBuf)-exeStrlen, "%s/%s", prefix, exe);
   }
#else
   snprintf(exeBuf, sizeof(exeBuf), "%s/%s", prefix, exe);
#endif

   cmsLog_notice("final exeBuf=%s args=%s", exeBuf, args);
   ...
   if (ret != CMSRET_SUCCESS)
   {
      cmsLog_error("could not spawn child %s args %s", exeBuf, args);
   }
   else
   {
      char argsBuf[1024] = {0};

      /* calling "rtpolicy" to config apps individually */
      snprintf(argsBuf, sizeof(argsBuf), "rtpolicy auto proc %s >> /var/rtpolicy.log 2>&1", exe);
      prctl_runCommandInShellBlocking(argsBuf);
...
void prctl_initDaemon()
{
   int i;

   /*
    * man 7 daemon has a lot of recommended actions, but this should be good
    * enough.
    */
...
int prctl_runCommandInShellWithTimeout(char *command)
{
   ...
   if (ret != CMSRET_SUCCESS)
   {
      cmsLog_error("%s (pid %d) did not exit in timeout (%d) secs, ret=%d, kill it with SIGKILL",
                   command, collectInfo.pid, collectInfo.timeout/MSECS_IN_SEC, ret);
      // We expect the process to exit in timeout secs, if not, something is
      // wrong, so kill it with SIGKILL (instead of SIGTERM) to make sure it
      // gets killed.
      prctl_signalProcess(collectInfo.pid, SIGKILL);
      collectInfo.timeout = 20;  // should be able to collect in 20ms
      if ((ret = prctl_collectProcess(&collectInfo, &procInfo)) != CMSRET_SUCCESS)
      {
         cmsLog_error("Still could not collect pid=%d after SIGKILL, ret=%d",
                      collectInfo.pid, ret);
