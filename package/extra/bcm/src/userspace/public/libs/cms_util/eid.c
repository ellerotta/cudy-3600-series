...
#define VERIFY_GENERIC_EID(e) \
   if (THREAD_ID_IN_EID(e) != 0) { \
      cmsLog_error("A generic EID should be passed in, got 0x%x", e); \
      return NULL; }
...
const CmsEntityInfo *cmsEid_getEntityInfoAutoCreate(CmsEntityId eid)
{
   ...
   if (eInfo == NULL)
   {
      ...
      /*
       * Could not find an existing eInfo, so dynamically create a "blank" one.
       * This allows apps which do not need to be launched by smd, and do not
       * create a pre-defined eid info entry to still connect to smd.
       */
      ...
      else
      {
         ...
         newInfo->eid = eid;
         newInfo->flags |= EIF_NEW_AUTO_CREATE;
