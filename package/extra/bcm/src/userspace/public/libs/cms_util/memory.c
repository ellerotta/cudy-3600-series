...
static UBOOL8 leakTracingInitialized=FALSE;
...
typedef struct alloc_record {
   ...
   UINT32 seq;  // just a number to differentiate between two allocations of the same buffer addr and size 
   time_t t;
   void *stackAddr[NUM_STACK_ENTRIES];
} AllocRecord;

// This global var needs to be protected against multi-threaded access.
DLIST_HEAD(glbAllocRec);

// pthread mutex for protecting glbAllocRec.  Note that if an app does not
// link against libpthread, all pthread operations become no-op.  However,
// in newer versions of libc and musl, pthread is actually built into the
// base libc.
static pthread_mutex_t leakTracing_mutex = PTHREAD_MUTEX_INITIALIZER;

int cmsMem_leakTracing_lock(void)
{
   int rc;

   rc = pthread_mutex_lock(&leakTracing_mutex);
   return rc;
}

void cmsMem_leakTracing_unlock(void)
{
   pthread_mutex_unlock(&leakTracing_mutex);
   return;
}

static void initLeakTracing(void)
{
   CmsTimestamp tms;

   if (leakTracingInitialized)
   {
      return;
   }
   leakTracingInitialized = TRUE;

   // Initialize the allocSeq
   cmsTms_get(&tms);
   srand(tms.nsec);
   allocSeq = rand() << 16;
...
void cmsMem_initSharedMem(SINT32 keyOffset, void *addr, UINT32 len)
{
...
#ifdef MDM_SHARED_MEM
   bpool(keyOffset, addr, len);
#endif /* MDM_SHARED_MEM */
}


void cmsMem_initSharedMemPointer(SINT32 keyOffset, void *addr, UINT32 len)
{
   /*
...
#ifdef MDM_SHARED_MEM
   bcm_secondary_bpool(keyOffset, addr);
#endif /* MDM_SHARED_MEM */
}
...
static inline void set_footer(UINT8 *buf, UINT32 size)
#ifdef CMS_MEM_DEBUG
{
   UINT32 i, roundup4Size = ROUNDUP4(size);
   UINT32 allocSize = REAL_ALLOC_SIZE(size);
   UINT32 intSize = allocSize / sizeof(UINT32);
   UINT32 hdrIntSize = CMS_MEM_HEADER_LENGTH / sizeof(UINT32);

   for (i=size; i < roundup4Size; i++)
   {
      buf[i] = CMS_MEM_FOOTER_PATTERN & 0xff;
   }

   ((UINT32 *)buf)[intSize - hdrIntSize - 1] = CMS_MEM_FOOTER_PATTERN;
   ((UINT32 *)buf)[intSize - hdrIntSize - 2] = CMS_MEM_FOOTER_PATTERN;
}
#else
{}
#endif
...
void *cmsMem_alloc(UINT32 size, UINT32 allocFlags)
{
   ...
   if (CMS_MEM_HEADER_LENGTH != 16)
   {
      cmsLog_error("Invalid code modification: CMS_MEM_HEADER_LENGTH must be 16!");
      return NULL;
   }
   ...
   if (buf != NULL)
   {
       ...
       */
      intBuf[CMS_MEM_HEADER_OFFSET_FLAGS] = allocFlags;
      intBuf[CMS_MEM_HEADER_OFFSET_RESERVED] = 0;
      intBuf[CMS_MEM_HEADER_OFFSET_SIZE] = size;
      intBuf[CMS_MEM_HEADER_OFFSET_SIZE_XOR] = size ^ 0xffffffff;

      buf = &(intBuf[CMS_MEM_HEADER_OFFSET_USER_BUF]);
...
#ifdef CMS_MEM_LEAK_TRACING
      {
         ...
         else
         {
             ...
             * the head.
             */
            if (cmsMem_leakTracing_lock())
            {
               cmsLog_error("Could not get leakTracing lock!");
            }
            else
            {
               dlist_append((struct dlist_node *)allocRec, &glbAllocRec);
               cmsMem_leakTracing_unlock();
            }
         }
...
void *cmsMem_realloc(void *origBuf, UINT32 size)
{
   ...
   if (size == 0)
   {
      /* matches behavior of standard realloc */
      cmsMem_free(origBuf);
      return NULL;
   }
   ...
   origAllocFlags = intBuf[CMS_MEM_HEADER_OFFSET_FLAGS];
   origSize = intBuf[CMS_MEM_HEADER_OFFSET_SIZE];

   /* sanity check the original length */
   if (intBuf[CMS_MEM_HEADER_OFFSET_SIZE] != (intBuf[CMS_MEM_HEADER_OFFSET_SIZE_XOR] ^ 0xffffffff))
   {
      cmsLog_error("memory underflow detected on %p [0x%x 0x%x]",
                   origBuf,
                   intBuf[CMS_MEM_HEADER_OFFSET_SIZE],
                   intBuf[CMS_MEM_HEADER_OFFSET_SIZE_XOR]);
      cmsAst_assert(0);
      return NULL;
   }
   ...
   {
      /*  update size in the header when the new size is larger than old size 
       *  but actual allocated size doesn't change 
       */
      if (size > origSize)
      {
         intBuf[CMS_MEM_HEADER_OFFSET_SIZE] = size;
   ...
   else
   {
       ...
       * Return NULL but leave the original buffer untouched.  (matches
       * behavior of standard realloc)
       */
   }
...
void cmsMem_free(void *buf)
{
   ...
   if (buf != NULL)
   {
      ...
      size = intBuf[CMS_MEM_HEADER_OFFSET_SIZE];

      if (intBuf[CMS_MEM_HEADER_OFFSET_SIZE] != (intBuf[CMS_MEM_HEADER_OFFSET_SIZE_XOR] ^ 0xffffffff))
      {
         cmsLog_error("memory underflow detected on %p [%d %d]",
                      buf,
                      intBuf[CMS_MEM_HEADER_OFFSET_SIZE],
                      intBuf[CMS_MEM_HEADER_OFFSET_SIZE_XOR]);
         cmsAst_assert(0);
...
#ifdef CMS_MEM_DEBUG
      {
         ...
         roundup4Size = ROUNDUP4(size);

         for (i=size; i < roundup4Size; i++)
         {
            if (charBuf[i] != (UINT8) (CMS_MEM_FOOTER_PATTERN & 0xff))
            {
               cmsLog_error("memory overflow detected on %p at idx=%d 0x%x 0x%x 0x%x",
                            buf, i, charBuf[i], intBuf[intSize-1], intBuf[intSize-2]);
               cmsAst_assert(0);
               return;
            }
         }
         ...
         {
            cmsLog_error("memory overflow detected on %p: [0x%x 0x%x]",
                         buf, intBuf[intSize - 1], intBuf[intSize - 2]);
            cmsAst_assert(0);
            return;
         }
...
#ifdef CMS_MEM_LEAK_TRACING
   printf("\n");
   printf("Memory Leak Tracing is ENABLED.\n\n");
#endif
...
// There are 3 possible ways a string fits into a buf:
// 1. The string fits perfectly in the buf.
// 2. The buf is larger than what is actually needed for the string, but
//    it has been zeroized, so the buf starts with the string and then is
//    all zero's after that.
// 3. The buf is larger than what is actually needed for the string, and
//    it is not zeroized.  In this case, it is very easy to mis-identify
//    some data as a string.
// This function can detect cases 1 and 2, but not 3.
static UBOOL8 isStringBuf(const char *buf, UINT32 len)
{
   UINT32 i, j;

   if (!isascii(buf[0]) || buf[len-1] != '\0')
   {
      // Does not start with ascii char or no null termination, not a string buf
      return FALSE;
   }
   ...
   for (i=1; i < len-1; i++)
   {
      if (buf[i] == '\0')
      {
         // null inside the buf has been detected, so everything after this
         // must also be null, otherwise, it could be some other data.
         for (j=i+1; j < len-1; j++)
         {
            if (buf[j] != '\0')
               return FALSE;
         }
         // We've scanned to the end, and it is all null's, so assume string.
         return TRUE;
      }
...
static UINT16 getMdmObjectId(const UINT16 *shortp)
{
   UINT16 oid = shortp[0];

   // MDM ObjectIds are from 1-5000.  Depending on whether we are using the
   // TR98 or 181 data model, various features (voice, storage, PON, etc),
   // and customer extensions, we could narrow this range even further to
   // reduce false positives.  But we cannot query the MDM because this is in
   // cms_util.  See data-model/README-OID.txt for MDM OID range allocations.
   if (oid > 0 && oid < 5000)
      return oid;
   else
      return 0;
...
static void dumpAllocRec(AllocRecord *allocRec, UBOOL8 dumpStackTrace)
{
   ...
   // This timestamp is a bit verbose, but is the standard ISO8601 format.
   // Also it is in GMT.  Matches the bcmuLog DATETIME timestamp.
   memset(&gmt_tm, 0, sizeof(gmt_tm));
   gmtime_r(&(allocRec->t), &gmt_tm);
   strftime(dateTimeBuf, sizeof(dateTimeBuf), "%Y-%m-%dT%H:%M:%SZ", &gmt_tm);
   printf("%s ", dateTimeBuf);

   // TODO: since our meminfo reports shared and heap mem stats separately,
   // it would be nice if user can just dump or trace shared or heap memory.
   // (Requires changes in all the apps that support leak tracing though...)
   printf("addr=%p ", allocRec->bufAddr);
   ...
   // Dump contents of buffer, it could provide a useful clue.
   if (isStringBuf((const char *) allocRec->bufAddr, allocRec->userSize))
   {
      UINT32 i;
      // Dump as string buf
      for (i=0; i < allocRec->userSize-1 && allocRec->bufAddr[i] != '\0'; i++)
         printf("%c", allocRec->bufAddr[i]);
   }
   else
   {
      UINT16 oid;
      UINT32 i;
      const int maxWords=20;  // dump max 20 words
      int *ip = (int *) allocRec->bufAddr;

      oid = getMdmObjectId((const UINT16 *) allocRec->bufAddr);
      if (oid > 0)
         printf("possible MDM OID %d: ", oid);

      // Dump in hex, 4 bytes at a time.
      for (i=0; i < allocRec->userSize/4 && i < maxWords; i++)
         printf("%08x ", ip[i]);
      if (allocRec->userSize > (maxWords*4))
         printf("...");
   }
   printf("]\n");
   ...
   // Use this in case print_backtrace does not work.
...
void cmsMem_dumpTraceClones()
{
   ...
   {
      /*
       * It is not clear how well all this fancy clone matching code works.
       * It did not the last time I tried in May 2020.  You must also include
       * backtrace_arm.o in the library, see Makefile.  It might be easier
       * to just dump all the allocRecs and do some post-processing on the
       * data to look for buffers that are repeatedly left over.  One approach:
       * sort output based on the timestamps and look for clusters of allocs.
       *
       * dumpAllocRec(allocRec, FALSE);
       */
...
// TODO: move the tracking of shared memory allocRec's into bget.c, then we
// will not have to do garbage collection anymore and the shared memory alloc's
// will always be 100% accurate.  (Also can get rid of the seq field and isShmAddrInUse).
