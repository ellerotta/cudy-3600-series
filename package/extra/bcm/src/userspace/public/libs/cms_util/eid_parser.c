...
CmsRet parse_eid_line(const char *line, UINT32 len, CmsEntityInfo *eInfo)
{
   ...
   case EID_TOKEN_SCHEDPOLICY:
      cmsLog_error("WARRING! \"schedPolicy\" is no longer supported by EID files, "
                    "please go to the new place [userspace/public/apps/rtpolicy/scripts/rt_settings_*.txt] to configure it instead.");
      //eInfo->schedPolicy = symbol_to_value(pair.valuebuf, sizeof(pair.valuebuf));
      break;

   case EID_TOKEN_SCHEDPRIORITY:
      cmsLog_error("WARRING! \"schedPriority\" is no longer supported by EID files, "
                    "please go to the new place [userspace/public/apps/rtpolicy/scripts/rt_settings_*.txt] to configure it instead.");
      //eInfo->schedPriority = symbol_to_value(pair.valuebuf, sizeof(pair.valuebuf));
      break;

   case EID_TOKEN_CPUMASK:
      cmsLog_error("WARRING! \"cpuMask\" is no longer supported by EID files, "
                    "please go to the new place [userspace/public/apps/rtpolicy/scripts/rt_settings_*.txt] to configure it instead.");
      //eInfo->cpuMask = symbol_to_value(pair.valuebuf, sizeof(pair.valuebuf));
      break;

   case EID_TOKEN_CPUGROUPNAME:
      cmsLog_error("WARRING! \"cpuGroupName\" is no longer supported by EID files, "
                    "please go to the new place [userspace/public/apps/rtpolicy/scripts/rt_settings_*.txt] to configure it instead.");
