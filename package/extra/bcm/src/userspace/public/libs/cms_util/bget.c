...
#endif
{
    ...
    printf("bget: could not allocate %lu (no more shared memory) type meminfo in CMS CLI to get stats.\n",
           requested_size);
    fflush(stdout);
    return NULL;
}
...
void bcm_secondary_bpool(int keyOffset, void *buf)
{
   /*
   ...
   if( (bg_semid = semget(BGET_SEMAPHORE_KEY+keyOffset, 1, 0666)) == -1) {
...
void bpool(int keyOffset, void *buf, bufsize len)
{
    struct bfhead *b = BFH(buf);
    struct bhead *bn;

    if( g_bg == NULL ) {
        if( (bg_semid = semget(BGET_SEMAPHORE_KEY+keyOffset, 1, IPC_CREAT|0666)) == -1) {
	        perror("bget: semget\n");
	        return;
        }
