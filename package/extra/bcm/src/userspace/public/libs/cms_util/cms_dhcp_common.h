...
#define DHCP6C_PREFIX_LEN_HINT        60  /* make it < 64 */

#define DHCP6C_CONF_TEMPLATE "\
interface %s\n\
{\n\
...
#define DHCP6C_IAPD "send ia-pd 0;"

#define DHCP6C_IAPD_ASSOC "\
id-assoc pd 0\n\
{\n\
...
#define DHCP6C_IANA "send ia-na 1;"
#define DHCP6C_IANA_ASSOC "\
id-assoc na 1\n\
{\n\
};\n\
"
...
typedef struct {
    ...
    int (*needFn)(const void * parm); /* check if the subCode is needed*/
    char type;  
    char *name;   
    char *valDef; /* default value */
    int (*valFn)(const void * parm, char* string, uint16_t * len);
}DhcpSubOptionTable;


/** This function is to make the dir for saving config files. 
 *
 * @param dhcpVer  (IN) DHCPv4 or DHCPv6.
 * @param ifName  (IN)  interface name on which dhcpc is launched.
 *
 * @return CmsRet enum, CMSRET_SUCCESS: success, others: fail.
 */
CmsRet cmsDhcp_mkCfgDir(DhcpVersion dhcpVer, const char *ifName);


/** This function is to read DHCP optionX string from optionX file. 
...
/** This function is to save DHCP optionX string to file, so that other application(dhcpc) can get it. 
...
/** This function is to encapsulate DHCP sub-options into packet based on TLV(type/length/value)
...
/** This function is to parse option data TLV, then get sub-option DATA with given sub-option code
 *
...
/** This function is to construct option 43 value
 *
 * @param wanType  (IN) PKTCBL_WAN_TYPE enum
 * @param duid     (IN)
 * @param optionValue  (INOUT) option value buffer
 * @param optionLen    (INOUT) option vlaue len
 *
 * @return CmsRet enum, CMSRET_SUCCESS: success, others: fail.
 */
CmsRet cmsDhcp_constructOption43Value(PKTCBL_WAN_TYPE wanType, const char* duid,
                                                   char* optionValue, int* optionLen);

/** This function is to construct option 60 value
 *
 * @param optionValue  (INOUT) option value buffer
 * @param optionLen    (INOUT) option vlaue len
 *
...
/** This function is to construct option 125 value
 *
 * @param optionValue  (INOUT) option value buffer
 * @param optionLen    (INOUT) option vlaue len
 *
...
/** This function is to construct option 17 value
 *
 * @param wanType  (IN) PKTCBL_WAN_TYPE enum
 * @param duid     (IN)
 * @param optionValue  (INOUT) option value buffer
 ...
 * @return CmsRet enum, CMSRET_SUCCESS: success, others: fail.
 */
CmsRet cmsDhcp_constructOption17Value(PKTCBL_WAN_TYPE wanType, char* duid,
                                                   char* optionValue, int* optionLen);
