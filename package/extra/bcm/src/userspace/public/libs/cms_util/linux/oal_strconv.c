...
int oal_getRandomUuid(char *uuidStr, int len)
{
   ...
   // fgets reads at most len-1 bytes into buffer.
   if( fgets(uuidStr, len, fp) )
   {
      bytesRead = strlen(uuidStr);
      uuidStr[bytesRead-1] = '\0'; // strip the newline at the end of the str
   }

   fclose(fp);

   // Technically, we read bytesRead, but we are returning bytesRead-1 bytes
   // of real data due to the strip.
   return bytesRead;
}
