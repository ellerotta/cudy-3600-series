...
//brcm begin
#include "framework_cli.h"  // for FRAMEWORK_CLI_MAX_MAX_BUF_SZ
// brcm end
...
#ifdef CONFIG_FEATURE_SH_FANCY_PROMPT
static const char * const bb_msg_unknown = "(unknown)";
#endif
...
static ssize_t safe_read(int fd, void *buf, size_t count)
{
    ...
    // brcm
    if (frameworkCli_waitForInputAvailable() != BCMRET_SUCCESS)
    {
       return -1;
    }
...
static char **username_tab_completion(char *ud, int *num_matches)
{
	...
	if (num_matches == 0) {		/* "~/..." or "~user/..." */
		...
		if (home) {
			if ((userlen + strlen(home) + 1) < FRAMEWORK_CLI_MAX_BUF_SZ) {
				char temp2[FRAMEWORK_CLI_MAX_BUF_SZ]={0};	/* argument size */
					...
					int type)
{
	...
	DIR *dir;
	struct dirent *next;
	char dirbuf[FRAMEWORK_CLI_MAX_BUF_SZ]={0};
	int nm = *num_matches;
	struct stat st;
	char *path1[1];
	char **paths = path1;
...
#define collapse_pos(is, in) { \
	memcpy(int_buf+(is), int_buf+(in), (FRAMEWORK_CLI_MAX_BUF_SZ+1-(is)-(in))*sizeof(int)); \
	memcpy(pos_buf+(is), pos_buf+(in), (FRAMEWORK_CLI_MAX_BUF_SZ+1-(is)-(in))*sizeof(int)); }
...
static int find_match(char *matchBuf, int *len_with_quotes)
{
	...
	int command_mode;
	int c, c2;
	int int_buf[FRAMEWORK_CLI_MAX_BUF_SZ + 1]={0};
	int pos_buf[FRAMEWORK_CLI_MAX_BUF_SZ + 1]={0};
...
static void input_tab(int *lastWasTab)
{
	...
	if (! *lastWasTab) {
		...
		char *tmp;
		int len_found;
		char matchBuf[FRAMEWORK_CLI_MAX_BUF_SZ]={0};
		int find_type;
		int recalc_pos;
		...
		*lastWasTab = TRUE;		/* flop trigger */
		...
		if ((len_found - strlen(matchBuf) + len) < FRAMEWORK_CLI_MAX_BUF_SZ) {
	...
	if (( fp = fopen ( fromfile, "r" ))) {
		...
		for ( hi = 0; hi < MAX_HISTORY; ) {
			...
			if(!hl)
				break;
			l = strlen(hl);
			if(l >= FRAMEWORK_CLI_MAX_BUF_SZ)
				hl[FRAMEWORK_CLI_MAX_BUF_SZ-1] = 0;
			if(l == 0 || hl[0] == ' ') {
				free(hl);
				continue;
			}
...
int cmdedit_read_input(char *prompt, char *command)
{
...
prepare_to_die:
				...
				printf("CLI exiting now.");
				goto_new_line();
				/* cmdedit_reset_term() called in atexit */
				// exit(EXIT_SUCCESS);
            // brcm: return to caller so we have a common exit path
            return -1;
#else
...
rewrite_line:
...
#if 0 /* brcm (Esc + number) is entered mistakenly quite easy for menu driven CLI; remove */
			if (c >= '1' && c <= '9')
				do
					if (safe_read(0, &c, 1) < 1)
						goto prepare_to_die;
		...
		default:	/* If it's regular input, do the normal thing */
			...
			if (len >= (FRAMEWORK_CLI_MAX_BUF_SZ - 2))	/* Need to leave space for enter */
				break;
...
{
	char buff[FRAMEWORK_CLI_MAX_BUF_SZ]={0};
	char *prompt =
#if defined(CONFIG_FEATURE_SH_FANCY_PROMPT)
