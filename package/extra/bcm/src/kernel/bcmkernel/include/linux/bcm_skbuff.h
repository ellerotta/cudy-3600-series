...
#if defined(CONFIG_HAVE_SKBUFF_WL_SPC)
				/* this SKB holds data frags for other SKBs */
				uint32_t is_spc:1;
				uint32_t nic_reserved1:9;
#else
...
#if defined(CONFIG_HAVE_SKBUFF_WL_SPC)
		/*
		 * In XBOW/WFD to WLAN handoff path, ucast/pktfwd are used
		 * interchangeably. Even though is_spc *should* only be used
		 * after pktfwd, let's still keep the bit common and reserved.
		 */
		uint32_t is_spc:1;
		uint32_t pktfwd_reserved    : 5;
#else
