...
static int spinand_init(struct spinand_device *spinand)
{
...

	{
		...
		char ram[64];
		//unsigned int row;
		unsigned int idx = 0, status = 0, uid_error = 0, step = 0;
		u8 cfg;
		u8 uuid[129] = {0};
		u8 tmp[5] = {0};
		struct nand_pos cpos;
		struct nand_page_io_req creq = {
			//.pos = cpos,
			.dataoffs = 0,
			.datalen = 64, //get 1st UUID block.
			.mode = MTD_OPS_RAW,
		};

		//spinand_read_status(spinand, &cfg);
		spinand_read_reg_op(spinand, REG_CFG, &cfg);
		printk("[%s][%d]......before set OTP:%02X\n", __FUNCTION__, __LINE__, cfg);

		//make sure OTP feature is enabled
		status = spinand_upd_cfg(spinand, CFG_OTP_ENABLE, CFG_OTP_ENABLE);
		if (status)
		{
			dev_err(dev, 
					"[%s][%d]enable OTP error %d.\n",
		...
		//spinand_read_status(spinand, &cfg);
		spinand_read_reg_op(spinand, REG_CFG, &cfg);
		printk("[%s][%d]......after set OTP:%02X\n", __FUNCTION__, __LINE__, cfg);

		if( !(cfg & CFG_OTP_ENABLE) )
		{
			printk("[%s][%d]......set OTP error.\n", __FUNCTION__, __LINE__);
			goto _cont;
		}

		//start read page op.
		//set all pos member to 0, indicating UUID row/page address:0
		memset(&cpos, 0, sizeof(struct nand_pos));
		creq.pos = cpos;
		memset(ram, 0, sizeof(ram));

		creq.databuf.in = ram;

		//row = nanddev_pos_to_row(spinand_to_nand(spinand), &(creq.pos));
		//printk("[%s][%d]...............UUID row is %d\n", __FUNCTION__, __LINE__, row);

		status = spinand_read_page(spinand, &creq, false);

		if(status)
		{
			dev_err(dev, 
					"[%s][%d]read snand uuid return %d.\n",
					__FUNCTION__, __LINE__, status);
			goto _cont;
		}

		for(idx=0; idx<sizeof(ram); idx++)
		{
			if (idx == 32)
				printk("\n");
			printk(" %02X ", ram[idx]);
		...
		//32-47 bytes is UID, 47-63 bytes is UID complement(bit-wise)
		if (uid_error==1)
		{
			printk("[%s][%d]...............UUID 1st get error, try next 32bytes.\n", __FUNCTION__, __LINE__);
			uid_error = 0;
			step = 32;
			for (idx=32; idx <48; idx++)
			{
				if ((ram[idx] ^ ram[idx+16]) != 0xFF)
				{
					uid_error = 1;
		...
		if (uid_error==1)
		{
			dev_err(dev, "ESMT Flash Device read UUID error\n");
			goto _cont;
		}
		else
		{
			//spinand->id.data has been got in broadcom.c :spinand_detect();
			sprintf(uuid, "%02X%02X-", spinand->id.data[0], spinand->id.data[1]);
			for (idx=step+0; idx <step+16; idx++)
			{
				sprintf(tmp, "%02X", ram[idx]);
				strcat(uuid, tmp);
			}
