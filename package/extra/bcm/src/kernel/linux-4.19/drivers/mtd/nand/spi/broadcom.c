...
static SPINAND_OP_VARIANTS(x1_read_cache_variants,
		SPINAND_PAGE_READ_FROM_CACHE_OP(true, 0, 1, NULL, 0),
		SPINAND_PAGE_READ_FROM_CACHE_OP(false, 0, 1, NULL, 0));
...
#define WINBOND_BFD_MASK		GENMASK(6, 4) /* provides mask high bits 6 through 4 */
...
{ // set bit flip detect to two bits, this should enable it such that <= 2 bits and > 2 bits are detected seperately when reading the status
	...
	ret = spi_mem_exec_op(spinand->spimem, &op2);
...
static const struct spinand_info esmt_spinand_table[] = {
	...
	SPINAND_INFO("F50L1G41LB", 0x01,
		     NAND_MEMORG(1, 2048, 64, 64, 1024, 1, 1, 1),
		     NAND_ECCREQ(1, 512),
		     SPINAND_INFO_OP_VARIANTS(&x1_read_cache_variants,
					      &write_cache_variants,
					      &update_cache_variants),
		     0,
