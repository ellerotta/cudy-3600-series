...
static int port_unimac_init(mac_dev_t *mac_dev)
{
    ...
    err_len_chk = (flags & UNIMAC_DRV_PRIV_FLAG_EXTSW_CONNECTED) ? 0 : 1;
#endif
    mac_hwapi_init_emac(emac, err_len_chk);
...
static int port_unimac_cfg_set(mac_dev_t *mac_dev, mac_cfg_t *mac_cfg)
{
    ...
    mac_dev->stats_refresh_interval = mac_dev_get_stats_refresh_interval(mac_dev, UMAC_B64_BIT_HI, UMAC_B64_BIT_LO, mac_cfg->speed);
#else
    mac_dev->stats_refresh_interval = mac_dev_get_stats_refresh_interval(mac_dev, 32, 32, mac_cfg->speed);
#endif
...
static int port_unimac_loopback_set(mac_dev_t *mac_dev, mac_loopback_t loopback_set)
{
    ...
    switch (loopback_set)
    {
    case MAC_LOOPBACK_NONE:     loopback = MAC_LPBK_NONE; break;
    case MAC_LOOPBACK_LOCAL:    loopback = MAC_LPBK_LOCAL; break;
    case MAC_LOOPBACK_REMOTE:   loopback = MAC_LPBK_REMOTE; break;
//    case MAC_LOOPBACK_BOTH:     loopback = MAC_LPBK_BOTH; break;  // BOTH not supported 
    default:  return -1;
    }
...
static int port_unimac_loopback_get(mac_dev_t *mac_dev, mac_loopback_t *op)
{
    rdpa_emac emac = mac_dev->mac_id;
    MAC_LPBK loopback;

    mac_hwapi_get_loopback(emac, &loopback);
    
    switch (loopback)
    {
    case MAC_LPBK_NONE:     *op = MAC_LOOPBACK_NONE; break;
    case MAC_LPBK_LOCAL:    *op = MAC_LOOPBACK_LOCAL; break;
    case MAC_LPBK_REMOTE:   *op = MAC_LOOPBACK_REMOTE; break;
    default: return -1;
    }
...
static int port_unimac_dt_priv(const dt_handle_t handle, int mac_id, void **priv)
{
    ...
    if (dt_property_read_bool(handle, "shrink-ipg"))
        unimac_flags |= UNIMAC_DRV_PRIV_FLAG_SHRINK_IPG;
    if (dt_parse_phandle(handle, "link", 0))
        unimac_flags |= UNIMAC_DRV_PRIV_FLAG_EXTSW_CONNECTED;
    if (dt_property_read_bool(handle, "multi-imp"))
        unimac_flags |= UNIMAC_DRV_PRIV_FLAG_EXTSW_CONNECTED;
...
mac_drv_t mac_drv_unimac =
{
    ...
    .loopback_get = port_unimac_loopback_get,
