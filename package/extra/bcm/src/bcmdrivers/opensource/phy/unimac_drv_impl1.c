...
#define     UNIMAC_VER1  1  /* for PON, older runner based DSL devices */
#define     UNIMAC_VER2  2  /* for 47622 device */
#define     UNIMAC_VER3  3  /* for 63146 & 4912 & 6888 & 68880 & 6837 devices */

static int unimac_ver = UNIMAC_VER1;

void __iomem * unimac_conf_base;
void __iomem * unimac_mib_base;
void __iomem * unimac_top_base;
uint32_t conf_offset;
uint32_t mib_offset;
uint32_t top_offset;

int unimac2_init(dt_device_t *pdev)
{
    int ret;
    void __iomem * unimac_conf_base2;
    void __iomem * unimac_mib_base2;
    void __iomem * unimac_top_base2;

    unimac_ver = UNIMAC_VER2;
    unimac_conf_base = dt_dev_remap_resource(pdev, 0);
    unimac_conf_base2= dt_dev_remap_resource(pdev, 3);
    if (IS_ERR(unimac_conf_base) || IS_ERR(unimac_conf_base2))
    {
        ret = PTR_ERR(unimac_conf_base);
        unimac_conf_base = NULL;
        dev_err(&pdev->dev, "Missing unimac_conf_base entry\n");
        goto Exit;
    }

    unimac_mib_base = dt_dev_remap_resource(pdev, 1);
    unimac_mib_base2= dt_dev_remap_resource(pdev, 4);
    if (IS_ERR(unimac_mib_base) || IS_ERR(unimac_mib_base2))
    {
        ret = PTR_ERR(unimac_mib_base);
        unimac_mib_base = NULL;
        dev_err(&pdev->dev, "Missing unimac_mib_base entry\n");
        goto Exit;
    }

    unimac_top_base = dt_dev_remap_resource(pdev, 2);
    unimac_top_base2= dt_dev_remap_resource(pdev, 5);
    if (IS_ERR(unimac_top_base) || IS_ERR(unimac_top_base2))
    {
        ret = PTR_ERR(unimac_top_base);
        unimac_top_base = NULL;
        dev_err(&pdev->dev, "Missing unimac_top_base entry\n");
        goto Exit;
    }

    conf_offset = (uint32_t)((unsigned long)unimac_conf_base2 - (unsigned long)unimac_conf_base);
    mib_offset  = (uint32_t)((unsigned long)unimac_mib_base2  - (unsigned long)unimac_mib_base);
    top_offset  = (uint32_t)((unsigned long)unimac_top_base2  - (unsigned long)unimac_top_base);
...
int unimac3_init(dt_device_t *pdev)
{
    unimac_ver = UNIMAC_VER3;
    return 0;
}

static int unimac_probe(dt_device_t *pdev)
{
    int ret;
    int (*init_func)(dt_device_t *);
    ...
    if (unimac_ver == UNIMAC_VER2)
        goto Done;

    unimac_conf_base = dt_dev_remap_resource(pdev, 0);
    if (IS_ERR(unimac_conf_base))
    {
        ret = PTR_ERR(unimac_conf_base);
        unimac_conf_base = NULL;
        dev_err(&pdev->dev, "Missing unimac_conf_base entry\n");
    ...
    if (unimac_ver == UNIMAC_VER3)
    {
        /* Version 3 devices have mib counter as in part of conf,
         * so mib_base is conf_base + mib_offset.
         * however, the original mib_offset is meant for the difference
         * between each unimac's mib register area, which in this case
         * is the same as conf_offset. */
        unimac_top_base = dt_dev_remap_resource(pdev, 1);
        if (IS_ERR(unimac_top_base))
        {
            ret = PTR_ERR(unimac_top_base);
            unimac_top_base = NULL;
            dev_err(&pdev->dev, "Missing unimac_top_base entry\n");
    ...
    conf_offset = dt_property_read_u32(node, "conf_offset");
    if (conf_offset == (uint32_t)(-1))
    {
        ret = -1;
        dev_err(&pdev->dev, "Missing conf_offset entry\n");
        goto Exit;
    }

    mib_offset = dt_property_read_u32(node, "mib_offset");
    if (mib_offset == (uint32_t)(-1))
    {
        ret = -1;
        dev_err(&pdev->dev, "Missing mib_offset entry\n");
        goto Exit;
    }

    if (unimac_ver == UNIMAC_VER3)
    {
        unimac_mib_base = unimac_conf_base + mib_offset;
        mib_offset = conf_offset;
    }

    top_offset = dt_property_read_u32(node, "top_offset");
    if (top_offset == (uint32_t)(-1))
    {
        ret = -1;
        dev_err(&pdev->dev, "Missing top_offset entry\n");
        goto Exit;
    }

Done:
    dev_dbg(&pdev->dev, "unimac_conf_base=0x%px\n", unimac_conf_base);
    dev_dbg(&pdev->dev, "unimac_mib_base=0x%px\n", unimac_mib_base);
    dev_dbg(&pdev->dev, "unimac_top_base=0x%px\n", unimac_top_base);
    dev_dbg(&pdev->dev, "conf_offset=0x%x\n", conf_offset);
    dev_dbg(&pdev->dev, "mib_offset=0x%x\n", mib_offset);
    dev_dbg(&pdev->dev, "top_offset=0x%x\n", top_offset);
...
static const struct of_device_id of_platform_table[] = {
    ...
    { .compatible = "brcm,unimac2", .data = unimac2_init },
    { .compatible = "brcm,unimac3", .data = unimac3_init },
    { /* end of list */ },
};
...
/*                                                                           */
/*****************************************************************************/
#if !(defined(CONFIG_BCM963158) || defined(CONFIG_BCM96846) || defined(CONFIG_BCM96856) || defined(CONFIG_BCM947622) || \
defined(CONFIG_BCM96878) || defined(CONFIG_BCM963146) || defined(CONFIG_BCM94912) || defined(CONFIG_BCM96813) || defined (CONFIG_BCM96888))
#include "clk_rst.h"
#endif
...
#define UNIMAC_NUMBER_OF_SPEEDS 	( UNIMAC_SPEED_MAX )
#define UNIMAC_NUMBER_OF_DUPLEXES	( UNIMAC_DUPLEX_MAX )
...
#define _cmd_eth_speed_get(r)   (r.eth_speed | (r.eth_speed_bit2?4:0))
#define _cmd_eth_speed_set(r,s) r.eth_speed = s & 3; r.eth_speed_bit2 = (s & 4)?1:0
#define _cmd_eth_speed_set_field(n,s)   \
            UNIMAC_WRITE_FIELD(n,CMD,eth_speed,s&3);    \
            UNIMAC_WRITE_FIELD(n,CMD,eth_speed_bit2,(s&4)?1:0)
#else
#define _cmd_eth_speed_get(r)   r.eth_speed
#define _cmd_eth_speed_set(r,s) r.eth_speed = s
#define _cmd_eth_speed_set_field(n,s)   \
            UNIMAC_WRITE_FIELD(n,CMD,eth_speed,s)
#endif
...
void mac_hwapi_get_configuration(rdpa_emac emacNum,rdpa_emac_cfg_t *emac_cfg)
{
    ...
    emac_cfg->rate					=	_cmd_eth_speed_get(mCfgReg);
...
void mac_hwapi_set_configuration(rdpa_emac emacNum,rdpa_emac_cfg_t *emac_cfg)
{
    ...
    {
        _cmd_eth_speed_set(mCfgReg, emac_cfg->rate);
    ...
    {
        uint32_t val;
        uintptr_t address = (uintptr_t)UNIMAC_XIB_CTRL + UNIMAC_CONF_INSTANCE_OFFSET(emacNum);

        READ_32(address,val);
        if ((int)emac_cfg->rate ==  (int)UNIMAC_SPEED_2500 || (int)emac_cfg->rate ==  (int)UNIMAC_SPEED_5000)
            val |= XIB_CTRL_STRICT_PREAMBLE_DIS;
        else
            val &= ~XIB_CTRL_STRICT_PREAMBLE_DIS;
        WRITE_32(address,val);
    }
#endif
}
...
void mac_hwapi_get_speed(rdpa_emac emacNum,rdpa_emac_rate *rate)
{
    ...
    UNIMAC_READ32_REG(emacNum,CMD,mCfgReg);
    *rate = _cmd_eth_speed_get(mCfgReg);
}
...
void mac_hwapi_set_speed(rdpa_emac emacNum,rdpa_emac_rate rate)
{
    ...
    if (!mCfgReg.ena_ext_config) /* Change only if auto_config is not set */
    {
        ...
        _cmd_eth_speed_set_field(emacNum, rate);
...
#if defined(CONFIG_BCM94908) || defined(CONFIG_BCM963158) || defined(CONFIG_BCM947622) || defined(CONFIG_BCM963146) || defined(CONFIG_BCM94912) || defined(CONFIG_BCM96813) || defined (CONFIG_BCM96888) || defined(CONFIG_BCM968880) || defined(CONFIG_BCM96837)
...
/*                                                                            */
/******************************************************************************/
#if defined(CONFIG_BCM947622) || defined(CONFIG_BCM963146) || defined(CONFIG_BCM94912) || defined(CONFIG_BCM96813) || defined(CONFIG_BCM96888) || defined(CONFIG_BCM968880) || defined(CONFIG_BCM96837)
void mac_hwapi_get_tx_max_frame_len(rdpa_emac emacNum,uint32_t *maxTxFrameLen )
{
    ...
    uintptr_t address = (uintptr_t)UNIMAC_TOPCTRL_MIB_MAX_PKT_SIZE + UNIMAC_MISC_INSTANCE_OFFSET(emacNum);
#else
    uintptr_t address = (uintptr_t)UNIMAC_TOPCTRL_MIB_MAX_PKT_SIZE + UNIMAC_CONF_INSTANCE_OFFSET(emacNum);
#endif
...
void mac_hwapi_set_rx_max_frame_len(rdpa_emac emacNum,uint32_t maxRxFrameLen )
{
    ...
    UNIMAC_WRITE_FIELD(emacNum,FRM_LEN,frame_length,0x3fff);
...
void mac_hwapi_get_mac_status(rdpa_emac emacNum,S_HWAPI_MAC_STATUS *macStatus)
{
    ...
    UNIMAC_READ32_REG(emacNum, MODE, *macStatus);
}
...
#elif defined(CONFIG_BCM963146) || defined(CONFIG_BCM94912) || defined(CONFIG_BCM96813) || defined(CONFIG_BCM96888) || defined(CONFIG_BCM968880) || defined(CONFIG_BCM96837)
void mac_hwapi_set_flow_control(rdpa_emac emacNum,S_MAC_HWAPI_FLOW_CTRL *flowControl)
{
    uintptr_t misc_top_address;
    uint32_t value;
...
void mac_hwapi_get_tx_counters(rdpa_emac emacNum,rdpa_emac_tx_stat_t *txCounters)
{
    ...
    UNIMAC_READ32_MIB(emacNum,TRMGV,  tempVal);
    txCounters->frame_1519_mtu			+=   tempVal;
...
#define UNIMAC_READ64_MIB(e,r,o) READ_64( UNIMAC_CONFIGURATION_UMAC_MIB_0Module_RDP_##r + UNIMAC_MIB_INSTANCE_OFFSET(e) , (o) )

#else

#define UNIMAC_READ64_MIB(e,r,o) \
    do{\
        uint32_t valLsb, valMsb; \
        READ_32( UNIMAC_CONFIGURATION_UMAC_MIB_0Module_RDP_##r + UNIMAC_MIB_INSTANCE_OFFSET(e) , valLsb ); \
        READ_32( UNIMAC_CONFIGURATION_UMAC_MIB_0Module_RDP_##r##_UPPER + UNIMAC_MIB_INSTANCE_OFFSET(e) , valMsb ); \
        (o) = (uint64_t)valMsb << 32 | valLsb; \
    }while(0)

#endif

static rdpa_emac_rx_stat64_t rxCountersLast[MAX_NUM_OF_EMACS];
static rdpa_emac_tx_stat64_t txCountersLast[MAX_NUM_OF_EMACS];
#define UNIMAC_CURRENT_COUNTERS(emacNum, cur, last, bits, param) \
    do{\
        uint64_t  paramCurrent = cur->param; \
        cur->param = (cur->param - last[emacNum].param) & (((uint64_t)1 << bits)-1); \
        last[emacNum].param   = paramCurrent; \
    }while(0)

void mac_hwapi_get_rx_counters64(rdpa_emac emacNum,rdpa_emac_rx_stat64_t *rxCounters)
{
    uint64_t		tempVal;

    if (!(enabled_emac & (1 << emacNum)))
    {
        memset(rxCounters, 0, sizeof(rdpa_emac_rx_stat64_t));
        return;
    }

    UNIMAC_READ64_MIB(emacNum,GRALN,rxCounters->alignment_error);
    UNIMAC_READ64_MIB(emacNum,GRBCA,rxCounters->broadcast_packet);
    UNIMAC_READ64_MIB(emacNum,GRUC, rxCounters->unicast_packet);
    UNIMAC_READ64_MIB(emacNum,GRBYT,rxCounters->byte);
    UNIMAC_READ64_MIB(emacNum,RRBYT,tempVal);
    rxCounters->byte                    +=	tempVal;
    UNIMAC_READ64_MIB(emacNum,GRFCR,rxCounters->carrier_sense_error);
    UNIMAC_READ64_MIB(emacNum,GRCDE,rxCounters->code_error);
    UNIMAC_READ64_MIB(emacNum,GRXCF,rxCounters->control_frame);
    UNIMAC_READ64_MIB(emacNum,GRFCS,rxCounters->fcs_error);
    rxCounters->fragments				=	0;//?????
    UNIMAC_READ64_MIB(emacNum,GR255,rxCounters->frame_128_255);
    UNIMAC_READ64_MIB(emacNum,GR2047,tempVal);
    rxCounters->frame_1519_mtu			=	tempVal;
    UNIMAC_READ64_MIB(emacNum,GRMGV,tempVal);
    rxCounters->frame_1519_mtu			+=	tempVal;
    UNIMAC_READ64_MIB(emacNum,GR4095,tempVal);
    rxCounters->frame_1519_mtu			+=	tempVal;
    UNIMAC_READ64_MIB(emacNum,GR9216,tempVal);
    rxCounters->frame_1519_mtu			+=	tempVal;

    UNIMAC_READ64_MIB(emacNum,GR511,rxCounters->frame_256_511);

    UNIMAC_READ64_MIB(emacNum,GR1023,rxCounters->frame_512_1023);

    UNIMAC_READ64_MIB(emacNum,GR1518,rxCounters->frame_1024_1518);

    UNIMAC_READ64_MIB(emacNum,GR64,rxCounters->frame_64);

    UNIMAC_READ64_MIB(emacNum,GR127,rxCounters->frame_65_127);

    UNIMAC_READ64_MIB(emacNum,GRFLR,rxCounters->frame_length_error);

    UNIMAC_READ64_MIB(emacNum,GRJBR,rxCounters->jabber);

    UNIMAC_READ64_MIB(emacNum,GRMCA,rxCounters->multicast_packet);

    rxCounters->overflow				=	0;//no matched counter
    UNIMAC_READ64_MIB(emacNum,GROVR,rxCounters->oversize_packet);
	
    UNIMAC_READ64_MIB(emacNum,RRPKT,rxCounters->undersize_packet);

    UNIMAC_READ64_MIB(emacNum,GRPKT,rxCounters->packet);
    
    UNIMAC_READ64_MIB(emacNum,GRXPF,rxCounters->pause_control_frame);

    UNIMAC_READ64_MIB(emacNum,GRXUO,rxCounters->unknown_opcode);

    UNIMAC_CURRENT_COUNTERS(emacNum, rxCounters, rxCountersLast, UMAC_B64_BIT_LO, alignment_error);
    UNIMAC_CURRENT_COUNTERS(emacNum, rxCounters, rxCountersLast, UMAC_B64_BIT_LO, broadcast_packet);
    UNIMAC_CURRENT_COUNTERS(emacNum, rxCounters, rxCountersLast, UMAC_B64_BIT_LO, unicast_packet);
    UNIMAC_CURRENT_COUNTERS(emacNum, rxCounters, rxCountersLast, UMAC_B64_BIT_HI, byte);
    UNIMAC_CURRENT_COUNTERS(emacNum, rxCounters, rxCountersLast, UMAC_B64_BIT_LO, carrier_sense_error);
    UNIMAC_CURRENT_COUNTERS(emacNum, rxCounters, rxCountersLast, UMAC_B64_BIT_LO, code_error);
    UNIMAC_CURRENT_COUNTERS(emacNum, rxCounters, rxCountersLast, UMAC_B64_BIT_LO, control_frame);
    UNIMAC_CURRENT_COUNTERS(emacNum, rxCounters, rxCountersLast, UMAC_B64_BIT_LO, fcs_error);
    UNIMAC_CURRENT_COUNTERS(emacNum, rxCounters, rxCountersLast, UMAC_B64_BIT_LO, frame_64);
    UNIMAC_CURRENT_COUNTERS(emacNum, rxCounters, rxCountersLast, UMAC_B64_BIT_LO, frame_65_127);
    UNIMAC_CURRENT_COUNTERS(emacNum, rxCounters, rxCountersLast, UMAC_B64_BIT_LO, frame_128_255);
    UNIMAC_CURRENT_COUNTERS(emacNum, rxCounters, rxCountersLast, UMAC_B64_BIT_LO, frame_256_511);
    UNIMAC_CURRENT_COUNTERS(emacNum, rxCounters, rxCountersLast, UMAC_B64_BIT_LO, frame_512_1023);
    UNIMAC_CURRENT_COUNTERS(emacNum, rxCounters, rxCountersLast, UMAC_B64_BIT_LO, frame_1024_1518);
    UNIMAC_CURRENT_COUNTERS(emacNum, rxCounters, rxCountersLast, UMAC_B64_BIT_LO, frame_1519_mtu);
    UNIMAC_CURRENT_COUNTERS(emacNum, rxCounters, rxCountersLast, UMAC_B64_BIT_LO, frame_length_error);
    UNIMAC_CURRENT_COUNTERS(emacNum, rxCounters, rxCountersLast, UMAC_B64_BIT_LO, jabber);
    UNIMAC_CURRENT_COUNTERS(emacNum, rxCounters, rxCountersLast, UMAC_B64_BIT_LO, multicast_packet);
    UNIMAC_CURRENT_COUNTERS(emacNum, rxCounters, rxCountersLast, UMAC_B64_BIT_LO, oversize_packet);
    UNIMAC_CURRENT_COUNTERS(emacNum, rxCounters, rxCountersLast, UMAC_B64_BIT_LO, undersize_packet);
    UNIMAC_CURRENT_COUNTERS(emacNum, rxCounters, rxCountersLast, UMAC_B64_BIT_LO, packet);
    UNIMAC_CURRENT_COUNTERS(emacNum, rxCounters, rxCountersLast, UMAC_B64_BIT_LO, pause_control_frame);
    UNIMAC_CURRENT_COUNTERS(emacNum, rxCounters, rxCountersLast, UMAC_B64_BIT_LO, unknown_opcode);
}


void mac_hwapi_get_tx_counters64(rdpa_emac emacNum,rdpa_emac_tx_stat64_t *txCounters)
{
    /*fill the rdpa counters structure*/
    uint64_t		tempVal;
    ...
    if (!(enabled_emac & (1 << emacNum)))
    {
        memset(txCounters, 0, sizeof(rdpa_emac_tx_stat64_t));
        return;
    }

    UNIMAC_READ64_MIB(emacNum,GTBYT,txCounters->byte);

    UNIMAC_READ64_MIB(emacNum,GTBCA,txCounters->broadcast_packet);

    UNIMAC_READ64_MIB(emacNum,GTMCA, txCounters->multicast_packet);

    UNIMAC_READ64_MIB(emacNum,GTUC, txCounters->unicast_packet);

    UNIMAC_READ64_MIB(emacNum,GTXCF,txCounters->control_frame);

    UNIMAC_READ64_MIB(emacNum,GTDRF,txCounters->deferral_packet);


    UNIMAC_READ64_MIB(emacNum,GTNCL,tempVal);
    txCounters->error						= tempVal;
    UNIMAC_READ64_MIB(emacNum,GTOVR,tempVal);
    txCounters->error						+= tempVal;
    UNIMAC_READ64_MIB(emacNum,GTFCS,tempVal);
    txCounters->error						+= tempVal;
    UNIMAC_READ64_MIB(emacNum,GTEDF,txCounters->excessive_deferral_packet);

    UNIMAC_READ64_MIB(emacNum,GTFCS,txCounters->fcs_error	);

    UNIMAC_READ64_MIB(emacNum,GTFRG,txCounters->fragments_frame);

    UNIMAC_READ64_MIB(emacNum,TR1518,txCounters->frame_1024_1518);

    UNIMAC_READ64_MIB(emacNum,TR255,  txCounters->frame_128_255);

    UNIMAC_READ64_MIB(emacNum,TR2047,  tempVal);
    txCounters->frame_1519_mtu			=   tempVal;
    UNIMAC_READ64_MIB(emacNum,TRMGV,  tempVal);
    txCounters->frame_1519_mtu			+=   tempVal;
    UNIMAC_READ64_MIB(emacNum,TR4095,  tempVal);
    txCounters->frame_1519_mtu			+=   tempVal;
    UNIMAC_READ64_MIB(emacNum,TR9216,  tempVal);
    txCounters->frame_1519_mtu			+=   tempVal;
    UNIMAC_READ64_MIB(emacNum,TR511,  txCounters->frame_256_511);

    UNIMAC_READ64_MIB(emacNum,TR1023, txCounters->frame_512_1023);

    UNIMAC_READ64_MIB(emacNum,TR64,txCounters->frame_64);

    UNIMAC_READ64_MIB(emacNum,TR127, txCounters->frame_65_127);

    UNIMAC_READ64_MIB(emacNum,GTJBR, txCounters->jabber_frame);

    UNIMAC_READ64_MIB(emacNum,GTLCL, txCounters->late_collision);

    UNIMAC_READ64_MIB(emacNum,GTMCL, txCounters->multiple_collision);

    UNIMAC_READ64_MIB(emacNum,GTOVR,txCounters->oversize_frame);

    UNIMAC_READ64_MIB(emacNum,GTPOK, txCounters->packet);

    UNIMAC_READ64_MIB(emacNum,GTXPF, txCounters->pause_control_frame);

    UNIMAC_READ64_MIB(emacNum,GTSCL, txCounters->single_collision);

    UNIMAC_READ64_MIB(emacNum,GTNCL, txCounters->total_collision);

    UNIMAC_READ64_MIB(emacNum,GTXCL, txCounters->excessive_collision);

    txCounters->underrun					=	0;
    txCounters->undersize_frame				=	0;

    UNIMAC_CURRENT_COUNTERS(emacNum, txCounters, txCountersLast, UMAC_B64_BIT_HI, byte);
    UNIMAC_CURRENT_COUNTERS(emacNum, txCounters, txCountersLast, UMAC_B64_BIT_LO, packet);
    UNIMAC_CURRENT_COUNTERS(emacNum, txCounters, txCountersLast, UMAC_B64_BIT_LO, broadcast_packet);
    UNIMAC_CURRENT_COUNTERS(emacNum, txCounters, txCountersLast, UMAC_B64_BIT_LO, multicast_packet);
    UNIMAC_CURRENT_COUNTERS(emacNum, txCounters, txCountersLast, UMAC_B64_BIT_LO, unicast_packet);
    UNIMAC_CURRENT_COUNTERS(emacNum, txCounters, txCountersLast, UMAC_B64_BIT_LO, deferral_packet);
    UNIMAC_CURRENT_COUNTERS(emacNum, txCounters, txCountersLast, UMAC_B64_BIT_LO, excessive_deferral_packet);
    UNIMAC_CURRENT_COUNTERS(emacNum, txCounters, txCountersLast, UMAC_B64_BIT_LO, control_frame);
    UNIMAC_CURRENT_COUNTERS(emacNum, txCounters, txCountersLast, UMAC_B64_BIT_LO, jabber_frame);
    UNIMAC_CURRENT_COUNTERS(emacNum, txCounters, txCountersLast, UMAC_B64_BIT_LO, oversize_frame);
    UNIMAC_CURRENT_COUNTERS(emacNum, txCounters, txCountersLast, UMAC_B64_BIT_LO, pause_control_frame);
    UNIMAC_CURRENT_COUNTERS(emacNum, txCounters, txCountersLast, UMAC_B64_BIT_LO, fragments_frame);
    UNIMAC_CURRENT_COUNTERS(emacNum, txCounters, txCountersLast, UMAC_B64_BIT_LO, error);
    UNIMAC_CURRENT_COUNTERS(emacNum, txCounters, txCountersLast, UMAC_B64_BIT_LO, fcs_error);
    UNIMAC_CURRENT_COUNTERS(emacNum, txCounters, txCountersLast, UMAC_B64_BIT_LO, frame_64);
    UNIMAC_CURRENT_COUNTERS(emacNum, txCounters, txCountersLast, UMAC_B64_BIT_LO, frame_65_127);
    UNIMAC_CURRENT_COUNTERS(emacNum, txCounters, txCountersLast, UMAC_B64_BIT_LO, frame_128_255);
    UNIMAC_CURRENT_COUNTERS(emacNum, txCounters, txCountersLast, UMAC_B64_BIT_LO, frame_256_511);
    UNIMAC_CURRENT_COUNTERS(emacNum, txCounters, txCountersLast, UMAC_B64_BIT_LO, frame_512_1023);
    UNIMAC_CURRENT_COUNTERS(emacNum, txCounters, txCountersLast, UMAC_B64_BIT_LO, frame_1024_1518);
    UNIMAC_CURRENT_COUNTERS(emacNum, txCounters, txCountersLast, UMAC_B64_BIT_LO, frame_1519_mtu);
    UNIMAC_CURRENT_COUNTERS(emacNum, txCounters, txCountersLast, UMAC_B64_BIT_LO, single_collision);
    UNIMAC_CURRENT_COUNTERS(emacNum, txCounters, txCountersLast, UMAC_B64_BIT_LO, total_collision);
    UNIMAC_CURRENT_COUNTERS(emacNum, txCounters, txCountersLast, UMAC_B64_BIT_LO, late_collision);
    UNIMAC_CURRENT_COUNTERS(emacNum, txCounters, txCountersLast, UMAC_B64_BIT_LO, multiple_collision);
    UNIMAC_CURRENT_COUNTERS(emacNum, txCounters, txCountersLast, UMAC_B64_BIT_LO, excessive_collision);
}
...
/*                                                                            */
/******************************************************************************/
void mac_hwapi_init_emac(rdpa_emac emacNum, int err_len_chk)
{
    ...
    mCfgReg.no_lgth_check	=	err_len_chk? 0 : 1;
    mCfgReg.rmt_loop_ena	=	0;
#if defined(CONFIG_BCM963146) || defined(CONFIG_BCM94912) || defined(CONFIG_BCM96813) || defined(CONFIG_BCM96888) || defined(CONFIG_BCM968880) || defined(CONFIG_BCM96837)
    mCfgReg.eth_speed_bit2  =   0;
#else
...
#ifdef UNIMAC_RXERR_MASK
    if (err_len_chk) {
        uintptr_t address = (uintptr_t)UNIMAC_RXERR_MASK(emacNum);
        uint32_t val;
        READ_32(address,val);
        val |= 1<<5;        // b5 - mark 802.3 length error pkt as error frame
        WRITE_32(address,val);
    }
#endif
...
{
#if defined(CONFIG_BCM947622) || defined(CONFIG_BCM963146) || defined(CONFIG_BCM94912) || defined(CONFIG_BCM96813) || defined(CONFIG_BCM96855) || defined(CONFIG_BCM96888) || defined(CONFIG_BCM968880) || defined(CONFIG_BCM96837)
    if (!(enabled_emac & (1 << emacNum)))
        return;
#else
...
{
    uint32_t value;
    uintptr_t mac_address;

    if (!(enabled_emac & (1 << emacNum)))
        return;

    value = __be32_to_cpu(*((uint32_t *)&mac.b[0]));
    mac_address = (uintptr_t)UNIMAC_CONFIGURATION_UMAC_0_RDP_MAC0 + UNIMAC_CONF_INSTANCE_OFFSET(emacNum);
    WRITE_32(mac_address, value);

    value = __be16_to_cpu(*((uint16_t *)&mac.b[4]));
...
void mac_hwapi_set_eee(rdpa_emac emacNum, int32_t enable)
{
    ...
    if (enable)
    {
        ...
        defined(CONFIG_BCM947622) || defined(CONFIG_BCM96878) || defined(CONFIG_BCM963146) || defined(CONFIG_BCM94912) || defined(CONFIG_BCM96813) || defined(CONFIG_BCM96855)  || defined(CONFIG_BCM96888) || defined(CONFIG_BCM968880) || defined(CONFIG_BCM96837)
