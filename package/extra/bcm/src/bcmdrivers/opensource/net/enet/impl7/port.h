...
typedef struct
{
	...
	unsigned long tx_dropped_blog_drop;
...
typedef struct port_info_t
{
    ...
    uint32_t is_macvalid		:1;
    uint32_t reserved           :19;
    int is_macsec;
    uint8_t macddr[ETH_ALEN];
} port_info_t;
...
#define MUX_PORTS 128
            ...
            struct enetx_port_t *mux_map[MUX_PORTS];
            /* parent->port_id -> enetx_port_t map, register here with mux_set_rx_index() */
            struct enetx_port_t *demux_map[MUX_PORTS];
...
#define     PORT_CFG_AS_WAN     (1<<1)  /* When this port is configured as WAN, for DSL Dev */
#define     PORT_IS_CFG_AS_WAN(port) ((port)->n.flags & PORT_CFG_AS_WAN)
#define     PORT_FLAG_8_TXQS    (1<<2)  /* for SF2 switch ports */
#define     PORT_HAS_8_TXQS(port)    ((port)->n.flags & PORT_FLAG_8_TXQS)
    } n;
} enetx_port_t;
...
typedef struct port_ops_t
{
    ...
    int (*dispatch_dg_pkt)(dispatch_info_t *dispatch_info);
...
#endif
    int (*post_init)(enetx_port_t *self);
} port_ops_t;
...
#define COMPAT_MAX_SWITCHES         MAX_SWITCHES
#define COMPAT_MAX_SWITCH_PORTS     24
extern enetx_port_t *unit_port_array[COMPAT_MAX_SWITCHES][COMPAT_MAX_SWITCH_PORTS]; 
...
static inline int port_mib_dump(enetx_port_t *self, int all, int reset)
{
    int rc;
    ...
    if (self->p.ops->mib_dump) {
        rc = self->p.ops->mib_dump(self, all);
        if (reset)
            port_mib_clear(self);
        return rc;
    }
