...
static int _ethsw_phy_access(struct ethswctl_data *e, enetx_port_t *port)
{
    ...
    if (e->type == TYPE_GET) {
        ...
        if (phy->phy_drv && phy->phy_drv->priv_fun)
            rc = phy_priv_fun(phy, PHY_OP_RD_MDIO, reg_offset, &phy_reg_val);
        else
            rc = phy_bus_read(phy, reg_offset, &phy_reg_val);
        data = phy_reg_val;
        up(&sw_p->s.conf_sem);
        ...
        down(&sw_p->s.conf_sem);
        if (phy->phy_drv && phy->phy_drv->priv_fun)
            rc = phy_priv_fun(phy, PHY_OP_WR_MDIO, reg_offset, phy_reg_val);
        else
            rc = phy_bus_write(phy, reg_offset, phy_reg_val);
        up(&sw_p->s.conf_sem);
    }
...
int ioctl_extsw_regaccess(struct ethswctl_data *e, enetx_port_t *port)
{
    ...
    if (e->offset & IS_PHY_ADDR_FLAG)
        return (port) ? _ethsw_phy_access(e, port) : -EINVAL;
...
int ioctl_extsw_port_mirror_ops(struct ethswctl_data *e)
{
    ...
    if (e->type == TYPE_GET) {
        ...
        if (e->port_mirror_cfg.enable && (e->port_mirror_cfg.ing_pmap || e->port_mirror_cfg.eg_pmap))
        {
...
// ----------- SIOCETHSWCTLOPS ETHSWPDEFVLAN functions ---
int ioctl_extsw_defvlan(struct ethswctl_data *e)
{
    ...
    if (e->type == TYPE_GET) {
        down(&sw_p->s.conf_sem);
        SF2SW_RREG(e->unit, PAGE_8021Q_VLAN, REG_VLAN_DEFAULT_TAG + (e->port*2), &val16, 2);
        up(&sw_p->s.conf_sem);
        e->vid = val16;
    } else {
        val16 = (uint32_t)e->vid;
        down(&sw_p->s.conf_sem);
        SF2SW_WREG(e->unit, PAGE_8021Q_VLAN, REG_VLAN_DEFAULT_TAG + (e->port*2), &val16, 2);
        up(&sw_p->s.conf_sem);
    }
...
void _enet_arl_dump_ext_multiport_arl(int unit)
{
    ...
    printk("\nSwitch Multiport Address Dump: Function %s\n", enabled? "Enabled": "Disabled");
    if (!enabled) return;
    ...
    printk("Switch Multiport Address Dump Done\n");
}
...
int remove_arl_entry_wrapper(void *ptr)
{
    ...
    for (sw_unit=0; sw_unit < MAX_SWITCHES; sw_unit++) {
        if (!IS_UNIT_SF2(sw_unit)) continue;
        if (!_enet_arl_read_ext(sw_unit, ptr, &vid, &val)) {  // found entry
            uint8_t port = val & 0xf;
...
int ioctl_extsw_port_irc_set(struct ethswctl_data *e)
{
    ...
    if (e->limit == 0) { /* Disable ingress rate control */
        ...
        if ( hg_enable == TRUE)
            val32 |= REG_PN_BUCK1_HG_ENABLE_MASK;
        else
            val32 &= ~REG_PN_BUCK1_HG_ENABLE_MASK;
...
int ioctl_extsw_port_irc_get(struct ethswctl_data *e)
{
    ...
    else
    {
        ...
        if ( val32 & REG_PN_BUCK1_HG_ENABLE_MASK)
        {
            if (rf <= 75)
            {
                e->limit = (rf + 50) * 8 * 5000 / 2;
            }
            else if (rf <= 138)
            {
                e->limit = (rf - 13 ) * 8 * 5000;
            }
            else
            {
                if (rf > 201) rf = 201;
                e->limit = (rf - 76) * 2 * 8 * 5000;
            }
        }
        else
        {
...
int ioctl_extsw_snoop_ctrl(struct ethswctl_data *e, char *buf, int usz)
{
    ...
    {
        v32 = e->snoopCtrl.val;
        SF2SW_WREG(e->unit, PAGE_MANAGEMENT, REG_HL_PRTC_CTRL, (uint8 *)&v32, 4);
    }
...
#else
        port_role.sysport = PORT_ON_ROOT_SW(self) ? 0 : self->p.parent_sw->s.parent_port->port_info.port;
