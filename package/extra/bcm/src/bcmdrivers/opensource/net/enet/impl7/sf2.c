...
{
    uint32_t *platform_all_portmap = (uint32_t *)_ctx;
    *platform_all_portmap |= 1<<port->port_info.port;
    return 0;
}

static uint32_t platform_scan_portmap(enetx_port_t *sw, int sw_unit)
{
    static uint32_t platform_all_portmap[MAX_SWITCHES] = {0};
    if (sw_unit < 0 || sw_unit >= MAX_SWITCHES)
        return 0;
    if (sw == 0)
        return platform_all_portmap[sw_unit];

    _port_traverse_ports(sw, tr_all_portmap, PORT_CLASS_PORT, &platform_all_portmap[sw_unit], 1);
    return 0;
}
#define PLATFORM_ALL_PORTMAP(sw_unit) platform_scan_portmap(0, sw_unit)
...
static int tr_imp_ports_op(enetx_port_t *port, void *ctx)
{
    ...
    case IMP_PORTS_SETUP:
        ...
        if (id == IMP_PORT_ID)
            ...
            val16 = (id == P4_PORT_ID) ? LINK_OVERRIDE_1000FDX : LINK_OVERRIDE_1000FDX | REG_PORT_GMII_SPEED_UP_2G;
        SF2SW_WREG(unit, PORT_OVERIDE_PAGE, PORT_OVERIDE_REG(id), &val16, sizeof(val16));
...
static int tr_imp_grp_pmap_get(enetx_port_t *port, void *ctx)
{
    ...
    if (port->has_interface)
        *port_map |= 1 << port->port_info.port;
    return 0; 
}
...
static void _extsw_cfg_port_imp_grouping(int unit, int port_imp_map[])
{
    ...
    for (port = 0; port < MAX_SWITCH_PORTS+1; port++)
    {
        ...
        {
            v16 = _extsw_get_pbvlan(unit, port) & PLATFORM_ALL_PORTMAP(1); /* Get current PBVLAN Map */
...
static void _sf2_tc_to_cos_default(int unit)
{
    ...
    {
        if (!(PLATFORM_ALL_PORTMAP(unit) & (1<<i))) continue; // skip undefined ports
...
static void _sf2_qos_default(int unit)
{
    ...
    {
        if (!(PLATFORM_ALL_PORTMAP(unit) & (1<<port))) continue; // skip undefined ports
...
#if defined(CONFIG_BCM963138) || defined(CONFIG_BCM94908) || defined(CONFIG_BCM963158)
#define ACB_ALG2_OPTION     1   //  chips after 63158 always use ALG2 so no more option
#endif
...
#if defined(ACB_ALGORITHM2) && defined(ACB_ALG2_OPTION)
...
static int _sf2_prio_control(struct ethswctl_data *e)
{
    ...
    } else if ((e->port < SF2_IMP0_PORT) && (PLATFORM_ALL_PORTMAP(e->unit) & (1<<e->port))) {
...
#if defined(ARCHER_DEVICE)
static int tr_set_port_num_of_q(enetx_port_t *port, void *ctxt)
{
    int ndx;
...
#if defined(PORT_WITH_8TXQ)
        if ((port->port_info.port == PORT_WITH_8TXQ) && PORT_ON_ROOT_SW(port) &&
            (port->n.port_netdev_role == PORT_NETDEV_ROLE_WAN)) {
            port->n.flags |= PORT_FLAG_8_TXQS;
        } else
#endif
            port->n.flags &= ~PORT_FLAG_8_TXQS;
    }
...
int bcmenet_sysport_q_map(uint32_t queRemap)
{
    ...
    port_traverse_ports(root_sw, tr_set_port_num_of_q, PORT_CLASS_PORT, NULL);
...
int ioctl_extsw_cosq_port_mapping(struct ethswctl_data *e)
{
    ...
    if (e->port >= TOTAL_SWITCH_PORTS || !(PLATFORM_ALL_PORTMAP(e->unit) & (1<<e->port))) {
...
int ioctl_extsw_cfg_acb(struct ethswctl_data *e)
{
    ...
    uint32_t val32, val;
    acb_q_params_t acb_conf_info = {0};
    volatile uint32_t *sf2_acb_control    = (void *) (SF2_ACB_CONTROL_REG);
    volatile uint32_t *sf2_acb_xon_thresh = (void *) (sf2_acb_control + 1);
    volatile uint32_t *sf2_acb_que_config = (void *) (sf2_acb_control + 2);
#if defined(ACB_ALGORITHM2)   // For 138, Algorithm2 is applied
    ...
    if (e->type == TYPE_GET) {
        switch (e->sw_ctrl_type) {
            ...
            case acb_parms_all:
                ...
                break;
            case 100:
                if (e->unit >= 0 && e->unit < MAX_SWITCHES)
...
int ioctl_extsw_cos_priority_method_cfg(struct ethswctl_data *e)
{
    ...
    if (e->port < 0 || e->port > SF2_IMP0_PORT ||  !(PLATFORM_ALL_PORTMAP(e->unit) & (1<<e->port))) {
...
#if defined(SF2_DUAL)
int sf2_sw_ext_txrate[MAX_SWITCH_PORTS];    // in kbits
...
int ioctl_extsw_port_erc_config(struct ethswctl_data *e)
{
    ...
    if (e->unit<0 || e->unit>=ARRAY_SIZE(unit_port_array) || 
        e->port<0 || e->port>=ARRAY_SIZE(unit_port_array[0]) || 
        !unit_port_array[e->unit][e->port])
        return BCM_E_ERROR;
...
#if defined(SF2_DUAL)
        {
            enetx_port_t *port = unit_port_array[e->unit][e->port];
            // if byte shaping port on ext sw, need to update interconnect port tx q shaping
            if (port && e->unit && e->port + LS_PORT_TXQ_OFFSET < 8 &&
                (!e->limit || pkt_flag != SHAPER_PACKET_MODE)) {
                sf2_sw_ext_txrate[e->port] = e->limit;
                port_sf2_dual_tx_shape(port, port->p.phy->speed);
            }
        }
#endif
        return 0;
...
{
    if (self->p.mac && self->port_info.port == IMP_PORT_ID && self->dev != NULL)
    {
        netif_carrier_off(self->dev);
        printk((KERN_CRIT "%s (%s switch port: %d) (Logical Port: %d) Virtual link DOWN\n"),
               self->dev->name, PORT_ON_ROOT_SW(self)?"Int":"Ext", self->port_info.port, 
               PHYSICAL_PORT_TO_LOGICAL_PORT(self->port_info.port, PORT_ON_ROOT_SW(self)?0:1));
        return 1;
    }
    return 0;
}
...
static int tr_fill_sysp_conf(enetx_port_t *port, void *ctxt)
{
    ...
    if (port->has_interface) {
...
#if defined(PORT_WITH_8TXQ)
        ch->nbr_of_queues = (port->port_info.port == PORT_WITH_8TXQ) ? 8 : 4;
#else
        ch->nbr_of_queues = 4;
#endif
#elif defined(CONFIG_BCM947622)
        if (PORT_ON_ROOT_SW(port)) {
            ch->sysport = port->port_info.port;
            ch->port = 0;
        } else {
            ch->sysport = port->p.parent_sw->s.parent_port->port_info.port;
            ch->port = port->port_info.port;
        }
        ch->switch_id = 0;
        ch->nbr_of_queues = 8;
        ...
        ch->port = port->port_info.port;
#if defined(PORT_WITH_8TXQ)
        ch->nbr_of_queues = (port->port_info.port == PORT_WITH_8TXQ && PORT_ON_ROOT_SW(port)) ? 8 : 4;
#else
        ch->nbr_of_queues = 4;
#endif
#else
...
int port_sf2_sw_init(enetx_port_t *self)
{
    ...
    platform_scan_portmap(self, unit);
    ...
    // set lightstacking port to L2 rate for 6756
    if ((unit == 0) && sf2_sw_ext)
        val16 &= ~(1<< sf2_sw_ext->s.parent_port->port_info.port); 
#endif
...
int port_sf2_sw_config_trunk(enetx_port_t *sw, enetx_port_t *port, int grp_no, int add)
{
    ...
    if (add)
    {
        v16 |= ( ( (1<< port->port_info.port) & TRUNK_EN_GRP_M ) << TRUNK_EN_GRP_S );
...
void port_sf2_dual_tx_shape(enetx_port_t *port, int speed)
{
    if (port->p.parent_sw == sf2_sw_ext)
    {
        ...
        e.unit = 0;
        e.port = sf2_sw_ext->s.parent_port->port_info.port;
        e.queue = port->port_info.port + LS_PORT_TXQ_OFFSET;
        ...
        e.limit = speed * 1000;
        // also check current port shaping value, if lower use lower value
        if (sf2_sw_ext_txrate[port->port_info.port] && e.limit > sf2_sw_ext_txrate[port->port_info.port])
            e.limit = sf2_sw_ext_txrate[port->port_info.port];
    ...
    // 6765 XPHY XGMII mode workaround interop issue
    if (PORT_ON_ROOT_SW(port) && (port->port_info.port == 0)) {
        volatile uint32_t *reg_p = (volatile uint32_t *) SWITCH_LFH_PORT0_CONTROL_REG;

        if (speed && !phy_dev_is_xgmii_mode(port->p.phy))
            *reg_p &= ~LFH_XGMII_SEL;
        else
            *reg_p |= LFH_XGMII_SEL;
        *reg_p |= LFH_XGMII_SEL_OVRD;
    }
#endif
}
#endif //SF2_DUAL
