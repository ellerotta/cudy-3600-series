...
static int of_port_probe(struct device *dev, enetx_port_t *sw, struct device_node *port_dn, int unit, int compat_port)
{
    ...
    ret = of_get_mac_address_mtd(port_dn, maddr_mtd);
    if (!ret && is_valid_ether_addr(maddr_mtd))
    {
        p->port_info.is_macvalid = 1;
        memcpy(p->port_info.macddr, maddr_mtd, ETH_ALEN);
    }
