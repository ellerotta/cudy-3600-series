...
#ifdef CONFIG_BCM_MACSEC
extern const struct macsec_ops enet_macsec_ops;
#endif
...
int port_mac_phy_init(enetx_port_t *port)
{
    ...
    {
        port->p.mac = port->p.delayed_mac;
        port->p.phy = port->p.delayed_phy;
        port->p.delayed_mac = NULL;
        port->p.delayed_phy = NULL;
    }

    if (port->p.mac)
    {
        if ((rc = (mac_dev_init(port->p.mac) ||
            mac_dev_eee_set(port->p.mac, 0))))
        {
    ...
    if (port->p.phy)
    {
        ...
        phy_dev_leds_init(port->p.phy, port->p.leds_info);
#endif

        if (port->p.handle_phy_link_change)
        {
#if defined(DSL_DEVICES)
            if (!phy_is_pon_wan_ae_serdes(port->p.phy) && !phy_is_pon_wan_ae_serdes(port->p.phy->cascade_prev) && 
                port->p.phy->phy_drv->phy_type != PHY_TYPE_PON)
...
int port_init(enetx_port_t *port)
{
    ...
    if (port->is_init)
    {
        enet_dbg("port object already initialized: %s\n", port->obj_name);
        return EALREADY;
    }
    else
    {
        enet_dbgv("init %s, port->class=%d, port->has_interface=%d, port->p.ops=%p\n",
            port->obj_name, port->port_class, port->has_interface, port->p.ops);
    }
    ...
    rc = _assert_class_port_defaults(port);
    if (rc)
        goto exit;
    ...
    if (!port->p.delayed_mac)
    {
        if ((rc = port_mac_phy_init(port)))
            goto exit;
    }

    if (port->p.ops->post_init)
    {
        if ((rc = port->p.ops->post_init(port)))
            goto exit;
    }
...
static int port_role_set(enetx_port_t *self, port_netdev_role_t role)
{
    if (!self->dev)
    {
...
int port_generic_mtu_set(enetx_port_t *self, int mtu)
{
    ...
    if (self->p.mac)
        return mac_dev_mtu_set(self->p.mac, mtu + ENET_MAX_MTU_EXTRA_SIZE);
...
void port_generic_stats_get(enetx_port_t *self, struct rtnl_link_stats64 *net_stats)
{
    ...
    if (!self->p.mac || !self->p.mac->enabled)
        return;
...
int port_generic_pause_get(enetx_port_t *self, int *rx_enable, int *tx_enable)
{
    ...
    if (phy && phy->phy_drv->caps_get && !PhyIsPortConnectedToExternalSwitch(phy) && !PhyIsFixedConnection(phy))
    {
        uint32_t caps;
...
int port_generic_pause_set(enetx_port_t *self, int rx_enable, int tx_enable)
{
    ...
    if (phy && phy->phy_drv->caps_set && !PhyIsPortConnectedToExternalSwitch(phy) && !PhyIsFixedConnection(phy))
    {
