...
void port_print_status_verbose(enetx_port_t *p)
{
    ...
    if (phy->link)
    {
		...
		gmac_hotplug_event(PHYSICAL_PORT_TO_LOGICAL_PORT(p->port_info.port, PORT_ON_ROOT_SW(p)?0:1),1);
		printk("polling: port %d link up\n", PHYSICAL_PORT_TO_LOGICAL_PORT(p->port_info.port, PORT_ON_ROOT_SW(p)?0:1));
    }
    else
    {
        printk((KERN_CRIT "%s (%s switch port: %d) (Logical Port: %d) (phyId: %x) Link DOWN.\n"),
                p->dev->name, PORT_ON_ROOT_SW(p)?"Int":"Ext", p->port_info.port, 
                    PHYSICAL_PORT_TO_LOGICAL_PORT(p->port_info.port, PORT_ON_ROOT_SW(p)?0:1), phy->addr);
        gmac_hotplug_event(PHYSICAL_PORT_TO_LOGICAL_PORT(p->port_info.port, PORT_ON_ROOT_SW(p)?0:1),0);
		printk("polling: port %d link down\n", PHYSICAL_PORT_TO_LOGICAL_PORT(p->port_info.port, PORT_ON_ROOT_SW(p)?0:1));
    }
}
