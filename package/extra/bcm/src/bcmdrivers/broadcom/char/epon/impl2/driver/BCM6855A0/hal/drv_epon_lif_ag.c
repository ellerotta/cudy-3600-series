...
bdmf_error_t ag_drv_lif_pon_control_set(const lif_pon_control *pon_control)
{
    ...
    if((pon_control->cfg_lif_sgmii_mode >= _2BITS_MAX_VAL_) ||
       (pon_control->cfgdisruntfilter >= _1BITS_MAX_VAL_) ||
    ...
    reg_pon_control = RU_FIELD_SET(0, LIF, PON_CONTROL, CFG_LIF_SGMII_MODE, reg_pon_control, pon_control->cfg_lif_sgmii_mode);
...
bdmf_error_t ag_drv_lif_pon_control_get(lif_pon_control *pon_control)
{
    ...
    pon_control->cfg_lif_sgmii_mode = RU_FIELD_GET(0, LIF, PON_CONTROL, CFG_LIF_SGMII_MODE, reg_pon_control);
...
bdmf_error_t ag_drv_lif_int_status_set(const lif_int_status *int_status)
{
    ...
    if((int_status->sgmiiabortfrint >= _1BITS_MAX_VAL_) ||
       (int_status->int_sop_sfec_ipg_violation >= _1BITS_MAX_VAL_) ||
    ...
    reg_int_status = RU_FIELD_SET(0, LIF, INT_STATUS, SGMIIABORTFRINT, reg_int_status, int_status->sgmiiabortfrint);
...
bdmf_error_t ag_drv_lif_int_status_get(lif_int_status *int_status)
{
    ...
    int_status->sgmiiabortfrint = RU_FIELD_GET(0, LIF, INT_STATUS, SGMIIABORTFRINT, reg_int_status);
...
bdmf_error_t ag_drv_lif_int_mask_set(const lif_int_mask *int_mask)
{
    ...
    if((int_mask->sgmiiabortfrintmask >= _1BITS_MAX_VAL_) ||
       (int_mask->int_sop_sfec_ipg_violation_mask >= _1BITS_MAX_VAL_) ||
    ...
    reg_int_mask = RU_FIELD_SET(0, LIF, INT_MASK, SGMIIABORTFRINTMASK, reg_int_mask, int_mask->sgmiiabortfrintmask);
...
bdmf_error_t ag_drv_lif_int_mask_get(lif_int_mask *int_mask)
{
    ...
    int_mask->sgmiiabortfrintmask = RU_FIELD_GET(0, LIF, INT_MASK, SGMIIABORTFRINTMASK, reg_int_mask);
...
bdmf_error_t ag_drv_lif_sec_key_sel_get(uint8_t link_idx, bdmf_boolean *data)
{
    ...
    *data = RDP_FIELD_GET(reg_sec_key_sel, (link_idx % 32) *1, 0x1);
...
bdmf_error_t ag_drv_lif_dn_encrypt_stat_set(uint8_t link_idx, bdmf_boolean enEncrypt)
{
    ...
    RDP_FIELD_SET(reg_dn_encrypt_stat, (link_idx % 32) *1, 0x1, enEncrypt);
...
bdmf_error_t ag_drv_lif_dn_encrypt_stat_get(uint8_t link_idx, bdmf_boolean *enEncrypt)
{
    ...
    *enEncrypt = RDP_FIELD_GET(reg_dn_encrypt_stat, (link_idx % 32) *1, 0x1);
...
bdmf_error_t ag_drv_lif_sec_up_key_stat_get(uint8_t link_idx, bdmf_boolean *keyUpSel)
{
    ...
    *keyUpSel = RDP_FIELD_GET(reg_sec_up_key_stat, (link_idx % 32) *1, 0x1);
...
bdmf_error_t ag_drv_lif_sec_up_encrypt_stat_get(uint8_t link_idx, bdmf_boolean *enUpEncrypt)
{
    ...
    *enUpEncrypt = RDP_FIELD_GET(reg_sec_up_encrypt_stat, (link_idx % 32) *1, 0x1);
...
bdmf_error_t ag_drv_lif_fec_per_llid_set(uint8_t link_idx, bdmf_boolean cfFecTxFecEnLlid)
{
    ...
    RDP_FIELD_SET(reg_fec_per_llid, (link_idx % 32) *1, 0x1, cfFecTxFecEnLlid);
...
bdmf_error_t ag_drv_lif_fec_per_llid_get(uint8_t link_idx, bdmf_boolean *cfFecTxFecEnLlid)
{
    ...
    *cfFecTxFecEnLlid = RDP_FIELD_GET(reg_fec_per_llid, (link_idx % 32) *1, 0x1);
...
bdmf_error_t ag_drv_lif_p2p_inter_op_control_set(bdmf_boolean cfp2pmodetransmitremfault, bdmf_boolean cfp2pmodetransmitdeficitidleenable, bdmf_boolean cfp2pmodeadaptiveshortpre)
{
    uint32_t reg_p2p_inter_op_control=0;

#ifdef VALIDATE_PARMS
    if((cfp2pmodetransmitremfault >= _1BITS_MAX_VAL_) ||
       (cfp2pmodetransmitdeficitidleenable >= _1BITS_MAX_VAL_) ||
       (cfp2pmodeadaptiveshortpre >= _1BITS_MAX_VAL_))
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_RANGE), BDMF_ERR_RANGE);
        return BDMF_ERR_RANGE;
    }
    ...
    reg_p2p_inter_op_control = RU_FIELD_SET(0, LIF, P2P_INTER_OP_CONTROL, CFP2PMODETRANSMITREMFAULT, reg_p2p_inter_op_control, cfp2pmodetransmitremfault);
    reg_p2p_inter_op_control = RU_FIELD_SET(0, LIF, P2P_INTER_OP_CONTROL, CFP2PMODETRANSMITDEFICITIDLEENABLE, reg_p2p_inter_op_control, cfp2pmodetransmitdeficitidleenable);
    reg_p2p_inter_op_control = RU_FIELD_SET(0, LIF, P2P_INTER_OP_CONTROL, CFP2PMODEADAPTIVESHORTPRE, reg_p2p_inter_op_control, cfp2pmodeadaptiveshortpre);

    RU_REG_WRITE(0, LIF, P2P_INTER_OP_CONTROL, reg_p2p_inter_op_control);

    return BDMF_ERR_OK;
}

bdmf_error_t ag_drv_lif_p2p_inter_op_control_get(bdmf_boolean *cfp2pmodetransmitremfault, bdmf_boolean *cfp2pmodetransmitdeficitidleenable, bdmf_boolean *cfp2pmodeadaptiveshortpre)
{
    uint32_t reg_p2p_inter_op_control=0;

#ifdef VALIDATE_PARMS
    if(!cfp2pmodetransmitremfault || !cfp2pmodetransmitdeficitidleenable || !cfp2pmodeadaptiveshortpre)
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_PARM), BDMF_ERR_PARM);
        return BDMF_ERR_PARM;
    }
    ...
    RU_REG_READ(0, LIF, P2P_INTER_OP_CONTROL, reg_p2p_inter_op_control);

    *cfp2pmodetransmitremfault = RU_FIELD_GET(0, LIF, P2P_INTER_OP_CONTROL, CFP2PMODETRANSMITREMFAULT, reg_p2p_inter_op_control);
    *cfp2pmodetransmitdeficitidleenable = RU_FIELD_GET(0, LIF, P2P_INTER_OP_CONTROL, CFP2PMODETRANSMITDEFICITIDLEENABLE, reg_p2p_inter_op_control);
    *cfp2pmodeadaptiveshortpre = RU_FIELD_GET(0, LIF, P2P_INTER_OP_CONTROL, CFP2PMODEADAPTIVESHORTPRE, reg_p2p_inter_op_control);
...
{
    uint32_t reg_pon_control_2=0;

#ifdef VALIDATE_PARMS
    if((cfgdisp2pdnindexmap >= _1BITS_MAX_VAL_) ||
    ...
    reg_pon_control_2 = RU_FIELD_SET(0, LIF, PON_CONTROL_2, CFGDISP2PDNINDEXMAP, reg_pon_control_2, cfgdisp2pdnindexmap);
    reg_pon_control_2 = RU_FIELD_SET(0, LIF, PON_CONTROL_2, CFGDISP2PUPINDEXMAP, reg_pon_control_2, cfgdisp2pupindexmap);

    RU_REG_WRITE(0, LIF, PON_CONTROL_2, reg_pon_control_2);
...
bdmf_error_t ag_drv_lif_pon_control_2_get(bdmf_boolean *cfgdisp2pdnindexmap, bdmf_boolean *cfgdisp2pupindexmap)
{
...
#ifdef VALIDATE_PARMS
    if(!cfgdisp2pdnindexmap || !cfgdisp2pupindexmap)
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_PARM), BDMF_ERR_PARM);
        return BDMF_ERR_PARM;
    }
    ...
    RU_REG_READ(0, LIF, PON_CONTROL_2, reg_pon_control_2);

    *cfgdisp2pdnindexmap = RU_FIELD_GET(0, LIF, PON_CONTROL_2, CFGDISP2PDNINDEXMAP, reg_pon_control_2);
    *cfgdisp2pupindexmap = RU_FIELD_GET(0, LIF, PON_CONTROL_2, CFGDISP2PUPINDEXMAP, reg_pon_control_2);
...
bdmf_error_t ag_drv_lif_vlan_type_1_set(uint16_t cfgvlantype_1)
{
    ...
    reg_vlan_type_1 = RU_FIELD_SET(0, LIF, VLAN_TYPE_1, CFGVLANTYPE_1, reg_vlan_type_1, cfgvlantype_1);

    RU_REG_WRITE(0, LIF, VLAN_TYPE_1, reg_vlan_type_1);
...
bdmf_error_t ag_drv_lif_vlan_type_1_get(uint16_t *cfgvlantype_1)
{
    ...
    RU_REG_READ(0, LIF, VLAN_TYPE_1, reg_vlan_type_1);

    *cfgvlantype_1 = RU_FIELD_GET(0, LIF, VLAN_TYPE_1, CFGVLANTYPE_1, reg_vlan_type_1);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_16_set(uint32_t cfgp2psci_lo_16)
{
    ...
    reg_p2p_ae_sci_lo_16 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_LO_16, CFGP2PSCI_LO_16, reg_p2p_ae_sci_lo_16, cfgp2psci_lo_16);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_LO_16, reg_p2p_ae_sci_lo_16);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_16_get(uint32_t *cfgp2psci_lo_16)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_LO_16, reg_p2p_ae_sci_lo_16);

    *cfgp2psci_lo_16 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_LO_16, CFGP2PSCI_LO_16, reg_p2p_ae_sci_lo_16);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_16_set(uint32_t cfgp2psci_hi_16)
{
    ...
    reg_p2p_ae_sci_hi_16 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_HI_16, CFGP2PSCI_HI_16, reg_p2p_ae_sci_hi_16, cfgp2psci_hi_16);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_HI_16, reg_p2p_ae_sci_hi_16);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_16_get(uint32_t *cfgp2psci_hi_16)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_HI_16, reg_p2p_ae_sci_hi_16);

    *cfgp2psci_hi_16 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_HI_16, CFGP2PSCI_HI_16, reg_p2p_ae_sci_hi_16);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_17_set(uint32_t cfgp2psci_lo_17)
{
    ...
    reg_p2p_ae_sci_lo_17 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_LO_17, CFGP2PSCI_LO_17, reg_p2p_ae_sci_lo_17, cfgp2psci_lo_17);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_LO_17, reg_p2p_ae_sci_lo_17);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_17_get(uint32_t *cfgp2psci_lo_17)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_LO_17, reg_p2p_ae_sci_lo_17);

    *cfgp2psci_lo_17 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_LO_17, CFGP2PSCI_LO_17, reg_p2p_ae_sci_lo_17);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_17_set(uint32_t cfgp2psci_hi_17)
{
    ...
    reg_p2p_ae_sci_hi_17 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_HI_17, CFGP2PSCI_HI_17, reg_p2p_ae_sci_hi_17, cfgp2psci_hi_17);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_HI_17, reg_p2p_ae_sci_hi_17);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_17_get(uint32_t *cfgp2psci_hi_17)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_HI_17, reg_p2p_ae_sci_hi_17);

    *cfgp2psci_hi_17 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_HI_17, CFGP2PSCI_HI_17, reg_p2p_ae_sci_hi_17);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_18_set(uint32_t cfgp2psci_lo_18)
{
    ...
    reg_p2p_ae_sci_lo_18 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_LO_18, CFGP2PSCI_LO_18, reg_p2p_ae_sci_lo_18, cfgp2psci_lo_18);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_LO_18, reg_p2p_ae_sci_lo_18);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_18_get(uint32_t *cfgp2psci_lo_18)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_LO_18, reg_p2p_ae_sci_lo_18);

    *cfgp2psci_lo_18 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_LO_18, CFGP2PSCI_LO_18, reg_p2p_ae_sci_lo_18);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_18_set(uint32_t cfgp2psci_hi_18)
{
    ...
    reg_p2p_ae_sci_hi_18 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_HI_18, CFGP2PSCI_HI_18, reg_p2p_ae_sci_hi_18, cfgp2psci_hi_18);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_HI_18, reg_p2p_ae_sci_hi_18);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_18_get(uint32_t *cfgp2psci_hi_18)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_HI_18, reg_p2p_ae_sci_hi_18);

    *cfgp2psci_hi_18 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_HI_18, CFGP2PSCI_HI_18, reg_p2p_ae_sci_hi_18);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_19_set(uint32_t cfgp2psci_lo_19)
{
    ...
    reg_p2p_ae_sci_lo_19 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_LO_19, CFGP2PSCI_LO_19, reg_p2p_ae_sci_lo_19, cfgp2psci_lo_19);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_LO_19, reg_p2p_ae_sci_lo_19);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_19_get(uint32_t *cfgp2psci_lo_19)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_LO_19, reg_p2p_ae_sci_lo_19);

    *cfgp2psci_lo_19 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_LO_19, CFGP2PSCI_LO_19, reg_p2p_ae_sci_lo_19);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_19_set(uint32_t cfgp2psci_hi_19)
{
    ...
    reg_p2p_ae_sci_hi_19 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_HI_19, CFGP2PSCI_HI_19, reg_p2p_ae_sci_hi_19, cfgp2psci_hi_19);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_HI_19, reg_p2p_ae_sci_hi_19);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_19_get(uint32_t *cfgp2psci_hi_19)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_HI_19, reg_p2p_ae_sci_hi_19);

    *cfgp2psci_hi_19 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_HI_19, CFGP2PSCI_HI_19, reg_p2p_ae_sci_hi_19);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_20_set(uint32_t cfgp2psci_lo_20)
{
    ...
    reg_p2p_ae_sci_lo_20 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_LO_20, CFGP2PSCI_LO_20, reg_p2p_ae_sci_lo_20, cfgp2psci_lo_20);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_LO_20, reg_p2p_ae_sci_lo_20);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_20_get(uint32_t *cfgp2psci_lo_20)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_LO_20, reg_p2p_ae_sci_lo_20);

    *cfgp2psci_lo_20 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_LO_20, CFGP2PSCI_LO_20, reg_p2p_ae_sci_lo_20);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_20_set(uint32_t cfgp2psci_hi_20)
{
    ...
    reg_p2p_ae_sci_hi_20 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_HI_20, CFGP2PSCI_HI_20, reg_p2p_ae_sci_hi_20, cfgp2psci_hi_20);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_HI_20, reg_p2p_ae_sci_hi_20);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_20_get(uint32_t *cfgp2psci_hi_20)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_HI_20, reg_p2p_ae_sci_hi_20);

    *cfgp2psci_hi_20 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_HI_20, CFGP2PSCI_HI_20, reg_p2p_ae_sci_hi_20);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_21_set(uint32_t cfgp2psci_lo_21)
{
    ...
    reg_p2p_ae_sci_lo_21 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_LO_21, CFGP2PSCI_LO_21, reg_p2p_ae_sci_lo_21, cfgp2psci_lo_21);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_LO_21, reg_p2p_ae_sci_lo_21);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_21_get(uint32_t *cfgp2psci_lo_21)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_LO_21, reg_p2p_ae_sci_lo_21);

    *cfgp2psci_lo_21 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_LO_21, CFGP2PSCI_LO_21, reg_p2p_ae_sci_lo_21);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_21_set(uint32_t cfgp2psci_hi_21)
{
    ...
    reg_p2p_ae_sci_hi_21 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_HI_21, CFGP2PSCI_HI_21, reg_p2p_ae_sci_hi_21, cfgp2psci_hi_21);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_HI_21, reg_p2p_ae_sci_hi_21);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_21_get(uint32_t *cfgp2psci_hi_21)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_HI_21, reg_p2p_ae_sci_hi_21);

    *cfgp2psci_hi_21 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_HI_21, CFGP2PSCI_HI_21, reg_p2p_ae_sci_hi_21);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_22_set(uint32_t cfgp2psci_lo_22)
{
    ...
    reg_p2p_ae_sci_lo_22 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_LO_22, CFGP2PSCI_LO_22, reg_p2p_ae_sci_lo_22, cfgp2psci_lo_22);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_LO_22, reg_p2p_ae_sci_lo_22);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_22_get(uint32_t *cfgp2psci_lo_22)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_LO_22, reg_p2p_ae_sci_lo_22);

    *cfgp2psci_lo_22 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_LO_22, CFGP2PSCI_LO_22, reg_p2p_ae_sci_lo_22);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_22_set(uint32_t cfgp2psci_hi_22)
{
    ...
    reg_p2p_ae_sci_hi_22 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_HI_22, CFGP2PSCI_HI_22, reg_p2p_ae_sci_hi_22, cfgp2psci_hi_22);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_HI_22, reg_p2p_ae_sci_hi_22);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_22_get(uint32_t *cfgp2psci_hi_22)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_HI_22, reg_p2p_ae_sci_hi_22);

    *cfgp2psci_hi_22 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_HI_22, CFGP2PSCI_HI_22, reg_p2p_ae_sci_hi_22);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_23_set(uint32_t cfgp2psci_lo_23)
{
    ...
    reg_p2p_ae_sci_lo_23 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_LO_23, CFGP2PSCI_LO_23, reg_p2p_ae_sci_lo_23, cfgp2psci_lo_23);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_LO_23, reg_p2p_ae_sci_lo_23);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_23_get(uint32_t *cfgp2psci_lo_23)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_LO_23, reg_p2p_ae_sci_lo_23);

    *cfgp2psci_lo_23 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_LO_23, CFGP2PSCI_LO_23, reg_p2p_ae_sci_lo_23);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_23_set(uint32_t cfgp2psci_hi_23)
{
    ...
    reg_p2p_ae_sci_hi_23 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_HI_23, CFGP2PSCI_HI_23, reg_p2p_ae_sci_hi_23, cfgp2psci_hi_23);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_HI_23, reg_p2p_ae_sci_hi_23);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_23_get(uint32_t *cfgp2psci_hi_23)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_HI_23, reg_p2p_ae_sci_hi_23);

    *cfgp2psci_hi_23 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_HI_23, CFGP2PSCI_HI_23, reg_p2p_ae_sci_hi_23);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_24_set(uint32_t cfgp2psci_lo_24)
{
    ...
    reg_p2p_ae_sci_lo_24 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_LO_24, CFGP2PSCI_LO_24, reg_p2p_ae_sci_lo_24, cfgp2psci_lo_24);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_LO_24, reg_p2p_ae_sci_lo_24);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_24_get(uint32_t *cfgp2psci_lo_24)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_LO_24, reg_p2p_ae_sci_lo_24);

    *cfgp2psci_lo_24 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_LO_24, CFGP2PSCI_LO_24, reg_p2p_ae_sci_lo_24);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_24_set(uint32_t cfgp2psci_hi_24)
{
    ...
    reg_p2p_ae_sci_hi_24 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_HI_24, CFGP2PSCI_HI_24, reg_p2p_ae_sci_hi_24, cfgp2psci_hi_24);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_HI_24, reg_p2p_ae_sci_hi_24);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_24_get(uint32_t *cfgp2psci_hi_24)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_HI_24, reg_p2p_ae_sci_hi_24);

    *cfgp2psci_hi_24 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_HI_24, CFGP2PSCI_HI_24, reg_p2p_ae_sci_hi_24);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_25_set(uint32_t cfgp2psci_lo_25)
{
    ...
    reg_p2p_ae_sci_lo_25 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_LO_25, CFGP2PSCI_LO_25, reg_p2p_ae_sci_lo_25, cfgp2psci_lo_25);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_LO_25, reg_p2p_ae_sci_lo_25);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_25_get(uint32_t *cfgp2psci_lo_25)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_LO_25, reg_p2p_ae_sci_lo_25);

    *cfgp2psci_lo_25 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_LO_25, CFGP2PSCI_LO_25, reg_p2p_ae_sci_lo_25);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_25_set(uint32_t cfgp2psci_hi_25)
{
    ...
    reg_p2p_ae_sci_hi_25 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_HI_25, CFGP2PSCI_HI_25, reg_p2p_ae_sci_hi_25, cfgp2psci_hi_25);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_HI_25, reg_p2p_ae_sci_hi_25);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_25_get(uint32_t *cfgp2psci_hi_25)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_HI_25, reg_p2p_ae_sci_hi_25);

    *cfgp2psci_hi_25 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_HI_25, CFGP2PSCI_HI_25, reg_p2p_ae_sci_hi_25);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_26_set(uint32_t cfgp2psci_lo_26)
{
    ...
    reg_p2p_ae_sci_lo_26 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_LO_26, CFGP2PSCI_LO_26, reg_p2p_ae_sci_lo_26, cfgp2psci_lo_26);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_LO_26, reg_p2p_ae_sci_lo_26);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_26_get(uint32_t *cfgp2psci_lo_26)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_LO_26, reg_p2p_ae_sci_lo_26);

    *cfgp2psci_lo_26 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_LO_26, CFGP2PSCI_LO_26, reg_p2p_ae_sci_lo_26);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_26_set(uint32_t cfgp2psci_hi_26)
{
    ...
    reg_p2p_ae_sci_hi_26 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_HI_26, CFGP2PSCI_HI_26, reg_p2p_ae_sci_hi_26, cfgp2psci_hi_26);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_HI_26, reg_p2p_ae_sci_hi_26);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_26_get(uint32_t *cfgp2psci_hi_26)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_HI_26, reg_p2p_ae_sci_hi_26);

    *cfgp2psci_hi_26 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_HI_26, CFGP2PSCI_HI_26, reg_p2p_ae_sci_hi_26);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_27_set(uint32_t cfgp2psci_lo_27)
{
    ...
    reg_p2p_ae_sci_lo_27 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_LO_27, CFGP2PSCI_LO_27, reg_p2p_ae_sci_lo_27, cfgp2psci_lo_27);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_LO_27, reg_p2p_ae_sci_lo_27);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_27_get(uint32_t *cfgp2psci_lo_27)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_LO_27, reg_p2p_ae_sci_lo_27);

    *cfgp2psci_lo_27 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_LO_27, CFGP2PSCI_LO_27, reg_p2p_ae_sci_lo_27);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_27_set(uint32_t cfgp2psci_hi_27)
{
    ...
    reg_p2p_ae_sci_hi_27 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_HI_27, CFGP2PSCI_HI_27, reg_p2p_ae_sci_hi_27, cfgp2psci_hi_27);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_HI_27, reg_p2p_ae_sci_hi_27);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_27_get(uint32_t *cfgp2psci_hi_27)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_HI_27, reg_p2p_ae_sci_hi_27);

    *cfgp2psci_hi_27 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_HI_27, CFGP2PSCI_HI_27, reg_p2p_ae_sci_hi_27);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_28_set(uint32_t cfgp2psci_lo_28)
{
    ...
    reg_p2p_ae_sci_lo_28 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_LO_28, CFGP2PSCI_LO_28, reg_p2p_ae_sci_lo_28, cfgp2psci_lo_28);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_LO_28, reg_p2p_ae_sci_lo_28);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_28_get(uint32_t *cfgp2psci_lo_28)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_LO_28, reg_p2p_ae_sci_lo_28);

    *cfgp2psci_lo_28 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_LO_28, CFGP2PSCI_LO_28, reg_p2p_ae_sci_lo_28);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_28_set(uint32_t cfgp2psci_hi_28)
{
    ...
    reg_p2p_ae_sci_hi_28 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_HI_28, CFGP2PSCI_HI_28, reg_p2p_ae_sci_hi_28, cfgp2psci_hi_28);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_HI_28, reg_p2p_ae_sci_hi_28);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_28_get(uint32_t *cfgp2psci_hi_28)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_HI_28, reg_p2p_ae_sci_hi_28);

    *cfgp2psci_hi_28 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_HI_28, CFGP2PSCI_HI_28, reg_p2p_ae_sci_hi_28);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_29_set(uint32_t cfgp2psci_lo_29)
{
    ...
    reg_p2p_ae_sci_lo_29 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_LO_29, CFGP2PSCI_LO_29, reg_p2p_ae_sci_lo_29, cfgp2psci_lo_29);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_LO_29, reg_p2p_ae_sci_lo_29);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_29_get(uint32_t *cfgp2psci_lo_29)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_LO_29, reg_p2p_ae_sci_lo_29);

    *cfgp2psci_lo_29 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_LO_29, CFGP2PSCI_LO_29, reg_p2p_ae_sci_lo_29);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_29_set(uint32_t cfgp2psci_hi_29)
{
    ...
    reg_p2p_ae_sci_hi_29 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_HI_29, CFGP2PSCI_HI_29, reg_p2p_ae_sci_hi_29, cfgp2psci_hi_29);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_HI_29, reg_p2p_ae_sci_hi_29);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_29_get(uint32_t *cfgp2psci_hi_29)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_HI_29, reg_p2p_ae_sci_hi_29);

    *cfgp2psci_hi_29 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_HI_29, CFGP2PSCI_HI_29, reg_p2p_ae_sci_hi_29);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_30_set(uint32_t cfgp2psci_lo_30)
{
    ...
    reg_p2p_ae_sci_lo_30 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_LO_30, CFGP2PSCI_LO_30, reg_p2p_ae_sci_lo_30, cfgp2psci_lo_30);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_LO_30, reg_p2p_ae_sci_lo_30);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_30_get(uint32_t *cfgp2psci_lo_30)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_LO_30, reg_p2p_ae_sci_lo_30);

    *cfgp2psci_lo_30 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_LO_30, CFGP2PSCI_LO_30, reg_p2p_ae_sci_lo_30);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_30_set(uint32_t cfgp2psci_hi_30)
{
    ...
    reg_p2p_ae_sci_hi_30 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_HI_30, CFGP2PSCI_HI_30, reg_p2p_ae_sci_hi_30, cfgp2psci_hi_30);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_HI_30, reg_p2p_ae_sci_hi_30);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_30_get(uint32_t *cfgp2psci_hi_30)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_HI_30, reg_p2p_ae_sci_hi_30);

    *cfgp2psci_hi_30 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_HI_30, CFGP2PSCI_HI_30, reg_p2p_ae_sci_hi_30);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_31_set(uint32_t cfgp2psci_lo_31)
{
    ...
    reg_p2p_ae_sci_lo_31 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_LO_31, CFGP2PSCI_LO_31, reg_p2p_ae_sci_lo_31, cfgp2psci_lo_31);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_LO_31, reg_p2p_ae_sci_lo_31);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_lo_31_get(uint32_t *cfgp2psci_lo_31)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_LO_31, reg_p2p_ae_sci_lo_31);

    *cfgp2psci_lo_31 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_LO_31, CFGP2PSCI_LO_31, reg_p2p_ae_sci_lo_31);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_31_set(uint32_t cfgp2psci_hi_31)
{
    ...
    reg_p2p_ae_sci_hi_31 = RU_FIELD_SET(0, LIF, P2P_AE_SCI_HI_31, CFGP2PSCI_HI_31, reg_p2p_ae_sci_hi_31, cfgp2psci_hi_31);

    RU_REG_WRITE(0, LIF, P2P_AE_SCI_HI_31, reg_p2p_ae_sci_hi_31);
...
bdmf_error_t ag_drv_lif_p2p_ae_sci_hi_31_get(uint32_t *cfgp2psci_hi_31)
{
    ...
    RU_REG_READ(0, LIF, P2P_AE_SCI_HI_31, reg_p2p_ae_sci_hi_31);

    *cfgp2psci_hi_31 = RU_FIELD_GET(0, LIF, P2P_AE_SCI_HI_31, CFGP2PSCI_HI_31, reg_p2p_ae_sci_hi_31);

    return BDMF_ERR_OK;
}

bdmf_error_t ag_drv_lif_sec_control_2_set(bdmf_boolean cfgkeynum_4_an_3, bdmf_boolean cfgkeynum_4_an_2, bdmf_boolean cfgkeynum_4_an_1, bdmf_boolean cfgkeynum_4_an_0)
{
    uint32_t reg_sec_control_2=0;

#ifdef VALIDATE_PARMS
    if((cfgkeynum_4_an_3 >= _1BITS_MAX_VAL_) ||
       (cfgkeynum_4_an_2 >= _1BITS_MAX_VAL_) ||
       (cfgkeynum_4_an_1 >= _1BITS_MAX_VAL_) ||
       (cfgkeynum_4_an_0 >= _1BITS_MAX_VAL_))
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_RANGE), BDMF_ERR_RANGE);
        return BDMF_ERR_RANGE;
    }
    ...
    reg_sec_control_2 = RU_FIELD_SET(0, LIF, SEC_CONTROL_2, CFGKEYNUM_4_AN_3, reg_sec_control_2, cfgkeynum_4_an_3);
    reg_sec_control_2 = RU_FIELD_SET(0, LIF, SEC_CONTROL_2, CFGKEYNUM_4_AN_2, reg_sec_control_2, cfgkeynum_4_an_2);
    reg_sec_control_2 = RU_FIELD_SET(0, LIF, SEC_CONTROL_2, CFGKEYNUM_4_AN_1, reg_sec_control_2, cfgkeynum_4_an_1);
    reg_sec_control_2 = RU_FIELD_SET(0, LIF, SEC_CONTROL_2, CFGKEYNUM_4_AN_0, reg_sec_control_2, cfgkeynum_4_an_0);

    RU_REG_WRITE(0, LIF, SEC_CONTROL_2, reg_sec_control_2);

    return BDMF_ERR_OK;
}

bdmf_error_t ag_drv_lif_sec_control_2_get(bdmf_boolean *cfgkeynum_4_an_3, bdmf_boolean *cfgkeynum_4_an_2, bdmf_boolean *cfgkeynum_4_an_1, bdmf_boolean *cfgkeynum_4_an_0)
{
    uint32_t reg_sec_control_2=0;

#ifdef VALIDATE_PARMS
    if(!cfgkeynum_4_an_3 || !cfgkeynum_4_an_2 || !cfgkeynum_4_an_1 || !cfgkeynum_4_an_0)
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_PARM), BDMF_ERR_PARM);
        return BDMF_ERR_PARM;
    }
    ...
    RU_REG_READ(0, LIF, SEC_CONTROL_2, reg_sec_control_2);

    *cfgkeynum_4_an_3 = RU_FIELD_GET(0, LIF, SEC_CONTROL_2, CFGKEYNUM_4_AN_3, reg_sec_control_2);
    *cfgkeynum_4_an_2 = RU_FIELD_GET(0, LIF, SEC_CONTROL_2, CFGKEYNUM_4_AN_2, reg_sec_control_2);
    *cfgkeynum_4_an_1 = RU_FIELD_GET(0, LIF, SEC_CONTROL_2, CFGKEYNUM_4_AN_1, reg_sec_control_2);
    *cfgkeynum_4_an_0 = RU_FIELD_GET(0, LIF, SEC_CONTROL_2, CFGKEYNUM_4_AN_0, reg_sec_control_2);
...
bdmf_error_t ag_drv_lif_sec_mpcp_offset_set(uint32_t cfgsecdnmpcpoffset)
{
    ...
    reg_sec_mpcp_offset = RU_FIELD_SET(0, LIF, SEC_MPCP_OFFSET, CFGSECDNMPCPOFFSET, reg_sec_mpcp_offset, cfgsecdnmpcpoffset);

    RU_REG_WRITE(0, LIF, SEC_MPCP_OFFSET, reg_sec_mpcp_offset);
...
bdmf_error_t ag_drv_lif_sec_mpcp_offset_get(uint32_t *cfgsecdnmpcpoffset)
{
    ...
    RU_REG_READ(0, LIF, SEC_MPCP_OFFSET, reg_sec_mpcp_offset);

    *cfgsecdnmpcpoffset = RU_FIELD_GET(0, LIF, SEC_MPCP_OFFSET, CFGSECDNMPCPOFFSET, reg_sec_mpcp_offset);
...
typedef enum
{
    ...
    bdmf_address_p2p_inter_op_control,
    bdmf_address_pon_control_2,
...
static int bcm_lif_cli_set(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
        lif_pon_control pon_control = { .cfg_lif_sgmii_mode=parm[1].value.unumber, .cfgdisruntfilter=parm[2].value.unumber, .cfmaxcommaerrcnt=parm[3].value.unumber, .cfsyncsmselect=parm[4].value.unumber, .cfponrxforcenonfecabort=parm[5].value.unumber, .cfponrxforcefecabort=parm[6].value.unumber, .cfgrxdatabitflip=parm[7].value.unumber, .cfgenmpcpoampad=parm[8].value.unumber, .cfgentxidlepkt=parm[9].value.unumber, .cfenablesoftwaresynchold=parm[10].value.unumber, .cfenableextendsync=parm[11].value.unumber, .cfenablequicksync=parm[12].value.unumber, .cfppsen=parm[13].value.unumber, .cfppsclkrbc=parm[14].value.unumber, .cfrx2txlpback=parm[15].value.unumber, .cftx2rxlpback=parm[16].value.unumber, .cftxdataendurlon=parm[17].value.unumber, .cfp2pmode=parm[18].value.unumber, .cfp2pshortpre=parm[19].value.unumber, .cflaseren=parm[20].value.unumber, .cftxlaseron=parm[21].value.unumber, .cftxlaseronacthi=parm[22].value.unumber, .liftxrstn_pre=parm[23].value.unumber, .lifrxrstn_pre=parm[24].value.unumber, .liftxen=parm[25].value.unumber, .lifrxen=parm[26].value.unumber};
    ...
    {
        lif_int_status int_status = { .sgmiiabortfrint=parm[1].value.unumber, .int_sop_sfec_ipg_violation=parm[2].value.unumber, .laseronmax=parm[3].value.unumber, .laseroff=parm[4].value.unumber, .secdnreplayprotctabort=parm[5].value.unumber, .secuppktnumoverflow=parm[6].value.unumber, .intlaseroffdurburst=parm[7].value.unumber, .intrxberthreshexc=parm[8].value.unumber, .intfecrxfecrecvstatus=parm[9].value.unumber, .intfecrxcorerrfifofullstatus=parm[10].value.unumber, .intfecrxcorerrfifounexpempty=parm[11].value.unumber, .intfecbufpopemptypush=parm[12].value.unumber, .intfecbufpopemptynopush=parm[13].value.unumber, .intfecbufpushfull=parm[14].value.unumber, .intuptimefullupdstat=parm[15].value.unumber, .intfroutofalignstat=parm[16].value.unumber, .intgrntstarttimelagstat=parm[17].value.unumber, .intabortrxfrmstat=parm[18].value.unumber, .intnorxclkstat=parm[19].value.unumber, .intrxmaxlenerrstat=parm[20].value.unumber, .intrxerraftalignstat=parm[21].value.unumber, .intrxsynchacqstat=parm[22].value.unumber, .intrxoutofsynchstat=parm[23].value.unumber};
        err = ag_drv_lif_int_status_set(&int_status);
        break;
    }
    case BDMF_int_mask:
    {
        lif_int_mask int_mask = { .sgmiiabortfrintmask=parm[1].value.unumber, .int_sop_sfec_ipg_violation_mask=parm[2].value.unumber, .laseronmaxmask=parm[3].value.unumber, .laseroffmask=parm[4].value.unumber, .secdnreplayprotctabortmsk=parm[5].value.unumber, .secuppktnumoverflowmsk=parm[6].value.unumber, .intlaseroffdurburstmask=parm[7].value.unumber, .intrxberthreshexcmask=parm[8].value.unumber, .intfecrxfecrecvmask=parm[9].value.unumber, .intfecrxcorerrfifofullmask=parm[10].value.unumber, .intfecrxcorerrfifounexpemptymask=parm[11].value.unumber, .intfecbufpopemptypushmask=parm[12].value.unumber, .intfecbufpopemptynopushmask=parm[13].value.unumber, .intfecbufpushfullmask=parm[14].value.unumber, .intuptimefullupdmask=parm[15].value.unumber, .intfroutofalignmask=parm[16].value.unumber, .intgrntstarttimelagmask=parm[17].value.unumber, .intabortrxfrmmask=parm[18].value.unumber, .intnorxclkmask=parm[19].value.unumber, .intrxmaxlenerrmask=parm[20].value.unumber, .intrxerraftalignmask=parm[21].value.unumber, .intrxsynchacqmask=parm[22].value.unumber, .intrxoutofsynchmask=parm[23].value.unumber};
    ...
    case BDMF_p2p_autoneg_ability_config_reg:
        ...
        break;
    case BDMF_p2p_inter_op_control:
        err = ag_drv_lif_p2p_inter_op_control_set(parm[3].value.unumber, parm[4].value.unumber, parm[5].value.unumber);
        break;
    case BDMF_pon_control_2:
        err = ag_drv_lif_pon_control_2_set(parm[3].value.unumber, parm[4].value.unumber);
        break;
    case BDMF_vlan_type_1:
        err = ag_drv_lif_vlan_type_1_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_lo_16:
        err = ag_drv_lif_p2p_ae_sci_lo_16_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_hi_16:
        err = ag_drv_lif_p2p_ae_sci_hi_16_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_lo_17:
        err = ag_drv_lif_p2p_ae_sci_lo_17_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_hi_17:
        err = ag_drv_lif_p2p_ae_sci_hi_17_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_lo_18:
        err = ag_drv_lif_p2p_ae_sci_lo_18_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_hi_18:
        err = ag_drv_lif_p2p_ae_sci_hi_18_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_lo_19:
        err = ag_drv_lif_p2p_ae_sci_lo_19_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_hi_19:
        err = ag_drv_lif_p2p_ae_sci_hi_19_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_lo_20:
        err = ag_drv_lif_p2p_ae_sci_lo_20_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_hi_20:
        err = ag_drv_lif_p2p_ae_sci_hi_20_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_lo_21:
        err = ag_drv_lif_p2p_ae_sci_lo_21_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_hi_21:
        err = ag_drv_lif_p2p_ae_sci_hi_21_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_lo_22:
        err = ag_drv_lif_p2p_ae_sci_lo_22_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_hi_22:
        err = ag_drv_lif_p2p_ae_sci_hi_22_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_lo_23:
        err = ag_drv_lif_p2p_ae_sci_lo_23_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_hi_23:
        err = ag_drv_lif_p2p_ae_sci_hi_23_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_lo_24:
        err = ag_drv_lif_p2p_ae_sci_lo_24_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_hi_24:
        err = ag_drv_lif_p2p_ae_sci_hi_24_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_lo_25:
        err = ag_drv_lif_p2p_ae_sci_lo_25_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_hi_25:
        err = ag_drv_lif_p2p_ae_sci_hi_25_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_lo_26:
        err = ag_drv_lif_p2p_ae_sci_lo_26_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_hi_26:
        err = ag_drv_lif_p2p_ae_sci_hi_26_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_lo_27:
        err = ag_drv_lif_p2p_ae_sci_lo_27_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_hi_27:
        err = ag_drv_lif_p2p_ae_sci_hi_27_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_lo_28:
        err = ag_drv_lif_p2p_ae_sci_lo_28_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_hi_28:
        err = ag_drv_lif_p2p_ae_sci_hi_28_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_lo_29:
        err = ag_drv_lif_p2p_ae_sci_lo_29_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_hi_29:
        err = ag_drv_lif_p2p_ae_sci_hi_29_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_lo_30:
        err = ag_drv_lif_p2p_ae_sci_lo_30_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_hi_30:
        err = ag_drv_lif_p2p_ae_sci_hi_30_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_lo_31:
        err = ag_drv_lif_p2p_ae_sci_lo_31_set(parm[3].value.unumber);
        break;
    case BDMF_p2p_ae_sci_hi_31:
        err = ag_drv_lif_p2p_ae_sci_hi_31_set(parm[3].value.unumber);
        break;
    case BDMF_sec_control_2:
        err = ag_drv_lif_sec_control_2_set(parm[3].value.unumber, parm[4].value.unumber, parm[5].value.unumber, parm[6].value.unumber);
        break;
    case BDMF_sec_mpcp_offset:
        err = ag_drv_lif_sec_mpcp_offset_set(parm[3].value.unumber);
        break;
    default:
...
static int bcm_lif_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case BDMF_pon_control:
    {
        ...
        bdmf_session_print(session, "cfg_lif_sgmii_mode = %u = 0x%x\n", pon_control.cfg_lif_sgmii_mode, pon_control.cfg_lif_sgmii_mode);
    ...
    case BDMF_int_status:
    {
        ...
        bdmf_session_print(session, "sgmiiabortfrint = %u = 0x%x\n", int_status.sgmiiabortfrint, int_status.sgmiiabortfrint);
    ...
    case BDMF_int_mask:
    {
        ...
        bdmf_session_print(session, "sgmiiabortfrintmask = %u = 0x%x\n", int_mask.sgmiiabortfrintmask, int_mask.sgmiiabortfrintmask);
    ...
    case BDMF_p2p_inter_op_control:
    {
        bdmf_boolean cfp2pmodetransmitremfault;
        bdmf_boolean cfp2pmodetransmitdeficitidleenable;
        bdmf_boolean cfp2pmodeadaptiveshortpre;
        err = ag_drv_lif_p2p_inter_op_control_get(&cfp2pmodetransmitremfault, &cfp2pmodetransmitdeficitidleenable, &cfp2pmodeadaptiveshortpre);
        bdmf_session_print(session, "cfp2pmodetransmitremfault = %u = 0x%x\n", cfp2pmodetransmitremfault, cfp2pmodetransmitremfault);
        bdmf_session_print(session, "cfp2pmodetransmitdeficitidleenable = %u = 0x%x\n", cfp2pmodetransmitdeficitidleenable, cfp2pmodetransmitdeficitidleenable);
        bdmf_session_print(session, "cfp2pmodeadaptiveshortpre = %u = 0x%x\n", cfp2pmodeadaptiveshortpre, cfp2pmodeadaptiveshortpre);
        break;
    }
    case BDMF_pon_control_2:
    {
        ...
        err = ag_drv_lif_pon_control_2_get(&cfgdisp2pdnindexmap, &cfgdisp2pupindexmap);
    ...
    case BDMF_vlan_type_1:
    {
        ...
        err = ag_drv_lif_vlan_type_1_get(&cfgvlantype_1);
    ...
    case BDMF_p2p_ae_sci_lo_16:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_lo_16_get(&cfgp2psci_lo_16);
    ...
    case BDMF_p2p_ae_sci_hi_16:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_hi_16_get(&cfgp2psci_hi_16);
    ...
    case BDMF_p2p_ae_sci_lo_17:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_lo_17_get(&cfgp2psci_lo_17);
    ...
    case BDMF_p2p_ae_sci_hi_17:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_hi_17_get(&cfgp2psci_hi_17);
    ...
    case BDMF_p2p_ae_sci_lo_18:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_lo_18_get(&cfgp2psci_lo_18);
    ...
    case BDMF_p2p_ae_sci_hi_18:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_hi_18_get(&cfgp2psci_hi_18);
    ...
    case BDMF_p2p_ae_sci_lo_19:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_lo_19_get(&cfgp2psci_lo_19);
    ...
    case BDMF_p2p_ae_sci_hi_19:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_hi_19_get(&cfgp2psci_hi_19);
    ...
    case BDMF_p2p_ae_sci_lo_20:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_lo_20_get(&cfgp2psci_lo_20);
    ...
    case BDMF_p2p_ae_sci_hi_20:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_hi_20_get(&cfgp2psci_hi_20);
    ...
    case BDMF_p2p_ae_sci_lo_21:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_lo_21_get(&cfgp2psci_lo_21);
    ...
    case BDMF_p2p_ae_sci_hi_21:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_hi_21_get(&cfgp2psci_hi_21);
    ...
    case BDMF_p2p_ae_sci_lo_22:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_lo_22_get(&cfgp2psci_lo_22);
    ...
    case BDMF_p2p_ae_sci_hi_22:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_hi_22_get(&cfgp2psci_hi_22);
    ...
    case BDMF_p2p_ae_sci_lo_23:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_lo_23_get(&cfgp2psci_lo_23);
    ...
    case BDMF_p2p_ae_sci_hi_23:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_hi_23_get(&cfgp2psci_hi_23);
    ...
    case BDMF_p2p_ae_sci_lo_24:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_lo_24_get(&cfgp2psci_lo_24);
    ...
    case BDMF_p2p_ae_sci_hi_24:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_hi_24_get(&cfgp2psci_hi_24);
    ...
    case BDMF_p2p_ae_sci_lo_25:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_lo_25_get(&cfgp2psci_lo_25);
    ...
    case BDMF_p2p_ae_sci_hi_25:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_hi_25_get(&cfgp2psci_hi_25);
    ...
    case BDMF_p2p_ae_sci_lo_26:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_lo_26_get(&cfgp2psci_lo_26);
    ...
    case BDMF_p2p_ae_sci_hi_26:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_hi_26_get(&cfgp2psci_hi_26);
    ...
    case BDMF_p2p_ae_sci_lo_27:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_lo_27_get(&cfgp2psci_lo_27);
    ...
    case BDMF_p2p_ae_sci_hi_27:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_hi_27_get(&cfgp2psci_hi_27);
    ...
    case BDMF_p2p_ae_sci_lo_28:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_lo_28_get(&cfgp2psci_lo_28);
    ...
    case BDMF_p2p_ae_sci_hi_28:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_hi_28_get(&cfgp2psci_hi_28);
    ...
    case BDMF_p2p_ae_sci_lo_29:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_lo_29_get(&cfgp2psci_lo_29);
    ...
    case BDMF_p2p_ae_sci_hi_29:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_hi_29_get(&cfgp2psci_hi_29);
    ...
    case BDMF_p2p_ae_sci_lo_30:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_lo_30_get(&cfgp2psci_lo_30);
    ...
    case BDMF_p2p_ae_sci_hi_30:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_hi_30_get(&cfgp2psci_hi_30);
    ...
    case BDMF_p2p_ae_sci_lo_31:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_lo_31_get(&cfgp2psci_lo_31);
    ...
    case BDMF_p2p_ae_sci_hi_31:
    {
        ...
        err = ag_drv_lif_p2p_ae_sci_hi_31_get(&cfgp2psci_hi_31);
    ...
    case BDMF_sec_control_2:
    {
        ...
        err = ag_drv_lif_sec_control_2_get(&cfgkeynum_4_an_3, &cfgkeynum_4_an_2, &cfgkeynum_4_an_1, &cfgkeynum_4_an_0);
        bdmf_session_print(session, "cfgkeynum_4_an_3 = %u = 0x%x\n", cfgkeynum_4_an_3, cfgkeynum_4_an_3);
        bdmf_session_print(session, "cfgkeynum_4_an_2 = %u = 0x%x\n", cfgkeynum_4_an_2, cfgkeynum_4_an_2);
        bdmf_session_print(session, "cfgkeynum_4_an_1 = %u = 0x%x\n", cfgkeynum_4_an_1, cfgkeynum_4_an_1);
        bdmf_session_print(session, "cfgkeynum_4_an_0 = %u = 0x%x\n", cfgkeynum_4_an_0, cfgkeynum_4_an_0);
        break;
    }
    case BDMF_sec_mpcp_offset:
    {
        uint32_t cfgsecdnmpcpoffset;
        err = ag_drv_lif_sec_mpcp_offset_get(&cfgsecdnmpcpoffset);
        bdmf_session_print(session, "cfgsecdnmpcpoffset = %u = 0x%x\n", cfgsecdnmpcpoffset, cfgsecdnmpcpoffset);
        break;
    }
    default:
        err = BDMF_ERR_NOT_SUPPORTED;
...
static int bcm_lif_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
        bdmf_boolean cfp2pmodetransmitremfault=gtmv(m, 1);
        bdmf_boolean cfp2pmodetransmitdeficitidleenable=gtmv(m, 1);
        bdmf_boolean cfp2pmodeadaptiveshortpre=gtmv(m, 1);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_inter_op_control_set( %u %u %u)\n", cfp2pmodetransmitremfault, cfp2pmodetransmitdeficitidleenable, cfp2pmodeadaptiveshortpre);
        if(!err) ag_drv_lif_p2p_inter_op_control_set(cfp2pmodetransmitremfault, cfp2pmodetransmitdeficitidleenable, cfp2pmodeadaptiveshortpre);
        if(!err) ag_drv_lif_p2p_inter_op_control_get( &cfp2pmodetransmitremfault, &cfp2pmodetransmitdeficitidleenable, &cfp2pmodeadaptiveshortpre);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_inter_op_control_get( %u %u %u)\n", cfp2pmodetransmitremfault, cfp2pmodetransmitdeficitidleenable, cfp2pmodeadaptiveshortpre);
        if(err || cfp2pmodetransmitremfault!=gtmv(m, 1) || cfp2pmodetransmitdeficitidleenable!=gtmv(m, 1) || cfp2pmodeadaptiveshortpre!=gtmv(m, 1))
            return err ? err : BDMF_ERR_IO;
    }
    {
        bdmf_boolean cfgdisp2pdnindexmap=gtmv(m, 1);
        bdmf_boolean cfgdisp2pupindexmap=gtmv(m, 1);
        if(!err) bdmf_session_print(session, "ag_drv_lif_pon_control_2_set( %u %u)\n", cfgdisp2pdnindexmap, cfgdisp2pupindexmap);
        if(!err) ag_drv_lif_pon_control_2_set(cfgdisp2pdnindexmap, cfgdisp2pupindexmap);
        if(!err) ag_drv_lif_pon_control_2_get( &cfgdisp2pdnindexmap, &cfgdisp2pupindexmap);
        if(!err) bdmf_session_print(session, "ag_drv_lif_pon_control_2_get( %u %u)\n", cfgdisp2pdnindexmap, cfgdisp2pupindexmap);
        if(err || cfgdisp2pdnindexmap!=gtmv(m, 1) || cfgdisp2pupindexmap!=gtmv(m, 1))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint16_t cfgvlantype_1=gtmv(m, 16);
        if(!err) bdmf_session_print(session, "ag_drv_lif_vlan_type_1_set( %u)\n", cfgvlantype_1);
        if(!err) ag_drv_lif_vlan_type_1_set(cfgvlantype_1);
        if(!err) ag_drv_lif_vlan_type_1_get( &cfgvlantype_1);
        if(!err) bdmf_session_print(session, "ag_drv_lif_vlan_type_1_get( %u)\n", cfgvlantype_1);
        if(err || cfgvlantype_1!=gtmv(m, 16))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_lo_16=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_16_set( %u)\n", cfgp2psci_lo_16);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_16_set(cfgp2psci_lo_16);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_16_get( &cfgp2psci_lo_16);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_16_get( %u)\n", cfgp2psci_lo_16);
        if(err || cfgp2psci_lo_16!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_hi_16=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_16_set( %u)\n", cfgp2psci_hi_16);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_16_set(cfgp2psci_hi_16);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_16_get( &cfgp2psci_hi_16);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_16_get( %u)\n", cfgp2psci_hi_16);
        if(err || cfgp2psci_hi_16!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_lo_17=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_17_set( %u)\n", cfgp2psci_lo_17);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_17_set(cfgp2psci_lo_17);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_17_get( &cfgp2psci_lo_17);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_17_get( %u)\n", cfgp2psci_lo_17);
        if(err || cfgp2psci_lo_17!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_hi_17=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_17_set( %u)\n", cfgp2psci_hi_17);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_17_set(cfgp2psci_hi_17);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_17_get( &cfgp2psci_hi_17);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_17_get( %u)\n", cfgp2psci_hi_17);
        if(err || cfgp2psci_hi_17!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_lo_18=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_18_set( %u)\n", cfgp2psci_lo_18);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_18_set(cfgp2psci_lo_18);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_18_get( &cfgp2psci_lo_18);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_18_get( %u)\n", cfgp2psci_lo_18);
        if(err || cfgp2psci_lo_18!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_hi_18=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_18_set( %u)\n", cfgp2psci_hi_18);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_18_set(cfgp2psci_hi_18);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_18_get( &cfgp2psci_hi_18);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_18_get( %u)\n", cfgp2psci_hi_18);
        if(err || cfgp2psci_hi_18!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_lo_19=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_19_set( %u)\n", cfgp2psci_lo_19);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_19_set(cfgp2psci_lo_19);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_19_get( &cfgp2psci_lo_19);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_19_get( %u)\n", cfgp2psci_lo_19);
        if(err || cfgp2psci_lo_19!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_hi_19=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_19_set( %u)\n", cfgp2psci_hi_19);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_19_set(cfgp2psci_hi_19);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_19_get( &cfgp2psci_hi_19);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_19_get( %u)\n", cfgp2psci_hi_19);
        if(err || cfgp2psci_hi_19!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_lo_20=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_20_set( %u)\n", cfgp2psci_lo_20);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_20_set(cfgp2psci_lo_20);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_20_get( &cfgp2psci_lo_20);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_20_get( %u)\n", cfgp2psci_lo_20);
        if(err || cfgp2psci_lo_20!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_hi_20=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_20_set( %u)\n", cfgp2psci_hi_20);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_20_set(cfgp2psci_hi_20);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_20_get( &cfgp2psci_hi_20);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_20_get( %u)\n", cfgp2psci_hi_20);
        if(err || cfgp2psci_hi_20!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_lo_21=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_21_set( %u)\n", cfgp2psci_lo_21);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_21_set(cfgp2psci_lo_21);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_21_get( &cfgp2psci_lo_21);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_21_get( %u)\n", cfgp2psci_lo_21);
        if(err || cfgp2psci_lo_21!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_hi_21=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_21_set( %u)\n", cfgp2psci_hi_21);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_21_set(cfgp2psci_hi_21);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_21_get( &cfgp2psci_hi_21);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_21_get( %u)\n", cfgp2psci_hi_21);
        if(err || cfgp2psci_hi_21!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_lo_22=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_22_set( %u)\n", cfgp2psci_lo_22);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_22_set(cfgp2psci_lo_22);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_22_get( &cfgp2psci_lo_22);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_22_get( %u)\n", cfgp2psci_lo_22);
        if(err || cfgp2psci_lo_22!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_hi_22=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_22_set( %u)\n", cfgp2psci_hi_22);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_22_set(cfgp2psci_hi_22);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_22_get( &cfgp2psci_hi_22);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_22_get( %u)\n", cfgp2psci_hi_22);
        if(err || cfgp2psci_hi_22!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_lo_23=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_23_set( %u)\n", cfgp2psci_lo_23);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_23_set(cfgp2psci_lo_23);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_23_get( &cfgp2psci_lo_23);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_23_get( %u)\n", cfgp2psci_lo_23);
        if(err || cfgp2psci_lo_23!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_hi_23=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_23_set( %u)\n", cfgp2psci_hi_23);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_23_set(cfgp2psci_hi_23);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_23_get( &cfgp2psci_hi_23);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_23_get( %u)\n", cfgp2psci_hi_23);
        if(err || cfgp2psci_hi_23!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_lo_24=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_24_set( %u)\n", cfgp2psci_lo_24);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_24_set(cfgp2psci_lo_24);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_24_get( &cfgp2psci_lo_24);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_24_get( %u)\n", cfgp2psci_lo_24);
        if(err || cfgp2psci_lo_24!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_hi_24=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_24_set( %u)\n", cfgp2psci_hi_24);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_24_set(cfgp2psci_hi_24);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_24_get( &cfgp2psci_hi_24);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_24_get( %u)\n", cfgp2psci_hi_24);
        if(err || cfgp2psci_hi_24!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_lo_25=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_25_set( %u)\n", cfgp2psci_lo_25);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_25_set(cfgp2psci_lo_25);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_25_get( &cfgp2psci_lo_25);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_25_get( %u)\n", cfgp2psci_lo_25);
        if(err || cfgp2psci_lo_25!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_hi_25=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_25_set( %u)\n", cfgp2psci_hi_25);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_25_set(cfgp2psci_hi_25);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_25_get( &cfgp2psci_hi_25);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_25_get( %u)\n", cfgp2psci_hi_25);
        if(err || cfgp2psci_hi_25!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_lo_26=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_26_set( %u)\n", cfgp2psci_lo_26);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_26_set(cfgp2psci_lo_26);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_26_get( &cfgp2psci_lo_26);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_26_get( %u)\n", cfgp2psci_lo_26);
        if(err || cfgp2psci_lo_26!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_hi_26=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_26_set( %u)\n", cfgp2psci_hi_26);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_26_set(cfgp2psci_hi_26);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_26_get( &cfgp2psci_hi_26);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_26_get( %u)\n", cfgp2psci_hi_26);
        if(err || cfgp2psci_hi_26!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_lo_27=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_27_set( %u)\n", cfgp2psci_lo_27);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_27_set(cfgp2psci_lo_27);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_27_get( &cfgp2psci_lo_27);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_27_get( %u)\n", cfgp2psci_lo_27);
        if(err || cfgp2psci_lo_27!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_hi_27=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_27_set( %u)\n", cfgp2psci_hi_27);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_27_set(cfgp2psci_hi_27);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_27_get( &cfgp2psci_hi_27);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_27_get( %u)\n", cfgp2psci_hi_27);
        if(err || cfgp2psci_hi_27!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_lo_28=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_28_set( %u)\n", cfgp2psci_lo_28);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_28_set(cfgp2psci_lo_28);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_28_get( &cfgp2psci_lo_28);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_28_get( %u)\n", cfgp2psci_lo_28);
        if(err || cfgp2psci_lo_28!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_hi_28=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_28_set( %u)\n", cfgp2psci_hi_28);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_28_set(cfgp2psci_hi_28);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_28_get( &cfgp2psci_hi_28);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_28_get( %u)\n", cfgp2psci_hi_28);
        if(err || cfgp2psci_hi_28!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_lo_29=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_29_set( %u)\n", cfgp2psci_lo_29);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_29_set(cfgp2psci_lo_29);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_29_get( &cfgp2psci_lo_29);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_29_get( %u)\n", cfgp2psci_lo_29);
        if(err || cfgp2psci_lo_29!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_hi_29=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_29_set( %u)\n", cfgp2psci_hi_29);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_29_set(cfgp2psci_hi_29);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_29_get( &cfgp2psci_hi_29);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_29_get( %u)\n", cfgp2psci_hi_29);
        if(err || cfgp2psci_hi_29!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_lo_30=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_30_set( %u)\n", cfgp2psci_lo_30);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_30_set(cfgp2psci_lo_30);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_30_get( &cfgp2psci_lo_30);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_30_get( %u)\n", cfgp2psci_lo_30);
        if(err || cfgp2psci_lo_30!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_hi_30=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_30_set( %u)\n", cfgp2psci_hi_30);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_30_set(cfgp2psci_hi_30);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_30_get( &cfgp2psci_hi_30);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_30_get( %u)\n", cfgp2psci_hi_30);
        if(err || cfgp2psci_hi_30!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_lo_31=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_31_set( %u)\n", cfgp2psci_lo_31);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_31_set(cfgp2psci_lo_31);
        if(!err) ag_drv_lif_p2p_ae_sci_lo_31_get( &cfgp2psci_lo_31);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_lo_31_get( %u)\n", cfgp2psci_lo_31);
        if(err || cfgp2psci_lo_31!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgp2psci_hi_31=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_31_set( %u)\n", cfgp2psci_hi_31);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_31_set(cfgp2psci_hi_31);
        if(!err) ag_drv_lif_p2p_ae_sci_hi_31_get( &cfgp2psci_hi_31);
        if(!err) bdmf_session_print(session, "ag_drv_lif_p2p_ae_sci_hi_31_get( %u)\n", cfgp2psci_hi_31);
        if(err || cfgp2psci_hi_31!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        bdmf_boolean cfgkeynum_4_an_3=gtmv(m, 1);
        bdmf_boolean cfgkeynum_4_an_2=gtmv(m, 1);
        bdmf_boolean cfgkeynum_4_an_1=gtmv(m, 1);
        bdmf_boolean cfgkeynum_4_an_0=gtmv(m, 1);
        if(!err) bdmf_session_print(session, "ag_drv_lif_sec_control_2_set( %u %u %u %u)\n", cfgkeynum_4_an_3, cfgkeynum_4_an_2, cfgkeynum_4_an_1, cfgkeynum_4_an_0);
        if(!err) ag_drv_lif_sec_control_2_set(cfgkeynum_4_an_3, cfgkeynum_4_an_2, cfgkeynum_4_an_1, cfgkeynum_4_an_0);
        if(!err) ag_drv_lif_sec_control_2_get( &cfgkeynum_4_an_3, &cfgkeynum_4_an_2, &cfgkeynum_4_an_1, &cfgkeynum_4_an_0);
        if(!err) bdmf_session_print(session, "ag_drv_lif_sec_control_2_get( %u %u %u %u)\n", cfgkeynum_4_an_3, cfgkeynum_4_an_2, cfgkeynum_4_an_1, cfgkeynum_4_an_0);
        if(err || cfgkeynum_4_an_3!=gtmv(m, 1) || cfgkeynum_4_an_2!=gtmv(m, 1) || cfgkeynum_4_an_1!=gtmv(m, 1) || cfgkeynum_4_an_0!=gtmv(m, 1))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgsecdnmpcpoffset=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_lif_sec_mpcp_offset_set( %u)\n", cfgsecdnmpcpoffset);
        if(!err) ag_drv_lif_sec_mpcp_offset_set(cfgsecdnmpcpoffset);
        if(!err) ag_drv_lif_sec_mpcp_offset_get( &cfgsecdnmpcpoffset);
        if(!err) bdmf_session_print(session, "ag_drv_lif_sec_mpcp_offset_get( %u)\n", cfgsecdnmpcpoffset);
        if(err || cfgsecdnmpcpoffset!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    return err;
}
...
static int bcm_lif_cli_address(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case bdmf_address_p2p_inter_op_control : reg = &RU_REG(LIF, P2P_INTER_OP_CONTROL); blk = &RU_BLK(LIF); break;
    case bdmf_address_pon_control_2 : reg = &RU_REG(LIF, PON_CONTROL_2); blk = &RU_BLK(LIF); break;
    case bdmf_address_vlan_type_1 : reg = &RU_REG(LIF, VLAN_TYPE_1); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_lo_16 : reg = &RU_REG(LIF, P2P_AE_SCI_LO_16); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_hi_16 : reg = &RU_REG(LIF, P2P_AE_SCI_HI_16); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_lo_17 : reg = &RU_REG(LIF, P2P_AE_SCI_LO_17); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_hi_17 : reg = &RU_REG(LIF, P2P_AE_SCI_HI_17); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_lo_18 : reg = &RU_REG(LIF, P2P_AE_SCI_LO_18); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_hi_18 : reg = &RU_REG(LIF, P2P_AE_SCI_HI_18); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_lo_19 : reg = &RU_REG(LIF, P2P_AE_SCI_LO_19); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_hi_19 : reg = &RU_REG(LIF, P2P_AE_SCI_HI_19); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_lo_20 : reg = &RU_REG(LIF, P2P_AE_SCI_LO_20); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_hi_20 : reg = &RU_REG(LIF, P2P_AE_SCI_HI_20); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_lo_21 : reg = &RU_REG(LIF, P2P_AE_SCI_LO_21); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_hi_21 : reg = &RU_REG(LIF, P2P_AE_SCI_HI_21); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_lo_22 : reg = &RU_REG(LIF, P2P_AE_SCI_LO_22); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_hi_22 : reg = &RU_REG(LIF, P2P_AE_SCI_HI_22); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_lo_23 : reg = &RU_REG(LIF, P2P_AE_SCI_LO_23); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_hi_23 : reg = &RU_REG(LIF, P2P_AE_SCI_HI_23); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_lo_24 : reg = &RU_REG(LIF, P2P_AE_SCI_LO_24); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_hi_24 : reg = &RU_REG(LIF, P2P_AE_SCI_HI_24); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_lo_25 : reg = &RU_REG(LIF, P2P_AE_SCI_LO_25); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_hi_25 : reg = &RU_REG(LIF, P2P_AE_SCI_HI_25); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_lo_26 : reg = &RU_REG(LIF, P2P_AE_SCI_LO_26); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_hi_26 : reg = &RU_REG(LIF, P2P_AE_SCI_HI_26); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_lo_27 : reg = &RU_REG(LIF, P2P_AE_SCI_LO_27); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_hi_27 : reg = &RU_REG(LIF, P2P_AE_SCI_HI_27); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_lo_28 : reg = &RU_REG(LIF, P2P_AE_SCI_LO_28); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_hi_28 : reg = &RU_REG(LIF, P2P_AE_SCI_HI_28); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_lo_29 : reg = &RU_REG(LIF, P2P_AE_SCI_LO_29); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_hi_29 : reg = &RU_REG(LIF, P2P_AE_SCI_HI_29); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_lo_30 : reg = &RU_REG(LIF, P2P_AE_SCI_LO_30); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_hi_30 : reg = &RU_REG(LIF, P2P_AE_SCI_HI_30); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_lo_31 : reg = &RU_REG(LIF, P2P_AE_SCI_LO_31); blk = &RU_BLK(LIF); break;
    case bdmf_address_p2p_ae_sci_hi_31 : reg = &RU_REG(LIF, P2P_AE_SCI_HI_31); blk = &RU_BLK(LIF); break;
    case bdmf_address_sec_control_2 : reg = &RU_REG(LIF, SEC_CONTROL_2); blk = &RU_BLK(LIF); break;
    case bdmf_address_sec_mpcp_offset : reg = &RU_REG(LIF, SEC_MPCP_OFFSET); blk = &RU_BLK(LIF); break;
    default :
...
bdmfmon_handle_t ag_drv_lif_cli_init(bdmfmon_handle_t driver_dir)
{
...

    {
        static bdmfmon_cmd_parm_t set_pon_control[]={
            BDMFMON_MAKE_PARM("cfg_lif_sgmii_mode", "cfg_lif_sgmii_mode", BDMFMON_PARM_NUMBER, 0),
        ...
        static bdmfmon_cmd_parm_t set_int_status[]={
            BDMFMON_MAKE_PARM("sgmiiabortfrint", "sgmiiabortfrint", BDMFMON_PARM_NUMBER, 0),
        ...
        static bdmfmon_cmd_parm_t set_int_mask[]={
            BDMFMON_MAKE_PARM("sgmiiabortfrintmask", "sgmiiabortfrintmask", BDMFMON_PARM_NUMBER, 0),
        ...
        static bdmfmon_cmd_parm_t set_p2p_inter_op_control[]={
            BDMFMON_MAKE_PARM("cfp2pmodetransmitremfault", "cfp2pmodetransmitremfault", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_MAKE_PARM("cfp2pmodetransmitdeficitidleenable", "cfp2pmodetransmitdeficitidleenable", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_MAKE_PARM("cfp2pmodeadaptiveshortpre", "cfp2pmodeadaptiveshortpre", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_PARM_LIST_TERMINATOR
        };
        static bdmfmon_cmd_parm_t set_pon_control_2[]={
        ...
        static bdmfmon_cmd_parm_t set_sec_control_2[]={
        ...
        static bdmfmon_cmd_parm_t set_sec_mpcp_offset[]={
            BDMFMON_MAKE_PARM("cfgsecdnmpcpoffset", "cfgsecdnmpcpoffset", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_PARM_LIST_TERMINATOR
        };
        static bdmfmon_enum_val_t selector_table[] = {
            ...
            { .name="p2p_inter_op_control", .val=BDMF_p2p_inter_op_control, .parms=set_p2p_inter_op_control },
            { .name="pon_control_2", .val=BDMF_pon_control_2, .parms=set_pon_control_2 },
            ...
            { .name="sec_control_2", .val=BDMF_sec_control_2, .parms=set_sec_control_2 },
    ...
    }
    {
        ...
        static bdmfmon_enum_val_t selector_table[] = {
            ...
            { .name="p2p_inter_op_control", .val=BDMF_p2p_inter_op_control, .parms=set_default },
            { .name="pon_control_2", .val=BDMF_pon_control_2, .parms=set_default },
            ...
            { .name="sec_control_2", .val=BDMF_sec_control_2, .parms=set_default },
    ...
    }
    {
        static bdmfmon_enum_val_t enum_table_address[] = {
            ...
            { .name="P2P_INTER_OP_CONTROL" , .val=bdmf_address_p2p_inter_op_control },
            { .name="PON_CONTROL_2" , .val=bdmf_address_pon_control_2 },
            ...
            { .name="SEC_CONTROL_2" , .val=bdmf_address_sec_control_2 },
