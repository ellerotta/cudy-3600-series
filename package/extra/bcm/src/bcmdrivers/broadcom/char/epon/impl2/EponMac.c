...
#else
#if defined(EPON_10G_DSONLY)
#define UpstreamStatsFakeOffset              272
#else
...
LaserRate EponGetMacUpRate (void)
    {
...
LaserRate EponGetMacDnRate (void)
    {
    if (EponGetMacMode() == EPON_MAC_AE_MODE)
        return AeGetRate();
    else
...
#if defined(CONFIG_BCM96888)  || defined(CONFIG_BCM96813) || \
...
#if defined(CONFIG_BCM96888)  || defined(CONFIG_BCM96813) || \
    defined(CONFIG_BCM968880) || defined(CONFIG_BCM96837)
    drv_error += ag_drv_epon_top_control_get(&control);
    control.cfgtwogigpondns = dn2g;
    control.cfgtengigponup = up10g;
    control.cfgtengigdns = dn10g;
    drv_error += ag_drv_epon_top_control_set(&control);
#else 
...
#ifdef EPON_NORMAL_MODE
        /* show epon mac interrupt and debug info */
        DisplayEpnInrptInfo();
#endif
...
#elif defined(EPON_10G_DSONLY)
    drv_error += ag_drv_epn_tx_l1s_shp_que_en_get(shpr, (U16 *)map);
#else
...
#elif defined(EPON_10G_DSONLY)
    drv_error += ag_drv_epn_l1_sva_overflow_get((U16 *)l1shpOverflow);
    drv_error += ag_drv_epn_l1_uva_overflow_get((U16 *)l1unshpOverflow);
#elif defined(EPON_10G_BIDIRECT)
...
void EponEpnInit (U16 maxFrameSize, BOOL preDraft2Dot1, BOOL add16TqToFront,
    ...
    if (upRate == LaserRate10G)
        {
        drv_error += ag_drv_epn_dn_gnt_misalign_pause_set(11);
        }
    else if (upRate == LaserRate1G)
        {
        drv_error += ag_drv_epn_dn_gnt_misalign_pause_set(145);
        }
    else if (upRate == LaserRate2G)
        {
        drv_error += ag_drv_epn_dn_gnt_misalign_pause_set(182);
        }
    else
        {
        drv_error += ag_drv_epn_dn_gnt_misalign_pause_set(0);
        } 
    ...
    drv_error += ag_drv_epn_minimum_grant_setup_set(0x64UL);
    {
        ...
        if (EponGetMacMode() == EPON_MAC_AE_MODE)
            ...
            {
            ecoutxsnfenable  = 0;
            ecojira758enable = 0; /* Transmit data depends on L1 reporting*/
            }
...
////////////////////////////////////////////////////////////////////////////////
/// \brief Enable/Disable cfgIdlePacketTxEnable
///
...
////////////////////////////////////////////////////////////////////////////////
//extern
void EponCfgIdlePacketTxEnable (BOOL isEnable)
    {
#ifdef IDLE_PACKET_TX_SUPPORT
    epn_control_1 epn_ctrl_1;
    bdmf_error_t drv_error = BDMF_ERR_OK;

    drv_error += ag_drv_epn_control_1_get(&epn_ctrl_1);
    epn_ctrl_1.cfgidlepackettxenable = isEnable;
    drv_error += ag_drv_epn_control_1_set(&epn_ctrl_1);
#endif
    } // EponCfgIdlePacketTxEnable
