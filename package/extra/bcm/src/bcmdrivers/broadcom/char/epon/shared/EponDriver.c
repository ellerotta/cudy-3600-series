...
module_param(xepon_mode, int, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP);
...
extern void EponAeDriverExit(void);
...
long eponCoreDrvIoctlCompat(struct file *filp, unsigned int cmd, unsigned long arg)
    ...
    mutex_lock(&EponIoctlMutex);
#ifdef EPON_NORMAL_MODE    
    ret = EponIoctlAccessCompat(cmd,arg);
#endif
    mutex_unlock(&EponIoctlMutex);
    return ret;
    }
...
static int __init moduleInit(void)
    ...
    {
#if defined(CONFIG_BRCM_SMC_BOOT)
        if (bcm_license_check_msg(BP3_FEATURE_10G_EPON) <= 0)
            return -1;
#endif

        if (pmc_wan_interface_power_control(WAN_INTF_10GEPON, 1))
            return -1;
    }
    else
    {
#if defined(CONFIG_BRCM_SMC_BOOT)
        if (bcm_license_check_msg(BP3_FEATURE_EPON) <= 0)
            return -1;
#endif

        if (pmc_wan_interface_power_control(WAN_INTF_EPON, 1))
            return -1;
    }

    rc = ru_remap_properties("brcm,epon-drv", RU_EPON_BLOCKS[0]->addr[0], RU_EPON_BLOCKS);
    if (rc)
        {
        BCM_LOG_ERROR(BCM_LOG_ID_EPON, "EPON dev register remap failed.");
        return -1;
        }
...
static void __exit moduleCleanup(void)
    ...
    ru_unmap_properties("brcm,epon-drv");

    if (xepon_mode)
        pmc_wan_interface_power_control(WAN_INTF_10GEPON, 0);
    else
        pmc_wan_interface_power_control(WAN_INTF_EPON, 0);
