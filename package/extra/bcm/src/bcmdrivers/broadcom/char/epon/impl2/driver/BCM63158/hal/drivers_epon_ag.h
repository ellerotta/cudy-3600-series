...
#ifndef _EPON_COMMON_AG_H_
#define _EPON_COMMON_AG_H_
...
{
	int addr_idx;
	for (addr_idx = 0; addr_idx < RU_EPON_BLOCKS[block_idx]->addr_count; addr_idx++)
	{
		bdmf_trace("block %s[%d] address = 0x%16lx\n", RU_EPON_BLOCKS[block_idx]->name, addr_idx,
			RU_EPON_BLOCKS[block_idx]->addr[addr_idx]);
	}
}
