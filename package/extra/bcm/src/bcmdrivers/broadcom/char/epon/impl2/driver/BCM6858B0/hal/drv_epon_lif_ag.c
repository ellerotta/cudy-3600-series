...
bdmf_error_t ag_drv_lif_llid_set(uint8_t llid_index, uint32_t cfgllid)
{
    ...
    reg_llid = RU_FIELD_SET(0, LIF, LLID, CFGLLID, reg_llid, cfgllid);
...
bdmf_error_t ag_drv_lif_llid_get(uint8_t llid_index, uint32_t *cfgllid)
{
    ...
    *cfgllid = RU_FIELD_GET(0, LIF, LLID, CFGLLID, reg_llid);
...
#ifdef VALIDATE_PARMS
    if((cfrxmaxframelength >= _11BITS_MAX_VAL_))
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_RANGE), BDMF_ERR_RANGE);
        return BDMF_ERR_RANGE;
    }
...
static int bcm_lif_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
        uint32_t cfgllid;
        err = ag_drv_lif_llid_get(parm[1].value.unumber, &cfgllid);
        bdmf_session_print(session, "cfgllid = %u = 0x%x\n", cfgllid, cfgllid);
        break;
    }
    case BDMF_time_ref_cnt:
    {
...
static int bcm_lif_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    }
    {
        ...
        uint32_t cfgllid=gtmv(m, 17);
        if(!err) bdmf_session_print(session, "ag_drv_lif_llid_set( %u %u)\n", llid_index, cfgllid);
        if(!err) ag_drv_lif_llid_set(llid_index, cfgllid);
        if(!err) ag_drv_lif_llid_get( llid_index, &cfgllid);
        if(!err) bdmf_session_print(session, "ag_drv_lif_llid_get( %u %u)\n", llid_index, cfgllid);
        if(err || cfgllid!=gtmv(m, 17))
            return err ? err : BDMF_ERR_IO;
    }
    {
    ...
    {
        uint16_t cfrxmaxframelength=gtmv(m, 11);
        ...
        if(err || cfrxmaxframelength!=gtmv(m, 11))
            return err ? err : BDMF_ERR_IO;
    }
    {
...
bdmfmon_handle_t ag_drv_lif_cli_init(bdmfmon_handle_t driver_dir)
{
...

    {
        ...
        static bdmfmon_cmd_parm_t set_llid[]={
            ...
            BDMFMON_MAKE_PARM("cfgllid", "cfgllid", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_PARM_LIST_TERMINATOR
        };
