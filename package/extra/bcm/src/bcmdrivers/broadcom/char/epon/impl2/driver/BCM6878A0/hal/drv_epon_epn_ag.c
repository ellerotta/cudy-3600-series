...
bdmf_error_t ag_drv_epn_control_1_set(const epn_control_1 *control_1)
{
    ...
    if((control_1->cfgdisablempcpcorrectiondithering >= _1BITS_MAX_VAL_) ||
...
bdmf_error_t ag_drv_epn_reset_l1_accumulator_set(uint8_t cfgl1sclracum)
{
...
bdmf_error_t ag_drv_epn_reset_l1_accumulator_get(uint8_t *cfgl1sclracum)
{
    uint32_t reg_reset_l1_accumulator=0;
...
#ifdef VALIDATE_PARMS
    if((cfgl1suvasizesel >= _3BITS_MAX_VAL_) ||
       (cfgl1ssvasizesel >= _3BITS_MAX_VAL_))
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_RANGE), BDMF_ERR_RANGE);
        return BDMF_ERR_RANGE;
    }
...
bdmf_error_t ag_drv_epn_l1_sva_overflow_get(uint8_t *l1ssvaoverflow)
{
...
bdmf_error_t ag_drv_epn_l1_uva_overflow_get(uint8_t *l1suvaoverflow)
{
...
#ifdef VALIDATE_PARMS
    if((cfgmaxframesize >= _11BITS_MAX_VAL_))
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_RANGE), BDMF_ERR_RANGE);
        return BDMF_ERR_RANGE;
    }
...
bdmf_error_t ag_drv_epn_fake_report_value_en_set(uint8_t fakereportvalueen)
{
...
bdmf_error_t ag_drv_epn_fake_report_value_en_get(uint8_t *fakereportvalueen)
{
    uint32_t reg_fake_report_value_en=0;
...
bdmf_error_t ag_drv_epn_shared_bcap_ovrflow_get(uint8_t *sharedburstcapoverflow)
{
...
bdmf_error_t ag_drv_epn_l2s_flush_config_set(bdmf_boolean cfgflushl2sen, bdmf_boolean flushl2sdone, uint8_t cfgflushl2ssel)
{
    ...
    if((cfgflushl2sen >= _1BITS_MAX_VAL_) ||
       ...
       (cfgflushl2ssel >= _3BITS_MAX_VAL_))
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_RANGE), BDMF_ERR_RANGE);
        return BDMF_ERR_RANGE;
    }
...
bdmf_error_t ag_drv_epn_tx_l1s_shp_que_mask_set(uint8_t shaper_idx, uint8_t cfgshpmask)
{
...
bdmf_error_t ag_drv_epn_tx_l1s_shp_que_mask_get(uint8_t shaper_idx, uint8_t *cfgshpmask)
{
    uint32_t reg_tx_l1s_shp_que_mask_=0;
...
bdmf_error_t ag_drv_epn_up_ts_offset_hi_set(uint8_t cfgcmhoffset, uint16_t cfguptsoffset_hi)
{
    ...
    reg_up_ts_offset_hi = RU_FIELD_SET(0, EPN, UP_TS_OFFSET_HI, CFGCMHOFFSET, reg_up_ts_offset_hi, cfgcmhoffset);
...
bdmf_error_t ag_drv_epn_up_ts_offset_hi_get(uint8_t *cfgcmhoffset, uint16_t *cfguptsoffset_hi)
{
    uint32_t reg_up_ts_offset_hi=0;

#ifdef VALIDATE_PARMS
    if(!cfgcmhoffset || !cfguptsoffset_hi)
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_PARM), BDMF_ERR_PARM);
        return BDMF_ERR_PARM;
    }
    ...
    *cfgcmhoffset = RU_FIELD_GET(0, EPN, UP_TS_OFFSET_HI, CFGCMHOFFSET, reg_up_ts_offset_hi);
...
static int bcm_epn_cli_set(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case BDMF_control_0:
    {
        epn_control_0 control_0 = { .cfgen1588ts=parm[1].value.unumber, .cfgreplaceupfcs=parm[2].value.unumber, .cfgappendupfcs=parm[3].value.unumber, .cfgdropscb=parm[4].value.unumber, .moduncappedreportlimit=parm[5].value.unumber, .modmpquesetfirst=parm[6].value.unumber, .prvlocalmpcppropagation=parm[7].value.unumber, .prvtekmodeprefetch=parm[8].value.unumber, .prvnounmapppedfcs=parm[9].value.unumber, .prvsupressdiscen=parm[10].value.unumber, .cfgvlanmax=parm[11].value.unumber, .fcserronlydatafr=parm[12].value.unumber, .prvdropunmapppedllid=parm[13].value.unumber, .prvsuppressllidmodebit=parm[14].value.unumber, .moddiscoverydafilteren=parm[15].value.unumber, .rptselect=parm[16].value.unumber, .prvdisablesvaquestatusbp=parm[17].value.unumber, .utxloopback=parm[18].value.unumber, .utxen=parm[19].value.unumber, .utxrst_pre_n=parm[20].value.unumber, .cfgdisabledns=parm[21].value.unumber, .drxloopback=parm[22].value.unumber, .drxen=parm[23].value.unumber, .drxrst_pre_n=parm[24].value.unumber};
        err = ag_drv_epn_control_0_set(&control_0);
        break;
    }
    case BDMF_control_1:
    {
        epn_control_1 control_1 = { .cfgdisablempcpcorrectiondithering=parm[1].value.unumber, .prvoverlappedgntenable=parm[2].value.unumber, .rstmisalignthr=parm[3].value.unumber, .cfgstalegntchk=parm[4].value.unumber, .fecrpten=parm[5].value.unumber, .cfgl1l2truestrict=parm[6].value.unumber, .cfgctcrpt=parm[7].value.unumber, .cfgtscorrdis=parm[8].value.unumber, .cfgnodiscrpt=parm[9].value.unumber, .disablediscscale=parm[10].value.unumber, .clronrd=parm[11].value.unumber};
    ...
    case BDMF_fake_report_value:
        ...
        break;
    case BDMF_burst_cap:
        err = ag_drv_epn_burst_cap_set(parm[40].value.unumber, parm[41].value.unumber);
        break;
    case BDMF_queue_llid_map:
        err = ag_drv_epn_queue_llid_map_set(parm[40].value.unumber, parm[41].value.unumber);
        break;
    ...
    case BDMF_unused_tq_cnt:
        err = ag_drv_epn_unused_tq_cnt_set(parm[41].value.unumber, parm[42].value.unumber);
        break;
    ...
    case BDMF_tx_l1s_shp_que_mask:
        err = ag_drv_epn_tx_l1s_shp_que_mask_set(parm[42].value.unumber, parm[43].value.unumber);
        break;
    case BDMF_tx_l2s_queue_config:
        err = ag_drv_epn_tx_l2s_queue_config_set(parm[42].value.unumber, parm[43].value.unumber, parm[44].value.unumber);
        break;
    case BDMF_tx_ctc_burst_limit:
        err = ag_drv_epn_tx_ctc_burst_limit_set(parm[42].value.unumber, parm[43].value.unumber);
        break;
    ...
    case BDMF_up_ts_offset_hi:
        err = ag_drv_epn_up_ts_offset_hi_set(parm[42].value.unumber, parm[43].value.unumber);
        break;
...
static int bcm_epn_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
        uint8_t sharedburstcapoverflow;
    ...
    case BDMF_up_ts_offset_hi:
    {
        ...
        err = ag_drv_epn_up_ts_offset_hi_get(&cfgcmhoffset, &cfguptsoffset_hi);
...
static int bcm_epn_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
        uint8_t cfgl1sclracum=gtmv(m, 8);
        ...
        if(err || cfgl1sclracum!=gtmv(m, 8))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint8_t cfgl1suvasizesel=gtmv(m, 3);
        uint8_t cfgl1ssvasizesel=gtmv(m, 3);
        ...
        if(err || cfgl1suvasizesel!=gtmv(m, 3) || cfgl1ssvasizesel!=gtmv(m, 3))
            return err ? err : BDMF_ERR_IO;
    }
    {
    ...
    {
        uint16_t cfgmaxframesize=gtmv(m, 11);
        ...
        if(err || cfgmaxframesize!=gtmv(m, 11))
            return err ? err : BDMF_ERR_IO;
    }
    {
    ...
    {
        uint8_t fakereportvalueen=gtmv(m, 8);
        ...
        if(err || fakereportvalueen!=gtmv(m, 8))
            return err ? err : BDMF_ERR_IO;
    }
    {
    ...
    {
        uint8_t que_idx=gtmv(m, 3);
        uint8_t quellidmap=gtmv(m, 3);
    ...
    {
        uint8_t sharedburstcapoverflow=gtmv(m, 2);
    ...
    }
    {
        ...
        uint8_t cfgflushl2ssel=gtmv(m, 3);
        ...
        if(err || cfgflushl2sen!=gtmv(m, 1) || flushl2sdone!=gtmv(m, 1) || cfgflushl2ssel!=gtmv(m, 3))
            return err ? err : BDMF_ERR_IO;
    }
    {
    ...
    {
        uint8_t shaper_idx=gtmv(m, 3);
        uint8_t cfgshpmask=gtmv(m, 8);
    ...
    }
    {
        ...
        if(!err) bdmf_session_print(session, "ag_drv_epn_up_ts_offset_hi_set( %u %u)\n", cfgcmhoffset, cfguptsoffset_hi);
        if(!err) ag_drv_epn_up_ts_offset_hi_set(cfgcmhoffset, cfguptsoffset_hi);
        if(!err) ag_drv_epn_up_ts_offset_hi_get( &cfgcmhoffset, &cfguptsoffset_hi);
        if(!err) bdmf_session_print(session, "ag_drv_epn_up_ts_offset_hi_get( %u %u)\n", cfgcmhoffset, cfguptsoffset_hi);
        if(err || cfgcmhoffset!=gtmv(m, 8) || cfguptsoffset_hi!=gtmv(m, 16))
            return err ? err : BDMF_ERR_IO;
    }
    {
