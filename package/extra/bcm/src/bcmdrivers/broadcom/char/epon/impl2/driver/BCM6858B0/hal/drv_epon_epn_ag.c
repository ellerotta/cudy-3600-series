...
bdmf_error_t ag_drv_epn_10g_abc_size_set(uint8_t size_idx, uint16_t cfgaddburstcap2, uint16_t cfgaddburstcap1)
{
    ...
    reg_10g_abc_size = RU_FIELD_SET(0, EPN, 10G_ABC_SIZE, CFGADDBURSTCAP2, reg_10g_abc_size, cfgaddburstcap2);
    reg_10g_abc_size = RU_FIELD_SET(0, EPN, 10G_ABC_SIZE, CFGADDBURSTCAP1, reg_10g_abc_size, cfgaddburstcap1);

    RU_REG_RAM_WRITE(0, size_idx, EPN, 10G_ABC_SIZE, reg_10g_abc_size);
...
bdmf_error_t ag_drv_epn_10g_abc_size_get(uint8_t size_idx, uint16_t *cfgaddburstcap2, uint16_t *cfgaddburstcap1)
{
...
#ifdef VALIDATE_PARMS
    if(!cfgaddburstcap2 || !cfgaddburstcap1)
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_PARM), BDMF_ERR_PARM);
        return BDMF_ERR_PARM;
    }
    ...
    RU_REG_RAM_READ(0, size_idx, EPN, 10G_ABC_SIZE, reg_10g_abc_size);

    *cfgaddburstcap2 = RU_FIELD_GET(0, EPN, 10G_ABC_SIZE, CFGADDBURSTCAP2, reg_10g_abc_size);
    *cfgaddburstcap1 = RU_FIELD_GET(0, EPN, 10G_ABC_SIZE, CFGADDBURSTCAP1, reg_10g_abc_size);
...
static int bcm_epn_cli_set(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case BDMF_min_report_value_difference:
        ...
        break;
    case BDMF_spare_ctl:
        err = ag_drv_epn_spare_ctl_set(parm[43].value.unumber, parm[44].value.unumber, parm[45].value.unumber);
        break;
    ...
    case BDMF_persistent_report_request_size:
        ...
        break;
    case BDMF_10g_abc_size:
        err = ag_drv_epn_10g_abc_size_set(parm[44].value.unumber, parm[45].value.unumber, parm[46].value.unumber);
        break;
    default:
...
static int bcm_epn_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case BDMF_10g_abc_size:
    {
        uint16_t cfgaddburstcap2;
        uint16_t cfgaddburstcap1;
        err = ag_drv_epn_10g_abc_size_get(parm[1].value.unumber, &cfgaddburstcap2, &cfgaddburstcap1);
        bdmf_session_print(session, "cfgaddburstcap2 = %u = 0x%x\n", cfgaddburstcap2, cfgaddburstcap2);
        bdmf_session_print(session, "cfgaddburstcap1 = %u = 0x%x\n", cfgaddburstcap1, cfgaddburstcap1);
        break;
    }
    default:
        err = BDMF_ERR_NOT_SUPPORTED;
...
static int bcm_epn_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
        uint8_t size_idx=gtmv(m, 4);
        uint16_t cfgaddburstcap2=gtmv(m, 16);
        uint16_t cfgaddburstcap1=gtmv(m, 16);
        if(!err) bdmf_session_print(session, "ag_drv_epn_10g_abc_size_set( %u %u %u)\n", size_idx, cfgaddburstcap2, cfgaddburstcap1);
        if(!err) ag_drv_epn_10g_abc_size_set(size_idx, cfgaddburstcap2, cfgaddburstcap1);
        if(!err) ag_drv_epn_10g_abc_size_get( size_idx, &cfgaddburstcap2, &cfgaddburstcap1);
        if(!err) bdmf_session_print(session, "ag_drv_epn_10g_abc_size_get( %u %u %u)\n", size_idx, cfgaddburstcap2, cfgaddburstcap1);
        if(err || cfgaddburstcap2!=gtmv(m, 16) || cfgaddburstcap1!=gtmv(m, 16))
            return err ? err : BDMF_ERR_IO;
    }
    return err;
}
...
static int bcm_epn_cli_address(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case bdmf_address_10g_abc_size : reg = &RU_REG(EPN, 10G_ABC_SIZE); blk = &RU_BLK(EPN); break;
    default :
...
bdmfmon_handle_t ag_drv_epn_cli_init(bdmfmon_handle_t driver_dir)
{
...

    {
        ...
        static bdmfmon_cmd_parm_t set_10g_abc_size[]={
            BDMFMON_MAKE_PARM("size_idx", "size_idx", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_MAKE_PARM("cfgaddburstcap2", "cfgaddburstcap2", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_MAKE_PARM("cfgaddburstcap1", "cfgaddburstcap1", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_PARM_LIST_TERMINATOR
        };
        static bdmfmon_enum_val_t selector_table[] = {
            ...
            { .name="10g_abc_size", .val=BDMF_10g_abc_size, .parms=set_10g_abc_size },
            BDMFMON_ENUM_LAST
        };
    ...
    }
    {
        static bdmfmon_enum_val_t enum_table_address[] = {
            ...
            { .name="10G_ABC_SIZE" , .val=bdmf_address_10g_abc_size },
            BDMFMON_ENUM_LAST
        };
