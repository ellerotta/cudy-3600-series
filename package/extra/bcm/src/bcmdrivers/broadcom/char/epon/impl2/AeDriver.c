...
extern void epon_ae_mac_drv_exit(void);
extern int ae_int_phy_drv_unset(void);
...
BOOL AeCheckRunnerEmpty(void)
{
    ...
    rc = rdpa_port_is_empty_get(*rdpa_port_obj, &is_empty);
    if (rc)
    {
...
void AeSetRunnerPortTraffic(BOOL enable)    
{
    ...
    rc = rdpa_port_enable_set(*rdpa_port_obj, enable);
    if (rc)
    {
...
static Polarity _AeTxPolarity;

static Polarity AeTxPolarity (UINT8  ponIf)
    {
    return _AeTxPolarity;
    }
...
void AeMacInit(LaserRate rate)
{
    ...
    if (rate < LaserRate10G)
        ...
        {
        BCM_LOG_ERROR(BCM_LOG_ID_AE, "unsupported wan rate\n");
        return;
        }
...
static int trx_detect(int bus, void *opticaldet_desc)
{
    ...
    if (_trx_get_lbe_polarity(opticaldet_desc, &lbe_polarity))
    {
        BCM_LOG_ERROR(BCM_LOG_ID_AE, "error setting trx polarity\n");
        return 0;
    }

    _AeTxPolarity = (TRX_ACTIVE_HIGH == lbe_polarity) ? ActiveHi: ActiveLo;
    if (rate < LaserRate10G)
    {
        LifLbePolaritySet(_AeTxPolarity);
    }
#ifdef CONFIG_EPON_10G_SUPPORT
    else if (rate == LaserRate10G)
    {
        XifXpcsLbePolaritySet(_AeTxPolarity);
    }
#endif
    else
    {
    ...
    return 1; /* MAC is ready to transmit */
}

#if defined(CONFIG_BCM96855)
void AeSetSgmiiSpeed(mac_speed_t speed)
{
    lif_pon_control pon_control;

    ag_drv_lif_pon_control_get(&pon_control);
    
    switch(speed)
    {
        case MAC_SPEED_10:
            pon_control.cfg_lif_sgmii_mode = 1;
            break;
        case MAC_SPEED_100:
            pon_control.cfg_lif_sgmii_mode = 2;
            break;
        default:
            pon_control.cfg_lif_sgmii_mode = 0;
            break;
    }

    ag_drv_lif_pon_control_set(&pon_control);
}
#endif

#if !defined(CONFIG_BCM96858)
void AeSetLifP2PAutoNegoEnable(void)
{
#if defined(CONFIG_BCM96846)
    uint8_t rxsyncsqq;
...
void EponAeDriverInit(BOOL serdesInit)
{
    ...
    _AeTxPolarity = GetEponDefTxPolarity();
    trxbus_register_mac(wantop_bus_get(), 1, trx_detect, NULL);
    BCM_LOG_INFO(BCM_LOG_ID_AE, "AE driver init done");
}

void EponAeDriverExit(void)
{
    epon_ae_mac_drv_exit();
    ae_int_phy_drv_unset();
    mac_driver_unset(MAC_TYPE_EPON_AE);
}
