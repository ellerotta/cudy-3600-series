...
#define FLWSTATS_TOS_DSCP_RSHIFT       2

#define FLWSTATS_DSCP_ALL               64
#define FLWSTATS_TOS_ALL               256
#define FLWSTATS_ARRAY_MAX_ENTRIES      FLWSTATS_TOS_ALL    // highest val

typedef struct {
#define FLWSTATS_QUERYMASK_ALL				0x80000000
/* For flows with one TX VLAN, TX innervid/pbits must be filled in the query */
    uint16_t        rxchannel;                          /* RX channel */
#define FLWSTATS_QUERYMASK_RXCHANNEL		0x40000000	/* RX channel mask */
    uint16_t        txinnervid;                         /* TX Inner VLAN */
#define FLWSTATS_QUERYMASK_TXINVID		0x20000000	/* TX Inner VLAN mask */
    uint8_t         txinnerpbits;                       /* TX Inner Pbits */
#define FLWSTATS_QUERYMASK_TXINPBITS		0x10000000	/* TX Inner Pbits mask */
    uint16_t        txoutervid;                         /* TX Outer VLAN */
#define FLWSTATS_QUERYMASK_TXOUTVID			0x08000000	/* TX Outer VLAN mask */
    uint8_t         txouterpbits;                       /* TX Outer Pbits */
#define FLWSTATS_QUERYMASK_TXOUTPBITS		0x04000000	/* TX Outer Pbits mask */

    uint16_t        tos;								/* TOS */
#define FLWSTATS_QUERYMASK_TXDSCP			0x02000000	/* TX DSCP mask */
#define FLWSTATS_QUERYMASK_TXTOS			0x01000000	/* TX TOS mask */
#define FLWSTATS_QUERYMASK_RXDSCP			0x00800000	/* RX DSCP mask */
#define FLWSTATS_QUERYMASK_RXTOS			0x00400000	/* RX TOS mask */
    uint8_t         inTxMacSA[ETH_ALEN];                /* Inner Source MAC(L2 GRE) */
#define FLWSTATS_QUERYMASK_INTXMACSRC		0x00200000	/* Source MAC mask(L2 GRE) */
    uint8_t         inTxMacDA[ETH_ALEN];                /* Inner Destination MAC(L2 GRE) */
#define FLWSTATS_QUERYMASK_INTXMACDST		0x00100000	/* Destination MAC mask(L2 GRE) */ 
    uint8_t         inRxMacSA[ETH_ALEN];                /* Inner Source MAC(L2 GRE) */
#define FLWSTATS_QUERYMASK_INRXMACSRC		0x00080000	/* Source MAC mask(L2 GRE) */
    uint8_t         inRxMacDA[ETH_ALEN];                /* Inner Destination MAC(L2 GRE) */
#define FLWSTATS_QUERYMASK_INRXMACDST		0x00040000	/* Destination MAC mask(L2 GRE) */ 
    uint32_t        v4rxdstaddr;                    /* IPV4  RX dst addr */
#define FLWSTATS_QUERYMASK_V4RXDST		0x00020000	/* IPV4  RX dst mask */
	FlwStatsQueryIpVer_t srcipver;					/* src ip version */
#define FLWSTATS_QUERYMASK_SRCIPVER		0x00010000	/* src ip version mask */
	FlwStatsQueryIpVer_t dstipver;					/* dst ip version */
#define FLWSTATS_QUERYMASK_DSTIPVER		0x00008000	/* dst ip version mask */
    uint32_t        v4srcaddr;                      /* IPV4  src addr */
#define FLWSTATS_QUERYMASK_V4SRC		0x00004000	/* IPV4  src mask */
    uint32_t        v4dstaddr;                      /* IPV4  dst addr */
#define FLWSTATS_QUERYMASK_V4DST		0x00002000	/* IPV4  dst mask */
    uint32_t        v6srcaddr[4];                   /* IPV6  src addr */
#define FLWSTATS_QUERYMASK_V6SRC		0x00001000	/* IPV6  src mask */
    uint32_t        v6dstaddr[4];                   /* IPV6  dst addr */
#define FLWSTATS_QUERYMASK_V6DST		0x00000800	/* IPV6  dst mask */
    uint16_t        l4srcport;                      /* L4    src port */
#define FLWSTATS_QUERYMASK_L4SRCPRT		0x00000400	/* L4    src mask */
    uint16_t        l4dstport;                      /* L4    dst port */
#define FLWSTATS_QUERYMASK_L4DSTPRT		0x00000200	/* L4    dst mask */
    uint8_t         ipproto;                        /* ip    protocol */
#define FLWSTATS_QUERYMASK_IPPROTO		0x00000100	/* ip protocol mask */
/* For flows with one VLAN, outervid must be filled in the query */
    uint16_t        rxinnervid;                     /* RX Inner VLAN */
#define FLWSTATS_QUERYMASK_RXINVID		0x00000080	/* RX Inner VLAN mask */
    uint8_t         rxinnerpbits;                   /* RX Inner Pbits */
#define FLWSTATS_QUERYMASK_RXINPBITS	0x00000040	/* RX Inner Pbits mask */
    uint16_t        rxoutervid;                     /* RX Outer VLAN */
#define FLWSTATS_QUERYMASK_RXOUTVID		0x00000020	/* RX Outer VLAN mask */
    uint8_t         rxouterpbits;                   /* RX Outer Pbits */
#define FLWSTATS_QUERYMASK_RXOUTPBITS	0x00000010	/* RX Outer Pbits mask */
...
#define FLWSTATS_QUERYMASK_DSTPHY		0x00000001	/* Destination MAC mask */ 
    uint32_t        mask;							/* Mask */
    uint16_t        txchannel;						/* TX channel */
#define FLWSTATS_QUERYMASK_TXCHANNEL	0x00000001	/* TX channel mask */
...
#define FLWSTATS_FIELDNAME_TXCHANNEL	"txchannel"		/* TX Channel */
#define FLWSTATS_FIELDNAME_RXCHANNEL	"rxchannel"		/* RX Channel */
#define FLWSTATS_FIELDNAME_TXINVID		"txinvid"		/* TX Inner VLAN */
#define FLWSTATS_FIELDNAME_TXINPBITS	"txinpbits"		/* TX Inner Pbits */
#define FLWSTATS_FIELDNAME_TXOUTVID		"txoutvid"		/* TX Outer VLAN */
#define FLWSTATS_FIELDNAME_TXOUTPBITS	"txoutpbits"	/* TX Outer Pbits */
...
#define FLWSTATS_FIELDNAME_RXINVID		"rxinvid"		/* RX Inner VLAN */
#define FLWSTATS_FIELDNAME_RXINPBITS		"rxinpbits"		/* RX Inner Pbits */
#define FLWSTATS_FIELDNAME_RXOUTVID		"rxoutvid"	/* RX Outer VLAN */
#define FLWSTATS_FIELDNAME_RXOUTPBITS		"rxoutpbits"	/* RX Outer Pbits */
...
#define FLWSTATS_FIELDNAME_TXDSCP		"txdscp"		/* tx dscp */ 
#define FLWSTATS_FIELDNAME_RXDSCP		"rxdscp"		/* rx dscp */ 
#define FLWSTATS_FIELDNAME_TXTOS		"txtos"			/* tx tos */ 
#define FLWSTATS_FIELDNAME_RXTOS		"rxtos"			/* rx tos */ 
...
#define FILL_TUPLE_FIELD_2(P,FIELD,KEYTOKEN,MASK,VALUE) \
	if(!strcasecmp(pcField, KEYTOKEN))\
	{\
		P->create.queryTuple.FIELD = VALUE;  		\
		P->create.queryTuple.mask2 |= MASK;	\
	}

#define FILL_TUPLE_TOS_FIELD(P,KEYTOKEN,MASK,VALUE,TOSCOUNT) \
	if(!strcasecmp(pcField, KEYTOKEN))\
	{\
		P->create.queryTuple.tos = VALUE;  		\
		if (!P->create.queryTuple.tos && !strcasecmp(pcValue,"all")) \
			P->create.queryTuple.mask |= FLWSTATS_QUERYMASK_ALL;   \
		P->create.queryTuple.mask |= MASK;	\
		TOSCOUNT++; \
	}
...
#define DUMP_TUPLE_FIELD_INT_2(P,FIELD,KEYTOKEN,MASK) \
	if(P->mask2 & MASK)\
	{\
		printk(KEYTOKEN "=%d ", P->FIELD ); \
	}
...
#define DUMP_TUPLE_TOS_FIELD(P,KEYTOKEN,MASK) \
	if(P->mask & MASK)\
	{\
	    if (P->mask & FLWSTATS_QUERYMASK_ALL) \
	        printk(KEYTOKEN "=all "); \
        else \
            printk(KEYTOKEN "=%d ", P->tos ); \
	}
...
typedef struct {
    ...
    OUT uint32_t              size;     // size of array returned
    BCM_IOC_PTR(FlwStats_t *, priv);
...
typedef union {
    ...
    FlwStatsReserveQueryInfo_t reserve;     // nf-flwstats
}FlwStatsQueryInfo_t;
...
typedef struct {
    ...
    int32_t			prevFlwStIDs[PREV_FLW_STATS_DUMP_ENTRIES];
...
typedef struct {
    ...
    OUT  uint8_t               nodeFlag;
    OUT  FlwStats_t            flwSt;
}NfFlwStatsDumpEntryInfo_t;

typedef struct {
    uint32_t	num_entries;
    BCM_IOC_PTR(NfFlwStatsDumpEntryInfo_t*, FlwStDumpEntry);
}NfFlwStatsDumpInfo_t;
...
#define MAX_NF_FSMARK_QUERIES         128   // update include/uapi/linux/netfilter/xt_FSMARK.h also
...
#define FLWSTATS_ERR_NO_MEM             -5

#define FLWSTATS_ERR_QUERY_DUPLICATED	-6		
...
int nfFlwStatsReserveQuery(FlwStatsQueryInfo_t *Query);
...
int nfFlwStatsGetQuery(FlwStatsQueryInfo_t *Query);
...
int nfFlwStatsClearQuery(FlwStatsQueryInfo_t *Query);
...
int nfFlwStatsDeleteQuery(FlwStatsQueryInfo_t *Query);
