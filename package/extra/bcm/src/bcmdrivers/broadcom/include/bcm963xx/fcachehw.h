 ...
 *------------------------------------------------------------------------------
 */
extern void fhw_bind_fc(FhwBindHwHooks_t *hwHooks_p, int flag);         /* disable[flag=0] enable[flag=1] */
...
typedef struct {
    ...
    uint64_t  hw_bytes_curr;  /* byte count from HW during last fetch */
    uint64_t  hw_bytes_cumm;  /* cummulative byte count in HW */
    uint32_t  flow_type;
} HwEnt_t;
