...
typedef enum {
    ...
    CMDLIST_BRCM_TAG_TX,
    CMDLIST_BRCM_TAG_PT_SINGLE,   // Passthrough
    CMDLIST_BRCM_TAG_PT_DUAL_10B, // Passthrough
    CMDLIST_BRCM_TAG_PT_DUAL_8B,  // Passthrough
    CMDLIST_BRCM_TAG_MAX
} cmdlist_brcm_tag_t;

int cmdlist_bind(cmdlist_hooks_t *hooks_p);
void cmdlist_unbind(void);
void cmdlist_init(uint32_t *cmd_list_p, uint32_t cmd_list_length_max,
                  uint32_t cmd_list_start_offset);
int cmdlist_ucast_create(Blog_t *blog_p, cmdlist_cmd_target_t target,
                         BCM_HwAccelPrepend_t *prepend_p, void **buffer_pp,
                         cmdlist_brcm_tag_t brcm_tag, int *tx_adjust_p);
...
int cmdlist_get_length(int *data_length_p);
