...
#ifdef MODULE  /* compiling for kernel module */
#include <linux/time.h>
#else          /* compiling for userspace */
#include <time.h>
#endif
#include "bcm_gpon_api_common.h"
#include "pon_pwm_sm.h"
...
#define BCM_PLOAM_DEVICE_NAME_PATH         "/dev/" BCM_PLOAM_DEVICE_NAME
#define BCM_PLOAM_USR_DEVICE_NAME_PATH     "/dev/" BCM_PLOAM_USR_DEVICE_NAME
#define BCM_PLOAM_TOD_USR_DEVICE_NAME_PATH "/dev/" BCM_PLOAM_TOD_USR_DEVICE_NAME
...
/* IOCTL cmd values 1 and 2 are mapped to FIBMAP and FIGETBSZ on ARM
   ...
   BCM_PLOAM_IOC_INIT_PWM_STATS               ,
   BCM_PLOAM_IOC_GET_PWM_STATS                ,
   BCM_PLOAM_IOC_GET_STATS                    ,
   BCM_PLOAM_IOC_GET_STACK_MODE               ,
   BCM_PLOAM_IOC_GET_GEM_PORTS_LIST           ,
   BCM_PLOAM_IOC_GET_EQD                      ,
   BCM_PLOAM_IOC_SET_STATS_SIM_RATE           ,
   BCM_PLOAM_IOC_GET_STATS_SIM_RATE           ,
   BCM_PLOAM_IOC_SET_DG_GPIO                  ,  /*Return codes: EINVAL_PLOAM_ARG*/
   BCM_PLOAM_IOC_BW_MON_START_STOP            ,
   BCM_PLOAM_IOC_BW_MON_SET_TCONT             ,
   BCM_PLOAM_IOC_BW_MON_GET_STAT_RECS         ,
   BCM_PLOAM_IOC_BW_MON_SET_REPORTING         ,
   BCM_PLOAM_IOC_BW_MON_GET_EVENTS            ,
   BCM_PLOAM_IOC_BW_MON_LINK_START_STOP       ,
   BCM_PLOAM_IOC_BW_MON_LINK_GET_STATS        ,
   BCM_PLOAM_IOC_BW_MON_GET_PARAMS            ,
   BCM_PLOAM_IOC_GET_TCONT_2_GEM_MAP          ,
   BCM_PLOAM_IOC_GET_PWM_STATE                ,
   BCM_PLOAM_IOC_SET_PWM_LOCAL_INDICATOR
} BCM_Ploam_CommandId;
...
#define BCM_PLOAM_EVENT_ALLOC_ID_OCC_THR_CROSS (1<<9)
#define BCM_PLOAM_EVENT_ALLOC_ID_PACKETS_DROP  (1<<10)
#define BCM_PLOAM_EVENT_SA_ON                  (1<<11)
#define BCM_PLOAM_EVENT_SA_OFF                 (1<<12)
...
typedef struct {
  UINT64 alloc_id_queue_occupancy;
  UINT64 alloc_id_packets_drop;
} BCM_Ploam_AllocId_Events_Map;
...
typedef struct
{
    ...
    OUT UINT32 fsHecErr;          /* DS FS header HEC error count */
    OUT UINT32 fsHecFixedErr;     /* DS FS header fixed HEC error count */
    OUT UINT32 bwmapHecErr;       /* BWMAP HEC error count */
    OUT UINT32 bwmapHecFixedErr;  /* BWMAP fixed HEC error count */
    ...
    OUT UINT64 fecTotalCw;        /* FEC total codewords */
    OUT UINT32 fecUcCw;           /* FEC uncorrectable codewords */
    OUT UINT32 xgemFramesOnPonLink; /* total XGEM frames before filtering */

    OUT UINT64 dsFSbytes;         /* Total bytes received in DS */
    OUT UINT64 dsPSBdFrames;      /* Total PSBd frames counter XGPON/XGS/NGPON */
    OUT UINT64 dsGtcFrames;       /* Total (X)GTC layer frames */
    ...
    OUT UINT32 rxRangingInO5;     /* Rx ranging PLOAMs in O5 counter */
    OUT UINT32 rxRangingAdj;      /* Rx ranging adj PLOAMs counter */
    ...
    OUT UINT32 successAct;        /* successful activations counter
                                   ...
                                   */
    OUT UINT32 rogueOnuCnt;       /* rogue onu counter */
} BCM_Ploam_StatCounters;

/* Current equalization delay and equalization delay on entry to O5 */
typedef struct {
  OUT UINT32 equalization_delay;
  OUT UINT32 equalization_delay_o5_entry;
} BCM_Ploam_EqD;


typedef enum
{
    BCM_PLOAM_MODE_XGPON,            /* ITU-T G.987 XGPON */
    BCM_PLOAM_MODE_NGPON2_10G,       /* ITU-T G.989 NGPON2 10Gbps US */
    BCM_PLOAM_MODE_NGPON2_2_5G,      /* ITU-T G.989 NGPON2 10Gbps DS - 2.5Gbps US */
    BCM_PLOAM_MODE_XGS,              /* ITU-T G.tbd XGS 10Gbps US */
    BCM_PLOAM_MODE_GPON,             /* ITU-T G.984 GPON */
} BCM_Ploam_StackModeEnum;

typedef struct {
  OUT UINT32 stackMode;
} BCM_Ploam_StackMode;
...
typedef struct {
  OUT UINT8 gemPortNum;
  BCM_Ploam_DecfgGemPortInfo gemPort[BCM_PLOAM_NUM_DATA_GEM_PORTS+1];
} BCM_Ploam_GemPortsList;
...
typedef struct {
  ...
  OUT UINT16 tcontIDs[BCM_PLOAM_NUM_DATA_TCONTS+1];
  OUT UINT16 allocIDs[BCM_PLOAM_NUM_DATA_TCONTS+1];
  OUT UBOOL8 management[BCM_PLOAM_NUM_DATA_TCONTS+1];
...
/* BCM_PLOAM_IOC_SET_STATS_SIM_LEVEL, begin */
#define PLOAM_MAX_POWER_OF_10   9
typedef enum {
  BCM_MACSTAT_MIN,
  BCM_MACSTAT_BIPERR = BCM_MACSTAT_MIN,
  BCM_MACSTAT_PSBDHECERR,
  BCM_MACSTAT_XGTCHECERR,
  BCM_MACSTAT_XGEMHECERR,
  BCM_MACSTAT_FECCCCW,
  BCM_MACSTAT_FECUCCW,
  BCM_MACSTAT_RXPKTDISCARD,
  BCM_MACSTAT_TXPKTDISCARD,
  BCM_MACSTAT_MAX
} BCM_Ploam_MacStatErrType;

/* Type of ioctl argument pointer for command BCM_PLOAM_IOC_SET_STATS_SIM_LEVEL */
typedef struct {
  IN UINT32 reset;
  INOUT SINT32 counterType; /* BCM_Ploam_MacStatErrType. */
  INOUT UINT32 rate;        /* Coefficient m and power n in m ? 10^(-n). */
} BCM_Ploam_ErrStatsSimInfo;
/* BCM_PLOAM_IOC_SET_STATS_SIM_LEVEL, end */
...
#define BCM_PLOAM_PASSWORD_SIZE_BYTES 36
...
/*Type of ioctl argument pointer for command BCM_PLOAM_IOC_SET_DG_GPIO */
typedef struct {
  INOUT UINT32 gpio;
} BCM_Ploam_DG_GPIOInfo;
...
typedef struct
{
  OUT UINT64 doze_time;
  OUT UINT64 cs_time;
  OUT UINT64 ws_time;
} BCM_Ploam_PowerManagementStats;
...
/*
 * Upstream Bandwidth Monitor related definitions
 */
#define BCM_Ploam_BW_Mon_MAX_STAT_REC  100
...
typedef struct {
    ...
    UINT64             timestamp;
    UINT64             avg_us_rate;
    UINT64             avg_alloc_size;
    UINT16             num_of_allocs;
    UINT64             above_thr_or_dropped_pkt;
    UINT64             queue_occ;
    SINT64             queue_occ_delta;
    UINT64             queue_pass_pkt;
    UINT64             queue_pass_byte;
    UINT64             queue_drop_pkt;
    UINT64             queue_drop_byte;
}
BCM_Ploam_BW_Mon_Stat_Rec;

typedef struct {
  IN UINT8   tcont_idx;  
  IN UINT16  rec_num;     /* number of records to get, starting from current */
  IN UINT64  timestamp;
  OUT UINT16 read_rec_num;
  OUT BCM_Ploam_BW_Mon_Stat_Rec stat_records[BCM_Ploam_BW_Mon_MAX_STAT_REC];
}
BCM_Ploam_BW_Mon_Stat_Recs;

typedef struct {
  OUT UBOOL8 is_enabled;
  OUT UINT32 sampling_period; /* us */
  OUT UINT32 holdoff_time_in_ticks;
  OUT UINT32 holdoff_time_in_usec;
  OUT UINT32 num_rec_to_print;
} BCM_Ploam_BwMonParams;

typedef struct {
  IN UINT8              bw_mon_start_req;   /* 1 to start BW_MON, 0 - to stop it */
  IN UINT32             period_in_usec;
}
BCM_Ploam_BwMonStartStop;

typedef struct {
   IN UINT8  tcont_idx;
   IN UINT8  tcont_status;
}
BCM_Ploam_BwMonSetTcont;
...
typedef struct {
  IN BW_MON_REPORT_TYPE report;
  IN UINT32 holdoff_time_in_usec;
  IN UINT32 q_thr_in_bytes;
  IN UINT32 q_thr_in_percents;
  IN UINT32 num_rec; 
}
BCM_Ploam_BwMonSetReporting;
...
BCM_Ploam_BW_Mon_Link_StartStop;

typedef struct {
  OUT UINT64 total_bw_pure;  /* bps, pure data rate */
  OUT UINT64 total_bw_mixed; /* bps, data + overhead(e.g. FEC) rate */
  OUT UINT32 access_read_num;
  OUT UINT32 ds_frame_num;
}
BCM_Ploam_BW_Mon_Link_Stats;
...
BCM_Ploam_Tcont2GemMapRecord;

typedef struct {
  IN UINT8 tcont_idx;
  OUT UINT8 num_gems; 
  OUT BCM_Ploam_Tcont2GemMapRecord recs[BCM_PLOAM_NUM_DATA_GEM_PORTS];
}
BCM_Ploam_Tcont2GemMap;


typedef PON_PWM_STATE BCM_Ploam_Pwm_States;

typedef PON_PWM_LOCAL_INDICATORS BCM_Ploam_Pwm_Local_Indicators;
