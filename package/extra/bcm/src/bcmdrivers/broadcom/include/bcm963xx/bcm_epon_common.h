...
typedef enum
    ...
    EponOnuHgu                = 0x01,
    EponOnuSbu                = 0x02,
    EponOnuCassetteMdu        = 0x03,
    EponOnuMiniCardMdu        = 0x04,
    EponOnuCassetteCardMdu    = 0x05,
    EponOnuFrameCardMdu       = 0x06,
    EponOnuCardMdu            = 0x07,
    EponOnuMtu                = 0x08,
    EponOnuCmc                = 0x09,
    EponOnuNum                = 0x0A
...
typedef enum 
    ...
    BCMEPONCfgGetLinkMac,
    BCMEPONCfgGetLinkEncryptLatestPktNum,
    BCMEPONCfgKeyClear,
    BCMEPONCfgReInitLink,
    BCMEPONUniPortNumSync,
    BCMEPONUniPortNameSync,
    BCMEPONGetRogueCount,
    BCM_EPON_IOCTL_MAX
    } EponioctlCmd_e;
...
typedef struct {
	U32  regStart;
	U8  regCount;
	unsigned long  regval; //if more than one ,the regval should be a point.	
} PACK RegRWPara;
...
typedef struct tagLinkMac {
    U8 link;
    MacAddr mac;
}PACK LinkMac;

typedef struct tagEncryptPtkNum {
    U8 link;
    U32 pktnumber;
}PACK LinkEncryptLatestPktNum;

typedef struct {
    U8  portIdx;
    U8  portName[IFNAMSIZ];
} PACK UniPortInfo;
...
typedef union {
        ...
        U8 fatalErrRstEn;
        LinkMac linkMac;
        LinkEncryptLatestPktNum linkPktNum;
        U8 uniPortNum;
        UniPortInfo uniPortInfo;
        U32 rogueCount;
} PACK EponParm;
...
typedef enum
    ...
    StatIdEponIfRxGoodFrames,
    StatIdEponIfRxErroredFrames,
