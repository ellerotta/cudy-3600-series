...
usage() {
	...
	printf "\n\t-f ==> specify cudy device description for root node\n"
	exit 1
}

FDTNUM=1

while getopts ":A:a:c:C:D:d:e:k:n:o:v:s:S:r:R:f:" OPTION
do
	case $OPTION in
		A ) ARCH=$OPTARG;;
		a ) LOAD_ADDR=$OPTARG;;
		...
		f ) DESCRIPTION_ROOT_NODE=$OPTARG;;
...
/ {
	description = \"${DESCRIPTION_ROOT_NODE}\";
	#address-cells = <1>;
